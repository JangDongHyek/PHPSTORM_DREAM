...
class JlCsv extends Jl{
    ...
    public function __construct($path = "",$header = "",$field = "",$data = "") {
        //부모 생성자
        parent::__construct();

        if(!$header) throw new \Exception("JlCsv : 헤더가 없습니다.");
        if(!$field) throw new \Exception("JlCsv : 필드가 없습니다.");
        if(!$data) throw new \Exception("JlCsv : 데이터가 없습니다.");
