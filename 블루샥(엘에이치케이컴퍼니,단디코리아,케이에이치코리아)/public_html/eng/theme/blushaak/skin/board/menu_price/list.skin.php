...
<!--                     <a href="<?php echo $list[$i]['href'] ?>"> -->
                    ...
                    <?php 
                        ...
                        if($thumb['src']) {
                            $img_content = '<a class="fancybox" href="'.$thumb['src'].'" title="'.$list[$i]['subject'].'"width="'.$imgmaxwidth.'" height="'.$imgmaxheight.'">
...
<!--							바닐라 아이스크림 그리고 바닐라 블렌딩 밀크에 리스트레또 에스프레소를 더한 시간이 지날수록 깊은 풍미가 더해지는 커피스토어만의 시그니처-->
						...
						?>
						<p class="pro_price">
							<span><?php echo $list[$i]['wr_1'] ?></span>원
						</p>
						<?php }?>
						</a>
                    <?php
    ...
    <?php } ?>
        <div id="archi01" class="text-center">
            <p class="t_padding40" style="user-select: auto;">The above image is staged and may look different from the actual product.</p>
        </div>
    </form>
</div>
