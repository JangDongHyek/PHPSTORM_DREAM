...
if (isset($wr_id) && $wr_id) {
    ...
    if ($member['mb_level'] < $board['bo_list_level']) {
        if ($member['mb_id'])
			...
			if($bo_table=="b_order_eng"){
				alert('If you are a store owner, please log in first before using it.\\nIf you need faster approval, please contact the head office at 82-1644-8067.', './login.php?'.$qstr.'&url='.urlencode(G5_BBS_URL.'/board.php?bo_table='.$bo_table.($qstr?'&amp;':'')));
			}else{
				alert('If you are a store owner, please log in first before using it.\\nIf you need faster approval, please contact the head office at 82-1644-8067.', './login.php?'.$qstr.'&url='.urlencode(G5_BBS_URL.'/board.php?bo_table='.$bo_table.($qstr?'&amp;':'')));
			}
		}
