	...
	$(document).ready(function() {
		...
		$('[data-fancybox="images"]').fancybox({
			afterShow: function(instance, current) {
			  var prevButton = $('<button class="fancybox-arrow fancybox-arrow--left" title="Previous"></button>')
				.appendTo('.fancybox-inner')
				.on('click', function() {
				  $.fancybox.prev();
				});

			  var nextButton = $('<button class="fancybox-arrow fancybox-arrow--right" title="Next"></button>')
				.appendTo('.fancybox-inner')
				.on('click', function() {
				  $.fancybox.next();
				});
            ...
            <ul class="gall_con">
                <li class="gall_href">
                    ...
                    <?php 
                        ...
                        if($thumb['src']) {
							$sql="select * from g5_board_file where bo_table = '{$board['bo_table']}' and wr_id='{$list[$i][wr_id]}' and bf_no = '0'";
							$fileRow=sql_fetch($sql);
							if($fileRow[bf_file]){
								$thumb[src]=G5_DATA_URL."/file/".$bo_table."/".$fileRow[bf_file];
								
							}else{
								$thumb[src]=get_list_view_thumbnail($list[$i]['wr_content']);
							}
                            $img_content = '<a class="fancybox" href="'.$thumb['src'].'" title="'.$list[$i]['subject'].'""  data-fancybox="images">
					...
					<?php
					 ...
					 ?>
						 <script>
						 	srcs.push('<?php echo $thumb[src]?>');
						 </script>
					<?php }?>
                </li>
                <li class="gall_text_href" style="width:<?php echo $board['bo_gallery_width'] ?>px;display:<?php echo $member[mb_level]==10?"":"none"?>">
                    <?php
