...
<section id="bo_w">
    ...
    <div class="tbl_frm01 tbl_wrap">
        ...
        <tr>
        	...
        	<td>
        		<input type="text" class="frm_input" name="wr_1" id="wr_1" size="50" value="<?php echo $write['wr_1'] ?>" onkeyup="number_format(this.value,'wr_1')">원
        	</td>
        </tr>
    ...
    <script>
	function number_format(value,id) {
	  value = value.replace(/[^0-9]/g,'');   // 입력값이 숫자가 아니면 공백
	  value = value.replace(/,/g,'');          // ,값 공백처리
	  $(`#${id}`).val(value.replace(/\B(?=(\d{3})+(?!\d))/g, ",")); // 정규식을 이용해서 3자리 마다 , 추가 
	}
    <?php if($write_min || $write_max) { ?>
    // 글자수 제한
