...
if($bo_table=="b_order"&&!$is_admin){
	$orderWhere=" and mb_id='$member[mb_id]'";
}
if($_GET['wr_1']){
	$orderWhere.=" and wr_1='$wr_1'";
}
// 분류 선택 또는 검색어가 있다면
$stx = trim($stx);
if ($sca || $stx) {
    ...
    $sql_search .= " and (wr_num between {$spt} and ({$spt} + {$config['cf_search_part']})) {$orderWhere}";
