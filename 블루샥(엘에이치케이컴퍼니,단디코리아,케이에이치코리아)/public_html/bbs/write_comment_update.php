...
{
    $mb_id = '';
	$wr_password = "admin0808";
    $wr_password = get_encrypt_string($wr_password);
}

if($bo_table == "presentation"){
	$sql = "select * from `g5_write_presentation` where `wr_id` = '$wr_id'";
	$pre_row = sql_fetch($sql);
	$full_count = $pre_row['wr_6'];
	$now_count = $pre_row['wr_8']+1 + $wr_count;

	if($full_count < $now_count){
		alert("정원초과 되었습니다. 다음에 신청해주세요.");
	} else {
		$sql = "update `g5_write_presentation` set `wr_8` = '$now_count' where `wr_id` = '$wr_id'";
		sql_query($sql);
	}

	$wr_3 = $wr_3[0]."-".$wr_3[1]."-".$wr_3[2];
	$wr_9 = $wr_count;
