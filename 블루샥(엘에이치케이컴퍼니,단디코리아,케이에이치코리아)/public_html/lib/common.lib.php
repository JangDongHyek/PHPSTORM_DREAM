...
function get_category_option($bo_table='', $ca_name='')
{
    ...
    for ($i=0; $i<count($categories); $i++) {
		...
		if($bo_table=="b_qna"){
			$str.='<input type="radio" name="ca_name" id="ca_name'.$i.'" value="'.$categories[$i].'"';
			if ($category == $ca_name) {
				$str .= ' checked';
			}else if($i==0){
				//$str .= ' checked';
			}
			$str.="><label for='ca_name".$i."'>".$categories[$i]."</label>";
		}else{
...
function make_thumbnail($src_path, $width_dst, $height_dst, $thumbnail_path=""){
    ...
    if (!$thumbnail_path) {
        ...
        $thumbnail_path = $path_parts["dirname"].' b_'.$path_parts["basename"];
    }
