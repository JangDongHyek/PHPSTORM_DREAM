...
ini_set("session.cache_expire", 180); // 세션 캐쉬 보관시간 (분) // 180에서 변경 (24시간으로)
ini_set("session.gc_maxlifetime", 10800); // session data의 garbage collection 존재 기간을 지정 (초) // 10800에서 변경 (24시간으로)
...
if ($_REQUEST['app_mb_id'] != "") {
    // sql_query("insert into g5_flag set mb_id = '{$_REQUEST['app_mb_id']}', mode = 'is_app_mb_id', url = '".$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI']."', referer = '{$_SERVER['HTTP_REFERER']}', agent = '{$_SERVER['HTTP_USER_AGENT']}', wr_datetime = '".G5_TIME_YMDHIS."' ");

    $mb = get_member($_REQUEST['app_mb_id']);
    if ($mb) {
        // 회원아이디 세션 생성
        ...
        set_cookie("ck_auto", $key);

        // 내부확인용
        sql_query(" update g5_member set app_mb_id = '{$_REQUEST['app_mb_id']}', app_last_login = '".G5_TIME_YMDHIS."' where mb_id = '{$_REQUEST['app_mb_id']}' and use_yn = 'Y' ");
        // sql_query("insert into g5_flag set mb_id = '{$_REQUEST['app_mb_id']}', mode = 'is_app_login', url = '".$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI']."', referer = '{$_SERVER['HTTP_REFERER']}', agent = '{$_SERVER['HTTP_USER_AGENT']}', wr_datetime = '".G5_TIME_YMDHIS."' ");
    }
}
...
if ($_SESSION['ss_mb_id']) { // 로그인중이라면
    ...
    if(strpos($_SERVER['PHP_SELF'], '/ajax.') === false) { // 내부확인용 ajax페이지 제외
        // sql_query("insert into g5_flag set mb_id = '{$member['mb_id']}', mode = 'is_ss_login', url = '".$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI']."', referer = '{$_SERVER['HTTP_REFERER']}', agent = '{$_SERVER['HTTP_USER_AGENT']}', wr_datetime = '".G5_TIME_YMDHIS."' ");
    }

    // 차단된 회원이면 ss_mb_id 초기화
    if($member['mb_intercept_date'] && $member['mb_intercept_date'] <= date("Ymd", G5_SERVER_TIME)) {
        // sql_query("insert into g5_flag set mb_id = '{$member['mb_id']}', mode = 'is_ss_intercept', url = '".$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI']."', referer = '{$_SERVER['HTTP_REFERER']}', agent = '{$_SERVER['HTTP_USER_AGENT']}', wr_datetime = '".G5_TIME_YMDHIS."' ");
    ...
    if ($tmp_mb_id = get_cookie('ck_mb_id')) {
        if(strpos($_SERVER['PHP_SELF'], '/ajax.') === false) { // 내부확인용 ajax페이지 제외
            // sql_query("insert into g5_flag set mb_id = '{$tmp_mb_id}', mode = 'is_ck_mb_id', url = '" . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'] . "', referer = '{$_SERVER['HTTP_REFERER']}', agent = '{$_SERVER['HTTP_USER_AGENT']}', wr_datetime = '" . G5_TIME_YMDHIS . "' ");
        }

        //$tmp_mb_id = substr(preg_replace("/[^a-zA-Z0-9_]*/", "", $tmp_mb_id), 0, 20);
        // 최고관리자는 자동로그인 금지
        if (strtolower($tmp_mb_id) != strtolower($config['cf_admin'])) {
            $sql = " select mb_id, mb_password, mb_intercept_date, mb_leave_date, mb_email_certify from {$g5['member_table']} where mb_id = '{$tmp_mb_id}' ";
            $row = sql_fetch($sql);

            //$key = md5($_SERVER['SERVER_ADDR'] . $_SERVER['REMOTE_ADDR'] . $_SERVER['HTTP_USER_AGENT'] . $row['mb_password']);
			//$key = md5($_SERVER['SERVER_ADDR'] . $_SERVER['HTTP_USER_AGENT'] . $row['mb_password']);
            // $key = md5($_SERVER['SERVER_ADDR'] . $row['mb_id'] . $row['mb_password']);
            $key = md5($row['mb_id'] . $row['mb_password']);

            // 쿠키에 저장된 키와 같다면
            $tmp_key = get_cookie('ck_auto');
            if ($tmp_key == $key && $tmp_key) {
                // sql_query("insert into g5_flag set mb_id = '{$tmp_mb_id}', mode = 'is_ck_auto', url = '".$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI']."', referer = '{$_SERVER['HTTP_REFERER']}', agent = '{$_SERVER['HTTP_USER_AGENT']}', wr_datetime = '".G5_TIME_YMDHIS."' ");
                ...
                if ($row['mb_intercept_date'] == '' &&
                    ...
                    // 내부 확인용 데이터
                    $ck_id = get_cookie('ck_mb_id');
                    $ck_key = get_cookie('ck_auto');
                    sql_query(" update g5_member set ck_id = '{$ck_id}', ck_key = '{$ck_key}', last_login = '".G5_TIME_YMDHIS."' where mb_id = '{$tmp_mb_id}' and use_yn = 'Y' ");
                    // sql_query("insert into g5_flag set mb_id = '{$tmp_mb_id}', mode = 'is_ck_login', url = '".$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI']."', referer = '{$_SERVER['HTTP_REFERER']}', agent = '{$_SERVER['HTTP_USER_AGENT']}', wr_datetime = '".G5_TIME_YMDHIS."' ");
                    ...
                    // 내부확인용
                    // sql_query("insert into g5_flag set mb_id = '{$tmp_mb_id}', mode = 'is_ck_intercept', url = '".$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI']."', referer = '{$_SERVER['HTTP_REFERER']}', agent = '{$_SERVER['HTTP_USER_AGENT']}', wr_datetime = '".G5_TIME_YMDHIS."' ");
                }
            }
            // $row 배열변수 해제
            unset($row);
...
// IOS 버전에서 팝업창을 location.href로 변경하기 위한 플래그
$ios_flag = false;
if(strpos($_SERVER['HTTP_USER_AGENT'],'IOSJNGK') !== false) {
    $ios_flag = true;
}

// 안드로이드
$android_flag = false;
if(strpos($_SERVER['HTTP_USER_AGENT'], 'AJNGK') !== false) {
    $android_flag = true;
}

// 업체에 오픈하지않고 내부적으로 작업 시
$private = false;
if($ip == '183.103.22.103') { $private = true; }

//==============================================================================
// JNGK
//------------------------------------------------------------------------------
define('G5_CSS_VER', 0.2);
define('G5_JS_VER', 0.55);
...
// https 리다이렉션
if (0 < strpos(G5_URL, "ttp://www.jngk.kr") || 0 < strpos(G5_URL, "ttp://jngk.kr") || 0 < strpos(G5_URL, "ttps://jngk.kr")) {
    goto_url("https://www.jngk.kr");
}
