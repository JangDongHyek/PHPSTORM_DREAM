...
$mb_center      = isset($_POST['mb_center'])        ? trim($_POST['mb_center'])      : "";
...
if ($w == '') {
    ...
    $center_code = sql_fetch(" select center_code from g5_center where center_name = '{$mb_center}' ")['center_code']; // 센터코드
    ...
    $sql_password = "";
    if ($mb_password)
        $sql_password = " mb_password = '".get_encrypt_string($mb_password)."', pw_change = '".G5_TIME_YMDHIS."', pw_pre_data = '{$mb['mb_password']}' ";

    $sql = " update {$g5['member_table']} set {$sql_password} where mb_id = '{$mb_id}' ";
    sql_query($sql);
...
if ($w == '') {
    ...
    alert('비밀번호가 변경되었습니다.', G5_BBS_URL.'/mypage.php');
