...
/** 회원 - 레슨예약 - 예약 신청 또는 예약 수정 시 예약 가능 시간 데이터 조회 (ajax) **/
/** 예약 가능 시간 조회 시 속도가 문제가 될 경우 이 파일 사용 **/

// adm/lesson_time_set.php -- 현재 파일 수정 시 왼쪽 파일 같이 확인
...
//$sql .= " having tset.idx {$sql_add} in({$time_set_idx}) and date_format(now(), '%Y.%m.%d %H:%i:%s') < STR_TO_DATE(select_date, '%Y.%m.%d %H:%i:%s') "; // 현재 시간으로부터 지난 시간은 나오지 않음
$sql .= " where tset.idx {$sql_add} in({$time_set_idx}) and date_format(now(), '%Y.%m.%d %H:%i:%s') < STR_TO_DATE(concat('{$reser_date}', ' ', tset.set_time), '%Y.%m.%d %H:%i:%s') "; // 현재 시간으로부터 지난 시간은 나오지 않음
$sql .= " order by tset.set_time ";
//if($private) { echo $sql; }
$result = sql_query($sql);

$tmp_time = array(); // 전체 예약 시간 배열
...
// 예약수정 -- 예약되지 않은 시간 + 본인이 예약한 시간
    ...
    for($i=1; $row=sql_fetch_array($result); $i++) {
        ...
        if($row['set_time'] == $reser_time) { //  본인이 예약한 시간
        ?>
        <input type=checkbox name="ck_reser_time" id="reser_time<?=$i?>" value="<?=$row['reser_time']?>" checked onclick="reser_select(this.value, '<?=$row['idx']?>');"><label for="reser_time<?=$i?>"><div><?=$row['set_time']?></div></label>
        <?php
        }
    }
}
