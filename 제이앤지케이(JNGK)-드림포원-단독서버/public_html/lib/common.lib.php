...
// 한페이지에 보여줄 행, 현재페이지, 총페이지수, URL
function get_paging_mod($write_pages, $cur_page, $total_page, $url, $add="")
{
    //$url = preg_replace('#&amp;page=[0-9]*(&amp;page=)$#', '$1', $url);
    $url = preg_replace('#&amp;page=[0-9]*#', '', $url) . '&amp;page=';
    ...
    if ($start_page > 1) $str .= '<li><span><a href="'.$url.($start_page-1).$add.'"><i class="fal fa-angle-double-left"></i></a></span></li>'.PHP_EOL;

    if ($total_page > 1) {
        for ($k=$start_page;$k<=$end_page;$k++) {
            if ($cur_page != $k)
                $str .= '<li><span><a href="'.$url.$k.$add.'">'.$k.'<span class="sound_only">페이지</span></a></span></li>'.PHP_EOL;
            else
                $str .= '<li><span class="page_now">'.$k.'</span></li>'.PHP_EOL;
        }
    }

    if ($total_page > $end_page) $str .= '<li><span><a href="'.$url.($end_page+1).$add.'"><i class="fal fa-angle-double-right"></i></a></span></li>'.PHP_EOL;
...
function get_member($mb_id, $fields='*')
{
    ...
    return sql_fetch(" select $fields from {$g5['member_table']} where BINARY mb_id = TRIM('$mb_id') and use_yn = 'Y' "); // 대소문자 구분 필요시 BINARY로 TRIM 감싸줌 BINARY(TRIM('$mb_id'))
}
...
function get_member_no($mb_no, $fields='*')
{
    ...
    return sql_fetch(" select $fields from {$g5['member_table']} where BINARY mb_no = TRIM('$mb_no') and use_yn = 'Y' ");
}
...
function is_admin($mb_id)
{
    ...
    if ($config['cf_admin'] == $mb_id || $mb_id == "lets080" || $mb['mb_level'] > 7) return 'super';
...
// 21.02.04 푸시보내기
function sendFcm_old($tokens, $message)
{
    ...
    $fields = array(
        ...
        'data' => $message,
        'priority' => 'high',
        'notification' => array( 'title' => $message['subject'], 'body' => $message['message'],'url' => "https://www.jngk.kr/")
    );
    $headers = array(
...
{
    $url = 'https://dreamforone.kr/~fcmproject/fcm/request_fcm';
    $fields = array(
        'registration_ids' => $tokens,
        'token' => $tokens,
        //'content_available'=>true,
...
//// 프로 - 레슨예약 회원 선택 시 예약 가능한 레슨 회차가 남아있는지 확인
//function lessonReserCheck($mb_no) {
...
// 레슨금액
function lessonPrice($idx) {
    return sql_fetch(" select * from g5_lesson where idx = '{$idx}' ")['lesson_price'];
}
?>
