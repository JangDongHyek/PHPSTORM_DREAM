...
$sql_search = " where 1=1 and mb.use_yn = 'Y' and mb.center_code != 'center8' ";
...
if ($_REQUEST['option'] == '당일매출' || $_REQUEST['option'] == '주간매출') {
...
if ($_REQUEST['option'] == '당일매출') {
    ...
    $sql_select = " select MONTH(sa.mb_reg_date) AS month, YEAR(sa.mb_reg_date) AS year,
                    ...
                    sum(cash_price) + sum(credit_card_price) + sum(check_card_price) - sum(fees) as total_sales, sum(headoffice_pay) as headoffice_pay, pay_percent ";
    $sql = " {$sql_select} {$sql_common} {$sql_search} {$sql_group} {$sql_order} limit {$from_record}, {$rows} ";
    $sql2 = " {$sql_select} {$sql_common} {$sql_search} GROUP BY month, mb.mb_no {$sql_order} ";
    echo $sql2;
}
else if ($_REQUEST['option'] == '주간매출') {
...
<div id="state_div">
    <ul class="tabs" style="margin-bottom: 10px;">
        ...
        <?php
        $sql = " select * from g5_center where (close_date is null or close_date = '0000-00-00') and center_code != 'center8' "; // 폐쇄일 입력한 아카데미는 조회하지 않음
...
<div class="tbl_head02 tbl_wrap mb_tbl">
    <table id="static" style="margin-bottom: 15px;border-top: unset !important;">
        ...
        for ($i=0; $row=sql_fetch_array($result); $i++) {
            ...
            $link = G5_ADMIN_URL.'/sales_list_admin_test.php?option='.$option.'&start='.$start.'&end='.$end.'&center='.$center.'&pro='.$row['pro_mb_no'];
        ?>
...
function tabClick(value) {
    location.replace('<?=G5_ADMIN_URL?>/sales_list_admin_test.php?option='+value+'&center=<?=$center?>');
    // fsearch.submit();
}
...
$("#state_div .tabs li").on("click", function() {
    ...
    location.replace(g5_admin_url + "/sales_list_admin_test.php" + params)
});
