...
$sql_group = " GROUP BY `date`, mb.pro_mb_no ";
...
$sql = " select DATE_FORMAT(DATE_SUB(`pay_date`, INTERVAL (DAYOFWEEK(`pay_date`)-1) DAY), '%Y-%m-%d') as start,
                ...
                sum(`cash_price`) as cash_price, sum(`credit_card_price`) as credit_card_price, sum(`check_card_price`) as check_card_price, sum(`fees`) as fees, mb.mb_charge_pro, sum(`pro_extra_pay`) as pro_extra_pay
...
<div class="lre_ldate">
    ...
    <input type="hidden" id="sales_option" name="sales_option" value="주간매출">
...
<div class="tbl_head02 tbl_wrap mb_tbl">
    ...
    for ($i=0; $row=sql_fetch_array($result); $i++) {
        ...
        $sql_search2 = " WHERE 1 = 1 AND mb.center_code = '{$member['center_code']}' and DATE_FORMAT(pay_date, '%Y-%m-%d') = '{$search}' ";
        $sql_having2 = " having `date` is not null and cnt > 1 ";
        $sql2 = " select DATE_FORMAT(`pay_date`, '%Y%U') AS `date`, sum(`cash_price`) as cash_price, sum(`credit_card_price`) as credit_card_price, sum(`check_card_price`) as check_card_price, mb.mb_charge_pro, sum(1) as cnt
	...
	<tr class="<?php echo $bg; ?>">
        ...
        <td><?=number_format($total)?></td>
        <!--<td class="lv_op"><?php /*if(!empty($sum)) { echo number_format($sum).'_'.$row['date']; } else { echo number_format($total).'_'.$row['year'].$row['month'].$row['week_of_month']; } */?></td>-->
	</tr>
    <?php
    }
    if ($i == 0)
