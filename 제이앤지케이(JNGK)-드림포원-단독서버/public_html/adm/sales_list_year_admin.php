...
$sql_group = " group by mb.center_code, `year`, mb.pro_mb_no ";
...
$sql = " select YEAR(`pay_date`) AS `year`, MONTH(`pay_date`) AS `month`, sum(`cash_price`) as cash_price, sum(`credit_card_price`) as credit_card_price, sum(`check_card_price`) as check_card_price,
         sum(`fees`) as fees, mb.mb_center, mb.mb_charge_pro, mb.pro_mb_no, sum(`pro_extra_pay`) as pro_extra_pay
...
<div class="tbl_head02 tbl_wrap mb_tbl">
    ...
    for ($i=0; $row=sql_fetch_array($result); $i++) {
        ...
        /*$sql_group2 = " GROUP BY mb.center_code, `year`, mb.pro_mb_no with rollup ";
...
<?php echo get_paging(G5_IS_MOBILE ? $config['cf_mobile_pages'] : $config['cf_write_pages'], $page, $total_page, '?'.$qstr."&start_date=".$_REQUEST['start_date']."&end_date=".$_REQUEST['end_date']."&center=".$_REQUEST['center']."&pro=".$_REQUEST['pro'].'&amp;page='); ?>
...
$("#state_div .tabs li").on("click", function() {
    ...
    location.replace(g5_admin_url + "/sales_list_year_admin.php" + params)
});
</script>
