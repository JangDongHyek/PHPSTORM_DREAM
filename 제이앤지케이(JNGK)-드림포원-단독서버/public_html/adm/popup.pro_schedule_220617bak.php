...
<div id="adm_pro_lesson">
    ...
    ?>
    <div class="apl_name" style=" <?php if($row['mb_no'] == $mb_no) { echo $style; } ?>" onclick="location.href='<?=$s_mod?>'">
        ...
        <ul>
            ...
            for($i=0; $row=sql_fetch_array($today_result); $i++) {
                ...
                if(empty($diary_count)) {
                    ...
                    $remain .= $row['lesson_remain_count'].'회'; // 잔여회차
                }
...
<!--<form id="fsearch" name="fsearch" method="get">
    ...
    <div class="lre_ldate" style="margin: 20px;">
        <input type="date" name="start_date" value="<?php /*if(!empty($_GET['start_date'])) { echo $_GET['start_date']; } else { echo date('Y-m').'-'.'01'; } */?>" class="input_ldate"/> ~ <input type="date" name="end_date" value="<?php /*if(!empty($_GET['end_date'])) { echo $_GET['end_date']; } else { echo date('Y-m').'-'.$last_day; } */?>" class="input_ldate"/>
