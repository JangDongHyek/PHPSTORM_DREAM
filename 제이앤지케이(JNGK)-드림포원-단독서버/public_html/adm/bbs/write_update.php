...
if ($w == '' || $w == 'r') {
    ...
    $wr_name = sql_fetch(" select mb_name from g5_member where mb_id = '{$mb_id}'; ")['mb_name'];
    ...
    // 21.02.05 푸시
    if($member['mb_category'] == '팀장') { // 팀장 게시글 등록
        $sql = " select mb_id from g5_member where center_code = '{$member['center_code']}' and mb_category = '회원' ";
        $result = sql_query($sql);
        for($i=0; $row=sql_fetch_array($result); $i++) {
            $sql="select * from g5_fcm where mb_id = '{$row['mb_id']}'"; // 아카데미 팀장이 게시글 작성 시 아카데미 회원들에게 알림 (회원 id 필요)
            $fRow=sql_fetch($sql);
            $tokens[$i]=$fRow[token];
        }
        $message=array(
            "subject"=>"{$board['bo_subject']}",
            "message"=>"{$board['bo_subject']}에 새 글이 등록되었습니다",
    ...
    if($member['mb_id'] == 'admin') { // 관리자 게시글 등록
        $sql = " select fc.mb_id, fc.token from g5_fcm as fc left join g5_member as mb on fc.mb_id = mb.mb_id where mb.mb_category = '회원' ";
        $result = sql_query($sql);
        for($i=0; $fRow=sql_fetch_array($result); $i++) {
            $tokens[$i]=$fRow[token];
        }
        $message=array(
            "subject"=>"{$board['bo_subject']}",
    ...
    */
    ...
    $mb = get_member($write['mb_id']);
    $mb = get_member($write['mb_id']);
    if ($member['mb_level'] < $mb['mb_level'] || ($member['mb_level'] == $mb['mb_level'] && $member['mb_id'] != $write['mb_id'])) // 자신의 레벨이 크거나 같다면 통과 || 본인이 작성자일 경우 통과
        alert('수정 권한이 없습니다.', $return_url);
