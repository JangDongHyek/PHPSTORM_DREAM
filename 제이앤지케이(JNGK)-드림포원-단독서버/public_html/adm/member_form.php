...
else if ($w == 'u')
{
     ...
     if(!empty($mb['sales_idx'])) {
         $sql = " select * from g5_member as mb left join g5_sales as sa on mb.sales_idx = sa.idx where mb.mb_no = {$mb_no} ";
         $mb = sql_fetch($sql);
     }
    ...
    $one_point_le_date = str_replace('-','.',$mb['one_point_le_date']);
    $reser = sql_fetch(" select idx, time_set_idx from g5_lesson_reser where mb_no = {$mb_no} and center_code = '{$mb['center_code']}' and lesson_code = '{$mb['lesson_code']}' and reser_date = '{$one_point_le_date}' and reser_time = '{$mb['one_point_le_time']}' ");
}
else
...
<input type="hidden" id="time_set_idx" name="time_set_idx" value="<?=$reser['time_set_idx']?>">
<input type="hidden" id="reser_idx" name="reser_idx" value="<?=$reser['idx']?>"> <!-- 원포인트레슨정보idx -->
    ...
    <div class="tbl_frm01 tbl_wrap">
        <table>
            ...
            <tr>
                ...
                <td>
                <div class="checks">
                    <input type="radio" name="mb_state" value="new_member" id="state1" checked <?php if($mb['mb_state'] == 'new_member') echo 'checked="checked"'; ?>>
                    <label for="state1">신규회원</label>
                    <input type="radio" name="mb_state" value="re_member"  id="state2"<?php if($mb['mb_state'] == 're_member') echo 'checked="checked"'; ?>>
                    <label for="state2">재등록회원</label>
                    <input type="radio" name="mb_state" value="one_point_lesson" id="state3"<?php if($mb['mb_state'] == 'one_point_lesson') echo 'checked="checked"'; ?>>
                    <label for="state3">원포인트 레슨</label>
                </div>
                </td>
            </tr>
            </tbody>
    ...
    <div class="tbl_frm01 tbl_wrap">
            <table>
                ...
                <tr>
                    ...
                    <td>
                     <div class="checks">
                        <input type="radio" id="state1" name="mb_state" value="<?=$mb['mb_state']?>" checked>
                        <?php if($mb['mb_state'] == 'new_member') {$mb_state = '신규회원';} else if($mb['mb_state'] == 're_member') {$mb_state = '재등록회원';} else if($mb['mb_state'] == 'one_point_lesson') {$mb_state = '원포인트 레슨';} ?>
                        <label for="state1"><?=$mb_state?></label>
                     </div>
                    </td>
                </tr>
                </tbody>
...
<?php } ?>
    ...
    <div class="tbl_frm01 tbl_wrap member_state new_member">
        ...
        <tr>
            ...
            <td>
                <div class="checks">
                    <input type="radio" name="mb_option" value="지정" id="option1_1" checked <?php if($mb['mb_option'] == '지정') echo 'checked="checked"'; ?>>
                    <label for="option1_1">지정</label>
                    <input type="radio" name="mb_option" value="배정"  id="option1_2"<?php if($mb['mb_option'] == '배정') echo 'checked="checked"'; ?>>
                    <label for="option1_2">배정</label>
                </div>
            </td>
        </tr>
        <?php } ?>
        ...
        <tr>
            ...
            <td>
                ...
                <select class="frm_select" name="pro_mb_no" id="pro_mb_no">
                    ...
                    ?>
                    <option value="<?=$row['mb_no']?>"><?=$row['mb_name']?></option>
                    <?php } ?>
                </select>
                <?php } ?>
            </td>
        ...
        <tr>
            <th scope="row"><label for="lesson_idx">레슨정보<?php echo $sound_only ?></label></th>
            <td>
                <select class="frm_select frm_sel" name="lesson_idx" id="lesson_idx">
                    <option value="">레슨 선택</option>
                    <?php
                    $sql = " select le.* from g5_lesson as le left join g5_center ce on ce.center_code = le.center_code where ce.center_code = '{$member['center_code']}' and le.use_yn = 'Y' ";
                    $result = sql_query($sql);
        ...
        <tr>
            <th scope="row"><label for="lesson_start_date">레슨시작일<strong class="sound_only">필수</strong></label></th>
            <td>
                <input type="date" name="lesson_start_date" value="<?php echo empty($mb['lesson_start_date']) ? '' : $mb['lesson_start_date'] ?>" id="lesson_start_date" maxlength="100" class=" frm_input">
                <?php if($w == 'u' && ( $member['mb_level'] >= 8 )) { ?>
                        <!--
                <input type="date" name="lesson_end_date" value="<?php echo $mb['lesson_end_date'] == '1970-01-01' ? '' : $mb['lesson_end_date'] ?>" id="lesson_end_date" maxlength="100" class=" frm_input">
                    -->
                <?php } ?>
            </td>
        </tr>
        <?php if($w == 'u' && $member['mb_level'] >= 8) { ?>
        <tr>
            <th scope="row"><label for="lesson_end_date">레슨종료일<strong class="sound_only">필수</strong></label></th>
            <td>
                <input type="date" name="lesson_end_date" value="<?php echo $mb['lesson_end_date'] == '1970-01-01' ? '' : $mb['lesson_end_date'] ?>" id="lesson_end_date" maxlength="100" class=" frm_input">
                * 레슨시작일 변경 시 레슨종료일이 자동 계산됩니다.
            </td>
        </tr>
        <?php } ?>
        <tr>
            ...
            <td>
                ...
                <span class="cash"><input type="text" name="cash_price" value="<?php echo empty($mb['cash_price']) ? '' : number_format($mb['cash_price']) ?>" id="cash_price"  class="required frm_input" size="30" placeholder="금액(현금)" style="margin-bottom: 5px;text-align: right;" onkeyup="add_comma(this);" onblur="price_cal('new_member','cash', this.value);">원<br></span>
                ...
                <input type="text" name="fees" value="<?php echo empty($mb['fees']) ? '' : number_format($mb['fees']) ?>" id="fees" class="frm_input readonly" size="30" placeholder="수수료(자동계산)" style="margin-bottom: 5px;text-align: right;" onkeyup="add_comma(this);" readonly>원<br>
        ...
        <tr>
            ...
            <td class="row">
                <input type="text" name="mb_id" value="<?php echo $mb['mb_id'] ?>" id="mb_id" <?php echo $required_mb_id ?> class="frm_input <?php echo $required_mb_id_class ?>" size="30" minlength="3" maxlength="20"placeholder="아이디">
        ...
        <tr>
            ...
            <td><input type="password" name="mb_password" id="mb_password" <?php echo $required_mb_password ?> class="frm_input <?php echo $required_mb_password ?>" size="30" maxlength="20"placeholder="비밀번호"></td>
        </tr>
        <tr>
            <th scope="row"><label for="mb_info">개인정보<strong class="sound_only">필수</strong></label></th>
            <td>
                <input type="text" name="mb_name" value="<?php echo $mb['mb_name'] ?>" id="mb_name" required class="required frm_input name_ko" size="30" minlength="2" maxlength="20" placeholder="이름" style="margin-bottom: 5px;"><br>
        ...
        <tr>
            ...
            <td>
                ...
                <select name="birth_year" id="birth_year" class="frm_select"></select>
                <select name="birth_month" id="birth_month" class="frm_select"></select>
                <select name="birth_day" id="birth_day" class="frm_select"></select>
            </td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <div class="golf_box">
                <div class="golf_box_li">
                    ...
                    <div class="checks">
                    <input type="radio" id="score1" name="mb_score" value="81↓" <?php if($mb['mb_score'] == '81↓') echo 'checked="checked"'; ?>>
                ...
                <div class="golf_box_li">
                    ...
                    <div class="checks">
                    <input type="radio" id="career1" name="mb_career" value="초보" <?php if($mb['mb_career'] == '초보') echo 'checked="checked"'; ?>>
                ...
                <div class="golf_box_li">
                    ...
                    <div class="checks">
                    <input type="radio" id="lesson1" name="mb_lesson" value="있다" <?php if($mb['mb_lesson'] == '있다') echo 'checked="checked"'; ?>>
                ...
                <div class="golf_box_li">
                    ...
                    <div class="checks">
                    <input type="radio" id="rounding1" name="mb_rounding" value="2번 이상/주" <?php if($mb['mb_rounding'] == '2번 이상/주') echo 'checked="checked"'; ?>>
        ...
        <tr>
            <td colspan="2"><textarea name="mb_wish" id="mb_wish" class="frm_text" placeholder="회원님의 희망사항을 알려주세요!" rows="3"><?=$mb['mb_wish']?></textarea></td>
        </tr>
        <?php if($w == '') { ?>
        <tr>
        ...
        <tr>
            <td colspan="2"><textarea placeholder="메모할 내용을 입력하세요." name="mb_notice" class="frm_text" ><?=$mb['mb_notice']?></textarea></td>
        </tr>
        </tbody>
        </table>
    </div>

    <div class="tbl_frm01 tbl_wrap member_state re_member" style="display: none;">
        <table>
            <caption><?php echo $g5['title']; ?></caption>
            <colgroup>
                <col class="grid_5">
            ...
            <tbody>
            <tr>
                <th scope="row" colspan="2" class="th_pln">
                <label for="mb_state">회원검색 <span>찾으실 회원이름을 검색하세요</span><?php echo $sound_only ?></label>
                </th>
            </tr>
            <tr>
                <td colspan="2">
                    <input type="text" class="frm_input" id="search_member" name="search_member" size="30" onkeyup="enter_key();" autocomplete="off">
                    <input type="button" class="btn2" value="검색" onclick="searchMember();">
                    <!--<div id="search_member_result" class="search_m_result"></div>-->
    ...
    <div class="tbl_frm01 tbl_wrap member_state one_point_lesson" style="display: none;">
        <table>
            <caption><?php echo $g5['title']; ?></caption>
            <colgroup>
                <col class="grid_5">
            ...
            <tbody>
            <tr>
                <th scope="row"><label for="mb_option2">구분<?php echo $sound_only ?></label></th>
                <td>
                    <div class="checks">
                        <input type="radio" name="mb_option2" value="지정" id="option2_1" checked <?php if($mb['mb_option'] == '지정') echo 'checked="checked"'; ?>>
                        <label for="option2_1">지정</label>
                        <input type="radio" name="mb_option2" value="배정"  id="option2_2" <?php if($mb['mb_option'] == '배정') echo 'checked="checked"'; ?>>
                        <label for="option2_2">배정</label>
                    </div>
                </td>
            </tr>
            <tr>
                <th scope="row"><label for="mb_center2">센터/아카데미<?php echo $sound_only ?></label></th>
                <td>
                    <input type="text" name="mb_center2" value="<?php echo $member['mb_center'] ?>" id="mb_center2" class=" frm_input readonly" size="30" readonly><br>
                </td>
            </tr>
            <tr>
                <th scope="row"><label for="pro_mb_no2">담당프로<?php echo $sound_only ?></label></th>
                <td>
                <?php if($member['mb_level'] == 8) { ?> <!-- 프로 로그인 시 담당 프로 고정 -->
                    <input type="text" name="mb_charge_pro_name2" value="<?php echo $member['mb_name'] ?>" id="mb_charge_pro_name2" class=" frm_input readonly" size="30" readonly>
                    <input type="hidden" name="pro_mb_no2" value="<?php echo $member['mb_no'] ?>" id="pro_mb_no2">
                    <br>
                <?php } ?>
                <?php if($member['mb_level'] == 9) { ?>
                    <select class="frm_select" name="pro_mb_no2" id="pro_mb_no2" onchange="change_date('<?=date('Y-m-d')?>', 'pro');">
                        <?php if($w == '') { ?>
                        <option value="">이름 선택</option>
                        <?php }
            ...
            <tr>
                <th scope="row"><label for="mb_info">회원 레슨 정보<strong class="sound_only">필수</strong></label></th>
                <td>
                	<div class="cf">
                        <div style="margin:0 5px 5px 0;">
                        <input type="text" name="mb_name2" value="<?=$mb['mb_name'] ?>" id="mb_name2" required class="required frm_input name_ko" size="30" minlength="2" maxlength="20" placeholder="이름">
                        </div>
                        <div style="margin-bottom:5px;">
                        <input type="text" name="mb_hp4" value="<?php echo explode('-', $mb['mb_hp'])[0] ?>" id="mb_hp4" required class="required frm_input" size="5" maxlength="3" placeholder="ex)010" onkeyup="number_check(this)"> -
                        <input type="text" name="mb_hp5" value="<?php echo explode('-', $mb['mb_hp'])[1] ?>" id="mb_hp5" required class="required frm_input" size="5" maxlength="4" placeholder="ex)1234" onkeyup="number_check(this)"> -
                        <input type="text" name="mb_hp6" value="<?php echo explode('-', $mb['mb_hp'])[2] ?>" id="mb_hp6" required class="required frm_input" size="5" maxlength="4" placeholder="ex)1234" onkeyup="number_check(this)">
                        </div>
                    </div>
                    <!--<input type="text" name="mb_goods_name" value="<?/*=$mb['mb_goods_name']*/?>" id="mb_goods_name"  class=" frm_input" size="80" placeholder="상품명" style="margin-bottom: 5px;"><br>-->
                    <select class="frm_select frm_sel" name="lesson_idx2" id="lesson_idx2" style="margin-bottom: 5px;">
                        <option value="">레슨 선택</option>
                        <?php
                        ...
                        for($i=0; $row=sql_fetch_array($result); $i++) {
                            $selected = '';
                            if(strpos($row['lesson_name'], '원포인트') !== false) { $selected = 'selected'; }
                        ?>
                        <option <?=$selected?> value="<?=$row['idx']?>"><?=$row['lesson_name']?> / <?=$row['lesson_time']?> / <?=$row['lesson_count']?> / <?=number_format($row['lesson_price'])?></option>
                        <?php
                        }
                        ?>
                    </select>
                    <div class="checks">
                        <input type="radio" id="pay_cash2" name="pay_option2" value="현금" checked <?php if($mb['pay_option'] == '현금') echo 'checked="checked"'; ?>>
                        <label for="pay_cash2">현금</label>
                        <input type="radio" id="pay_card2" name="pay_option2" value="신용카드" <?php if($mb['pay_option'] == '신용카드') echo 'checked="checked"'; ?>>
                        <label for="pay_card2">신용카드</label>
                        <input type="radio" id="pay_check_card2" name="pay_option2" value="체크카드" <?php if($mb['pay_option'] == '체크카드') echo 'checked="checked"'; ?>>
                        <label for="pay_check_card2">체크카드</label>
                        <input type="radio" id="pay_cash_credit_card2" name="pay_option2" value="현금+신용카드" <?php if($mb['pay_option'] == '현금+신용카드') echo 'checked="checked"'; ?>>
                        <label for="pay_cash_credit_card2">현금+신용카드</label>
                        <input type="radio" id="pay_cash_check_card2" name="pay_option2" value="현금+체크카드" <?php if($mb['pay_option'] == '현금+체크카드') echo 'checked="checked"'; ?>>
                        <label for="pay_cash_check_card2">현금+체크카드</label>
                    </div><!--.checks-->
                    <span class="cash2"><input type="text" name="cash_price2" value="<?php echo empty($mb['cash_price']) ? '' : number_format($mb['cash_price']) ?>" id="cash_price2"  class="required frm_input" size="30" placeholder="금액(현금)" style="margin-bottom: 5px;text-align: right;" onkeyup="add_comma(this);" onblur="price_cal('one_point_lesson','cash', this.value);">원<br></span>
                    <span class="card2" style="display: none;"><input type="text" name="card_price2" value="<?php echo empty($mb['credit_card_price']) ? number_format($mb['check_card_price']) : number_format($mb['credit_card_price']) ?>" id="card_price2"  class="required frm_input" size="30" placeholder="금액(카드)" style="margin-bottom: 5px;text-align: right;" onkeyup="add_comma(this);" onblur="price_cal('one_point_lesson','card', this.value);">원<br></span>
                    <input type="text" name="fees2" value="<?php echo empty($mb['fees']) ? '' : number_format($mb['fees']) ?>" id="fees2" class="frm_input readonly" size="30" placeholder="수수료(자동계산)" style="margin-bottom: 5px;text-align: right;" onkeyup="add_comma(this);" readonly>원<br>
                    <span class="company2" style="display: none;"><input type="text" name="card_company2" value="<?php echo $mb['card_company'] ?>" id="card_company2" required class="required frm_input" size="30" placeholder="카드사" style="margin-bottom: 5px;"><br></span>
                    <input type="date" name="one_point_le_date" value="<?php echo empty($mb['one_point_le_date']) ? date('Y-m-d') : $mb['one_point_le_date'] ?>" id="one_point_le_date"  class=" frm_input" onchange="change_date(this.value, 'pro');"><!-- 일자 -->
                    <input type="time" name="one_point_le_time" value="<?php echo empty($mb['one_point_le_time']) ? date('H:i') : $mb['one_point_le_time'] ?>" id="one_point_le_time"  class=" frm_input readonly" readonly><!-- 시간 -->
                    <div class="ptime_sel">
                    	<div class="check_bx2"></div>
                    </div><!--.ptime_sel-->
                </td>
            </tr>
            <?php if($w == '') { ?>
            ...
            <tr>
                <td colspan="2">
                    <textarea class="frm_text" style="margin-bottom: 5px;" readonly><?=$config['cf_stipulation']?></textarea><br>
                    <input type="checkbox" name="agree_chk2" id="agree2" value="Y"> <label for="agree2" style="margin-bottom: unset;">이용약관 및 회원 준수사항에 동의합니다.</label>
                </td>
            </tr>
            <?php } ?>
            <tr>
                <th scope="row" class="th_pln" colspan="2"><label for="mb_notice2">Notice<strong class="sound_only">필수</strong></label></th>
            </tr>
            <tr>
                <td colspan="2"><textarea name="mb_notice2" id="mb_notice2" class="frm_text" placeholder="메모할 내용을 입력하세요."><?=$mb['mb_notice']?></textarea></td>
            </tr>
            </tbody>
        </table>
    </div>
...
$(function() {
    ...
    var tab = '<?=$mb['mb_state']?>';
    if('<?=$w?>' == 'u') {
        if(tab == 're_member') { tab = 'new_member'; }
        $(".member_state").hide();
        $("."+tab).show();
        ...
        // 담당 프로, 레슨명
        if(tab == 'new_member') {
            $('#pro_mb_no').val('<?=$mb['pro_mb_no']?>').attr('selected', 'selected');
            $('#lesson_idx').val('<?=$mb['lesson_idx']?>').attr('selected', 'selected');
        } else {
            $('#pro_mb_no2').val('<?=$mb['pro_mb_no']?>').attr('selected', 'selected');
            $('#lesson_idx2').val('<?=$mb['lesson_idx']?>').attr('selected', 'selected');
        }
    }
    else {
        // 재등록회원은 회원검색 > 회원선택 후 진행
        if(tab == 're_member') {
            $('#btn').hide();
        } else {
            $('#btn').show()
    ...
    // 회원 상품 구분에 따른 화면 변경 -- 등록
    $("input:radio[name='mb_state']").change(function () {
        var tab = $(this).val();
        ...
        if(tab == 'one_point_lesson') {
            // 예약가능시간 조회
            if('<?=$member['mb_level']?>' == '8' || '<?=$w?>' == 'u') {
                change_date('<?=$mb['one_point_le_date']?>');
            }
        }
    });
    ...
    // 결제 방식 선택
    $("input:radio[name='pay_option2']").change(function() {
        var option = $(this).val();
    ...
    if(tab == 'one_point_lesson') {
        var option = $('input[name="pay_option2"]:checked').val();
        pay_option2(option);
    } else {
        var option = $('input[name="pay_option"]:checked').val()
        pay_option(option);
    }
});

function pay_option(option) {
    if(option == '현금') {
        $('.cash').show();
        $('.card').hide();
        $('.company').hide();
...
function form_ajax() {
    ...
    if(state == 'one_point_lesson') {
        ...
        if($('#mb_hp4').val() == '' || $('#mb_hp5').val() == '' || $('#mb_hp6').val() == '') {
            alert('휴대폰번호를 입력하세요.');
            is_post = false;
            return false;
        }
        ...
        // 레슨 시간 선택 필수
        if($('#one_point_le_time').val() == '' || $('#time_set_idx').val() == '') {
            alert('레슨 시간을 선택하세요.');
            is_post = false;
            return false;
        }

        // 21.01.25 원포인트 금액 필수값 제외
        /*var pay = $('input:radio[name=pay_option2]:checked').val();
        if(pay == '현금') {
            if($('#cash_price2').val() == '') {
                alert('금액을 입력하세요.');
                return false;
        ...
        // 이용약관 동의 확인
        if('<?=$w?>' == '') {
            var agree = $('input:checkbox[name=agree_chk2]:checked').val();
            if (agree != 'Y') {
                alert('이용약관 및 회원 준수사항에 동의해주세요.');
                is_post = false;
                return false;
        ...
        success: function (data) {
            if(data.length > 0) {
                ...
                for(var i=0; i<data.length; i++) {
                    var url = g5_admin_url+'/member_form_re.php?w=u&mb_no='+data[i]['mb_no'];

                    html += '<a href="'+url+'" style="cursor: pointer;text-decoration: none;">';
                    html += '<div class="search_mr_box">';
                    html += '<span class="smb" style="margin-right: 10px;">'+data[i]['mb_id_no']+'</span>';
...
// 날짜 선택 시 레슨 가능한 시간 조회
function change_date(date, op) {
    if(op == 'pro') { // 프로 또는 날짜 변경 시 레슨 시간 재선택
        if(date != $("#one_point_le_date").val()) {
            date = $("#one_point_le_date").val();
        }
        $('#one_point_le_time').val(''); // 초기화

        // 22.01.07 오늘보다 이전 날짜는 선택 불가
        if(date < '<?=date('Y-m-d')?>') {
            alert('날짜를 다시 선택해 주세요.');
            $('#one_point_le_date').val('');
            $('#one_point_le_time').val(''); // 초기화
            return false;
        }
    }
    ...
    if($('#pro_mb_no2').val() == '') {
        alert('담당프로를 선택하세요.');
        $('#one_point_le_date').val(getToday());
        return false;
    }
    ...
    $.ajax({
        ...
        data: {reser_date : date, pro_mb_no : $('#pro_mb_no2').val(), reser_time : $('#one_point_le_time').val(), mode : '<?=$w?>'},
        type: 'POST',
        success : function(data) {
            if(data){
                $('.check_bx2').html(data); // 예약시간설정(예약가능시간)
            }
            else{
                if('<?=$w?>' == 'u') {
                    $('.check_bx2').html('');
                } else {
                    $('.check_bx2').html('<p>예약 가능 시간이 없습니다.</p>');
                }
            }
        },
    });
...
function price_cal(state, option, price) {
    ...
    var pay_option = ''; // 결제방법
    if(state == 'new_member') { pay_option = $('input[name="pay_option"]:checked').val(); } // 결제방법
    else if(state == 'one_point_lesson') { pay_option = $('input[name="pay_option2"]:checked').val(); } // 결제방법
    ...
    if(option == 'card') {
        ...
        if(state == 'new_member') { $('#fees').val(fees); }
        else if(state == 'one_point_lesson') { $('#fees2').val(fees); }
    }
}
