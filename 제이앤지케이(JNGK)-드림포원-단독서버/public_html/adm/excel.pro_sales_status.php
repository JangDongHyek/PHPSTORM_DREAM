...
if($_REQUEST['option'] == '당일매출') {
    $sql = " select DATE(sa.mb_reg_date) as date, 
             ...
             card_company, mb.mb_charge_pro, sum(pro_extra_pay) as pro_extra_pay, sa.pro_mb_no, mb.mb_name, sa.mb_reg_date, count(idx) as count, sum(headoffice_pay) as headoffice_pay, pay_percent
    ...
    $sql = " select MONTH(sa.mb_reg_date) AS month, YEAR(sa.mb_reg_date) AS 'year', DATE(sa.mb_reg_date) as date,
             ...
             sum(fees) as fees, mb.mb_charge_pro, sum(pro_extra_pay) as pro_extra_pay, sa.pro_mb_no, mb.mb_name, sa.mb_reg_date, count(idx) as count, sum(headoffice_pay) as headoffice_pay, pay_percent
             {$sql_common} {$sql_search} {$sql_group} {$sql_order} ";
}
else if($_REQUEST['option'] == '주간매출') {
    ...
    $sql = " select DATE_FORMAT(DATE_SUB(sa.mb_reg_date, INTERVAL (DAYOFWEEK(sa.mb_reg_date)-1) DAY), '%Y-%m-%d') as start,
             ...
             sum(cash_price) as cash_price, sum(credit_card_price) as credit_card_price, sum(check_card_price) as check_card_price, sum(fees) as fees, 
             mb.mb_charge_pro, sum(pro_extra_pay) as pro_extra_pay, sa.pro_mb_no, mb.mb_name, sa.mb_reg_date, count(idx) as count, sum(headoffice_pay) as headoffice_pay, pay_percent
             {$sql_common} {$sql_search} {$sql_group} {$sql_order} ";
}
else { // 연간매출
    $sql = " select YEAR(sa.mb_reg_date) AS year, MONTH(sa.mb_reg_date) AS month, DATE(sa.mb_reg_date) as date, sum(cash_price) as cash_price, sum(credit_card_price) as credit_card_price, sum(check_card_price) as check_card_price, 
             sum(fees) as fees, mb.mb_charge_pro, sa.pro_mb_no, sum(pro_extra_pay) as pro_extra_pay, mb.mb_name, sa.mb_reg_date, count(idx) as count, sum(headoffice_pay) as headoffice_pay, pay_percent
             {$sql_common} {$sql_search} {$sql_group} {$sql_order} ";
}
//echo $sql;exit;
...
if ($_REQUEST['option'] == '당일매출') {
    ...
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue("K2", "매출");
}
...
for($i=3; $row=sql_fetch_array($result); $i++)
{
    ...
    if ($_REQUEST['option'] == '당일매출') {
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue("A".$i, $row['date']);
    } else if ($_REQUEST['option'] == '주간매출') {
...
$filename = iconv("UTF-8", "EUC-KR", $_REQUEST['option'].'('.$start.'~'.$end.')'."_".$_REQUEST['pro_name']."프로");
