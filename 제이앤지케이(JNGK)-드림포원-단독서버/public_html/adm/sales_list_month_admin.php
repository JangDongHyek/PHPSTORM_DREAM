...
$sql_group = " group by mb.center_code, `month`, mb.pro_mb_no ";
...
$sql = " select MONTH(`pay_date`) AS 'month', YEAR(`pay_date`) AS 'year',
         ...
         sum(`fees`) as fees, mb.mb_center, mb.mb_charge_pro, sum(`pro_extra_pay`) as pro_extra_pay, mb.pro_mb_no
...
<div class="local_ov01 local_ov">
    <div class="lre_list_tit">
        ...
        <span class="les_sdate">
            기간 :
            <?php
            if(!empty($_GET['start']) && !empty($_GET['end'])) { echo $_GET['start'].' ~ '.$_GET['end']; }
            else if(!empty($_GET['start'])) { echo $_GET['start'].' ~ '; }
            else if(!empty($_GET['end'])) { echo ' ~ '.$_GET['end']; }
...
<div class="tbl_head02 tbl_wrap mb_tbl">
    ...
    for ($i=0; $row=sql_fetch_array($result); $i++) {
        ...
        /*$search = $row['date'];
        $sql_search2 = " WHERE 1 = 1 AND mb.center_code = '{$center}' and DATE_FORMAT(pay_date, '%Y-%m-%d') = '{$search}' ";
        ...
        $link = G5_ADMIN_URL.'/sales_list_month_admin.php?start='.$start.'&end='.$end.'&center='.$center.'&pro='.$row['pro_mb_no'];
    ?>
	<tr class="<?php echo $bg; ?>">
        <td><?=$row['mb_center']?></td>
        <td><?=$row['month']?>월</td>
        <td><a href="<?=$link?>"><?=$row['mb_charge_pro']?></a></td>
...
<?php echo get_paging(G5_IS_MOBILE ? $config['cf_mobile_pages'] : $config['cf_write_pages'], $page, $total_page, '?'.$qstr."&start=".$_REQUEST['start']."&end=".$_REQUEST['end']."center=".$_REQUEST['center']."&pro=".$_REQUEST['pro'].'&amp;page='); ?>
...
function tabClick(value) {
    if(value == '당일매출') {
        ...
        location.replace('<?=G5_ADMIN_URL?>/sales_list_week_admin.php?center=<?=$center?>');
    }
    // fsearch.submit();
}
...
$("#state_div .tabs li").on("click", function() {
    ...
    location.replace(g5_admin_url + "/sales_list_month_admin.php" + params)
});
</script>
