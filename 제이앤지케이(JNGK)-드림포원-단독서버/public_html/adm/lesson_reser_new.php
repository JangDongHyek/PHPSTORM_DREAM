...
//          left join g5_member as mb on mb.mb_no = re.mb_no
//          where re.pro_mb_no = '{$member['mb_no']}' {$sql_search}
//          order by re.reser_date, re.reser_time limit {$from_record}, {$rows} ";
...
<link rel="stylesheet" href="<?=G5_PLUGIN_URL?>/powerful-calendar/theme.css">
<link rel="stylesheet" href="<?=G5_PLUGIN_URL?>/powerful-calendar/style.css">
...
<div id="lesson_reslist">
    ...
    <form id="fsearch" name="fsearch" method="get">
        <div class="lre_ldate">
            <input type="date" id="start_date" name="start_date" value="<?php echo empty($_GET['start_date']) ? date('Y-m-d') : $_GET['start_date'] ?>" class="input_ldate"/> ~ <input type="date" id="end_date" name="end_date" value="<?php echo empty($_GET['end_date']) ? date('Y-m-d') : $_GET['end_date'] ?>" class="input_ldate"/>
            <input type="button" class="btn_ldate" value="검색" onclick="getLessonReserList()">
        </div><!--.lre_ldate-->
    </form>
...
<script src="<?=G5_PLUGIN_URL?>/powerful-calendar/calendar.js"></script>
<script>
    var pro_mb_no = '<?=$mb_no?>';
    ...
    $(function() {
        ...
        var date_format = cur_date.getFullYear() + '.' + addZero(cur_date.getMonth()+1) + '.' + addZero(cur_date.getDate()); // xxxx.xx.xx

        // 달력
        $('.lesson_calendar').calendar({
            prevButton:"이전",
            nextButton:"다음",
            ...
            onClickDate:function (date) {
                getLessonTimeSet(date);
            }
        });

        getLessonTimeSet(cur_date);
    });

    // 예약시간 표시
    function getLessonTimeSet(date) {
        var chk_date = new Date(date);
        var obj = {};
        ...
        obj.year = chk_date.getFullYear();
        obj.month = addZero(chk_date.getMonth() + 1);
        obj.day = addZero(chk_date.getDate());
        var reser_date = obj.year + '.' + obj.month + '.' + obj.day;
        ...
        $.ajax({
            ...
            success : function(data) {
                var html_data = '<p>예약 가능 시간이 없습니다.</p>';
                if(data) html_data = data;

                // 예약시간
                $('.check_bx').html(html_data);

                // 레슨예약자 명단
                getLessonReserList('<?=date('Y.m.d')?>');
            },
    ...
    // 레슨예약자 명단
    function getLessonReserList(reser_date, reser_date2) {
        if(reser_date == undefined) {
            reser_date = $("#start_date").val().replace(/-/gi, '.');
            reser_date2 = $("#end_date").val().replace(/-/gi, '.');
        }
        $.ajax({
            url : g5_admin_url + "/ajax.lesson_reser_list_new.php",
            data : {reser_date : reser_date, reser_date2: reser_date2},
            type : 'POST',
            dataType: 'html',
            async: false,
            success : function(data) {
