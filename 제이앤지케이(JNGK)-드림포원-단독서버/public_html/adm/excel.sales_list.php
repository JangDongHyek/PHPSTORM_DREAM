...
// ===== 데이터 =====
$sql_common = " from g5_sales as sa left join g5_member as mb on mb.mb_no = sa.mb_no and mb.use_yn = 'Y' ";
...
for($i=1; $i<4; $i++) {
    if($i==1) { // 주간
        $objPHPExcel->setActiveSheetIndex($i)->setCellValue("A2", "주차");
    } else if($i==2) { // 월간
        $objPHPExcel->setActiveSheetIndex($i)->setCellValue("A2", "월");
    } else { // 연간
        $objPHPExcel->setActiveSheetIndex($i)->setCellValue("A2", "연도");
    }

    if($member['mb_level'] == 8) {
        $objPHPExcel->setActiveSheetIndex($i)->mergeCells('A1:H1');
        $objPHPExcel->setActiveSheetIndex($i)->mergeCells('I1:K1');

        $objPHPExcel->setActiveSheetIndex($i)->setCellValue("B2", "회원명");
        $objPHPExcel->setActiveSheetIndex($i)->setCellValue("C2", "등록일자");
        $objPHPExcel->setActiveSheetIndex($i)->setCellValue("D2", "건수");
        $objPHPExcel->setActiveSheetIndex($i)->setCellValue("E2", "현금");
        $objPHPExcel->setActiveSheetIndex($i)->setCellValue("F2", "신용카드");
        $objPHPExcel->setActiveSheetIndex($i)->setCellValue("G2", "체크카드");
        $objPHPExcel->setActiveSheetIndex($i)->setCellValue("H2", "현금+카드");
        $objPHPExcel->setActiveSheetIndex($i)->setCellValue("I2", "카드수수료");
        $objPHPExcel->setActiveSheetIndex($i)->setCellValue("J2", "프로수수료");
        $objPHPExcel->setActiveSheetIndex($i)->setCellValue("K2", "매출");
    }
    else if($member['mb_level'] == 9) {
        $objPHPExcel->setActiveSheetIndex($i)->mergeCells('A1:I1');
        $objPHPExcel->setActiveSheetIndex($i)->mergeCells('J1:L1');

        $objPHPExcel->setActiveSheetIndex($i)->setCellValue("B2", "프로명");
        $objPHPExcel->setActiveSheetIndex($i)->setCellValue("C2", "회원명");
        $objPHPExcel->setActiveSheetIndex($i)->setCellValue("D2", "등록일자");
        $objPHPExcel->setActiveSheetIndex($i)->setCellValue("E2", "건수");
        $objPHPExcel->setActiveSheetIndex($i)->setCellValue("F2", "현금");
        $objPHPExcel->setActiveSheetIndex($i)->setCellValue("G2", "신용카드");
        $objPHPExcel->setActiveSheetIndex($i)->setCellValue("H2", "체크카드");
        $objPHPExcel->setActiveSheetIndex($i)->setCellValue("I2", "현금+카드");
        $objPHPExcel->setActiveSheetIndex($i)->setCellValue("J2", "카드수수료");
        $objPHPExcel->setActiveSheetIndex($i)->setCellValue("K2", "프로수수료");
        $objPHPExcel->setActiveSheetIndex($i)->setCellValue("L2", "매출");
    }
}
// ===== 셀 입력 =====
...
for($i=3; $row_date=sql_fetch_array($result_date); $i++)
{
    ...
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue("A".$i, $row_date['date']);
    if($member['mb_level'] == 8) {
        $sheet1->setCellValue('J1', '총 합계 : '.number_format($total_sales));

        $objPHPExcel->setActiveSheetIndex(0)
            ->setCellValue("B".$i, $row_date['mb_name'])
            ->setCellValue("C".$i, substr($row_date['mb_reg_date'],0,10))
            ->setCellValue("D".$i, $row_date['count'])
            ->setCellValue("E".$i, !empty($row_date['cash_price']) ? number_format($row_date['cash_price']*1) : '')
            ->setCellValue("F".$i, !empty($row_date['credit_card_price']) ? number_format($row_date['credit_card_price']*1) : '')
            ->setCellValue("G".$i, !empty($row_date['check_card_price']) ? number_format($row_date['check_card_price']*1) : '')
            ->setCellValue("H".$i, number_format($cash_card))
            ->setCellValue("I".$i, $row_date['card_company'])
            ->setCellValue("J".$i, !empty($row_date['fees']) ? number_format($row_date['fees']*1) : '')
            ->setCellValue("K".$i, !empty($row_date['pro_extra_pay']) ? number_format($row_date['pro_extra_pay']*1) : '')
            ->setCellValue("L".$i, number_format($total));
    }
    else if($member['mb_level'] == 9) {
        $sheet1->setCellValue('K1', '총 합계 : '.number_format($total_sales));
...
for($i=3; $row_week=sql_fetch_array($result_week); $i++)
{
    ...
    $objPHPExcel->setActiveSheetIndex(1)->setCellValue("A".$i, $row_week['year'].'년 '.$row_week['month'].'월 '.$row_week['week_of_month'].'주');
    if($member['mb_level'] == 8) {
        $sheet2->setCellValue('I1', '총 합계 : '.number_format($total_sales));

        $objPHPExcel->setActiveSheetIndex(1)
            ->setCellValue("B".$i, $row_week['mb_name'])
            ->setCellValue("C".$i, substr($row_week['mb_reg_date'],0,10))
            ->setCellValue("D".$i, $row_week['count'])
            ->setCellValue("E".$i, !empty($row_week['cash_price']) ? number_format($row_week['cash_price']*1) : '')
            ->setCellValue("F".$i, !empty($row_week['credit_card_price']) ? number_format($row_week['credit_card_price']*1) : '')
            ->setCellValue("G".$i, !empty($row_week['check_card_price']) ? number_format($row_week['check_card_price']*1) : '')
            ->setCellValue("H".$i, number_format($cash_card))
            ->setCellValue("I".$i, !empty($row_week['fees']) ? number_format($row_week['fees']*1) : '')
            ->setCellValue("J".$i, !empty($row_week['pro_extra_pay']) ? number_format($row_week['pro_extra_pay']*1) : '')
            ->setCellValue("K".$i, number_format($total));
    }
    else if($member['mb_level'] == 9) {
        $sheet2->setCellValue('J1', '총 합계 : '.number_format($total_sales));
...
for($i=3; $row_month=sql_fetch_array($result_month); $i++)
{
    ...
    $objPHPExcel->setActiveSheetIndex(2)->setCellValue("A".$i, $row_month['month'].'월');
    if($member['mb_level'] == 8) {
        $sheet3->setCellValue('I1', '총 합계 : '.number_format($total_sales));

        $objPHPExcel->setActiveSheetIndex(2)
            ->setCellValue("B".$i, $row_month['mb_name'])
            ->setCellValue("C".$i, substr($row_month['mb_reg_date'],0,10))
            ->setCellValue("D".$i, $row_month['count'])
            ->setCellValue("E".$i, !empty($row_month['cash_price']) ? number_format($row_month['cash_price']*1) : '')
            ->setCellValue("F".$i, !empty($row_month['credit_card_price']) ? number_format($row_month['credit_card_price']*1) : '')
            ->setCellValue("G".$i, !empty($row_month['check_card_price']) ? number_format($row_month['check_card_price']*1) : '')
            ->setCellValue("H".$i, number_format($cash_card))
            ->setCellValue("I".$i, !empty($row_month['fees']) ? number_format($row_month['fees']*1) : '')
            ->setCellValue("J".$i, !empty($row_month['pro_extra_pay']) ? number_format($row_month['pro_extra_pay']*1) : '')
            ->setCellValue("K".$i, number_format($total));
    }
    else if($member['mb_level'] == 9) {
        $sheet3->setCellValue('J1', '총 합계 : '.number_format($total_sales));
...
for($i=3; $row_year=sql_fetch_array($result_year); $i++)
{
    ...
    $objPHPExcel->setActiveSheetIndex(3)->setCellValue("A".$i, $row_year['year'].'년');
    if($member['mb_level'] == 8) {
        $sheet4->setCellValue('I1', '총 합계 : '.number_format($total_sales));

        $objPHPExcel->setActiveSheetIndex(3)
            ->setCellValue("B".$i, $row_year['mb_name'])
            ->setCellValue("C".$i, substr($row_year['mb_reg_date'],0,10))
            ->setCellValue("D".$i, $row_year['count'])
            ->setCellValue("E".$i, !empty($row_year['cash_price']) ? number_format($row_year['cash_price']*1) : '')
            ->setCellValue("F".$i, !empty($row_year['credit_card_price']) ? number_format($row_year['credit_card_price']*1) : '')
            ->setCellValue("G".$i, !empty($row_year['check_card_price']) ? number_format($row_year['check_card_price']*1) : '')
            ->setCellValue("H".$i, number_format($cash_card))
            ->setCellValue("I".$i, !empty($row_year['fees']) ? number_format($row_year['fees']*1) : '')
            ->setCellValue("J".$i, !empty($row_year['pro_extra_pay']) ? number_format($row_year['pro_extra_pay']*1) : '')
            ->setCellValue("K".$i, number_format($total));
    }
    else if($member['mb_level'] == 9) {
        $sheet4->setCellValue('J1', '총 합계 : '.number_format($total_sales));
...
for($i=0; $i<4; $i++) {
    ...
    $objPHPExcel->setActiveSheetIndex($i)->getStyle('I1')->getFont()->setSize(13)->setBold(true);
    $objPHPExcel->setActiveSheetIndex($i)->getStyle('J1')->getFont()->setSize(13)->setBold(true);
    $objPHPExcel->setActiveSheetIndex($i)->getStyle('A2:N2')->getFont()->setSize(11)->setBold(true);
    ...
    $objPHPExcel->setActiveSheetIndex($i)->getStyle('I1')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
    $objPHPExcel->setActiveSheetIndex($i)->getStyle('J1')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
}
