...
$sql = " select count(*) as count from g5_lesson_reser as re left join g5_member as mb on mb.mb_no = re.mb_no where re.pro_mb_no = {$member['mb_no']} and re.reser_date = date_format(curdate(), '%Y.%m.%d') and (re.reser_state = '예약완료' || re.reser_state = '노쇼') ";
$count = sql_fetch($sql)['count'];

$sql = " select re.*, mb.mb_name, mb.mb_id_no from g5_lesson_reser as re 
         left join g5_member as mb on mb.mb_no = re.mb_no 
         where re.pro_mb_no = {$member['mb_no']}
         and re.reser_date = date_format(curdate(), '%Y.%m.%d') and (re.reser_state = '예약완료' || re.reser_state = '노쇼')
...
//$weekNo = sql_fetch(" select FLOOR((DATE_FORMAT('2021-02-28','%d')+(DATE_FORMAT(DATE_FORMAT('2021-02-28','%Y-%m-%01'),'%w')-1))/7)+1 as week_of_month from dual; ")['week_of_month']; // 이번주의 주차

$this_sunday = sql_fetch(" SELECT ADDDATE( CURDATE(), - WEEKDAY(CURDATE()) + 6 ) as sunday from dual " )['sunday']; // 이번주의 마지막일(일요일)
//$this_sunday = sql_fetch(" SELECT ADDDATE( date_format('2021-02-28', '%Y-%m-%d'), - WEEKDAY(date_format('2021-02-28', '%Y-%m-%d')) + 6 ) as sunday from dual " )['sunday']; // 이번주의 마지막일(일요일)
...
<script src="<?php echo G5_THEME_JS_URL ?>/calendar.adm.pro_lesson.js"></script>
<style>
    .clearfix:after { content: ''; display: block; clear: both; float: none; }
	#pro_schedule{ border:0;}
    /* ======== Calendar ======== */
    .my-calendar {width: inherit; padding:30px 30px 0px 30px;text-align: center; cursor: default;/* border:1px solid #ddd;*/}
	...
	background:#eee; line-height:40px; text-align:center; outline:none; position:absolute; top:5px; left:250px;}
    ...
    .ctr-box .btn-cal.next {/*float: right;*/ left:auto; right:250px;}
    ...
    .cal-table .les_yes { background:#f3d421; width:7px; height:7px; border-radius:50%; position:absolute; bottom:13px; left:50%; margin-left:-3.5px;}
	...
	.my-calendar2 .ctr-box:before{ display:block; content:""; width:10px; height:10px; border-radius:50%; background:#ffd255; position:absolute; top:15px; left:355px;}
	.my-calendar2 .ctr-box:after{ display:block; content:""; width:10px; height:10px; border-radius:50%; background:#ffd255; position:absolute; top:15px; right:355px;}
        ...
        <div class="psch_list">
            <ul>
                ...
                <li>
                    ...
                    <div class="lc_member"><?=$row['mb_id_no']?> <?=$row['mb_name']?> 고객님</div> <!-- 회원 클릭 시 해당 회원 레슨일지 페이지 이동 필요 -->
                </li>
                <?php
                }
                if($i == 0) {
...
<script>
    ...
    function weekNumberByMonth(dateFormat) {
        ...
        const weekNumberByThurFnc = (paramDate) => {
            ...
            const firstWeekCheck = firstDayOfWeek !== 0;
            // 마지막 날의 요일이 월, 화, 수라면 true
            const lastWeekCheck = lastDayOfweek !== 0;
    ...
    function tabClick(op) {
        if(op == 'month') {
            ...
            // $('.cal-week-year').text(weekNumberByMonth(getToday()).year);
            // $('.cal-week-month').text(addZero(weekNumberByMonth(getToday()).month));
    ...
    function lessonInfo(date, op) {
        ...
        $('.week-today').removeClass('week-today');
        ...
        var reser_date = $('.cal-week-year').text() + '.' + $('.cal-week-month').text() + '.' + date;
        $.ajax({
