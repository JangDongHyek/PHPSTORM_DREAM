...
$sql_group = " GROUP BY `year`, mb.pro_mb_no ";
...
$sql = " select YEAR(`pay_date`) AS `year`, MONTH(`pay_date`) AS `month`, sum(`cash_price`) as cash_price, sum(`credit_card_price`) as credit_card_price, sum(`check_card_price`) as check_card_price, 
         sum(`fees`) as fees, mb.mb_charge_pro, mb.pro_mb_no, sum(`pro_extra_pay`) as pro_extra_pay
...
<div id="tab_box">
    <ul class="tab">
        ...
        <li class="current" id="tab3"><a href="javascript:void(0);">연간매출</a></li>
    </ul>
</div><!--#tab_box-->
...
<div class="lre_ldate">
    ...
    <input type="hidden" id="sales_option" name="sales_option" value="연간매출">
...
<div class="tbl_head02 tbl_wrap mb_tbl">
    ...
    for ($i=0; $row=sql_fetch_array($result); $i++) {
        ...
        /*$search = $row['year'];
        $sql_search2 = " WHERE 1 = 1 AND mb.center_code = '{$member['center_code']}' and DATE_FORMAT(pay_date, '%Y') = '{$search}' ";
        $sql_having2 = " having `year` is null ";
        $sql2 = " select YEAR(`pay_date`) AS `year`, sum(`cash_price`) as cash_price, sum(`credit_card_price`) as credit_card_price, sum(`check_card_price`) as check_card_price, mb.mb_charge_pro, sum(1) as cnt
	...
	<tr class="<?php echo $bg; ?>">
        ...
        <td><?=number_format($total)?></td>
        <!--<td class="lv_op"><?php /*if(!empty($sum)) { echo number_format($sum).'_'.$row['date']; } else { echo number_format($total).'_'.$row['year']; } */?></td>-->
	</tr>
    <?php
    }
    if ($i == 0)
