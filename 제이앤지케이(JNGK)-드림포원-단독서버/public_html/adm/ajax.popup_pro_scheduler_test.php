    ...
    <div class="table_hd">
        <table>
            ...
            <tr>
                ...
                //$reser_count = sql_fetch(" select count(*) as count from g5_lesson_reser where reser_date = '{$reser_date}' and pro_mb_no in ({$tmp_pro}); ")['count']; // 예약 수
                //if($reser_count) { // 선택 날짜에 예약이 있으면 조회
                    ...
                    if($res_cnt > 0) {
                        while($rs = sql_fetch_array($res)) {
                            ...
                            if($diary_count == 0) { // 레슨일지 카운트가 0이면 레슨일지 한번도 등록되지 않은 회원이므로 잔여회차와 레슨회차가 동일
                                ...
                                $diary_info = sql_fetch(" select min(lesson_remain_count) as lesson_remain_count from g5_lesson_diary where mb_no = '{$rs['mb_no']}' and history_idx = '{$rs['history_idx']}'; "); // 레슨일지 정보
                ...
                /*for ($k=0; $k<count($pro_mb_no); $k++) {
                    $reser_count = sql_fetch(" select count(*) as count from g5_lesson_reser where reser_date = '{$reser_date}' and pro_mb_no = {$pro_mb_no[$k]}; ")['count'];
