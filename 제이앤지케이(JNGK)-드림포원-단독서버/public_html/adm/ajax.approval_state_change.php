...
if(!empty($state)) {
    ...
    if($count == 0) {
        ...
        if($reser_state == '노쇼') {
            ...
            if($diary_count == 0) { // 레슨일지가 하나도 없음
                $lesson_count = 1; // 현재회차: 레슨일지가 하나도 없으면 처음 등록이라서 lesson_count가 1
                if($mb['mb_state'] == 'one_point_lesson') { // 원포인트회원은 1회성 레슨
                    $lesson_remain_count = 0; // 잔여회차
                } else {
                    $lesson_remain_count = explode('/', $mb['lesson_count'])[0] - 1; // 잔여회차: 일지 등록 시 전체 회차 - 1
                }
            }
            else { // 등록된 레슨일지가 있음
                // 현재회차
                $lesson_count = sql_fetch(" select max(lesson_count*1) + 1 as lesson_count from g5_lesson_diary where mb_no = {$mb_no} and history_idx = {$mb['history_idx']} ")['lesson_count'];
                // 잔여회차: 일지 등록 시 남은 회차 - 1
                $lesson_remain_count = sql_fetch(" select min(lesson_remain_count*1) -1 as lesson_remain_count from g5_lesson_diary where mb_no = {$mb_no} and history_idx = {$mb['history_idx']} ")['lesson_remain_count'];
                if($lesson_remain_count < 0) $lesson_remain_count = 0;
            }
