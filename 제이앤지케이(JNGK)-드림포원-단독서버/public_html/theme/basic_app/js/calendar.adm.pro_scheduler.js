$(document).ready(function() {
    ...
    function loadYYMM (fullDate) {
        ...
        var state;
        $.ajax({
            url : g5_admin_url + "/ajax.reser_info_scheduler.php",
            data: {year_month : prev_month, last_day : new Date(yy, mm+1, 0).getDate()},
            type: 'POST',
            dataType: 'json',
            async: false,
    ...
    $calBody.addEventListener('click', (e) => {
        ...
        if (e.target.classList.contains('day')) {
            ...
            $.ajax({
                url : g5_admin_url + "/ajax.popup_pro_scheduler.php",
                data: {reser_date : reser_date},
                type: 'POST',
                success : function(data) {
                    // console.log(data);
                    $('.schedule_title').text(reser_date);
                    $('#calendar').html(data);
                },
                complete: function() {
                    hideLoadingBar();
