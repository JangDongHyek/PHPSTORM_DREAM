...
function get_paging2($write_pages, $cur_page, $total_page)
{
    ...
    $str = '';
    if ($cur_page > 1) {
        $str .= '<a href="javascript:get_page(1);" class="pg_page pg_start">처음</a>'.PHP_EOL;
    }
    ...
    if ($start_page > 1) $str .= '<a href="javascript:get_page('.($start_page-1).');" class="pg_page pg_prev"></a>'.PHP_EOL;

    if ($total_page > 1) {
        for ($k=$start_page;$k<=$end_page;$k++) {
            if ($cur_page != $k)
                $str .= '<a href="javascript:get_page('.$k.');" class="pg_page">'.$k.'<span class="sound_only">페이지</span></a>'.PHP_EOL;
            else
                $str .= '<span class="sound_only">열린</span><strong class="pg_current">'.$k.'</strong><span class="sound_only">페이지</span>'.PHP_EOL;
        }
    }

    if ($total_page > $end_page) $str .= '<a href="javascript:get_page('.($end_page+1).');" class="pg_page pg_next"></a>'.PHP_EOL;

    if ($cur_page < $total_page) {
        $str .= '<a href="javascript:get_page('.$total_page.');" class="pg_page pg_end">맨끝</a>'.PHP_EOL;
    }
...
// 쿠키변수 어플에서 생성
function set_cookie_app($cookie_name, $value, $expire)
{
    global $g5;

    setcookie($cookie_name, $value, G5_SERVER_TIME + $expire, '/', G5_COOKIE_DOMAIN);
}
...
function get_member($mb_id, $fields='*')
{
    ...
    return sql_fetch(" select $fields from {$g5['member_table']} where BINARY mb_id = TRIM('$mb_id') ");
}
...
function is_admin($mb_id)
{
    ...
    if ($config['cf_admin'] == $mb_id || $mb_id == "lets080") return 'super';
...
function member_delete($mb_id)
{
    ...
    $sql = " update {$g5['member_table']} set mb_leave_date = '".date('Ymd', G5_SERVER_TIME)."', mb_password = '', mb_level = 1, mb_email = '', mb_homepage = '', mb_tel = '', mb_hp = '', mb_zip1 = '', mb_zip2 = '', mb_addr1 = '', mb_addr2 = '', mb_birth = '', mb_sex = '', mb_signature = '', mb_memo = '".date('Ymd', G5_SERVER_TIME)." 삭제함\n{$mb['mb_memo']}' where mb_id = '{$mb_id}' ";
    sql_query($sql);
...
//2016-03-31 이지원 DB 컬럼리스트 불러오기
function get_columns_list($sql, $error=G5_DISPLAY_SQL_ERROR, $link=null){
    global $g5;
...
//2016-03-31 이지원 DB 컬럼 추가
function set_columns($column, $table, $type="varchar(255)", $error=G5_DISPLAY_SQL_ERROR, $link=null){
	global $g5;
...
//2016-04-22 이지원 ipconfig 함수
function ipconfig($ip){
	global $g5;

	$link = $g5['connect_db'];

	$sql = "select * from g5_ipconfig where ip_code = '{$ip}'";
	$result = sql_query($sql, $error, $link);
...
// 프로필(일반) 또는 회사로고(기업)
function getProfileImg($mb_id, $category, $open) {
    $folder = 'member';
    if($category == '기업') {
        $sql_search = " and category = '로고' ";
        $folder = 'company';
    }

    if($open == 'private') { // 커뮤니티 - 익명
        return '<img class="basic toggle" src="'.G5_IMG_URL.'/img_smile.svg">';
    }

    $img = sql_fetch(" select * from g5_member_img where mb_id = '{$mb_id}' {$sql_search}; ")['img_file'];
    if(empty($img)) { // 기본이미지
        if($category == '기업') {
            return '<img class="com toggle" src="'.G5_IMG_URL.'/img_nlogo.jpg">';
        } else { // 일반
            return '<img class="basic toggle" src="'.G5_IMG_URL.'/img_smile.svg">';
        }
    } else {
        return '<img class="toggle" src="'.G5_DATA_URL.'/file/'.$folder.'/'.$img.'" height="100%">';
    }
}

// 파일 업로드 (경로, 파일명, 임시파일명)
function fileUpload_up($path, $filename, $tmp_file) {
    //$dotIndexOf = strpos($filename, ".") + 1;
    //$imgLength = strlen($filename);
    //$ext = strtolower(substr($filename, $dotIndexOf, $imgLength));//확장자
    $fileArr = explode('.', $filename);
    $ext = end($fileArr);
    $uploadPath = $path;
    $rand = rand(0, 99);
    $img_file = date("YmdHis") . $rand . "." . $ext;
    move_uploaded_file($tmp_file, $uploadPath . $img_file);

    return $img_file; // 서버에 저장되는 파일명 반환
}

// 쿼리 select count (테이블, 컬럼, 데이터) * 조건 최대 2개, join문, date_format, 문자열 검색에 사용불가
function selectCount($table, $column1, $data1, $column2, $data2) {
    if(!empty($column2)) { // 조건 2개
        $rlt = sql_fetch(" select count(*) as count from {$table} where {$column1} = '{$data1}' and {$column2} = '{$data2}'; ")['count'];
    }
    else { // 조건 1개
        $rlt = sql_fetch(" select count(*) as count from {$table} where {$column1} = '{$data1}'; ")['count'];
    }
    return $rlt;
}

// 쿼리 select count 새로운 버전 (테이블, where 조건)
function selectCount_n($table, $data1, $data2, $data3) {
    $sql_add = '';
    if(!empty($data1)) { $sql_add .= ' and '.$data1; }
    if(!empty($data2)) { $sql_add .= ' and '.$data2; }
    if(!empty($data3)) { $sql_add .= ' and '.$data3; }

    return sql_fetch(" select count(*) as count from {$table} where 1 {$sql_add} ")['count'];
}

// 로그인 여부 및 일반/기업 회원 체크
function loginCheck($mb_id, $category) {
    if(empty($mb_id)) alert('로그인 후 이용해 주세요.', G5_BBS_URL.'/login.php');

    $mb = get_member($mb_id);
    // // 22.12.06 기업회원 접근 못하게 함
    // if($mb['mb_category'] == '기업') alert("올바른 경로가 아닙니다.", G5_URL, true);

    // 기업회원만 접근
    if($mb['mb_category'] != $category) alert('올바른 경로가 아닙니다.', G5_URL);
}
...
// 내 벙커 확인 (BUNKER 부족 여부)
function myBunkerCheck($mb_id, $bunker) {
    $mb = get_member($mb_id);

    if($mb['mb_bunker'] + $mb['mb_bunker'] < $bunker) {
        return false;
    }
...
// BUNKER HISTORY (대상아이디, 구분(적립/차감), 포인트, 내용, 연관아이디, 연관테이블, 연관idx, 기타, 기타2))
function bunkerHistory($mb_id, $mode, $bunker, $contents, $rel_mb_id, $rel_table, $rel_idx, $etc, $etc2) {
    //if($mb_id == 'admin') { return ''; } // admin은 기록하지 않음 -- 확인용으로 INSERT

    $mb = get_member($mb_id);
    $update_sql = ''; // g5_member
    $insert_add = ''; // g5_bunker_history
    $bonus_flag = false; // 보너스 벙커 사용 여부
    $normal_bunker = 0; // 일반
    $bonus_bunker = 0; // 보너스
    $re_normal = 0; // 일반 잔여
    $re_bonus = 0; // 보너스 잔여
    ...
    /*if($mode == '정정') { // 헬프미 질문 벙커 수정 시 사용
        $normal_bunker = $normal;
        $bonus_bunker = $bonus;
        $re_normal = $mb['mb_bunker'] + $normal; // 일반 잔여
        $re_bonus = $mb['mb_bunker_bonus'] + $bonus; // 보너스 잔여

        $update_sql .= " mb_bunker = {$re_normal}, mb_bunker_bonus = {$re_bonus} ";
    }*/

    // 잔여포인트
    if($mode == '적립') {
        if($etc == 'correct') { // 지급했다가 회수하는 경우 사용한 벙커가 일반 벙커인지 보너스 벙커인지 확인 필요
            // 차감된 벙커 종류 확인
            $info = sql_fetch(" select * from g5_bunker_history where mb_id = '{$mb_id}' and mode = '차감' and rel_table = '{$rel_table}' and rel_idx = '{$rel_idx}' order by idx desc limit 1 ");
            $normal_bunker = $info['normal']; // 일반
            $bonus_bunker = $info['bonus']; // 보너스
            $re_normal = $mb['mb_bunker'] + $info['normal']; // 일반 잔여
            $re_bonus = $mb['mb_bunker_bonus'] + $info['bonus']; // 보너스 잔여

            $update_sql .= " mb_bunker = {$re_normal}, mb_bunker_bonus = {$re_bonus} "; // g5_member update
        }
        else if($etc == 'bonus') { // 보너스 BUNKER -- 충전 시 보너스 지급
            $bonus_bunker = $bunker; // 보너스
            $re_bonus = $mb['mb_bunker_bonus'] + $bunker; // 보너스 잔여

            $update_sql .= " mb_bunker_bonus = {$re_bonus} "; // g5_member update
        }
        else {
            $normal_bunker = $bunker; // 일반
            $re_normal = $mb['mb_bunker'] + $bunker; // 일반 잔여

            $update_sql .= " mb_bunker = {$re_normal} "; // g5_member update
        }
    }
    else if($mode == '차감') {
        if($etc != 'withdraw' && $etc != 'gift') { // 출금/선물은 보너스 벙커 사용하지 않음
            if(!empty($mb['mb_bunker_bonus']) && $mb['mb_bunker_bonus'] >= $bunker) { // 보너스 BUNKER가 0이 아니고 차감 BUNKER보다 많거나 같을 때
                $re_bonus = $mb['mb_bunker_bonus'] - $bunker; // 보너스 잔여
                $re_normal = $mb['mb_bunker']; // 일반 잔여

                $update_sql .= " mb_bunker_bonus = {$re_bonus} "; // g5_member update
                $bonus_flag = true; // 보너스 O
                $bonus_bunker = $bunker; // 사용 보너스 벙커
            }
            else if(!empty($mb['mb_bunker_bonus']) && $mb['mb_bunker_bonus'] < $bunker) { // 보너스 BUNKER가 0이 아니고 차감 BUNKER보다 부족할 때
                $temp_bunker = $bunker - $mb['mb_bunker_bonus']; // 차감 BUNKER에서 보너스 BUNKER만큼 제외하여 남은 차감 BUNKER 구함
                // 내 BUNKER 잔액 확인
                if(!empty($mb['mb_bunker']) && $mb['mb_bunker'] >= $temp_bunker) { // 내 BUNKER가 0이 아니고 남은 차감 BUNKER보다 많을 때
                    $temp_bunker = $bunker - $mb['mb_bunker_bonus']; // 차감 BUNKER에서 보너스 BUNKER만큼 제외하여 남은 차감 BUNKER 구함
                    $re_bonus = 0; // 보너스 잔여
                    $re_normal = $mb['mb_bunker'] - $temp_bunker; // 일반 잔여

                    $update_sql .= " mb_bunker = {$re_normal}, mb_bunker_bonus = 0 ";  // g5_member update
                    $bonus_flag = true; // 보너스 O
                    $bonus_bunker = $mb['mb_bunker_bonus']; // 사용 보너스 벙커
                    $normal_bunker = $temp_bunker; // 사용 일반 벙커
                }
                else {
                    return '';
                }
            }
            else if(empty($mb['mb_bunker_bonus'])) { // 보너스 BUNKER 0일 때
                // 내 BUNKER 잔액 확인
                if(!empty($mb['mb_bunker']) && $mb['mb_bunker'] >= $bunker) { // 내 BUNKER가 0이 아니고 차감 BUNKER보다 많을 때
                    $re_normal = $mb['mb_bunker'] - $bunker; // 일반 잔여
                    $re_bonus = 0; // 보너스 잔여

                    $update_sql .= " mb_bunker = {$re_normal} "; // g5_member update
                    $bonus_bunker = 0; // 사용 보너스 벙커
                    $normal_bunker = $bunker; // 사용 일반 벙커
                }
                else {
                    return '';
                }
        ...
        else if($etc == 'withdraw' || $etc == 'gift') { // 출금 || 선물 (츨금과 선물은 보너스로 할 수 없음)
            if(!empty($mb['mb_bunker']) && $mb['mb_bunker'] >= $bunker) { // 내 BUNKER가 0이 아니고 출금 BUNKER보다 많을 때
                $re_normal = $mb['mb_bunker'] - $bunker; // 일반 잔여
    ...
    // g5_bunker_history insert
    if($bonus_flag) { $etc = 'bonus'; }
    if(!empty($rel_mb_id)) { $insert_add .= ", rel_mb_id = '{$rel_mb_id}' "; }
    if(!empty($rel_table)) { $insert_add .= ", rel_table = '{$rel_table}' "; }
    if(!empty($rel_idx)) { $insert_add .= ", rel_idx = '{$rel_idx}' "; }
    if(!empty($etc) || $bonus_flag) { $insert_add .= ", etc = '{$etc}' "; }
    if(!empty($etc2)) { $insert_add .= ", etc2 = '{$etc2}' "; } // 관리자인지 표시

    // 회원 BUNKER UPDATE
    sql_query(" update g5_member set {$update_sql} where mb_id = '{$mb_id}'; ");
    // BUNKER DB INSERT
    return sql_query(" insert into g5_bunker_history set mb_id = '{$mb_id}', mode = '{$mode}', bunker = '{$bunker}', contents = '{$contents}', wr_datetime = '".G5_TIME_YMDHIS."' {$insert_add}, normal = '{$normal_bunker}', normal_remain = '{$re_normal}', bonus = '{$bonus_bunker}', bonus_remain = '{$re_bonus}'; ");
}

// 날짜형식 변경 (년 월 일 형식으로 변경, '-' 기호 => '.' 기호로 변경)
function replaceDateFormat($date) {
    return str_replace('-','.',substr($date,0,10));
}

// AES 암호화 (엔잡러 암호화 사용)
function Encrypt($str, $secret_key='dd772556546e4733e514e0561181f2925dc3a144', $secret_iv = 'dd772556546e4733e514') {
...
// AES 복호화 (엔잡러 복호화 사용)
function Decrypt($str, $secret_key='dd772556546e4733e514e0561181f2925dc3a144', $secret_iv='dd772556546e4733e514') {
    $key = hash('sha256', $secret_key);
    $iv = substr(hash('sha256', $secret_iv), 0, 32);
...
// 기업평점
function CompanyScore($mb_id, $decimal) {
    // 작성된 리뷰의 별점으로 평점을 구함 -- 견적을 보낸 상대 회사 선택 시 리뷰를 작성하므로 견적 정보를 기준으로 리뷰 정보를 찾아야 함 (estimate_mb_id = 선택 회사 정보)
    $rlt = sql_query(" select * from g5_company_inquiry_result where estimate_mb_id = '{$mb_id}' and type = '거래후기' ");

    $score_sum = 0;
    $i = 0;
    while($row = sql_fetch_array($rlt)) {
        $i++;
        $score_sum += $row['star_score'];
    }

    if($i != 0) {
        if($decimal) { // 소수점 표시
            return $score_sum / $i; // 평점
        } else { // 소수점 삭제
            return str_replace('.', '', $score_sum / $i); // 평점
        }
    } else {
        return 0;
    }
...
// 리뷰개수
function reviewCount($mb_id) {
    return number_format(selectCount("g5_company_inquiry_result", "estimate_mb_id", $mb_id, "type", "거래후기"));
}

// 의뢰건수 (내가 요청한 의뢰 건)
function inquiryCount($mb_id) {
    return sql_fetch(" select count(*) as cnt from g5_company_inquiry where mb_id = '{$mb_id}' and del_yn is null ")['cnt'];
}

// 거래건수 (의뢰 상태가 거래완료 건)
function completeCount($mb_id) {
    // 총 거래건수 (나의의뢰 + 나의견적)
    $count1 = selectCount("g5_company_inquiry", "mb_id", $mb_id, "ci_state", "거래완료"); // 내가 의뢰한 견적이 거래 체결
    $count2 = selectCount("g5_company_estimate", "mb_id", $mb_id, "ce_selection", "Y"); // 내가 보낸 견적으로 거래 체결

    return number_format($count1 + $count2);
}

// 등급포인트 (대상아이디, 구분(적립/차감), 적립기준, 포인트, 내용, 연관아이디, 연관테이블, 연관idx)
function gradePointInsert($mb_id, $mode, $category, $point, $contents, $rel_mb_id, $rel_table, $rel_idx) {
    //if($mb_id == 'admin') { return ''; } // admin은 기록하지 않음 -- 확인용으로 INSERT

    $mb = get_member($mb_id);

    if($mb['mb_category'] == '기업') { // 기업 회원은 적립 X
        return;
    }

    if($mode == '적립') {
        $acc_point = $mb['mb_grade_point'] + $point; // 누적 포인트
    } else { // 차감
        $acc_point = $mb['mb_grade_point'] - $point; // 누적 포인트
    }
    ...
    // 포인트 DB INSERT
    sql_query(" insert into g5_member_point set mb_id = '{$mb_id}', mode = '{$mode}', category = '{$category}', point = '{$point}', acc_point = '{$acc_point}', contents = '{$contents}', wr_datetime = '".G5_TIME_YMDHIS."' {$insert_add} ");
    // 회원 DB UPDATE
    if($mode == '적립') {
        $result = sql_query(" update g5_member set mb_grade_point = mb_grade_point + '{$point}' where mb_id = '{$mb_id}' ");
    } else { // 차감
        $result = sql_query(" update g5_member set mb_grade_point = mb_grade_point - '{$point}' where mb_id = '{$mb_id}' ");
    }
    ...
    **/
    ...
    // 현재 회원이 가지고 있는 포인트가 위 범위 안에 들어갈 경우 등급 유지 또는 승급
    $mb = get_member($mb_id); // 회원 정보 재조회
    $sql_update = '';
    if($mb['mb_grade_point'] > 4999) {
        $sql_update = "mb_grade = '선장'";
    }
    else if($mb['mb_grade_point'] > 1799) {
        $sql_update = "mb_grade = '1등항해사'";
    }
    else if($mb['mb_grade_point'] > 599) {
        $sql_update = "mb_grade = '2등항해사'";
    }
    else if($mb['mb_grade_point'] > 199) {
        $sql_update = "mb_grade = '3등항해사'";
    }

    sql_query(" update g5_member set {$sql_update} where mb_id = '{$mb_id}' ");

    return $result;
}

// 시간 데이터를 오전, 오후 포맷으로 변경
function getTime_Format1($date_str) {
...
// 헬프미/커뮤니티 좋아요/싫어요 체크 (메뉴, 액션, 아이디, idx(질문), sub_idx(답변))
function actionCheck($menu, $mode, $mb_id, $idx, $sub_idx) {
    if($menu == 'helpme') {
        if(strpos($mode, 'answer') !== false) { // 답변 관련 액션
            $search_add = " and helpme_an_idx = {$sub_idx} ";
        }
        $count = sql_fetch(" select count(*) as count from g5_helpme_action where helpme_idx = {$idx} {$search_add} and mb_id = '{$mb_id}' and mode = '{$mode}' and date_format(wr_datetime,'%Y-%m-%d') = date_format(now(), '%Y-%m-%d'); ")['count']; // 금일 해당 동작을 했는지 확인
    }
    else { // community
        if(strpos($mode, 'answer') !== false) { // 답변 관련 액션
            $search_add = " and community_an_idx = {$sub_idx} ";
        }
        $count = sql_fetch(" select count(*) as count from g5_community_action where community_idx = {$idx} {$search_add} and mb_id = '{$mb_id}' and mode = '{$mode}' and date_format(wr_datetime,'%Y-%m-%d') = date_format(now(), '%Y-%m-%d'); ")['count']; // 금일 해당 동작을 했는지 확인
    }

    if($count == 0) { // 금일 해당 동작을 안했으면
        return '';
    } else {
        return 'on';
    }
...
// 닉네임 없으면 아이디
function getNickOrId($mb_id) {
    $mb = get_member($mb_id);
    return !empty($mb['mb_nick']) ? $mb['mb_nick'] : $mb['mb_id'];
}
...
function sendFcm($tokens, $message)
{
    ...
    $fields = array(
        ...
        'data' => $message,
        'priority' => 'high',
        'notification' => array( 'title' => $message['subject'], 'body' => $message['message'],'url' => G5_URL)
    );
    $headers = array(
        'Authorization:key = AAAAryfKiV4:APA91bGWf8Gg1GN7XoGrGk1D1v88HyY26Gsd0dN5YrCAichu6jK7l7QXdlNvVt9BHX0ts5KFt3IvwlBP85HEvhrkR1vL-QXeT8WGZ1WSsg8XiZLJ7lJe-6nisj_TgstnJxr0fC-efGby', // 서버키 - console.firebase - 프로젝트 선택 - 클라우드 메시징 - 서버 키
...
// 태그 검색 (콤마로 디비에 들어가있기 때문에 검색 시 분리해서 처리)
function tagSearch($column, $tag) {
    $arr = explode(',', $tag);

    $str = ' (';
    for($i=0; $i<count($arr); $i++) {
        $str .= " find_in_set('".$arr[$i]."', ".$column.") or ";
    }
    $str = substr($str, 0, -4);
    $str .= ')';
...
// 관리자체크 (헬프미/커뮤니티에서 관리자가 쓴 글이면 수정도 함께 보이고 관리자가 쓴글 아니면 삭제만 보임)
function adminChk($mb_id) {
    $mb = get_member($mb_id);
...
function blockUser($mb_id) {
    $block_rlt = sql_query(" select * from g5_block where mb_id = '{$mb_id}' ");
    $block = '';
    while($block_row = sql_fetch_array($block_rlt)) {
        $block .= "'".$block_row['target_mb_id']."',";
    }
...
// 신고한 사용자
function reportUser($mb_id) {
    $report_rlt = sql_query(" select * from g5_report where mb_id = '{$mb_id}' ");
    $report = '';
    while($report_row = sql_fetch_array($report_rlt)) {
        $report .= "'".$report_row['target_mb_id']."',";
    }
...
// 프로필 업데이트 체크
function profileUpdateCheck($mb_id, $mb_level) {
    if($mb_level == 10) { return true; } // 관리자는 패스

    // 프로필 업데이트 완료 여부 확인
    if($mb_level == 2) { // mb_level == 2 (일반회원)
        $profile = sql_fetch(" select count(*) as cnt from g5_bunker_history where mb_id = '{$mb_id}' and contents like '%프로필 업데이트 완료%' ")['cnt'];
    }
    else if($mb_level == 3) { // mb_level == 3 (기업회원)
        $profile = sql_fetch(" select count(*) as cnt from g5_member where mb_id = '{$mb_id}' and mb_hashtag is not null ")['cnt']; // 해시태그까지 입력완료해야 프로필 작성 완료한걸로 판단
    }
    if($profile > 0) {
        return true;
    } else {
