var url = "https://www.podosea.kr:5003"; // chat.js 충돌 오류로 const에서 var로 변경
...
socket.on('chat_view', function (data) {
    ...
    if (user_id != data.user_id) {
        ...
        if(img_class == 'file') { // 파일
            chat_html += `<a class="file_box" href="http://www.podosea.kr/bbs/file_download.php?mode=chat&temp=${data.server_file_name}&real=${data.file_name}" width="100%">
                                <div class="icon"><img src="http://www.podosea.kr/img/icon_chat_file.svg"></div>
                                <div class="file_info">
            ...
            chat_html += `<a href="http://www.podosea.kr/bbs/file_download.php?mode=chat&temp=${data.server_file_name}&real=${data.file_name}">`
            chat_html += `${msg}`;
            chat_html += `</a>`;
        } else { // 그 외
            chat_html += `${msg}`;
    ...
    const scrolltop = Math.floor(chat_msg.scrollHeight - chat_msg.offsetHeight);
    if (!is_chat_scroll) {
...
function viewScrollTop() {
    ...
    chat_msg.scrollTop = Math.floor(scrolltop);
    // if (scrolltop != Math.floor(chat_msg.scrollTop)) {
    //     setTimeout("viewScrollTop()", 500);
    // }
}
...
try {
    ...
    document.getElementById("msg").addEventListener('keyup', function (event) {
        ...
        if($.trim(document.getElementById("msg").value).length == 0) { // 빈칸이면
    ...
    document.getElementById("chat-msg").addEventListener('scroll', function () {
        ...
        // console.log('chat_msg scrolltop', Math.floor(chat_msg.scrollTop));
        ...
        if (Math.floor(chat_msg.scrollTop) == 0 && is_chat_scroll) {
            // console.log("이전페이지");
...
function getChatData(data, func) {
    ...
    for (var i = 0; i < data.chats.length; i++) {
        ...
        if (data.user_id != data.chats[i].user_id) {
            ...
            chat_html += `<div class="cont_box msg `+img_class+`">`;
            if(img_class == 'file') { // 파일
                chat_html += `<a class="file_box" href="http://www.podosea.kr/bbs/file_download.php?mode=chat&temp=${data.chats[i].server_file_name}&real=${data.chats[i].file_name}" width="100%">
                                    <div class="icon"><img src="http://www.podosea.kr/img/icon_chat_file.svg"></div>
                                    <div class="file_info">
                ...
                chat_html += `<a href="http://www.podosea.kr/bbs/file_download.php?mode=chat&temp=${data.chats[i].server_file_name}&real=${data.chats[i].file_name}">`
                chat_html += `${msg}`;
                chat_html += `</a>`;
            } else { // 그 외
                chat_html += `${msg}`;
        ...
        // 내 메세지
            ...
            chat_html += `<div class="cont_box msg `+img_class+`">`;
            if(img_class == 'file') { // 파일
                //<a class="file_box" href="http://www.podosea.kr/node/uploads/${data.chats[i].server_file_name}" download="${data.chats[i].file_name}" width="100%">
                //<a class="file_box" href="http://www.podosea.kr/bbs/file_download.php?mode=chat&temp=${data.chats[i].server_file_name}&real=${data.chats[i].file_name}" width="100%">
                chat_html += `<a class="file_box" href="http://www.podosea.kr/bbs/file_download.php?mode=chat&temp=${data.chats[i].server_file_name}&real=${data.chats[i].file_name}" width="100%">
                                    <div class="icon"><img src="http://www.podosea.kr/img/icon_chat_file.svg"></div>
                                    <div class="file_info">
                                        <div class="subject">${data.chats[i].file_name}</div>
