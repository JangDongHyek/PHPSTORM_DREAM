...
// moid(moid) '-' mb_no '-' idx '-' use_bunker '-' all_use
$moid_arr = explode('-',$_REQUEST["MOID"]);
$mb_no = $moid_arr[1]; // 회원 no
$rr_idx = $moid_arr[2]; // 자료실 idx
$use_bunker = $moid_arr[3]; // 사용 벙커
$all_use = $moid_arr[4]; // 항상 보유 벙커 전액 사용
...
$sql = "INSERT INTO g5_reference_room_payment SET 
        mb_no = '{$mb_no}',
        reference_idx = '{$rr_idx}',
...
$code = base64_encode("refer".rand(0,100).'_'.$rr_idx);
$url = G5_BBS_URL."/shop_view.php?code=".$code;
...
if ($pg_status == 1) // 결제성공
{
    if ($result == 1)
    {
        ...
        // 구매 기록
        $reference = sql_fetch(" select * from g5_reference_room where idx = '{$rr_idx}' ");
        $sql = " insert into g5_reference_room_sale set reference_idx = '{$rr_idx}', buy_mb_id = '{$member['mb_id']}', sale_mb_id = '{$reference['mb_id']}', bunker = '{$use_bunker}', price = '{$_REQUEST['Amt']}', wr_datetime = '".G5_TIME_YMDHIS."' ";
        $result = sql_query($sql);

        if(!empty($use_bunker)) {
            // BUNKER HISTORY INSERT (대상아이디, 구분(적립/차감), 포인트, 내용, 연관아이디, 연관테이블, 연관idx, 기타))
            bunkerHistory($member['mb_id'], '차감', $use_bunker, '자료실 결제', '', 'g5_reference_room', $rr_idx, '', '');

            // 판매자에게 벙커 지급
            bunkerHistory($reference['mb_id'], '적립', $use_bunker, '자료실 결제', $member['mb_id'], 'g5_reference_room', $rr_idx, 'reference', '');
        }

        // 항상 보유 벙커 전액 사용
        sql_query("update g5_member set always_use = '{$all_use}' where mb_id = '{$member['mb_id']}' ");

        // 판매자에게 판매 대금 지급 (PG결제 금액만)
        sql_query(" update g5_member set sales_proceeds = sales_proceeds + '{$_REQUEST["Amt"]}', sales_proceeds_acc = sales_proceeds_acc + '{$_REQUEST["Amt"]}' where mb_id = '{$reference['mb_id']}' ");
    }
    else
    {
