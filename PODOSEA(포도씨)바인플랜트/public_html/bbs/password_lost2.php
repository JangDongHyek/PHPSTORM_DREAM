...
$sql = " select mb_no, mb_id, mb_name, mb_nick, mb_datetime, sns from {$g5['member_table']} where mb_id= '{$mb_id}' and mb_email = '{$mb_email}' ";
$mb = sql_fetch($sql);
if (!$mb['mb_id'])
    alert('존재하지 않는 회원입니다.');
else if (is_admin($mb['mb_id']))
    alert('관리자 아이디는 접근 불가합니다.');
else if(!empty($mb['sns'])) {
    $sns_str = "";
    switch ($mb['sns']) {
        case "naver" :
            $sns_str = "네이버";
