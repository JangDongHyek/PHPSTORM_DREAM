...
if($w == "") { // 등록
    ...
    // 답변 작성 시 질문 등록자에게 푸시
    $help = sql_fetch(" select * from g5_helpme where idx = '{$helpme_idx}' "); // 헬프미 정보
    $push_status = "helpme";
    $push_data = array('subject'=>$help['he_subject'], 'url'=>G5_BBS_URL."/help_view.php?idx=".$helpme_idx, 'ref_idx'=>$helpme_idx, 'ref_table'=>'g5_helpme', 'mb_id'=>$help['mb_id']);
    @include_once(G5_BBS_PATH.'/send_push.php');

    alert('답변이 등록되었습니다', G5_BBS_URL.'/help_view.php?idx='.$helpme_idx, false);
}
else { // 수정
    $help = sql_fetch(" select * from g5_helpme where idx = '{$helpme_idx}' ");
    if($help['he_answer_state'] == '답변완료') {
        alert('답변이 완료되어 수정할 수 없습니다.', G5_BBS_URL.'/help_view.php?idx='.$helpme_idx);
    }
    ...
    alert('답변이 수정되었습니다', G5_BBS_URL.'/help_view.php?idx='.$helpme_idx, false);
}
?>