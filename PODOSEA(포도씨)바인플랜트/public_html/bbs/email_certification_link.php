...
if($count > 0) {
    echo '<script>alert("이미 인증이 완료되었습니다.");parent.window.close();</script>';
    exit;
} else {
    ...
    if($ch_count != $request_count) {
        echo '<script>alert("인증이 만료되었습니다.");parent.window.close();</script>';
        exit;
    }
    ...
    echo '<script>alert("인증이 완료되었습니다.");parent.window.close();</script>';
    exit;
}
?>