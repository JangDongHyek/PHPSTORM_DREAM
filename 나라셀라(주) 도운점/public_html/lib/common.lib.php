...
function getClassInfo($class_idx, $mb_id = ''){
    ...
    $sql = "
        ...
        FROM
            ...
            heart_list AS H ON H.class_idx = C.class_idx AND H.mb_id = '{$mb_id}' AND H.isUse = 'Y'
        WHERE 
            C.class_idx = '{$class_idx}'; 
    ";
                
    $info = sql_fetch($sql);
    $classInfo = sortClassInfo($info);
    
    return $classInfo;
}

function getClassAppInfo($class_app_idx){    
    return sql_fetch("SELECT * FROM class_app_list WHERE class_app_idx = '{$class_app_idx}';");
}

function getPayInfo($class_app_idx){
    return sql_fetch("SELECT * FROM pay_list WHERE class_app_idx = '{$class_app_idx}';");
}

function getClassAppMemberList($class_app_idx){
...
function sortClassInfo($info){
    
    if(empty($info['thumbnailImgJson'])){
        $info['thumbnailImgJson'] = array();
        $info['thumbnail'] = "";
    }else{
        $info['thumbnailImgJson'] = json_decode($info['thumbnailImgJson'], true);
        $info['thumbnail'] = G5_CLASS_THUMBNAIL_URL.$info['thumbnailImgJson'][0]['fileName'];
    }
        
    $info['classStatus'] = ($info['eventDateTime'] > DATE_HI)? '진행중' : '종료';
    
    switch($info['classStatus']){            
        case 'ready' : $info['clsClassStatus'] = 'ready'; break;
        case '종료' : $info['clsClassStatus'] = 'done'; break;
    }
    
    return $info;
}

function checkClassEff($class_idx, $mb_id, $personCnt = 1){
    $result = array('result' => false, 'msg' => '', 'type' => '');
    ...
    $isMyClassApp = ((int)sql_fetch($sql)['cnt'] > 0);
    
//    if($isMyClassApp){
//        $result['msg'] = '이미 신청한 내역이 존재합니다.<br>MY PAGE에서 이용내역을 확인해주세요.';
//        return $result;
//    }
    
    // 신청 시간이 끝났는지 체크
    if(DATE_HI > $info['eventDateTime']){
        $result['msg'] = '마감된 CLASS입니다.';
        return $result;
    }
    ...
    /* 추가할 인원 */
    $personCnt = (int)$personCnt;
    
    if(($nowPerson + $personCnt) > $info['maxPerson']){        
        $result['msg'] = "현재 정원초과로 인해 대기상태로 접수됩니다.<br>진행하시겠습니까?";
        $result['type'] = 'WAIT';        
    }
...
function replaceHyphenWithDot($str){
    return str_replace("-", ".", $str);
}

/* 파일 업로드 공통 */
function cmmFileUpload($files, $folderDir){            
    
    include G5_LIB_PATH."/php-image-resize-master/ImageResize.php";
    ...
    for($index=0; $index < $filesLength; $index++){
        ...
        $uploadsDir = G5_PATH.$folderDir; /* 업로드 경로 */
        $originalFileName = $files['name'][$index]; /* 파일 이름 */
        $ext = array_pop(explode('.', $originalFileName)); /* 파일 확장자 */        
        ...
        if(move_uploaded_file($files['tmp_name'][$index], $filePath)){                        
            ...
            $fileType = explode('/', $files['type'][$index])[0]; /* 파일 타입(image, video..) */
            
            if($fileType == 'image'){                
                $resizeImg = new \Gumlet\ImageResize($filePath);
                $resizeImg -> resizeToWidth(700);
...
function innopayCurl($apiSubUrl, $data){
    /* url */
    $tossApiUrl = "https://api.innopay.co.kr".$apiSubUrl;
    
    $headers = array('Content-Type:application/json');

    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, $tossApiUrl);
...
// sms, mms발송
function goMMS($reserv_phone, $send_phone, $content, $type = 'text', $img="", $img2="") {
	...
	$img_arr = array();
	if ($img != '') $img_arr[] = $img;
	if ($img2 != '') $img_arr[] = $img2;

	if($type == 'img'){
		if (count($img_arr) > 0) { //if ($img != '' || $img2 != '') {
			/*
			$file_cnt = ($img != '' && $img2 != '')? 3 : 2;
			$query = "Insert into em_tran_mms (file_cnt, mms_body, mms_subject, file_type1, file_name1, file_type2, file_name2, service_dep1, service_dep2) values 
					  (2,'$content','$subject','IMG','$img', 'IMG', '$img2', 'ALL', 'ALL')";
			*/
			$file_cnt = count($img_arr)+1;
			$query = "Insert into em_tran_mms (file_cnt, mms_body, mms_subject";
			$i = 1;
			foreach ($img_arr AS $key=>$val) {
				$query .= ", file_type{$i}, file_name{$i}, service_dep{$i}";
				$i++;
			}
			$query .= ") values ('{$file_cnt}', '$content','$subject'";
			$i = 1;
			foreach ($img_arr AS $key=>$val) {
				$query .= ", 'IMG', '{$val}', 'ALL'";
				$i++;
		...
		$mms_query = "Insert into em_tran (tran_pr,tran_id,tran_phone, tran_callback, tran_status, tran_date, tran_msg , tran_type, tran_etc4) values 
					  (null,'$tran_id','$reserv_phone','$send_phone','1',sysdate(),'', '6', '$tran_etc4')";
		...
		$mms_query = "Insert into em_tran (tran_pr,tran_id,tran_phone, tran_callback, tran_status, tran_date, tran_msg, tran_type) values				
					  (null,'$tran_id','$reserv_phone','$send_phone','1','$send_date','$content', '$tran_type')";
		$result=mysqli_query($conn_db,$mms_query);

		$mms_query = "Insert into em_tran_log (tran_pr,tran_id, tran_phone, tran_callback, tran_status, tran_date, tran_msg, tran_type) values 
					  (null,'$tran_id','$reserv_phone','$send_phone','1','$send_date','$content', '$tran_type')";
		$result=mysqli_query($conn_db,$mms_query);
...
// 랜덤난수생성
function getRandomString($leng = 6, $type="") {
...
// 카카오 알림톡 발송
// type : 템플릿종류, params:템플릿변수, hp:휴대폰
function sendAlimTalk($type, $params=array(), $mb_hp, $mb_no="") {
    //if ((int)$type > 6) return;	// 탬플릿 추가시 변경할것!!!

    //'-' hypen이 포함될경우 S405(파라미터가 올바르지 않음) 에러 뱉음 : 포함되지 않도록 주의할것!!
...
//
    // 1) 발송
    //템플릿 1번
    $template[0]['code'] = "SJB_075624";
    $template[0]['title'] = "가입 완료안내";
    $template[0]['content'] = "[나라셀라 도운]\n";
    $template[0]['content'] .= "\n";
    $template[0]['content'] .= "안녕하세요. #{mb_name}님!";
    $template[0]['content'] .= "\n";
    $template[0]['content'] .= "회원가입이 완료 되었습니다.";
    $template[0]['content'] .= "\n";
    $template[0]['content'] .= "앞으로 나라셀라 도운빌딩에 많은 관심 부탁드리며";
    $template[0]['content'] .= "\n";
    $template[0]['content'] .= "와인 복합문화공간으로서 도운에서 특별하고 새로운";
    $template[0]['content'] .= "\n";
    $template[0]['content'] .= "와인 문화를 경험하실 수 있기를 기대합니다.";

    $template[0]['btn_name'] = "홈페이지 바로가기";
    $template[0]['btn_type'] = "WL";

    $template[0]['btn_url'] = "http://dreamforone.kr/~naracelllar";
    $template[0]['btn_url'] = str_replace(":443","",$template[0]['btn_url']);

    //템플릿 2번 - 결제완료
    $template[1]['code'] = "SJB_075940";
    $template[1]['title'] = "신청 완료안내2";
    $template[1]['content'] = "[나라셀라 도운]\n";
    $template[1]['content'] .= "안녕하세요. #{mb_name}님!\n";
    $template[1]['content'] .= "\n";
    $template[1]['content'] .= "결제 완료 되었습니다.\n";
    $template[1]['content'] .= "강의 상세 안내는 별도 문자 공지 예정입니다.\n";
    $template[1]['content'] .= "\n";
    $template[1]['content'] .= "감사합니다.";

    $template[1]['btn_name'] = "예약 내역 조회";
    $template[1]['btn_type'] = "WL";

    $template[1]['btn_url'] = "http://dreamforone.kr/~naracelllar/mypage.php?tab=#{tab}";
    $template[1]['btn_url'] = str_replace(":443","",$template[1]['btn_url']);

    //템플릿 3번 - 결제취소
    $template[2]['code'] = "SJB_075941";
    $template[2]['title'] = "신청 취소안내";
    $template[2]['content'] = "[나라셀라 도운]\n";
    $template[2]['content'] .= "안녕하세요. #{mb_name}님!\n";
    $template[2]['content'] .= "\n";
    $template[2]['content'] .= "취소 신청 되었습니다.";
    $template[2]['content'] .= "카드사 영업일 기준 3~5일 소요될 수 있습니다.\n";
    $template[2]['content'] .= "\n";
    $template[2]['content'] .= "감사합니다.";

    $template[2]['btn_name'] = "예약 내역 조회";
    $template[2]['btn_type'] = "WL";

    $template[2]['btn_url'] = "http://dreamforone.kr/~naracelllar/mypage.php?tab=#{tab}";
    $template[2]['btn_url'] = str_replace(":443","",$template[2]['btn_url']);


    // 변수 치환 - content, url
    foreach ($rep_field AS $key=>$field) {
        //echo "변수치환 실행";
        //echo $key;
        //echo "\n";
        ...
        if($key == 'mb_name'){
            ...
            //echo $val;
            //echo "\n";
            $template[$type]['content'] = str_replace(urldecode($field), $val, $template[$type]['content']);
        }else if($key == 'tab'){ //tab
            //echo "tab변경";
            //echo "\n";
            //echo "val확인";
            //echo $val;
            //echo "\n";
            $template[$type]['btn_url'] = str_replace(urldecode($field), $val, $template[$type]['btn_url']);
        }
        //echo "<br>";
        //echo "템플릿 확인";
        //echo "<br>";
        ...
        //$template[$type]['btn_url'] = str_replace($field, $val, $template[$type]['btn_url']);
    }

    $send_data = $template[$type];
    //echo nl2br(print_r($send_data, true));

    // 발송데이터 전문생성
    $post_body = array();
    $post_body['plusFriendId'] = WIDESHOT_PULSFRIEND;			//플러스친구
    $post_body['senderKey'] = WIDESHOT_CH_KEY;					//발신프로필키
    $post_body['templateCode'] = $send_data['code'];			//템플릿 코드
    $post_body['Title'] = $send_data['title'];					//템플릿 제목
    $post_body['contents'] = $send_data['content'];				//템플릿 내용
    $post_body['receiverTelNo'] = $mb_hp;						//수신자전화번호
    $post_body['userKey'] = getRandomString(8);			//발송코드 (response sendcode로 수신)
    $post_body['attachment'] = "";								//버튼정보

    if ($send_data['btn_name'] != "") {
        $btn = array();
        $btn['name'] = $send_data['btn_name'];
        $btn['type'] = $send_data['btn_type'];
        $btn['url_mobile'] = $send_data['btn_url'];
        $btn['url_pc'] = $send_data['btn_url'];

        $tmp_arr['attachment']['button'][] = $btn;
        $post_body['attachment'] = json_encode($tmp_arr, JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES|JSON_PRETTY_PRINT);
    }
    //print_r($post_body);

    $api_server = WIDESHOT_SERVER."/api/v1/message/alimtalk";
    $headers = array('sejongApiKey: '.WIDESHOT_KEY);
    //$post_json = curlArrToStr($post_body);
    //print_r($post_body);
    ...
    $opts = array(
        ...
        CURLOPT_POST => true,
        CURLOPT_POSTFIELDS => http_build_query($post_body),	//$post_json,
    ...
    if ($resMessage != null) {
        ...
        $res_data['sendCode'] = $body_de['sendCode'];
