...
$sql_search = " where mb_id!='lets080' and mb_memo not like '%삭제함%' ";
...
<select name="sfl" id="sfl">
    ...
    <option value="mb_addr"<?php echo get_selected($_GET['sfl'], "mb_addr"); ?>>주소</option>
    <option value="mb_addr2"<?php echo get_selected($_GET['sfl'], "mb_addr2"); ?>>상세주소</option>
	<? /*
...
<!-- 		<td><?=$row['mb_hp']?></td> -->
		...
		<td>
            <? //echo get_annualfee($mb_id); ?>

            <select name="slc_entrypay" id="<?=$row['mb_no']?>">
                    <option value=0 <?if($row['mb_1'] ==0) echo 'selected';?>>미납부</option>
                    <option value=1 <?if($row['mb_1'] ==1) echo 'selected';?>>납부</option>
            </select>
...
<script>
	...
	$("select[name='slc_entrypay']").change(function(){

			var value = $(this).val();
			var id = $(this).attr("id");

		$.ajax({
				url: "./ajax.chg_paystate.php",
				type: "POST",
				data: {
						"value" : value,
						"id" : id,
