...
else if ($w == 'u')
{
  ...
  /*  $sql = " select mb_id, mb_name, mb_nick, mb_email from {$g5['member_table']} where mb_nick = '{$_POST['mb_nick']}' and mb_id <> '$mb_id' ";
	...
	if(!empty($_POST['annual_value']) || !empty($_POST['annual_date'])){

		$now = date("Y-m-d H:i:s");
		
		$sql ="select count(*) as cnt from g5_member_annualfee where pay_year = '{$_POST['annual_year']}' and mb_id = '{$mb_id}' ";
		$cnt_year = sql_fetch($sql);
		if($cnt_year['cnt']>0){
					$sql="update g5_member_annualfee set  
									pay_year = '{$_POST['annual_year']}', 
									pay_date = '{$_POST['annual_date']}', 
									pay_value = '{$_POST['annual_value']}', 
									pay_flg = '{$_POST['annual_flg']}', 
									pay_text = '{$_POST['annual_text']}', 
									wr_datetime = '{$now}' 
									where mb_id = '{$mb_id}' and pay_year = '{$_POST['annual_year']}'";

					sql_query($sql);
		}
		else{
					$sql="insert into g5_member_annualfee set 
					mb_id = '{$mb_id}', 
					pay_year = '{$_POST['annual_year']}', 
					pay_date = '{$_POST['annual_date']}' , 
					pay_value = '{$_POST['annual_value']}' , 
