...
function mailer($fname, $fmail, $to, $subject, $content, $type=0, $file="", $cc="", $bcc="")
{
  ...
  //  if (!$config['cf_email_use']) return;
	...
	if ($fname == "") $fname = $config['cf_admin_email_name'];
	if ($fmail == "") $fmail = $config['cf_admin_email'];

	$user_name_arr = explode("@", $fmail);
	$user_name = $user_name_arr[0];
	$user_password = "dlatlqlqjs";
	...
	$mail->Port = 465; 
	$mail->Username = "koreanfiguretherapy@naver.com"; 
...
//itforoneMailer( 'tabassum@gg56.world','fingerate', $reg_mb_email, $reg_mb_email, $subject, $content);
// 파일을 첨부함
