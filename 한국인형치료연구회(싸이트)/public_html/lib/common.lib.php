...
function get_annualfee($mb_id){
    $year = date('Y');
    $cnt = sql_fetch(" SELECT COUNT(*) AS cnt FROM g5_member_annualfee WHERE mb_id = '{$mb_id}' AND pay_year = '{$year}' AND pay_flg = '납부' ")['cnt'];
    $str = "";
    if($cnt > 0){
        $str = "납부";
    }else{
...
// 신청현황 게시물 파일 불러오기
function get_wr_file($table,$wr_id){
    $sql = " SELECT wr_6 FROM g5_write_{$table} WHERE wr_id = '{$wr_id}' ";
    return sql_fetch($sql)['wr_6'];
}
