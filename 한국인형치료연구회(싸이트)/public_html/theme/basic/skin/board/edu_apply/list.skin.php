...
<div id="bo_list" class="apply" style="width:<?php echo $width; ?>">
    ...
    <div class="tbl_head01 tbl_wrap">
        ...
        for ($i=0; $i<count($list); $i++) {
				...
				$sql = "select * from g5_write_edu where wr_id = {$list[$i]['wr_4']}";
         ...
         ?>
        <tr style="cursor:pointer;" onClick="location.href='<?php echo G5_BBS_URL?>/board.php?bo_table=edu&wr_id=<?=$list[$i]['wr_4']?>'" class="<?php if ($list[$i]['is_notice']) echo "bo_notice"; ?>">
            <td class="td_number">
            <?php
...
<!-- http://itforone.co.kr/~kdoll2/bbs/board.php?bo_table=edu&wr_id=1 -->
               <!-- <a href="<?php echo G5_BBS_URL?>/board.php?bo_table=edu&wr_id=<?=$list[$i]['wr_4']?>">-->
                     <?=$info_wr['wr_subject']?>
                <!--</a>-->
...
<!-- 	<td class="td_status"><span class="cancle">환불</span></td> -->
	...
	<!-- 	<tr class="<?php if ($list[$i]['is_notice']) echo "bo_notice"; ?>">
        ...
        <?php } ?>
        <?php if ($i == 0 || ($i-$count_continue) == 0 ) { echo '<tr><td colspan="'.$colspan.'" class="empty_table">게시물이 없습니다.</td></tr>'; } ?>
        </tbody>
        </table>
    </div>
