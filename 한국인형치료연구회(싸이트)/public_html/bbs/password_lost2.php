...
$sql = " select mb_no, mb_id, mb_name, mb_nick, mb_datetime from {$g5['member_table']} where mb_email='{$mb_email}' ";
$mb = sql_fetch($sql);
if (!$mb['mb_id'])
    alert('존재하지 않는 회원입니다.');
...
$content .= '</div>';
    ...
    //   kaft1@naver.com
	$fmail = "koreanfiguretherapy@naver.com";
	$to = $_POST['mb_email'];
	//$subject = "테스트메일입니다.";
	//$content = "테스트메일입니다.";

	$temp = mailer("한국인형치료연구회",$fmail,$to, $subject, $content, 1,"");	

//itforoneMailer( $config['cf_admin_email'],$config['cf_admin_email_name'], $_POST['mb_email'],$mb['mb_name'], $subject, $content);
