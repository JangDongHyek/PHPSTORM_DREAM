...
for ($i=0; $row=sql_fetch_array($result); $i++) {
    if ($i == 0) {
        if ($this->css) {
            ...
            echo "<ul id=\"sct_wrap\" class=\"sct sct_10 sct_wrap2\">\n";
        }
    }
	...
	if (0 < $row['it_cust_price']) {
		$percentage=round(100-($row[it_price]/$row[it_cust_price]*100),0);
		
	?>
	
	<div id="pro_sale">SAVE<br /><?=$percentage?>%</div><!--상품 할인률-->
	<?
	}
    if ($this->href) {
