...
if($is_guest)
    alert('회원이시라면 로그인 후 이용해 보십시오.', '../bbs/login.php?url='.urlencode(G5_SHOP_URL.'/qaview.php?qa_id='.$qa_id));
...
if(is_file($skin_file)) {
    ...
    if (isset($prev['qa_id']) && $prev['qa_id']) {
        ...
        $prev_href = G5_SHOP_URL.'/qaview.php?qa_id='.$prev['qa_id'].$qstr;
    }
    ...
    if (isset($next['qa_id']) && $next['qa_id']) {
        ...
        $next_href = G5_SHOP_URL.'/qaview.php?qa_id='.$next['qa_id'].$qstr;
    }
    ...
    for($i=0; $row=sql_fetch_array($result); $i++) {
        ...
        $rel_list[$i]['view_href'] = G5_SHOP_URL.'/qaview.php?qa_id='.$row['qa_id'].$qstr;
        $rel_count++;
    }
    ...
    $update_href = '';
    $delete_href = '';
    $write_href = G5_SHOP_URL.'/qawrite.php';
    $rewrite_href = G5_SHOP_URL.'/qawrite.php?w=r&amp;qa_id='.$view['qa_id'];
    $list_href = G5_SHOP_URL.'/qalist.php'.preg_replace('/^&amp;/', '?', $qstr);

    /*
    if($view['qa_type']) {
        if($is_admin)
            $update_href = G5_SHOP_URL.'/qawrite.php?w=u&amp;qa_id='.$view['qa_id'].$qstr;
    } else {
        if($view['qa_status'] == 0)
            $update_href = G5_SHOP_URL.'/qawrite.php?w=u&amp;qa_id='.$view['qa_id'].$qstr;
    }
    */
    if(($view['qa_type'] && $is_admin) || (!$view['qa_type'] && $view['qa_status'] == 0)) {
        $update_href = G5_SHOP_URL.'/qawrite.php?w=u&amp;qa_id='.$view['qa_id'].$qstr;
        $delete_href = G5_SHOP_URL.'/qadelete.php?qa_id='.$view['qa_id'].$qstr;
    }
    ...
    if(!$view['qa_type'] && $view['qa_status']) {
        ...
        if($is_admin) {
            $answer_update_href = G5_SHOP_URL.'/qawrite.php?w=u&amp;qa_id='.$answer['qa_id'].$qstr;
            $answer_delete_href = G5_SHOP_URL.'/qadelete.php?qa_id='.$answer['qa_id'].$qstr;
        }
    }
    ...
    for ($i=1; $i<=2; $i++) {
        if(preg_match("/\.({$config['cf_image_extension']})$/i", $view['qa_file'.$i])) {
            $view['img_file'][] = '<a href="'.G5_SHOP_URL.'/view_image.php?fn='.urlencode('/data/qa/'.$view['qa_file'.$i]).'" target="_blank" class="view_image"><img src="'.G5_DATA_URL.'/qa/'.$view['qa_file'.$i].'"></a>';
            $view['img_count']++;
            continue;
        }

        if ($view['qa_file'.$i]) {
            $view['download_href'][] = G5_SHOP_URL.'/qadownload.php?qa_id='.$view['qa_id'].'&amp;no='.$i;
