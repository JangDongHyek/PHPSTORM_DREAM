...
if(!sql_query(" select co_skin from {$g5['content_table']} limit 1 ", false)) {
    ...
    sql_query(" update {$g5['content_table']} set co_skin = 'theme/basic', co_mobile_skin = 'theme/basic' ");
}
...
else
{
    ...
    $co['co_mobile_skin'] = 'theme/basic';
}

include_once (G5_ADMIN_PATH.'/admin.head.php');
?>

<form name="frmcontentform" id="frmcontentform" action="./contentformupdate.php" onsubmit="return frmcontentform_check(this);" method="post" enctype="MULTIPART/FORM-DATA" >
...
<div class="tbl_frm01 tbl_wrap">
    ...
    <tr>
        ...
        <td>
            ...
            <?php if ($w == 'u') { ?>
			<a href="<?php echo G5_BBS_URL; ?>/content.php?co_id=<?php echo $co_id; ?>" class="btn_frmline">내용확인</a>
			<input type="button" value="내용저장" class="btn_frmline" accesskey="s" onclick="contentSave()" style="background:#ff3061; cursor:pointer;">
			<?php } ?>
        </td>
    </tr>
    <tr>
...
function contentSave(){
	...
	if(confirm("내용을 저장하시겠습니까?")==true){
		var formData = $("#frmcontentform").serialize();
		$.ajax({
			type:"POST",
			url:"<?php echo G5_ADMIN_URL?>/ajax.contentsave_update.php",
			data: formData,
			dataType:"html",
