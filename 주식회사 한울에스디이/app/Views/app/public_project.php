<!--시행사 : 회사 공개 프로젝트-->
<div class="flex ai-c">
    <p class="txt-bold txt-darkblue">
        <span>현재 진행중인 프로젝트 8건</span>
        <span>완료된 프로젝트 25건</span>
    </p>
    <!--관리자는 전체 / 직원은 본인 프로젝트만!-->
</div>
</div>

<section class="list_table">
    <div class="area_filter flex ai-c jc-sb">
        <div class="flex ai-c">
            <strong class="total">총 25건</strong>
            <input type="date" name="sdt" placeholder="날짜 선택" value="">
            ~
            <input type="date" name="edt" placeholder="날짜 선택" value="">
            <div class="search">
                <select name="sfl">
                    <option value="">프로젝트 명</option>
                    <option value="">시공사 명</option>
                </select>
                <input type="search" name="stx" placeholder="검색어 입력" value="">
                <button type="submit" class="btn-search"><i class="fa-regular fa-magnifying-glass"></i></button>
            </div>
        </div>
        <div class="flex ai-c">
            <span class="i_green">원활</span>
            <span class="i_orange">주의</span>
            <span class="i_red">중단</span>
            <span class="i_black">완료</span>
        </div>
    </div>
    <div class="table">
        <table>
            <colgroup>
                <col width="20px">
                <col width="auto">
                <col width="auto">
                <col width="auto">
                <col width="auto">
                <col width="auto">
                <col width="auto">
                <col width="auto">
            </colgroup>
            <thead>
            <tr>
                <th></th>
                <th>프로젝트 명</th>
                <th>공사 위치</th>
                <th>공사 기간</th>
                <th class="text-center">담당자</th>
                <th class="text-center">시공사명</th>
                <th class="text-center">비용 예산(억원)</th>
                <th class="text-center">소요 비용(억원)</th>
                <th class="text-center">진행율</th>
            </tr>
            </thead>
            <tbody>
            <tr>
                <th>25</th>
                <th><p class="i_green">블루워터 프라자 리모델링</p></th>
                <td>당진 수청지구 도시개발사업 부지</td>
                <td>2018. 6. 18 - 2020. 11. 30 (총 897일)</td>
                <td class="text-center">김드림</td>
                <td class="text-center">대우건설</td>
                <td class="text-center">5</td>
                <td class="text-center">3</td>
                <td class="text-center">40%</td>
            </tr>
            <tr>
                <th>24</th>
                <th><p class="i_green">하늘 마을 주택 단지</p></th>
                <td>서울 강남구 역삼동 상업지구</td>
                <td>2018. 6. 18 - 2020. 11. 30 (총 897일)</td>
                <td class="text-center">한서준</td>
                <td class="text-center">현대건슬</td>
                <td class="text-center">1.5</td>
                <td class="text-center">0.8</td>
                <td class="text-center">10%</td>
            </tr>
            <tr>
                <th>23</th>
                <th><p class="i_orange">주거단지 개발</p></th>
                <td>경기도 수원시 영동구 공공기관 건물</td>
                <td>2018. 6. 18 - 2020. 11. 30 (총 897일)</td>
                <td class="text-center">박세현</td>
                <td class="text-center">한화건설</td>
                <td class="text-center">5</td>
                <td class="text-center">3</td>
                <td class="text-center">40%</td>
            </tr>
            <tr>
                <th>22</th>
                <th><p class="i_red">상업시설 리모델링</p></th>
                <td>인천 서구 길산동 주택 건설 부지</td>
                <td>2018. 6. 18 - 2020. 11. 30 (총 897일)</td>
                <td class="text-center">류선재</td>
                <td class="text-center">GS건설</td>
                <td class="text-center">1.5</td>
                <td class="text-center">0.8</td>
                <td class="text-center">10%</td>
            </tr>
            <tr>
                <th>21</th>
                <th><p class="i_black">상업시설 리모델링</p></th>
                <td>인천 서구 길산동 주택 건설 부지</td>
                <td>2018. 6. 18 - 2020. 11. 30 (총 897일)</td>
                <td class="text-center">류선재</td>
                <td class="text-center">GS건설</td>
                <td class="text-center">1.5</td>
                <td class="text-center">0.8</td>
                <td class="text-center">10%</td>
            </tr>
            </tbody>
        </table>
    </div>
    <div class="paging">
        <div class="pagingWrap">
            <a class="first disabled"><i class="fa-light fa-chevrons-left"></i></a>
            <a class="prev disabled"><i class="fa-light fa-chevron-left"></i></a>
            <a class="active">1</a>
            <a>2</a>
            <a>3</a>
            <a>4</a>
            <a>5</a>
            <a>6</a>
            <a>7</a>
            <a class="next disabled"><i class="fa-light fa-chevron-right"></i></a>
            <a class="last disabled"><i class="fa-light fa-chevrons-right"></i></a>
        </div>
    </div>


</section>

