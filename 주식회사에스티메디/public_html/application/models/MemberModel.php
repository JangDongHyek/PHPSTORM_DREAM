...
class MemberModel extends CI_Model
{
    ...
    public function getMemberList($param = array(), $orderBy = ""): array
    {
        // 공통쿼리
        if ($param['mb_level']) {
            $sqlCommon = "FROM bs_member WHERE  mb_level = {$param['mb_level']} ";
        } else {
            $sqlCommon = "FROM bs_member WHERE  mb_level < 6 ";
        }

        if (!empty($param['del_yn'])){
            $sqlCommon .= " AND del_yn = '{$param['del_yn']}'";
        }else{
            $sqlCommon .= " AND del_yn = 'N'";
        }
        ...
        if ($param['stx'] != "") {
            switch ($param['sfl']) {
                ...
                case "brno" : // 사업자등록번호
                    ...
                    break;
                case "biz_rno" : // 사업자등록번호
                    $sqlCommon .= " AND biz_rno LIKE '%{$param['stx']}%'";
                    break;
                case "rep_name" : // rep_name
                    $sqlCommon .= " AND rep_name LIKE '%{$param['stx']}%'";
                    break;
                case "tel" : // 대표전화
                ...
                case "cn_tel" : // 대표전화
                    ...
                    break;
                case "addr" : // 주소
                    $sqlCommon .= " AND ( cn_addr LIKE '%{$param['stx']}%' or cn_addr_detail LIKE '%{$param['stx']}%' )";
                    break;
                case "agency" : // 에이전시
                    $sqlCommon .= " AND ( mb_name LIKE '%{$param['stx']}%' or mb_id LIKE '%{$param['stx']}%' or cn_name LIKE '%{$param['stx']}%' )";
                    break;
            }
        }
        // 그룹 선택시
        ...
        if (!empty($param['agency_div'])) $sqlCommon .= " AND agency_div = '{$param['agency_div']}'";
        if (!empty($param['agency'])) $sqlCommon .= " AND agency = '{$param['agency']}'";
        if (!empty($param['agencyOnly'])) $sqlCommon .= " AND agency <> ''";

        $page_setting = 20;
        if (!empty($param['page_setting'])) $page_setting = $param['page_setting'];
        ...
        $paging = getPaging($param['page'], $totalCount, $page_setting);

        //log_message('error', "sqlCommon :" . $sqlCommon);
        ...
        if ($orderBy == "agency") $sqlOrderBy = "ORDER BY reg_date DESC"; // 에이전시
    ...
    // 회원 에이전시 업데이트
    public function updateAgencyMember($agency = '', $idxArr = array()): bool
    {
        try {
            $sql = "UPDATE bs_member SET agency = ? WHERE idx IN ?";
            return $this->db->query($sql, [$agency, $idxArr]);
            // log_message('debug', $this->db->last_query());

        } catch (\Exception $e) {
            log_message('error', "회원 에이전시 업데이트 실패" . $e->getMessage());
            return false;
        }
    }

    // 회원 보험가적용 업데이트
    public function updateINSUMember($INSU_CHECK = '', $idxArr = array()): bool
    {
        try {
            $sql = "UPDATE bs_member SET INSU_CHECK = ? WHERE idx IN ?";
            return $this->db->query($sql, [$INSU_CHECK, $idxArr]);
            // log_message('debug', $this->db->last_query());

        } catch (\Exception $e) {
            log_message('error', "회원 보험가적용 업데이트 실패" . $e->getMessage());
            return false;
        }
    }
    ...
    // 회원 삭제
    public function deleteMember($del_yn = 'Y', $idxArr = array()): bool
    {
        try {
            $sql = "UPDATE bs_member SET del_yn = ? WHERE idx IN ?";
            return $this->db->query($sql, [$del_yn, $idxArr]);
            // log_message('debug', $this->db->last_query());

        } catch (\Exception $e) {
            log_message('error', "회원 삭제 업데이트 실패" . $e->getMessage());
            return false;
        }
    }
