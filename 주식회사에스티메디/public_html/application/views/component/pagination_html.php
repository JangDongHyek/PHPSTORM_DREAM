...
<div class="paging">
    <div class="pagingWrap">
        ...
        <?php if ($paging['page'] > 1 && $paging['totalPage'] > 0) { ?>
            <a class="first disabled" onclick="callContent(1,'<?=$qstr?>')"><i class="fa-light fa-chevrons-left"></i></a>
        <?php } ?>

        <!--이전-->
        <?php if ($paging['currentBlock'] > 1) { ?>
            <a class="prev disabled" onclick="callContent(<?=$paging['startPage']-1?>,'<?=$qstr?>')"><i class="fa-light fa-chevron-left"></i></a>
        <?php } ?>
        ...
        if ($paging['totalPage'] != 0) {
            foreach (range(1, $paging['totalPage']) as $number) {
                if ($number >= $paging['startPage'] && $number <= $paging['endPage']) {
                    ...
                    ?>
                    <a class="<?=($paging['page'] == $number) ? 'active' : ''?>" onclick="callContent(<?=$number?>,'<?=$qstr?>')"><?=$number?></a>
                <?php } } } ?>

        <!--다음-->
        <?php if ($paging['totalBlock'] > 1 && $paging['totalBlock'] != $paging['currentBlock']) { ?>
            <a class="next disabled" onclick="callContent(<?=$paging['endPage']+1?>,'<?=$qstr?>')" ><i class="fa-light fa-chevron-right"></i></a>
        <?php } ?>

        <!--마지막-->
        <?php if ($paging['page'] < $paging['totalPage']) { ?>
            <a class="last disabled" onclick="callContent(<?=$paging['totalPage']+1?>,'<?=$qstr?>')"><i class="fa-light fa-chevrons-right"></i></a>
        <?php } ?>
    </div>
</div>
