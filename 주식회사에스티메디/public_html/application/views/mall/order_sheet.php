                        ...
                        <div class="details">
                            <div class="cart_list">
                                <ul>
                                    ...
                                    $agency_fee2_totalPrice = 0; // 에이전시 수수료 총액
                                    ...
                                    foreach ($agencyData as $item) {
                                        if (isset($item['listData'][0]['idx'])) {
                                            $agencyFeeList[$item['listData'][0]['product_idx']] = $item['listData'][0]['fee']; // idx 값 추출
                                        }
                                    }
                                    ...
                                    foreach ($listData as $list) {
                                        ...
                                        $list['agency_fee'] = $agencyFeeList[$list['product_idx']];
                                        if($list['agency_fee']){
                                            $agency_fee2_totalPrice += $itemAmt * ( $list['agency_fee'] / 100 );
                                            //echo $itemAmt * ( $list['agency_fee'] / 100 );
                                        }

                                        $list['pay_method_list'] = 'CARD,CASH';
                                        $tmpPayList = explode(",", $list['pay_method_list']); // 결제수단 추가
                                        $tmpPayList = array_filter($tmpPayList); // 공백제거
                                        ...
                                        <li>
                                            <div class="div_pro">
                                                ...
                                                <div class="div_name">
                                                    <h5><i class="fa-duotone fa-solid fa-pills"></i><strong><?=$list['PRODUCT_NM']?></strong></h5>
                                            ...
                                            <input type="hidden" name="productName[<?=$uid?>]" value="<?=$list['PRODUCT_NM']?>"/>
                                            <input type="hidden" name="productPrice[<?=$uid?>]" value="<?=$price?>"/>
                                            <input type="hidden" name="productCnt[<?=$uid?>]" value="<?=$count?>"/>
                                            <input type="hidden" name="agency_fee[<?=$uid?>]" value="<?=$list['agency_fee']?>"/>
                                            <? // 장바구니 인덱스 ?>
            ...
            <div class="sticky">
                <div class="payment">
                    ...
                    <div class="areatop">
                        ...
                        <p class="circle">
                            <?php
                            // 결제수단 상품에 결제수단안해서 일단이걸로 대기

                            foreach ($paymentList AS $payType=>$payName) {
                                ?>
                                <input type="radio" id="pay<?=$key?>" name="payMethod" value="<?=$payType?>" <?=$key==0?"checked":"";?> onclick="changePayMethod(this.value)"/>
                        ...
                        <!--
                        <p class="circle">
                            <input type="radio" id="pay2" name="payMethod" value="CARD" onclick="changePayMethod(this.value)" checked>
                            <label for="pay2"> 카드결제</label>
                        </p>
                        -->
                        <div class="box" id="cash_info_wrap" style="display: none;">
                            <h5>발행목록</h5>
                            <p class="circle">
                                <input type="radio" id="is1" name="cashIssueType" value="1" onclick="changeIssueType(this.value)"> <label for="is1"> 계산서</label>
                                <input type="radio" id="is2" name="cashIssueType" value="2" onclick="changeIssueType(this.value)"> <label for="is2"> 현금영수증</label>
                                <input type="radio" id="is3" name="cashIssueType" value="3" onclick="changeIssueType(this.value)" checked=""> <label for="is3"> 미발행</label>
                            </p>
                            <!--계산서-->
                            <div class="issue" id="issue_tab1" style="display: none;">
                                <div class="account">
                                    <dl>
                                        <dt>입금계좌</dt>
                ...
                <div class="total">
                    ...
                    <input type="hidden" name="agency_fee2" value="<?=$agency_fee2_totalPrice?>"/><!--에이전시 수수료금액-->
                    ...
                    <div class="total_price">
                        ...
                        <p class="price txt_sky" id="totalPrice_txt"><?=number_format($totalPrice)?>원</p>
                    </div>
                    <div class="btn_wrap">
                        <?php if($member['mb_id'] == 'stmedi'){ ?>

                            <a class="btn btn_middle btn_blue">결제불가 계정</a>

                        <?php }else{ ?>
                            <a class="btn btn_middle btn_blue" onclick="productOrderSubmit()">결제하기</a>
                        <?php } ?>
                    </div>
