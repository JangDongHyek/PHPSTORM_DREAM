    ...
    <div class="vertical_wrap">
                                        ...
                                        <div class="div_option">
                                            <div>
                                                <dl>
                                                    <dt><!--기본옵션--></dt>
                                                    <dd>
                                                        <div class="number"><?=number_format(removeComma($count))?>개</div>
                                                        <p class="p_price"><?=number_format(removeComma($price))?>원</p>
                                                    </dd>
                                                </dl>
                                            </div>
                                        </div>
                                        <p class="p_price2"><?=number_format(removeComma($itemAmt))?>원</p>
                                    </li>
                                <? } ?>
                            </ul>
                        </div>
        ...
        <div class="sticky">
            ...
            <div class="total">
                <dl class="price_info">
                    <dt>총 상품금액</dt>
                    <dd><?=number_format(removeComma($orderData['subtotal_price']))?>원</dd>
                    <dt>배송비</dt>
                    <dd><i class="fa-solid fa-plus-circle"></i> <?=number_format(removeComma($orderData['delivery_fee']))?>원</dd>

                    <? if($orderData['delivery_fee2']){ ?>
                        <dt>추가 배송비</dt>
                        <dd><i class="fa-solid fa-plus-circle"></i> <?=number_format(removeComma($orderData['delivery_fee2']))?>원</dd>
                    <? } ?>
                </dl>
                <div class="total_price">
                    <p class="text">총 결제금액</p>
                    <p class="price txt_blue"><?=number_format(removeComma($orderData['total_price']))?>원</p>
                </div>
                <div class="btn_wrap">
                    <a class="btn btn_middle btn_blue" href="<?=PROJECT_URL?>/order">목록으로</a>
                </div>
            </div>
        </div>
