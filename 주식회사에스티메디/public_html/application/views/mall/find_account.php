					...
					<div role="tabpanel">
						...
						<div class="tab-content">
							<div role="tabpanel" class="tab-pane active" id="find_id">
								...
								<div class="btn_wrap">
									<button type="button" id="btnFindId" class="btn btn_middle btn_blueline">아이디 찾기</button>
								</div>
							</div>
							<div role="tabpanel" class="tab-pane" id="find_pw">
								...
								<div class="result"><!--찾기결과--></div>
								<div class="btn_wrap">
									<button type="button" id="btnFindPw" class="btn btn_middle btn_blueline">비밀번호 찾기</button>
								</div>
							</div>
						</div>
					</div>
...
<script>
	...
	document.querySelector('#btnFindId').addEventListener('click', async (e) => {
		...
		if (response.result.length == 0) {
			html = `<div class="txt_blue">입력하신 정보와 일치하는 아이디가 없습니다.</div>`;
		} else {
			...
			list.forEach(data => {
				html += `
					<div style="margin: 7px 0;font-size:15px;">
						<strong class="txt_blue">${data.mb_id}</strong> (가입일: ${data.reg_date})
					</div>`;
			});
		}
	...
	document.querySelector('#btnFindPw').addEventListener('click', async (e) => {
		...
		if (response.result) {
			...
			html += `<div>회원님의 <span class="txt_blue">이메일로 임시비밀번호를 발송</span>하였습니다.`;
		} else {
			html = `<div class="txt_blue">입력하신 정보와 일치하는 회원정보가 없습니다.</div>`;
		}
		resultWrap.innerHTML = html;
	});
