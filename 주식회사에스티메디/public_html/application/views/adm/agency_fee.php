    ...
    <form name="searchFrm" autocomplete="off">
        <div class="panel">
            ...
            <div>
                <select name="sfl">
                    ...
                    $sflList = ['oName' => '주문자명', 'ordNo'=>'주문번호', 'rId' => '주문자아이디', 'item' => '상품명'];
        ...
        <div class="box">
            <div class="tagbox">
                ...
                <div>
                    <p><strong>주문상태</strong></p>
                </div>

                <div>
                    <select name="status" onchange="searchFilter('status',this.value)">
        ...
        <div class="table adm">
            <table>
                ...
                <tr class="total">
                    <td></td>
                    <td></td>
                    <td><?=number_format($totalCnt)?>건</td>
                    <td>
                        <?php
                        if ($_GET['year']) {
                            echo $_GET['year'].'년';
...
<script>
    ...
    const set_agency_members = async () => {
        ...
        if(response.listData){
            ...
            for (var i=0 ; i < list_data.length ; i++){
                ...
                if(now_agency == list_data[i]['mb_id']){
                    html += '<p><a><span class="tag active" onclick="searchFilter(\'agency\',\'' + list_data[i]['mb_id'] + '\');searchFilter(\'mb_id\',\'\')">'+list_data[i]['mb_id']+'</span></a></p>';
                }else{
                    html += '<p><a><span class="tag" onclick="searchFilter(\'agency\',\'' + list_data[i]['mb_id'] + '\');searchFilter(\'mb_id\',\'\')">'+list_data[i]['mb_id']+'</span></a></p>';
                }
        ...
        <?php if(!$_GET['agency']){ ?>
            location.replace(`${baseUrl}adm/agencyFee?agency=` + now_agency);
        <?php } ?>
