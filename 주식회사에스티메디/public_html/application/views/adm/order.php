...
<section class="order1">
        ...
        <div class="box">
            <div class="tagbox">
                ...
                <dl class="search">
					...
					<dd>
                    <span class="select">
                         <input type="checkbox" id="UPDATE_KEY_CHECK" name="UPDATE_KEY" class="green" value="true" <?=$_GET['UPDATE_KEY'] ? 'checked' : ''?> onclick="this.form.submit()"/><label for="UPDATE_KEY_CHECK">없음</label>
                    </span>
					</dd>
                    <!--<dt>카테고리</dt>
                    <dd>
    ...
    <div class="boxline caption">
        <dl>
            <dt>※ 주문연동</dt>
            <dd>주문연동 완료시 <span class="icon gr">주문코드</span>으로 변경됨</dd>
            <dd>주문연동 실패시 <span class="icon or">재전송</span>으로 변경됨</dd>
        </dl>
    </div>
            ...
            <table class="order">
                ...
                foreach($listData as $list) {
                    ...
                    -->
                        ...
                        <td>
                            <?php if(strpos($list['mb_id'], 'test') !== false){ ?>
                                <button type="button" class="btn btn_green btn_mini">테스트 주문</button>
                            <?php } ?>
                            <?//주문번호?>
                            <span class="txt_sky txt_under txt_bold"><a href="<?=PROJECT_URL?>/adm/order/<?=$list['idx']?>"><?=$list['ord_no']?></a></span>
                        </td>
                        <td>
                            <dl class="order_info">
						...
						<td>
                            <?php if($list['UPDATE_KEY']){ ?>
                                <button type="button" class="btn btn_green btn_mini" onclick="postApiOrderCheck('<?=$list['UPDATE_KEY']?>')">주문코드</button>
                                <p onclick="postApiOrderCheck('<?=$list['UPDATE_KEY']?>')"><?=$list['UPDATE_KEY']?></p>
                            <?php }else{ ?>
                                <button type="button" class="btn btn_orange" onclick="postApiOrder('<?=$list['idx']?>')">재전송</button>
                            <?php } ?>
						</td>
                    </tr>
                    <tr>
...
<?php include_once MODAL_PATH. "api_order_check_modal.php" // api 주문확인 모달 ?>
...
<script>
    ...
    // api 주문정보 연동
    const postApiOrder = async (idx) => {

        if(!idx){
            showAlert(`주문번호가 없습니다.`);
            return false;
        }

        var response = await fetchData(`/api/postApiOrder/`, {idx});
    ...
    // api 주문정보 연동
    const postApiOrderCheck = async (UPDATE_KEY) => {

        const defModal = $('#apiOrderCheckModal');

        if(!UPDATE_KEY){
            showAlert(`주문 코드가 없습니다.`);
            return false;
        }

        var response = await fetchData(`/api/postApiOrderCheck/`, {UPDATE_KEY});
        console.log(response);
        ...
        if (response.ERRCODE == 'E0000') {
            ...
            for (var i=0 ; i < data.length ; i++){
                ...
                var PRODUCT_CD = data[i]['PRODUCT_SIMS_CODE'];
                var response2 = await fetchData(`/api/getProductInfoAPIPRODUCT_CD`, {PRODUCT_CD});
                console.log(response2);

                html += '<span class="icon w100 gr">' + data[i]['ORDER_NO'] + '</span><br>';
                html += '<p><span>서버 등록시간</span>' + data[i]['IF_SVR_WRITE_TIME'] + '</p>';
                html += '<p><span>심스연동여부</span>' + data[i]['SIMS_IF_YN'] + '</p>';
                html += '<p><span>심스연동시간</span>' + data[i]['SIMS_IF_TIME'] + '</p>';
                html += '<p><span>거래처코드(SIMS)</span>' + data[i]['CUSTOMER_SIMS_CODE'] + '</p>';
                html += '<p><span>제품명</span>' + response2.PRODUCT_NM + '</p>';
                html += '<p><span>제품코드(SIMS)</span>' + data[i]['PRODUCT_SIMS_CODE'] + '</p>';
                html += '<p><span>주문수량</span>' + data[i]['ORDER_QUANTITY'] + '</p>';
                html += '<p><span>제품단가</span>' + data[i]['PRODUCT_PRICE'] + '</p>';
                html += '<p><span>공급가액</span>' + data[i]['SUPPLY_PRICE'] + '</p>';
                html += '<p><span>세액</span>' + data[i]['TAX_PRICE'] + '</p>';
                html += '<br><br>';
            }

            if(html){
                defModal.modal();
                $('#OrderCheckModal_list').html(html);
            }
            console.log(data);

        } else {
            showAlert(`주문확인에 실패했습니다.`, () => {
                //location.reload();
            });
        }
    }
