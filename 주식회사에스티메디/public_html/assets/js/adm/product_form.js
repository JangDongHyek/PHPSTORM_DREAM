...
const noImageUrl = baseUrl + 'assets/img/common/mediimg.jpg';
...
productFrm.addEventListener('submit', async (e) => {
	...
	if (response.idx) {
		...
		location.href = baseUrl + 'adm/productForm/'+response.idx;
		// });
...
//제품코드 중복체크
const checkPRODUCT_CD = async () => {
	var PRODUCT_CD = $('#PRODUCT_CD').val();
	var response = await fetchData(`/apiAdmin/postPRODUCT_CD`, {PRODUCT_CD});
	//console.log(response);

	if(response.result){

		if(PRODUCT_CD == $('#PRODUCT_CD_reg').val()){
			$('#PRODUCT_CD_txt').html('');
		}else{
			$('#PRODUCT_CD_txt').html(response.msg);
		}
