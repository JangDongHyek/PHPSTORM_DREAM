...
$mb_gender          = isset($_POST['mb_gender'])        ? trim($_POST['mb_gender'])      : "";
$mb_school          = isset($_POST['mb_school'])        ? trim($_POST['mb_school'])      : "";
...
if ($w == '') {
    ...
    if (!isset($_POST['agree']) || !$_POST['agree']) {
        alert('회원가입약관의 내용에 동의하셔야 회원가입 하실 수 있습니다.', G5_BBS_URL.'/register_form.php');
    }

    if (!isset($_POST['agree2']) || !$_POST['agree2']) {
        alert('개인정보처리방침안내의 내용에 동의하셔야 회원가입 하실 수 있습니다.', G5_BBS_URL.'/register_form.php');
    }
    ...
    if ($config['cf_email_mb_super_admin']) {
        $subject = '['.$config['cf_title'].'] '.$mb_name .' 님께서 회원으로 가입하셨습니다.';
        ...
        mailer($mb_name, $mb_email, $config['cf_admin_email'], $subject, $content, 1);
    }

    // 메일인증 사용하지 않는 경우에만 로그인, 관리자 승인 후 로그인 되어야해서 주석 시킴
//    if (!$config['cf_use_email_certify'])
