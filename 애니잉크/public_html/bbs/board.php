...
if($bo_table == 'new' && $sch_wr_2 == '임대해지' && $member['mb_level'] < 9){
	alert('접근권한이 없습니다!', G5_URL);
}

if($bo_table == 'sell' && $member['mb_level'] < 9){
	alert('접근권한이 없습니다!', G5_URL);
}
...
if (isset($wr_id) && $wr_id) {
    ...
    if ($member['mb_level'] < $board['bo_list_level']) {
        if ($member['mb_id']){
			...
			goto_url('./login.php?'.$qstr.'&url='.urlencode(G5_BBS_URL.'/board.php?bo_table='.$bo_table.($qstr?'&amp;':'')));
		}
    }
