...
$new_file_path = G5_DATA_PATH.'/new_type/';
...
if ($wr_content == '' && $bo_table != 'new' && $bo_table != 'sell' && $bo_table != 'as') {
...
if($bo_table == 'as'){
	$add_qry = ", wr_4_text = '{$_POST['wr_4_text']}',wr_11 = '{$_POST['wr_11']}' ";
}
...
if ($w == '' || $w == 'r') {
	...
	if($bo_table == 'new'){
		...
		if(count($_POST['nt_cnt']) == 1 && $_POST['nt_date'][0] == '' && $_POST['nt_list'][0] == '' && $_POST['nt_model'][0] == ''){
			...
			if(count($_POST['nt_cnt']) > 0){
				// 최근점검일자
				$latest_check_date = 0;
				// 다음점검일자
				$next_check_date = 0;

				for($a=0; $a<count($_POST['nt_cnt']); $a++){
					$nt_sql = " insert into g5_write_new_type set 
					nt_wr_id = '{$wr_id}',
					nt_date = '{$_POST['nt_date'][$a]}',
					nt_list = '{$_POST['nt_list'][$a]}',
					nt_model = '{$_POST['nt_model'][$a]}',
					nt_page1 = '{$_POST['nt_page1'][$a]}',
					nt_page1_2 = '{$_POST['nt_page1_2'][$a]}',
					...
					";
					if(sql_query($nt_sql)){
						$nt_idx = sql_insert_id();
						
						if($_FILES['nt_file']['name'][$a] != ''){
							$nt_file_name = $nt_idx.'_'.urlencode(basename($_FILES['nt_file']['name'][$a]));
							$nt_upload = $new_file_path.$nt_file_name;
							if(move_uploaded_file($_FILES['nt_file']['tmp_name'][$a], $nt_upload)){
								$nt_up_sql = " update g5_write_new_type set nt_file='{$nt_file_name}' where nt_idx='{$nt_idx}' ";
								sql_query($nt_up_sql);
							}
						}

						// 최근점검일자 값이 설치일자보다 적으면 최근점검일자 값을 설치일자로 업데이트 해준다
						if($latest_check_date < str_replace('-','',$_POST['nt_date'][$a])){
							$latest_check_date = str_replace('-','',$_POST['nt_date'][$a]);
							
							/* 다음점검일자 등록하기 STR */
							$latest_date_arr = explode('-',$_POST['nt_date'][$a]);
							$latest_month = (int)$latest_date_arr[1];
							if($_POST['wr_16'] != ''){
								$next_month = $latest_month + str_replace('개월','',$_POST['wr_16']);
								if($next_month > 12){
								...
								$next_check_date = $plus_year.$next_month.$latest_date_arr[2];
							}
							/* 다음점검일자 등록하기 END */
						}
					}
				...
				// 최근점검일자 값을 업데이트 해준다
				if($latest_check_date > 0){
					$up_sql = " update $write_table set latest_check_date = '{$latest_check_date}' where wr_id='{$wr_id}' ";
					sql_query($up_sql);
				}

				// 다음점검일자 값을 업데이트 해준다
				if($next_check_date > 0){
					$up_sql = " update $write_table set next_check_date = '{$next_check_date}' where wr_id='{$wr_id}' ";
					sql_query($up_sql);
				}
			}
	...
	/* 최근점검일자 AND 다음점검일자 변경 STR */
	if($bo_table == 'as' && $_POST['wr_2'] == '정기점검'){
		$latest_sql = " select max(wr_3) as max_wr_3 from $write_table where wr_2='정기점검' and wr_1='{$_POST['wr_1']}' ";
		$latest_row = sql_fetch($latest_sql);
		$max_wr_3 = str_replace('-','',$latest_row['max_wr_3']);
		$max_wr_3_arr = explode('-',$latest_row['max_wr_3']);

		if($latest_row['max_wr_3'] != '' && $max_wr_3 > 0){
			$new_sql = " select wr_16 from g5_write_new where wr_id='{$_POST['wr_1']}' ";
			...
			if($new_row['wr_16'] != ''){
				...
				$up_sql = " update g5_write_new set next_check_date = '{$next_check_date}' where wr_id='{$_POST['wr_1']}' ";
				sql_query($up_sql);
			}

			$up_sql = " update g5_write_new set latest_check_date = '{$max_wr_3}' where wr_id='{$_POST['wr_1']}' ";
			sql_query($up_sql);
		}
	}
	/* 최근점검일자 변경 END */
	...
	if($bo_table == 'new'){
		...
		if(count($_POST['nt_cnt']) == 1 && $_POST['nt_date'][0] == '' && $_POST['nt_list'][0] == '' && $_POST['nt_model'][0] == ''){
			...
			$check_sql = " select nt_idx, nt_file from g5_write_new_type where nt_wr_id='{$wr_id}' ";
			$check_qry = sql_query($check_sql);
			$check_num = sql_num_rows($check_qry);
			for($cn=0; $cn<$check_num; $cn++){
				$check_row = sql_fetch_array($check_qry);
				if($check_row['nt_file'] != ''){
					@unlink($new_file_path.$check_row['nt_file']);
				}
				$delete_sql = " delete from g5_write_new_type where nt_idx='{$check_row['nt_idx']}' ";
				sql_query($delete_sql);
			}

			// 임대기종이 없고 정기점검및AS에 정기점검이 없을 경우에 최근점검일자 AND 다음점검일자를 빈값으로 처리한다
			$as_sql = " select count(*) as cnt from g5_write_as where wr_1='{$wr_id}' and wr_2='정기점검' ";
			$as_row = sql_fetch($as_sql);
			if($as_row['cnt'] == 0){
				$up_sql = " update $write_table set latest_check_date = '', next_check_date = '' where wr_id='{$wr_id}' ";
				sql_query($up_sql);
			}
			...
			if(count($_POST['nt_cnt']) > 0){
				...
				/* 등록되어있는 데이터의 우선순위를 9999999 로 초기화 STR */
				$reset_sql = " update g5_write_new_type set nt_order='9999999' where nt_wr_id='{$wr_id}' ";
				sql_query($reset_sql);
				/* 등록되어있는 데이터의 우선순위를 9999999 로 초기화 END */

				for($a=0; $a<count($_POST['nt_cnt']); $a++){
					if($_POST['nt_cnt'][$a] != ''){
						// 수정일때
						$up_sql = " update g5_write_new_type set 
						nt_date = '{$_POST['nt_date'][$a]}', 
						nt_list = '{$_POST['nt_list'][$a]}',
						nt_model = '{$_POST['nt_model'][$a]}',
						nt_page1 = '{$_POST['nt_page1'][$a]}',
						...
						";
						if(sql_query($up_sql)){
							$nt_idx = $_POST['nt_cnt'][$a];

							// 삭제에 체크가 되어있을때
							if($_POST['nt_file_del'][$a] == 'y' && $_POST['pre_nt_file'][$a] != ''){
								@unlink($new_file_path.$_POST['pre_nt_file'][$a]);
								$set_sql = " update g5_write_new_type set nt_file='' where nt_idx='{$nt_idx}' ";
								sql_query($set_sql);
							}

							if($_FILES['nt_file']['name'][$a] != ''){
								if($_POST['pre_nt_file'][$a] != ''){
									@unlink($new_file_path.$_POST['pre_nt_file'][$a]);
									$set_sql = " update g5_write_new_type set nt_file='' where nt_idx='{$nt_idx}' ";
									sql_query($set_sql);
								}
				...
				/* 우선순위가 그대로 9999999 로 되어있는 데이터는 삭제라고 판단하여 삭제 STR */
				$check_sql = " select nt_idx, nt_file from g5_write_new_type where nt_wr_id='{$wr_id}' and nt_order='9999999' ";
				$check_qry = sql_query($check_sql);
				$check_num = sql_num_rows($check_qry);
				for($cn=0; $cn<$check_num; $cn++){
					$check_row = sql_fetch_array($check_qry);
					...
					$delete_sql = " delete from g5_write_new_type where nt_idx='{$check_row['nt_idx']}' and nt_order='9999999' ";
					sql_query($delete_sql);
				}
				/* 우선순위가 그대로 9999999 로 되어있는 데이터는 삭제라고 판단하여 삭제 END */

				// 정기점검및AS에 정기점검이 없을 경우에 최근점검일자 AND 다음점검일자를 업데이트 해준다
				$as_sql = " select count(*) as cnt from g5_write_as where wr_1='{$wr_id}' and wr_2='정기점검' ";
				$as_row = sql_fetch($as_sql);
				if($as_row['cnt'] == 0){
					$up_sql = " update $write_table set latest_check_date = '{$latest_check_date}', next_check_date = '{$next_check_date}' where wr_id='{$wr_id}' ";
					sql_query($up_sql);
				}else{
					$latest_sql = " select max(wr_3) as max_wr_3 from g5_write_as where wr_1='{$wr_id}' and wr_2='정기점검' ";
					$latest_row = sql_fetch($latest_sql);
					$max_wr_3 = str_replace('-','',$latest_row['max_wr_3']);
					$max_wr_3_arr = explode('-',$latest_row['max_wr_3']);

					if($_POST['pre_wr_16'] != '' && $_POST['wr_16'] != ''){
						if($_POST['pre_wr_16'] != $_POST['wr_16']){
							$latest_month = (int)$max_wr_3_arr[1];
							$wr_16 = str_replace('개월','',$_POST['wr_16']);
							$next_month = $latest_month + $wr_16;
							if($next_month > 12){
								$plus_year = $max_wr_3_arr[0] + floor($next_month / 12);
								$next_month = $next_month % 12;
							...
							$up_sql = " update g5_write_new set next_check_date = '{$next_check_date}' where wr_id='{$_POST['wr_id']}' ";
							sql_query($up_sql);
						}
					}
				}
...
if($bo_table == 'new' && $_POST['wr_2'] == '임대해지'){
	goto_url(G5_HTTP_BBS_URL.'/board.php?bo_table='.$bo_table.'&wr_id='.$wr_id.'&sch_wr_2='.urlencode('임대해지'));
}

if($bo_table == 'as'){
	goto_url(G5_HTTP_BBS_URL.'/board.php?bo_table=new&wr_id='.$_POST['wr_1']);
}
