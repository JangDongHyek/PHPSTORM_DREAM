...
<section id="bo_w">
    ...
    {
        if (obj.checked) {
            result = confirm("자동 줄바꿈을 하시겠습니까?\n\n자동 줄바꿈은 게시물 비고중 줄바뀐 곳을<br>태그로 변환하는 기능입니다.");
            if (result)
                obj.value = "html2";
            else
                obj.value = "html1";
    ...
    {
        <?php echo $editor_js; // 에디터 사용시 자바스크립트에서 비고을 폼필드로 넣어주며 비고이 입력되었는지 검사함   ?>
        ...
        if (content) {
            alert("비고에 금지단어('"+content+"')가 포함되어있습니다");
        ...
        if (document.getElementById("char_count")) {
            if (char_min > 0 || char_max > 0) {
                ...
                if (char_min > 0 && char_min > cnt) {
                    alert("비고은 "+char_min+"글자 이상 쓰셔야 합니다.");
                    return false;
                }
                else if (char_max > 0 && char_max < cnt) {
                    alert("비고은 "+char_max+"글자 이하로 쓰셔야 합니다.");
                    return false;
                }
            }
        }
