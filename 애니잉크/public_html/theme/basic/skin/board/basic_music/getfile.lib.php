...
function get_list_file($bo_table, $wr_id)
{
    ...
    $sql = " select bf_file, bf_content from {$g5['board_file_table']}
                where bo_table = '$bo_table' and wr_id = '$wr_id' and bf_type  ='0' order by bf_no limit 0, 1 ";
    $row = sql_fetch($sql);
...
// 게시글보기 썸네일 생성
function get_view_file($contents, $thumb_width=0)
{
    global $board, $config;
    ...
    for($i=0; $i<count($matches[1]); $i++) {
        ...
        if(is_file($srcfile)) {
            ...
            // 썸네일 생성
            if(!$is_animated)
                $thumb_file = xfile($filename, $filepath, $filepath, $thumb_width, $thumb_height, false);
            else
                $thumb_file = $filename;
...
function xfile($filename, $source_path, $target_path, $thumb_width, $thumb_height, $is_create, $is_crop=false, $crop_mode='center', $is_sharpen=false, $um_value='80/0.5/3')
{
    global $g5;
