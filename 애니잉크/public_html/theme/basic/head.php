...
<div id="hd">
    ...
    <div id="hd_wrapper">
        ...
        <ul id="tnb">
            ...
            <?php if ($is_member) {  ?>
            <?php if ($is_admin) {  ?>
            <li><a href="<?php echo G5_ADMIN_URL ?>" target="_blank"><b>회원관리</b></a></li>
            <?php }  ?>
    ...
    <nav id="gnb">
        ...
        <ul id="gnb_1dul">
            ...
            for ($i=0; $row=sql_fetch_array($result); $i++) {
				if($row['me_name'] == '통계' && !$is_admin) continue;

				if($member['mb_level'] < 9 && ($row['me_name'] == '수리/판매' || $row['me_name'] == '해지고객리스트')) continue;
...
<? if(defined('_INDEX_')) {?>
  <div id="container_index"></div>
	...
	<!--서브비쥬얼-->
	<?php /*?><div id="svisual" style="background:url(<?php echo G5_THEME_IMG_URL?>/svisual.jpg) no-repeat center top;"></div><?php */?>
	<div id="container">
		<?php /*?><?php 
			//서브메뉴 추가
				...
				<div id="sub_title">
					<div class="p_info">
						<ul>
							...
							<li>&nbsp;&nbsp;|&nbsp;&nbsp;</li>
							<li class="pt"> 
								<?php if($bo_table == 'new' && $sch_wr_2 == '임대해지') $board['bo_subject'] = '해지고객리스트'; ?>
								<?php if($bo_table) {?>
