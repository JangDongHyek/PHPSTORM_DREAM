<?php
	...
	*/
    ...
    //$returnurl = G5_URL."/CPClient/checkplus_success.php";	// 성공시 이동될 URL

    if($get_return_url){
        $returnurl = G5_URL."/app/".$get_return_url.".php";	// 성공시 이동될 URL
    }else{
        $returnurl = G5_URL."/app/register.php";	// 성공시 이동될 URL
    }
    
    // 박완열 추가함
    // 임의로 url 지정 가능하도록
    if(!empty($get_return_url2)){
        $returnurl = $get_return_url2;
    }

    $errorurl = G5_URL."/CPClient/checkplus_fail.php";		// 실패시 이동될 URL
