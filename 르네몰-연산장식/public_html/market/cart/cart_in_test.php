...
if($mode == "del"){
	...
	echo "<meta http-equiv='refresh' content='0; URL=cart_test.html?mart_id=$mart_id&provider_id=$provider_id&item_no=$item_no&category_num=$category_num&category_num1=$category_num1&category_num2=$category_num2&cate_num=$cate_num&flag=$flag'>";
	exit;
}
if($mode == "cart_del"){
	...
	for($i=0; $i<count($quantity); $i++){
		...
		$jaego = mysql_result($dbresult,0,1);
	 	if($jaego_use == '1' && $quantity[$i] > $quantity_prev && $quantity[$i] > $jaego){
			...
			$SQL = "update $Order_ProTable set quantity='$quantity[$i]' $where_opts where mart_id='$mart_id' and order_pro_no='$order_pro_no[$i]' and status='0'";
			$dbresult = mysql_query($SQL, $dbconn);
			$where_opts = "";
		}
	}
...
/*
echo "본제품 가격:".$z_price."<BR>";
echo "본제품 갯수:".$quantity."<BR>";
echo "옵션1&nbsp;&nbsp;가격:".$opt1_price."<BR>";
echo "옵션1&nbsp;&nbsp;갯수:".$opt1_quantity_total."<BR>";
echo "옵션2&nbsp;&nbsp;가격:".$opt2_price."<BR>";
echo "옵션2&nbsp;&nbsp;갯수:".$opt2_quantity_total."<BR>";
echo "옵션3&nbsp;&nbsp;가격:".$opt3_price."<BR>";
echo "옵션3&nbsp;&nbsp;갯수:".$opt3_quantity_total."<BR>";
echo "옵션1 값:".$opt1."<BR>";
echo "옵션2 값:".$opt2."<BR>";
echo "옵션3 값:".$opt3."<BR>";
echo $flag."<BR>";
echo $Order_ProTable."<BR>";
exit;
*/
	...
	if ($numRows > 0) {
		...
		$SQL = "insert into $Order_ProTable (order_num, mart_id, item_no, item_name, item_code, opt, z_price, quantity, opt1_price, opt2_price, opt3_price, opt1_quantity_total, opt2_quantity_total, opt3_quantity_total, bonus, use_bonus, status, date, provider_id, provide_price,parcel_price) values ('$order_num', '$mart_id', $item_no, '$item_name', '$item_code', '$opt', '$z_price', '$quantity', '$opt1_price', '$opt2_price', '$opt3_price', '$opt1_quantity_total', '$opt2_quantity_total', '$opt3_quantity_total', $bonus, $use_bonus, '0', '$cur_date', '$provider_id', '$provide_price','$parcel_price')";
		$dbresult = mysql_query($SQL, $dbconn);
	}
	if( $direct_submit_flag == "direct_submit" ){//바로 구매일때
		echo "<meta http-equiv='refresh' content='0; URL=../cart/order_test.html?mart_id=$mart_id&provider_id=$provider_id&item_no=$item_no&category_num=$category_num&category_num1=$category_num1&category_num2=$category_num2&cate_num=$cate_num&flag=$flag&direct_submit_flag=$direct_submit_flag&taekbae=$taekbae'>";
	}else{
		echo "<meta http-equiv='refresh' content='0; URL=../cart/cart_test.html?mart_id=$mart_id&provider_id=$provider_id&item_no=$item_no&category_num=$category_num&category_num1=$category_num1&category_num2=$category_num2&cate_num=$cate_num&flag=$flag&direct_submit_flag=$direct_submit_flag&taekbae=$taekbae'>";
	}
