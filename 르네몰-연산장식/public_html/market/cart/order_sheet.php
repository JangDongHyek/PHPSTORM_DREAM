...
if($numRows > 0){
	...
	$buyer_passport_use = $ary["buyer_passport_use"];
...
var Digit = '1234567890'
  ...
  <?
	...
	*/
	...
	if($bonus_ok == 't'){
		echo ("
		if(form.if_use_bonus.checked){
			
			if (form.use_bonus_tot.value==''){
			  	alert(\"\\n사용하실 마일리지양을 입력해 주세요.\");
			  	form.use_bonus_tot.focus();  
			  	return false;
			}
			else{
			  	var len =form.use_bonus_tot.value.length;
			  	var ret;
			  	ret =false;  
		  
			  	for(var i=0;i<len;i++){  
			      	var ch = form.use_bonus_tot.value.substring(i,i+1);
			...
			if(parseInt(form.use_bonus_tot.value) > parseInt(form.user_bonus.value)){
				alert(\"\\n사용할 수 있는 마일리지 이상을 입력하셨습니다.\");
				form.use_bonus_tot.focus();
			    return false;
			}
			if(parseInt(form.use_bonus_tot.value) > parseInt(form.mon_tot_freight.value)){
				alert(\"\\n결제 금액 이상으로 마일리지를 입력하셨습니다.\");
				form.use_bonus_tot.focus();
			    return false;
			}
			if(parseInt(form.use_bonus_tot.value) == parseInt(form.mon_tot_freight.value)){
...
?>
				<form action='order_sheet_ok.php' id='form1' name='f' onsubmit="return checkform(this)" method='post'>
        		...
        		<table border="0" width="540" cellspacing="0" cellpadding="0">
          		...
          		<tr>
            		<td width="536" align="left" height="25" colspan="4" bgcolor="#EFEFEF">
            			<p style="padding-left: 10px">주문자 정보</p></span>
					</td>
          		</tr>
          		<tr>
...
<?
if($buyer_name_use < 2 || $buyer_passport_use < 2){
?>
...
function pastadr_win(){
	window.open ( './pastadr.php?id=<?=$UnameSess?>&mart_id=<?=$mart_id?>', 'pastadr', 'toolbar=no,menubar=no,status=no,scrollbars=yes,resizable=no,width=582,height=300');
          		...
          		<tr>
            		<td width="403" height="25" align="left" colspan="3" bgcolor="#EFEFEF">
            			<p style="padding-left: 10px">수령자 정보&nbsp;&nbsp;&nbsp; 
            			<input name="C1" onclick="val_sel(this.form)" type="checkbox"> 주문자와 동일</p></td>
            			<td align=left width=159 bgColor=#efefef height=25>
            			<a href="javascript:pastadr_win()"><img src="../images/pastadr.gif" width="94" height="18" border="0"></a>
            		</td>
          		</tr>
          		<tr>
...
?>
          		...
          		<tr>
            		...
            		<td width="463" height="25" colspan="3">
						<font color="#FF0000"><?=number_format($mon_tot_freight)?>원</font></span>
					</td>
          		</tr>
<?
if($bonus_ok == 't'){ //관리자가 마일리지 사용가능하게 세팅했을 때
	...
	if($sum >= $bonus_limit){ //마일리지 합이 최소사용액보다 많을때.. 
?>
				<input type='hidden' name='user_bonus' value='$sum'>
				<tr>
            		<td width='99' height='25' align='left'>
						마일리지
					</td>
            		<td width='463' height='25' colspan='3'>
            			<input name='if_use_bonus' type='checkbox' value='1'>마일리지를 사용합니다.
            			현재 <?=$MemberName?> 님의 마일리지는 <?=$sum_str?>원 입니다.<br>
            			&nbsp;마일리지중 <input class='bb' name='use_bonus_tot' size='8' style='BORDER-BOTTOM: #7b7d7b 1px solid; BORDER-LEFT: #7b7d7b 1px solid; BORDER-RIGHT: #7b7d7b 1px solid; BORDER-TOP: #7b7d7b 1px solid; HEIGHT: 18px'>원을 구매에 사용하겠습니다.
					</td>
          		</tr>
<?
	}else{
?>
				<tr>
            		...
            		<td width='463' height='25' colspan='3'>
            			<input name='if_use_bonus' type='checkbox' value='1' disabled>마일리지를 사용할 수 없습니다.
            			현재 <?=$MemberName?> 님의 마일리지는 <?=$sum_str?>원 입니다.<br>
            			&nbsp;마일리지총합이 <?=$bonus_limit_str?>원 이상일 때만 구매에 사용할 수 있습니다.</span>
					</td>
          		</tr>
<?
	}
...
?>
          		<tr>
            		<td width="99" height="1" align="left" bgcolor="#C0C0C0"></td>
...
<?		  
	if($if_cash_flag == 0){
		if($account_yes=='t'){//결제액이 계좌이체 결제금액 가능액보다 클때 카드 결제창과 연결
			if(!empty($card_module)){
				$pay1_how = "by_account";
				$pay1_how_name = "$card_module 계좌이체 서비스를 이용합니다.";
			}else{
				$pay1_how = "";
				$pay1_how_name = "계좌이체 서비스를 준비중입니다.";
			}
?>
			    <tr>
            		<td width='562' height='1' align='left' colspan='4' bgcolor='#C0C0C0'></td>
