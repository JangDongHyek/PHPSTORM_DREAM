...
if($mode == "update"){
	if($ud == 'u'){	
		$SQL = "select quantity from $Order_ProTable where order_pro_no='$order_pro_no' and status='0'";
			$dbresult = mysql_query($SQL, $dbconn);
			$rows = mysql_fetch_array($dbresult);

			$SQL = "select jaego_use,jaego from $ItemTable where item_no='$item_no'";
			$dbresult = mysql_query($SQL, $dbconn);
			$jaego_use = mysql_result($dbresult,0,0);
			$jaego = mysql_result($dbresult,0,1);

			if($jaego_use == '1' && $jaego <= $rows[quantity]){
				echo "
				<script>
		...
		$SQL = "update $Order_ProTable set quantity=quantity+1 where mart_id='$mart_id' and order_pro_no='$order_pro_no' and status='0'";
...
$item_money123 = mysql_result($dbresult, 0, "z_price");
$opts = explode("=",$opt);
...
if($flag == "addorder"){
	...
	if(!empty($opt1)){
		$opt = $opt1;
		$opt1_money_arr = explode("^",$opt1);
		$opt1_money = $opt1_money_arr[1];
		$opt_money += $opt1_money;
	}else{
		$opt = "";
	}	
	...
	if(!empty($opt2)){
		$opt = $opt."!".$opt2;
		$opt2_money_arr = explode("^",$opt2);
		$opt2_money = $opt2_money_arr[1];
		$opt_money += $opt2_money;
	}else{
		$opt = $opt."!";
	}
	...
	if(!empty($opt3)){
		$opt = $opt."!".$opt3;
		$opt3_money_arr = explode("^",$opt3);
		$opt3_money = $opt3_money_arr[1];
		$opt_money += $opt3_money;
	}else{
		$opt = $opt."!";
	}

	$total_item_money123 = $opt_money + $item_money123;

	if($total_item_money123 != $z_price){
		$z_price = $total_item_money123;
	}
	...
	if ($numRows > 0) {
		...
		$SQL = "insert into $Order_ProTable (order_num, mart_id, item_no, item_name, item_code, opt, z_price, quantity, bonus, use_bonus, status, date, provider_id, provide_price,parcel_price) values ('$order_num', '$mart_id', $item_no, '$item_name', '$item_code', '$opt', $z_price, $quantity, $bonus, $use_bonus, '0', '$cur_date', '$provider_id', '$provide_price','$parcel_price')";
		$dbresult = mysql_query($SQL, $dbconn);
	}

	if( $direct_submit_flag == "direct_submit" ){//바로 구매일때
		echo "<meta http-equiv='refresh' content='0; URL=../cart/order.html?mart_id=$mart_id&provider_id=$provider_id&item_no=$item_no&category_num=$category_num&category_num1=$category_num1&category_num2=$category_num2&cate_num=$cate_num&flag=$flag&direct_submit_flag=$direct_submit_flag&taekbae=$taekbae'>";
	}else{
		echo "<meta http-equiv='refresh' content='0; URL=../cart/cart.html?mart_id=$mart_id&provider_id=$provider_id&item_no=$item_no&category_num=$category_num&category_num1=$category_num1&category_num2=$category_num2&cate_num=$cate_num&flag=$flag&direct_submit_flag=$direct_submit_flag&taekbae=$taekbae'>";
	}
