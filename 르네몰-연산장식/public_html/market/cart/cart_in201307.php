...
$sql="alter table $Order_ProTable add username varchar(255)";
@mysql_query($sql);
$sql="alter table $Order_ProTable add wdate varchar(255)";
@mysql_query($sql);
if($mode == "del"){
...
if($flag == "addorder"){
	...
	//비회원으로 먼저 장바구니 담고 회원 로그인 할 때 장바구니에 회원아이디 업데이트 시켜주는 부분
	if(!$_SESSION[UnameSess]){
		$sql="update $Order_ProTable set username='$_SESSION[UnameSess]' where mart_id='$mart_id' and order_num='$order_num' and status=0";
		$result=mysql_query($sql);
	}
	if($_SESSION[UnameSess]){
		//회원이 로그인후 기존에 있던 장바구니에 주문번호를 바꿔주는 부분
		$sql="update $Order_ProTable set order_num='$order_num' where mart_id='$mart_id' and username='$_SESSION[UnameSess]' and status=0 and order_num!='$order_num'";
		mysql_query($sql);
	}
	...
	if ($numRows > 0) {
		...
		$SQL = "insert into $Order_ProTable (order_num, mart_id, item_no, item_name, item_code, opt, z_price, quantity, bonus, use_bonus, status, date, provider_id, provide_price,parcel_price,username,wdate) values ('$order_num', '$mart_id', $item_no, '$item_name', '$item_code', '$opt', $z_price, $quantity, $bonus, $use_bonus, '0', '$cur_date', '$provider_id', '$provide_price','$parcel_price','$_SESSION[UnameSess]','$wdate')";
		$dbresult = mysql_query($SQL, $dbconn);
	}
