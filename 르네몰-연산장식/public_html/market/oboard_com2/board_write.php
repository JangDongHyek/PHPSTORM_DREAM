...
function IDcheck()
{
		...
		winobject.document.location = "../oboard_com2/id_exist.php?value11=" + f.value11.value;
		winobject.focus();
		return false;
}
...
if ((form.value11.value.length < 4) || (form.value11.value.length > 12)) {
	...
	if(form.value31.value == ""){
		alert('대표자생년월일을 입력하세요');
		form.value31.focus();
		return false;
	}
	/*if(form.value32.value == ""){
		alert('대표자 주민번호를 입력하세요');
		form.value32.focus();
		return false;
	}*/
...
<form name="writeform" method="post" action="../oboard_com2/board_write_process.php?code_url=<?=$code_url?>&board=<?=$board?>&uid=<?echo $uid?>&thread=<?echo $thread?>&thread2=<?echo $thread2?>&check_array=<?echo $check_array?>&search_word=<?echo $search_word?>&page=<?echo $page?>&depth=<?echo $depth?>&sort1=<?=$sort1?>&sort2=<?=$sort2?>&board_type=<?=$board_type?>" ENCTYPE='multipart/form-data' onsubmit="return code_pass()">
...
?>
                 <tr>
                    <td height="25" colspan="2" bgcolor="#FFFFFF" class="txt_B"><div align="center"><img src="../images/oboard_com2_title.gif" width="673" height="88"></div></td>
                  </tr>
                  <tr>
                  ...
                  <tr>
                    <td width="100" height="25" bgcolor="#FFFFFF"><div align="right"><b>생년월일</b></div></td>
                    <td bgcolor="#FFFFFF"><input name="value31" type="text" value="<?=$value3[0]?>" size="7" class="input_03" id="name"><!---<input name="value32" type="text" value="<?=$value3[1]?>" size="7" class="input_03" id="name">--></td>
                  </tr>
