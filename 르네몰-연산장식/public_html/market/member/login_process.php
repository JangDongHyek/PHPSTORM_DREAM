...
if( strcmp( $db_passwd, get_password_str($password) ) ){ // 두 변수를 비교하여 같으면 else{}, 다르면 if()를 실행함
	...
	if( headers_sent() ){
		...
		if( $member_type == "1" ){
			...
			//==================== 회원 구매횟수 3회이상시 우수회원으로  =============================

				$sql2 = "select * from order_buy where id='$username' and status='3'";
				$res2 = mysql_query($sql2,$dbconn);
				$cnt2 = mysql_num_rows($res2);

				if($cnt2 >= 3){
					$sql3 = "update mart_member_new set mem_grade = '2' where username='$username' and mem_grade = '1'";
					$res3 = mysql_query($sql3,$dbconn);
				}else{
					$sql3 = "update mart_member_new set mem_grade = '1' where username='$username'";
					$res3 = mysql_query($sql3,$dbconn);				
				}
...
/*
		...
		echo ("
			...
			<meta http-equiv='Refresh' content='0; url=http://www.renemall.co.kr$url'>
