...
$item_sql = "select * from item where firstno!='287' and z_price > '0' and if_hide='0' order by item_no desc";
$item_res = mysql_query($item_sql, $dbconn);
for($i=0;$i<$item_row = mysql_fetch_array($item_res);$i++){

	//item_no
	$mapid=$item_row[item_no];
	$pname=trim($item_row[item_name]);
	...
	$pgurl_mobile = "http://renemall.co.kr/mobile/main/product_info.html?mart_id=yensan&category_num=$cate1_num&cate_num=$cate3&item_no=$item_row[item_no]";
...
$txt_data .= "\n{$mapid}{$tab}{$pname}{$tab}{$price}{$tab}{$pgurl}{$tab}{$igurl}{$tab}{$cate1_name}{$tab}{$cate2_name}{$tab}{$djqtdma}{$tab}{$cate4}{$tab}{$row['it_model']}{$tab}{$row['it_brand']}{$tab}{$row['it_maker']}{$tab}{$row['it_origin']}{$tab}{$point}{$tab}{$deliv}";
