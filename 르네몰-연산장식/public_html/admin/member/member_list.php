...
if($mem_grade_search == "y"){
	$add_query = " and mem_grade='2' ";
				 ...
				 $SQL = "select * from $Mart_Member_NewTable where mart_id='$mart_id' $add_query";
					...
					회원수 : 총 <font color="#ff0000"><?echo $numRows?></font> 명</b> <a href='../to_excel/member.php?keyset=<?=$keyset?>&searchword=<?=$searchword?>&order=<?=$order?>&orderby=<?=$orderby?>&mem_grade_search=<?=$mem_grade_search?>'><img src='../images/excel.gif' border='0' width='84' height='18' align='absmiddle'></a></td>
					<td width="50%" height="20"><p align="right">

					<input type=checkbox name="mem_grade_search" value="y" <?if($mem_grade_search=="y"){echo "checked";}?>>우수회원
...
<a href="javascript:selectAll()"><b>전체선택</b></a>
</td>
...
if($order == 'mem_grade'){
	if($orderby == 'desc') {
?>
						<a class='dd' href='member_list.php?page=<?=$page?>&keyset=<?=$keyset?>&searchword=<?=$searchword?>&order=mem_grade&orderby=asc'>회원등급</a><small>▼</small>
<?
	}else {
?>
						<a class='dd' href='member_list.php?page=<?=$page?>&keyset=<?=$keyset?>&searchword=<?=$searchword?>&order=mem_grade&orderby=desc'>회원등급</a><small>▲</small>
<?
	}	
}else{
?>
						<a class='dd' href='member_list.php?page=<?=$page?>&keyset=<?=$keyset?>&searchword=<?=$searchword?>&order=mem_grade&orderby=desc'>회원등급</a>
<?
}
?>						 
						</b></td>
...
for($i=$skipNum; $i < ($cnfPagecount+$skipNum); $i++){
	...
	$mem_grade = $ary[mem_grade];//쿠폰을 지급한 회원사
...
?>
				<a href='member_list.php?mem_grade_search=<?=$mem_grade_search?>&page=1&keyset=<?=$keyset?>&searchword=<?=$searchword?>&order=<?=$order?>&orderby=<?=$orderby?>'>처음</a> 
<?
}
if($start_page > 1){
?>
...
?>
				<a href='member_list.php?mem_grade_search=<?=$mem_grade_search?>&page=<?=$total_page?>&keyset=<?=$keyset?>&searchword=<?=$searchword?>&order=<?=$order?>&orderby=<?=$orderby?>'>끝</a> 
<?
}
?>
				</td>
			...
			<tr align="middle">
			  ...
			  <input onClick="return check('<?echo $keyset?>','<?echo $searchword?>','<?echo $mem_grade_search?>')" style="BORDER-RIGHT: #5a5a5a 1px solid; BORDER-TOP: #5a5a5a 1px solid; BORDER-LEFT: #5a5a5a 1px solid; COLOR: black; BORDER-BOTTOM: #5a5a5a 1px solid; HEIGHT: 18px; BACKGROUND-COLOR: white" type="button" value="검색하여 메일">&nbsp; 
			  <input onClick="opensub('today_login.php', 540, 400)" style="BORDER-RIGHT: #5a5a5a 1px solid; BORDER-TOP: #5a5a5a 1px solid; BORDER-LEFT: #5a5a5a 1px solid; COLOR: black; BORDER-BOTTOM: #5a5a5a 1px solid; HEIGHT: 18px; BACKGROUND-COLOR: white" type="button" value="오늘 로그인한 회원">&nbsp; 
			...
			<tr>
			  <td vAlign="top" width="100%" bgColor="#ffffff"><div align="center"><center><table width="95%" border="0">
				 <tr>
					...
					<br>
					<b>검색하여 메일보내기 : </b>주소, 생년등 필요한 항목을 검색하여 검색결과로 메일을 
					보내는 기능입니다.<br>
