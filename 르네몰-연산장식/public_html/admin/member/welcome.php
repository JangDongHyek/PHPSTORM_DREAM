...
<!--------------------------------------- 에디트 끝 ------------------------------------------------------>	<!-- <OBJECT id=editBox data=../editor/Editor_sml.htm width=530 height=160 type=text/x-scriptlet></OBJECT> --></td>
				</tr>
				<tr>
...
elseif ($flag == "update") {
	...
	$numRows = mysql_num_rows($dbresult);
	if($numRows > 0){
		$SQL = "update $Member_WelcomeTable set message = '$content', bg_img = '$bg_img' where mart_id='$mart_id'";
	}else{
		$SQL = "insert into $Member_WelcomeTable (mart_id, message, bg_img) values ('$mart_id', '$content', '$bg_img')";
	}
