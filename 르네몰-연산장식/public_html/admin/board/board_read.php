...
<script>
function really(){
if (confirm("현재글을 정말로 삭제하시겠습니까?")) return true;
return false;
}
														...
														<td>
															<table width="100%"  border="0" cellspacing="0" cellpadding="0">
																<tr>
																	...
																	<td width="100" align="center" class="search_type"></td>
																</tr>
																<tr>
...
<?
$SQL1 = "select count(*) from $New_BoardTable where bbs_no='$bbs_no' and area='$item_no' and mart_id='$mart_id' and step='1' and ansno='$ansno2' "; 
...
$dbresult = mysql_query($SQL, $dbconn);
$numRows_tot123 = mysql_result($dbresult,0,0);

$ansno2 = $ansno+1;

$SQL1 = "select * from $New_BoardTable where bbs_no='$bbs_no' and area='$item_no' and mart_id='$mart_id' and step='1' and ansno='$ansno2' "; 
$SQL2 = "and binary $keyset like '%$searchword%' ";
$SQL2_1 = "and (binary writer like '%$searchword%' or binary subject_new like '%$searchword%' or binary content like '%$searchword%') ";
...
for ($i=0; $i < $numRows; $i++){
	...
	$index_no123 = $row[index_no];
	$writer123 = $row[writer];
	$user_id123 = $row[username];
	$write_date123 = $row[write_date];
	$read_num123 = $row[read_num];
	$subject_new123 = $row[subject_new];
	$step123 = $row[step];
	$if_secret123 = $row[if_secret];
	$content123 = $row[content];
	$step123 = $row[step];

	$write_date_tmp123 = substr($write_date123,0,8);
	$today_tmp123 = date("Ymd");

	if(strlen($subject_new) > 50){
		$subject_new123 = substr($subject_new123, 0, 50);
		preg_match('/^([\x00-\x7e]|.{2})*/', $subject_new123, $subject_new_tmp123);
		$subject_new_tmp123[0] .= '..';
		$subject_new123 = $subject_new_tmp123[0];
	}

	$member_img123 = $writer123;

	$k = $numRows_tot123 - $i;
	$write_date_str123 = substr($write_date123,0,4)."/".substr($write_date123,4,2)."/".substr($write_date123,6,2);
...
?>
																<tr height="30" bgcolor='#F9F7EA'>
																	...
																	<td height="30"><A HREF="javascript:span_sw( show<?=$k?> );"><?=$subject_new123?></a></td>
																	<td align="center"><?=$member_img123?></td>
																	<td align="center" class="search_type"><?=$write_date_str123?></td>
																</tr>
																<tr>
																	<td height="1" colspan="4" bgcolor="#E2E2E2"></td>
																</tr>
	...
	echo ("
					<tr>
						<td bgColor='#F6F6F6' height='25'>&nbsp; 
							...
							<a href='board_read.php?mart_id=$mart_id&flag=del&index_no=$index_no&bbs_no=$bbs_no&page=$page&keyset=$keyset&searchword=$searchword' onclick='return really()'><img border='0' src='../../market/images/del.gif' width='49' height='15'></a>
	");
if( $bbs_no != 3 ){
	echo ("
