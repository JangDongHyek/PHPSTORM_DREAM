...
function baesongbi_2500(f){
  if(!checkitems())
  {
	return false;
  }
  if (confirm("선택한 상품의 배송비를 2500원으로 변경 하시겠습니까?")){
		f.flag.value='baesongbi_2500';
		f.submit();
	}
	return true;
}
function baesongbi(f,val){
  if(!checkitems())
  {
	return false;
  }
  if (confirm("선택한 상품의 배송비를 "+val+"원으로 변경 하시겠습니까?")){
		f.flag.value='baesongbi/'+val;
		f.submit();
	}
}
function baesongbi_2700(f){
  if(!checkitems())
  {
	return false;
  }
  if (confirm("선택한 상품의 배송비를 3500원으로 변경 하시겠습니까?")){
		f.flag.value='baesongbi_2700';
		f.submit();
	}
	return true;
}

function baesongbi_3000(f){
  if(!checkitems())
  {
	return false;
  }
  if (confirm("선택한 상품의 배송비를 4200원으로 변경 하시겠습니까?")){
		f.flag.value='baesongbi_3000';
		f.submit();
	}
	return true;
}
function baesongbi_3500(f){
  if(!checkitems())
  {
	return false;
  }
  if (confirm("선택한 상품의 배송비를 5500원으로 변경 하시겠습니까?")){
		f.flag.value='baesongbi_5500';
		f.submit();
	}
	return true;
}
function baesongbi_4500(f){
  if(!checkitems())
  {
	return false;
  }
  if (confirm("선택한 상품의 배송비를 4500원으로 변경 하시겠습니까?")){
		f.flag.value='baesongbi_4500';
		f.submit();
	}
	return true;
}

function baesongbi_7000(f){
  if(!checkitems())
  {
	return false;
  }
  if (confirm("선택한 상품의 배송비를 7000원으로 변경 하시겠습니까?")){
		f.flag.value='baesongbi_7000';
		f.submit();
	}
	return true;
}
function baesongbi_9000(f){
  if(!checkitems())
  {
	return false;
  }
  if (confirm("선택한 상품의 배송비를 9000원으로 변경 하시겠습니까?")){
		f.flag.value='baesongbi_9000';
		f.submit();
	}
	return true;
}
...
?>
							...
							<input onclick="check_ver('<?=$first_no?>','<?=$second_no?>','<?=$category_num?>')" style='background-color: #4CAABE; color: white; height: 18px; border: 1px solid #4CAABE' type='button' value='상 품 등 록'>&nbsp;
					...
					<tr>
						...
						<td width="50%" bgcolor="#FFFFFF" height="0" align="center">
							<select name="select_key">
								...
								<!-- <option value="provider_id" <?if($select_key == "provider_id") echo " selected";?>>입점몰명</option> -->
							</select>
					...
					<table border="0" width="100%">
					  ...
					  <tr>
						...
						<font color="#3D918A">선택한 상품을 </font>
							...
							<input onclick="prefee_item(this.form)" style="BACKGROUND-COLOR: white; BORDER-BOTTOM: #3D918A 1px solid; BORDER-LEFT:  #3D918A 1px solid; BORDER-RIGHT: #3D918A 1px solid; BORDER-TOP: #3D918A 1px solid; COLOR:#3D918A;  HEIGHT: 18px" type="button" value="선불">


<BR>

							배송비:<input onclick="baesongbi(this.form,'3000')" style="BACKGROUND-COLOR: white; BORDER-BOTTOM: #3D918A 1px solid; BORDER-LEFT:  #3D918A 1px solid; BORDER-RIGHT: #3D918A 1px solid; BORDER-TOP: #3D918A 1px solid; COLOR:#3D918A;  HEIGHT: 18px" type="button" value="3000">&nbsp;&nbsp;
							<input onclick="baesongbi(this.form,'3500')" style="BACKGROUND-COLOR: white; BORDER-BOTTOM: #3D918A 1px solid; BORDER-LEFT:  #3D918A 1px solid; BORDER-RIGHT: #3D918A 1px solid; BORDER-TOP: #3D918A 1px solid; COLOR:#3D918A;  HEIGHT: 18px" type="button" value="3500">&nbsp;&nbsp;
							<input onclick="baesongbi(this.form,'4200')" style="BACKGROUND-COLOR: white; BORDER-BOTTOM: #3D918A 1px solid; BORDER-LEFT:  #3D918A 1px solid; BORDER-RIGHT: #3D918A 1px solid; BORDER-TOP: #3D918A 1px solid; COLOR:#3D918A;  HEIGHT: 18px" type="button" value="4200">&nbsp;&nbsp;
							<input onclick="baesongbi(this.form,'5500')" style="BACKGROUND-COLOR: white; BORDER-BOTTOM: #3D918A 1px solid; BORDER-LEFT:  #3D918A 1px solid; BORDER-RIGHT: #3D918A 1px solid; BORDER-TOP: #3D918A 1px solid; COLOR:#3D918A;  HEIGHT: 18px" type="button" value="5500">&nbsp;&nbsp;
							<!--<input onclick="baesongbi(this.form,'')" style="BACKGROUND-COLOR: white; BORDER-BOTTOM: #3D918A 1px solid; BORDER-LEFT:  #3D918A 1px solid; BORDER-RIGHT: #3D918A 1px solid; BORDER-TOP: #3D918A 1px solid; COLOR:#3D918A;  HEIGHT: 18px" type="button" value="4500">&nbsp;&nbsp;-->

							<input onclick="baesongbi(this.form,'10000')" style="BACKGROUND-COLOR: white; BORDER-BOTTOM: #3D918A 1px solid; BORDER-LEFT:  #3D918A 1px solid; BORDER-RIGHT: #3D918A 1px solid; BORDER-TOP: #3D918A 1px solid; COLOR:#3D918A;  HEIGHT: 18px" type="button" value="10000">&nbsp;&nbsp;
<!--							<input onclick="baesongbi_9000(this.form)" style="BACKGROUND-COLOR: white; BORDER-BOTTOM: #3D918A 1px solid; BORDER-LEFT:  #3D918A 1px solid; BORDER-RIGHT: #3D918A 1px solid; BORDER-TOP: #3D918A 1px solid; COLOR:#3D918A;  HEIGHT: 18px" type="button" value="9000">-->
...
if(0 < strpos($flag,"aesongbi")){
	$parcel_priceArr=explode("/",$flag);
	$parcel_price=$parcel_priceArr[1];
	for($i=0; $i<count($checkSel); $i++) {
		$checkSels = explode("!", $checkSel[$i]);
		$item_no = $checkSels[0];
		$mart_id = $checkSels[1];
		
		$SQL = "update $ItemTable set parcel_price='$parcel_price' where item_no = '$item_no' and mart_id='$mart_id'";
		$dbresult = mysql_query($SQL, $dbconn);
	}
	echo "<meta http-equiv='refresh' content='0; URL=item_list.php?category_num=$prevno&searchword=$searchword&select_key=$select_key&page=$page&pu=$pu'>";
}
//========================================================================================
//= 2500 3000 3500 7000 ================= 배송비변경 ===========================================================
if($flag == "baesongbi_2500"){					
	for($i=0; $i<count($checkSel); $i++) {
		$checkSels = explode("!", $checkSel[$i]);
		$item_no = $checkSels[0];
		$mart_id = $checkSels[1];
		
		$SQL = "update $ItemTable set parcel_price='2500' where item_no = '$item_no' and mart_id='$mart_id'";
		$dbresult = mysql_query($SQL, $dbconn);
	}
	echo "<meta http-equiv='refresh' content='0; URL=item_list.php?category_num=$prevno&searchword=$searchword&select_key=$select_key&page=$page&pu=$pu'>";
}
if($flag == "baesongbi_2700"){					
	for($i=0; $i<count($checkSel); $i++) {
		$checkSels = explode("!", $checkSel[$i]);
		$item_no = $checkSels[0];
		$mart_id = $checkSels[1];
		
		$SQL = "update $ItemTable set parcel_price='3500' where item_no = '$item_no' and mart_id='$mart_id'";
		$dbresult = mysql_query($SQL, $dbconn);
	}
	echo "<meta http-equiv='refresh' content='0; URL=item_list.php?category_num=$prevno&searchword=$searchword&select_key=$select_key&page=$page&pu=$pu'>";
}

if($flag == "baesongbi_3000"){					
	for($i=0; $i<count($checkSel); $i++) {
		$checkSels = explode("!", $checkSel[$i]);
		$item_no = $checkSels[0];
		$mart_id = $checkSels[1];
		
		$SQL = "update $ItemTable set parcel_price='4200' where item_no = '$item_no' and mart_id='$mart_id'";
		$dbresult = mysql_query($SQL, $dbconn);
	}
	echo "<meta http-equiv='refresh' content='0; URL=item_list.php?category_num=$prevno&searchword=$searchword&select_key=$select_key&page=$page&pu=$pu'>";
}

if($flag == "baesongbi_3500"){					
	for($i=0; $i<count($checkSel); $i++) {
		$checkSels = explode("!", $checkSel[$i]);
		$item_no = $checkSels[0];
		$mart_id = $checkSels[1];
...
if($flag == "baesongbi_4500"){					
	for($i=0; $i<count($checkSel); $i++) {
		$checkSels = explode("!", $checkSel[$i]);
		$item_no = $checkSels[0];
		$mart_id = $checkSels[1];
		
		$SQL = "update $ItemTable set parcel_price='4500' where item_no = '$item_no' and mart_id='$mart_id'";
		$dbresult = mysql_query($SQL, $dbconn);
	}
	echo "<meta http-equiv='refresh' content='0; URL=item_list.php?category_num=$prevno&searchword=$searchword&select_key=$select_key&page=$page&pu=$pu'>";
}
...
if($flag == "baesongbi_7000"){					
	for($i=0; $i<count($checkSel); $i++) {
		$checkSels = explode("!", $checkSel[$i]);
		$item_no = $checkSels[0];
		$mart_id = $checkSels[1];
		
		$SQL = "update $ItemTable set parcel_price='7000' where item_no = '$item_no' and mart_id='$mart_id'";
		$dbresult = mysql_query($SQL, $dbconn);
	}
	echo "<meta http-equiv='refresh' content='0; URL=item_list.php?category_num=$prevno&searchword=$searchword&select_key=$select_key&page=$page&pu=$pu'>";
}

if($flag == "baesongbi_9000"){					
	for($i=0; $i<count($checkSel); $i++) {
		$checkSels = explode("!", $checkSel[$i]);
		$item_no = $checkSels[0];
		$mart_id = $checkSels[1];
		
		$SQL = "update $ItemTable set parcel_price='9000' where item_no = '$item_no' and mart_id='$mart_id'";
		$dbresult = mysql_query($SQL, $dbconn);
	}
	echo "<meta http-equiv='refresh' content='0; URL=item_list.php?category_num=$prevno&searchword=$searchword&select_key=$select_key&page=$page&pu=$pu'>";
}
...
//		if($Mall_Admin_ID == $mart_id){ // 내상품이면
			...
			for ($i=0; $i < $numRows; $i++) {
				...
				$item_no = $ary["item_no"];
				$mart_id = $ary["mart_id"];
				$item_name = addslashes($ary["item_name"]);
				$price = $ary["price"];
				$z_price = $ary["z_price"];
				$bonus = $ary["bonus"];
				...
				$opt = $ary["opt"];
				$doctype = $ary["doctype"];
				$short_explain = addslashes($ary["short_explain"]);
