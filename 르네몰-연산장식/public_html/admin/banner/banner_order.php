...
?>
				  ...
				  <td width="100%" bgcolor="#FFFFFF" valign="top"><div align="center"><center><table border="0" width="95%">
					 <tr>
						<td width="90%" bgcolor="#999999"><table border="0" width="100%" cellspacing="1" cellpadding="3">
						  ...
						  <tr>
							 <td width="33%" bgcolor="#C8DFEC" align="center">왼편 
							 배너</td>
							 <td width="33%" bgcolor="#C8DFEC" align="center">오른편 
							 배너</td>
							 <td width="33%" bgcolor="#C8DFEC" align="center">스크롤
							 배너</td>
						  </tr>
						  <tr>
							 <td width="33%" bgcolor="#FFFFFF" align="center" valign="top">
							 <table border="0" width="100%" cellspacing="3" cellpadding="3">
		<?
			$SQL = "select * from $BannerTable where mart_id='$mart_id' and banner_pos='left' order by banner_order desc";
			...
			for ($i=0; $i<$numRows; $i++) {
				...
				echo "
								  ...
								  <td width='41%' bgcolor='#FFFFFF' align='center'>
								  ";
								  if($if_flash == 0){
		...
		?>
							 ...
							 <table border="0" width="100%" cellspacing="3" cellpadding="3">
		<?
			$SQL = "select * from $BannerTable where mart_id='$mart_id' and banner_pos='right' order by banner_order desc";
				$dbresult = mysql_query($SQL, $dbconn);
				$numRows = mysql_num_rows($dbresult);
				for ($i=0; $i<$numRows; $i++) {
		...
		?>
							 ...
							 <table border="0" width="100%" cellspacing="3" cellpadding="3">
		<?
			$SQL = "select * from $BannerTable where mart_id='$mart_id' and banner_pos='scrol' order by banner_order desc";
				$dbresult = mysql_query($SQL, $dbconn);
				$numRows = mysql_num_rows($dbresult);
				for ($i=0; $i<$numRows; $i++) {
					mysql_data_seek($dbresult,$i);
...
if($flag == "up"){
	$SQL = "select banner_order from $BannerTable where banner_pos='$banner_pos' and banner_order > $banner_order and mart_id='$mart_id' order by banner_order Asc";
	$dbresult = mysql_query($SQL, $dbconn);
	$numRows = mysql_num_rows($dbresult);
...
if($flag == "down"){
	$SQL = "select banner_order from $BannerTable where banner_pos='$banner_pos' and banner_order < $banner_order and mart_id='$mart_id' order by banner_order Desc";
	$dbresult = mysql_query($SQL, $dbconn);
	$numRows = mysql_num_rows($dbresult);
	mysql_data_seek($dbresult, 0);	
	$ary=mysql_fetch_array($dbresult);	
