...
?>
<?
$SQL1 = "select order_num from $Order_BuyTable_Temp where index_no='$index_no'";
...
function really(index_no){
	if(confirm("\n정말로 삭제하시겟습니까?\n\n삭제하시면 현주문과 관련된 데이타가\n\n삭제되며 복구는 되지않습니다.")){
		window.location.href='order_detail2.php?page=<?=$page?>&keyset=<?=$keyset?>&searchword=<?=$searchword?>&cnfPagecount=<?=$cnfPagecount?>&QryFromDate=<?=$QryFromDate?>&QryToDate=<?=$QryToDate?>&update_flag=delete_data&index_no='+index_no;
		return true;
	}
	return false; 
}
...
<?
	$SQL = "select * from $Order_BuyTable_Temp where order_num='$order_num' and mart_id='$mart_id'";
	$dbresult = mysql_query($SQL, $dbconn);
	$numRows = mysql_num_rows($dbresult);
	if($numRows > 0){
		mysql_data_seek($dbresult,0);
		...
		$taxbill_use = $ary[taxbill_use];
		$pay_tax = $ary[pay_tax];
		$pay_tax_type = $ary[pay_tax_type];
		$pay_tax_num1 = $ary[pay_tax_num1];
		$pay_tax_num2 = $ary[pay_tax_num2];
		
		$co_num = $ary[co_num];
		$co_name = $ary[co_name];
		$co_daepyo = $ary[co_daepyo];
		$co_addr = $ary[co_addr];
		$co_uptae = $ary[co_uptae];
		$co_jongmok = $ary[co_jongmok];
...
if($index_no > 103712){
	...
	$bonus_1 = round($mon_tot * 0.02);
}else{
	$bonus_1 = round($mon_tot * 0.01);

}
?>
										포인트 적립액: <?=number_format($bonus_1)?> 원<br>
<?
	if($if_use_bonus == 1){
		echo ("
...
<?										
$bonus_sql = "select * from $BonusTable where mart_id ='$mart_id' and id = '$id' order by num desc";
...
$ssum_str = number_format($ssum);
if( $bonus_res ){
	mysql_free_result( $bonus_res );
}
?>
<br>포인트 적립총액 : <?=$ssum_str?> 원
...
if($mobile_chk == 'y'){
	$xpay_id = "lets08078_smt";
	$xpay_key = "fa1865ca207c711910627982412c734e";
}else{
	$xpay_id = "lets08078";
	$xpay_key = "d029110e594e9f1950c25ce6f0ce4e3c";
}
...
?>									
	...
	<input onClick="javascript:window.open('../../market/cart/CashReceipt.html?order_num=<?=$order_num?>&mon_tot_freight=<?=$mon_tot_freight?>&item_name=<?=$item_name?>&xpay_id=<?=$xpay_id?>','','left=100,top=50,width=400,height=500,scrollbars=yes');" style="BACKGROUND-COLOR: white; BORDER-BOTTOM: #7b7d7b 1px solid; BORDER-LEFT: #7b7d7b 1px solid; BORDER-RIGHT: #7b7d7b 1px solid; BORDER-TOP: #7b7d7b 1px solid; COLOR: #7b7d7b; HEIGHT: 18px; width=120" type="button" value="현금영수증 발급">
<?
...
<? }else{?>
	<input onClick="window.open('https://pg.innopay.co.kr/pay/issue/CardIssue.jsp?TID=<?php echo $field4?>','','width=600,height=800,scrollbars=yes')" type="button" value="카드전표 출력">
<? }}?>
<?###################################################################End 현금영수증 ####################################################################?>
						...
						<tr>
							<td width="90%" bgcolor="#999999">
								...
								<tr>
									 ...
									 <?
									 if($id!=''){
										$SQL = "select order_num from $Order_BuyTable_Temp where id='$id' and status='3' and mart_id='$mart_id'";//배송완료
													//echo "sql=$SQL";
													$dbresult = mysql_query($SQL, $dbconn);
													$numRows = mysql_num_rows($dbresult);
													$mon_tot = 0;
												...
												구매내역이 없습니다.
													...
													$SQL0 = "select order_num from $Order_BuyTable_Temp where status='3' and mart_id='$mart_id' ";
...
?>
								...
								<tr>
									<td width="12%" bgcolor="#FFFFFF" align="center">세금계산서</td>
									<td width="29%" bgcolor="#FFFFFF" align="left">
									<?php
									if($taxbill_use == 'y'){
									?>
									...
									<?
									}else if($taxbill_use == 'n'){
										echo '신청안함';
									}
									?>
									</td>
									<td width="12%" bgcolor="#FFFFFF" align="center">현금영수증</td>
									<td width="22%" bgcolor="#FFFFFF" align="left">
<?php
if($pay_tax == 'n') echo '미신청';
if($pay_tax == 'y'){
echo '신청<br>'.$pay_tax_type.'&nbsp;&nbsp;['.$pay_tax_num1.']&nbsp;&nbsp;'.$pay_tax_num2;
}
?>
									</td>
								</tr>
...
<?
if($card_paid == "t" && ($status == 1 || $status == 2) && $pay_cancel == "y"){
...
?>
					...
					<input onClick="javascript:return really('<?=$index_no?>')" style="BACKGROUND-COLOR: white; BORDER-BOTTOM: #5a5a5a 1px solid; BORDER-LEFT: #5a5a5a 1px solid; BORDER-RIGHT: #5a5a5a 1px solid; BORDER-TOP: #5a5a5a 1px solid; COLOR: black; HEIGHT: 18px" type="button" value="주문정보 삭제">
					</p>
				</td>
				</tr>
			</form>
...
?>
			  ...
			  <form action='order_detail2.php' method='post'>
...
$order_num = $rows1[order_num];
	...
	$status = '';
	//구매자 id 알아내기
	$SQL = "select id,status from $Order_BuyTable_Temp where order_num = '$order_num' and mart_id='$mart_id'";
	$dbresult = mysql_query($SQL, $dbconn);
	$numRows = mysql_num_rows($dbresult);
	if($numRows > 0){
	...
	$SQL = "update $Order_BuyTable_Temp set status='9' where order_num = '$order_num' and mart_id='$mart_id'";
	$dbresult = mysql_query($SQL, $dbconn);
	
	//사용한 보너스 복구
	$SQL = "select id, use_bonus_tot from $Order_BuyTable_Temp where order_num = '$order_num' and mart_id='$mart_id'";;
	$dbresult = mysql_query($SQL, $dbconn);
	$numRows = mysql_num_rows($dbresult);
	...
	echo "<meta http-equiv='refresh' content='0; URL=order_new2.php?page=$page&keyset=$keyset&searchword=$searchword&cnfPagecount=$cnfPagecount&QryFromDate=$QryFromDate&QryToDate=$QryToDate'>";
}
?>
<?
mysql_close($dbconn);
