...
if($update_flag == ''){
	...
	$method = "<a href='http://admin.kcp.co.kr/' target='_new'>[KCP 결제내역]</a>  <a href='http://pgweb.uplus.co.kr/' target='_new'>[LG U+ 결제내역]</a>";
...
<br>
			...
			<table border="1" cellpadding="5" cellspacing="0" width="90%" bordercolordark="white" bordercolorlight="#E1E1E1" align="center">
				...
				<form action='order_new.php' id='form1' name='form1' method='post'>
										...
										<tr height="10">
										 ...
										 <tr>
											...
											<td width="59%" colspan="2">
												...
												&nbsp;&nbsp;&nbsp;접속구분:<select name="mobile_chk_flag" style="BORDER-RIGHT: black 1px solid; BORDER-TOP: black 1px solid; BORDER-LEFT: black 1px solid; BORDER-BOTTOM: black 1px solid; height: 18px" size="1">
												<option selected value="">전체</option>
												<option value="y" <?if($mobile_chk_flag == "y") echo " selected";?>>모바일</option>
												<option value="n" <?if($mobile_chk_flag == "n") echo " selected";?>>PC</option>
												</select>	
												...
												<a href='../to_excel/order_list.php?flag=<?=$flag?>&QryFromDate=<?=$QryFromDate?>&QryToDate=<?=$QryToDate?>&status_flag=<?=$status_flag?>&mobile_chk_flag=<?=$mobile_chk_flag?>'><img src='../images/excel.gif' align='bottom' border='0' width='84' height='18'></a>
											</td>
										 </tr>
										</table>
									</td>
					...
					<td width="100%" bgcolor="#FFFFFF" valign="top" align="center">
						...
						<tr>
							<td width="90%" bgcolor="#999999">
								...
								<tr>
									<td width="100%" bgcolor="#8FBECD" colspan="8">
										<table border="0" width="100%" cellspacing="0" cellpadding="0">
											...
											<form action='order_new.php' method='post' onsubmit='return checkform(this)'>
											...
											<td width="50%" align="right">
												...
												<option value="money_sender" <?if($keyset == 'money_sender') echo "selected";?>>입금자명</option>
								...
								<input type='hidden' name='mobile_chk_flag' value='<?=$mobile_chk_flag?>'>
								...
								<tr>
									...
									<td width="14%" bgcolor="#FFFFFF" align="center">결제방법</td>
...
if($flag == "today"){
	$SQL = "select * from $Order_BuyTable where mart_id='$mart_id' and status != '9' $status_str and date like '$today%' order by date desc";
}else if($flag == "search"){
	if($keyset == 'item_name'){
		$SQL = "select DISTINCT T1.* from $Order_BuyTable T1, $Order_ProTable T2 where T1.mart_id='$mart_id' and T1.status != '9' $status_str and T1.order_num=T2.order_num and binary T2.item_name like '%$searchword%' order by date desc";
	}else if($keyset == 'name'){
		$SQL = "select * from $Order_BuyTable where mart_id='$mart_id' and status != '9' $status_str and  binary name like '%$searchword%' order by date desc";
	}else if($keyset == 'tel'){
		$SQL = "select * from $Order_BuyTable where mart_id='$mart_id' and status != '9' $status_str and  (binary tel1 like '%$searchword%' or binary tel2 like '%$searchword%') order by date desc";
	}else{
		$SQL = "select * from $Order_BuyTable where mart_id='$mart_id' and status != '9' $status_str and binary $keyset like '%$searchword%' order by date desc";
	}
}else if($flag == 'find'){
	$SQL = "select * from $Order_BuyTable where mart_id='$mart_id' and status != '9' $status_str and ( date >= '$QryFromDate1' and date <= '$QryToDate1') order by date desc";
}else{
	$SQL = "select * from $Order_BuyTable where mart_id='$mart_id' and status != '9' $status_str order by date desc";
}			



if($REMOTE_ADDR=="115.94.232.187"){
	//echo $flag."<br>";
	//echo $SQL;
}
...
for ($i=$skipNum; $i < ($cnfPagecount+$skipNum); $i++) {	
	...
	$item_detail_head = "
							<tr>
								<td align='middle' width='98%' bgColor='#F3F3F3' colspan='8'>
	...
	for ($k=0; $k<$numRows1; $k++) {
		...
		$opt1_price = $ary1[opt1_price];
		$opt2_price = $ary1[opt2_price];
		$opt3_price = $ary1[opt3_price];
		$opt1_quantity_total = $ary1[opt1_quantity_total];
		$opt2_quantity_total = $ary1[opt2_quantity_total];
		$opt3_quantity_total = $ary1[opt3_quantity_total];
		...
		$item_detail_body .= "<td width='50%'><img src='../images/dot.gif' width='5' height='7'>$item_name$item_code_str : $quantity"."개 [$good_status_str]<BR>▼옵션▼<BR>".$opt1_qtt_str.$opt2_qtt_str.$opt3_qtt_str."</td>";
...
?>
							...
							<input type='hidden' name='status_old[]' value='<?=$good_status?>'>
								...
								<td  bgcolor='#FFFFFF'><a href='order_detail.php?order_num=<?=$order_num?>&page=<?=$page?>&keyset=<?=$keyset?>&searchword=<?=$searchword?>&flag=<?=$flag?>&cnfPagecount=<?=$cnfPagecount?>&QryFromDate=<?=$QryFromDate?>&QryToDate=<?=$QryToDate?>&status_flag=<?=$status_flag?>&mobile_chk_flag=<?=$mobile_chk_flag?>'><?=$order_num?></a></td>
								<td bgcolor='#FFFFFF'><?=$name?></td>
								<td bgcolor='#FFFFFF'><?=$date_str?></td>
								<td bgcolor='#FFFFFF'><?=$mon_tot2?>원</td>
								<td bgcolor='#FFFFFF'>
<?
	if($status_temp == '9'){
?>
...
?>
								...
								<?if($ary[mobile_chk]=='y'){echo"모바일";}else{echo"PC";}?>
								</td>
								</tr>
								<?=$item_detail_str?>
<?
			...
			<tr align="center">
				<td width="100%" bgcolor="#FFFFFF">
					...
					if($page == 1){
						...
						echo ("
						<a href='order_new.php?page=1&keyset=$keyset&searchword=$searchword&flag=$flag&cnfPagecount=$cnfPagecount&QryFromDate=$QryFromDate&QryToDate=$QryToDate&status_flag=$status_flag&mobile_chk_flag=$mobile_chk_flag'>처음</a> 
						");
					}
				
					if($start_page > 1){
						echo ("
						<a href='order_new.php?page=$prev_start_page&keyset=$keyset&searchword=$searchword&flag=$flag&cnfPagecount=$cnfPagecount&QryFromDate=$QryFromDate&QryToDate=$QryToDate&status_flag=$status_flag&mobile_chk_flag=$mobile_chk_flag'>
						◁&nbsp; 
						</a>
						");
					}
					...
					for($i=$start_page;$i<=$end_page;$i++){
						if($i == $page){
							...
							echo ("
						<a href='order_new.php?page=$i&keyset=$keyset&searchword=$searchword&flag=$flag&cnfPagecount=$cnfPagecount&QryFromDate=$QryFromDate&QryToDate=$QryToDate&status_flag=$status_flag&mobile_chk_flag=$mobile_chk_flag'>$i</a> 
							");
						}
					}
					if($end_page < $total_page){
						echo ("
						<a href='order_new.php?page=$next_start_page&keyset=$keyset&searchword=$searchword&flag=$flag&cnfPagecount=$cnfPagecount&QryFromDate=$QryFromDate&QryToDate=$QryToDate&status_flag=$status_flag&mobile_chk_flag=$mobile_chk_flag'>
						&nbsp;▷
						</a>
						");
					}
					...
					if($page == $total_page){
						...
						echo ("
						<a href='order_new.php?page=$total_page&keyset=$keyset&searchword=$searchword&flag=$flag&cnfPagecount=$cnfPagecount&QryFromDate=$QryFromDate&QryToDate=$QryToDate&status_flag=$status_flag&mobile_chk_flag=$mobile_chk_flag'>끝</a> 
						");
					}
					?>
					</td>
...
if($update_flag == 'add_list_print'){
	...
	echo "<meta http-equiv='refresh' content='0; URL=order_new.php?page=$page&keyset=$keyset&searchword=$searchword&flag=$flag&cnfPagecount=$cnfPagecount&QryFromDate=$QryFromDate&QryToDate=$QryToDate&status_flag=$status_flag&mobile_chk_flag=$mobile_chk_flag'>";
}
...
/**	//================== SMS DB 설정 파일을 불러옴 =======================================
	...
	for($i=0; $i<count($checkSel); $i++){
		$num = $checkSel[$i];
		$SQL = "update $Order_BuyTable set status='$status_all' where order_num='$num' and mart_id='$mart_id'";
		$dbresult = mysql_query($SQL, $dbconn);
		...
		if( $status_old[$i] != '2' && $status_all == '2' ){  //입금완료시 ip, 시간 기록하기
			$ipgum_date = date("Y-m-d H:i:s");
			$SQL = "update $Order_BuyTable set ipgum_date='$ipgum_date', ipgum_ip='$REMOTE_ADDR' where order_num = '$num' and mart_id='$mart_id'";
			$dbresult = mysql_query($SQL, $dbconn);
		}
		...
		if($status_all=="5"||$status_all=="10"||$status_all=="4"){
			if($status_old[$i]!="5"&&$status_old[$i]!="10"&&$status_old[$i]!="4"){
				$sql="select * from $Order_ProTable where order_num='$num'";
