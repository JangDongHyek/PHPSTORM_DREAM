...
?>
<?
$SQL1 = "select order_num from $Order_BuyTable where index_no='$index_no'";
...
function really(index_no){
	if(confirm("\n정말로 삭제하시겟습니까?\n\n삭제하시면 현주문과 관련된 데이타가\n\n삭제되며 복구는 되지않습니다.")){
		window.location.href='order_detail.php?page=<?=$page?>&keyset=<?=$keyset?>&searchword=<?=$searchword?>&cnfPagecount=<?=$cnfPagecount?>&QryFromDate=<?=$QryFromDate?>&QryToDate=<?=$QryToDate?>&update_flag=delete_data&index_no='+index_no;
		return true;
	}
	return false; 
}
