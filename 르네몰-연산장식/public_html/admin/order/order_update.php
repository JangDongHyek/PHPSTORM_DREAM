...
if($update_flag == 'update_all'){	
	...
	if($status == 2 && $bonus_ok == 't'){ //입금확인, 포인트 사용할 때만 포인트 추가
	...
	if($status_old != '10' && $status == '10'){ //주문취소시 보너스에서 빼기
		...
		$SQL1 = "select sum(bonus) as bonus from $BonusTable where order_num='$order_num' and 0 < bonus and mart_id='$mart_id' group by order_num";
		$dbresult1 = mysql_query($SQL1, $dbconn);
		$numRows1 = mysql_num_rows($dbresult1);
		if($numRows > 0 &&$numRows1>0){
			...
			$id = mysql_result($dbresult,0,0);
			$use_bonus_tot = mysql_result($dbresult,0,1);
			$row=mysql_fetch_array($dbresult1);
			$bonus=$row[bonus];
			
			//if(!empty($id)&&!empty($use_bonus_tot)){
			if(!empty($id)){
				//회원테이블에서 보너스 총액 복구
				$SQL = "update $Mart_Member_NewTable set bonus_total=bonus_total-$bonus where username='$id' and mart_id='$mart_id'";
				$dbresult = mysql_query($SQL, $dbconn);				
				
				//보너스 테이블에 취소된 주문의 사용포인트 다시입력
				$write_date = date("Ymd H:i:s");
				$SQL = "insert into $BonusTable (mart_id, id, write_date, bonus, content, order_num) values ('$mart_id', '$id', '$write_date', '-$bonus', '주문번호 : <a href=\"../stat/order_view.html?mart_id=$mart_id&order_num=$order_num_query\">$order_num_query</a> 주문취소', '$order_num')";
				$dbresult = mysql_query($SQL, $dbconn);
			}	
		}
		echo "
