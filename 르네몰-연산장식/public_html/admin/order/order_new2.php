...
if($update_flag == ''){
	...
	$method = "<a href='http://admin.kcp.co.kr/' target='_new'>[KCP 결제내역]</a>  <a href='http://pgweb.uplus.co.kr/' target='_new'>[LG U+ 결제내역]</a> <a href='https://admin.innopay.co.kr/mer/' target='_new'>[이노페이 결제내역]</a>";
...
if($flag == "today"){
	$SQL = "select * from $Order_BuyTable_Temp where mart_id='$mart_id' and status != '9' $status_str and date like '$today%' order by date desc";
}else if($flag == "search"){
	if($keyset == 'item_name'){
		$SQL = "select DISTINCT T1.* from $Order_BuyTable_Temp T1, $Order_ProTable T2 where T1.mart_id='$mart_id' and T1.status != '9' $status_str and T1.order_num=T2.order_num and binary T2.item_name like '%$searchword%' order by date desc";
	}else if($keyset == 'name'){
		$SQL = "select * from $Order_BuyTable_Temp where mart_id='$mart_id' and status != '9' $status_str and  binary name like '%$searchword%' order by date desc";
	}else if($keyset == 'tel'){
		$SQL = "select * from $Order_BuyTable_Temp where mart_id='$mart_id' and status != '9' $status_str and  (binary tel1 like '%$searchword%' or binary tel2 like '%$searchword%') order by date desc";
	}else{
		$SQL = "select * from $Order_BuyTable_Temp where mart_id='$mart_id' and status != '9' $status_str and binary $keyset like '%$searchword%' order by date desc";
	}
}else if($flag == 'find'){
	$SQL = "select * from $Order_BuyTable_Temp where mart_id='$mart_id' and status != '9' $status_str and ( date >= '$QryFromDate1' and date <= '$QryToDate1') order by date desc";
}else{
	$SQL = "select * from $Order_BuyTable_Temp where mart_id='$mart_id' and status != '9' $status_str order by date desc";
}			
...
?>
							...
							<input type='hidden' name='status_old[]' value='<?=$good_status?>'>
								...
								<td  bgcolor='#FFFFFF'><a href='order_detail2.php?index_no=<?=$ary[index_no]?>&page=<?=$page?>&keyset=<?=$keyset?>&searchword=<?=$searchword?>&flag=<?=$flag?>&cnfPagecount=<?=$cnfPagecount?>&QryFromDate=<?=$QryFromDate?>&QryToDate=<?=$QryToDate?>&status_flag=<?=$status_flag?>&mobile_chk_flag=<?=$mobile_chk_flag?>'><?=$order_num?></a></td>
...
?>
									<?=$status_temp_str?>
									<?
									if($ary[card_paid] == "t" && ($ary[status] == 1 || $ary[status] == 2) && $ary[pay_cancel] == "y"){
									echo "<br/><span style='color:#ff0000;'>결제취소요청</span>";
									}
									?>
<?
	}
			...
			<tr align="center">
				<td width="100%" bgcolor="#FFFFFF">
					...
					if($page == 1){
						...
						echo ("
						<a href='order_new2.php?page=1&keyset=$keyset&searchword=$searchword&flag=$flag&cnfPagecount=$cnfPagecount&QryFromDate=$QryFromDate&QryToDate=$QryToDate&status_flag=$status_flag&mobile_chk_flag=$mobile_chk_flag'>처음</a> 
						");
					}
				
					if($start_page > 1){
						echo ("
						<a href='order_new2.php?page=$prev_start_page&keyset=$keyset&searchword=$searchword&flag=$flag&cnfPagecount=$cnfPagecount&QryFromDate=$QryFromDate&QryToDate=$QryToDate&status_flag=$status_flag&mobile_chk_flag=$mobile_chk_flag'>
						◁&nbsp; 
						</a>
						");
					}
					...
					for($i=$start_page;$i<=$end_page;$i++){
						if($i == $page){
							...
							echo ("
						<a href='order_new2.php?page=$i&keyset=$keyset&searchword=$searchword&flag=$flag&cnfPagecount=$cnfPagecount&QryFromDate=$QryFromDate&QryToDate=$QryToDate&status_flag=$status_flag&mobile_chk_flag=$mobile_chk_flag'>$i</a> 
							");
						}
					}
					if($end_page < $total_page){
						echo ("
						<a href='order_new2.php?page=$next_start_page&keyset=$keyset&searchword=$searchword&flag=$flag&cnfPagecount=$cnfPagecount&QryFromDate=$QryFromDate&QryToDate=$QryToDate&status_flag=$status_flag&mobile_chk_flag=$mobile_chk_flag'>
						&nbsp;▷
						</a>
						");
					}
					...
					if($page == $total_page){
						...
						echo ("
						<a href='order_new2.php?page=$total_page&keyset=$keyset&searchword=$searchword&flag=$flag&cnfPagecount=$cnfPagecount&QryFromDate=$QryFromDate&QryToDate=$QryToDate&status_flag=$status_flag&mobile_chk_flag=$mobile_chk_flag'>끝</a> 
						");
					}
					?>
					</td>
...
if($update_flag == 'add_list_print'){
	...
	echo "<meta http-equiv='refresh' content='0; URL=order_new2.php?page=$page&keyset=$keyset&searchword=$searchword&flag=$flag&cnfPagecount=$cnfPagecount&QryFromDate=$QryFromDate&QryToDate=$QryToDate&status_flag=$status_flag&mobile_chk_flag=$mobile_chk_flag'>";
}
...
/**	//================== SMS DB 설정 파일을 불러옴 =======================================
	...
	for($i=0; $i<count($checkSel); $i++){
		$num = $checkSel[$i];
		$SQL = "update $Order_BuyTable_Temp set status='$status_all' where order_num='$num' and mart_id='$mart_id'";
		$dbresult = mysql_query($SQL, $dbconn);
		...
		if($status_old[$i] != '3' && $status_all == '3' && $bonus_ok == 't'){ //배송완료일때만, 포인트 사용할 때만 포인트 추가
			...
			if($numRows == 0){
				//구매자 id 알아내기
				$SQL = "select id from $Order_BuyTable_Temp where order_num = '$num' and mart_id='$mart_id'";
				$dbresult = mysql_query($SQL, $dbconn);
				$numRows = mysql_num_rows($dbresult);
				if($numRows > 0){
		...
		if( $status_old[$i] != '2' && $status_all == '2' ){  //입금완료시 ip, 시간 기록하기
			...
			$SQL = "update $Order_BuyTable_Temp set ipgum_date='$ipgum_date', ipgum_ip='$REMOTE_ADDR' where order_num = '$num' and mart_id='$mart_id'";
			$dbresult = mysql_query($SQL, $dbconn);
		}
		...
		if($status_old[$i] != '3' && $status_all == 3){ //배송완료일때만, 구매총액 추가
			$id = '';
			//구매자 정보 알아내기
			$SQL = "select id, tel2, name from $Order_BuyTable_Temp where order_num = '$num' and mart_id='$mart_id'";
			$dbresult = mysql_query($SQL, $dbconn);
			$numRows = mysql_num_rows($dbresult);
			$id = mysql_result($dbresult,0,0);
			...
			$SQL = "update $Order_BuyTable_Temp set send_date='$send_date' where order_num='$num' and mart_id='$mart_id'";
			$dbresult = mysql_query($SQL, $dbconn);
		}	
		...
		if($status_old[$i] == '3' && $status_all != 3){ //이전상태가 배송완료일때만, 구매총액 마이너스
			...
			$SQL = "update $Order_BuyTable_Temp set send_date='' where order_num='$num' and mart_id='$mart_id'";
			$dbresult = mysql_query($SQL, $dbconn);	
		}	
		...
		if(($status_old[$i] != '5' && $status_all == '5')||($status_old[$i] != '4' && $status_all == '4')){ //주문취소,환불시 보너스에서 빼기
			...
			//사용한 보너스 복구
			$SQL = "select id,use_bonus_tot from $Order_BuyTable_Temp where order_num = '$num' and mart_id='$mart_id'";
			$dbresult = mysql_query($SQL, $dbconn);
			$numRows = mysql_num_rows($dbresult);
