...
function really(order_num){
	if(confirm("\n정말로 삭제하시겟습니까?\n\n삭제하시면 현주문과 관련된 데이타가\n\n삭제되며 복구는 되지않습니다.")){
		window.location.href='order_detail.php?page=<?=$page?>&keyset=<?=$keyset?>&searchword=<?=$searchword?>&cnfPagecount=<?=$cnfPagecount?>&QryFromDate=<?=$QryFromDate?>&QryToDate=<?=$QryToDate?>&update_flag=delete_data&order_num='+order_num;
		return true;
	}
	return false; 
}
...
function order_update(f){
	f.action='order_update_test.php';
...
function jaego_back(){
	...
	if(conf == true)
	window.open("order_update_test.php?page=<?=$page?>&keyset=<?=$keyset?>&searchword=<?=$searchword?>&flag=<?=$flag?>&cnfPagecount=<?=$cnfPagecount?>&QryFromDate=<?=$QryFromDate?>&QryToDate=<?=$QryToDate?>&status_flag=<?=$status_flag?>&update_flag=jaego_back&order_num=<?=$order_num?>","0x0","top=3000,left=3000,width=0,height=0");
}
*/

function receipt_win(mart_id,order_num){
	var url = "../../market/receipt/receipt_test.php?page=<?=$page?>&keyset=<?=$keyset?>&searchword=<?=$searchword?>&flag=<?=$flag?>&cnfPagecount=<?=$cnfPagecount?>&QryFromDate=<?=$QryFromDate?>&QryToDate=<?=$QryToDate?>&status_flag=<?=$status_flag?>&mart_id="+mart_id+"&order_num="+order_num
...
function product_send_mail(order_num){
	var conf = confirm("배송완료메일을 보내시겠습니까?");
	if(conf == true)
	window.open("./product_send_mail_test.php?page=<?=$page?>&keyset=<?=$keyset?>&searchword=<?=$searchword?>&flag=<?=$flag?>&cnfPagecount=<?=$cnfPagecount?>&QryFromDate=<?=$QryFromDate?>&QryToDate=<?=$QryToDate?>&status_flag=<?=$status_flag?>&order_num="+order_num,"0x0","top=3000,left=3000,width=0,height=0");
}

function money_ok_mail(order_num){
	var conf = confirm("입금확인메일을 보내시겠습니까?");
	if(conf == true)
	window.open("./money_ok_mail_test.php?page=<?=$page?>&keyset=<?=$keyset?>&searchword=<?=$searchword?>&flag=<?=$flag?>&cnfPagecount=<?=$cnfPagecount?>&QryFromDate=<?=$QryFromDate?>&QryToDate=<?=$QryToDate?>&status_flag=<?=$status_flag?>&order_num="+order_num,"0x0","top=3000,left=3000,width=0,height=0");
}



function quantity_count(aa){
	/***** 수량 계산 START *****/
	var qtt = aa.value;	// 수량
	var qtt_total = qtt;	// 최종 입력될 수량
	var qtt_arr;	// 수량의 맨 앞자리 값
	if(qtt.substring(0,1) == 0){
		for(var ai=0; ai<qtt.length; ai++){	
			if(ai == 0){
				qtt_arr = qtt.substring(0,1);
				if(parseInt(qtt_arr) == 0){
					qtt_total = qtt.substring(1);
				}else{
					qtt_total = qtt;
					break;
				}
			}else{
				qtt_arr = qtt_total.substring(0,1);
				if(parseInt(qtt_arr) == 0){
					qtt_total = qtt_total.substring(1);
				}else{
					qtt_total = qtt_total;
				}
			}
...
<br>
			<span id="d1">
			<table border="1" cellpadding="5" cellspacing="0" width="90%" bordercolordark="white" bordercolorlight="#E1E1E1" align="center" style="word-break:break-all">
				<tr>
...
?>
			<tr>
				<td width="100%" bgcolor="#FFFFFF" height="35" align="center">
					...
					<input onclick="receipt_win('<?=$Mall_Admin_ID?>', '<?=$order_num?>')" style="BACKGROUND-COLOR: white; BORDER-BOTTOM: #7b7d7b 1px solid; BORDER-LEFT: #7b7d7b 1px solid; BORDER-RIGHT: #7b7d7b 1px solid; BORDER-TOP: #7b7d7b 1px solid; COLOR: #7b7d7b; HEIGHT: 18px; width=90" type="button" value="영수증출력">
<?
	if($jaego_back == '0'){
?>
...
?>
								<tr bgcolor="#FFFFFF" align="center">
									<td width="26%">상품명(포인트<?=$coupon_str?>)</td>
									<td width="10%">수 량</td>
									<td width="13%">가 격</td>
									<td width="14%">합 계</td>
									<td width="12%">상 태</td>
...
?>
								<tr bgcolor='#FFFFFF'>
									...
									<td><a onclick="window.open('../good/item_edit_old.php?item_no=<?=$item_no?>', 'mainpage','toolbar=no,width=700,height=600,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=no');" style='cursor:hand'><?=$item_name?>(<?=$bonus?><?=$cpntype_str?>) </a>
<?
		if(isset($opt)&&$opt!=""&&$opt!="!!"){
			...
			if($opts[0] != ""){
				...
				if($opt1_arr[1] != ""){
					echo $opt1_arr[0]."<BR>";
					$opt1_qtt_str = "옵션1 : <input type=\"text\" size=\"3\" id=\"opt1_quantityid[".$order_pro_no."]\" name=\"opt1_quantity[".$h."]\" value=\"".$opt1_quantity_total."\" style=\"IME-MODE:disabled;border: 1px solid rgb(136,136,136);\" onkeypress=\"return chkNum(event);\" onKeyDown=\"onOnlyNumber(this);\" onKeyUp=\"quantity_count(this);\"><BR>";
					$opt1_price_str = "옵션1 : ".number_format($opt1_price)."원"."<input type=\"text\" style=\"width:1px;border:1px solid #ffffff;cursor:arow;\" readonly>"."<BR>";
			...
			if($opts[1] != ""){
				...
				if($opt2_arr[1] != ""){
					echo $opt2_arr[0]."<BR>";
					$opt2_qtt_str = "옵션2 : <input type=\"text\" size=\"3\" id=\"opt2_quantityid[".$order_pro_no."]\" name=\"opt2_quantity[".$h."]\" value=\"".$opt2_quantity_total."\" style=\"IME-MODE:disabled;border: 1px solid rgb(136,136,136);\" onkeypress=\"return chkNum(event);\" onKeyDown=\"onOnlyNumber(this);\" onKeyUp=\"quantity_count(this);\"><BR>";
					$opt2_price_str = "옵션2 : ".number_format($opt2_price)."원"."<input type=\"text\" style=\"width:1px;border:1px solid #ffffff;cursor:arow;\" readonly>"."<BR>";
			...
			if($opts[2] != ""){
				...
				if($opt3_arr[1] != ""){
					echo $opt3_arr[0]."<BR>";
					$opt3_qtt_str = "옵션3 : <input type=\"text\" size=\"3\" id=\"opt3_quantityid[".$order_pro_no."]\" name=\"opt3_quantity[".$h."]\" value=\"".$opt3_quantity_total."\" style=\"IME-MODE:disabled;border: 1px solid rgb(136,136,136);\" onkeypress=\"return chkNum(event);\" onKeyDown=\"onOnlyNumber(this);\" onKeyUp=\"quantity_count(this);\"><BR>";
					$opt3_price_str = "옵션3 : ".number_format($opt3_price)."원"."<input type=\"text\" style=\"width:1px;border:1px solid #ffffff;cursor:arow;\" readonly>"."<BR>";
...
?>
									...
									<td align='center'>
									제품1 : <input type="text" size="3" id='quantityid[<?=$order_pro_no?>]' name="quantity[]" value='<?=$quantity?>' style="IME-MODE:disabled;border: 1px solid rgb(136,136,136);" onkeypress="return chkNum(event);" onKeyDown="onOnlyNumber(this);" onKeyUp="quantity_count(this);"><BR>
								  <?
								  echo $opt1_qtt_str;
								  echo $opt2_qtt_str;
								  echo $opt3_qtt_str;
								  ?>
									...
									<td align='left'>
									제품1 : <?=$z_price_str?>원<input type="text" style="width:1px;border:1px solid #ffffff;cursor:arow;" id='' name='' value='' readonly><BR>
								  <?
								  echo $opt1_price_str;
								  echo $opt2_price_str;
								  echo $opt3_price_str;
								  ...
								  ?>
									...
									<td align='center'><?=number_format($sum_total)?>원</td>
									<td align='center'>
										<select name="good_status[]" size="1" style="BORDER-BOTTOM: black 1px solid; BORDER-LEFT: black 1px solid; BORDER-RIGHT: black 1px solid; BORDER-TOP: black 1px solid; HEIGHT: 18px">
											<option value="1" <?if($good_status == 1) echo " selected";?>>주문</option>
											<option value="5" <?if($good_status == 5) echo " selected";?>>주문취소</option>
											<option value="2" <?if($good_status == 2) echo " selected";?>>입금확인</option>
											<option value="6" <?if($good_status == 6) echo " selected";?>>배송중</option>
											<option value="3" <?if($good_status == 3) echo " selected";?>>배송완료</option>
											<option value="7" <?if($good_status == 7) echo " selected";?>>교환</option>
											<option value="4" <?if($good_status == 4) echo " selected";?>>환불</option>
											</option>
										</select>									
									</td>
									<td align="center"><input name="pro_freight_code[]" size="15" value='<?=$pro_freight_code?>' class="input_03"></td>
									<td>
									
								<?
										$query_Freight = "select * from $Add_Freight_Name where 1 order by num asc";
...
<?
	...
	if($freight_fee == ''){
		if($tot_price_str >= $freight_limit){		// 배송료설정
			$freight_fee = 0;
		}else{
...
?>
								...
								<!--<tr>
									...
									<td width="65%" bgcolor="#FFFFFF" align="center" colspan="6">
...
배송비: <?=number_format($freight_fee)?>원<BR>
...
<?
$LGD_HASHDATA = md5("lets08078".$field1."d029110e594e9f1950c25ce6f0ce4e3c");	
?>
<?
if($paymethod== 'byonline' || $paymethod== 'byonline_point'){
?>									
	<input onclick="javascript:showCashReceipts('lets08078','<?=$order_num?>','seqno','CR','service')" style="BACKGROUND-COLOR: white; BORDER-BOTTOM: #7b7d7b 1px solid; BORDER-LEFT: #7b7d7b 1px solid; BORDER-RIGHT: #7b7d7b 1px solid; BORDER-TOP: #7b7d7b 1px solid; COLOR: #7b7d7b; HEIGHT: 18px; width=120" type="button" value="현금영수증 출력">
...
?>	
	<input onclick="javascript:showCashReceipts('lets08078','<?=$order_num?>','seqno','BANK','service')" style="BACKGROUND-COLOR: white; BORDER-BOTTOM: #7b7d7b 1px solid; BORDER-LEFT: #7b7d7b 1px solid; BORDER-RIGHT: #7b7d7b 1px solid; BORDER-TOP: #7b7d7b 1px solid; COLOR: #7b7d7b; HEIGHT: 18px; width=120" type="button" value="현금영수증 출력">
<?
}else{//카드결제
?>
	<input onclick="javascript:showReceiptByTID('lets08078', '<?=$field1?>', '<?=$LGD_HASHDATA?>')" style="BACKGROUND-COLOR: white; BORDER-BOTTOM: #7b7d7b 1px solid; BORDER-LEFT: #7b7d7b 1px solid; BORDER-RIGHT: #7b7d7b 1px solid; BORDER-TOP: #7b7d7b 1px solid; COLOR: #7b7d7b; HEIGHT: 18px; width=120" type="button" value="카드전표 출력">
<?}?>
<?###################################################################End 현금영수증 ####################################################################?>
							...
							<td width="90%" bgcolor="#999999">
								...
								<tr>
									...
									<td width="22%" bgcolor="#FFFFFF" align="left">
										<input name="email" size="25" value='<?=$email?>' class="input_03">
									</td>
								</tr>
								<tr>
...
?>
									...
									<td width="12%" bgcolor="#FFFFFF" align="center">접속구분</td>
									<td  width="22%" bgcolor="#FFFFFF" align="left" ><?if($mobile_chk=='y'){echo"모바일";}else{echo"PC";}?></td>
					...
					<input onclick="javascript:product_send_mail('<?=$order_num?>')" style="BACKGROUND-COLOR: white; BORDER-BOTTOM: #5a5a5a 1px solid; BORDER-LEFT: #5a5a5a 1px solid; BORDER-RIGHT: #5a5a5a 1px solid; BORDER-TOP: #5a5a5a 1px solid; COLOR: black; HEIGHT: 18px" type="button" value="배송완료메일">
					...
					<input onclick="javascript:printWin('order_print_test.php?order_num=<?=$order_num?>')" style="BACKGROUND-COLOR: white; BORDER-BOTTOM: #5a5a5a 1px solid; BORDER-LEFT: #5a5a5a 1px solid; BORDER-RIGHT: #5a5a5a 1px solid; BORDER-TOP: #5a5a5a 1px solid; COLOR: black; HEIGHT: 18px" type="button" value="프린트">
...
<?
	...
	?>	
			  ...
			  <form action='order_detail.php' method='post'>
			  <input type='hidden' name='update_flag' value='add_mymemo'>
			  <input type='hidden' name='order_num' value='<?=$order_num?>'>
			  ...
			  <tr>
				 <td width="100%" bgColor="#999999"><table cellSpacing="1" cellPadding="3" width="100%" border="0">
					...
					<tr>
					  <td align="middle" width="100%" bgColor="#ffffff" rowspan="3"><p align="left">
					  <textarea style="width: 90%"  class="input_03" name="content" rows="6" cols="93"><?=$content1?></textarea></td>
					</tr>
				 </table>
				 </td>
			  </tr>
...
if($update_flag == "delete_data"){
	...
	if($status!="5"&&$status!="4"&&$status!="10"){
	...
	echo "<meta http-equiv='refresh' content='0; URL=order_new.php?page=$page&keyset=$keyset&searchword=$searchword&cnfPagecount=$cnfPagecount&QryFromDate=$QryFromDate&QryToDate=$QryToDate'>";
}
?>
<?
mysql_close($dbconn);
