			 ...
			 $SQL = "select order_num from $Order_BuyTable where mart_id='$mart_id' and id = '$username' and status='3' order by index_no desc";
					...
					for ($i=0; $i < $numRows; $i++) {
						...
						for ($j=0; $j<$numRows1; $j++) {
							...
							$mon_tot += $sum; //합계금액
							$total_tax_price = $mon_tot * 0.1;
							$mon_tot = $mon_tot + $total_tax_price;
						}
					}
					$mon_tot_str = number_format($mon_tot);	
					?>
					<tr>
				<td width="100%" height="20"><p align="right">총 <?=$numRows?>건 <b><?=$mon_tot_str?> 원</b>의 
				배송완료 구매이력이 있습니다.<BR>※모두 부가세 포함 금액입니다</td>
			 </tr>
			 <tr>
				<td width="100%" bgcolor="#C0C0C0"><table border="0" width="100%" cellspacing="1">
				  ...
				  <?
							...
							";
										...
										$mon_tot = $mon_tot + $freight_fee + $tax_price;
									}
									$mon_tot_str = number_format($mon_tot);
					 ...
					 <b>$mon_tot_str 원</b>&nbsp;<BR>"; if($freight_fee > 0){echo"(배송비포함)";} echo" </font></td>
				  </tr>
						";
					}
				  }
