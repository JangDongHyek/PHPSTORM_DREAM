...
function really(index_no){
	if(confirm("\n정말로 삭제하시겟습니까?\n\n삭제하시면 현주문과 관련된 데이타가\n\n삭제되며 복구는 되지않습니다.")){
		window.location.href='order_detail.php?status_flag=<?=$status_flag?>&page=<?=$page?>&keyset=<?=$keyset?>&searchword=<?=$searchword?>&cnfPagecount=<?=$cnfPagecount?>&QryFromDate=<?=$QryFromDate?>&QryToDate=<?=$QryToDate?>&update_flag=delete_data&index_no='+index_no;
		return true;
	}
	return false; 
}
...
?>
								<tr bgcolor='#FFFFFF'>
									...
									<td><a onclick="window.open('../good/item_edit_old.php?item_no=<?=$item_no?>', 'mainpage','toolbar=no,width=700,height=600,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=no');" style='cursor:hand'><?=$item_name?>(<?=$item_code?>)<? 
										
												if(0<$bonus){?>(<?=$bonus?><?=$cpntype_str?>)<? }else{}?> </a>
					...
					?>	
									...
									?>
											<select name="pro_delivery[]">
												...
												<option value="롯데택배" <?if($pro_delivery =="롯데택배"){echo"selected";}?>>롯데택배</option>
												<option value="로젠택배" <?if($pro_delivery =="로젠택배"){echo"selected";}?>>로젠택배</option>
												<option value="동부택배" <?if($pro_delivery =="동부택배"){echo"selected";}?>>동부택배</option>
											<?
											...
											<!--로젠택배-->
											<!--<input type="hidden" name="pro_delivery[]" value="로젠택배">-->
									<?
									}else{
									?>
...
?>
								...
								<tr>
									...
									부가세 : <?=number_format($tax_price)?>원포함
									<BR>
<? if($freight_fee){ ?>
									<span class="text_red">배송료 <?=number_format($freight_fee)?>원포함</span>
<? } ?><BR>
...
if( mysql_num_rows($shop_res2) > 0 ){
	...
	$xpay_id = "jsbusan4122";
	$xpay_key = "5a0ac3c10d39bd08550df96e09bf54ea";
}
...
if($paymethod=="bycard" || $paymethod=="bycard_point")
{
	...
	$pay_info_str .= "KCP 거래번호 : ".$field1."<br>";
