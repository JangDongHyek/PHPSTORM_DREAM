...
function jaego_back(){
	...
	if(conf == true)
	window.open("order_update2.php?page=<?=$page?>&keyset=<?=$keyset?>&searchword=<?=$searchword?>&flag=<?=$flag?>&cnfPagecount=<?=$cnfPagecount?>&QryFromDate=<?=$QryFromDate?>&QryToDate=<?=$QryToDate?>&status_flag=<?=$status_flag?>&update_flag=jaego_back&order_num=<?=$order_num?>","0x0","top=3000,left=3000,width=0,height=0");
}
					...
					?>	
									...
									?>
											<select name="pro_delivery[]">
												...
												<option value="옐로우캡" <?if($pro_delivery =="옐로우캡"){echo"selected";}?>>옐로우캡</option>
