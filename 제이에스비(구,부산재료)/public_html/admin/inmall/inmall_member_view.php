...
if(isset($flag) == false) {
	...
	if($numRows > 0){
		...
		$me_delivery_price = $ary[me_delivery_price];
...
function really(){
	if(confirm("\n정말 입점몰을 삭제하시겠습니까?\n\n복구는 되지 않습니다.")){
    window.location.href='inmall_member_view.php?flag=del&username=<?=$username?>';
  }
				...
				<table border="1" cellpadding="5" cellspacing="0" width="90%" bordercolordark="white" bordercolorlight="#E1E1E1" align="center">
					...
					<td width="100%" bgcolor="#FFFFFF" height="3" valign="top">
						<table border="0" width="100%">
							<tr>
								<td width="100%" height="20"><p align="center"><b>[입점몰 상세정보 및 수정]</b></td>
							</tr>
						</table>
					</td>
					</tr>
...
if($flag == "update"){
	...
	$SQL = "update $MemberTable set name='$name', password='$password', email= '$email', tel1='$tel1', tel2='$tel2', zip='$zip', place='$place', place_detail='$place_detail', passport='$passport', message='$message', me_bank='$me_bank', me_bankno='$me_bankno', me_bankowner='$me_bankowner', me_delivery='$me_delivery', me_delivery_price='$me_delivery_price' where username='$username' and mart_id ='$mart_id'";
	$dbresult = mysql_query($SQL, $dbconn);
	if( $dbresult ){
		echo "<meta http-equiv='refresh' content='0; URL= inmall_member_view.php?username=$username'>";
	}else{
		echo ("
			<script>
			alert('수정하는데 실패했습니다.');
