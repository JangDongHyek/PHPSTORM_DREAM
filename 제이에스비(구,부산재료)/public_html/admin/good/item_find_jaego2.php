...
<form action='item_find_jaego2.php' method='post'>
							...
							<table cellSpacing="1" cellPadding="3" width="100%" bgColor="#f7f7f7" border="0">
							  ...
							  <?
								...
								if(!empty($keyset)&&!empty($searchword)){
									if($keyset == 'item_name')
									{
										$SQL = "select mart_id, item_no, item_name, item_company, z_price, reg_date, jaego, jaego_use, jaego_use,opt,opt2,opt3,opt4,if_opt_jaego,if_opt_jaego2,if_opt_jaego3,if_opt_jaego4
											from $ItemTable 
						 ...
						 <tr>
							<td width="91%" bgColor="#ffffff"><p align="right"><br>
								...
								if($page == 1){
									...
									echo ("
									<a href='item_find_jaego2.php?page=1&keyset=$keyset&searchword=$searchword&order=$order&cnfPagecount=$cnfPagecount'>처음</a> 
									");
								}
							
								if($start_page > 1){
									echo ("
									<a href='item_find_jaego2.php?page=$prev_start_page&keyset=$keyset&searchword=$searchword&order=$order&cnfPagecount=$cnfPagecount'>
									◁&nbsp; 
									</a>
									");
								}
								...
								for($i=$start_page;$i<=$end_page;$i++){
									if($i == $page){
										...
										echo ("
									<a href='item_find_jaego2.php?page=$i&keyset=$keyset&searchword=$searchword&order=$order&cnfPagecount=$cnfPagecount'>$i</a> 	
										");
									}
								}
								if($end_page < $total_page){
									echo ("
									<a href='item_find_jaego2.php?page=$next_start_page&keyset=$keyset&searchword=$searchword&order=$order&cnfPagecount=$cnfPagecount'>
									&nbsp;▷
									</a>
									");
								}
								...
								if($page == $total_page){
									...
									echo ("
									<a href='item_find_jaego2.php?page=$total_page&keyset=$keyset&searchword=$searchword&order=$order&cnfPagecount=$cnfPagecount'>끝</a> 
									");
								}
								?></td>
						 </tr>
...
if($flag == 'update'){
	...
	echo "<meta http-equiv='refresh' content='0; URL=item_find_jaego2.php?page=$next_start_page&keyset=$keyset&searchword=$searchword&order=$order&cnfPagecount=$cnfPagecount'>";
}
if($flag == 'diff'){
	for($i=0; $i<count($item_no); $i++) {
		...
		$mart_id_tmp = mysql_result($dbresult0,0,0);
			...
			$SQL = "update $ItemTable set jaego = jaego $p_m $amount, jaego_use='$jaego_use[$i]'
				,if_opt_jaego='$if_opt_jaego[$i]',if_opt_jaego2='$if_opt_jaego2[$i]'
				,if_opt_jaego3='$if_opt_jaego3[$i]',if_opt_jaego4='$if_opt_jaego4[$i]' where item_no = $item_no[$i] and mart_id='$mart_id'";
