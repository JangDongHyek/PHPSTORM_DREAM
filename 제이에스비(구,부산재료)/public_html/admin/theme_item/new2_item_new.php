    	...
    	<table border="1" cellpadding="5" cellspacing="0" width="90%" bordercolordark="white" bordercolorlight="#E1E1E1" align="center">
      	<tr>
        	<td width="90%" bgcolor="#FFFFFF" valign="top">
        		<strong><span class="cc">[신상품 등록]</span></strong> 등록되어진 제품 중 신상품으로 등록합니다.<br>등록한 신상품은 메인화면의 신상품영역에 출력됩니다<br>
					</td>
      	</tr>
      	<tr>
          		...
          		<tr>
            		<td width="100%" bgcolor="#CCCCCC">
            			<table border="0" width="100%" bgcolor="#F7F7F7" cellspacing="1" cellpadding="3">
              			...
              			<form action='new2_item_new.php' method='post' name='list'>
			  			<input type='hidden' name='flag' value='to_new_item'>
						<input type='hidden' name='keyset' value='<?=$keyset?>'>
<?
              			if($flag == 'search'){
              				if($keyset != 'category_num'){
          						$SQL = "select item_no, item_name, item_company, z_price, reg_date, jaego_use, jaego
          						...
          						UNION 
          						select T1.item_no, T1.item_name, T1.item_company, T1.z_price, T1.jaego_use, T1.jaego, T1.reg_date 
								...
								$cate_SQL = "select category_degree from $CategoryTable where category_num = $searchword";
          						...
          						$SQL = "select T1.mart_id, item_no, item_name, z_price, item_company, jaego_use, jaego, reg_date           						from $ItemTable T1,$CategoryTable T2 			          			where T1.if_hide='0' and T1.category_num=T2.category_num and T2.if_hide='0'			          			and T1.$item_keyset ='$searchword' 			UNION           						select T1.mart_id, T1.item_no, T1.item_name, T1.z_price, T1.item_company, T1.jaego_use, T1.jaego, T1.reg_date           						from $ItemTable T1, $Gnt_ItemTable T2           						where T2.seller_id = '$Mall_Admin_ID' and T1.item_no = T2.item_no and T2.$keyset ='$searchword'          						order by $order Asc";
          						}
            				...
            				<td width='38%' height='10' bgcolor='$bgcolor'>
            					<span class='bb'>$item_name $icon_str</span></td>
            						");
	            					if($i % 2 == 1)
	            						echo ("</tr><tr>");
...
if ($flag == "to_new_item"){
	for($i=0; $i<count($checkSel); $i++) {
		...
		$SQL = "select count(*) from $New2_ItemTable where mart_id='$mart_id' and item_no = $item_no";
		$dbresult = mysql_query($SQL, $dbconn);
		...
		$SQL = "select max(new_item_order), count(*) from $New2_ItemTable";
		$dbresult = mysql_query($SQL, $dbconn);

		if ($dbresult == false) echo "쿼리 실행 실패!";

		if (mysql_result($dbresult,0,1) > 0) 
			$maxNew_Item_order = mysql_result($dbresult, 0, 0);
		else
			$maxNew_Item_order = 0;
		
		$maxNew_Item_order = $maxNew_Item_order + 1;

		$SQL = "insert into $New2_ItemTable (item_no, firstno, prevno, category_num, mart_id, provider_id, new_item_order) values ( '$item_no', '$firstno', '$prevno', '$category_num', '$mart_id', '$provider_id', $maxNew_Item_order)";
		$dbresult = mysql_query($SQL, $dbconn);
		if ($dbresult == false) echo "쿼리 실행 실패!";
	}
