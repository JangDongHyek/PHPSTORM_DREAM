...
<br>
			...
			<table border="1" cellpadding="5" cellspacing="0" width="90%" bordercolordark="white" bordercolorlight="#E1E1E1" align="center">
				...
				<td width="90%" bgcolor="#FFFFFF" height="35">
					...
					인기상품로 등록합니다.<br>
					등록한 인기상품은 메인화면의 인기상품영역에 출력됩니다</td>
				</tr>
					...
					<table border="0" width="100%">
						...
						<tr>
							<td width="100%" bgcolor="#CCCCCC">
								...
								<form action='fav2_item_new.php' method='post' name='list'>
...
if ($flag == "to_fav_item"){
	for($i=0; $i<count($checkSel); $i++) {
		...
		$SQL = "select count(*) from $Fav2_ItemTable where mart_id='$mart_id' and item_no = $item_no";
		$dbresult = mysql_query($SQL, $dbconn);
		...
		$SQL = "select max(fav_item_order), count(*) from $Fav2_ItemTable";
		$dbresult = mysql_query($SQL, $dbconn);
		if ($dbresult == false) echo "쿼리 실행 실패!";
		...
		$SQL = "insert into $Fav2_ItemTable (item_no, firstno, prevno, category_num, mart_id, provider_id, fav_item_order) values ( '$item_no', '$firstno', '$prevno', '$category_num', '$mart_id', '$provider_id', $maxFav_Item_order)";
		$dbresult = mysql_query($SQL, $dbconn);
	}
...
if($flag=="del"){
	...
	if($target == 'fav'){
		$SQL = "delete from $Fav2_ItemTable where fav_item_no = $fav_item_no and mart_id='$mart_id'";
		$dbresult = mysql_query($SQL, $dbconn);
	}
	if($target == 'rec'){
...
if($flag == "up"){
	...
	if(!empty($fav_item_no)&&!empty($fav_item_order)){
		$SQL = "select fav_item_order from $Fav2_ItemTable where fav_item_order > $fav_item_order and mart_id='$mart_id' order by fav_item_order Asc";
		$dbresult = mysql_query($SQL, $dbconn);
		if ($dbresult == false) echo "쿼리 실행 실패!";
		$up_fav_item_order = mysql_result($dbresult,0,0);
		
		$SQL = "select fav_item_no from $Fav2_ItemTable where fav_item_order = $up_fav_item_order and mart_id='$mart_id'";
		$dbresult = mysql_query($SQL, $dbconn);
		if ($dbresult == false) echo "쿼리 실행 실패!";
		$up_fav_item_no = mysql_result($dbresult,0,0);
		
		$SQL = "update $Fav2_ItemTable set fav_item_order = $up_fav_item_order where fav_item_no = $fav_item_no and mart_id='$mart_id'";
		$dbresult = mysql_query($SQL, $dbconn);
		if ($dbresult == false) echo "쿼리 실행 실패!";
		
		$SQL = "update $Fav2_ItemTable set fav_item_order = $fav_item_order where fav_item_no = $up_fav_item_no and mart_id='$mart_id'";
		$dbresult = mysql_query($SQL, $dbconn);
		if ($dbresult == false) echo "쿼리 실행 실패!";
	}
...
if($flag == "down"){
	...
	if(!empty($fav_item_no)&&!empty($fav_item_order)){
		$SQL = "select fav_item_order from $Fav2_ItemTable where fav_item_order < $fav_item_order and mart_id='$mart_id' order by fav_item_order Desc";
		$dbresult = mysql_query($SQL, $dbconn);
		if ($dbresult == false) echo "쿼리 실행 실패!";
		$down_fav_item_order = mysql_result($dbresult,0,0);
		
		$SQL = "select fav_item_no from $Fav2_ItemTable where fav_item_order = $down_fav_item_order and mart_id='$mart_id'";
		$dbresult = mysql_query($SQL, $dbconn);
		if ($dbresult == false) echo "쿼리 실행 실패!";
		$down_fav_item_no= mysql_result($dbresult,0,0);
		
		$SQL = "update $Fav2_ItemTable set fav_item_order = $down_fav_item_order where fav_item_no = $fav_item_no and mart_id='$mart_id'";
		$dbresult = mysql_query($SQL, $dbconn);
		if ($dbresult == false) echo "쿼리 실행 실패!";
		
		$SQL = "update $Fav2_ItemTable set fav_item_order = $fav_item_order where fav_item_no = $down_fav_item_no and mart_id='$mart_id'";
		$dbresult = mysql_query($SQL, $dbconn);
		if ($dbresult == false) echo "쿼리 실행 실패!";
	}
