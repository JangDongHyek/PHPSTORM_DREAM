...
//		if($text_field=='')$text_field=$key_field;
	...
	function rg_get_site_cfg() {
		global $dbconn,$db_table_site_cfg;
	
		$dbqry = "
			SELECT *
				FROM `$db_table_site_cfg`
			";
		$rs = query($dbqry,$dbconn);
	...
	function rg_get_bbs_cfg($key,$type=0) {
		global $dbconn,$db_table_bbs_cfg;
	...
	function rg_get_group_cfg($key,$type=0) {
		global $dbconn,$db_table_group_cfg;
	...
	function rg_get_group_member_info($gr_key,$mb_key='',$type='') {
		global $dbconn,$db_table_group_member,$db_table_member;
		/*
		 $type 
		 	 0: 그룹아이디,회원아이디
			 1: 그룹아이디,회원번호
	...
	function rg_get_doc_info($bbs_id,$key,$type='0') {
		global $dbconn,$db_table_prefix,$db_table_suffix_body;
	...
	{
		global $dbconn,$db_table_memo,$db_table_member;
	...
	function rg_set_point($key,$point,$type='0') {
		global $dbconn,$db_table_member,$bbs,$site;
		$now = time();
	...
	function rg_get_connect_count($type='total',$key='') {
		global $dbconn,$db_table_connect;
...
/*
	...
	// 기존 password 함수와 동일한 알고리즘의 코드로 바뀌었다.
      ...
      // skipp space in password 
		...
		global $dbconn;
		// mysql서버의 버전을 구한다
		$rs = query("SHOW VARIABLES like 'version'",$dbconn);
		...
		if($mysql_version>3) { // 4.0 버전 이상이라면
			$rs = query("SELECT old_password('$str')",$dbconn);
		} else { // 3.xx 버전 이하라면
			$rs = query("SELECT password('$str')",$dbconn);
		}
...
**************************************************************************/
	function rg_lastest($skin='',$bbs='',$bbs_title='',$list=10,
		...
		global $dbconn,$site_url,$site_path,$skin_path,$skin_url,$skin_lastest_dir,
	...
	function rg_outlogin($skin='',$url=NULL) {
		global $dbconn,$site_url,$skin_path,$skin_url,$skin_outlogin_dir,
	...
	function rg_bbs_doc_del($bbs_id,$rg_doc_num) {
		global $dbconn,$data_path,$bbs_cfg,
		...
		// 파일을 지운다.
		for($fi=1;$fi<3;$fi++) {
			$del_file = $rg_doc_num.'$'.$fi.'$'.$data["rg_file{$fi}_name"];
			@unlink($bbs_data_path.$del_file);
      $del_file = $rg_doc_num.'$'.$fi.'$th$'.$data["rg_file{$fi}_name"]; 
      @unlink($bbs_data_path.$del_file); 
		}
	...
	function rg_bbs_doc_copy($target_bbs_id,$source_bbs_id,$source_rg_doc_num) {
		...
		for($i=1;$i<3;$i++) {
			if(${"rg_file{$i}_name"}) {
				${"rg_file{$i}_src_server_name"} = 
				        $source_rg_doc_num."\$$i\$".${"rg_file{$i}_name"};
				${"rg_file{$i}_tar_server_name"} = 
				        $rg_doc_num."\$$i\$".${"rg_file{$i}_name"};
				if(!@copy($src_bbs_data_path.${"rg_file{$i}_src_server_name"},
				          $tar_bbs_data_path.${"rg_file{$i}_tar_server_name"})) {
					${"rg_file{$i}_name"} = '';
					${"rg_file{$i}_size"} = 0;
				}
			}
	...
	function rg_vote_preview($skin,$vote_popup) {
		global $dbconn,$site_url,$skin_path,$skin_url,$skin_vote_preview_dir,
