...
if($flag == "addorder"){
	...
	$SQL = "select order_pro_no, quantity from $Order_ProTable where order_num='$order_num' and mart_id='$mart_id' and item_no='$item_no' and opt='$opt' and opt='$opt1_5' and status = '0'";
	$dbresult = mysql_query($SQL, $dbconn);
	$numRows = mysql_num_rows($dbresult);
	if ($numRows > 0) {
		...
		$SQL = "insert into $Order_ProTable (order_num, mart_id, item_no, item_name, item_code, opt, opt1_5, z_price, quantity, bonus, use_bonus, status, date, provider_id, provide_price,opt2,opt3,opt4,opt_price,opt_price2,opt_price3,opt_price4) values ('$order_num', '$mart_id', $item_no, '$item_name', '$item_code', '$opt', '$opt1_5', $z_price, $quantity, $bonus, $use_bonus, '0', '$cur_date', '$provider_id', '$provide_price','$opt2','$opt3','$opt4','$opt_price','$opt_price2','$opt_price3','$opt_price4')";		
		$dbresult = mysql_query($SQL, $dbconn);
	}
