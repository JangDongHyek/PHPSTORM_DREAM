...
while($rs=mysql_fetch_array($result)){
	...
	if($jaego<$quantity&&$jaego_use=="1"&&$jaego){
		...
		echo "alert('추가 구매를 원하시면 사무실 문의 바랍니다.');";
		echo "history.go(-1);";
		echo "</script>";
		exit;
	}
...
if($paymethod == 'bycard' || $paymethod == 'bycard_point' ){
	...
	echo "
		...
		<form name='FormA'  method='post' action ='http://www.jsbusan.com/market/cart/card_account.html?order_num=$order_num&mart_id=$mart_id&category_num=$category_num&category_num1=$category_num1&category_num2=$category_num2&cate_num=$cate_num&flag=$flag&item_no=$item_no&provider_id=$provider_id&paymethod=$paymethod'>
		<!-- 결제를 위한 필수 hidden정보 -->
...
if($paymethod == 'byaccount' || $paymethod == 'byaccount_point' ){
	...
	echo "
		...
		<form name='FormA'  method='post' action ='http://www.jsbusan.com/market/cart/card_account1.html?order_num=$order_num&mart_id=$mart_id&category_num=$category_num&category_num1=$category_num1&category_num2=$category_num2&cate_num=$cate_num&flag=$flag&item_no=$item_no&provider_id=$provider_id&paymethod=$paymethod'>
		<!-- 결제를 위한 필수 hidden정보 -->
		<input type='hidden' name='OrderID' value='$order_num'>
		<input type='hidden' name='Amount' value='$mon_tot_freight'>
