...
while($rs=mysql_fetch_array($result)){
	...
	if($jaego<$quantity&&$jaego_use=="1"&&$jaego){
		...
		$opt_ea=$rs4[opt_ea];
		$opt_name=$rs4[opt_name];
		
		if($opt_ea<$quantity&&$if_opt_jaego4&&$rs4[opt4]){
			echo "
			<script>
...
if(substr($address,0,4)=="제주"){
	...
	echo ("
		<script language=javascript>
			if(!confirm('배송지 주소 : 제주도는  배송비(3,850원) 추가 지역입니다.\\n\\n그래도 주문하시겠습니까?'))
				history.back();
		</script>
	");
	$mon_tot_freight=$mon_tot_freight+3850;
	$freight_fee=$freight_fee+3850;
}else{
