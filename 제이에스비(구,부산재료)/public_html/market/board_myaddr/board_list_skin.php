						<table width='96%' align="center"  border='0' cellspacing='0' cellpadding='0'>
							<tr>
								...
								<td width='100' align='center' background='../image/helpdesk/table1_bg.gif'>배송지</td>
								<td width='1'><img src='../image/helpdesk/table1_line.gif' width='1' height='40'></td>
								<td width='60' align='center' background='../image/helpdesk/table1_bg.gif'>수령인</td>
								<td width='1'><img src='../image/helpdesk/table1_line.gif' width='1' height='40'></td>
								<td width='100' align='center' background='../image/helpdesk/table1_bg.gif'>전화번호</td>
								<td width='1'><img src='../image/helpdesk/table1_line.gif' width='1' height='40'></td>
								<td align='center' background='../image/helpdesk/table1_bg.gif'>주소</td>
								<td width='1'><img src='../image/helpdesk/table1_line.gif' width='1' height='40'></td>
								<td width='60' align='center' background='../image/helpdesk/table1_bg.gif'>관리</td>
...
<?
	...
	$SQL1 = "select count(*) from $New_BoardTable where username='$UnameSess' and bbs_no = '$bbs_no' and mart_id = '$mart_id' "; 
	...
	$SQL1 = "select * from $New_BoardTable where username='$UnameSess' and bbs_no = '$bbs_no' and mart_id = '$mart_id' "; 
	$SQL2 = "and binary $keyset like '%$searchword%' ";
	$SQL2_1 = "and (binary writer like '%$searchword%' or binary subject_new like '%$searchword%' or binary content like '%$searchword%') ";
...
?>
							<tr height='30'>
								<td colspan='5' align='center'><b>등록된 주소가 없습니다</b></td>
							</tr>
							<tr>
...
?>
							<tr height='30' align='center'>
								<td width='110' class='point'><span style="cursor:hand;" onclick="val_sel('<?=$subject_new?>','<?=$address?>','<?=$address_d?>','<?=$tel?>','<?=$mobile?>','<?=$zip?>')"><?=$subject_new?></span></td>
								<td width='60' class='help' align='center'><?=$member_img?></td>
								<td width='100'><?=$mobile?></td>
								<td class='point'>[<?=$zip?>]<?=$address?> <?=$address_d?></td>
								<td width='60' class='point' align='center'><a href='board_read.php?index_no=<?=$index_no?>&bbs_no=<?=$bbs_no?>&page=<?=$page?>&mart_id=<?=$mart_id?>&keyset=<?=$keyset?>&searchword=<?=$searchword?>'>[수정]</a></td>
							</tr>
							<tr>
								<td bgcolor='E1E1E1' height='1' colspan='5'></td>
							</tr>
