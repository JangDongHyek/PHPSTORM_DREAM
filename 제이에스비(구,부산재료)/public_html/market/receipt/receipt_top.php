...
<?
$SQL = "select * from $Order_ProTable where mart_id='$mart_id' and order_num = '$order_num'  order by order_pro_no desc";
$dbresult = mysql_query($SQL, $dbconn);
...
?>
      ...
      <tr>
        ...
        <td width="50%" style="border-bottom: thin"><p style="padding-left: 5px"><span class="aa"><?=number_format($mon_tot+$tax_price)?>원</span></td>
      </tr>
      <tr>
    ...
    bordercolor="#7D7D7D" cellspacing="0" cellpadding="0" style="border: thin">
      ...
      <tr>
        ...
        <td width="71%" style="border-top: thin; border-bottom: thin"><p style="padding-left: 5px"><span class="aa"><?=number_format($mon_tot+$tax_price)?>원</span></td>
      </tr>
      <tr>
  ...
  <tr>
    ...
    cellpadding="0" bordercolor="#7D7D7D" style="border: thin">
      <tr>
        ...
        <td width="19%" align="center"><span class="aa">단가</span></td>
        <td width="10%" align="center"><span class="aa">수량</span></td>
        <td width="12%" align="center"><span class="aa">금액</span></td>
      </tr>
      <?
      	$SQL = "select * from $Order_ProTable where mart_id='$mart_id' and order_num = '$order_num'  order by order_pro_no desc";
		//echo "sql=$SQL";
		...
		for ($i=0; $i<$numRows; $i++) {
			...
			$tax_price1 = ($z_price) * 0.1;
			
			$z_price = $z_price + $tax_price1;
			$bonus = $ary[bonus];
        ...
        <td width='19%' align='center'><span class='aa'><?=$z_price_str?><BR>(부가세:<?=$tax_price1?>원포함)<BR>(옵션가격:<?=$opt_tot?>)</span></td>
        <td width='10%' align='center'><span class='aa'><?=$quantity?></span></td>
        <td width='12%' align='center'><span class='aa'><?=$sum_str?></span></td>
      </tr>
     <?
      }
      if($freight_fee == ''){
		...
		?>	
		 <tr>
        <td width="100%" colspan="5" height="25"><p align="right"><span class="aa"><b>총금액:<?=number_format($total_sum)?>원</b>(배송비:<?=number_format($freight_fee)?>원포함)
        &nbsp;&nbsp; </span></td>
      </tr> 
    </table>
    </td>
