...
if($_FILES['upimage']['name'] && $_FILES['upimage']['size']>0)
{
    ...
    if($result)
    {
       echo "
		...
		opener.document.getElementById('".$op_id."').contentWindow.document.body.innerHTML;
        opener.document.getElementById('".$op_id."').contentWindow.document.body.innerHTML=opener.document.getElementById('".$op_id."').contentWindow.document.body.innerHTML+str;
		 opener.document.getElementById('".$op_id."').contentWindow.document.body.focus();
		 self.close();
        </script>
        ";
    }
    ...
    {
        echo "<script>alert('이미지 첨부 에러입니다!'); 
               self.close(); 
               </script>";
    }
