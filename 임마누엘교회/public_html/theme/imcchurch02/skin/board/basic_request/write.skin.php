...
$subject=$_GET[subject]?$_GET[subject]:$write[wr_subject];
?>
...
<section id="bo_w">
    ...
    <div class="tbl_frm01 tbl_wrap">
		...
		<tr>
            ...
            <td><input type="text" name="wr_1" value="<?php echo $wr_1 ?>" id="wr_1" required class="frm_input required" size="10" maxlength="20" oninput="autoHyphen2(this)"></td>
        </tr>
		...
		<tr>
			<th scope="row">선택한부품</th>
            <td class="pro_li">
				<?php
					$total=0;
					$wIdx=str_replace("|",",",substr($partIdx,0,strlen($partIdx)-1));
					$sql="select * from g5_part where idx in ($wIdx)";
					$result=sql_query($sql);
					for($i=0;$row=sql_fetch_array($result);$i++){
						$total+=$row[partprice];
				?>
				<span>
					<img src="<?php echo G5_DATA_URL ?>/file/part/image/<?php echo $row[partimg]?>" alt="">
					<p><?php echo $row[partname]?></p>
				</span>
				<?php }?>
				<input type="hidden" name="wr_2" id="wr_2" value="<?php echo $partIdx?>">
			</td>
		</tr>
		<tr>
			<th scope="row">가격</th>
			<td class="price_li">
				<span><?php echo number_format($total)?></span>원
				<input type="hidden" name="wr_3" value="<?php echo $total?>">
			</td>
		</tr>
