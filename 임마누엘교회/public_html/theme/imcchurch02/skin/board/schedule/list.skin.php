...
$col_height= 160 ;//내용 들어갈 사각공간의 세로길이를 가로 폭과 같도록
$today = getdate(); 
$b_mon = $today['mon']; 
$b_day = $today['mday']; 
$b_year = $today['year']; 
...
<table width="<?php echo $width; ?>" border=0 cellpadding="0" cellspacing="0">
    <tr>
        ...
        <td width="20%" align="right">
            ...
            <?php if ($admin_href) { ?><li><a href="<?php echo $admin_href ?>" class="btn_admin2 btn2"><font color="#ffffff">관리자</font></a></li><?php } ?>
...
<table width="<?php echo $width; ?>" bgcolor="#cfcfcf" border="0" cellspacing="1" cellpadding="5">
    ...
    while ($row = sql_fetch_array($result)) {  // 제목글 뽑아서 링크 문자열 만들기..
        ...
        for ($i = $start_day; $i <= $end_day;  $i++) {
			...
			$html_day[$i].= "<span class='day'><img src=\"{$board_skin_url}/img/{$imgown}.gif\" align=\"absmiddle\"> <a href=\"".G5_BBS_URL."/board.php?bo_table={$bo_table}&year={$year}&month={$month}&wr_id={$row['wr_id']}&sc_no={$sc_no}\" id=\"subject_{$j}\" {$showLayer}>{$row['wr_subject']}</a></span>";
			?>
			<!-- 뷰 팝업레이어 -->
		...
		for ($iz = 1; $iz <= $lastcount; $iz++) { // 42번을 칠하게 된다.
			...
			if ($b_year==$year && $b_mon==$month && $b_day==$cday) $bgcolor = "#f8faff";
			if (($iz%7) == 1) echo ("        <tr>\n"); // 주당 7개씩 한쎌씩을 쌓는다.
			...
			if ($myarray['day']==1 || $myarray['day']==11 || $myarray['day']==21) {
				$moonday ="<font color=\"gray\" class='hidden-xs'>&nbsp;(음){$myarray['month']}.{$myarray['day']}{$myarray['leap']}</font>";
			} else {
				$moonday="";
			}
			...
			if ($write_href) { 
				...
				if ($config['cf_bbs_rewrite']=='1' || $config['cf_bbs_rewrite']=='2') {
		...
		else { echo ("            <td width=\"{$col_width}\" height=\"{$col_height}\" bgcolor=\"#f8f8f8\" valign=\"top\">&nbsp;</td>\n"); }
		if (($iz%7) == 0) echo ("        </tr>\n");
