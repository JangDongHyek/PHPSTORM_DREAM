...
// *-- BBS_LIB_INC_INCLUDED START --*
	...
	require_once "{$site_path}include/mobile.lib.inc.php";
  ...
  if(in_array($mb[mb_id],$tmp) && !empty($_SESSION[ss_mb_id])) {
	...
	// 스킨 셋업
	if (file_exists($skin_board_path."mobile_setup.php")) 
    include($skin_board_path."mobile_setup.php");
...
//	for($i=0;$i < count($ss_key);$i++) {
	...
	// 링크변수정의
	$u_category = "$PHP_SELF?bbs_id=$bbs_id&ss[fc]=";
	$u_navigation = "$PHP_SELF?$p_str&bbs_id=$bbs_id&page=";
	$u_navigation2 = "./mobile_list2.php?$p_str&bbs_id=$bbs_id&page=";
	$u_search = "$PHP_SELF?bbs_id=$bbs_id";
	$u_view = "./mobile_view.php?$p_str&bbs_id=$bbs_id&page=$page&doc_num=";
	$u_images_view = "{$site_url}images_view.php?$p_str&bbs_id=$bbs_id&doc_num=";

	$u_comment_write = "{$site_url}mobile_comment.php?$p_str&bbs_id=$bbs_id&page=$page&type=write&doc_num=$doc_num";
	$u_comment_delete = "{$site_url}mobile_comment.php?$p_str&bbs_id=$bbs_id&page=$page&type=delete&cmt_num=";

	$u_all_list = "{$site_url}list.php?bbs_id=$bbs_id";
	$a_all_list = "<a href=\"$u_list\" $class[link_bbs]>";
	$u_list = "{$site_url}mobile_list.php?$p_str&bbs_id=$bbs_id&page=$page";
	$a_list = "<a href=\"$u_list\" $class[link_bbs]>";
	
	if($auth[bbs_edit]) {
		$u_edit = "mobile_edit.php?$p_str&bbs_id=$bbs_id&page=$page&doc_num=";
	...
	//echo $auth[bbs_write];
	if($auth[bbs_write]) {
		$u_write = "mobile_write.php?bbs_id=$bbs_id";
	...
	if($auth[bbs_delete]) {
		$u_delete = "mobile_delete.php?$p_str&bbs_id=$bbs_id&page=$page&doc_num=";
	...
	if($auth[bbs_reply]) {
		$u_reply = "mobile_reply.php?$p_str&bbs_id=$bbs_id&page=$page&doc_num=";
	...
	if($auth[admin]) {
		$a_setup="<a href={$site_url}admin/board_edit.php?mode=$mode&bbs_num=$bbs[bbs_num] $class[link_header] target=\"_blank\">";
