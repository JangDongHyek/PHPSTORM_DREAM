<?
	...
	if(!$auth[bbs_read]) {
		if( $mb && ($HTTP_SESSION_VARS[ss_login_ok]=='ok') && !empty($HTTP_SESSION_VARS[ss_mb_id]))
		{
			...
			echo"<script>location.href='../bbs/mb_login.php?url=./board_list.htm?bbs_id=$bbs_id';</script>";
			exit;
		}
...
//	$data=rg_get_doc_info($bbs_id,$doc_num);
	...
	$u_list = "./mobile_list.php?$p_str&bbs_id=$bbs_id&page=$page&doc_num=$doc_num";
...
/************************************/
	...
	include($skin_board_path."mobile_view_main.php");
...
/****************************************************************************/
	...
	include($skin_board_path."mobile_view_cmt_head.php");
	...
	while ($cdata=mysql_fetch_array($rs)) {
		...
		include($skin_board_path."mobile_view_cmt_main.php");
	}
	if($mb) {
		$cmt_name = $mb[mb_show_name];
		$cmt_email = $mb[mb_email];
	...
	include($skin_board_path."mobile_view_cmt_foot.php");
	include($skin_board_path."mobile_view_foot.php");
