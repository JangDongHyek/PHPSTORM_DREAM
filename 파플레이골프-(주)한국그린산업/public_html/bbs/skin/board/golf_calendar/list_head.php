...
<TABLE cellspacing=0 cellpadding=0 border=0  width=100%>
	<tr>
        ...
        <td align="left" style="font-weight:bold; font-size:12px;"> <img src=<?=$skin_board_url?>images/arrow_1.gif border=0 align=absmiddle> 
          <?=$year?>
          년 
          <?=$month?>
          월의 일정</td>
					...
					<a href="javascript:;" onclick="window.open('./admin/board_category2.php?bbs_id=golf','','width=400,height=600,scrollbars=1')">[시간설정]</a>
					<? }?>
	...
	<tr>
		<td valign="top">
				  <table cellpadding="0" cellspacing="1" border="0" bgcolor=<?=$table_line?> width="99%">
				    <tr bgcolor="#FFFFFF">
					  <td height="20">
						  ...
						  <tr>
							...
							$sql="select * from $bbs_table where rg_ext5='$book'";
										...
										<?}else{
											...
											<img src="<?=$skin_board_path?>images/not.gif" alt=예약불가>
											<? }else{
											$book_2 = rg_date($book,'%Y-%m-%d');
											$to_day = time();
											$to_day_2 = rg_date($to_day,'%Y-%m-%d');
											if($book_2 == $to_day_2){
											?>
											<img src="<?=$skin_board_path?>images/not.gif" alt=예약불가>
											<?
										...
										<B>< 휴 장 ></B>
										<? }else{ ?>
										<a href="time_list.php?bbs_id=<?=$bbs_id?>&year=<?=$year?>&month=<?=$month?>&day=<?=$day?>&book=<?=$book?>"><img src="<?=$skin_board_path?>images/yes.gif" alt=예약가능 border=0></a>
										<? } ?>
										<?}}}?>
							</td>
						  </tr>
								...
								for($s=0;$s<sizeof($seong_data);$s++)	{
									if($seong_data[$s][rg_ext5]==$book)	{
										...
										$rg_mb_num= $seong_data[$s][rg_mb_num];
										...
										if($_SESSION[ss_mb_num]==$rg_mb_num||$_SESSION[ss_mb_level]==10){
											$a_href="<a href='view.php?$p_str&bbs_id=$bbs_id&doc_num=$doc_num2&year=$year&month=$month&day=$day&' title='$rg_content'>{$img}";
										}else{
											$a_href="{$img}";
										}
