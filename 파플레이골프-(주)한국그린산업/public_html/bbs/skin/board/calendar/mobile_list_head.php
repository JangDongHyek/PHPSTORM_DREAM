...
<TD valign=bottom>
		<? include ($skin_board_path."mobile_today_schedule.php"); ?>
</TD>
</TR>
</TABLE>
<br>
		...
		<td height="26" valign="top" align="center">
			<table cellpadding="0" cellspacing="0" border="0" width=100%>
				<tr>
			        ...
			        <a href="mobile_list.php?bbs_id=<?=$bbs_id?>&year=<?=$prevyeary?>&month=<?=$pmonth?>"><img src=<?=$skin_board_url?>images/prev_year.gif border=0 align=absmiddle alt="<?=$prevyeary?>년 <?=$pmonth?>월 보기"></a>
					<a href="mobile_list.php?bbs_id=<?=$bbs_id?>&year=<?=$prevyear?>&month=<?=$prevmonth?>"><img src=<?=$skin_board_url?>images/prev_month.gif border=0 align=absmiddle alt="<?=$prevyear?>년 <?=$prevmonth?>월 보기"></a>
					<a href="mobile_list.php?bbs_id=<?=$bbs_id?>&year=<?=$thisyear?>&month=<?=$thismonth?>"><img src=<?=$skin_board_url?>images/this_month.gif border=0 align=absmiddle alt="<?=$thisyear?>년 <?=$thismonth?>월 보기"></a>
					<a href="mobile_list.php?bbs_id=<?=$bbs_id?>&year=<?=$nextyear?>&month=<?=$nextmonth?>"><img src=<?=$skin_board_url?>images/next_month.gif border=0 align=absmiddle alt="<?=$nextyear?>년 <?=$nextmonth?>월 보기"></a>
					<a href="mobile_list.php?bbs_id=<?=$bbs_id?>&year=<?=$nextyeary?>&month=<?=$pmonth?>"><img src=<?=$skin_board_url?>images/next_year.gif border=0 align=absmiddle alt="<?=$nextyeary?>년 <?=$pmonth?>월 보기"></a>
					...
					<td valign="top" height=<?=$height?> width='<?=$width1?>' <?=$today_bg?>>
						<table cellpadding="0" cellspacing="0" border="0" width=99% style="table-layout:fixed;">
						  <tr>
							...
							<?=$show_write_begin?>
							<a href="mobile_write.php?bbs_id=<?=$bbs_id?>&book=<?=$book?>">							<?=$show_write_end?>
							...
							<?=$show_write_end?>
								...
								for($s=0;$s<sizeof($seong_data);$s++)	{
									if($seong_data[$s][rg_ext5]==$book)	{
										...
										echo("<tr><td width=100% colspan=2 style=\"padding:2;color:666666;font-size:8pt;\"><a href='mobile_view.php?$p_str&bbs_id=$bbs_id&doc_num=$doc_num2&year=$year&month=$month&day=$day&' title='$rg_content'>{$img}");
										echo($rg_title."</a> ");
										if($cmt_count!=0)
