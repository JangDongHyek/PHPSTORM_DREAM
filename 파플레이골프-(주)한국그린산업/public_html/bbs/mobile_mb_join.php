...
//		if($url==NULL) $url = $HTTP_SERVER_VARS['REQUEST_URI'];
	...
	if($act=='ok') {
		if($mb_password=='') {
			$error_msg = '암호를 입력해주세요.';
			echo"<script>history.go(-1);</script>";
			exit;
		}

        if($tmp = rg_str_inword("url,http,href,www,link",$mb_greet)) {
			echo ("
					<script language=javascript>
							alert(\"$tmp (은)는 사용할수 없는 단어입니다.\");
					</script>
					<form name='form' action='mb_join.php' method='post'>
						<input type='hidden' name='mb_id' value='$mb_id'>
						<input type='hidden' name='mb_name' value='$mb_name'>
						<input type='hidden' name='mb_nick' value='$mb_nick'>
						<input type='hidden' name='mb_homepage' value='$mb_homepage'>
						<input type='hidden' name='mb_tel[0]' value='$mb_tel[0]'>
						<input type='hidden' name='mb_tel[1]' value='$mb_tel[1]'>
						<input type='hidden' name='mb_tel[2]' value='$mb_tel[2]'>
						<input type='hidden' name='mb_mobile[0]' value='$mb_mobile[0]'>
						<input type='hidden' name='mb_mobile[1]' value='$mb_mobile[1]'>
						<input type='hidden' name='mb_mobile[2]' value='$mb_mobile[2]'>				
						<input type='hidden' name='mb_email' value='$mb_email'>
						<input type='hidden' name='mb_post' value='$mb_post'>
						<input type='hidden' name='mb_address1' value='$mb_address1'>
						<input type='hidden' name='mb_address2' value='$mb_address2'>
						<input type='hidden' name='mb_greet' value='$mb_greet'>
					</form>
					<script>
...
//			session_register("ss_login_ok");
	...
	include($skin_site_path."mobile_head.php");
	include($skin_site_path."mobile_mb_join.php");
	include($skin_site_path."mobile_foot.php");
?>