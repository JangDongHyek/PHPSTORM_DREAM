  ...
  <table width="796" border="1" align="center" cellpadding="0" cellspacing="0" bordercolorlight="#E1E1E1" bordercolordark="white">
    ...
    <tr> 
      <td align="right" bgcolor="#f7f7f7"> 
        <?=($need_id)?'*':''?>
        아이디&nbsp;:&nbsp;</td>
      <td> <input type=text name='mb_id' size=20 maxlength=20 minlength=2 itemname='아이디' required value="<?=$mem[mb_id]?>"> 
    ...
    <tr> 
      <td align="right" bgcolor="#f7f7f7"> 
        <?=($need_password)?'*':''?>
        암호&nbsp;:&nbsp;</td>
      <td> <input name='mb_password' type=password id="mb_password3" size=20 maxlength=20 itemname='암호'> 
      </td>
    </tr>
    <?=$show_nick_begin?>
    <tr> 
      <td align="right" bgcolor="#f7f7f7"> 
        <?=($need_nick)?'*':''?>
        닉네임&nbsp;:&nbsp;</td>
      <td><input name='mb_nick' type=text id="mb_nick" value='<?=$mem[mb_nick]?>' size=20 maxlength=20 itemname='닉네임'> 
    ...
    <tr> 
      <td align="right" bgcolor="#f7f7f7"> 
        <?=($need_name)?'*':''?>
        이름&nbsp;:&nbsp;</td>
      <td><input type=text name='mb_name' size=20 maxlength=20  itemname='이름' value='<?=$mem[mb_name]?>'> 
      </td>
    </tr>
    <?=$show_name_end?>
    <?=$show_email_begin?>
    <tr> 
      <td align="right" bgcolor="#f7f7f7"> 
        <?=($need_email)?'*':''?>
        e-mail&nbsp;:&nbsp;</td>
      <td><input type=text name='mb_email' size=40 maxlength=100 itemname='e-mail' value='<?=$mem[mb_email]?>'> 
      </td>
    </tr>
    <?=$show_email_end?>
    <?=$show_homepage_begin?>
    <tr> 
      <td align="right" bgcolor="#f7f7f7"> 
        <?=($need_homepage)?'*':''?>
        홈페이지&nbsp;:&nbsp;</td>
      <td><input type=text name='mb_homepage' size=40 maxlength=255 itemname='홈페이지' value='<?=$mem[mb_homepage]?>'> 
      </td>
    </tr>
    <?=$show_homepage_end?>
    <?=$show_jumin_begin?>
    <tr> 
      <td align="right" bgcolor="#f7f7f7"> 
        <?=($need_jumin)?'*':''?>
        주민등록번호&nbsp;:&nbsp;</td>
      <td><input type=text name='mb_jumin1' size=7 maxlength=6 itemname='주민등록번호 앞자리' onkeyup='if (this.value.length >= 6) this.form.mb_jumin2.focus();'>
        - 
    ...
    <tr> 
      <td align="right" bgcolor="#f7f7f7"> 
        <?=($need_tel)?'*':''?>
        전화번호&nbsp;:&nbsp;</td>
      <td><input type=text name='mb_tel' size=21 maxlength=20 itemname='전화번호' value='<?=$mem[mb_tel]?>'> 
      </td>
    </tr>
    <?=$show_tel_end?>
    <?=$show_mobile_begin?>
    <tr> 
      <td align="right" bgcolor="#f7f7f7"> 
        <?=($need_mobile)?'*':''?>
        핸드폰번호&nbsp;:&nbsp;</td>
      <td><input name='mb_mobile' type=text id="mb_mobile3" value='<?=$mem[mb_mobile]?>' size=21 maxlength=20 itemname='핸드폰번호'> 
      </td>
    </tr>
    <?=$show_mobile_end?>
    <?=$show_address_begin?>
    <tr> 
      ...
      <td><input type=text name='mb_post' size=8 maxlength=7 readonly itemname='우편번호' value='<?=$mem[mb_post]?>'> 
        <input name="button" type=button class=button onclick="popup_zip('mb_form', '../', 'mb_post', '', 'mb_address1', 'mb_address2');" value='우편번호 검색'> 
      </td>
    </tr>
    <tr> 
      <td align="right" bgcolor="#f7f7f7"> 
        <?=($need_address)?'*':''?>
        주소&nbsp;:&nbsp;</td>
      <td><input name='mb_address1' type=text id="mb_address1" value='<?=$mem[mb_address1]?>' size="60" readonly> 
    ...
    <tr> 
      <td align="right" bgcolor="#f7f7f7"> 
        <?=($need_birth)?'*':''?>
        생일&nbsp;:&nbsp;</td>
      <td><input type=text name=mb_birth size=9 maxlength=8 value='<?=$mem[mb_birth]?>' itemname='생일'>
    ...
    <tr> 
      <td align="right" bgcolor="#f7f7f7"> 
        <?=($need_job)?'*':''?>
        직업&nbsp;:&nbsp;</td>
      <td><input name='mb_job' type=text id="mb_job3" value='<?=$mem[mb_job]?>' size=21 maxlength=20 itemname='직업'> 
      </td>
    </tr>
    <?=$show_job_end?>
    <?=$show_hobby_begin?>
    <tr> 
      <td align="right" bgcolor="#f7f7f7"> 
        <?=($need_hobby)?'*':''?>
        취미&nbsp;:&nbsp;</td>
      <td> <input name='mb_hobby' type=text id="mb_hobby3" value='<?=$mem[mb_hobby]?>' size=21 maxlength=20 itemname='취미'> 
      </td>
    </tr>
    <?=$show_hobby_end?>
    ...
    <tr> 
      <td align="right" bgcolor="#f7f7f7"> 
        <?=($need_signature)?'*':''?>
        서명&nbsp;:&nbsp;</td>
      <td><textarea name=mb_signature class=textarea rows=5 cols=60 itemname='서명'><?=$mem[mb_signature]?></textarea></td>
    </tr>
    <?=$show_signature_end?>
    <?=$show_greet_begin?>
    <tr> 
      <td align="right" bgcolor="#f7f7f7"> 
        <?=($need_greet)?'*':''?>
        자기소개&nbsp;:&nbsp;</td>
      <td><textarea name=mb_greet cols=60 rows=5 class=textarea id="mb_greet" itemname='자기소개'><?=$mem[mb_greet]?></textarea></td>
    </tr>
    <?=$show_greet_end?>
    <?=$show_photo_begin?>
    <tr> 
      <td align="right" bgcolor="#f7f7f7">회원 사진&nbsp;:&nbsp;</td>
      <td> <input name='mb_photo' type=file id="mb_photo3" size=40> 
        <?=$show_del_photo_begin?>
    ...
    <tr> 
      ...
      <td> <input type=file name='mb_icon' size=40>
    ...
    <tr> 
      ...
      <td><input name="mb_mailing" type="checkbox" id="mb_mailing" value="1" <?=($mem[mb_mailing])?'checked':''?>>
    ...
    <tr> 
      ...
      <td><input name="mb_point" type="text" id="mb_point" value="<?=$mem[mb_point]?>"></td>
    </tr>
    <tr> 
      <td align="right" bgcolor="#f7f7f7">오늘얻은 포인트&nbsp;:&nbsp;</td>
      <td><input name="mb_today_point" type="text" id="mb_today_point" value="<?=$mem[mb_today_point]?>"></td>
    </tr>
    <tr>
      <td align="right" bgcolor="#f7f7f7">최근포인트 적립일&nbsp;:&nbsp;</td>
      <td><?=rg_date($mem[mb_point_date])?>&nbsp;</td>
    </tr>
    <tr> 
      <td align="right" bgcolor="#f7f7f7">레벨&nbsp;:&nbsp;</td>
      <td> <select name="mb_level">
...
?>
    ...
    <tr> 
      ...
      <td> 
        <?=rg_date($mem[mb_reg_date])?>
      </td>
    </tr>
    <tr> 
      <td height="22" align="right" bgcolor="#f7f7f7">정보수정일&nbsp;:&nbsp;</td>
      <td> 
        <?=($mem[mb_modi_date]==0)?'-':rg_date($mem[mb_modi_date])?>
      </td>
    </tr>
    <tr> 
      <td height="22" align="right" bgcolor="#f7f7f7">최근접속일&nbsp;:&nbsp;</td>
      <td> 
        <?=($mem[mb_login_date]==0)?'-':rg_date($mem[mb_login_date])?>
      </td>
    </tr>
    <tr> 
