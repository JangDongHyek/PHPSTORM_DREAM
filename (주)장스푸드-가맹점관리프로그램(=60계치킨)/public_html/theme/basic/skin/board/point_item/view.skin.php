...
<article id="bo_v" style="width:<?php echo $width; ?>">
	...
	<form method="post" name="buy_frm" action="<?php echo G5_BBS_URL ?>/point_cart_in.php" onsubmit="">
					...
					<?php if($view['opt_use1'] == 'y'){ ?>
						<div class="opt_view_container">
							...
							<select name="opt1" id="opt1">
								...
								?>
								<option value="<?php echo $opt1_row['opt_idx'] ?>|<?php echo str_replace('|','',$opt1_row['opt_name']); ?>|<?php echo $opt1_row['opt_price'] ?>|<?php echo str_replace('|','',$view['wr_opt1']); ?>">
								<?php echo $opt1_row['opt_name'] ?> <?php echo $opt_price_str ?>
								</option>
								<?php
									}
								}
					...
					<?php if($view['opt_use2'] == 'y'){ ?>
						<div class="opt_view_container">
							...
							<select name="opt2" id="opt2">
								...
								?>
								<option value="<?php echo $opt2_row['opt_idx'] ?>|<?php echo str_replace('|','',$opt2_row['opt_name']); ?>|<?php echo $opt2_row['opt_price'] ?>|<?php echo str_replace('|','',$view['wr_opt2']); ?>">
								<?php echo $opt2_row['opt_name'] ?> <?php echo $opt_price_str ?>
								</option>
								<?php
									}
								}
					...
					<?php if($view['opt_use3'] == 'y'){ ?>
						<div class="opt_view_container">
							...
							<select name="opt3" id="opt3">
								...
								?>
								<option value="<?php echo $opt3_row['opt_idx'] ?>|<?php echo str_replace('|','',$opt3_row['opt_name']); ?>|<?php echo $opt3_row['opt_price'] ?>|<?php echo str_replace('|','',$view['wr_opt3']); ?>">
								<?php echo $opt3_row['opt_name'] ?> <?php echo $opt_price_str ?>
								</option>
								<?php
									}
								}
	...
	<div id="form_box_view">
		...
		<div style="border-bottom:1px solid #cacaca;"><?php echo $co_row['co_content'] ?></div>
		...
		<div style="border-bottom:1px solid #cacaca;"><?php echo $co_row['co_mobile_content'] ?></div>
	</div>
</article>
<!-- } 게시판 읽기 끝 -->
...
$(function(){
	...
	$("#cart_btn").on('click', function(){
		...
		$.ajax({
			type : 'post',
			url : '<?php echo G5_BBS_URL ?>/point_cart_in.php',
			data : frm_query,
			dataType : 'html',
			success : function(html){
				if(confirm('장바구니에 상품이 담겼습니다.\n장바구니로 이동하시겠습니까?')){
					location.href = '<?php echo G5_BBS_URL ?>/content.php?co_id=point_cart';
				}else{
					return false;
				}
			},
...
/*
function checkPoint(){
	var select_price = $("#tot_price_text").text();
	select_price = select_price.replace("원", "");
	select_price = parseInt(select_price.replace(/,/g,""));

	if(member_point >= select_price)
		return true;
	else
		return false;
}
