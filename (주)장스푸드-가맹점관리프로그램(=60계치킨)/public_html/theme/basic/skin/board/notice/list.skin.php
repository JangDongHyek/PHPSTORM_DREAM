...
<div id="bo_list" style="width:<?php echo $width; ?>">
	...
	<tr>
		...
		<? if ($member['mb_level'] == "2") { ?><th class="list_tbl_th">확인</th><? } ?>
	...
	<tr>
		...
		<? if ($member['mb_level'] == "2") { 
			$read = sql_fetch("SELECT COUNT(*) as cnt FROM g5_notice_chk WHERE mb_id = '{$member['mb_id']}' AND wr_id = '{$list[$i]['wr_id']}'");
			$read_chk = ((int)$read['cnt'] == 0)? "<strong style='color: #e4624a;'>안읽음</strong>" : "읽음";
		?>
		<!-- 읽음, 안읽음 -->
		<td class="list_tbl_td x110 talign_c <?php echo $tr_bg ?>"><?=$read_chk?></td>
		<? } ?>
