...
if ($member['mb_level'] == "2") { 
	$read = sql_fetch("SELECT COUNT(*) as cnt FROM g5_notice_chk WHERE mb_id = '{$member['mb_id']}' AND wr_id = '{$wr_id}'");
	if ((int)$read['cnt'] == 0) {
		$sql = "INSERT INTO g5_notice_chk SET 
				wr_id = '{$wr_id}',
				mb_id = '{$member['mb_id']}',
...
<article id="bo_v" style="width:<?php echo $width; ?>">
	...
	<div id="form_box">
		...
		<tr>
			<th class="form_tbl_th x110"><label>확인매장</label></th>
			<td class="form_tbl_td">
				<?
				$sql = "SELECT A.mb_2, A.mb_id FROM g5_member A INNER JOIN g5_notice_chk B
						ON A.mb_id = B.mb_id WHERE B.wr_id = '{$wr_id}' ORDER BY A.mb_2 ASC";
				...
				if ($result_cnt == 0) {
					...
					// echo "<div>총 {$result_cnt}개점</div>";
					for ($i = 0; $row = sql_fetch_array($result); $i++) {
						if ($i > 0) echo ', ';
						echo '<a href="/adm/member_form.php?w=u&mb_id='.$row['mb_id'].'" target="_blank">'.$row["mb_2"].'</a>';
					}
				}
				?>
			</td>
