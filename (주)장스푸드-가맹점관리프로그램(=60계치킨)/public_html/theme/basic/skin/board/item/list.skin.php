...
<h2 id="container_title"><?php echo $board['bo_subject'] ?> <? if($member[mb_level]<10){?><span style="color:red">* 2020년 5월 22일 종료됩니다.</span><? }?><span class="sound_only"> 목록</span></h2>
<!-- 게시판 목록 시작 { -->
...
<?php } ?>
    ...
    <div class="bo_fx">
        <div style="float:left; padding-top:5px;">
			...
			<select name="change_category" class="change_category">
				<?php echo item_cate_option($change_category) ?>
			</select>
	...
	<tr>
		...
		<td class="list_tbl_td talign_c <?php echo $tr_bg ?>" rowspan="2">
            ...
            <input type="checkbox" name="chk_wr_id[]" value="<?php echo $list[$i]['wr_id'] ?>" id="chk_wr_id_<?php echo $i ?>" class="chkIdBox">
		</td>
		<?php } ?>
		<td class="list_tbl_td talign_c <?php echo $tr_bg ?>" rowspan="2">
			<?php
			$ca_sql = " select * from g5_category where ca_idx='{$list[$i]['wr_10']}' ";
		...
		<? /*
		<td class="list_tbl_td talign_c <?php echo $tr_bg ?>" rowspan="2">
			<input type="text" name="wr_order[]" class="orderBox" value="<?=$list[$i]['wr_order']?>">
		</th>
		*/ ?>
	</tr>
	<tr>
	...
	?>
	<tr>
		<td colspan="7" class="list_tbl_td talign_c">등록된 게시물이 없습니다.</td>
	</tr>
	<?php
	}
	?>
...
$(function(){
	...
	$('input.orderBox').keydown(function() {
		if (event.keyCode === 13) {
...
function fnOrderSubmit() {
	var leng = $(".chkIdBox").length,
		idxArr = [],
		orderArr = [];

	for (var i = 0; i < leng; i++) {
		idxArr[i] = $(".chkIdBox").eq(i).val();
		orderArr[i] = $(".orderBox").eq(i).val();
	}

	$.ajax({  
		type : "get",  
		url : "./ajax.item_sort.php",
		data : {"bo_table" : "<?=$bo_table?>", "idxArr" : idxArr, "orderArr" : orderArr},
		dataType : "text",  
