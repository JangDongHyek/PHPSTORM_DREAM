...
<div id="btn_wrap">
	...
	<a onclick="window.open('<?php echo G5_BBS_URL ?>/INIStdPaySample/INIStdcancel.html?tid=<?php echo $od_row['tid'] ?>&moid=<?php echo $od_row['moid'] ?>&od_idx=<?php echo $od_idx ?>&c_mode=point','pay_cancel','width=640,height=500')">결제취소</a>
	<? } ?>
</div>
<?php
}
...
$ct_sql = " select * from g5_point_cart where od_idx='{$od_idx}' and it_bo_table='point_item' and ct_status!='대기' order by ct_idx desc ";
...
<tr>
	...
	<td class="od_body_td">
		...
		<?php
		if($ct_row['opt_cnt'] > 0){
			$opt_sql = " select * from g5_point_cart_opt where ct_idx='{$ct_row['ct_idx']}' order by opt_sort asc ";
...
<?php if($member['mb_level'] > 2){ ?>
	...
	<tr>
		...
		<td class="pay_body_td">
			...
			switch($od_row['od_method']){
				case "card" : 
					$card_quota = ($od_row['card_quota'] == "00")? "일시불" : (int)$od_row['card_quota']."개월"; 
					$od_method = '신용카드 ('.$card_quota.")"; 
					break;
	...
	<input type="hidden" name="od_idx" value="<?php echo $od_idx ?>">
	<input type="hidden" name="page" value="<?php echo $page ?>">
	<input type="hidden" name="order_mode" value="point">
