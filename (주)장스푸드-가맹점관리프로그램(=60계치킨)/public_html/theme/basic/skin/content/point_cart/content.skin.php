...
$list_sql = " select * from g5_point_cart where mb_id='{$member['mb_id']}' and ct_status='대기' and ct_direct='n' order by ct_idx DESC ";
...
<div id="category_container">
	...
	<form method="post" name="od_frm" action="<?php echo G5_BBS_URL ?>/point_cart_update.php">
	...
	<input type="hidden" name="buy_mode" id="buy_mode" value="">
		...
		if($list_num > 0){
			for($l=0; $l<$list_num; $l++){
				...
				$item_sql = " select count(*) as cnt from g5_write_point_item where wr_id='{$list_row['it_id']}' ";
		...
		<tr>
			...
			<td class="list_td <?php echo $td_bg ?>">
				...
				if($list_row['opt_cnt'] > 0){
					$opt_sql = " select * from g5_point_cart_opt where ct_idx='{$list_row['ct_idx']}' and mb_id='{$member['mb_id']}' order by opt_sort asc ";
					$opt_qry = sql_query($opt_sql);
					$opt_num = sql_num_rows($opt_qry);

					if($item_row['cnt'] == 0){
						$opt_del_sql = " delete from g5_point_cart_opt where ct_idx='{$list_row['ct_idx']}' and mb_id='{$member['mb_id']}' ";
		...
		<?php
				if($item_row['cnt'] == 0){
					$cart_del_sql = " delete from g5_point_cart where ct_idx='{$list_row['ct_idx']}' ";
		...
		<tr>
			...
			<td style="margin:0; padding:10px; text-align:right;">
				...
				<span style="padding:0px 20px; font-weight:bold; color:#fa003b; font-size:22px;">= 총 결제금액 <span id="hap"><?=number_format($pointTotal+$feeTotal)?></span> 원</span>
			</td>
		</tr>
		</tbody>
		</table>
...
$(function(){
	...
	$("#buy1").on('click', function(){
		...
		$("#buy_mode").val('select');
		document.od_frm.action = "<?php echo G5_BBS_URL ?>/point_cart_in.php";
		document.od_frm.submit();
	});
