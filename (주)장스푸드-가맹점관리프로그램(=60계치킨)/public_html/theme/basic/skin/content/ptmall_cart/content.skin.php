...
$list_sql = " select * from g5_ptmall_cart where mb_id='{$member['mb_id']}' and ct_status='대기' and ct_direct='n' order by ct_idx DESC ";
...
<div id="category_container">
	...
	<form method="post" name="od_frm" action="<?php echo G5_BBS_URL ?>/ptmall_cart_update.php">
	...
	<input type="hidden" name="buy_mode" id="buy_mode" value="">
		...
		<tr>
            ...
            <th class="list_th x140">판매마일리지</th>
		...
		if($list_num > 0){
			for($l=0; $l<$list_num; $l++){
				...
				$item_sql = " select count(*) as cnt from g5_write_ptmall_item where wr_id='{$list_row['it_id']}' ";
		...
		<tr>
			<td class="list_td talgin_c <?php echo $td_bg ?>">
                ...
                <input type="hidden" class="it_tot_price2" value="<?php echo $list_row['it_tot_price2'] ?>">
			</td>
			...
			<td class="list_td <?php echo $td_bg ?>">
				...
				if($list_row['opt_cnt'] > 0){
					$opt_sql = " select * from g5_ptmall_cart_opt where ct_idx='{$list_row['ct_idx']}' and mb_id='{$member['mb_id']}' order by opt_sort asc ";
					$opt_qry = sql_query($opt_sql);
					$opt_num = sql_num_rows($opt_qry);

					if($item_row['cnt'] == 0){
						$opt_del_sql = " delete from g5_ptmall_cart_opt where ct_idx='{$list_row['ct_idx']}' and mb_id='{$member['mb_id']}' ";
				...
				$pointTotal+=$list_row['it_tot_price'];
                $pointTotal2+=$list_row['it_tot_price2'];
				$feeTotal+=$list_row['it_fee'];
				?>
			</td>
            ...
            <td class="list_td talgin_r <?php echo $td_bg ?>"><span class="opt_tot_price_text2"><?php echo number_format($list_row['it_tot_price2']) ?>P</span></td>
			<td class="list_td talgin_r <?php echo $td_bg ?>"><span class="fee_text"><?php echo number_format($list_row['it_fee']) ?>원</span></td>
		</tr>
		<?php
				if($item_row['cnt'] == 0){
					$cart_del_sql = " delete from g5_ptmall_cart where ct_idx='{$list_row['ct_idx']}' ";
		...
		<tr>
			...
			<td style="margin:0; padding:10px; text-align:right;">
				<span style="font-weight:bold; color:#333;">상품구매금액 : <span id="cart_sum"><?=number_format($pointTotal)?></span>원</span>
				<span style="padding:0px 15px; font-weight:bold; color:#333;"> + </span>

                <span style="font-weight:bold; color:#333;"><span id="cart_sum2"><?=number_format($pointTotal2)?></span>P</span>
                <span style="padding:0px 15px; font-weight:bold; color:#333;"> + </span>

				<span style="font-weight:bold; color:#333;">배송비 : <span id="cart_fee_sum"><?=number_format($feeTotal)?></span>원</span>
				<span style="padding:0px 20px; font-weight:bold; color:#fa003b; font-size:22px;">= 총 결제금액 <span id="hap"><?=number_format($pointTotal+$feeTotal)?></span> 원 + <span id="hap2"><?=number_format($pointTotal2)?></span> P</span>
			</td>
		</tr>
		</tbody>
		</table>
...
$(function(){
	...
	$("#buy1").on('click', function(){
		...
		$("#buy_mode").val('select');
		document.od_frm.action = "<?php echo G5_BBS_URL ?>/ptmall_cart_in.php";
		document.od_frm.submit();
	});
	...
	if($(".ct_chk").length > 0){
		for(var i=0; i<$(".ct_chk").length; i++){
			...
			if($(".ct_chk").eq(_idx).is(':checked') == true){
                ...
                cart_sum2 += Number($(".it_tot_price2").eq(_idx).val());
