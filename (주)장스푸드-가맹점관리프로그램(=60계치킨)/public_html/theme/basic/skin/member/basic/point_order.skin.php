...
// 200922.추가 cart DB에 상품정보 없으면 튕김
$chk = sql_fetch("SELECT COUNT(*) AS cnt FROM g5_point_cart WHERE od_idx = '{$od_idx}' AND ct_status != '' ");
if ((int)$chk['cnt'] < 1) {
	alert("주문상품정보를 가져오는데 실패하였습니다. 다시 주문해 주세요.", G5_BBS_URL."/board.php?bo_table=point_item");
}
// 210304.추가 결제실패인 주문번호는 튕김
$chk2 = sql_fetch("SELECT pay_status FROM g5_point_order WHERE od_idx = '{$od_idx}' ORDER BY od_idx DESC LIMIT 0, 1");
switch ($chk2['pay_status']) {
	case "결제완료" : alert("이미 결제완료된 주문입니다.", G5_BBS_URL."/board.php?bo_table=point_item"); break;
	case "결제실패" : alert("결제실패 된 주문입니다. 장바구니 삭제후 다시 주문해 주세요.", G5_BBS_URL."/board.php?bo_table=point_item"); break;
	case "결제취소" : alert("결제취소 된 주문입니다. 장바구니 삭제후 다시 주문해 주세요.", G5_BBS_URL."/board.php?bo_table=point_item"); break;
}
...
<div id="bo_l">
	...
	<input type="hidden" name="quotabase" value="<?php echo $cardQuotaBase ?>" ><!-- 카드할부개월 -->
	<!-- KG이니시스 연동 END -->
...
$(function(){
	...
	$("#buy_free").on('click',function(){
		...
		orderRegist(datas, 'free');
	});
});

function orderRegist(datas, mode) {

	$.ajax({  
		type : "post",  
		url : "./ajax.order_update.php",
		data : {"datas" : datas, "tblInfo" : "point_order"},
		dataType : "text",  
		success : function(result) {  
			console.log(result);
			if (result == "success") {
				var idx = "od_idx=" + $("#od_idx").val();
				$("#merchantData").val(idx);

				if (mode == "buy") {
					paybtn();
				} else {
					document.SendPayForm_name.action = g5_bbs_url + "/point_order_complete.php";
					document.SendPayForm_name.submit();
				}
			} else {
				alert("[002] 페이지에 오류가 발생하였습니다. 다시 시도해 주세요.");
				location.reload();
			}
		},  
		error : function(xhr,status,error) {
			alert("[001] 페이지에 오류가 발생하였습니다. 다시 시도해 주세요.");
			location.reload();
		}  
	});
}
