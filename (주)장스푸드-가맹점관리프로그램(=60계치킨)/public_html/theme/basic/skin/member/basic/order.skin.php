...
<div id="bo_l">
	...
	<input type="hidden" name="orderPage" value="mall1">
	...
	$ct_sql = " select * from g5_cart where od_idx='{$od_idx}' ";
	...
	<tr>
		...
		<td class="list_tbl_td2 <?php echo $tr_bg ?>">
			...
			<?php
			if($ct_row['opt_cnt'] > 0){
				$opt_sql = " select * from g5_cart_opt where ct_idx='{$ct_row['ct_idx']}' and mb_id='{$member['mb_id']}' order by opt_sort asc ";
	...
	$buyertel = preg_replace("/[^0-9]*/s", "", $od_row['mb_hp']);
	?>
...
function orderRegist(datas, mode) {
	$.ajax({  
		...
		data : {"datas" : datas, "tblInfo" : "order"},
		dataType : "text",  
		success : function(result) {  
			if (result == "success") {
				...
				if (mode == "buy") {
					...
					document.SendPayForm_name.action = g5_bbs_url + "/order_complete.php";
