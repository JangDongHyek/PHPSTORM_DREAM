...
try {
	...
	for($arr=0; $arr<count($merchantData_arr); $arr++){
		...
		} else if ($merchantData_arr2[0] == 'po_point'){
			$po_point = $merchantData_arr2[1];
		} else if ($merchantData_arr2[0] == 'it_name'){
			$po_content = urldecode($merchantData_arr2[1]);
		} else {
...
주문번호 : {$moid_arr[1]}";
				...
				$expire = 0;
				$po_type = "사용";
				insert_point3($od_row['mb_id'], "-".$po_point, $po_content, '@passive', $od_row['mb_id'], $member['mb_id'].'-'.uniqid(''), $expire, $po_type, $od_idx, 0);
