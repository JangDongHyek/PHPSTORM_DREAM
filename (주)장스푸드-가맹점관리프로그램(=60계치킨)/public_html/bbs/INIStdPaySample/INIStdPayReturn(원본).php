		...
		try {
			...
			if ((strcmp("0000", $resultMap["resultCode"]) == 0) && (strcmp($secureSignature, $resultMap["authSignature"]) == 0) ){	//결제보안 추가 2016-05-18
			   ...
			   ******************************************************************************/
				...
				$sql2 = " update g5_cart set ct_status = '신청' where od_idx='{$od_idx}' ";
				sql_query($sql2);
...
주문번호 : {$moid_arr[1]}";
				...
				$location_data = G5_BBS_URL."/content.php?co_id=myorder&od_idx=".$od_idx;
				...
				$sql2 = " update g5_cart set ct_status = '' where od_idx='{$od_idx}' ";
				sql_query($sql2);

				$location_data = G5_BBS_URL."/board.php?bo_table=item";
