...
<table border="1" class="tbl">
	<tr>
		...
		<td class="head">결제방법</td>
		<td class="head">결제상태</td>
		<? if($co_id == "order") { ?><td class="head">디자인검토</td><? } ?>
		<td class="head">배송업체/송장번호</td>
	</tr>
	<?
		if($list_num > 0) {
	...
	<tr>
		...
		<td align="center"><?=$od_method?></td>
		<td align="center"><?=$list_row['pay_status']?></td>
		<? if($co_id == "order") { ?><td align="center"><? echo ($list_row['edit_check'] == "")? "-" : $list_row['edit_check']; ?></td><? } ?>
		<td align="center">
			<? echo ($list_row['od_delivery_name'])? $list_row['od_delivery_name'] . "/" . $list_row['od_send_number'] : ""; ?>
		</td>
	</tr>
	<?		
			} // for
