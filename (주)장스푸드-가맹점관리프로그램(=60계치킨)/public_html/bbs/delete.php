...
$sql = " select wr_id, mb_id, wr_is_comment, wr_content,wr_1 from $write_table where wr_parent = '{$write['wr_id']}' order by wr_id ";
...
while ($row = sql_fetch_array($result))
{
    ...
    if (!$row['wr_is_comment'])
    {
		...
		if($bo_table=="movie"){
			@ftp_file_delete("14.48.175.188","jangsfood", "6kfa%hd7","public_html/movie/",$row[wr_1]);
			$wr_1="";
		}
        // 파일테이블 행 삭제
...
//sql_query(" delete from $write_table where wr_num = '{$write['wr_num']}' and wr_reply != '' ");
...
if(($bo_table == 'inquiry' || $bo_table == 'praise') && $reply != ''){
	$pa_sql = " select wr_id from {$write_table} where wr_num='{$write['wr_num']}' and wr_reply='' ";
	$pa_row = sql_fetch($pa_sql);

	goto_url(G5_HTTP_BBS_URL.'/board.php?bo_table='.$bo_table.'&amp;wr_id='.$pa_row['wr_id'].'&amp;page='.$page.$qstr);
	exit;
}
