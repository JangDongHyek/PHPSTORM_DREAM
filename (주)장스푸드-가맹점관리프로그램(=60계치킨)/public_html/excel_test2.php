...
echo "<th style='width:70px;'>매장명</th>";
...
for ($i = 1; $i <= $xls->sheets[0]['numRows']; $i++) {
	...
	$mb_2 = trim(str_replace(' ','',$xls->sheets[0]['cells'][$i][6]));
	$mb_hp = trim(str_replace(' ','',$xls->sheets[0]['cells'][$i][7]));
...
<td style="padding:5px 5px;"><?php echo $xls->sheets[0]['cells'][$i][7] ?></td>
<td style="padding:5px 5px;">
<?php
/*
$sql = " insert into g5_member set mb_id='{$mb_id}', mb_password='{$mb_password}', mb_name='{$mb_name}', mb_level='2', mb_1='{$mb_1}', mb_2='{$mb_2}', mb_hp='{$mb_hp}', mb_datetime='".date('Y-m-d H:i:s')."', mb_mailling='1', mb_sms='1', mb_open='1' ";
if(sql_query($sql)){
	echo 'ok';
}else{
	echo 'xxxxxxxxxx';
