...
function get_sql_search($search_ca_name, $search_field, $search_text, $search_operator='and')
{
    ...
    for ($i=0; $i<count($s); $i++) {
        ...
        for ($k=0; $k<count($field); $k++) { // 필드의 수만큼 다중 필드 검색 가능 (필드1+필드2...)
            ...
            switch ($field[$k]) {
				...
				// 181227 분류 검색 추가
				case "ic_ca_name" :
					$str .= "(SELECT COUNT(*) FROM g5_write_inquiry_cate A WHERE g5_write_inquiry.ca_name = A.ic_idx AND A.ic_ca_name LIKE '%".$s[$i]."%') > 0 ";
					break;
                // LIKE 보다 INSTR 속도가 빠름
                default :
					if ($_GET["bo_table"] == "inquiry") { 
						$str .= " REPLACE($field[$k], ' ', '') LIKE '%{$search_str}%' ";
	...
	//if($_SERVER['REMOTE_ADDR'] == "121.144.134.22") echo $str;
...
function is_admin($mb_id)
{
	...
	if ($mem['mb_level'] == 10) return 'super';
...
// 회원 구분 불러오기
function mc_mb_list($level){
	global $g5;
	$mc_list = "";

	$sql = " select * from {$g5['mc_table']} where mc_mb_level = '{$level}' {$level_sql} order by mc_order asc, mc_idx asc ";
	$qry = sql_query($sql);
	$num = sql_num_rows($qry);
	if($num > 0){

		if($level == 3) $mc_list .= "운영자|";

		for($i=0; $i<$num; $i++){
			$row = sql_fetch_array($qry);
			if($i < ($num-1))
				$mc_list .= $row['mc_ca_name'].'|';
			else
				$mc_list .= $row['mc_ca_name'];
		}
	}
...
function item_cate_option($cate_val){
	$sql = " select * from g5_category order by ca_order desc, ca_idx asc ";
	echo $sql;
	$qry = sql_query($sql);
	...
	while($row = sql_fetch_array($qry)){
		$selected = '';
		if($row['ca_idx'] == $cate_val) $selected = 'selected';
		$list .= '<option value="'.$row['ca_idx'].'" '.$selected.'>'.$row['ca_name'].'</option>';
	}
...
// 190207 물품쇼핑몰 추가
function item_point_cate_option($cate_val){
	$sql = " select * from g5_point_category order by ca_order desc, ca_idx asc ";
	echo $sql;
	$qry = sql_query($sql);
...
// 190213 포인트몰 추가
function item_ptmall_cate_option($cate_val){
	$sql = " select * from g5_ptmall_category order by ca_order desc, ca_idx asc ";
	echo $sql;
	$qry = sql_query($sql);
...
// 포인트 부여2 (컬럼추가 po_type)
// /bbs/pstore.list.update.php

function insert_point2($mb_id, $point, $content='', $rel_table='', $rel_id='', $rel_action='', $expire=0, $po_type,$po_etc = '')
{
    global $config;
    global $g5;
    global $is_admin;
...
// 포인트 부여3 (컬럼추가 po_type, po_od_idx, od_idx)
// /bbs/INIStdPaySample/INIStdcancel_point.php
function insert_point3($mb_id, $point, $content='', $rel_table='', $rel_id='', $rel_action='', $expire=0, $po_type, $po_od_idx = 0, $od_idx = 0)
{
    global $config;
    global $g5;
    global $is_admin;
...
//호스트(ip),ftp 아이디, ftp 비번, 업로드할 경로, 파일
function ftpUpload($host,$id,$pass,$path,$file){
	$movie_name = $file['name'];  //파일
	$ftp_file = $file['tmp_name']; //서버에 임시로 저장된 파일 경로
	$ftp = ftp_connect($host);
	ftp_login($ftp, $id, $pass);
	ftp_put($ftp,$path.$movie_name,$ftp_file,FTP_BINARY);
	ftp_close($ftp);
	return $movie_name;
  ...
  function ftp_file_delete($ftp_host,$ftp_id,$ftp_pw,$server_path,$file_name){
      if(!($fc = ftp_connect($ftp_host, "21"))) die("$server_host : $server_post - 연결에 실패하였습니다.");   
