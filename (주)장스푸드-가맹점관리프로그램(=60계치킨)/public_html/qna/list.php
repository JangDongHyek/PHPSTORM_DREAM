...
<div class="inr">
	...
	<div class="list_tbl">
		...
		<dl>
			...
			<dd class="notice">
				...
				<div class="title col60"><a
						href="./view.php?idx=<?= $val['idx'] ?>&noti=y"><?= $val['qa_subject'] ?></a></div>
			...
			<?php
				...
				foreach ($list['list'] as $key => $val) {
					...
					// 답변수
					$reply_cnt = (int)$val['reply_cnt'];
				?>
			<dd>
				<div class="num col10"><?= $list_no ?></div>
				<div class="title col60">
					<a href="./view.php?idx=<?= $val['idx'] ?>">
						<?= $val['qa_subject'] ?>
						<? if ($is_file) { ?><img src="./img/icon_file.gif"><? } ?>
						<?php if ($reply_cnt > 0) { ?>&nbsp;<span class="reply"><?= $reply_cnt ?></span><?php } ?>
					</a>
				</div>
				<div class="state col15 <?= $status_cls[$val['qa_status']] ?>"><span><?= $val['qa_status'] ?></span>
				</div>
				<div class="data col15"><?= substr($val['qa_regdate'], 0, 10) ?></div>
			</dd>
			<? $list_no--;
	...
	<!-- 광고영역 -->
	<iframe src="http://letsit.kr/~itforone_test2/qna/ad_list.php?iframe=y" class="adFrame" scrolling="no"
