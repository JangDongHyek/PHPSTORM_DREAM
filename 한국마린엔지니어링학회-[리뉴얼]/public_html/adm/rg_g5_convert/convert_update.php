...
for ($i=0; $i<count($_POST['chk']); $i++) {
	...
	for($j=0; $j<$row=sql_fetch_array($result); $j++){
		...
		if($row['rg_parent_num']!= 0){
			$sql = "select * from $write_table where wr_10 = '{$row['rg_parent_num']}' order by wr_id desc";
			$wr = sql_fetch($sql);
			// 게시글 배열 참조
			$reply_array = &$wr;
	...
	//echo $row[rg_next_num]."==".$row[rg_title]."<br>";
		echo print_r($file_name)."<br>";
		board_write($row['rg_next_num'], $row['rg_name'], $row['rg_name'], $w, $wr_num, $write_table, $_POST['bo_table'][$k], $wr_reply, $row['cat_num'], $html, $secret, $row['rg_title'], addslashes(stripslashes($row['rg_content'])), $row['rg_link1_url'], $row['rg_link2_url'], $row['mb_id'], $row['rg_email'], $row['rg_home_url'], date("Y-m-d H:i:s", $row['rg_reg_date']), $row['rg_reg_ip'], $file_name, $file_size )."<br>";
	}
