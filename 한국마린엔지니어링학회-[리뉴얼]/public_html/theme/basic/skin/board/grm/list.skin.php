    	...
    	<div class="responsive-table" style="overflow-x:scroll;">
            ...
            <?php
                ...
                for($a=0;$a<count($caName);$a++){
					if($sfl&&$stx){
						$addWhere = " and instr($sfl,'$stx')";
					}
                    $sql="select * from g5_write_mem02 where ca_name='$caName[$a]'$addWhere order by wr_id asc";
                ...
                <?php } ?>
                    ...
                    <?php if($i==0){?>
                    <td rowspan="<?=$rowCount?>" class="stitle" valign="top"><?=$row[ca_name]?></td>
                    <?php }?>
                    <td><a href="<?=$href?>" target="<?=$target?>"><?=$row[wr_subject]?></a></td>
					<td>
						<a href="<?=$href?>" target="<?=$target?>">
						<?php
                    if ($row['is_notice']) { // 공지사항  ?>
                    ...
                    <td><a href="<?=$href?>" target="<?=$target?>"><?=$row[wr_content]?></a></td>
<!--                    <td><a href="<?=$row[wr_1]?>" target="_blank"><?=$row[wr_1]?></a></td>-->
                </tr>
                <? $i++;}}?>
            </tbody>
            </table>
...
<fieldset id="bo_sch">
    ...
    <input type="text" name="stx" value="<?php echo stripslashes($stx) ?>"  id="stx" class="frm_input sch_input required" maxlength="20">
