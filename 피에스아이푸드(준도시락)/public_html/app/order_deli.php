...
/**
 * 주문하기 (정기배달도시락/샐러드팩) - 사용안함
 */
loginCheck($member['mb_id']);
orderCheck(date('H:i'), 'deli'); // 주문가능시간 체크

if($w == 'u') {
    $do = sql_fetch(" select ord.*, do_category, do_price from g5_order as ord left join g5_dosirak as do on do.idx = ord.dosirak_idx where ord.idx = '{$idx}'");
} else {
...
<div id="container">
	<div id="order">
        <form id="forder" name="forder" method="post" action="./order_complete.php">
            ...
            <input type="hidden" name="do_shipping_fee" value="<?=$do['do_shipping_fee']?>"> <!--배송비-->
