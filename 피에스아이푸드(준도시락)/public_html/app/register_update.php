...
// SNS 회원가입 시
if($sns == 'naver' && $w == '') {
    $mb_id = substr($sns_id, 0, 10).'@n'; // 네이버에서 넘어온 id 잘라서 기호 붙여서 조합
    $sql_add = ", sns = '".$sns."', naver_id = '".$sns_id."'";
}
else if($sns == 'kakao' && $w == '') {
    $mb_id = $sns_id.'@k'; // 카카오에서 넘어온 id 잘라서 기호 붙여서 조합
    $sql_add = ", sns = '".$sns."', kakao_id = '".$sns_id."'";
}
...
if ($w == '') { // 등록
    $sql = " insert into {$g5['member_table']} 
             ...
             mb_login_ip = '{$_SERVER['REMOTE_ADDR']}' {$sql_add} ";
    ...
    $sql = " update {$g5['member_table']} set mb_name = '{$mb_name}', mb_nick = '{$mb_nick}', mb_hp = '{$mb_hp}', mb_zip1 = '{$mb_zip1}', mb_addr1 = '{$mb_addr1}', mb_addr2 = '{$mb_addr2}', send_email = '{$send_email}', payment = '{$payment}' {$sql_password} where mb_id = '$mb_id' ";
    sql_query($sql);
}

// 푸시
if($is_inapp) {
    $sql = "select * from g5_fcm where mb_id = '{$mb_id}'";
    $result = sql_query($sql);
    if (sql_num_rows($result)) {
        $sql = "update g5_fcm set token='{$token}' where mb_id = '{$mb_id}' ";
    } else {
        $sql = "insert g5_fcm set mb_id = '{$mb_id}', token='{$token}'";
    }
    sql_query($sql);
}
