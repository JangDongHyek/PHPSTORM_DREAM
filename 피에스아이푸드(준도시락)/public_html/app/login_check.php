...
if($is_inapp) {
    ...
    if (sql_num_rows($result)) {
        $sql = "update g5_fcm set token='{$token}' where mb_id = '{$mb['mb_id']}' ";
    } else {
        $sql = "insert g5_fcm set mb_id = '{$mb['mb_id']}', token='{$token}'";
    }
    sql_query($sql);
}
...
if ($url) {
    // 관리자로 로그인 시 관리자페이지로
    if($mb['mb_level'] == 10 && !$is_inapp) {
        $link = G5_ADMIN_URL;
    }
    else if($mb['mb_level'] == 3) { // 기사 로그인 시 기사 배달내역 페이지로
        $link = APP_URL.'/rider_list.php';
    }
    else if($mb['mb_level'] == 9) { // 관리자 admin2 접속 시 주문내역 페이지로
        $link = APP_URL.'/adm_order.php';
    }
    else {
        // url 체크
        check_url_host($url);
