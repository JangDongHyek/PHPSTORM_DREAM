...
function get_paging($write_pages, $cur_page, $total_page, $url, $add="")
{
    ...
    $exp_url = explode("?", str_replace("&amp;", "&", $url));
    $exp_url2 = explode("&", $exp_url[1]);
    $get_params = array();
    foreach ($exp_url2 AS $key=>$val) {
        if (strpos($val, "page=") !== false) continue;
        else $get_params[] = $val;
    }
    $url = $exp_url[0] . "?" . implode("&", $get_params) . "&page=";
...
function get_paging2($write_pages, $cur_page, $total_page)
{
    ...
    $str = '';
    if ($cur_page > 1) {
        $str .= '<a href="javascript:getPage(1);" class="pg_page pg_start">처음</a>'.PHP_EOL;
    }
    ...
    if ($start_page > 1) $str .= '<a href="javascript:getPage('.($start_page-1).');" class="pg_page pg_prev">이전</a>'.PHP_EOL;

    if ($total_page > 1) {
        for ($k=$start_page;$k<=$end_page;$k++) {
            if ($cur_page != $k)
                $str .= '<a href="javascript:getPage('.$k.');" class="pg_page">'.$k.'<span class="sound_only">페이지</span></a>'.PHP_EOL;
            else
                $str .= '<span class="sound_only">열린</span><strong class="pg_current">'.$k.'</strong><span class="sound_only">페이지</span>'.PHP_EOL;
        }
    }

    if ($total_page > $end_page) $str .= '<a href="javascript:getPage('.($end_page+1).');" class="pg_page pg_next">다음</a>'.PHP_EOL;

    if ($cur_page < $total_page) {
        $str .= '<a href="javascript:getPage('.$total_page.');" class="pg_page pg_end">맨끝</a>'.PHP_EOL;
    }
...
// page 변수 중복 때문에 get_paging 함수 복사해서 사용 $url = page -> page2로 변경
...
function get_list($write_row, $board, $skin_url, $subject_len=40)
{
    ...
    if(strpos($_SERVER['PHP_SELF'], '/adm') !== false) {
        ...
        $list['href'] = APP_URL.'/board.php?bo_table='.$board['bo_table'].'&amp;wr_id='.$list['wr_id'].$qstr;
    }
...
function get_member($mb_id, $fields='*')
{
    ...
    return sql_fetch(" select $fields from {$g5['member_table']} where mb_id = TRIM('$mb_id') and mb_level != 1 ");
}
...
// $flag : asc 낮은 순서 , desc 높은 순서
// 제목별로 컬럼 정렬하는 QUERY STRING
// subject_sort_link 수정하여 사용
function subject_sort_link2($st_date, $ed_date, $st_date2, $ed_date2, $col, $query_string='', $flag='asc')
{
    global $sst, $sod, $sfl, $stx, $page;
    ...
    $arr_query[] = 'sfl='.$sfl;
    $arr_query[] = 'stx='.$stx;
    $arr_query[] = 'st_date='.$st_date.'&ed_date='.$ed_date.'&st_date2='.$st_date2.'&ed_date2='.$ed_date2;
    $arr_query[] = 'page='.$page;
    $qstr = implode("&amp;", $arr_query);
...
// 파일 업로드 (경로, 파일명, 임시파일명)
function fileUpload_mod($path, $filename, $tmp_file) {
...
function isMobilePage(){
    if(preg_match('/(alcatel|amoi|android|avantgo|blackberry|benq|cell|cricket|docomo|elaine|htc|iemobile|iphone|iPhone|iPad|ipad|ipaq|ipod|j2me|java|midp|mini|mmp|mobi|motorola|nec-|nokia|palm|panasonic|philips|phone|sagem|sharp|sie-|smartphone|sony|symbian|t-mobile|telus|up\.browser|up\.link|vodafone|wap|webos|wireless|xda|xoom|zte|ios)/i', $_SERVER['HTTP_USER_AGENT'])){
        return true;
    } else {
        return false;
    }
...
// 주문가능시간 지정
function orderCheck($time, $mode) {
    // 1. 22.01.21 * 수정접수내역 - 주문가능시간 수정
    // 2. 22.09.23 * 수정접수내역 - 주문가능시간 수정
    if(date('H:i', strtotime('08:00')) < $time && $time < date('H:i', strtotime('09:00'))) {
        alert('주문가능시간이 아닙니다.\n주문가능시간 : 08시 00분 ~ 09시 00분 제외');
    }
    /*if($mode == 'deli') { // 정기배달
        if(date('H:i', strtotime('09:10')) < $time && $time < date('H:i', strtotime('10:30'))) {
            alert('주문가능시간이 아닙니다.\n주문가능시간 : 09시 10분 ~ 10시 30분 제외');
        }
    }
    else {
        if(date('H:i', strtotime('09:10')) < $time && $time < date('H:i', strtotime('13:00'))) {
            alert('주문가능시간이 아닙니다.\n주문가능시간 : 09시 10분 ~ 13시 제외');
        }
    }*/
}

// 푸시(IOS)
function sendFcmIOS($tokens, $message)
{
...
function getHoliday() {
    ...
    while($row = sql_fetch_array($rlt)) {
        $day = $row['type'].'-'.$row['year'].$row['month'].$row['day'];
        array_push($arr, $day);
    }
