...
$sql_certify = ", agency_no = '{$agency_no}'";

if ($mb_user_acc == "Y") 
	$sql_certify .= ", mb_user_acc = '{$mb_user_acc}'";

if ($mb_namechk_tid) 
	$sql_certify .= ", mb_namechk_tid = '{$mb_namechk_tid}'";
...
if ($w == '') {
	...
	*/
    ...
    if (sql_query($sql)) {
        ...
        if ($app_token != "") {
            // mb_id 기존토큰 초기화 (선택사항)
            sql_query("UPDATE g5_fcm_token SET mb_id = '' WHERE mb_id = '{$mb_id}'");
            // 새토큰 업데이트
            $sql = "UPDATE g5_fcm_token SET mb_id = '{$mb_id}' WHERE token = '{$app_token}'";
            if (sql_query($sql)) {
