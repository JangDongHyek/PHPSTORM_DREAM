...
<style>
    ...
    .tbl_wrap td.head {background: #EFEFEF; font-weight: bold; font-size: 13px;}
</style>
...
<form name="fmember" id="fmember" action="./member_form_update.php" onsubmit="return fmember_submit(this);" method="post" enctype="multipart/form-data" autocomplete="off">
    ...
    <div class="tbl_frm01 tbl_wrap">
        ...
        <table>
            ...
            <? if ($tab == 3) { ?>
                <tr>
                    <th scope="row"><label for="">아이디<strong class="sound_only">필수</strong></label></th>
                    <td><input placeholder="아이디" type="text" value="<?=$mb['mb_id']?>" name="mb_id" id="mb_id" class="frm_input required" minlength="" maxlength="" required="" <? if($w=='u'){ echo 'readonly';}?>></td>

                    <th scope="row"><label for="">이메일<strong class="sound_only">필수</strong></label></th>
                    <td><input placeholder="이메일" type="text" value="<?=$mb['mb_email']?>" name="mb_email" id="mb_email" class="frm_input required" minlength="" maxlength="" required=""></td>
                </tr>
                <tr>
                    <th scope="row"><label for="">주거형태<strong class="sound_only">필수</strong></label></th>
                    <td>
                        <select class="frm_input" name="mb_house" id="mb_house">
                            <option value="" selected>주거형태 선택</option>
                            <option value="단독주택" <?=get_selected($mb['mb_house'], "단독주택")?>>단독주택</option>
                            <option value="아파트" <?=get_selected($mb['mb_house'], "아파트")?>>아파트</option>
                            <option value="연립주택" <?=get_selected($mb['mb_house'], "연립주택")?>>연립주택</option>
                            <option value="다세대주택" <?=get_selected($mb['mb_house'], "다세대주택")?>>다세대주택</option>
                ...
                <tr>
                    ...
                    <td>
                        <input placeholder="생년월일" type="date" id="mb_birth" name="mb_birth" value="<?=$mb['mb_birth']?>" class="frm_input" >
                    </td>

                    <th scope="row">성별</th>
                    <td>
                        <input type="radio" id="male" name="mb_gender" value="남" <?=get_checked($mb['mb_gender'], "남")?>><label for="male">남성</label>
                        <input type="radio" id="female" name="mb_gender" value="여" <?=get_checked($mb['mb_gender'], "여")?>><label for="female">여성</label>
                    </td>
                </tr>
                <tr>
                    <th scope="row">혼인여부</th>
                    <td>
                        <input type="radio" id="single" name="mb_maritalStatus" value="미혼" <?=get_checked($mb['mb_maritalStatus'], "미혼")?>><label for="single">미혼</label>
                        <input type="radio" id="married" name="mb_maritalStatus" value="재혼" <?=get_checked($mb['mb_maritalStatus'], "재혼")?>><label for="married">재혼</label>
                    </td>
                </tr>
                <tr>
                    <th scope="row"><label for="mb_9">프로필사진</label><!--<label for="mb_9">통장사본</label>--></th>
                    <td colspan="3">
                        ...
                        $img_url = "";
                        if ($w == "u" && $mb['mb_9'] != "") {
                            $img_path = G5_DATA_PATH."/ccm/".$mb['mb_9'];
                            if (file_exists($img_path)) $img_url = G5_DATA_URL."/ccm/".$mb['mb_9'];
                        }
                        ?>
        ...
        <? if($tab != 3) { ?>
            ...
            <table>
                ...
                <? if ($member['mb_level'] == "10") { // 최고관리자만 수정가능 ?>
                    ...
                    <th scope="row"><label for="mb_9">면허증사본</label><!--<label for="mb_9">통장사본</label>--></th>
                    <td colspan="3">
                        <input type="hidden" name="mb_9" id="mb_9" value="<?=$mb['mb_9']?>">
                        <input type="file" name="upload_mb_9" id="upload_mb_9" class="frm_input" accept="image/*" onchange="uploadImg(this)" />
                        <?
            ...
            <?php if ($member['mb_level'] == "10") { // 최고관리자만 수정가능 ?>
                ...
                <table>
                    ...
                    <tbody>
                    <tr>
                        <td colspan="4" class="head">월 차감</td>
                    </tr>
                    <!-- 월 차감 -->
                    <tr>
                        <th>차감방식</th>
                        <td>
                            <?
                            foreach (AT_POINT_MONTH AS $key=>$val) {
                                $id = "wtp{$key}";
                    ...
                    <!-- 일 차감 -->
                    <tr>
                        <td colspan="4" class="head">일 차감</td>
                    </tr>
                    <tr>
                        <th>차감방식</th>
                        <td>
                            <?
                            foreach (AT_POINT_DAY AS $key=>$val) {
                                $id = "wtp2{$key}";
                                $checked = ((int)$mb['at_point2_type'] == $key)? "checked" : "";
                                ?>
                                <input type="radio" name="at_point2_type" value="<?=$key?>" id="<?=$id?>" <?=$checked?>>
                                <label for="<?=$id?>"><?=$val?></label>
                                &nbsp;&nbsp;
                            <?}?>
                        </td>
                        ...
                        <td>
                            <?
                            $at_point2 = ((int)$mb['at_point2'] > 0 && (int)$mb['at_point2_type'] > 0)? number_format($mb['at_point2']) : "";
                            ?>
                            <input type="text" name="at_point2" class="frm_input f_amt" value="<?=$at_point2?>"> 원
                        </td>
                    </tr>
                    <tr>
                        <th>차감기간</th>
                        <td colspan="3" id="atm_wrap">
                            <div id="atm_area0" class="at_tab">차감방식이 선택되지 않았습니다.</div>

                            <div id="atm_area1" class="at_tab">
...
if ($_SERVER['REMOTE_ADDR'] == "183.103.22.103") {
    /*$mb_id = '0036-00334';
    $charge_memo = '정정'; //'세금계산서 2만';
    $rel_action = 'admin_use'; //'admin_save';
    $point = 767;
    $use_point = 767;
    $ymd_hi = '2022-08-29 21:04'; // date("Y-m-d H:i");
...
<script>
    ...
    $(function() {
        ...
        // 차감방식(일) 뷰
        let wtp2 = $("[name=at_point2_type]:checked").val();
        $("#atm_area" + wtp2).css('display', 'block');

        // 차감방식 선택시 차감기간 탭 노출
        $("[name=at_point_type], [name=at_point2_type]").on("change", function() {
            let type = $(this).val();

            if ($(this).prop("name") == "at_point_type") {
                // 월 차감
                $("#at_wrap .at_tab").css('display', 'none');
                $("#at_area" + type).css('display', 'block')
            } else {
                // 일 차감
                $("#atm_wrap .at_tab").css('display', 'none');
                $("#atm_area" + type).css('display', 'block')
            }
        });
    ...
    function pointSubmit(f) {
        ...
        $.ajax({
            ...
            data : {"mb_id" : f.mb_id.value, "po_type" : f.po_type.value, "po_content" : f.po_content.value, "po_point" : f.po_point.value, "po_charge_memo": f.po_charge_memo.value},
            dataType : "text",
            success : function(data) {
                console.log(data);
                getPointList(1);
    ...
    function fmember_submit(f) {
        ...
        if (!!document.querySelector("[name=at_point_type]")) {
            ...
            if (type_chk == "1" && f.at_point.value == "") {
                alert("차감금액을 입력하세요.");
                f.at_point.focus();
                return false;
            }
        ...
        // 포인트 자동차감 설정체크 - 일 차감
        if (!!document.querySelector("[name=at_point2_type]")) {
            var type_chk = document.querySelector("[name=at_point2_type]:checked").value;

            if (type_chk == "1") {
                if (f.at_point2.value == "") {
                    alert("차감금액을 입력하세요.");
                    f.at_point2.focus();
                    return false;
                }

                if (f.at_point_sdate.value == "") {
                    alert("시작일을 선택하세요.");
                    f.at_point_sdate.focus();
                    return false;
                }
            }
        }
