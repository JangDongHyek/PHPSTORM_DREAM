...
if (!$member['mb_id'])
{
    alert('로그인이 필요합니다.', G5_BBS_URL.'/login.php?url=' . urlencode(G5_ADMIN_URL));
}
else if ($is_admin != 'super')
{
	...
	// 티대리분양플랫폼 대리점 권한부여
	if ($config['cf_account'] == "tdaeri2" && $member['mb_level'] == "9") {
		if ($member['mb_use'] != "Y") {
			alert('관리자의 승인이 필요합니다.', G5_URL);
		}

		// 관리자페이지 대리점 권한
		$agency_auth_id = array("고객관리"=>"200100", "기사관리"=>"200200", "콜접수내역"=>"250100");

		foreach ($agency_auth_id as $key=>$val) {
			$auth[$val] = 'r,w,d';
		}
		$i = count($auth);
	}
...
if ($tab != "")		$arr_query[] = 'tab='.$tab;
if ($tab2 != "")		$arr_query[] = 'tab2='.$tab2;
if ($tab3 != "")		$arr_query[] = 'tab3='.$tab3;



if ($map_si != "")		$arr_query[] = 'map_si='.$map_si;
if ($map_gu != "")		$arr_query[] = 'map_gu='.$map_gu;
