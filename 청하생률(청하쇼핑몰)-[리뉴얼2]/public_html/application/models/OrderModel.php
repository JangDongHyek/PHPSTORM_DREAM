...
class OrderModel extends CI_Model
{
    ...
    // 주문 목록 엑셀다운
    public function getOrderListExcel($param = array()): array
    {
        // 공통 쿼리
        /*
        $sqlCommon = "FROM bs_order WHERE del_yn = 'N' AND tmp_save_yn = 'N' ";
        ...
        */
        ...
        // 시작일,종료일
        $sqlCommon = '';
        if (!empty($param['sdt'])) $sqlCommon .= "AND DATE(B.reg_date) >= '{$param['sdt']}' ";
        if (!empty($param['edt'])) $sqlCommon .= "AND DATE(B.reg_date) <= '{$param['edt']}' ";


        $sql = "SELECT A.*,B.*,A.idx as idx 
			FROM bs_order_item A LEFT JOIN bs_order B ON A.ord_idx = B.idx  
        	WHERE B.del_yn = 'N' AND B.tmp_save_yn = 'N'  {$sqlCommon} ORDER BY B.idx ASC";



        log_message('error', '엑셀다운 $sql:'.print_r($sql, true));
