...
{
	private $categoryList = ['notice', 'review'];
	...
	public function boardViewPage($idx = 0)
	{
		...
		if (empty($boardData['idx'])) {
            ...
            //위 함수가 상품idx로 먹어서..
            //24.01.29 상품에서 구매후기쓰면 그냥 리뷰게시판으로가게 wc
            if($category == 'review'){
                redirect(PROJECT_URL.'/board?cate=review');
            }
            
			$data = [
				'message' => '존재하지 않는 정보입니다.'.$idx,
				'historyBack' => true,
			];
