...
<script>
	...
	const executePayment = (data={}) => {
		...
		if(!f.BuyerEmail.value){
            f.BuyerEmail.value = 'itforyou@daum.net';
        }
		if(!f.BuyerName.value){
            f.BuyerName.value = $('#ordName').val();
        }
        if(!f.BuyerTel.value){
            f.BuyerTel.value = $('#ordTel').val();
        }
		const iPayJson = {
...
<form name="ipay" method="post">
	...
	<input type="hidden" name="BuyerTel" value="<?=$member['mb_tel']?>"/>
	<input type="hidden" name="BuyerEmail" value="<?=$member['mb_email']?$member['mb_email']:$member['cn_email']?>"/>
