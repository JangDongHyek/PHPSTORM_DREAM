...
// summernote 기본세팅 (공통)
const getSummerNoteSettings = (enableInsert) => {
  let toolbar = [
    // ['style', ['style']],
    ...
    callbacks: {
      ...
      /*onPaste: (e) => {
        ...
        if (clipboardData && clipboardData.items && clipboardData.items.length) {
          ...
          if (item.kind === 'string') {
            e.preventDefault();
            item.getAsString(function(s) {  // Get the string
              var parser = new DOMParser();
              var doc = parser.parseFromString(s, 'text/html');  // Parse it as HTML
              // $('#editor').summernote('editor.insertText', doc.body.textContent || doc.body.innerText || '');

              $(doc.body).find("*").removeAttr('style');
              $('#editor').summernote('editor.pasteHTML', doc.body.innerHTML);
