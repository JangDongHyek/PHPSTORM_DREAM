...
?>
    ...
    <div class="tbl_frm01 tbl_wrap" id="recommend_tab">
        <table>
            <caption>추천인 정보 입력</caption>
            <tr>
                <th scope="row"><label for="mb_1">이름<strong class="sound_only">필수</strong></label></th>
                <td>
                    <span class="frm_info">
                        아이템 참여시 추천인에게 일정 포인트가 추천금으로 적립 제공됩니다.<br>
                    </span>
                    <!--<input type="text" id="mb_1" name="mb_1" value="<?php /*echo get_text($member['mb_1']) */?>" <?php /*echo $required */?> <?php /*echo $readonly; */?> class="frm_input <?php /*echo $required */?> <?php /*echo $readonly */?>">-->
                    <input type="text" id="mb_1" name="mb_1" value="<?php echo get_text($member['mb_1']) ?>" <?php echo $readonly; ?> class="frm_input <?php echo $readonly ?>">
                </td>
            </tr>

            <tr>
                <th scope="row"><label for="mb_2">닉네임<strong class="sound_only">필수</strong></label></th>
                <td>
                    <input type="hidden" name="mb_nick_recommend_default" value="<?php echo isset($member['mb_2'])?get_text($member['mb_2']):''; ?>">
                    <!--<input type="text" name="mb_2" value="<?php /*echo isset($member['mb_2'])?get_text($member['mb_2']):''; */?>" id="mb_2" required class="frm_input required nospace" maxlength="20">-->
                    <input type="text" name="mb_2" value="<?php echo isset($member['mb_2'])?get_text($member['mb_2']):''; ?>" id="mb_2" class="frm_input nospace" maxlength="20">
                    <span id="msg_mb_nick"></span>
                </td>
            </tr>
            <tr>
                <th scope="row"><label for="mb_3">E-mail<strong class="sound_only">필수</strong></label></th>
                <td>
                    <!--<input type="email" name="mb_3" value="<?php /*echo isset($member['mb_3'])?$member['mb_3']:''; */?>" id="mb_3" class="frm_input email required" size="50" maxlength="100">-->
                    <input type="email" name="mb_3" value="<?php echo isset($member['mb_3'])?$member['mb_3']:''; ?>" id="mb_3" class="frm_input email" size="50" maxlength="100">
                    <button type="button" onclick="chkRecommendMember()" class="btn">추천인검증</button>
                    <span id="recommend_msg"></span>
                </td>
            </tr>
        </table>
    ※추천인 정보 입력이 있을시 참가하신 아이템 업로더어에게 일정 포인트가 적립제공되며, 추천인 또는 아이템 업로더어에게 제공된 포인트만큼은 쿠폰으로 참가자에게 반환됩니다.
    <br><br>
    </div>
    ...
    <script>
    let hasRecommendMember = false; //추천인 실제 존재여부
    $(function() {
    ...
    function fregisterform_submit(f)
    {
        ...
        //추천인 인증이 안되었을때에는 추천인 등록 되지 않음 : 빈값으로 변경
        if(!hasRecommendMember){
            $("#mb_1").val('');
            $("#mb_2").val('');
            $("#mb_3").val('');
        }
    ...
    function chkRecommendMember(){
        hasRecommendMember = false; //초기화
        $('#recommend_msg').text('');
        
        var mbName = $("#mb_1").val();
        var mbNick = $("#mb_2").val();
        var mbEmail = $("#mb_3").val();
        var isEmpty = false;

        // 입력 필드가 비어 있는지 확인하고, 비어 있다면 빨간색 테두리를 적용
        $("#mb_1, #mb_2, #mb_3").each(function() {
            if (!$(this).val()) {
                $(this).css("border", "1px solid red");
                isEmpty = true;
                setTimeout(function() {
                    $(this).css("border", ""); // 1초 후에 빨간색 테두리 제거
                }.bind(this), 2000);
            }
        });

        // 하나라도 비어 있다면 경고창을 띄우고 함수 종료
        if (isEmpty) {
            alert("추천인 관련 모든 필드를 입력해주세요.");
            return; // 함수 실행 중지
        }

        $.ajax({
            url: g5_bbs_url+"/check_recommend_user.php",
            type: 'POST',
            ...
            success: function(response) {
                if(response) {
                    hasRecommendMember = true;
                    alert("추천인 검증에 성공하였습니다!");
                    $('#recommend_msg').text('검증성공!');
                    $('#mb_1').attr('readonly', true);
                    $('#mb_2').attr('readonly', true);
                    $('#mb_3').attr('readonly', true);
                } else {
                    alert("추천인이 존재하지 않습니다. 확인후 다시 입력해주세요.");
                    $('#recommend_msg').text('검증실패 : 추천인 검증이 안될경우 추천인 등록이 되지않습니다.');
                }
            }
        });
    }
