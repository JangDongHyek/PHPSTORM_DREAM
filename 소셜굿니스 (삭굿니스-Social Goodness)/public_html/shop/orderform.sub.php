...
<div id="sod_frm">
  ...
  <div class="tbl_head01 tbl_wrap">
        ...
        <tr>
            ...
            <th scope="col">포인트</th>
            <th scope="col"> 배송비<br />
            <div style="font-size:0.76em; line-height:1.2em">(선정자만<br />해당)</div> </th>
        </tr>
        </thead>
        <tbody>
        <?php
        ...
        <tr>
            ...
            <td align="center" class="td_dvr"><!--?php echo $ct_send_cost; ?--> 후불</td>
        </tr>
	...
	※ 배송비는 기부하기 참가자에게는 부과되지 않습니다.<br /><br />
    ...
    <dl id="sod_bsk_tot">
        ...
        <?php } ?>
        <dt class="sod_bsk_dvr">배송비 <span style="font-size:0.8em; line-height:1.2em">(선정자만 해당)</span> </dt>
  ...
  <section id="sod_frm_pay">
        ...
        <div class="tbl_frm01 tbl_wrap">
			...
			<br />
			※ 배송비는 기부하기 참가자에게는 해당되지 않습니다.        </div>

        <?php
        if (!$default['de_card_point'])
            echo '<p id="sod_frm_pt_alert"><!--<strong>무통장입금</strong> 이외의 결제 수단으로 결제하시는 경우 포인트를 적립해드리지 않습니다.-->신용카드 및 후불결제 수단으로 입금된 예치금은 카드사 및 후불결제사의 수수료 또는 일부 결제비용이 제외된 예치금이 <strong>포인트로 반환 지급</strong>됩니다. 이용약관을 참조 하시길 바랍니다.</p>';
...
function forderform_check(f)
{
    ...
    check_field(f.od_name, "구매 및 참여하시는 분 이름을 입력하십시오.");
    if (typeof(f.od_pwd) != 'undefined')
    {
        clear_field(f.od_pwd);
        if( (f.od_pwd.value.length<3) || (f.od_pwd.value.search(/([^A-Za-z0-9]+)/)!=-1) )
            error_field(f.od_pwd, "회원이 아니신 경우 정보 작성시, 조회시 필요한 비밀번호를 3자리 이상 입력해 주십시오.");
    }
    check_field(f.od_tel, "구매 및 참여하시는 분 전화번호를 입력하십시오.");
    check_field(f.od_addr1, "주소검색을 이용하여 참여하시는 분 주소를 입력하십시오.");
    ...
    if (typeof(f.od_temp_point) != "undefined") {
        if (f.od_temp_point.value)
        {
            ...
            if (temp_point > od_price) {
                alert("상품 금액(배송비 제외) 보다 많이 포인트결제할 수 없습니다.");
                f.od_temp_point.select();
                return false;
            }
    ...
    if( jQuery(f).triggerHandler("form_sumbit_order_"+form_order_method) !== false ) {
        ...
        <?php } ?>
				...
				<?php if($default['de_pg_service'] == 'innopay') { ?>
					...
					f.BuyerTel.value = f.od_tel.value;//연락처
					/* 주소 */
