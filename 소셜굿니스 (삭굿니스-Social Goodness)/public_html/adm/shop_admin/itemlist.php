...
<div class="local_ov01 local_ov">
    <?php echo $listall; ?>
    <span class="btn_ov01"><span class="ov_txt">등록된 펀딩</span><span class="ov_num"> <?php echo $total_count; ?>건</span></span>
</div>
...
<form name="flist" class="local_sch01 local_sch">
	...
	<select name="sfl" id="sfl">
		<option value="it_name" <?php echo get_selected($sfl, 'it_name'); ?>>펀딩명</option>
		<option value="it_id" <?php echo get_selected($sfl, 'it_id'); ?>>펀딩코드</option>
	</select>
...
<form name="fitemlistupdate" method="post" action="./itemlistupdate.php" <?/*onsubmit="return fitemlist_submit(this);"*/?> autocomplete="off" id="fitemlistupdate">
	...
	<div class="btn_fixed_top">
		<a href="./itemform.php" class="btn btn_01">펀딩등록</a>
...
function fitemlist_submit(f, mode)
{
	...
	var msg = "선택하신 펀딩을 ";
	msg += (document.pressed == "선택삭제")? "삭제하시겠습니까? 삭제된 펀딩은 복구되지 않습니다." : "수정하시겠습니까?";
	if(confirm(msg)){
	...
	<div class="tbl_head01 tbl_wrap">
		...
		<tr>
			<th scope="col"><?php echo subject_sort_link('it_id', 'sca='.$sca); ?>펀딩코드</a></th>
		</tr>
		</thead>
		<tbody>
		<?
		...
		<tr class="<?=$bg?>" data-no="<?=$i?>">
			<td rowspan="2"><input type="checkbox" name="chk[]" value="<?php echo $i ?>" id="chk_<?=$i?>"></td>
			<td rowspan="2"><?=number_format($list_no)?></td>
			<!-- 이미지 -->
			<td rowspan="2"><?php echo get_it_image($it_id, 70, 70); ?></td>
			<!-- 펀딩명 -->
			...
			<!-- 재고 -->
			<td rowspan="2"><input type="text" name="it_stock_qty[<?=$i?>]" value="<?php echo $row['it_stock_qty']; ?>" id="stock_qty_<?=$i?>" class="tbl_input sit_qty" size="7"></td>
			<!-- 판매가격 -->
			<td rowspan="2"><input type="text" name="it_price[<?=$i?>]" value="<?php echo $row['it_price']; ?>" id="price_<?=$i?>" class="tbl_input sit_amt" size="7"></td>
			<!-- 시중가격 -->
			<td rowspan="2"><input type="text" name="it_cust_price[<?=$i?>]" value="<?php echo $row['it_cust_price']; ?>" id="cust_price_<?=$i?>" class="tbl_input sit_camt" size="7"></td>
			<!-- 판매 -->
			<td rowspan="2"><input type="checkbox" name="it_use[<?=$i?>]" <?php echo ($row['it_use'] ? 'checked' : ''); ?> value="1" id="use_<?=$i?>"></td>
			<!-- 품절 -->
			<td rowspan="2"><input type="checkbox" name="it_soldout[<?=$i?>]" <?php echo ($row['it_soldout'] ? 'checked' : ''); ?> value="1" id="soldout_<?=$i?>"></td>
			<!-- 조회 -->
			<td rowspan="2"><?=number_format($row['it_hit'])?></td>
			<!-- 관리 -->
			<td rowspan="2" class="td_mng td_mng_s">
				...
				<a href="javascript:;" onclick="window.open('./itemcopy.php?it_id=<?=$it_id?>&amp;ca_id=<?php echo $row['ca_id']; ?>')" class="itemcopy btn btn_02">복사</a>
		...
		if ($i == 0) { 
		?>
		<tr><td colspan="15" class="empty_table">등록된 펀딩이 없습니다.</td></tr>
		<? } ?>
		</tbody>
		</table>
	</div>
