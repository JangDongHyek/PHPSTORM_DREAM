...
if($w == '' && ($_POST['cp_sms_send'] || $_POST['cp_email_send'])) {
    ...
    if($_POST['chk_all_mb']) { // 전체회원 체크했을 시
    ...
    for($i=0; $row = sql_fetch_array($result); $i++) {
        $arr_send_list[] = $row; // 문자, 메일 발송받을 회원 list
    }
    ...
    $mb_hp_arr = array();
    $mb_hp = "";
    $sms_send_num = "052-257-7575"; // 발신자번호
    $sms_msg = $cp_subject.' 쿠폰이 **님께 발행됐습니다. 쿠폰만료 : '.$cp_end.' '.str_replace('http://', '', G5_URL);
    $sms_type = 4; // 4: sms
    $sms_img1 = "";
    $sms_img2 = "";
    $sms_rcv_type = ($_POST['chk_all_mb'])? "전체회원" : "일부회원";
    $mb_id_arr[] = $mb_id;
    $mb_hp_arr[] = $mb_hp;

    //$count = 0; // ***** sms기능 열어줄때 여기 코드 지워주기/
    for($i=0; $i<$count; $i++) { 
        if(!$arr_send_list[$i]['mb_id']) // 아이디 없을시 다음 실행
            continue;

        // SMS전송 : sms 전송 체크 여부, 회원 전화번호, 회원의 sms 사용 여부 (&& $arr_send_list[$i]['mb_sms']) 추가해주면 됨
        if($_POST['cp_sms_send'] && $arr_send_list[$i]['mb_hp']) {
        ...
        // E-MAIL
        // 이메일 발송받음으로 체크한 회원만 보낼시 코드 살려줄것
        // if($config['cf_email_use'] && $_POST['cp_email_send'] && $arr_send_list[$i]['mb_email'] && $arr_send_list[$i]['mb_mailling']) {
        if($config['cf_email_use'] && $_POST['cp_email_send'] && $arr_send_list[$i]['mb_email']) {
            ...
            if($cp_id) $contents .= '쿠폰코드 : '.$cp_id.'<br>';
            if($coupon_method) $contents .= '쿠폰종류 : '.$coupon_method.'<br><br>';
            if($cp_start) $contents .= '사용시작일 : '.$cp_start.'<br>';
            if($cp_end) $contents .= '사용종료일 : '.$cp_end.'<br>';
            // $contents .= '쿠폰타입 : '.$cp_type.'<br>';
            if($cp_price) $contents .= '할인금액 : '.$cp_price.'<br>';
            if($cp_minimum) $contents .= '최소주문금액 : '.$cp_minimum.'<br>';
