...
$status_normal = array('신청','입금','준비','배송','완료','참여','달성','선정');
$status_cancel = array('취소','반품','품절','미달','환불','말료');
...
for ($i=0; $i<$cnt; $i++)
{
    ...
    if ($ct['ct_stock_use'])
    {
        if ($ct_status == '신청' || $ct_status == '취소' || $ct_status == '반품' || $ct_status == '품절')
        {
            $stock_use = 0;
            // 재고에 다시 더한다.
            if($ct['io_id']) {
    ...
    // it_id를 배열에 저장
    if($ct_status == '신청' || $ct_status == '취소' || $ct_status == '반품' || $ct_status == '품절' || $ct_status == '완료')
        $arr_it_id[] = $ct['it_id'];
}
