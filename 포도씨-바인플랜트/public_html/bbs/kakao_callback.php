...
$shell_string = "curl -v -X POST https://kauth.kakao.com/oauth/token -d 'grant_type=authorization_code' -d 'client_id=".$kakao_javascript_key_new."' -d 'redirect_uri=".$redirect_uri."' -d 'code=".$code."'";
...
if (empty($access_token)) {
    ...
    if (isset($mb['mb_id'])) {
        ...
        //alert('회원 정보가 없습니다.\n회원 가입을 진행해 주세요.', G5_BBS_URL.'/register_form.php?id='.$id.'&email='.$email.'&sns=kakao', false);
        if($_SESSION['join_check'] == 'login') {
            alert('회원 정보가 없습니다.\n회원 가입을 진행해 주세요.', G5_BBS_URL.'/register.php', false);
        } else {
            goto_url(G5_BBS_URL.'/register_form.php?id='.$id.'&email='.$email.'&sns=kakao');
        }
    }
}