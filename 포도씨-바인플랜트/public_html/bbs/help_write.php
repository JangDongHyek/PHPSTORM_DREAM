            ...
            <div id="help_list" class="write">
                <form id="fhelpme" name="fhelpme" method="post" autocomplete="off" action="<?=G5_BBS_URL?>/help_write_update.php">
                    ...
                    <div class="help_write">
                        <div class="title">
                            <h3><input type="text" id="he_subject" name="he_subject" value="<?=$help['he_subject']?>" class="subject" placeholder="제목을 입력해 주세요." required> </h3>
                        </div>
                    ...
                    <div class="w_filter hash">
                        ...
                        <div class="area_tag">
                            ...
                            <input type="text" class="input_tag" id="input_tag" placeholder="질문에 맞는 태그를 입력해 주세요(엔터로 구분, 최대 5개)" onkeyup="lengthChk(this);add_hash(this);">
                            <ul class="tag_list">
                                <?php
                    ...
                    <div class="w_filter hash">
                        <h3>BUNKER</h3>
                        <div class="area_tag">
                            <input type="tel" class="input_tag bunker" id="he_bunker" name="he_bunker" <?php echo $w == 'u' ? 'readonly' : '' ?> value="<?php echo !empty($help['he_bunker']) ? number_format($help['he_bunker']) : ''; ?>" placeholder="벙커를 입력해 주세요(채택한 답변자에게 BUNKER 적립)" onkeyup="only_number(this);bunker_check(this.value, <?=$member['mb_bunker']?>+<?=$member['mb_bunker_bonus']?>);">
                        </div>
                    </div>
                    <button type="button" class="btn_confirm fixed" onclick="helpme_register();"><?=$btn_txt?>하기</button>
                </form>
            </div>
			<?php
...
<script>
    ...
    function helpme_register() {
        ...
        //$.trim(f.he_contents.value.replace(/&nbsp;/gi,' ')).length
        if($.trim(f.he_contents.value) == "") {
            swal('내용을 입력해 주세요.');
            is_post = false;
            return false;
        }
