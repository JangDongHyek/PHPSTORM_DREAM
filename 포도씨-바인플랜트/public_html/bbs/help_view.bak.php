...
<div id="basic_modal">
    <!-- Modal -->
    <div class="modal fade select" id="edit_home2" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-body">
                    <div class="txt confirm">
                        <h2>베스트 답변만 채택하셨습니다.<br>마감하시겠습니까?</h2>
                    </div>
                    <ul class="list_edit">
                        <li><a href="javascript:void(0);">마감하기</a></li>
                        <li class="deactivate"><a href="javascript:void(0);">우수 답변 채택하기</a></li>
                    </ul>
                </div>
            </div>
        </div>
			...
			<div id="help_list">
				...
				<div class="help_question top">
					<div class="title">
                        ...
                        <?php if(($help['mb_id'] == $member['mb_id'] && !$selection_flag)) { // 작성자 == 로그인자 && 채택된 답변 없어야 함?>
				...
				<div class="area_answer">
                    ...
                    <div class="help_question help_answer">
                        ...
                        <div class="title">
                            ...
                            <?php if(($help['mb_id'] == $member['mb_id']) && empty($row['an_selection']) && empty($help['he_selection'])) { // 질문자 == 로그인자 && 현재 답변이 채택이 안됐을 경우 && 현재 질문에 답변 채택이 완료되지 않음 ?>
...
// 조회/좋아요/싫어요 동작 (액션, idx, 채택 답변 구분)
function help_action(mode, idx, gubun) {
    if(mode == 'answer_update') { // 답변 수정 -- 답변 내용이 답변 등록 폼에 보여짐
        ...
        $.ajax({
            ...
            data: {mode : mode, helpme_idx : '<?=$idx?>', helpme_an_idx : idx, gubun : gubun},
            type: 'POST',
            cache: false,
            async: false,
            success : function(data) {
...
function modal_open(modal_id, an_idx, best, great) {
    ...
    if(!best && !great) { // 베스트 / 우수 둘다 채택 X
        $('.list_edit li:nth-child(1) a').attr("onclick", "javascript:help_action('select', "+an_idx+", 'best');"); // 베스트 답변
        $('.list_edit li:nth-child(2) a').attr("onclick", "javascript:void(0);"); // 우수 답변
    } else if(!best && great) { // 우수 채택 O
        $('.list_edit li:nth-child(1) a').attr("onclick", "javascript:help_action('select', "+an_idx+", 'best');"); // 베스트 답변
    ...
    } else if(best && great) { // 베스트 / 우수 둘다 채택 O
        $('.list_edit li:nth-child(1) a').attr("onclick", "javascript:void(0);"); // 베스트 답변
        $('.list_edit li:nth-child(2) a').attr("onclick", "javascript:void(0);"); // 우수 답변
    }
}
