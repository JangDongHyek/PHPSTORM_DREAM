...
if(!empty(trim($search))) {
    $sql_search .= " and (he_subject like '%{$search}%' or strip_tags(he_contents) like '%{$search}%' or he_hashtag like '%{$search}%' or an_hashtag like '%{$search}%' or FIND_IN_SET('{$search}', he_hashtag) or FIND_IN_SET('{$search}', an_hashtag)) ";
}
...
if(!$private) { // 테스트데이터
    $sql_search .= " and he.mb_id not in ('com01', 'test01') ";
}
...
if($member['mb_id'] == 'test01') {
    if(!empty(blockUser($member['mb_id']))) {
        ...
        $sql_search .= " and he.mb_id not in ({$block}) ";
    }
}
...
if($i==0) {
?>
<div style="text-align: center;">등록된 질문이 없습니다.</div>
<?php
}
?>
