...
$mb = sql_fetch(" select * from g5_member where sns = 'facebook' and sns_id = '{$id}' "); // 회원정보
...
if (isset($mb['mb_id'])) {
    ...
    //alert('회원 정보가 없습니다.\n회원 가입을 진행해 주세요.', G5_BBS_URL.'/register_form.php?id='.$id.'&email='.$email.'&name='.$name.'&sns=facebook', false);
    if($_SESSION['join_check'] == 'login') {
        alert('회원 정보가 없습니다.\n회원 가입을 진행해 주세요.', G5_BBS_URL.'/register.php', false);
    } else {
        goto_url(G5_BBS_URL.'/register_form.php?id='.$id.'&email='.$email.'&name='.$name.'&sns=facebook');
    }
}