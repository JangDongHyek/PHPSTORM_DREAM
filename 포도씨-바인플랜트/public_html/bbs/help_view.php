        ...
        <div class="modal-dialog">
            <div class="modal-content">
				<div class="modal-header">
                    ...
                    <h4 class="modal-title" id="appModalLabel">채택하기</h4>
                </div>
                <div class="modal-body">
					<ul class="list_edit">
						<li><a href="javascript:void(0);">베스트 답변</a></li>
						<li class="deactivate"><a href="javascript:void(0);">우수 답변</a></li>
					</ul>
                </div>
            </div>
        </div>
            ...
            <div class="modal-content">
                <div class="modal-body">
                    <div class="txt confirm">
                        <em>베스트 답변을 채택하셨습니다.<br>마감하시겠습니까?</em>
                    </div>
                    <ul class="madal_btn">
                        <li class="ok">마감하기</li>
                        <li class="ok">우수 답변 채택하기</li>
                    </ul>
                </div>
            </div>
        </div>
    ...
    <div class="modal fade" id="edit_home3" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" data-backdrop="static" data-keyboard="false">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-body">
                    <div class="txt confirm">
                        <em>24시간 이내에 우수 답변을 채택하지 않을 시<br>관리자가 답변을 채택할 수 있습니다.</em>
                    </div>
                    <ul class="madal_btn">
                        <li>취소</li>
                        <li class="ok">확인</li>
    ...
    <div class="modal fade" id="edit_home4" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" data-backdrop="static" data-keyboard="false">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-body">
                    <div class="txt confirm">
                        <em>채택할 답변이 존재하지 않습니다.<br>마감하시겠습니까?</em>
                    </div>
                    <ul class="madal_btn">
                        <li>취소</li>
                        <li class="ok">마감하기</li>
		...
		<div id="help_warp">
			...
			<div id="help_list">
					...
					<div class="bottom">
						...
						<div class="info">
							...
							<div class="list_info">
                                ...
                                <li onclick="profileOpen('<?=$help['mb_category']?>', '<?=$help['mb_id']?>')">프로필보기</li>
                                <?php } ?>
                                ...
                                <li><a href="<?=G5_BBS_URL?>/company.php?mb_no=<?=$help['mb_no']?>">기업홈피로 이동</a></li>
                                <li>의뢰건수 <em class="blue"><?=inquiryCount($help['mb_id'])?></em>건</li>
                                <li>거래건수 <em class="blue"><?=completeCount($help['mb_id'])?></em>건</li>
                                <?php } ?>
                                <?php if(!$self && $help['mb_category'] == '일반') { // 내가쓴글아님 && 작성자일반회원?>
                                <li onclick="chatting('<?=$help['mb_id']?>');">채팅하기</li>
                                <?php } ?>
                                <?php if(!$self) { // 내가쓴글아님?>
                                <li onclick="reportOpen('<?=$help['mb_id']?>', 'g5_helpme', '<?=$help['idx']?>')">신고하기</li>
                                <?php if($member['mb_id'] == 'test01') { ?>
                                <li onclick="userBlock('<?=$help['mb_id']?>', 'g5_helpme', '<?=$help['idx']?>', 'helpme')">차단하기</li>
                                <?php } ?>
                                <?php } ?>
                                </ul>
                                <!-- //토글메뉴 -->
                ...
                <form id="fanswer" name="fanswer" method="post" autocomplete="off" action="<?=G5_BBS_URL?>/help_view_update.php">
                    ...
                    <div class="help_write" >
                        <div class="title" >
							<h3 class="answer">
                                ...
                                <span class="bold"><?php echo !empty($member['mb_nick']) ? $member['mb_nick'] : $member['mb_id']; ?></span>님, 당신의 지식을 공유해 주세요!
                            </h3>
                        </div>
					...
					<div class="w_filter hash">
                        ...
                        <div class="area_tag">
                            ...
                            <input type="text" class="input_tag" id="input_tag" placeholder="답변에 맞는 태그를 입력해 주세요(엔터로 구분, 최대 5개)" onkeyup="add_hash(this);lengthChk(this);">
                            <ul class="tag_list">
                            </ul>
                        </div>
                    </div>
				...
				<div class="area_answer">
					<h2>총 <span id="answer_count"><?=number_format($answer_count)?></span>개의 답변이 있습니다.</h2>
                    <?php
                    ...
                    // 답변 리스트
                    $sql = " select an.*, mb.mb_no, mb.mb_nick, mb.mb_grade, mb.mb_category from g5_helpme_answer as an left join g5_member as mb on an.mb_id = mb.mb_id where helpme_idx = '{$idx}' and an.del_yn is null {$sql_search} order by an.wr_datetime desc ";
                    ...
                    for($i=0; $row=sql_fetch_array($result); $i++) {
                        ...
                        $display = true;
                        if($row['an_open'] == 'questioner' && $is_admin != 'super') { // 공개설정 질문자만 공개 시 질문자와 답변 작성자에게만 답변이 보여짐, 최고관리자에게도 보여짐
                    ...
                    <div class="help_question help_answer">
                        ...
                        <div class="area_select area_select_<?=$row['idx']?>" <?=$an_selection?>>
                            <span></span>
                            <em><?php echo $row['an_best'] == 'Y' ? '베스트' : '우수'; ?></em>
                        </div>
                        <div class="title">
                            <div class="area_name" onclick="userToggle('answer_<?=$row['idx']?>');">
                                ...
                                <div class="profile_info">
                                    ...
                                    <ul class="answer_<?=$row['idx']?> user_list answer01 sm">
                                        ...
                                        <?php if($member['mb_id'] == 'test01') { ?>
                                        <li onclick="userBlock('<?=$row['mb_id']?>', 'g5_helpme_answer', '<?=$row['idx']?>', 'helpme_view')">차단하기</li>
                                        <?php } ?>
                                        <?php } ?>
                                    </ul>
                                    <!-- //토글메뉴 -->
							...
							<a class="answer_select" href="javascript:modal_open('edit_home', '<?=$row['idx']?>', '<?=$best_flag?>', '<?=$great_flag?>');"><span></span><em>채택<em class="mhide">하기</em></em></a>
                            <?php } ?>
                        </div>
                        ...
                        <div class="area_bottom">
                            <div class="bottom">
                                ...
                                <?php } else { ?>
                                <!-- 질문자만 공개 했을대 나오는 문구 -->
                                <div class="secret"><span><i class="fas fa-lock"></i>질문자에게만 공개된 답변입니다</span></div>
                                <?php } ?>
                                ...
                                <div class="info">
                                    ...
                                    <ul class="thums">
                                        ...
                                        <?php if($is_member) { ?><li class="comment comment_<?=$row['idx']?> store_noshow"><i></i><span id="answer_reply_count_<?=$row['idx']?>"><?=number_format($answer_reply_count)?></span></li><? } ?> <!-- 댓글수 -->
                                    </ul>
                                </div>
                            </div>
                            <!-- 댓글 -->
                            <div class="area_comment area_comment_<?=$row['idx']?>">
                                ...
                                <ul class="list_comment list_comment_<?=$row['idx']?>">
                                    <?php
                                    $sql_reply = " select re.*, mb.mb_no, mb.mb_nick, mb.mb_category from g5_helpme_answer_reply as re left join g5_member as mb on re.mb_id = mb.mb_id where helpme_an_idx = {$row['idx']} and re.del_yn is null order by wr_datetime asc ";
                                    ...
                                    <li>
										<div class="area_name" onclick="userToggle('reply_<?=$reply['idx']?>');">
                                            ...
                                            <ul class="reply_<?=$reply['idx']?> user_list answer02 sm">
                                                ...
                                                <?php } ?>
                                                <?php if(!$self) { // 내가쓴글아님?>
                                                <li onclick="reportOpen('<?=$reply['mb_id']?>', 'g5_helpme_answer_reply', '<?=$reply['idx']?>')">신고하기</li>
                                                <?php if($member['mb_id'] == 'test01') { ?>
                                                <li onclick="userBlock('<?=$reply['mb_id']?>', 'g5_helpme_answer_reply', '<?=$reply['idx']?>', 'helpme_view')">차단하기</li>
                                                <?php } ?>
                                                <?php } ?>
                                            </ul>
                                            <!-- //토글메뉴 -->
		...
		<ul class="list">
            ...
            <li>
                <a href="<?=G5_BBS_URL?>/help_view.php?idx=<?=$row['idx']?>">
                    ...
                    <div class="list_info">
                        <span class="view">조회수 <em><?=number_format($view_count)?></em></span> <!-- 조회수 -->
                        <span class="reply">답변수 <em><?=number_format($a_count)?></em></span> <!-- 답변수 -->
            ...
            ?>
            <li class="nodata">비슷한 질문이 없습니다.</li>
            <?php
            }
            ?>
		</ul>
