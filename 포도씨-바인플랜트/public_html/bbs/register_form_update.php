...
// SNS 회원가입 시
if($w == '' && !empty($sns)) {
    $sql_add = ", sns = '".$sns."', sns_id = '".$sns_id."', sns_token = '".$sns_token."', mb_push = 'Y' ";
    $mb_id = explode('@', $mb_email)[0];
    for($i=0; $i<10; $i++) {
        $cnt = sql_fetch(" select count(*) as cnt from g5_member where mb_id = '{$mb_id}' ")['cnt']; // 동일한 아이디 있을 경우 랜덤하게 숫자 붙임
        $random = rand(1, 100);
        if($cnt == 0) { break; }
        else { $mb_id = $mb_id.$random; }
    }
}
...
if($mb_category == "일반") {
    ...
    if($w == '') { $sql_option .= " , mb_grade = '실습항해사' "; }
} else { // 기업
    $sql_option = " , mb_company_name = '{$mb_company_name}', mb_zip = '{$mb_zip}', mb_addr1_lat = '{$mb_addr1_lat}', mb_addr1_lng = '{$mb_addr1_lng}' ";
...
if ($w == '') {
    alert('회원가입이 완료되었습니다.', G5_BBS_URL.'/register_result.php', false);
} else if ($w == 'u') {
