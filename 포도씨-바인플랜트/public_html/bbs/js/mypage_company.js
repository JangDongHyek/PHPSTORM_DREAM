...
function state_modal(idx, state, estimate_cnt, selection) {
    ...
    if(state == '접수대기') { // 견적 있으면 견적검토중 or 거래완료 or 미체결 선택 / 견적 없으면 모달 X
        $('.li_check').show();
        $('.li_select').show();
        $('.li_no').show();
    }
    else if(state == '견적검토중') { // 거래완료 or 미체결 선택을 위한 대기 상태
        $('.li_wait').show();
        $('.li_select').show();
        $('.li_no').show();
    }
    ...
    if(estimate_cnt > 0 && selection == 0 && state != '미채결') {
