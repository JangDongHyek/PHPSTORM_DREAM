...
<select name="type" id="type" onchange="document.fsearch.submit();">
    <option value="" <?=$type == "" ? "selected" : ""; ?>>의뢰유형</option>
    <option value="서비스" <?=$type == "서비스" ? "selected" : ""; ?>>서비스</option>
    <option value="부품" <?=$type == "부품" ? "selected" : ""; ?>>부품</option>
    <option value="선용품" <?=$type == "선용품" ? "selected" : ""; ?>>선용품</option>
...
<?php echo get_paging(G5_IS_MOBILE ? $config['cf_mobile_pages'] : $config['cf_write_pages'], $page, $total_page, '?'.$qstr.'&lv='.$lv.'&type='.$type.'&category='.$category.'&amp;page='); ?>
