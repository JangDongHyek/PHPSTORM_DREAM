...
$sql_common = " from g5_reference_room_seller ";
...
<form name="fwithdrawlist" id="fwithdrawlist" method="post">
    ...
    <div class="tbl_head02 tbl_wrap mb_tbl">
        <table>
            ...
            <tr>
                <th>No.</th>
                <th><?php echo subject_sort_link('mb_id') ?>아이디</th>
                <th><?php echo subject_sort_link('mb_name') ?>이름</th>
                <th>휴대폰번호</th>
                <th>은행</th>
                <th>계좌번호</th>
                <th>예금주</th>
            ...
            for ($i = 0; $row = sql_fetch_array($result); $i++) {
                ...
                <tr class="<?php echo $bg; ?>">
                    ...
                    <td>
                        <select style="width: 50%;" name="state"
                                onchange="status_change(<?= $row['idx'] ?>, this.value, '<?= $row['mb_id'] ?>')">
...
<script>
    ...
    function status_change(idx, val, mb_id) {
        $.ajax({
            url: g5_admin_url + "/ajax.seller_state_change.php",
            type: "POST",
            data: {
                "idx": idx,
                "state": val,
