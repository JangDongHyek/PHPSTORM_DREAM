...
$sql_common = " from g5_reference_room_withdraw as a left join g5_member as mb on mb.mb_id = a.mb_id ";
...
$sql = " select a.*, mb.mb_name, mb.mb_bunker {$sql_common} {$sql_search} {$sql_order} limit {$from_record}, {$rows} ";
...
<select name="sfl" id="sfl">
    <option value="mb_id"<?php echo get_selected($_GET['sfl'], "a.mb_id"); ?>>아이디</option>
</select>
...
<div class="tbl_head02 tbl_wrap mb_tbl">
    ...
    <tr>
        <th>No.</th>
        <th><?php echo subject_sort_link('a.mb_id') ?>아이디</th>
        <th><?php echo subject_sort_link('mb.mb_name') ?>이름</th>
        <th>은행</th>
        <th>계좌번호</th>
        <th>예금주</th>
        <th>주민번호</th>
    ...
    <tr class="<?php echo $bg; ?>">
        ...
        <td><?=number_format($row['sales_proceeds'])?>원</td>
        <td><?=number_format($row['sales_proceeds']*85/100)?>원</td>
        <td>
            <select style="width: 50%;" name="state" onchange="status_change(<?=$row['idx']?>, this.value, '<?=$row['mb_id']?>')">
                ...
                <!--<option <?/* if ($row['state'] == '취소') echo "selected"; */?> value="취소">취소</option>-->
                <!--<option <?/* if ($row['request_state'] == '보류') echo "selected"; */?> value="보류">보류</option>-->
            </select>
        </td>
        <td><?=substr($row['wr_datetime'], 0, 10)?></td>
        <td><?php echo $row['payment_date'] == '0000-00-00 00:00:00' ? '' : substr($row['payment_date'], 0, 10)?></td>
    </tr>
    <?php
        $list_no--;
    }
...
function status_change(idx, val, mb_id) {
    $.ajax({
        url: g5_admin_url+"/ajax.reference_withdraw_state.php",
        type: "POST",
        data: {
            "idx": idx,
            "state": val,
