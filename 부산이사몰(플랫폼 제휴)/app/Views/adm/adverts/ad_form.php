                ...
                <form>
                    ...
                    <div class="box_gray">
                        <div class="grid grid2">
                            <dl class="form_wrap">
                                ...
                                <dd><input type="text" name="companyName" id="companyName" value="<?=$company_name ?? ''?>" placeholder="업체명" readonly /></dd>
                            </dl>
                            <dl class="form_wrap">
                                <dt><label for="cpTel">연락처</label></dt>
                                <dd><input type="tel" name="cpTel" id="cpTel" value="<?=$company_tel ?? ''?>" placeholder="연락처" data-format="tel" readonly /></dd>
                            </dl>
                        </div>
                    </div>
                    <hr>
                        ...
                        <dl class="form_wrap">
                            ...
                            <dd class="flex">
                                <select name="areaSi" id="areaSi" >
                                    <option value="부산" <?=($area_si ?? '') === '부산' ? 'selected' : ''?>>부산</option>
                                    <option value="울산" <?=($area_si ?? '') === '울산' ? 'selected' : ''?>>울산</option>
                                    <option value="경남" <?=($area_si ?? '') === '경남' ? 'selected' : ''?>>경남</option>
                                </select>
                    ...
                    <div class="box_gray">
                        <div class="ad_option grid grid2">
                            <div>
                                <input type="checkbox" id="main_ad" class="absolute" name="mainYn" value="Y" <?=($main_yn ?? '')==='Y' ? 'checked' : ''?>>
                                <label for="main_ad">
                                    <p>
                                        <span class="txt_color">타입1.</span>
                                        메인 노출 상품 <strong class="txt_black">(월 40만원)</strong>
                                    </p>
                                    <div class="info">
                                        <div class="img">
                            ...
                            <div>
                                <input type="checkbox" id="premium_ad" class="absolute" name="preYn" value="Y" <?=($pre_yn ?? '')==='Y' ? 'checked' : ''?> onchange="togglePremiumArea()">
                                ...
                                <div class="main_area" style="display: none;">
                                    <div class="box_color">
                                        <dl class="form_wrap">
                                            ...
                                            <dd class="select">
                                                <input type="checkbox" id="main_top_list" name="mainTop" value="Y" <?=($main_top ?? '')==='Y' ? 'checked' : ''?> >
                                                <label for="main_top_list">메인 상단 리스트 노출</label>

                                                <input type="checkbox" id="main_bottom_list" name="mainBottom" value="Y" <?=($main_bottom ?? '')==='Y' ? 'checked' : ''?>>
                                        ...
                                        <dl class="form_wrap">
                                            ...
                                            <dd class="flex">
                                                <select name="areaSi2" id="areaSi2" >
                                                    <option value="부산" <?=($pre_si ?? '') === '부산' ? 'selected' : ''?>>부산</option>
                                                    <option value="울산" <?=($pre_si ?? '') === '울산' ? 'selected' : ''?>>울산</option>
                                                    <option value="경남" <?=($pre_si ?? '') === '경남' ? 'selected' : ''?>>경남</option>
                                                </select>
                                            </dd>
                    ...
                    <div class="payment" >
                        ...
                        <div class="box_gray">
                            ...
                            <dl class="form_wrap" id="cardForm">
                                <input type="hidden" name="cardIdx" id="cardIdx" value="<?=$card['idx'] ?? 0?>">
                                <input type="hidden" name="cardMbIdx" id="cardMbIdx" value="<?=$card['mb_idx'] ?? 0?>">
                                <dt><label for="">카드번호 입력</label></dt>
                                <dd class="flex gap5">
                                    <?
                                    $number = $card['card_num'] ?? 0;
                                ...
                                <dt><label for="">유효기간</label></dt>
                                <?
                                $exp = $card['card_exp'] ?? 0;
                                $yymm = str_split($exp, 2);
                                ?>
                                <dd class="flex gap5"><input type="text" name="cardMm" id="cardMm" value="<?=$yymm[1] ?? ''?>" maxlength="2" placeholder="MM"/>
                                    <input type="text" name="cardYy" id="cardYy" value="<?=$yymm[0] ?? ''?>" placeholder="YY" maxlength="2"/></dd>
                                <dt><label for="cardPwd">카드 비밀번호 앞 2자리</label></dt>
                                <dd class="flex gap5">
                                    <input type="password" name="cardPwd" id="cardPwd" value="<?=$card['card_pwd'] ?? []?>" maxlength="2" placeholder="카드 비밀번호 앞 2자리"/>
                                </dd>
                                <dt><label for="idNum">카드인증번호</label></dt>
                                <dd><input type="text" name="idNum" id="idNum" value="<?=$card['idNum'] ?? []?>" placeholder="개인: 주민번호 앞6자리 / 법인: 사업자번호 10자리"/></dd>
                            </dl>
                            <br>
                            <button class="btn btn_large btn_color3" id="cardNum">결제정보 변경</button>
            ...
            <div>
                <div class="sticky">
                    <details class="result" open>
                        ...
                        <input type="hidden" name="idx" value="<?=$aidx ?? 0?>">
                        <input type="hidden" name="parentId" value="<?=$param['idx'] ?? 0?>">
                        <input type="hidden" name="basicPrice" value="<?=$basic_price ?? 0 ?>"> <!--기본가격-->
                        <input type="hidden" name="prePrice" value="<?=$pre_price ?? 0 ?>"> <!-- 프리미엄 가격 -->
                        <input type="hidden" name="mainPrice" value="<?=$main_price ?? 0?>"> <!-- 메인가격 -->
                        <input type="hidden" name="mainTopPrice" value="<?=$main_top_price ?? 0?>"> <!-- 메인상단 -->
                        <input type="hidden" name="mainBottomPrice" value="<?=$main_bottom_price ?? 0?>"> <!-- 메인하단 -->
                        <input type="hidden" name="areaGu" value="<?=$area_gu ?? ''?>"> <!--기본 지역-->
                        <input type="hidden" name="preArea" value="<?=$pre_area ?? ''?>"> <!-- 프리미엄 지역 -->
                        <input type="hidden" name="orderPrice" value="<?=$order_price ?? 0 ?>"> <!--주문 금액-->
                        <input type="hidden" name="discount" value="<?=$discount ?? 0?>"> <!--할인가격-->
                        <input type="hidden" name="totalAmt" value="<?=$total_amt ?? 0?>"> <!--총가격-->
                        <div class="details">
                            <div class="box_gray">
                                <ul>
                    ...
                    <div class="box_colorline flex ai-c jc-sb">
                        <p class="txt_up txt_bold">총 결제 진행 금액</p>
                        <div class="price flex ai-c">
                            <input type="text" placeholder="0" value="<?=number_format($order_price) ?? ''?>" data-format="amount" name="addPrice" class="text-right">원
                        </div>
                    </div>
                    <div class="box_colorline flex ai-c jc-sb">
                        <p class="txt_up txt_bold">할부</p>
                    ...
                    <button type="submit" class="btn btn_large btn_color">결제 진행</button>
                </div>
            </div>
        </div>
    </form>
</section>
<script src="<?= base_url()?>js/adm/ad_form.js?<?=JS_VER?>"></script>
<script>
    //결제정보
    ...
    document.addEventListener('DOMContentLoaded', () => {
        const preYn = '<?=($pre_yn ?? '')?>'; // PHP에서 pre_yn 값을 가져옵니다.
        if (preYn === 'Y'){
            toggleAdArea(); // 페이지 로드 시 초기 상태 설정
        }
    });
