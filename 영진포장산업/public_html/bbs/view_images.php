...
//	$data=rg_get_doc_info($bbs_id,$doc_num);
	...
	if($data[rg_mb_num] == '0' || $mb[mb_num] != $data[rg_mb_num]) {
		$tmp=explode(',',$HTTP_SESSION_VARS["ss_doc_hit"]);
...
//			session_register("ss_doc_hit");
			$HTTP_SESSION_VARS['ss_doc_hit']=$ss_doc_hit;
		}
		unset($tmp);
	}
...
//	require_once("_footer.php");
	...
	if(!$bbs_cfg[$C_USE_REMOTE_WRITE]) {
		$HTTP_SESSION_VARS['write_key']=$bbs[bbs_num].'.'.$bbs[bbs_id];
	}
?>