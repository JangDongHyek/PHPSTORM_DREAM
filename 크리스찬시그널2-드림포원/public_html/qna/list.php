	...
	<dl>
		...
		<?php
			...
			foreach ($list['list'] AS $key=>$val) {
                ...
                // 답글 카운트
                $reply = '';
                $reply_cnt = $db->getReplyDbRows($val['idx']);
                if(!empty($reply_cnt)) {
                    $reply = '<span class="reply">'.$reply_cnt.'</span>';
                }
		?>
		<dd>
			<div class="num col10"><?=$list_no?></div>
			<div class="title col60">
				<a href="./view.php?idx=<?=$val['idx']?>"><?=$val['qa_subject']?> <? if ($is_file) {?><img src="./img/icon_file.gif"><?}?><?=$reply?></a>
			</div>
