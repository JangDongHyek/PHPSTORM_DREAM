...
if($count == 0) {
    ...
    if ($pg_status == 1) // 결제성공
    {
        if ($result == 1)
        {
            ...
            $amt = $_REQUEST["Amt"];
            $manna = explode("_",$_REQUEST["GoodsName"])[1];
            //if($private || $member['mb_id'] == 'hong') { $point = $manna; }

            $mb = get_member($_REQUEST["mallUserID"]);
            $acc_point = $mb['cw_point'] + $manna;
//            $acc_section_cnt = $mb['cw_section_cnt'] + $section;


            // 포인트 이력 (결제 만나 지급)
//            $sql = " insert into g5_member_point set mb_id = '{$mb['mb_id']}', point_category = '섹션적립', point = {$section}, acc_point = {$acc_section_cnt}, point_content = '{$section} 섹션 충전', wr_datetime = '".G5_TIME_YMDHIS."' ";
//            sql_query($sql);
            $sql = " insert into g5_member_point set mb_id = '{$mb['mb_id']}', point_category = '적립', point = {$manna}, acc_point = {$acc_point}, point_content = '{$manna} 만나 충전', wr_datetime = '".G5_TIME_YMDHIS."' ";
            sql_query($sql);

            $sql = " update g5_member set cw_point = {$acc_point}, acc_cw_point = acc_cw_point + {$manna} where mb_id = '{$mb['mb_id']}' ";
            sql_query($sql);
//            $sql = " update g5_member set cw_section_cnt = {$acc_section_cnt}, acc_cw_point = acc_cw_point + {$section} where mb_id = '{$mb['mb_id']}' ";
//            sql_query($sql);
