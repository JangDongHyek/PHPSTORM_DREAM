...
// 로그인중이고 자신의 글이라면 또는 관리자라면 비밀번호를 묻지 않고 바로 수정, 삭제 가능 ==> 비회원 문의는 글 조회 시 비밀번호를 입력하고 들어오기 때문에 수정/삭제 시 비밀번호 확인 안함
if (($member['mb_id'] && ($member['mb_id'] == $write['mb_id'])) || $is_admin || $bo_table == 'qna') {
...
else if (!$write['mb_id'] && $bo_table != 'qna') { // 회원이 쓴 글이 아니라면
