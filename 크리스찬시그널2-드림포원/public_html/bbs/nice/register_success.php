...
if ($returnMsg != "") {
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function(){
            /*if (!mobilecheck() && !'<?=$ios_flag?>') {
                swal("이미 가입된 휴대폰번호입니다.")
                .then(()=>{
    ...
    <form name="niceFrm" id="niceFrm" action="<?=G5_BBS_URL?>/register_form.php" method="post">
        <input type="hidden" name="mb_certify" value="<?=$authtype?>"><!-- 인증수단 : M -->
        <input type="hidden" name="mb_name" value="<?=$name?>"><!-- 본인인증 이름 -->
        ...
        <input type="hidden" name="join_type_de" value="">
        <input type="hidden" name="mb_introduce" value="">
        <input type="hidden" name="mb_id" value="">
        <input type="hidden" name="secret_member" value="">
    </form>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function(){
            ...
            var mb_password_re = sessionStorage.getItem("mb_password_re");
            var join_type = sessionStorage.getItem("join_type");
            var join_type_de = sessionStorage.getItem("join_type_de");
            var mb_introduce = sessionStorage.getItem("mb_introduce");
            var mb_id = sessionStorage.getItem("mb_id");
            var type = sessionStorage.getItem("type");
            var secret_member = sessionStorage.getItem("secret_member");

            f.mb_password.value = mb_password;
            f.mb_password_re.value = mb_password_re;
            f.join_type.value = join_type;
            f.join_type_de.value = join_type_de;
            f.mb_introduce.value = mb_introduce;
            f.mb_id.value = mb_id;
            f.secret_member.value = secret_member;

            // 회원구분세션 초기화
            sessionStorage.removeItem("mb_password");
            sessionStorage.removeItem("mb_password_re");
            sessionStorage.removeItem("join_type");
            sessionStorage.removeItem("join_type_de");
            sessionStorage.removeItem("mb_introduce");
            sessionStorage.removeItem("mb_id");
            sessionStorage.removeItem("secret_member");
