...
if ($mode == "sms_send") {
    ...
    if ($type == 'id') {
        $sql = "select count(mb_no) cnt from g5_member where mb_name='$mb_name' and mb_hp='" . hyphen_hp_number($mb_hp) . "'";
        $msg = "[크리스찬시그널] 본인확인 인증번호는 [{$random}] 입니다.";
        $console_msg = "인증번호가 발송되었습니다.";
    }else{
        $sql = "select count(mb_no) cnt from g5_member where mb_id='$mb_id' and mb_hp='" . hyphen_hp_number($mb_hp) . "'";
        $msg = "[크리스찬시그널] 임시비밀번호는 [{$random}] 입니다. 로그인 후 보안을 위해 변경해주세요.";
    ...
    if ($type =='pw'){
        $sql = "update g5_member set mb_password = '".get_encrypt_string($random)."', mb_update_yn = 'N' where mb_id = '{$mb_id}' ";
        sql_query($sql);
