...
if($count == 0) {
    ...
    if ($pg_status == 1) // 결제성공
    {
        if ($result == 1)
        {
            ...
            $point = $_REQUEST["Amt"] / 10000 * 110; // 220만나 = 20000원, 330만나 = 30000원, 550만나 = 50000원, 1100만나 100000원 (계산식은 임의로 작성)
            //if($private || $member['mb_id'] == 'hong') { $point = $manna; }

            $mb = get_member($_REQUEST["mallUserID"]);
            $acc_point = $mb['cw_point'] + $point;

            // 포인트 이력 (결제 만나 지급)
            $sql = " insert into g5_member_point set mb_id = '{$mb['mb_id']}', point_category = '적립', point = {$point}, acc_point = {$acc_point}, point_content = '{$point} 만나 충전', wr_datetime = '".G5_TIME_YMDHIS."' ";
            sql_query($sql);

            $sql = " update g5_member set cw_point = {$acc_point}, acc_cw_point = acc_cw_point + {$point} where mb_id = '{$mb['mb_id']}' ";
            sql_query($sql);
        ...
        else
        {
            ...
            $url = G5_BBS_URL."/point_charge.php";
        }
    }
    else { // 결제실패
