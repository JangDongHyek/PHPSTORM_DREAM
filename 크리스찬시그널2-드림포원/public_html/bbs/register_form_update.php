...
$mb_recommend_hp   = isset($_POST['mb_recommend_hp'])     ? trim($_POST['mb_recommend_hp'])   : "";
...
if ($w == '') {
    ...
    $sql = " insert into {$g5['member_table']}
                    set mb_id = '{$mb_id}',
                        ...
                        mb_recommend_hp = '{$mb_recommend_hp}',
                        ...
                        mb_join_type = '{$_POST['join_type']}',
                        mb_join_type_de = '{$_POST['join_type_de']}',
                        mb_introduce = '{$_POST['mb_introduce']}',
                        secret_member = '{$_POST['secret_member']}'
                        {$sql_certify}
                        ";
    ...
    // 장애 유형 저장
    $disab_type1 = explode(',', $_POST['disab_type1']);
    $disab_type2 = explode(',', $_POST['disab_type2']);
    for($i=0; $i<count($disab_type1); $i++) {
        $sql = " insert into g5_member_disabled set mb_no = {$mb_no}, disab_type1 = '{$disab_type1[$i]}', disab_type2 = '{$disab_type2[$i]}', reg_date = now() ";
        sql_query($sql);
    }
    ...
    */
    ...
    if ($mb_password) {
        ...
        //임시비밀번호 발급 후 비밀번호 변경 시 update_yn Y로 변경
        $sql = " update {$g5['member_table']} set mb_update_yn = 'Y' where mb_id = '{$mb_id}' ";
        sql_query($sql);
    }
    ...
    $sql = " update {$g5['member_table']}
                set mb_join_type_de = '{$_POST['join_type_de']}',
                    mb_nick = '{$mb_nick}',
                    mb_introduce = '{$_POST['mb_introduce']}',
                    secret_member = '{$_POST['secret_member']}'
                    {$sql_password}
                    ...
                    mb_9 = '{$mb_9}',
                    mb_10 = '{$mb_10}',*/
                    // mb_sex = '{$mb_sex}', -- 성별 수정 시 주석 해제
    sql_query($sql);

    // 장애 유형 저장
    if($_POST['delete_yn'] == 'Y') {
        $mb_no = $member['mb_no'];
        sql_query(" delete from g5_member_disabled where mb_no = {$mb_no}; "); // 기존 데이터 삭제 후 새로 저장
        $disab_type1 = explode(',', $_POST['disab_type1']);
        $disab_type2 = explode(',', $_POST['disab_type2']);
        for($i=0; $i<count($disab_type1); $i++) {
            $sql = " insert into g5_member_disabled set mb_no = {$mb_no}, disab_type1 = '{$disab_type1[$i]}', disab_type2 = '{$disab_type2[$i]}', reg_date = now(); ";
...
if ($w == '') {
    //goto_url(G5_BBS_URL.'/payment.php?mb_no='.$mb_no);
    goto_url(G5_HTTP_BBS_URL.'/register_result.php');
} else if ($w == 'u') {
    alert('회원 정보가 수정되었습니다', G5_BBS_URL.'/mypage.php', false);
