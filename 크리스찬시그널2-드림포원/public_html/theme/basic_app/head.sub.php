...
<link href="<?php echo G5_THEME_CSS_URL; ?>/swiper.min.css" rel="stylesheet" type="text/css"><!--스와이퍼-->
<link href="<?php echo G5_THEME_CSS_URL; ?>/mobile.css" rel="stylesheet" type="text/css">
<?php
...
add_stylesheet('<link rel="stylesheet" href="'.G5_THEME_CSS_URL.'/slick.css">', 0);
add_stylesheet('<link rel="stylesheet" href="'.G5_THEME_CSS_URL.'/slick-theme.css">', 0);
...
<script src="<?php echo G5_THEME_JS_URL ?>/swiper.min.js"></script><!--스와이프js -->
...
<style>
    <?php if($member['mb_id'] == 'hong') { // 스토어 아이디는 비공개 처리 (리뷰 숨김 등) ?>
    .store_noshow{display: none !important;}
    <?php } ?>
</style>
...
<script type="text/javascript">
	/*function fcmKey(token){
		$.ajax({
			url:"<?=G5_BBS_URL?>/ajax.token.update.php",
			type:"POST",
			data:{token:token},
			dataType:"json",
