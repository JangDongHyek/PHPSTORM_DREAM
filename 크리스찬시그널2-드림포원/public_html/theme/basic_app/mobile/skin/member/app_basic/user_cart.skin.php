...
<div id="mem_love">
	<div class="inb">
    	<div class="cons"><!--2020년 10월 20일-->결제후회원님들을 보실 수 있습니다.</div><!--con-->
    	<div class="all_del"><a href="javascript:void(0);" onclick="member_cart_del();"><span>*</span> 선택한 회원님 <strong>삭제</strong></a></div><!--all_chk-->
        <div id="mem_list">
            <table width="100%" border="0" cellspacing="0" cellpadding="0">
                ...
                for($i=0; $mb=sql_fetch_array($result); $i++) {
                    ...
                    // 프로필 이미지 (첫번째 사진 한장)
                    $sql = " select * from g5_member_img where mb_no = {$mb['cart_mb_no']} order by idx limit 1";
                    $file = sql_fetch($sql);
...
-->
                                   <span><?=$name?><!-- 이름 --></span> / 
                                    ...
                                    <div class="con"><?=$mb['mb_introduce']?></div><!--나의 신앙고백 내용이 추출될예정)/최대 3줄까지만 표현되도록-->
                                    <div class="date"><i class="fas fa-heart"></i> <?=date("Y-m-d",strtotime($mb['wr_datetime']))?></div><!--관심등록한 날짜시간-->
                                </div>
                            </a>
                        </div><!--memb-->
                    </td>
            ...
            switch (value) {
                case "defeat":
                    $.ajax({
                        ...
                        data: {del_member: del_member, mode: "cart_out"},
