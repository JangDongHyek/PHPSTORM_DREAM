...
<div id="msg">
	<!--상단카테고리-->
    <ul class="cate cf">
        <li class="active"><a href="javascript:void(0);">받은 메세지</a></li>
        <li><a href="javascript:void(0);" onclick="tab_move('msg_to.php');">보낸 메세지</a></li>
    ...
    <div class="in" style="padding-top: 5px !important;">
        ...
        for($i=0; $row=sql_fetch_array($result); $i++) {
            ...
            // 수신확인
            if(!empty($row['message_state'])) $message_state = ''; else $message_state = 'new';
        ?>
            <div class="list cf">
                <div class="<?=$message_state?>"><?php if($message_state) echo 'N';?></div><!--새로운 메세지(읽지않은)가 있을시 new아이콘이 나타나게-->
                <?php if($row['mb_level'] != 10 && $row['propose'] == 'ON') { ?> <!-- 관리자에게 신청 X -->
                <div class="bts propose"><a href="javascript:void(0);" onclick="send_propose('<?=$row['mb_no']?>', '<?=$row['mb_nick']?>');"><i class="fas fa-hand-holding-heart"></i></a></div><!--데이트 신청-->
                <?php } ?>
                <div class="bts write"><a href="javascript:void(0);" onclick="send_message_modal('<?=$row['mb_no']?>', '<?=$row['mb_nick']?>');"><i class="fas fa-pencil-alt"></i></a></div><!--메세지 보내는 창 뜨게-->
                ...
                <div class="info">
                    <h2 class="nick"><strong>From.</strong><?php if($row['secret_member'] == 'Y') { ?><i class="secret"></i><?php } ?><span><?=$row['mb_nick']?></span>님</h2>
                    <div class="date">수신일 : <?=$row['message_date']?></div>
                    <div class="con"><?=$row['message']?></div><!--내용글 최대 두줄까지만 보여줌, ....처리-->
                    <!--<a href="<?php /*echo G5_BBS_URL */?>/msg_view.php?idx=<?/*=$row['idx']*/?>&mode=receive" class="view">메세지 더보기 +</a>-->
        ...
        ?>
        <!--받은 메세지가 없을때 띄워주세요.-->
        <div class="msg_none"><span><i class="fas fa-paper-plane"></i></span> 받은 메세지가 없습니다.</div>
        <!--받은 메세지가 없을때 띄워주세요.-->
        <?php
        }
        ?>
...
function view_move(idx) {
    ...
    location.href = '<?php echo G5_BBS_URL ?>/msg_view.php?idx='+idx+'&mode=receive';
}
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        if(data) {
                            swal('메세지가 삭제되었습니다.')
                            .then(() => {
                                location.replace(g5_bbs_url + '/msg_from.php');
                            });
                        }
                    }
                });
