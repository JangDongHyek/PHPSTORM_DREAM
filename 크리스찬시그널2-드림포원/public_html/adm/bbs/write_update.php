...
if ($w == '' || $w == 'r') {
    ...
    // 21.05.13 공지사항 푸시
    if($bo_table == 'notice' && $w == '') {
        $sql = " select * from g5_member where mb_level = 2 and alarm = 'ON' ";
        $result = sql_query($sql);
        for($i=0; $row=sql_fetch_array($result); $i++) {
            $sql = " select * from g5_fcm where mb_id = '{$row['mb_id']}' ";
            $fRow = sql_fetch($sql);
	...
	/*if($bo_table=="notice"){
			...
			for($j=0;$j<$total;$j++){
				...
				$message=array(
							"message"=>"크리스찬시그널에서 새로운 공지사항이 있습니다.",
							"subject"=>"크리스찬시그널에서 새로운 공지사항이 있습니다.",
							"goUrl"=>G5_URL."/bbs/board.php?bo_table=notice&wr_id=".$wr_id
							);
