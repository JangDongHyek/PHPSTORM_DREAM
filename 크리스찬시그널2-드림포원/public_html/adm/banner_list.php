...
$sql_serch = ' where 1=1 ';
if($_REQUEST['ba_place']){
    $sql_serch .= ' and ba_place = "'.$_REQUEST['ba_place'].'"';
}

$sql = " select count(*) as cnt FROM new_adm_banner {$sql_serch}";
$cnt =  sql_fetch($sql);
...
// 배너 리스트
$sql = "SELECT * FROM new_adm_banner {$sql_serch} ORDER BY FIELD(ba_place, 'top','btm','ctg') limit {$from_record}, {$rows} ";
...
<div class="btn_add01 btn_add" style="float:left;">
    <button onclick="location.href='<?=G5_ADMIN_URL?>/banner_list.php?ba_place=top'">상단</button>
    <button onclick="location.href='<?=G5_ADMIN_URL?>/banner_list.php?ba_place=btm'">하단</button>
</div>

<div class="btn_add01 btn_add" style="float:right;">
    <a onclick="getModalOpen()">배너등록</a>
</div>
<form name="fbanner" id="fbanner" action="<?=G5_ADMIN_URL?>/adm.ajax.controller.php" method="post">
            ...
            if ($result_cnt == 0) {
                ...
                for ($i=0; $row=sql_fetch_array($result); $i++) {
                    ...
                    <tr class="<?php echo $bg; ?>" style="text-align:center;">
                        ...
                        <td>
                            <?= $row['ba_new_tab'] == '1' ? '새탭으로 이동': "사이트에서<br>이동";?>
                        </td>
                        <td>
                            <? if ($img_exists) { ?>
                        ...
                        <td><?=$row['nw_begin_time']?></td>
                        <td><?=$row['nw_end_time']?></td>
                        <td>
                            <?=$yn_list[$row['ba_use']]?>
                        </td>
                        <td>
