...
function get_member_no($mb_id, $fields='*')
{
    global $g5;

    $result =  sql_fetch(" select $fields from {$g5['member_table']} where mb_no = TRIM('$mb_id') ");
    return $result;
...
function is_admin($mb_id)
{
    ...
    if ($config['cf_admin'] == $mb_id || $mb_id == "lets080" || $mb['mb_level'] > 8) return 'super';
...
$profile_color = array(
  1=>'빨간색', 2=>'파란색', 3=>'노란색', 4=>'초록색', 5=>'보라색',
);
$profile_flower = array(
    1=>'장미', 2=>'튤립', 3=>'백합', 4=>'라일락', 5=>'해바라기', 6=>'수선화', 7=>'개나리', 8=>'안개꽃', 9=>'민들레', 10=>'진달래',
);
$profile_sports = array(
    1=>'야구', 2=>'축구', 3=>'농구', 4=>'골프', 5=>'수영', 6=>'볼링', 7=>'테니스', 8=>'배드민턴', 9=>'탁구', 10=>'자전거',
);
$profile_age = array(
...
function sendFcm($tokens, $message)
{
	...
	$fields = array(
		...
		'data' => $message,
		'priority' => 'high',
		'notification' => array( 'title' => $message['subject'], 'body' => $message['message'],'url' => "http://itforone.com/~cwsignal1")
		);
	$headers = array(
...
function sendFcmIOS($tokens, $message) {
	...
	if($message['goUrl'] == "" || $message['goUrl'] == null){
		$message['goUrl'] = "http://itforone.com/~cwsignal1";
	}
	...
	$headers = array(
		'Authorization:key =AAAAk88RL1U:APA91bG2-VmkpmDoANNVWWucwo-VzrOF5SgRz_XiT-IpR5nB501kqjXqZS9hfPPD-AeLJGg7AoITib89JHGtUBGh_NHkUQ1hzxvnNLQeSWJ6WSs0-g_DjefZcmYg67ENjdUXksTamqVU',
		'Content-Type: application/json'
	);
...
function generateJWT($kid, $iss, $sub) {
	...
	$privKey = openssl_pkey_get_private(file_get_contents(G5_BBS_PATH.'/AuthKey_9FC7HLR4T7.pem'));
...
function blockUser($mb_id) {
    $block_rlt = sql_query(" select * from g5_member_report where mb_id = '{$mb_id}' ");
    $block = '';
    while($block_row = sql_fetch_array($block_rlt)) {
        $block .= "'".$block_row['report_mb_id']."',";
    }
...
function goSms($reserv_phone, $send_phone, $msg)
{
    ...
    //6회부터 오류창 표시
//    if ($result['cnt'] > 6){
//        die(json_encode(array( 'msg' => "하루 인증횟수(6회)를 초과하였습니다.")));
//        exit();
//    }
    if(!$tran_callback1){
