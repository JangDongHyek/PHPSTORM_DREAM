...
function pjax(options) {
  ...
  options.success = function(data, status, xhr) {
    ...
    if(options.container != "#pjax_view")
...
function locationReplace(url, options) {
  // visibled add
  if (options.visibled){
    $.get(options.session, function (e){
  ...
  if (state && state.container) {
    ...
    if (container.length) {
      ...
      if (contents) {
        ...
        // visibled add
		if($(container.selector).hasClass(pjax.options.visibled)){
			$(container.selector).removeClass(pjax.options.visibled);
			setTimeout(function (){
