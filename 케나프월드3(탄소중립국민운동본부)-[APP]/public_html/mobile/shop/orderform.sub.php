...
ob_start();
?>
    <p class="sod_frm_top"><i class="fal fa-map-marker-exclamation"></i> 주문하실 상품을 확인하세요.</p>
    ...
    <ul class="sod_list">
        ...
        $comm_tax_mny = 0; // 과세포인트
        $comm_vat_mny = 0; // 부가세
        $comm_free_mny = 0; // 면세포인트
        $tot_tax_mny = 0;
        ...
        <li class="sod_li">
            ...
            <div class="li_total">
                ...
                <?php /*?><span class="total_point total_span"><span>적립포인트 </span><strong><?php echo number_format($sum['point']); ?></strong></span><?php */?>
            </div>
	...
	<div id="sod_bsk_tot_wrap">
        ...
        <dl id="sod_bsk_tot">
            ...
            <dd class="sod_bsk_cnt">
                <?php $tot_price = $tot_sell_price + $send_cost; // 총계 = 주문상품포인트합계 + 배송비 ?>
                <strong id="ct_tot_price" style="color:#009975;"><?php echo number_format($tot_price); ?> 원</strong>
            </dd>
            <?php /*?><dt class="sod_bsk_point">포인트</dt>
            <dd class="sod_bsk_point"><strong><?php echo number_format($tot_point); ?> 점</strong></dd><?php */?>
        </dl>
	</div><!--#sod_bsk_tot_wrap-->
<?php
...
<div id="sod_frm">
    ...
    <section id="sod_frm_orderer" >
        ...
        <div class="odf_tbl">
            ...
            <tr>
                ...
                <td><input type="email" name="od_email" value="<?php echo $member['mb_email']; ?>" id="od_email" class="frm_input" maxlength="100"></td>
            </tr>
    ...
    <section id="sod_frm_pay">
        ...
        <div class="odf_tbl">
            ...
            <tr>
                <th scope="row">주문할인포인트</th>
                <td><span id="od_cp_price">0</span>P</td>
            </tr>
            <?php } ?>
            <?php if($sc_cnt > 0) { ?>
            <tr>
            ...
            <tr>
                <th scope="row">배송비할인포인트</th>
                <td><span id="sc_cp_price">0</span>P</td>
            </tr>
            <?php } ?>
            <tr>
                <th>총 주문금액</th>
             ...
             <tr>
                <th>추가배송비</th>
                <td><span id="od_send_cost2">0</span>원<br /> <span style="font-size:11px;">(지역에 따라 추가되는 도선료 등의 배송비입니다.)</span></td>
            </tr>
            </tbody>
            </table>
        </div>
        ...
        if ($is_member && $config['cf_use_point'])
        {
            ...
            if ($member['mb_point'] >= $default['de_settle_min_point'])
            {
                ...
                echo '<div id="sod_frm_pt"><input type="hidden" name="max_temp_point" value="'.$temp_point.'">결제포인트 : <input type="hidden" id="od_temp_point" name="od_temp_point" value="'.$tot_price.'" class="frm_input" size="10">'.number_format($tot_price).'포인트</div>';
...
$(function() {
    ...
    $(document).on("click", ".cp_apply", function() {
        ...
        if(parseInt(price) == 0) {
            if(!confirm(subj+"쿠폰의 할인 포인트은 "+price+"P입니다.\n쿠폰을 적용하시겠습니까?")) {
                return false;
            }
        }
        ...
        if(sell_price < 0) {
            alert("쿠폰할인포인트이 상품 주문포인트보다 크므로 쿠폰을 적용할 수 없습니다.");
            return false;
        }
    ...
    $("#od_coupon_btn").click(function() {
        ...
        if(price <= 0) {
            alert('상품포인트이 0P이므로 쿠폰을 사용할 수 없습니다.');
            return false;
        }
        $.post(
            "./ordercoupon.php",
    ...
    $(document).on("click", ".od_cp_apply", function() {
        ...
        if(od_price - price <= 0) {
            alert("쿠폰할인포인트이 주문포인트보다 크므로 쿠폰을 적용할 수 없습니다.");
            return false;
        }
...
function calculate_total_price()
{
    ...
    $("#ct_tot_coupon").text(number_format(String(tot_cp_price))+" P");
    $("#ct_tot_price").text(number_format(String(tot_sell_price))+" P");
...
{
    var f = document.forderform;
		if(parseInt("<?=$member[mb_point]?>")<parseInt("<?=$tot_price?>")){
			alert("포인트 충전 후에 주문하실 수 있습니다.");
			location.href='<?=G5_BBS_URL?>/mywallet.php';
			return false;
		}
    // 필드체크
...
function payment_check(f)
{
    ...
    if (typeof(f.od_temp_point) != "undefined") {
        if (f.od_temp_point.value)
        {
            ...
            if (temp_point > od_price) {
                alert("상품 주문포인트(배송비 제외) 보다 많이 포인트결제할 수 없습니다.");
                f.od_temp_point.select();
                return false;
            }
    ...
    if (document.getElementById("od_settle_card")) {
        if (document.getElementById("od_settle_card").checked) {
            if (tot_price < 1000) {
                alert("신용카드는 1000P 이상 결제가 가능합니다.");
                return false;
            }
        }
    }
