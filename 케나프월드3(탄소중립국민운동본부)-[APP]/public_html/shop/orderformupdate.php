...
//포인트 상위 추천인에게 적립
//$point = $tot_ct_price; 이거 0포인트로 오류같음
$point = $od_temp_point;
...
for($i=1;$i<=10;$i++){
	if($mb_recommend){
		...
		$po_id = insert_point($mb_recommend, $spoint*0.09, $i."대".$mb_name.'님 상품 구매', '@wallet', $mb_id, $wl_id);
		$po_id_l = insert_point_l($mb_recommend, $spoint*0.01, $i."대".$mb_name.'님 상품구매', '@wallet', $mb_id, $wl_id);
...
//if ($is_member && $od_receipt_point)
    ...
    insert_point($member['mb_id'], (-1) * $od_temp_point, "주문번호 $od_id 결제");
