...
<!--<form name="flist" id="flist" method="post" autocomplete="off" onsubmit="return frmSubmit(this)">-->
	...
	<div class="local_sch01">
		...
		<input type="text" name="param" class="frm_input" placeholder="이름, 가입경로">
...
<script>
	...
	// 이름검색 엔터
	$('input[name=param]').keydown(function() {
		if (event.keyCode === 13) {
...
function getMemberList(group) {
	...
	$.ajax({  
		type : "post",  
		url : "./ajax.sms_petition_member_list.php",
		data : {
		    "mb_group" : group,
            "param" : document.querySelector('[name=param]').value
        },
		dataType : "html",  
