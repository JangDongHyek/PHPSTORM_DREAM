...
$sql_common = " from {$g5['point_table_m']} ";
...
<div class="local_ov01 local_ov">
    ...
    if (isset($mb['mb_id']) && $mb['mb_id']) {
        echo '&nbsp;(' . $mb['mb_id'] .' 님 포인트 합계 : ' . number_format($mb['mb_point_m']) . '점)';
    } else {
        $row2 = sql_fetch(" select sum(po_point) as sum_point from {$g5['point_table_m']} ");
...
<section id="point_mng">
    ...
    <form name="fpointlist2" method="post" id="fpointlist2" action="./point_m_update.php" autocomplete="off">
    ...
    <div class="tbl_frm01 tbl_wrap">
        ...
        <tr>
            ...
            <td>
				...
				<input type="checkbox" name="mb_register" id="mb_register" value="1" class="">
				<label for="mb_register">회원가입 포인트 지급</label>
				<p>
					회원가입 포인트 지급시, 해당회원의 포인트(10%)가 추천인에게도 충전됩니다.
				</p>
			</td>
        </tr>
        <tr>
...
<form name="fpointlist" id="fpointlist" method="post" action="./point_m_list_delete.php" onsubmit="return fpointlist_submit(this);">
<input type="hidden" name="sst" value="<?php echo $sst ?>">
<input type="hidden" name="sod" value="<?php echo $sod ?>">
<input type="hidden" name="sfl" value="<?php echo $sfl ?>">
<input type="hidden" name="stx" value="<?php echo $stx ?>">
...
$("#mb_register").on("click", function (e){
	if($(this).prop("checked")){
		$("#po_content").attr("readonly", "readonly");
		$("#po_content").val("회원가입 포인트 지급");
	}else{
		$("#po_content").removeAttr("readonly");
		$("#po_content").val("");
	}
});
</script>
