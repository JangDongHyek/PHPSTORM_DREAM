...
$sql = " select count(*) as cnt from g5_movie_point where (1=1) {$sql_search}";
$row = sql_fetch($sql);
$total_count = $row['cnt'];
...
$sql = " select * from g5_movie_point where (1=1) {$sql_search} order by idx desc limit {$from_record}, {$rows} ";
$result = sql_query($sql);

$g5['title'] = '동영상 포인트 신청목록';

$write_pages = get_paging($rows, $page, $total_page, G5_ADMIN_URL.'/movie_point_list.php?'.$qstr.'&amp;page=');
$statusArr=array("0"=>"대기","1"=>"수락","-1"=>"거절");
include_once('./admin.head.php');
?>
<article class="text-center" style="padding:20px 0;">
	<form name="fboardlist" id="fboardlist" action="./movie_list_update.php" onsubmit="return fboardlist_submit(this);" method="post">
	...
	<div class="tbl_head01 tbl_wrap">
		...
		<tr class="<?php echo $bg; ?>">
			<td class="td_chk">
				...
				<input type="checkbox" name="chk[]" value="<?php echo $i ?>" id="chk_<?php echo $i ?>"<?php echo $row[status]!="0"?"disabled":"";?>>
        <input type="hidden" name="idx[<?php echo $i ?>]" value="<?php echo $row['idx'] ?>">
			...
			<td class="td_datetime" style="width:200px;"><?php echo number_format($row['point']);?></td>
			<td class="td_datetime" style="width:200px;"><?php echo date("Y-m-d H:i:s",$row['regdate']);?></td>
			<td><?php echo $statusArr[$row['status']];?></td>
			<!--<td><?php echo $row['po_id'];?></td>-->
			<td>
				<? if($row[status]=="0"){?>
				<input type="button" value="수락" class="btn btn-default btn-sm" onclick="return setStatus('<?php echo $row['idx'];?>', '1')">
				<input type="button" value="거절" class="btn btn-default btn-sm" onclick="return setStatus('<?php echo $row['idx'];?>', '-1')">
...
function setStatus(idx, status){
	var qTxt=status=="1"?"수락":"거절";
	if(confirm(qTxt+"(으)로 상태를 변경하시겠습니까? \n※ 수락시, 신청된 포인트가 지급됩니다.")){
		$.get("<?php echo G5_ADMIN_URL;?>/ajax.movie_point_update.php", {idx:idx, status:status}, function (e){
			console.log(e);
			if(e.success)
				location.reload();
			else
