		...
		*/
		...
		if($tmp = rg_str_inword($bbs[bbs_deny_word],$rg_title)) {
				echo ("
                        ...
                        <form name='form' action='write.php?bbs_id=$bbs_id' method='post'>
...
//		$rg_reply_mail 
		...
		if($bbs_id == "flash"){
			...
			if($tot_cot > 100){
				rg_href("list.php?bbs_id=$bbs_id","100개까지만 등록이 가능합니다");
			}
...
############################################# 다중 첨부파일 START ##########################################		
//필요한파일 : ./upload.php ./multi_upload.swf ./fu.js ./files(777) write.php스킨파일
...
if($_GET["cmd"]=="exec"){
	...
	$rg_files_count = count($_FILES['fu03']['tmp_name']);
	...
	`rg_top_num` = '$rg_top_num'	";
		for($i=0;$i<$rg_files_count;$i++){
			if(is_array($_FILES['fu03']['name'])){//이안에 파일이름 업데이트(갯수많큼)
	

				$_FILES['fu03']['name'][$i] = str_replace(" ","_",$_FILES['fu03']['name'][$i]);
				$_FILES['fu03']['name'][$i] = str_replace("\'","",$_FILES['fu03']['name'][$i]);
			
				########### DB저장 #########
					$fi = $i+1;
					$dbqry .= ", `rg_file{$fi}_name` = '".$_FILES[fu03][name][$i]."'";
					$dbqry .= ", `rg_file{$fi}_size` = '".$_FILES[fu03][size][$i]."'";
				########### DB저장 #########

	
				
				copy($_FILES['fu03']['tmp_name'][$i], "./data/$bbs_id/".$rg_doc_num.'$'.$fi.'$'.$_FILES['fu03']['name'][$i]) ;
				MakeThum_Gall($rg_doc_num,$fi,$bbs_id,$_FILES['fu03']['name'][$i]);
				@unlink($_FILES['fu03']['tmp_name'][$i]);
			}else{

				$_FILES['fu03']['name'] = str_replace(" ","_",$_FILES['fu03']['name']);
				$_FILES['fu03']['name'] = str_replace("\'","",$_FILES['fu03']['name']);

				########### DB저장 #########
					$fi = $i+1;
					$dbqry .= ", `rg_file{$fi}_name` = '".$_FILES[fu03][name]."'";
					$dbqry .= ", `rg_file{$fi}_size` = '".$_FILES[fu03][size]."'";
				########### DB저장 #########

				
				copy($_FILES['fu03']['tmp_name'], "./data/$bbs_id/".$rg_doc_num.'$'.$fi.'$'.$_FILES['fu03']['name']) ;
				MakeThum_Gall($rg_doc_num,$fi,$bbs_id,$_FILES['fu03']['name']);
				@unlink($_FILES['fu03']['tmp_name']);
			}
		}
		
	$dbqry .= ", `rg_ext1` = '$rg_files_count'";
