<?
  ...
  $order_str='';
	...
	if($bbs_id == "noticee"){
		$order_str = " ORDER BY rg_reg_date DESC, rg_next_num DESC";
	}
...
// 스킨 체크
	$bbs_notice_skin_file = "list_main.php";
	$bbs_secret_skin_file = "list_main.php";
	$bbs_current_doc_skin_file = "list_main.php";
	$bbs_deleted_skin_file = "list_main.php";


	if (file_exists($skin_board_path . "list_notice.php"))
		$bbs_notice_skin_file = "list_notice.php";
	if (file_exists($skin_board_path . "list_secret.php"))
		$bbs_secret_skin_file = "list_secret.php";
	...
	while ($data = mysql_fetch_array($rs)) {
		...
		if ($rg_doc_num == $current_doc_num) { // 현재글
			...
			include($skin_board_path . "list_main.php");
		}
	}
	...
	$dbqry="
		...
		FROM `$bbs_table` LEFT JOIN `$db_table_member`
			ON rg_mb_num = mb_num
		WHERE (1=1) AND rg_notice = 0 $where_str
		$order_str
