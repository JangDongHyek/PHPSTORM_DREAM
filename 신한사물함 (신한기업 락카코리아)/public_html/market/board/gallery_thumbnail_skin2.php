<?
	...
	$rows_count = "2";
	// 이미지 가로
	$gallery_img_width = "190";
	// 이미지 세로
	$gallery_img_height = "268";
	// 썸네일 지원 여부
	$pagecount = 20;
	//========================= 그림파일이 있을때 출력 =======================================
...
?>
						...
						<style type="text/css">
<!--
.style1 {font-size:18px; margin:6px 0;}
...
if($bbs_no == 8){
	...
	$SQL2 = "and binary $keyset like '%$searchword%' ";
	$SQL2_1 = "and (binary writer like '%$searchword%' or binary subject_new like '%$searchword%' or binary content like '%$searchword%') ";
	$SQL3 = "order by $order_by limit $skipNum, $cnfPagecount";
...
<?
	...
	for ($i=0; $i < $numRows; $i++){
		...
		$admin_orderby = $row[admin_orderby];

		//================= 파일 경로 ==================
		$target = "$upload"."$userfile";
		$target2 = "$upload"."$userfile1";
		$target3 = "$upload"."$userfile2";
...
?>
							<td width="<?=intval(100/$rows_count)?>%">
								<table width=100% height=100% border=0 cellpadding=0 cellspacing=0 >
									<tr>
										<td align="center">
											<table width="398" height="<?=$gallery_img_height+20?>" border=0 cellpadding=10 cellspacing=1 bgcolor="000" style="margin-right:8px; margin-left:4px;">
							
												<tr>
													<td height="100%" bgcolor="fdf0e8"><table width="100%" border="0" cellspacing="0" cellpadding="0">
                                                      <tr>
                                                        <td width="190">
														<?if($_SESSION["Mall_Admin_ID"]){?>
														<a href='board_read.php?index_no=<?=$index_no?>&amp;bbs_no=<?=$bbs_no?>&amp;page=<?=$page?>&amp;mart_id=<?=$mart_id?>&amp;keyset=<?=$keyset?>&amp;searchword=<?=$searchword?>'><img src="<?=$target?>" width="190" height="275" border="0" style="border:1px solid #CCCCCC" /></a>
														<?}else{?>
														<img src="<?=$target?>" width="190" height="275" border="0" style="border:1px solid #CCCCCC" />
														<?}?>
														</td>
                                                        <td align="right" valign="bottom"><table width="40%" border="0" cellspacing="0" cellpadding="10">
                                                          <tr>
                                                            <td><a href="./img_down.php?target=<?=$target2?>" target="_blank"><img src="img/down_icon.png" width="60" / border="0"></a></td>
                                                          </tr>
                                                          <tr>
                                                            <td><a href="./img_print.php?target=<?=$target3?>" target="_blank"><img src="img/printer_icon.png" width="60" border="0"></a></td>
                                                          </tr>
                                                        </table></td>
                                                      </tr>
                                                    </table>	
									...
									<tr>
										<td align=center nowrap height=34><span class="style2">
										<?if($_SESSION["Mall_Admin_ID"]){?>
										<a href='board_read.php?index_no=<?=$index_no?>&bbs_no=<?=$bbs_no?>&page=<?=$page?>&mart_id=<?=$mart_id?>&keyset=<?=$keyset?>&searchword=<?=$searchword?>'><span class="style1">
									    <?=$subject_new?>
										</a> 
										<?}else{?>
										<?=$subject_new?>
										...
										<?=$secret_str?>
										<?=$new_string?>
										<?if($_SESSION["Mall_Admin_ID"]){echo"($admin_orderby)";}?>
										</span></span></td>
							        </tr>
								</table>
							</td>
