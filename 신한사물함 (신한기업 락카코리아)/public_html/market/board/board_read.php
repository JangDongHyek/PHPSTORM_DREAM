...
$userfile2 = $ary[userfile2];
$mtm_telno1 = $ary[mtm_telno1];
$address = $ary[address];
...
if( eregi("\.jpg", $userfile) || eregi("\.gif", $userfile) || eregi("\.JPG", $userfile) || eregi("\.GIF", $userfile) ){
	...
	if( $userfile ){
		list( $height_new, $width_new ) = img_size( "$target", "800", "10000");
	}

	$img_file = "<img src='$encode_target' width='$width_new' height='$height_new' border='0' hspace='5' style='border: #000000; border-style: solid; border-width:0px'><br>".$img_file;
...
if( eregi("\.jpg", $userfile1) || eregi("\.gif", $userfile1) || eregi("\.JPG", $userfile1) || eregi("\.GIF", $userfile1)){
	...
	//=============================== 가로,세로 비율에 맞게 이미지 맞추기 ================
	if( $userfile1 ){
		list( $height_new1, $width_new1 ) = img_size( "$target1", "800", "10000");
	}

	$img_file1 = "<img src='$encode_target1' width='$width_new1' height='$height_new1' border='0' hspace='5' style='border: #000000; border-style: solid; border-width:0px'><br>".$img_file1;
...
//========================= 첨부파일처리 =================================================
//========================= userfile2 ====================================================
if( eregi("\.jpg", $userfile2) || eregi("\.gif", $userfile2) || eregi("\.JPG", $userfile2) || eregi("\.GIF", $userfile2)){
	//==================== 이미지 사이즈를 구함 ==========================================
	$img_size2 = GetImageSize("$target2");

	if( eregi("\.jpg", $userfile2) || eregi("\.gif", $userfile2) || eregi("\.JPG", $userfile2) || eregi("\.GIF", $userfile2)){
		$img_width2 = $img_size2[0]; //이미지의 넓이를 알 수 있음 
		$img_height2 = $img_size2[1]; //이미지의 높이를 알 수 있음
	}

	//=============================== 가로,세로 비율에 맞게 이미지 맞추기 ================
	if( $userfile2 ){
		list( $height_new2, $width_new2 ) = img_size( "$target2", "800", "10000");
	}

	$img_file2 = "<img src='$encode_target2' width='$width_new2' height='$height_new2' border='0' hspace='5' style='border: #000000; border-style: solid; border-width:0px'><br>".$img_file2;
}else if(eregi("\.swf", $userfile2)){
	$img_file2 = "<embed src='$encode_target2' border='0'><br>".$img_file2;
}

if( $userfile2 ){
	$size2 = filesize($target2);
	$size2 = GetFileSize($size2);
}
//========================= 첨부파일처리 =================================================
...
?>
					...
					<table width="100%" align="center"  border="0" cellspacing="0" cellpadding="0">
						...
						<tr>
							<td width="70" height="30" align="center"><img src="../image/helpdesk/view_tel.gif" width="40" height="30"></td>
							<td width="1"><img src="../image/helpdesk/view_line.gif" width="1" height="30"></td>
							<td width="10"></td>
							<td><?=$mtm_telno1 ?></td>
						</tr>
						...
						<tr>
							<td width="70" height="30" align="center"><img src="../image/helpdesk/view_address.gif" width="40" height="30"></td>
							<td width="1"><img src="../image/helpdesk/view_line.gif" width="1" height="30"></td>
							<td width="10"></td>
							<td><?=$address ?></td>
						</tr>
...
<? 
	if( $userfile || $userfile1 || $userfile2 ){ 
?>
						<tr>
							<td bgcolor="E1E1E1" height="1" colspan="4"></td>
						</tr>
...
<? 
		if( $userfile2 ){ 
?>
								<a href='<?=$target2?>' target="_blank"><?=$userfile2?></a> (<?=$size2?>)&nbsp;
<?
		}
?>
							</td>
...
?>
						<tr>
							<td align="center"><!--a href='#1' onclick="window.open('big.html?file=<?=$userfile?>&mart_id=<?=$mart_id?>','원본사진보기','width=<?=$img_width?>,height=<?=$img_height?>,toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=no,resizable=no')" onfocus='blur();'-->
							  <?=$img_file?>
							</a></td>
						</tr>
<?
...
?>
						...
						<tr>
							<td align="center"><!--a href='#1' onClick="window.open('big.html?file=<?=$userfile1?>&mart_id=<?=$mart_id?>','원본사진보기','width=<?=$img_width1?>,height=<?=$img_height1?>,toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=no,resizable=no')" onFocus='blur();'--><?=$img_file2?></a></td>
						</tr>
<?
	}
?>
