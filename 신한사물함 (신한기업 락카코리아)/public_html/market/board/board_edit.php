...
$user_file2 = $ary[userfile2];
$admin_orderby = $ary[admin_orderby];
...
//========================= userfile2 ====================================================
if( eregi("\.jpg", $user_file2) || eregi("\.gif", $user_file2) ){
	$img_file2 = "<img src='$target2' border='0' hspace='5' style='border: #000000; border-style: solid; border-width:1px'><br>".$img_file2;
	//==================== 이미지 사이즈를 구함 ==========================================
	$img_size2 = GetImageSize("$target2"); 

	if( eregi("\.jpg", $user_file2) || eregi("\.gif", $user_file2) || eregi("\.JPG", $user_file2) || eregi("\.GIF", $user_file2)){
		$img_width2 = $img_size2[0]; //이미지의 넓이를 알 수 있음 
		$img_height2 = $img_size2[1]; //이미지의 높이를 알 수 있음
	}
}else if(eregi("\.swf", $user_file2)){
	$img_file2 = "<embed src='$target2' border='0'><br>".$img_file2;
}
...
<script src="../board/editor/easyEditor.js"></script>
					...
					<input type='hidden' name='user_file2' value='<?=$user_file2?>'>
...
?>							
					...
					<?if($bbs_no==5){?>
						...
						<tr>
							...
							<td><img src="../image/helpdesk/view_line.gif" width="1" height="30"></td>
							<td width="10"></td>
							<td ><input name="mtm_telno1" value='<?=$mtm_telno1?>' type="text" class="input_03" size="30" style='ime-mode:inactive'></td>
						</tr>
						<tr>
						  <td bgcolor="E1E1E1" height="1" colspan="4"></td>
						</tr>
						<tr>
							...
							<td><img src="../image/helpdesk/view_line.gif" width="1" height="30"></td>
							<td width="10"></td>
							<td ><input name="address" value='<?=$address?>' type="text" class="input_03" size="50" style='ime-mode:inactive'></td>
						</tr>
						<?}?>
						<tr>
							<td bgcolor="E1E1E1" height="1" colspan="4"></td>
...
?>
						...
						<tr>
							...
							<td><img src="../image/helpdesk/view_line.gif" width="1" height="30"></td>
							<td width="10"></td>
							<td><input type='file' name='userfile2' size="50">
							<?if($user_file2){?>
							<input type=checkbox name=userfile_delete3 value="y">삭제(<?=$user_file2?>)</td>
							<?}?>
							</td>
						</tr>
						<tr>
...
?>
						<tr>
							...
							<td><img src="../image/helpdesk/view_line.gif" width="1" height="30"></td>
							<td width="10"></td>
							<td><input type='text' name='admin_orderby' size="5" value="<?=$admin_orderby?>" class="input_03"> ※숫자가 클수록 먼저 노출됩니다.</td>
						</tr>
						<tr>
							<td bgcolor="E1E1E1" height="1" colspan="4"></td>
						</tr>		
...
<?
	...
	if($userfile_delete3 == "y" && $user_file2){

		$sql = "update $New_BoardTable set userfile2='' where index_no='$index_no' and mart_id='$mart_id'";
		$res = mysql_query( $sql, $dbconn );
	
		unlink("$UploadRoot$mart_id"."/"."$user_file2");
	}
	...
	if( $userfile2_name ){//첨부 파일을 업로드 했으면 파일명을 수정함
		$file2 = FileUploadName( "$user_file2", "$UploadRoot$mart_id/", $userfile2, $userfile2_name );
		$sql = "update $New_BoardTable set userfile2='$file2' where index_no='$index_no' and mart_id='$mart_id'";
		$res = mysql_query( $sql, $dbconn );
	...
	$SQL = "update $New_BoardTable set writer='$writer', email='$email', subject_new='$subject_new_tmp[0]', content='$content', if_secret='$if_secret', mtm_telno1='$mtm_telno1',admin_orderby='$admin_orderby',address='$address' where index_no ='$index_no' and bbs_no='$bbs_no' and mart_id='$mart_id'";
