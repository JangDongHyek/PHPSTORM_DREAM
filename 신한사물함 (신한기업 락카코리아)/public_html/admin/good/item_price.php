...
?>
		...
		<td>
			...
			<table width="100%" border="0" cellpadding="0" cellspacing="0">
				<tr>
				<td width="100%" height="50" bgcolor="F2F2F2" class="title"><img src="../images/icon_1.gif" width="30" height="17" align="absmiddle"><b>가격변동상품 관리</b></td>
				</tr>
			</table>
			...
			<table border="1" cellpadding="5" cellspacing="0" width="97%" bordercolordark="white" bordercolorlight="#E1E1E1" align="center">
				<tr>
				  <td vAlign="top" width="90%" bgColor="#ffffff"><b>[가격변동상품 관리]</b></td>
				</tr>
				<tr>
					 ...
					 <tr>
						...
						cellpadding="0">
						  <tr>
							 ...
							 <form action='item_price.php' method='post'>
...
?>
								  ...
								  <td width="18%" height="25">
									<select name='order' size="1" style="height: 18px; border: 1px solid black">
										<option value="item_name" <?if($keyset=='category_num' && $order == 'item_name') echo "selected";?>>상품명</option>
										<option value="item_company" <?if($keyset=='category_num' && $order == 'item_company') echo " selected";?>>제조사순</option>
										<option value="z_price" <?if($keyset=='category_num' && $order == 'z_price') echo "selected";?>>가격순</option>
										<option value="reg_date" <?if($keyset=='category_num' && $order == 'reg_date') echo "selected";?>>등록순</option>
									</select>
								  </td>
					 ...
					 <tr>
						...
						<table cellSpacing="1" cellPadding="3" width="100%" bgColor="#f7f7f7" border="0">
						  <tr bgColor="#FCB663" height="25" align='center'>
							 ...
							 <td width="12%"><b><font color="#ffffff">변경일</font></b></td>
							 <td width="12%"><b><font color="#ffffff">출력여부</font></b></td>
						  </tr>
						  
						  <form name='list' action='item_price.php' method='post'>
...
if(!empty($keyset)&&!empty($searchword)){
	if($keyset == 'item_name'){
		$SQL = "select mart_id, item_no, item_name, item_company, z_price, reg_date, bonus, member_price, price, provider_id, price_ok, price_date from $ItemTable where mart_id='$mart_id' and price_ok='n' and binary lower($keyset) like lower('%$searchword%') UNION select T1.mart_id, T1.item_no, T1.item_name, T1.item_company, T1.z_price, T1.reg_date, T1.bonus, T1.member_price, T1.price, T1.provider_id, T1.price_ok, T1.price_date from $ItemTable T1, $Gnt_ItemTable T2 where T2.seller_id='$Mall_Admin_ID' and T1.item_no= T2.item_no and binary lower(T1.$keyset) like lower('%$searchword%') order by $order asc";
	}
	if($keyset == 'category_num'){
		$SQL = "select mart_id, item_no, item_name, item_company, z_price, reg_date, bonus, member_price, price, provider_id, price_ok, price_date from $ItemTable where mart_id='$mart_id' and price_ok='n' and $keyset= '$searchword' UNION select T1.mart_id, T1.item_no, T1.item_name, T1.item_company, T1.z_price, T1.reg_date, T1.bonus, T1.member_price, T1.price, T1.provider_id, T1.price_ok, T1.price_date from $ItemTable T1, $Gnt_ItemTable T2 where T2.seller_id='$Mall_Admin_ID' and T1.item_no= T2.item_no and T2.$keyset ='$searchword' order by $order asc";
	}
	if($keyset == 'provider_id' ){
		$SQL = "SELECT * FROM $ItemTable AS a LEFT JOIN $MemberTable AS b ON a.provider_id = b.username WHERE b.mart_id='$mart_id' and a.price_ok='n' AND b.name LIKE '%$searchword%' order by $order asc";
	}
	...
	for ($i=$skipNum; $i < ($cnfPagecount+$skipNum); $i++) {
		...
		$price_ok = $ary[price_ok];
		$price_date = $ary[price_date];
		$price_date_str = substr( $price_date, 0, 10 );
		...
		echo "					
						  ...
						  <tr align='center' bgcolor='$bgcolor'>
							 ...
							 <td>$price_date_str</td>
							 <td>
								 <select name='price_ok[]' class='bb' style='BORDER-RIGHT: black 1px solid; BORDER-TOP: black 1px solid; BORDER-LEFT: black 1px solid; BORDER-BOTTOM: black 1px solid; HEIGHT: 18px' size='1' $disabled>
								 <option value='y'
										";
										if($price_ok == 'y') echo " selected";
										echo ">확인</option>
								 <option value='n'
										";
										if($price_ok == 'n') echo " selected";
										echo ">미확인</option>
								 </select>
							 </td>
						  </tr>
					 ...
					 <tr>
								  ...
								  <tr>
									 ...
									 <table border="0" width="100%">
										<tr>
										  ...
										  <input style="BACKGROUND-COLOR: white; BORDER-BOTTOM: #3D918A 1px solid; BORDER-LEFT:  #3D918A 1px solid; BORDER-RIGHT: #3D918A 1px solid; BORDER-TOP: #3D918A 1px solid; COLOR:#3D918A;width:100px;  HEIGHT: 18px" type="submit" value="저장하기">
										  &nbsp;</td>
						...
						<td width="91%" bgColor="#ffffff"><p align="right"><br>
							...
							if($page == 1){
								...
								echo ("
								<a href='item_price.php?page=1&keyset=$keyset&searchword=$searchword&order=$order&cnfPagecount=$cnfPagecount'>처음</a> 
								");
							}
						
							if($start_page > 1){
								echo ("
								<a href='item_price.php?page=$prev_start_page&keyset=$keyset&searchword=$searchword&order=$order&cnfPagecount=$cnfPagecount'>
								◁&nbsp; 
								</a>
								");
							}
							...
							for($i=$start_page;$i<=$end_page;$i++){
								if($i == $page){
									...
									echo ("
								<a href='item_price.php?page=$i&keyset=$keyset&searchword=$searchword&order=$order&cnfPagecount=$cnfPagecount'>$i</a> 	
									");
								}
							}
							if($end_page < $total_page){
								echo ("
								<a href='item_price.php?page=$next_start_page&keyset=$keyset&searchword=$searchword&order=$order&cnfPagecount=$cnfPagecount'>
								&nbsp;▷
								</a>
								");
							}
							...
							if($page == $total_page){
								...
								echo ("
								<a href='item_price.php?page=$total_page&keyset=$keyset&searchword=$searchword&order=$order&cnfPagecount=$cnfPagecount'>끝</a> 
								");
							}
							?></td>
					 </tr>
...
if($flag == 'update'){
	for($i=0; $i<count($item_no); $i++) {
		...
		if($mart_id_tmp == $Mall_Admin_ID){//내상품
			if($if_gnt_item == '1')
				$SQL = "update $ItemTable set z_price = $z_price[$i], price_ok='$price_ok[$i]', price_date='$date', member_price='$member_price[$i]' where item_no = $item_no[$i] and mart_id='$mart_id'";
			else
				$SQL = "update $ItemTable set z_price = $z_price[$i], price_ok='$price_ok[$i]', price_date='$date', member_price='$member_price[$i]' where item_no = $item_no[$i] and mart_id='$mart_id'";
		}
		else{//gnt 사품
	...
	echo "<meta http-equiv='refresh' content='0; URL=item_price.php?page=$next_start_page&keyset=$keyset&searchword=$searchword&order=$order&cnfPagecount=$cnfPagecount'>";
}
if($flag == 'diff'){
	for($i=0; $i<count($item_no); $i++) {
		...
		if($mart_id_tmp == $Mall_Admin_ID){//내상품
			...
			if($target == 'bonus'){
				...
				//현재 gnt_item테이블에 포인트가 0보다 큰지 확인 하여 크면 gnt_item 테이블 그대로 수정.
				//아니면 item테이블에서 포인트 가져와서 수정
