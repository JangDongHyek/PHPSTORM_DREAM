    ...
    if ($total_page > 1) {
        for ($k=$start_page;$k<=$end_page;$k++) {
            if ($cur_page != $k)
                ...
                $str .= '<span class="current">'.$k.'<span class="sound_only">페이지</span></span>'.PHP_EOL;
        }
    }
...
function ctg_list($p_idx){

    $sql = "select * from new_category where c_p_idx = '{$p_idx}' and c_use_yn = 1 order by c_number desc , c_idx ";
    $result = sql_query($sql);
...
function isMobile() {
    ...
    $mobileDevices = array(
        ...
        'Opera Mini', 'IEMobile', 'Mobile', 'webOS', 'Palm', 'Symbian'
    );
