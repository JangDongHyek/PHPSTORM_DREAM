...
$sql = " select cr.*, mb.mb_name, cm.msg from chat_room as cr 
         ...
         where 1=1 {$sql_search} group by cr.id order by cr.updatedAt desc ";
...
if(!empty($msg_cnt) && !empty($chat_cnt)) {
    for($i=0; $row=sql_fetch_array($result); $i++) {
        ...
        $you = sql_fetch(" select cru.*, mb.mb_id, mb.mb_nick from chat_room_user as cru left join g5_member as mb on mb.mb_id = cru.user_id where room_idx = '{$row['id']}' and user_id != '{$member['mb_id']}' ");
        // 방이름
        ...
        if(!empty($msg['file_name'])) {
			$pos        = strpos($msg['msg'], "<img");
			if($pos !== false) {
				$msg['msg'] = "이미지 파일"; // 이미지 첨부 시 파일명을 표시
			} else {
				$msg['msg'] = "첨부 파일"; // 이미지 첨부 시 파일명을 표시
			}
