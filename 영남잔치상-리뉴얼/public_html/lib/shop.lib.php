...
function get_it_image($it_id, $width, $height=0, $anchor=false, $img_id='', $img_alt='', $is_crop=false)
{
    ...
    if($filename) {
        ...
        $thumb = thumbnail($filename, $filepath, $filepath, $width, $height, false, $is_crop, 'center', false, $um_value='100/0.5/3');
    }
...
function get_item_supply($it_id, $subject, $is_div='')
{
    ...
    for($i=0; $row=sql_fetch_array($result); $i++) {
        ...
        if(strlen($opt_id[1])) {
            ...
            $options[$opt_id[0]][] = '<option value="'.$opt_id[0].chr(30).$opt_id[1].','.$row['io_price'].','.$io_stock_qty.'">'.$opt_id[1].$price.$soldout.'</option>';
        }
    }
...
function get_sendcost($cart_id, $selected=1)
{
    ...
    $sql = " select distinct it_id
                ...
                where od_id = '$cart_id'
                  and ct_send_cost = '0'
                  and ct_status IN ( '미완료', '쇼핑', '주문', '입금', '준비', '배송', '완료' )
