...
function conv_content($content, $html, $filter=true)
{
	...
	$content = str_replace("http://itforone.com/~janchisang2","https://www.janchisang.com",$content);
...
function get_editor_image($contents, $view=true)
{
    if(!$contents)
        return false;
	$contents = str_replace("http://itforone.com/~janchisang2","https://www.janchisang.com",$contents);
...
function goLms($reserv_phone,$send_phone,$msg){
	$conn_db=mysqli_connect("211.51.221.167","janchisang","sbtpsxja!@#","janchisang");
	if(!$conn_db){
		exit;
	}
	$mart_id = "janchisang"; //계정명
	$tran_etc3 = "janchisang"; //계정명
	$rphone=$reserv_phone;
	$allPhone=$send_phone;
	$number_receive_people=0;
	$mbs_msg_member_join=$msg;
	$send_date = date("Y-m-d H:i:s");
	$groupby = date("YmdHis")."_".rand(0,1000);//한번에 여러건 전송시 묶음 구분값
	$strlen = strlen($mbs_msg_member_join);
	if($strlen > 90 ){ //90바이트 넘으면 mms로 처리
		$file_cnt="0";//파일갯수
		$mms_body=$mbs_msg_member_join;//본문
		$mms_subject = cut_str($mbs_msg_member_join,18); 
		//일단 파일타입
		$file_cnt="0";
		$mms_query = "Insert into em_tran_mms (mms_seq,file_cnt,mms_body,mms_subject,file_type1,file_name1,service_dep1) values (null,'$file_cnt','$mms_body','$mms_subject','$file_type1','$file_name1','$service_dep1')";
		mysqli_query($conn_db,$mms_query);


		$mms_seq = mysqli_insert_id($conn_db);
		$tran_etc4 = $mms_seq;//mms참조값
		$tran_phone1 = $allPhone;//받는 사람 번호 관리자
		$tran_callback1 = $rphone;//보내는 사람 번호
		$tran_msg1 = $mbs_msg_member_join;
		$sms_query = "Insert into em_tran (tran_pr,tran_id,tran_phone,tran_callback,tran_status,tran_date,tran_msg,tran_etc2,tran_etc3,tran_etc4,tran_type) values (null,'$mart_id','$tran_phone1','$tran_callback1','1','$send_date','$tran_msg1','$groupby','$tran_etc3','$tran_etc4','6')";
		echo $sms_query;
		mysqli_query($conn_db,$sms_query);
	}else{
			$tran_phone1 = $allPhone;//받는 사람 번호 관리자
			$tran_callback1 = $rphone;//보내는 사람 번호
			$tran_msg1 = $mbs_msg_member_join;
		...
		//	$sms_query = "Insert into em_tran (tran_pr,tran_id,tran_phone,tran_callback,tran_status,tran_date,tran_msg,tran_etc2,tran_etc3,tran_type) values (null,'$mart_id','$tran_phone1','$tran_callback1','1','$send_date','$tran_msg1','$groupby','$tran_etc3','4')";
			
			$sms_query= "insert em_tran set tran_pr = null,
							tran_id='$mart_id',
							tran_phone='$tran_phone1',
							tran_callback='$tran_callback1',
							tran_status='1',
							tran_date='$send_date',
							tran_msg='$tran_msg1',
							tran_etc2='$groupby',
