...
for ($i=1; $row=sql_fetch_array($result); $i++) {?>
    ...
    if ($i == 1) {
        if ($this->css) {
            ...
            echo "<ul class=\"sct sct_10 swiper-wrapper \">\n";
        }
    }
    echo "<li class=\"sct_li{$sct_last} swiper-slide\" >\n";

    if ($this->href) {
        //echo "<div class=\"sct_img\"><a href=\"#\" class=\"ad_cart\" onclick=\"eaSubmit('".$row[it_id]."','".$this->type."')\">장바구니</a><a href=\"{$this->href}{$row['it_id']}\" class=\"sct_a\">\n";
        echo "<div class=\"sct_img\"><a href=\"{$this->href}{$row['it_id']}\" class=\"sct_a\">\n";
    }

    if ($this->view_it_img) {
        echo get_it_image($row['it_id'], 366, 366, '', '', stripslashes($row['it_name']))."\n";
    }
