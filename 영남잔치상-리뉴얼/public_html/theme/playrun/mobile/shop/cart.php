...
$sql = " select a.ct_id,
          ...
          where a.od_id = '$s_cart_id' and a.ct_status='쇼핑'";
...
<div id="sod_bsk" class="od_prd_list">
	...
	<div class="tbl_head03 tbl_wrap">
        ...
        ?>
				...
				<tr>
					...
					<td class="td_chk chk_box">
						...
						<input type="checkbox" name="ct_chk[<?php echo $i; ?>]" value="1" class="chk" id="ct_chk_<?php echo $i; ?>" checked onclick="reCal(<?php echo $i?>)">
					</td> 
					...
					<td class="td_prd">
						...
						-->
                        <div class="sod_img"><a href="#"><?php echo $image; ?></a></div>
						<div class="sod_name">
					...
					<td class="td_numbig hidden-xs"><?php echo number_format($row['it_price']); ?></td>
					<td class="td_numbig hidden-xs">
						<span id="sell_price_0" class="total_prc"><?php echo number_format($sell_price); ?></span>
						<span id="sell_prices<?php echo $i?>" style="display:none"><?php echo $sell_price; ?></span>
						<span id="points<?php echo $i?>" style="display:none"><?php echo $point?></span>
					</td>
		...
		<script type="text/javascript">
			function reCal(no){
				...
				let total_point=0;
				let total_sell2=0;
				for(let i=0;i<$("input[name^=ct_chk]").length;i++){
					if($("input[name^=ct_chk]").eq(i).prop("checked")){
						total_sell+=parseInt($(`#sell_prices${i}`).html());
						total_point+=parseInt($(`#points${i}`).html());
					}
				}
				 total_sell2=total_sell;
				 <?php if ($send_cost > 0) { // 배송비가 0 보다 크다면 (있다면) 
				 ?>
					 total_sell2+=parseInt("<?php echo $send_cost?>");
				 <?php }?>
				 $("#tot_sell_price").html(number_format(total_sell)+" 원");
				 $("#tot_sell_price2").html(number_format(total_sell2)+" 원");
				 $("#total_point").html(number_format(total_point)+" 점");
			}
		</script>
	</div>
	<!-- /tbl_head03 -->
    ...
    <dl id="sod_bsk_tot">
        ...
        <dd class="sod_bsk_dvr"><strong id="tot_sell_price"><?php echo number_format($tot_sell_price); ?> 원</strong></dd>
		<? }?>
        <?php if ($send_cost > 0) { // 배송비가 0 보다 크다면 (있다면) ?>
        ...
        <dd class="sod_bsk_cnt"><strong id="tot_sell_price2"><?php echo number_format($tot_price); ?> 원</strong></dd>
        <dt>포인트</dt>
        <dd><strong id="total_point"><?php echo number_format($tot_point); ?> 점</strong></dd>
        <?php } ?>
    </dl>
    <?php } ?>
