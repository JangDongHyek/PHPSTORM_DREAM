...
if ($fr_date && $to_date) {
    $where[] = " od_delivery_date between '$fr_date' and '$to_date' ";
}
...
if ($sel_field == "")  $sel_field = "od_name";
...
<div>
    ...
    <input type="radio" name="od_settle_case" value="신용카드" id="od_settle_case05"   <?php echo get_checked($od_settle_case, '신용카드');    ?>>
	<label for="od_settle_case05">신용카드</label>
	<input type="radio" name="od_settle_case" value="후불결제" id="od_settle_case06"   <?php echo get_checked($od_settle_case, '후불결제');    ?>>
	<label for="od_settle_case06">후불결제</label>
...
<div class="sch_last">
    ...
    <input type="submit" value="검색" class="btn_submit">
<!--	<a href="./excel_orderlist.php" class="ov_listall" style="background-color:blue;">엑셀다운로드</a>-->
</div>
</form>
<script type="text/javascript">
	$(function(){
		$("#od_status1,#od_status-2").change(function(){
			let is_chk = false;
			const chk = document.getElementsByName("chk[]");
			let od_status = $(this).val();
			$("#od_status").val(od_status);
			$(`#od_status1`).val(od_status);
			$(`#od_status-2`).val(od_status);
			const data = $("form[name=forderlist]").serialize();
			...
			if (is_chk===false) {
				alert("상태 변경 하실 항목을 하나 이상 선택하세요.");
				$(`#od_status1 option:eq(0)`).prop("selected",true);
				$("#od_status-2 option:eq(0)").prop("selected",true);
				$("#od_status").val("");
				return;
			}
			
			if(confirm(`선택된 주문들을 ${od_status}로 변경하시겠습니까?`)){
				$.ajax({
					type:"post",
...
<input type="hidden" name="od_status_c" value="" id="od_status">
<input type="hidden" name="search_od_status" value="<?php echo $od_status; ?>">

선택된 주문 상태 변경
	<select name="od_status1" id="od_status1">
...
<div class="tbl_head01 tbl_wrap">
    ...
    <thead>
    <tr>
		<th scope="col" rowspan="2"><input type="checkbox" name="chkall" value="1" id="chkall" onclick="check_all(this.form)"></th>
		<th scope="col" rowspan="2">No.</th>
		<th scope="col" rowspan="2">주문일자</th>
		<th scope="col" rowspan="2">주문상태</th>
		<th scope="col"><a href="<?php echo title_sort("od_id", 1)."&amp;$qstr1"; ?>">주문번호</a></th>
		<th scope="col" rowspan="2">주문자정보</th>
		<th scope="col" rowspan="2">배송정보</th>
		<th scope="col" rowspan="2">주문수량</th>
		<th scope="col" rowspan="2">결제수단</th>
		<th scope="col" rowspan="2">주문금액</th>
		<th scope="col" rowspan="2">입금금액</th>
		<th scope="col" rowspan="2">주문취소금액</th>
		<th scope="col" rowspan="2">받는날짜</th>
    ...
    for ($i=0; $row=sql_fetch_array($result); $i++) {
		...
		*/
		...
		if ($config['cf_account'] == 'hotroad') {
			$sql = "SELECT A.mb_name, A.mb_id FROM g5_member A INNER JOIN g5_shop_cart B 
					ON A.mb_id = B.seller_id WHERE B.od_id = '{$row['od_id']}'";
			$seller = sql_fetch($sql);

			if ($seller) {
				$seller_name = ($is_admin)? "<a href='../member_form.php?w=u&mb_id={$seller['mb_id']}' target='_blank'>".$seller['mb_name']."</a>" : $seller['mb_name'];
			}
		}
	...
	<tr class="orderlist<?php echo ' '.$bg; ?>">
		...
		<td scope="col" rowspan="2">
			<input type="hidden" name="od_id[<?=$i?>]" value="<?php echo $row['od_id'] ?>" id="od_id_<?=$i?>">
            <label for="chk_<?php echo $i; ?>" class="sound_only">주문번호 <?=$row['od_id']?></label>
            <input type="checkbox" name="chk[]" value="<?=$i?>" id="chk_<?=$i?>">
		</td>
		
		<td scope="col" rowspan="2"><?=$list_no?></td>
		<!-- 주문일자 -->
		<td scope="col" rowspan="2"><?=date("Y-m-d", strtotime($row['od_time']))?><br><?=date("H:i:s", strtotime($row['od_time']))?></td>
		<!-- 주문상태 -->
		<td scope="col" rowspan="2">
            <input type="hidden" name="current_status[<?=$i?>]" value="<?php echo $row['od_status'] ?>">
			<div id="od_status<?php echo $i?>">
            <?php echo $row['od_status']=="완료"?"배송".$row[od_status]:$row[od_status]; ?>
			</div>
		</td>
		<!-- 주문번호 -->
		<td scope="col">
			<a href="<?php echo G5_SHOP_URL; ?>/orderinquiryview.php?od_id=<?=$row['od_id']?>&amp;uid=<?=$uid?>" class="orderitem"><?=$disp_od_id?></a>
		</td>
		<!-- 주문자정보 -->
		<td scope="col" rowspan="2" class="td_left">
			<? if ($row['mb_id']) { ?>
			<div><strong>주문자</strong> <?php echo $mb_nick; ?></div>
            <div><strong>아이디</strong> <a href="<?php echo $_SERVER['SCRIPT_NAME']; ?>?sort1=<?=$sort1?>&sort2=<?=$sort2?>&sel_field=mb_id&search=<?=$row['mb_id']?>"><?=$row['mb_id']?></a></div>
			<div><strong>연락처</strong> <?=get_text($row['od_tel'])?></div>
			<div><strong>입금자명</strong> <?php echo $row['od_deposit_name']?get_text($row['od_deposit_name']):$mb_nick; ?></div>
            <?php } else { ?>
            <div><strong>주문자</strong> <?php echo $mb_nick; ?></div>
			<div><strong>연락처</strong> <?=get_text($row['od_tel'])?></div>
			<div><strong>입금자명</strong> <?php echo $row['od_deposit_name']?get_text($row['od_deposit_name']):$mb_nick; ?></div>
		...
		<td scope="col" rowspan="2" class="td_left">
			...
			<div><strong>배송회사</strong> <? echo ($row['od_delivery_company'] ? $row['od_delivery_company'] : '-'); ?></div>
			<div><strong>배송일자</strong> <? echo (is_null_time($row['od_invoice_time']) ? '-' : substr($row['od_invoice_time'],2,14)); ?></div>
		</td>
		<!-- 주문수량 -->
		<td scope="col" rowspan="2"><?=$row['od_cart_count']?></td>
		<!-- 결제수단 -->
		<td scope="col" rowspan="2">
			<input type="hidden" name="current_settle_case[<?=$i?>]" value="<?php echo $row['od_settle_case'] ?>">
            <?php echo $s_receipt_way; ?>
		</td>
		<!-- 주문금액 -->
		<td scope="col" rowspan="2" class="td_num td_numsum"><?php echo number_format($row['od_cart_price'] + $row['od_send_cost'] + $row['od_send_cost2']); ?></td>
		<!-- 입금금액 -->
		<td scope="col" rowspan="2" class="td_num td_num_right"><?php echo number_format($row['od_receipt_price']); ?></td>
		<!-- 취소금액 -->
		<td scope="col" rowspan="2" class="td_num td_numcancel<?php echo $td_color; ?>"><?php echo number_format($row['od_cancel_price']); ?></td>
		<!-- 판매자 -->
		<td scope="col" rowspan="2"><?=$row[od_delivery_date]?></td>
		<!-- 보기 -->
		<td scope="col" rowspan="2" class="td_num td_mng">
	...
	<tr class="<?php echo $bg; ?>">
		<!-- 상품명 -->
		<td scope="col"><?=$item_names?></td>
	</tr>
	<?php
        $tot_itemcount     += $row['od_cart_count'];		// 수량합계
        $tot_orderprice    += ($row['od_cart_price'] + $row['od_send_cost'] + $row['od_send_cost2']);	//총합계
        $tot_ordercancel   += $row['od_cancel_price'];		// 주문취소
        $tot_receiptprice  += $row['od_receipt_price'];		// 입금
        //$tot_couponprice   += $row['couponprice'];		// 쿠폰합계
        //$tot_misu          += $row['od_misu'];			// 미수
...
<div>
	선택된 주문 상태 변경
	<select name="od_status2" id="od_status-2">
		<option value="">주문상태 선택</option>
		<option value="취소">취소</option>
		<option value="주문">주문</option>
		<option value="입금">입금</option>
