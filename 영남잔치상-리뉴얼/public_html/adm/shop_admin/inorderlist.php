...
<div class="tbl_head01 tbl_wrap" id="inorderlist">
    ...
    <tr>
        <th scope="col">
            <label for="chkall" class="sound_only">취소 및 미완료주문 전체</label>
    ...
    for ($i=0; $row=sql_fetch_array($result); $i++) {
        ...
        // 주문금액
        $sql = " select sum(if(io_type = '1', io_price, (ct_price + io_price)) * ct_qty) as price from {$g5['g5_shop_cart_table']} 
                    where od_id = '{$row['cart_id']}'
                    and (ct_status = '쇼핑' or ct_status='미완료') and ct_select= '1'";
        $ct = sql_fetch($sql);
