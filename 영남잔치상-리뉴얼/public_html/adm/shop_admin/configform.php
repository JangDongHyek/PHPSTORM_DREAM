...
// 이노페이 필드 추가
if(!isset($default['de_inno_mid'])){
	sql_query(" ALTER TABLE `{$g5['g5_shop_default_table']}`
                    ADD `de_inno_mid` varchar(255) NOT NULL DEFAULT '' AFTER `de_kcp_site_key`,
                    ADD `de_merchant_key` varchar(255) NOT NULL DEFAULT '' AFTER `de_inno_mid` ", true);
}
if(!isset($default['de_inno_mert_key'])){
	sql_query(" ALTER TABLE `{$g5['g5_shop_default_table']}`
                    ADD `de_inno_mert_key` varchar(255) NOT NULL DEFAULT '' AFTER `de_inno_mid`", true);
}
...
?>
<style>
.field_hide {height: 0; position: absolute; width: 0; opacity: 0; z-index: -1; top: -9999px; left: -9999px;}
</style>
...
<section id="anc_scf_skin" class="<?=$auth_cls?>">
...
<section id="anc_scf_index" class="<?=$auth_cls?>">
...
<section id="anc_mscf_index" class="<?=$auth_cls?>">
...
<section id ="anc_scf_payment">
    ...
    <div class="tbl_frm01 tbl_wrap">
            ...
            <td>
                ...
                <ul class="de_pg_tab">
                    ...
                    */ ?>
					...
					<li class="<?php if($default['de_pg_service'] == 'innopay') echo 'tab-current'; ?>"><a href="#inno_info_anchor" data-value="innopay" title="이노페이 선택하기">이노페이</a></li>
                </ul>
            </td>
        </tr>
		<?php /*
		...
		<tr class="pg_info_fld lg_info_fld" id="inno_info_anchor">
            <th scope="row">
                <label for="cf_lg_mid">이노페이 상점아이디</label><br>
                <!--<a href="http://sir.kr/main/service/lg_pg.php" target="_blank" id="scf_lgreg" class="lg_btn">LG유플러스 서비스신청하기</a>-->
            </th>
            <td>
				<?php echo $config['de_inno_mid']; ?>
				<input type="text" name="cf_inno_mid" value="<?php echo $default['de_inno_mid']; ?>" id="cf_inno_mid" class="frm_input code_input" size="10" maxlength="255">
            </td>
        </tr>
        <tr class="pg_info_fld lg_info_fld">
            <th scope="row"><label for="cf_inno_mert_key">이노페이 MERT KEY</label></th>
            <td>
                <input type="text" name="cf_inno_mert_key" value="<?php echo $default['de_inno_mert_key']; ?>" id="cf_inno_mert_key" class="frm_input " size="36" maxlength="255">
            </td>
        </tr>
    ...
    <div class="tbl_frm01 tbl_wrap">
		...
		*/ 
        ...
        <tr>
            ...
            <td>
				<?php 
				/*echo editor_html('de_baesong_content', get_text($default['de_baesong_content'], 0));*/  
				?>
				<textarea id="de_baesong_content" name="de_baesong_content"><?=get_text($default['de_baesong_content'])?></textarea>
			</td>
        </tr>
        <tr>
            ...
            <td>
				<?php 
				//echo editor_html('de_change_content', get_text($default['de_change_content'], 0)); 
				?>
				<textarea id="de_change_content" name="de_change_content"><?=get_text($default['de_change_content'])?></textarea>
			</td>
        </tr>
		<tr>
            <th scope="row">받지 않는 날짜</th>
            <td>
				<?php 
				//echo editor_html('de_change_content', get_text($default['de_change_content'], 0)); 
				?>
				<textarea id="no_delivery_date" name="no_delivery_date"><?=get_text($default['no_delivery_date'])?></textarea><br/>
				받지 않는 날짜 콤마로 구분이 됩니다.(예: 2021-01-01,2021-01-02)
			</td>
        </tr>
        </tbody>
        </table>
		...
		*/ ?>
        <tr>
            ...
            <td>
				...
				?>
				<textarea id="de_guest_privacy" name="de_guest_privacy"><?=get_text($default['de_guest_privacy'])?></textarea>
			</td>
        </tr>
