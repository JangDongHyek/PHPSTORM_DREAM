...
//}
/*if(!$is_member){
	alert("로그인 후에 이용이 가능합니다.",G5_BBS_URL."/login.php");
}*/
    ...
    for($i=0; $i<$count; $i++) {
        ...
        $opt_count = count($_POST['io_id']);
		/*

        if($opt_count && $_POST['io_type'] != 0)
		...
		//장바구니에 명절차례상과 제사상 및 시제음식이 같이 있는지 파악하기
		...
		for($i=0;$row=sql_fetch_array($result);$i++){
			...
			if($row3[ca_id]=="20"){
				if($row2[ca_id]=="10"||$row2[ca_id]=="60"){
					echo "명절차례상과 제사음식(시제음식)은 같이 장바구니에 담을 수 없습니다.\n개별 주문 부탁드립니다.";
					exit;
				}
			}else if($row3[ca_id]=="10"||$row3[ca_id]=="60"){
				if($row2[ca_id]=="20"){
					echo "명절차례상과 제사음식(시제음식)은 같이 장바구니에 담을 수 없습니다.\n개별 주문 부탁드립니다.";
					exit;
				}
        ...
        for($k=0; $k<$opt_count; $k++) {
            $io_id = preg_replace(G5_OPTION_ID_FILTER, '', $_POST['io_id'][$it_id]);
            $io_type = preg_replace('#[^01]#', '', $_POST['io_type'][$it_id]);
            $io_value = $_POST['io_value'][$it_id][$k];
