...
$sql = " select SUM(IF(io_type = 1, (io_price * ct_qty), ((ct_price + io_price) * ct_qty))) as od_price,
              COUNT(distinct it_id) as cart_count,it_id
...
//23.01.16방문수령시 배송비안되게하는거 wc
$it_options = print_item_options($row['it_id'], $tmp_cart_id);
...
if ((int)($row['od_price'] - $tot_cp_price) !== $i_price) {
    //die("Error.".$row['od_price'].'|'.$tot_cp_price.'|'.$i_price);
    //추가배송비때문에 배송비계산 여기서안해서.. wc
}
    ...
    if($is_visit) {
        ...
        if ((int)($send_cost - $tot_sc_cp_price) !== (int)($i_send_cost - $i_send_coupon)) {
            die("Error..".$send_cost."|||".$tot_sc_cp_price."|||".$i_send_cost."|||".$i_send_coupon);
        }
    }
...
//추가배송비때문에 배송비계산.. wc
if ((int)($row['od_price'] - $tot_cp_price + $send_cost + $send_cost2 - $i_temp_point) !== $i_price) {
    die("Error추가.".$row['od_price'].'|'.$tot_cp_price.'|'.$send_cost.'|'.$send_cost2.'|'.$i_temp_point.'|i_price'.$i_price);
}
...
결제금액 : ".number_format($order_price)."원
...
$result_send_cost =  $i_send_cost*1 + $i_send_cost2*1;
if($result_send_cost > 0){
    //$msg .= "
    //배송비 : ".number_format($result_send_cost)."원";
}
