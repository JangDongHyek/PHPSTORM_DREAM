...
//23.11.13 회원아디 안들어가는 주문은 이걸로처리 wc
if($member['mb_id']){
    sql_query(" update {$g5['g5_shop_cart_table']} set mb_id='{$member['mb_id']}' where od_id = '$s_cart_id' ");
}
    ...
    <div id="sod_frm">
        ...
        <div class="tbl_head01 tbl_wrap">
            <table id="sod_list">
                ...
                $ca_ids=Array();
                $od_send_cont_cnt=0; // 추가배송비는 상품 n개당 n번 부여하기 위한 상품수 count
                $firstCaId="";
                for ($i=0; $row=sql_fetch_array($result); $i++)
                {
                    ...
                    */
                    ...
                    $sql = " select SUM(IF(a.io_type = 1, (a.io_price * a.ct_qty), ((b.it_price + a.io_price) * a.ct_qty))) as price,
                            ...
                            SUM(a.ct_qty) as qty,
                            SUM(IF(a.io_type = 0, a.ct_qty, 0)) as real_qty
                        from {$g5['g5_shop_cart_table']} a left join {$g5['g5_shop_item_table']} b on ( a.it_id = b.it_id )
                        where a.it_id = '{$row['it_id']}'
                          and a.od_id = '$s_cart_id' ";
                    $sum = sql_fetch($sql);

                    //제사음식/명절차례상/시제음식/추가맞춤음식만 count
                    if(strval(array_search(substr($row[ca_id],0,2),$firstCaIds))!=""){
                        $od_send_cont_cnt += (int)$sum['real_qty'];
                    }
                    ...
                    <tr>
                        ...
                        <td class="tl">
                            ...
                            <input type="hidden" name="qtytest[<?php echo $i; ?>]" value="<?php echo $sum['qty']; ?>">
                        ...
                        <td class="td_numbig"><?php echo number_format($row['it_price']); ?></td>
                ...
                <div class="tbl_frm01 tbl_wrap">
                    <table>
                        ...
                        <tr>
                            ...
                            <td><input type="text" name="od_tel" value="<?php echo get_text($member['mb_tel']); ?>" id="od_tel" class="frm_input" maxlength="20" oninput="autoHyphen(this)"></td>
                        </tr>
                        <tr>
                            <th scope="row"><label for="od_hp">핸드폰</label></th>
                            <td><input type="text" name="od_hp" value="<?php echo get_text($member['mb_hp']); ?>" id="od_hp" class="frm_input" maxlength="20" oninput="autoHyphen(this)"></td>
                        </tr>
                        <tr>
                            <th scope="row">주소</th>
                            <td>
                                <label for="od_zip" class="sound_only">우편번호<strong class="sound_only"> 필수</strong></label>
                                <input type="text" name="od_zip" value="<?php echo $member['mb_zip1'].$member['mb_zip2']; ?>" id="od_zip" required class="frm_input required" size="5" maxlength="6" readonly   onclick="win_zip('forderform', 'od_zip', 'od_addr1', 'od_addr2', 'od_addr3', 'od_addr_jibeon');">
                                ...
                                <input type="text" name="od_addr1" value="<?php echo get_text($member['mb_addr1']) ?>" id="od_addr1" required class="frm_input frm_address required" size="60" placeholder="기본주소 (우편번호 검색으로 입력하십시오)" readonly>
                                <label for="od_addr1" class="sound_only">상세주소<strong class="sound_only"> 필수</strong></label><br>
                                <input type="text" name="od_addr2" value="<?php echo get_text($member['mb_addr2']) ?>" id="od_addr2" class="frm_input frm_address" size="60" placeholder ="기본주소">
                ...
                <div class="tbl_frm01 tbl_wrap">
                    <table>
                        ...
                        if($is_member) {
                            ...
                            // 주문자와 동일
                            $addr_list .= '<span class="radio_ico"><input type="radio" name="ad_sel_addr" value="same" id="ad_sel_addr_same">'.PHP_EOL;
                            $addr_list .= '<label for="ad_sel_addr_same">주문자와 동일</label></span>'.PHP_EOL;
                            ...
                            if($row['ad_id']) {
                                ...
                                $addr_list .= '<span class="radio_ico"><input type="radio" name="ad_sel_addr" value="'.get_text($val1).'" id="ad_sel_addr_def">'.PHP_EOL;
                            ...
                            for($i=0; $row=sql_fetch_array($result); $i++) {
                                /* $val1 = $row['ad_name'].$sep.$row['ad_tel'].$sep.$row['ad_hp'].$sep.$row['ad_zip1'].$sep.$row['ad_zip2'].$sep.$row['ad_addr1'].$sep.$row['ad_addr2'].$sep.$row['ad_addr3'].$sep.$row['ad_jibeon'].$sep.$row['ad_subject'];
                                 $val2 = '<label for="ad_sel_addr_'.($i+1).'">최근배송지('.($row['ad_subject'] ? $row['ad_subject'] : $row['ad_name']).')</label></span>';
                                 $addr_list .= '<span class="radio_ico"><input type="radio" name="ad_sel_addr" value="'.get_text($val1).'" id="ad_sel_addr_'.($i+1).'">'.PHP_EOL.$val2.PHP_EOL;*/
                            }

                            /* $addr_list .= '<span class="radio_ico"><input type="radio" name="ad_sel_addr" value="new" id="od_sel_addr_new">'.PHP_EOL;
                             $addr_list .= '<label for="od_sel_addr_new">신규배송지</label></span>'.PHP_EOL;*/

//                $addr_list .='<a href="'.G5_SHOP_URL.'/orderaddress.php" id="order_address" class="btn_frmline btn_dellist">배송지목록</a>';
                        } else {
                            // 주문자와 동일
                            $addr_list .= '<span class="chk_ico"><input type="checkbox" name="ad_sel_addr" value="same" id="ad_sel_addr_same">'.PHP_EOL;
                            $addr_list .= '<label for="ad_sel_addr_same">주문자와 동일</label></span>'.PHP_EOL;
                        }
                        ?>
                        <tr>
                            <th scope="row">배송지선택</th>
                            <td class="del_list_wrap">
                                <?php echo $addr_list; ?>
                            </td>
                        </tr>
                        <?php if($is_member) { ?>
                        ...
                        <tr>
                            ...
                            <td><input type="text" name="od_b_tel" id="od_b_tel" class="frm_input" maxlength="20" oninput="autoHyphen(this)"></td>
                        </tr>
                        <tr>
                            <th scope="row"><label for="od_b_hp">핸드폰</label></th>
                            <td><input type="text" name="od_b_hp" id="od_b_hp" class="frm_input" maxlength="20" oninput="autoHyphen(this)"></td>
                        </tr>
                        <tr>
                            <th scope="row">주소</th>
                            <td id="sod_frm_addr">
                                <label for="od_b_zip" class="sound_only">우편번호<strong class="sound_only"> 필수</strong></label>
                                <input type="text" name="od_b_zip" id="od_b_zip" required class="frm_input required" size="5" maxlength="6" readonly  onclick="win_zip('forderform', 'od_b_zip', 'od_b_addr1', 'od_b_addr2', 'od_b_addr3', 'od_b_addr_jibeon');">
                                ...
                                <input type="text" name="od_b_addr1" id="od_b_addr1" required class="frm_input frm_address required" size="60" placeholder="기본주소(우편번호 검색으로 입력하십시오)" readonly>
                                <label for="od_b_addr1" class="sound_only" >기본주소<strong class="sound_only"> 필수</strong></label><br>
                        ...
                        <tr>
                            <td colspan="2">
					<span style="color:#cc0000 !important">
						※ 배송지가 시외일 경우 아래 선택란을 꼭 체크를 하시고 음식을 받으실 <b>터미널 또는 주소</b>를 기입해주세요.<br/>
						- 부산/김해/양산 외 지역만 체크 바랍니다. <b>(명절차례상은 터미널 수령만 가능합니다.)</b>
					</span><br/><br/>
                                <span class="radio_ico"><input type="radio" name="od_b_receive" id="receive0" value="해당없음" checked><label for="receive0">해당없음</label></span>
                                <span class="radio_ico"><input type="radio" name="od_b_receive" id="receive1" value="터미널"><label for="receive1">터미널</label></span>
                                <span class="radio_ico"><input type="radio" name="od_b_receive" id="receive2" value="집까지 수령"><label for="receive2">집까지 수령</label></span>
                            </td>
                        </tr>
                        <tr>
                            <th scope="row"><label for="od_delivery_date">받는날짜</label></th>
                            <td>
                                <?php
                                $firstCaId=strval(array_search("20",$ca_ids))!=""?"20":$firstCaId;
                                $firstCaId=strval(array_search("10",$ca_ids))!=""?"10":$firstCaId;
                                $firstCaId=strval(array_search("60",$ca_ids))!=""?"60":$firstCaId;

                                $currentDate=date("Y-m-d",strtotime(date("Y-m-d")." +2 day"));
                                $currentSec=strtotime(date("Y-m-d")." +2 day");
                                if($firstCaId=="10"||$firstCaId=="60"||$firstCaId=="70"){
                                    $lastDate=date("Y-m-d",strtotime($currentDate." +2 month")-86400);
                                    $lastSec=strtotime($currentDate." +2 month")-86400;
                                }else{
                                    $lastDate=date("Y-m-d",strtotime($currentDate." +1 month")-86400);
                                    $lastSec=strtotime($currentDate." +1 month")-86400;
                                }
                                ...
                                if($firstCaId=="20"){
                                    ...
                                    $plusDate= -2 <= (($cDate-strtotime("2023-09-20"))/86400)?(($cDate-strtotime("2023-09-20"))/86400)+2:0;

                                    $currentSec=strtotime("2023-09-20 +".$plusDate." day");
                                    $currentDate=date("Y-m-d",strtotime("2023-09-28"));

                                    $lastDate=date("2023-10-28");
                                    $lastSec=strtotime("2023-09-28");
                                    $default[no_delivery_date]="2023-09-27";
                                    */
                                ...
                                if($firstCaId=="10"||$firstCaId=="60"){
                                    /*
                                    $fDate=strtotime("2023-09-20");
                                    $lDate=strtotime("2023-10-01");
                                ...
                                $noDeliveryDateArr=explode(",",$default[no_delivery_date]);

                                ?>
                                <!-- <input type="text" name="od_delivery_date" value="<?php echo date("Y-m-d",$currentSec)?>" readonly class="frm_input"/> -->
                                <select name="od_delivery_date" class="frm_input" required>
                                    <option value="">날짜선택</option>
                                    <?php
                                    $dayofweekArr=array("일","월","화","수","목","금","토","일");
                                    for($i=$currentSec;$i<=$lastSec;$i=$i+86400){
                                        $optionDate=date("Y-m-d",$i);
                                        $dayofweek=date("w",strtotime($optionDate));



                                        if(strval(array_search($optionDate,$noDeliveryDateArr))==""){
                                            ?>
                                            <option value="<?=$optionDate?>"><?=$optionDate?>(<?php echo $dayofweekArr[$dayofweek]?>)</option>
                                        <?php }}?>
                                </select>
                        ...
                        <tr>
                            <th scope="row"><label for="od_path">주문경로</label></th>
                            <td>
                                <span class="radio_ico"><input type="radio" name="od_path" value="인터넷검색" id="path1" checked/><label for="path1">인터넷검색</label>&nbsp;&nbsp;</span>
                                <span class="radio_ico"><input type="radio" name="od_path" value="지인소개" id="path2"/><label for="path2">지인소개</label>&nbsp;&nbsp;</span>
                                <span class="radio_ico"><input type="radio" name="od_path" value="블로그" id="path3"/><label for="path3">블로그</label>&nbsp;&nbsp;</span>
                                <span class="radio_ico"><input type="radio" name="od_path" value="재구매" id="path4"/><label for="path4">재구매</label>&nbsp;&nbsp;</span>
                                <span class="radio_ico"><input type="radio" name="od_path" value="기타" id="path5"/><label for="path5">기타</label></span>
                            </td>
                        </tr>
                        <tr>
        ...
        <div class="sod_right">
            ...
            <section id="sod_frm_pay">
                ...
                <div class="tbl_frm01 tbl_wrap right_tbl">
                    <table>
                        <tbody>
                        <?php if($oc_cnt > 0) { ?>
                            <tr>
                ...
                if($is_kakaopay_use) {
                    $multi_settle++;
                    echo '<span class="radio_ico"><input type="radio" id="od_settle_kakaopay" name="od_settle_case" value="KAKAOPAY" '.$checked.'> <label for="od_settle_kakaopay" class="kakaopay_icon">KAKAOPAY</label></span>'.PHP_EOL;
                    $checked = '';
                }
                ...
                if ($default['de_bank_use']) {
                    $multi_settle++;
                    echo '<span class="radio_ico"><input type="radio" id="od_settle_bank" name="od_settle_case" value="무통장" '.$checked.'> <label for="od_settle_bank">무통장입금</label></span>'.PHP_EOL;
                    $checked = '';
                }
                ...
                if ($default['de_vbank_use']) {
                    $multi_settle++;
                    echo '<span class="radio_ico"><input type="radio" id="od_settle_vbank" name="od_settle_case" value="가상계좌" '.$checked.'> <label for="od_settle_vbank">'.$escrow_title.'가상계좌</label></span>'.PHP_EOL;
                    $checked = '';
                }
                ...
                if ($default['de_iche_use']) {
                    $multi_settle++;
                    echo '<span class="radio_ico"><input type="radio" id="od_settle_iche" name="od_settle_case" value="계좌이체" '.$checked.'> <label for="od_settle_iche">'.$escrow_title.'계좌이체</label></span>'.PHP_EOL;
                    $checked = '';
                }
                ...
                if ($default['de_hp_use']) {
                    $multi_settle++;
                    echo '<span class="radio_ico"><input type="radio" id="od_settle_hp" name="od_settle_case" value="휴대폰" '.$checked.'> <label for="od_settle_hp">휴대폰</label></span>'.PHP_EOL;
                    $checked = '';
                }
                ...
                if ($default['de_card_use']) {
                    $multi_settle++;
                    echo '<span class="radio_ico"><input type="radio" id="od_settle_card" name="od_settle_case" value="신용카드" '.$checked.'> <label for="od_settle_card">신용카드</label></span>'.PHP_EOL;
                    $checked = '';
                }
                ...
                if ($is_member && $config['cf_use_point'])
                {
                    ...
                    if ($member['mb_point'] >= $default['de_settle_min_point'])
                    {
                        ...
                        <input type="text" name="od_temp_point" value="0" id="od_temp_point" class="frm_input" size="10">점
                        (포인트는 10,000포인트 이상 사용하실 수 있습니다.)
                        <?php
                        $multi_settle++;
                    }
                }
...
?>
<div id="innopay-mobile" class="innopay-layer">
    <iframe src="" style="width:100%;height:100%" frameborder="0" scrolling="1" name="innopay_frm"></iframe>
</div>
<script>
    ...
    $(function() {
        ...
        $("#od_b_addr2").focus(function() {
            ...
            calculate_sendcost(code,$("#od_b_addr1").val());
        });
        ...
        $("input[name=ad_sel_addr]").on("click", function() {
            ...
            if (addr[0] == "same") {
                ...
                if(zipcode != code) {
                    ...
                    calculate_sendcost(code,f.od_b_addr1);
    ...
    {
        <?php
        //제사음식/명절차례상/시제음식/추가맞춤음식만 추가배송비 구현이 되게
        if(0 < count($ca_ids) && $is_visit == 0){
        ?>

        // console.log('cnt:'+<?=$od_send_cont_cnt?>);
        if(0 < addr.indexOf("김해시 진영읍")){
            ...
            $.post(
                "./ordersendcost.php",
                { zipcode: code,sc_name:addr },
                function(data) {
                    // 추가배송비는 물건 n개당 *n 번 부여
                    data = (data * <?=$od_send_cont_cnt?>);
    ...
    function forderform_check(f)
    {
        ...
        if ((navigator.appName == 'Netscape' && agent.indexOf('trident') != -1) || (agent.indexOf("msie") != -1)) {
            alert("익스플로러에서 신용카드 결제시 오류가 발생하오니 크롬이나 엣지를 이용해주시길 바랍니다.");
            return false;
        }
        // 재고체크
        ...
        if (typeof(f.od_pwd) != 'undefined')
        {
            ...
            if( (f.od_pwd.value.length<3) || (f.od_pwd.value.search(/([^A-Za-z0-9]+)/)!=-1) )
                ...
                error_field(f.od_pwd, "회원이 아니신 경우 주문서 조회시 필요한 비밀번호를 (영문+숫자) 3자리 이상 입력해 주십시오.");
        }
        //  check_field(f.od_tel, "주문하시는 분 전화번호를 입력하십시오.");
        check_field(f.od_addr1, "주소검색을 이용하여 주문하시는 분 주소를 입력하십시오.");
        check_field(f.od_hp, "주문하신 분 휴대폰번호를 입력하세요.");
        //check_field(f.od_addr2, " 주문하시는 분의 상세주소를 입력하십시오.");
        check_field(f.od_zip, "주소검색을 이용하여 주문하시는 분 우편번호를 입력하십시오.");
        ...
        check_field(f.od_b_zip, "주소검색을 이용하여 받으시는 분 우편번호를 입력하십시오.");
        if($('#od_b_zip').val().length > 5 ){
        ...
        if (typeof(f.od_temp_point) != "undefined") {
            if (f.od_temp_point.value)
            {
                ...
                if(0 < temp_point){
                    if (temp_point < parseInt("<?php echo $default[de_settle_min_point]?>")) {
                        alert("포인트를 <?php echo $default[de_settle_min_point]?>포인트 이상 입력하세요.");
                        f.od_temp_point.select();
                        return false;
                    }
                }
        ...
        let mb_id = "<?=$member['mb_id']?>";
        //if(mb_id == "admin"){
            var price_msg = price_check(od_price, send_cost, send_cost2, send_coupon, temp_point);
            if(price_msg != "") {
                alert(price_msg);
        ...
        if( jQuery(f).triggerHandler("form_sumbit_order_"+form_order_method) !== false ) {
            ...
            //이노페이 결제방식
            <?php } else if($default['de_pg_service']=="innopay"){ ?>
            switch(settle_method)
            {
                case "계좌이체":
            ...
            <?php } ?>
            //이노페이 결제설정
            <?php if($default['de_pg_service'] == 'innopay') { ?>
            f.BuyerName.value = f.od_name.value;//이름
            f.BuyerEmail.value = f.od_email.value;//이메일
            f.BuyerTel.value = f.od_hp.value;//연락처
            /* 주소 */
            f.BuyerPostNo.value = f.od_b_zip.value;
            f.BuyerAddr.value = f.od_b_addr1.value;
            f.BuyerAuthNum.value = f.od_b_addr2.value;
            f.Amt.value = f.good_mny.value;//가격
            f.GoodsName.value=f.good_name.value;//상품명
            f.Moid.value="<?=$s_cart_id?>";//주문번호
...
<? if($member[mb_id]!="test"){?>
            alert("현제 테스트 중이니 무통장으로 주문을 하십시오.");
						<? }else{?>
...
//    f.od_b_addr_jibeon.value = f.od_addr_jibeon.value;
        <?php
        //제사음식/명절차례상/시제음식/추가맞춤음식만 추가배송비 구현
        if(0 < count($ca_ids)  && $is_visit == 0){
        ?>
        calculate_sendcost(String(f.od_b_zip.value),f.od_b_addr1.value);
        <?php }?>
    }
    ...
    $(window).on('pageshow', function(event) {
        ...
        if (navigationEntries.length > 0) {
            ...
            // 탐색 유형이 'back_forward'인지 확인
            if (navigationType === 'back_forward') {
                alert('뒤로가기가 감지되었습니다. 메인에서 다시 주문해주세요.');
                location.href = "<?=G5_URL?>";
                return;
            }
        }
