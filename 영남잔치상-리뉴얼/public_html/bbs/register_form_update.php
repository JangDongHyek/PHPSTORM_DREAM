...
if(empty($_SERVER['HTTP_REFERER'])){
	alert("직접 접근하실 수 없습니다.");
	exit;
}
$hpChecks = array("","02","031","032","033","041","042","043","044","052","053","054","055","061","063","064","070","080","010");
$hpCheck = 0 < strpos(" ".substr($_POST[mb_tel],0,3),"02")?substr($_POST[mb_tel],0,2):substr($_POST[mb_tel],0,3);
if($_POST[mb_tel]){
	...
	if(!is_numeric($mb_tel)){
		alert("연락처는 숫자만 입력을 받을 수 있습니다.");
		exit;
	}
}
if($_POST[mb_hp]){
	$hpCheck = 0 < strpos(" ".substr($_POST[mb_hp],0,3),"02")?substr($_POST[mb_hp],0,2):substr($_POST[mb_hp],0,3);
	if(!array_search($hpCheck,$hpChecks)){
	...
	if(!is_numeric($mb_hp)){
		alert("휴대폰번호가 숫자만 입력을 받을 수 있습니다.");
		exit;
	}
}
...
if ($w == '') {
	$sql="select * from {$g5['member_table']} 
			where mb_addr1='{$mb_addr1}'
			and mb_addr2='{$mb_addr2}'";

	$addrRow = sql_fetch($sql);
	if($addrRow){
		alert("입력하신 전화번호 또는 주소가 이미 존재합니다, 관리자에게 문의 바랍니다.");
		exit;
	}
	$sql="select * from {$g5['member_table']} 
			where mb_hp='{$mb_hp}'";
	$mobileRow = sql_fetch($sql);
	if($mobileRow){
		alert("입력하신 전화번호 또는 주소가 이미 존재합니다, 관리자에게 문의 바랍니다.");
		exit;
	}
...
if ($w == '') {
    ...
    if ($old_email != $mb_email && $config['cf_use_email_certify']) {
        ...
        alert('회원 정보가 수정 되었습니다.\n\nE-mail 주소가 변경되었으므로 다시 인증하셔야 합니다.', 'http://www.danjimuk.kr');
    } else {
       goto_url("/");
    }
}
