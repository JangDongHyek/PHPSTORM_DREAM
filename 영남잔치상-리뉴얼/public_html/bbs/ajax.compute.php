...
$now_date_timestemp = mktime('0','0','0',number_format($now_date[1]),number_format($now_date[2]),$now_date[0]);
$c = date('w',$now_date_timestemp);

$date = str_replace("-","",$date);
if($type == "solar"){
	$a = date("Y-m-d",SolaToLunar($date));
	$a2 = SolaToLunar($date);
	$b = date('w',$a2);
}else{
	$a = date("Y-m-d",LunarToSola($date));
	$a2 = LunarToSola($date);
	$b = date('w',$a2);
}
