...
$sql = " select mb_no, mb_id, mb_name, mb_nick, mb_email, mb_datetime from {$g5['member_table']} where replace(mb_hp,'-','') = '$mb_hp' and mb_id='$mb_id' ";
$mb = sql_fetch($sql);
if (!$mb['mb_id'])
...
$content .= '</div>';
$content .= '</div>';
itforoneMailer( "no-reply@dreamforone.com","영남잔치상", $email,$mb['mb_name'], $subject, $content);
