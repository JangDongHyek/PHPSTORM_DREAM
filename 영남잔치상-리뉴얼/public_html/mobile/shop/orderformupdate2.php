...
if( is_inicis_order_pay($od_settle_case) && !empty($_P_HASH) ){
...
if($default['de_pg_service'] == 'inicis' && !$P_HASH)
...
$i_price     = (int)$_REQUEST['od_price'];
$i_send_cost  = (int)$_REQUEST['od_send_cost'];
$i_send_cost2  = (int)$_REQUEST['od_send_cost2'];
$i_send_coupon  = (int)$_REQUEST['od_send_coupon'];
$i_temp_point = (int)$_REQUEST['od_temp_point'];
...
$sql = " select SUM(IF(a.io_type = 1, (a.io_price * a.ct_qty), ((b.it_price + a.io_price) * a.ct_qty))) as od_price,
                COUNT(distinct a.it_id) as cart_count,a.it_id
                from {$g5['g5_shop_cart_table']} a left join {$g5['g5_shop_item_table']} b on ( a.it_id = b.it_id )
                where a.od_id = '$tmp_cart_id' and a.ct_select = '1' ";
$row = sql_fetch($sql);
...
if($is_member) {
    ...
    $it_cp_cnt = count($_REQUEST['cp_id']);
    $arr_it_cp_prc = array();
    for($i=0; $i<$it_cp_cnt; $i++) {
        $cid = $_REQUEST['cp_id'][$i];
        $it_id = $_REQUEST['it_id'][$i];
    ...
    // 주문쿠폰
    if($_REQUEST['od_cp_id']) {
        $sql = " select cp_id, cp_type, cp_price, cp_trunc, cp_minimum, cp_maximum
                    from {$g5['g5_shop_coupon_table']}
                    where cp_id = '{$_REQUEST['od_cp_id']}'
                      and mb_id IN ( '{$member['mb_id']}', '전체회원' )
                      and cp_start <= '".G5_TIME_YMD."'
                      and cp_end >= '".G5_TIME_YMD."'
                      and cp_method = '2' ";
...
if($is_member && $send_cost > 0) {
    // 배송쿠폰
    if($_REQUEST['sc_cp_id']) {
        $sql = " select cp_id, cp_type, cp_price, cp_trunc, cp_minimum, cp_maximum
                    from {$g5['g5_shop_coupon_table']}
                    where cp_id = '{$_REQUEST['sc_cp_id']}'
                      and mb_id IN ( '{$member['mb_id']}', '전체회원' )
                      and cp_start <= '".G5_TIME_YMD."'
                      and cp_end >= '".G5_TIME_YMD."'
                      and cp_method = '3' ";
...
else if ($od_settle_case == "계좌이체")
{
    switch($default['de_pg_service']) {
        case 'lg':
            ...
            break;
        case 'inicis':
            include G5_MSHOP_PATH.'/inicis/pay_result2.php';
            break;
				case 'innopay':
							break;
        default:
...
if ($is_member)
    ...
    $od_pwd = get_encrypt_string($_REQUEST['od_pwd']);
...
if($default['de_tax_flag_use']) {
    $od_tax_mny = (int)$_REQUEST['comm_tax_mny'];
    $od_vat_mny = (int)$_REQUEST['comm_vat_mny'];
    $od_free_mny = (int)$_REQUEST['comm_free_mny'];
}
...
$sql = " insert {$g5['g5_shop_order_table']}
            set od_id             = '$od_id',
				...
				od_delivery_date  = '$_REQUEST[od_delivery_date]',
                od_path			  = '$_REQUEST[od_path]',
                od_b_receive	  = '$_REQUEST[od_b_receive]'
                ";
echo $sql;
exit;
...
if($od_settle_case=="무통장"){
	$msg = "[영남잔치상]고객님께서 주문하신 상품이 정상적으로 예약 접수 되었습니다. 정성껏 준비하여 배송 드리겠습니다.
...
합계 금액 : ".number_format($good_mny)."원
