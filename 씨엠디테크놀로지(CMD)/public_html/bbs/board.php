...
if (isset($wr_id) && $wr_id) {
    ...
    if ($member['mb_level'] < $board['bo_read_level']) {
        if ($is_member)
            ...
            alert('회원가입이 필요한 페이지 입니다.\n회원이시면 로그인 후 이용해 주시기 바랍니다.', './login.php?wr_id='.$wr_id.$qstr.'&amp;url='.urlencode(G5_BBS_URL.'/board.php?bo_table='.$bo_table.'&amp;wr_id='.$wr_id.$qstr));
    ...
    if ($member['mb_level'] < $board['bo_list_level']) {
        if ($member['mb_id'])
            ...
            alert('회원가입이 필요한 페이지 입니다.\n회원이시면 로그인 후 이용해 주시기 바랍니다.', './login.php?'.$qstr.'&url='.urlencode(G5_BBS_URL.'/board.php?bo_table='.$bo_table.($qstr?'&amp;':'')));
    }
