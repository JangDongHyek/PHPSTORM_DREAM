...
<?php if ($is_admin == 'super') {
    $sql="select wr_subject from g5_write_category order by wr_id asc";
    ...
    <div class="btn_add01 btn_add">
        ...
        <a href="./product_form.php?bo_table=product" id="member_add">제품추가</a>
    </div>
<?php } ?>
            ...
            for ($i=0; $row=sql_fetch_array($result); $i++) {
                ...
                <tr class="<?php echo $bg; ?>">
                    ...
                    <td scope="col"" align="center">
                        <a href="<?php echo G5_BBS_URL?>/board.php?bo_table=product&wr_id=<?php echo $row[wr_id]?>" target="_blank"><?php echo $row[ca_name]?>/<?php echo $row[wr_2]?></a>
                    </td>
                    <td scope="col"" align="center"><a href="<?php echo G5_BBS_URL?>/board.php?bo_table=product&wr_id=<?php echo $row[wr_id]?>" target="_blank"><?php echo $row[wr_1]?></a></td>
                    <td scope="col"" align="center"><a href="<?php echo G5_BBS_URL?>/board.php?bo_table=product&wr_id=<?php echo $row[wr_id]?>" target="_blank"><?php echo $row[wr_subject]?></a></td>
                    <td headers="mb_list_mng"  class="td_mngsmall">
                        <a href="./product_form.php?bo_table=product&w=u&wr_id=<?php echo $row[wr_id]?><?php echo $qstr?>">수정</a>
...
<script>
    const productRemove = (id,subject) => {
        if(confirm(`${subject}를 삭제하시겠습니까?`)){
            location.href=`<?php echo G5_BBS_URL?>/delete.php?bo_table=product&wr_id=${id}<?php echo $qstr?>`;
        }
    }
