...
function sendAlimTalk($type, $params=array(), $mb_hp) {
	//if ((int)$type > 6) return;	// 탬플릿 추가시 변경할것!!!
    
	$template = array();
    $rep_field = array(
        "wr_name" => "#{회원명}", "wr_subject" => "#{제품명}",
    );
            
    if ((int)$type > 1) return;	// 탬플릿 추가시 변경할것!!!

    // 1) 문의 도착 알림
    $template[0]['code'] = "SJB_074664";
    $template[0]['title'] = "견적 요청"; 
    $template[0]['content'] = "#{회원명}님이\n";
    $template[0]['content'] .= "#{제품명}\n";
    $template[0]['content'] .= "제품 견적을 요청하셨습니다.";
