...
<div class="gall_wrap">
    ...
    <?php if ($is_category) {
        $sql = "select wr_1 from g5_write_category where wr_subject='$sca'";
        $cateRow = sql_fetch($sql);
        $cate = explode(",", $cateRow[wr_1]);
        ?>
        <nav id="bo_cate">
            <h2><?php echo $board['bo_subject'] ?> 카테고리</h2>
            <ul id="bo_cate_ul">
                <li><a href="?bo_table=<?php echo $bo_table ?>&sca=<?php echo $sca ?>" id="<?php echo !$swr2?"bo_cate_on":"";?>">전체</a></li>
                <?php
                for ($i = 0; $i < count($cate); $i++) {
                    ?>
                    <li>
                        <a href="?bo_table=<?php echo $bo_table ?>&sca=<?php echo $sca ?>&swr2=<?php echo trim($cate[$i])?>" id="<?php echo $swr2==trim($cate[$i])?"bo_cate_on":"";?>">
                            <?php echo trim($cate[$i])?>
                        </a>
                    </li>
                <?php } ?>
                            ...
                            <li class="gall_href">
                                <a href="<?php echo $list[$i]['href'] ?>">
                                    ...
                                    <?php } else {
                                        ...
                                        if ($thumb['ori']) {
                                            $img_content = '<img src="' . $thumb['ori'] . '" alt="' . $thumb['alt'] . '" width="' . $board['bo_gallery_width'] . '" height="' . $board['bo_gallery_height'] . '" class="img">';
                                        } else {
                            ...
                            <li class="gall_text_href">
                                ...
                                if ($is_category && $list[$i]['ca_name']) {
                                    ...
                                    <a href="<?php echo $list[$i]['ca_name_href'] ?>"
                                       class="bo_cate_link"><?php echo $list[$i]['wr_2'] ?></a>
                                <?php } ?>
                                <a href="<?php echo $list[$i]['href'] ?>" class="title">
                                    ...
                                    <?php
                                        ...
                                        $sql="select * from g5_board_file where bo_table='product' and wr_id='{$list[$i]['wr_id']}' and bf_no='1'";
                                        $row2=sql_fetch($sql);
                                        //제품 카탈로그이 있을경우 다운로드하기
                                        if($row2[bf_no]){
                                            ?>
                                            <a class="down_btn" href="<?php echo G5_BBS_URL?>/download.php?bo_table=product&wr_id=<?php echo $list[$i][wr_id]?>&no=1">
                                                제품 카탈로그
