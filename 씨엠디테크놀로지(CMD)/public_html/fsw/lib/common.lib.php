...
function get_content($skin_dir, $co_id){
    global $g5, $config;
    ...
    if(preg_match('#^theme/(.+)$#', $skin_dir, $match)) {
        if (G5_IS_MOBILE) {
            $content_skin_path = G5_THEME_MOBILE_PATH.'/'.G5_SKIN_DIR.'/content/'.$match[1];
            if(!is_dir($content_skin_path))
                $content_skin_path = G5_THEME_PATH.'/'.G5_SKIN_DIR.'/content/'.$match[1];
            $content_skin_url = str_replace(G5_PATH, G5_URL, $content_skin_path);
        } else {
            $content_skin_path = G5_THEME_PATH.'/'.G5_SKIN_DIR.'/content/'.$match[1];
            $content_skin_url = str_replace(G5_PATH, G5_URL, $content_skin_path);
        }
        ...
        if(G5_IS_MOBILE) {
            $content_skin_path = G5_MOBILE_PATH.'/'.G5_SKIN_DIR.'/content/'.$skin_dir;
            $content_skin_url  = G5_MOBILE_URL.'/'.G5_SKIN_DIR.'/content/'.$skin_dir;
        } else {
            $content_skin_path = G5_SKIN_PATH.'/content/'.$skin_dir;
            $content_skin_url  = G5_SKIN_URL.'/content/'.$skin_dir;
        }
    }
	...
	if(is_file($skin_file)) {
		...
		if (file_exists($himg)) // 상단 이미지
			$himg_url = '<div id="ctt_himg" class="ctt_img"><img src="'.G5_DATA_URL.'/content/'.$co_id.'_h" alt=""></div>';

		$timg = G5_DATA_PATH.'/content/'.$co_id.'_t';
		if (file_exists($timg)) // 하단 이미지
			$timg_url = '<div id="ctt_timg" class="ctt_img"><img src="'.G5_DATA_URL.'/content/'.$co_id.'_t" alt=""></div>';
	}

    ob_start();
    include $content_skin_path.'/content.skin.php';
