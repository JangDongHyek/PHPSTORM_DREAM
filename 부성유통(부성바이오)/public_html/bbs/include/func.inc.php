...
**************************************************************************/
	...
	function rg_bbs_doc_del($bbs_id,$rg_doc_num) {
		...
		if($bbs_id == "flash"){
			...
			$del_file = $rg_doc_num."\$1$".$data["rg_file1_name"];
			@unlink($bbs_data_path.$del_file);
	...
	function rg_bbs_doc_copy($target_bbs_id,$source_bbs_id,$source_rg_doc_num) {
		...
		$dbqry="
			INSERT INTO `$tar_bbs_table`
				( `rg_doc_num` , `rg_top_num` , `rg_parent_num` ,
					...
					`rg_ext3` , `rg_ext4` , `rg_ext5`, `admin_orderby`
				)
			VALUES 
				( '$rg_doc_num', '$rg_top_num', '$rg_parent_num',
					'$rg_sequence', '$rg_depth', '$rg_next_num', 
		...
		// 파일 복사 
			if(${"rg_file1_name"}) {
				${"rg_file1_src_server_name"} =  $source_rg_doc_num."\$1\$".${"rg_file1_name"};
				${"rg_file1_tar_server_name"} =  $rg_doc_num."\$1\$".${"rg_file1_name"};
				if(!@copy($src_bbs_data_path.${"rg_file1_src_server_name"},
				          $tar_bbs_data_path.${"rg_file1_tar_server_name"})) {
				}

				${"rg_file1_src_server_name"} =  $source_rg_doc_num."\$1\$th\$".${"rg_file1_name"};
				${"rg_file1_tar_server_name"} =  $rg_doc_num."\$1\$th\$".${"rg_file1_name"};
				if(!@copy($src_bbs_data_path.${"rg_file1_src_server_name"},
				          $tar_bbs_data_path.${"rg_file1_tar_server_name"})) {
				}


				${"rg_file1_src_server_name"} =  $source_rg_doc_num."\$1\$th2\$".${"rg_file1_name"};
				${"rg_file1_tar_server_name"} =  $rg_doc_num."\$1\$th2\$".${"rg_file1_name"};
				if(!@copy($src_bbs_data_path.${"rg_file1_src_server_name"},
				          $tar_bbs_data_path.${"rg_file1_tar_server_name"})) {
					${"rg_file1_name"} = '';
					${"rg_file1_size"} = 0;
			...
			if(${"rg_file2_name"}) {
				${"rg_file2_src_server_name"} =  $source_rg_doc_num."\$2\$".${"rg_file2_name"};
				${"rg_file2_tar_server_name"} =  $rg_doc_num."\$2\$".${"rg_file2_name"};
				...
				${"rg_file2_src_server_name"} =  $source_rg_doc_num."\$2\$th2\$".${"rg_file2_name"};
				${"rg_file2_tar_server_name"} =  $rg_doc_num."\$2\$th2\$".${"rg_file2_name"};
				if(!@copy($src_bbs_data_path.${"rg_file2_src_server_name"},
				          $tar_bbs_data_path.${"rg_file2_tar_server_name"})) {
					${"rg_file2_name"} = '';
					${"rg_file2_size"} = 0;
