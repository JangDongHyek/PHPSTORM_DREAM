...
<div id="mIntro">
  ...
  <div class="swiper academySwiper wow fadeIn animated" data-wow-delay="0.4s" data-wow-duration="0.8s">
		...
		<div class="swiper-wrapper">
			<div class="swiper-slide">
				<a href="https://amzn.to/3taMZZ3" target="_blank">
					<div class="area_img"><img src="<?php echo G5_THEME_IMG_URL ?>/main/academy11.jpg" alt=""></div>
					<div class="area_txt">
						<h3>2022.03.11</h3>
						<span>세계최초 유방보형물 초음파 전문서적 </span>
						<em>단독저자</em>
					</div>
				</a>
			</div>
	...
	<div class="in">
        <div class="big_form">
            <h1>SMS 간편상담<span>문자로 보다 간편하게 답변을 받아보세요.</span></h1>
                <form method="post" action="<?=G5_URL?>/sms_send.php" id="form_sms" name="sms">
                <input type="hidden" name="uid" value="<?php echo get_uniqid(); ?>">
                <input type="hidden" name="bo_table" value="consult">
                <div class="f_box cf">
                    <div class="formbox a1">  
                        <strong class="title">이름</strong>
                        <div class="form"><input type="text" name="data[]" title="이름" ></div>
                    </div>
...
$("#form_sms").submit(function(e) {
    ...
    if($("#form_sms input[title='이름']").val() == '') {

        alert('이름을 입력해 주세요.');

        $("#form_sms input[title='이름']").focus();

        return false;

    }

    // if($("#form_sms input[name=sms]").is(":checked") == false) {

    //     alert('SMS 수신 동의해 주세요.');

    //     $("#form_sms input[name=sms]").focus();

    //     return false;

    // }

    // if($("#form_sms input[name=privacy]").is(":checked") == false) {

    //     alert('개인정보 수집 및 이용에 동의해 주세요.');

    //     $("#form_sms input[name=privacy]").focus();

    //     return false;

    // }

    if($("#form_sms select[id=phone1]").val() == '') {

        alert('핸드폰 번호를 선택해 주세요.');

        $("#form_sms select[id=phone1]").focus();

        return false;

    }

    if($("#form_sms input[id=phone2]").val() == '' || $("#form_sms input[id=phone2]").val().length < 3) {

        alert('핸드폰 번호를 입력해 주세요.');

        $("#form_sms input[id=phone2]").focus();

        return false;

    }

    if($("#form_sms input[id=phone3]").val() == '' || $("#form_sms input[id=phone3]").val().length < 4) {

        alert('핸드폰 번호를 입력해 주세요.');

        $("#form_sms input[id=phone3]").focus();
