...
<!-- 게시판 검색 시작 { -->
<fieldset id="bo_sch" class="hidden-lg hidden-md hidden-sm">
    ...
    <input type="text" name="stx" value="<?php echo stripslashes($stx) ?>" required id="stx" class="frm_input sch_input required" placeholder="찾으시는 모델을 입력 해 주세요" style="outline-style: none;">
    <button type="submit" id="sch_submit" class="newSch">검색</button>
	</div>
    </form>
</fieldset>
<!-- } 게시판 검색 끝 -->
...
<!-- 게시물 검색 시작 { -->
<fieldset id="bo_sch"  class="hidden-xs new_select">
    <legend>게시물 검색</legend>
    ...
    <span class="nSelBox">
		<select name="sfl" id="sfl">
			<option value="wr_subject"<?php echo get_selected($sfl, 'wr_subject', true); ?>>자동차 모델</option>
			<option value="wr_content"<?php echo get_selected($sfl, 'wr_content'); ?>>상세내용</option>
			<option value="wr_subject||wr_content"<?php echo get_selected($sfl, 'wr_subject||wr_content'); ?>>자동차 모델+상세내용</option>
    ...
    <label for="stx" class="sound_only">검색어<strong class="sound_only"> 필수</strong></label>
    <input type="text" name="stx" value="<?php echo stripslashes($stx) ?>" required id="stx" class="frm_input sch_input required" maxlength="20"  placeholder="찾으시는 모델을 입력 해 주세요">
    <input type="submit" value="검색" class="btn_submit02 newSch">
    </form>
</fieldset>
<!-- } 게시물 검색 끝 -->
    ...
    <ul id="gall_ul">
        ...
        <li class="gall_li <?php if ($wr_id == $list[$i]['wr_id']) { ?>gall_now<?php } ?>" style="<?php echo $style ?>width:<?php echo $board['bo_gallery_width'] ?>px">
            ...
            <ul class="gall_con">
                ...
                <li class="gall_href">
                    <a href="<?php echo $list[$i]['href'] ?>"><div class="over"></div>
                    <?php
                ...
                <li class="gall_text_href">
                    ...
                    <a href="<?php echo $list[$i]['href'] ?>" class="title">
                        <p class="t9"><?php echo $list[$i]['subject'] ?><!-- 제목 --></p>
                        <p class="t16">보증금 <?php echo number_format($list[$i]['wr_1']) ?> 원<!-- 제품코드 --></p>                        
						<p class="t10">월렌트료 <?php echo number_format($list[$i]['wr_2']) ?> 원
						<p class="allprice"><span>차량가격</span> <span><?php echo number_format($list[$i]['wr_3']) ?> 원</span><!-- 제품코드 --></p>
        ...
        <?php } ?>
        <?php if (count($list) == 0) { echo "<li class=\"empty_list\">등록된 상품이 없습니다.</li>"; } ?>
    </ul>
