...
if($bo_table=="reserve"&&!$is_admin){
	alert("예약확인은 이름과 연락처로만 열람을 하실 수 있습니다.",G5_BBS_URL."/reserve_search.php");
}
...
if ($sca || $stx) {
	...
	if($_GET[f_wr_id]){
		$sql_search.=" and wr_10='$_GET[f_wr_id]'";
	}

	if($_GET[swr_name]&&$_GET[swr_7]){
		$sql_search.=" and wr_name like '%$_GET[swr_name]%' and replace(wr_7,'-','')='$_GET[swr_7]'";
	}else if($_GET['swr_name']&&!$_GET[swr_7]){
...
if($_GET[f_wr_id]){
	$qstr2.="&amp;f_wr_id=".$_GET[f_wr_id];
	$qstr.="&amp;f_wr_id=".$_GET[f_wr_id];
}
if($_GET['swr_name']){
	$qstr.="&amp;swr_name=".$_GET['swr_name'];
}
if($_GET['swr_7']){
	$qstr.="&amp;swr_7=".$_GET['swr_7'];
}
