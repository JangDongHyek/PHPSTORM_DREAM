...
//예약하기 제목과 연락처 배열로 만들기
if($bo_table=="reserve"){
	$wr_subject = $wr_name."님이 ".$_POST['ca_name']." 예약신청하였습니다.";
	$wr_7Arr=$_POST['wr_7'];
}
//중고차 휴대폰번호 배열로 만들기
if($bo_table=="contact"){
...
//예약하기에서 연락처 필드 추가
if($bo_table=="reserve"){
	$wr_7=implode("-",$wr_7Arr);
}
//견적문의하기에서 연락처 필드추가하기
if($bo_table=="contact"){
	$wr_1=implode("-",$wr_1Arr);
}
$addColumn="";
if($_POST['wr_11']){
	$addColumn.=",wr_11='$_POST[wr_11]'";
}
if($_POST['wr_12']){
	$addColumn.=",wr_12='$_POST[wr_12]'";
}
...
if ($w == '' || $w == 'r') {
	...
	if($bo_table=="reserve"){
		...
		$content.="상품명 : {$wr_9}\n";
		$content.="예약자명 : {$wr_name}\n";
		$content.="연락처 : {$wr_7}\n";
		$content.="차종 : {$wr_9}\n";
		$content.="예약일시 : {$wr_2} {$wr_3}시\n";

		$content.=$ca_name=="단기대여 서비스"?"대여일시 : {$wr_2} {$wr_3}시\n":"";
		$content.=$ca_name=="단기대여 서비스"?"반납일시 : {$wr_4} {$wr_5}시\n":"";
		$wr_10=$wr_10!=""?"신청함":"신청안함";
		$content.=$ca_name=="단기대여 서비스"?"자차보험 : {$wr_10}\n":"";
		$content.="픽업장소 : {$wr_1}\n";
		$content.=$ca_name=="단기대여 서비스"?"반납장소 : {$wr_12}\n":"";
		/*		배차주소 : 부산
				회차주소 : 부산*/
		$content.=$ca_name=="단기대여 서비스"?"대여료 총금액 : ￦".number_format($wr_11)."\n":"";
		$content.="소중한 시간을 보다 편안하고 즐거운 여행이 되도록 항상 최선을 다할 것을 약속드립니다.
범아렌터카 콜센터(1566-9600)
자차 보험, 가입/비가입 문자발송시 설정확인 해주세요.";


		goMMS("01088008000","01088008000",$content,6);
	}
}  else if ($w == 'u') {
...
if($bo_table=="reserve"&&$w==""){
	alert($ca_name." 예약이 완료되었습니다. 확인 후 연락드리겠습니다.",G5_BBS_URL."/write.php?bo_table=".$bo_table."&ca_name=".$ca_name);
}else{
	if ($file_upload_msg)
