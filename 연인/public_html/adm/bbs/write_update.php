...
if ($w == '' || $w == 'r') {
    ...
    // 공지사항일 때 푸시 보내기
    if ($bo_table == "b_notice" && $member['mb_id'] != 'lets080') {
        $send_push = true;
    }
    ...
    if (get_session('ss_bo_table') != $_POST['bo_table'] || get_session('ss_wr_id') != $_POST['wr_id']) {
		alert('올바른 방법으로 수정하여 주십시오.', G5_ADMIN_URL.'/bbs/board.php?bo_table='.$bo_table);
    }
...
// 이미지업로드시 추가 (동적으로 bf_file[]생성시 bf_no 재정렬)
$sql = " SELECT * FROM g5_board_file WHERE bo_table = '{$bo_table}' AND wr_id = '{$wr_id}' ORDER BY bf_no ASC; ";
...
if ($result_cnt > 0) {
	...
	for ($i = 0; $row = sql_fetch_array($result); $i++) {
		$bf_file = $row['bf_file'];
		$sql_common = " WHERE bo_table = '{$bo_table}' AND wr_id = '{$wr_id}' AND bf_no = '{$row['bf_no']}' AND bf_file = '{$row['bf_file']}' AND bf_filesize = '{$row['bf_filesize']}' AND bf_datetime = '{$row['bf_datetime']}'";

		if ($bf_file == "") {
			$sql = "DELETE FROM g5_board_file ".$sql_common;
		} else {
			$sql = "UPDATE g5_board_file SET bf_no = '{$bf_no_new}' ".$sql_common;
			$bf_no_new++;
		}
		sql_query($sql);
	}
...
if (!($w == 'u' || $w == 'cu') && $config['cf_email_use'] && $board['bo_use_email']) {
	...
	$link_url = G5_ADMIN_URL.'/bbs/board.php?bo_table='.$bo_table.'&amp;wr_id='.$wr_id.'&amp;'.$qstr;
...
// 공지사항 푸시발송
if ($send_push) {
    $sql = "select * from g5_fcm order by idx asc";
    $result = sql_query($sql);
    $i = 0;
    $tokens = array();
    ...
    $message = array(
        ...
        "goUrl" => G5_URL . "/bbs/board.php?bo_table=b_notice&wr_id=" . $wr_id
    );

    while ($row = sql_fetch_array($result)) {
        $tokens[$i] = $row[token];
        $mb_idArr[$i] = $row[mb_id];
        $i++;
    }
    if (1000 < $i) {
        $total = floor($i / 1000) + 1;
    } else {
        $total = $i;
    }

    for ($j = 0; $j < $total; $j++) {
        $tokens2 = array_slice($tokens, $j * 1000, 1000);
...
if ($file_upload_msg) {
    alert($file_upload_msg, G5_ADMIN_URL.'/bbs/board.php?bo_table='.$bo_table.'&amp;wr_id='.$wr_id.'&amp;page='.$page.$qstr);
} else {
