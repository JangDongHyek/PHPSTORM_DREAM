...
// 공통쿼리
$sql_common = "FROM g5_bbs_coupon A LEFT JOIN g5_member B ON A.mb_no = B.mb_no WHERE A.del_yn = 'N' ";
...
$sql = "SELECT A.*, B.mb_no, B.mb_id, B.mb_name,
        ...
        (SELECT COUNT(idx) FROM g5_bbs_reply WHERE del_yn = 'N' AND tbl_name = 'coupon' AND tbl_idx = A.idx) AS reply_cnt
...
<div id="bo_list">
    <div class="bo_fx">
        ...
        <ul class="btn_bo_user">
            <li><a href="./coupon_form.php" class="btn_b02">등록하기</a></li>
        </ul>
    </div>
    ...
    <form name="fboardlist" id="fboardlist" method="post">
            ...
            <table>
                ...
                <?php
                    ...
                    for ($i=0; $row=sql_fetch_array($result); $i++) {
                        $link = "./coupon_view.php?idx=".$row['idx'];
                        if ($page > 1) $link .= "&page={$page}";
                        if ($qstr != "") $link .= "&{$qstr}";

                        // 발급완료
                        $status = ($row['reply_cnt'] == 0)? "<span class='btn02'>요청대기</span>" : "<span class='btn03'>발급완료</span>";
                ?>
                <tr style="text-align:center;">
                    ...
                    <td><a href="javascript:void(0)" onclick="getWinPop('<?=$row['mb_id']?>')"><?=$row['mb_name']?></a></td>
        ...
        <div class="bo_fx">
            <ul class="btn_bo_adm">
                <li><input type="button" name="btn_submit" value="선택삭제" onclick="deleteBbsList('couponDelete')"></li>
            </ul>
            <?/*
            <ul class="btn_bo_user">
...
<script>
    ...
    function getWinPop(mb_id) {
        ...
        var url = "<?=G5_URL?>/adm/member_form.php?w=u&mb_id="+mb_id;
