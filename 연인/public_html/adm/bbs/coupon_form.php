...
if ($idx > 0) { // 수정
    $sql = "SELECT A.*, B.mb_name FROM g5_bbs_coupon A LEFT JOIN g5_member B ON A.mb_no = B.mb_no 
            WHERE A.del_yn = 'N' AND A.idx = '{$idx}'";
    $write = sql_fetch($sql);
    if (!$write) alert("존재하지 않는 글 입니다.");

    // 첨부파일 가져오기
    $files = getBbsFiles('coupon', $idx);
...
<script>
    ...
    function formSubmit(f) {
        ...
        frmData.append("mode", "couponWrite");
