...
$sql = "SELECT A.*, B.mb_name,
        ...
        FROM g5_bbs_coupon A LEFT JOIN g5_member B ON A.mb_no = B.mb_no 
...
<article id="bo_v" class="max1200">
    <header>
        ...
        <div style="padding: 0 0 10px; border-bottom: 1px solid #ddd; margin: 0 0 10px;">
            요청회원 <a href="" style="font-weight:bold; margin-left: 5px;"><?=$row['mb_name']?></a>
        </div>
        <section id="bo_v_info">
            <h2>페이지 정보</h2>
            작성자 <strong><?=$row['helper_name']?> (<?=$row['writer_ip']?>)</strong>
    ...
    <div id="bo_v_top">
        <?php ob_start(); ?>
        <ul class="bo_v_com">
            <li><a href="./coupon_form.php?idx=<?=$idx?>&<?=$qstr?>" class="btn_b01">수정</a></li>
            <li><a href="javascript:void(0)" onclick="deleteBbsOne('couponDelete', <?=$idx?>)" class="btn_b01">삭제</a></li>
            <li><a href="./coupon.php?<?=$qstr?>" class="btn_b01">목록</a></li>
        </ul>
        <?php
