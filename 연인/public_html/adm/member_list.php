...
$g5_couponM = new Model("g5_coupon","idx");
            ...
            <td style="max-width: 350px">
                ...
                <div id="sub_loca_area">
                    <div class="lst">
                        ...
                        <div class="btn_area">
                            <p><strong>체크하지 않고</strong> 선택완료를 누르시면 <strong>'전체'로 추가</strong>됩니다.</p>
            ...
            for ($i=0; $row=sql_fetch_array($result); $i++) {
                ...
                <tr class="<?php echo $bg; ?>">
                    ...
                    <td>
                        ...
                        <!-- 쿠폰 (남자만) -->
                        <? 
                        $coupons = $g5_couponM->gets(array(
                            "mb_no" => $mb_no,
                            "use_date" => ""
                        ));
                        $coupon_btn_cls = ($coupons['count'] == 0)? "btn02" : "btn01"; 
                        ?>
                        <button type="button" class="<?=$coupon_btn_cls?>" onclick="getWinPop('coupon', '<?=$mb_no?>')"><?=$coupons['count']?>개</button>
                        <? } // end 남 ?>
                    </td>
                    <td>
                        <? if ($row['mb_sex'] == "남") { ?>
                            <!-- 하트 (남자만)-->
                            <?
                            $mb_heart = number_format((int)$row['mb_heart']);
                            if ($member['mb_status'] == "관리자") {
                                echo "<a href='javascript:void(0)' onclick=\"getWinPop('heart', '{$mb_no}')\">{$mb_heart}P</a>";
                            } else {
                                echo "{$mb_heart}개";
                            }
                            ?>
