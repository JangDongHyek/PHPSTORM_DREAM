...
<form name="fmember" id="fmember" action="./member_form_update.php" onsubmit="return fmember_submit(this);" method="post" enctype="multipart/form-data" class="max1200">
	...
	<div class="tbl_frm01 tbl_wrap">
        ...
        <tr>
            <th scope="row"><label for="mb_5">카카오아이디</label></th>
		...
		<tr>
			...
			<th scope="row">병역</th>
			<td>
				<input type="radio" name="mb_military" id="mt1" value="미필" <? if ($mb['mb_military'] == "미필") echo "checked"; ?>><label for="mt1">미필</label>
				<input type="radio" name="mb_military" id="mt2" value="군필" <? if ($mb['mb_military'] == "군필") echo "checked"; ?>><label for="mt2">군필</label>
			</td>
		</tr>
		<tr>
		...
		<tr>
			...
			<th scope="row"><label for="mb_blood_type">혈액형</label></th>
			<td>
				<select name="mb_blood_type" id="mb_blood_type">
					<option value="">--선택--</option>
					<? foreach ($blood_arr as $key=>$val) { ?>
					<option value="<?=$val?>" <? if ($mb['mb_blood_type'] == $val) echo "selected"; ?>><?=$val?></option>
					<? } ?>
				</select>
			</td>
		</tr>
		<tr>
			...
			<th scope="row"><label for="mb_car">자차</label></th>
			<td>
				<input type="radio" name="mb_car" id="car1" value="유" <? if ($mb['mb_car'] == "유") echo "checked"; ?>><label for="car1">유</label>
				<input type="radio" name="mb_car" id="car2" value="무" <? if ($mb['mb_car'] == "무") echo "checked"; ?>><label for="car2">무</label>
			</td>
		</tr>
		<tr>
			...
			<th scope="row"><label for="mb_religion">종교</label></th>
			<td>
				<select name="mb_religion" id="mb_religion" onchange="getSelctdChk(this);">
					<option value="">--선택--</option>
					<? 
					// 직접입력체크변수
					$mb_religion_str = "";
					...
					foreach ($religion_arr as $key=>$val) { 
						if ($mb['mb_religion'] != "" && !in_array($mb['mb_religion'], $religion_arr) && $val == "직접입력") {
							$mb_religion_str = $mb['mb_religion'];
							$input_class = "show";
						}
					?>
					<option value="<?=$val?>" <? if (($mb['mb_religion'] == $val || $mb['mb_religion'] == $mb_religion_str) && $mb['mb_religion'] != "") echo "selected"; ?>><?=$val?></option>
					<? } ?>
				</select>
				<input type="text" name="mb_religion_str" class="frm_input <?=$input_class?>" value="<?=$mb_religion_str?>">
			</td>
		</tr>
		<tr>
			<th scope="row"><label for="mb_edu">학력</label></th>
			<td>
				<select name="mb_edu" id="mb_edu">
					<option value="">--선택--</option>
					<? foreach ($edu_arr as $key=>$val) { ?>
					<option value="<?=$val?>" <? if ($mb['mb_edu'] == $val) echo "selected"; ?>><?=$val?></option>
					<? } ?>
				</select>
			</td>
			<th scope="row"><label for="mb_job">직업</label></th>
			<td>
				<select name="mb_job" id="mb_job" onchange="getSelctdChk(this);">
					<option value="">--선택--</option>
					<? 
					// 직접입력체크변수
					$mb_job_str = "";
		...
		<tr>
			...
			<td>
				<select name="mb_body_type" id="mb_body_type" onchange="getSelctdChk(this);">
					<option value="">--선택--</option>
					<? 
					// 직접입력체크변수
					$mb_body_type_str = "";
			...
			<td>
				<select name="mb_hobby" id="mb_hobby" onchange="getSelctdChk(this);">
					<option value="">--선택--</option>
					<? 
					// 직접입력체크변수
					$mb_hobby_str = "";
		...
		<tr>
			<th scope="row">나이</th>
			<td>
				<input type="text" name="ideal_age_min" value="<?=$mb['ideal_age_min'] ?>" id="ideal_age_min" class="frm_input step03_txt f_num" maxlength="2" placeholder="최소" data-name="ideal_age_min" size="10"> ~ <input type="text" name="ideal_age_max" value="<?=$mb['ideal_age_max'] ?>" id="ideal_age_max" class="frm_input step03_txt f_num" maxlength="2" placeholder="최대" data-name="ideal_age_min" size="10">
				<!--<?=$mb['ideal_age_min']?>세 ~ <?=$mb['ideal_age_max']?>세-->
			</td>
			<th scope="row">장거리유무</th>
			<td>
				<input type="radio" name="ideal_long_dist" id="logdi01" value="유" <? if ($mb['ideal_long_dist'] == "유") echo "checked"; ?>><label for="logdi01">유</label>&nbsp;&nbsp;
				<input type="radio" name="ideal_long_dist" id="logdi02" value="무" <? if ($mb['ideal_long_dist'] == "무") echo "checked"; ?>><label for="logdi02">무</label>
				<!--<?=$mb['ideal_long_dist']?>-->
			</td>
		</tr>
		<tr>
			<th scope="row">키</th>
			<td>
				<input type="text" name="ideal_height_min" value="<?=$mb['ideal_height_min'] ?>" id="ideal_height_min" class="frm_input step03_txt f_num" maxlength="3" placeholder="최소" data-name="ideal_height_min" size="10"> ~ <input type="text" name="ideal_height_max" value="<?=$mb['ideal_height_max'] ?>" id="ideal_height_max" class="frm_input step03_txt f_num" maxlength="3" placeholder="최대" data-name="ideal_height_min" size="10">
				<!--<?=$mb['ideal_height_min']?>cm ~ <?=$mb['ideal_height_max']?>cm-->
			</td>
			<th scope="row">체형</th>
			<td>
				<select name="ideal_body_type" id="ideal_body_type" onchange="getSelctdChk(this);" class="f_slct">
					...
					foreach ($body_arr as $key=>$val) { 
						if ($mb['ideal_body_type'] != "" && !in_array($mb['ideal_body_type'], $body_arr) && $val == "직접입력") {
							$ideal_body_type_str = $mb['ideal_body_type'];
							$input_class = "show";
						}
					?>
					<option value="<?=$val?>" <? if (($mb['ideal_body_type'] == $val || $mb['ideal_body_type'] == $ideal_body_type_str) && $mb['ideal_body_type'] != "") echo "selected"; ?>><?=$val?></option>
					<? } ?>
				</select>
		...
		<tr>
			...
			<td>
				<select name="ideal_religion" id="ideal_religion" onchange="getSelctdChk(this);" class="f_slct">
					...
					foreach ($religion_arr as $key=>$val) { 
						if ($mb['ideal_religion'] != "" && !in_array($mb['ideal_religion'], $religion_arr) && $val == "직접입력") {
							$ideal_religion_str = $mb['ideal_religion'];
							$input_class = "show";
						}
					?>
					<option value="<?=$val?>" <? if (($mb['ideal_religion'] == $val || $mb['ideal_religion'] == $ideal_religion_str) && $mb['ideal_religion'] != "") echo "selected"; ?>><?=$val?></option>
					<? } ?>
				</select>
			...
			<td>
				<select name="ideal_drinking" id="ideal_drinking" class="f_slct">
					<option value="">선택하세요</option>
					<? foreach ($drinking_arr as $key=>$val) { ?>
					<option value="<?=$val?>" <? if ($mb['ideal_drinking'] == $val) echo "selected"; ?>><?=$val?></option>
					<? } ?>
				</select>
				<!--<?=$mb['ideal_drinking']?>-->
			</td>
		...
		<tr>
			...
			<td colspan="3">
				<select name="ideal_smoking" id="ideal_smoking" class="f_slct">
					<option value="">선택하세요</option>
					<? foreach ($smoking_arr as $key=>$val) { ?>
					<option value="<?=$val?>" <? if ($mb['ideal_smoking'] == $val) echo "selected"; ?>><?=$val?></option>
					<? } ?>
				</select>
				<!--<?=$mb['ideal_smoking']?>-->
			</td>
		...
		<tr>
			...
			<td colspan="3" id="ideal_area">
				...
				<textarea name="ideal_char" id="ideal_char" class="frm_input f_txt" placeholder="원하는 이상형의 성격을 입력해주세요."><?=$mb['ideal_char']?></textarea>
				<!--<?=nl2br($mb['ideal_char'])?>-->
			</td>
		</tr>
		<tr>
			<th scope="row">원하는 데이트</th>
			<td colspan="3">
				<textarea name="ideal_date" id="ideal_date" class="frm_input f_txt" placeholder="원하는 데이트를 입력해주세요."><?=$mb['ideal_date']?></textarea>
				<!--<?=nl2br($mb['ideal_date'])?>-->
			</td>
		</tr>
		<tr>
			<th scope="row">자신만의 특별한 이상형</th>
			<td colspan="3">
				<textarea name="ideal_contents" id="ideal_contents" class="frm_input f_txt" placeholder="본인만의 특별한 이상형을 입력해주세요."><?=$mb['ideal_contents']?></textarea>
				<!--<?=nl2br($mb['ideal_contents'])?>-->
			</td>
		</tr>
		</tbody>
		</table>
