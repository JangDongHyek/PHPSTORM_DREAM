<?
/**********************************
 * status : 헬퍼관리 - 출퇴근on,off변경
 * match_del : 회원관리 - 소개이력 삭제, 정산현황 - 건별건수에서 삭제
 * delete : 헬퍼관리 - 헬퍼삭제 (헬퍼상태를 out으로 변경 mb_3)
...
switch ($mode) {
	case "status" :
		...
		// 본인정보인지 확인
		if ($member['mb_status'] == "헬퍼" && $member['mb_id'] != $_POST['mb_id']) {
			die("E");
		}
		...
		$sql = "UPDATE g5_member SET 
				mb_3 = '{$stt}'
				WHERE mb_id = '{$mb_id}' AND mb_status = '헬퍼'
				";
	...
	case "match_del" :
		...
		$sql = "DELETE FROM g5_matching WHERE idx = '{$_POST['idx']}'";
