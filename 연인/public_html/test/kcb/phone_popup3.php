...
<form action="<?=G5_BBS_URL?>/register_form.php" method="post">
	<input type="hidden" name="kcb_step" value="2">
	<input type="hidden" name="kcb_name" value="<?=$RSLT_NAME?>">
	<input type="hidden" name="kcb_birth" value="<?=$RSLT_BIRTHDAY?>">
	<input type="hidden" name="kcb_sex" value="<?=$RSLT_SEX_CD?>">
	<input type="hidden" name="kcb_hp" value="<?=$TEL_NO?>">
</form>
	...
	if($ret == 0) {
		...
		// 인증결과를 확인하여 페이지분기등의 처리를 수행해야한다.
	 	if ($RSLT_CD == "B000") {
			...
			echo ("<script>alert('본인인증에 실패하였습니다. 다시 시도해 주세요.'); location.href = '".G5_BBS_URL."/register_form.php';</script>");
		}
	} else {
		//인증결과 복호화 실패
