...
// 관리자페이지 게시판 페이징시 page=1&page=2 문제 해결함수 (파라미터 인식 때문에)
        ...
        for ($k=0; $k<count($field); $k++) { // 필드의 수만큼 다중 필드 검색 가능 (필드1+필드2...)
            ...
            switch ($field[$k]) {
                ...
                case "wr_password" :
                    ...
                    break;
				case "helper_name" :
					$str .= "wr_subject IN (SELECT mb_id FROM g5_member WHERE mb_name LIKE '%".$s[$i]."%')";
					break;
...
function getRandomString($leng = 6){
	$chars = "1234567890";
