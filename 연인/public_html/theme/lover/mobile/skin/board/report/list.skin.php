...
<div id="bo_list" style="width:<?php echo $width; ?>">
    ...
    <div class="tbl_head01 tbl_wrap">
        ...
        for ($i=0; $i<count($list); $i++) {
			...
			if ($comment_cnt > 0) {
				$ca_name = "<span class='btn03'>답변완료</span>";
			} else {
				$ca_name = "<span class='btn02'>답변대기</span>";
			}
         ?>
        <tr style="text-align:center;">
            ...
            <?php } ?>
            <td><a href="../helper_form.php?w=u&mb_id=<?=$helper_id?>" target="_blank"><?=$helper_name?></a></td>
            <td><a href="../member_form.php?w=u&mb_id=<?=$list[$i]['mb_id']?>" target="_blank"><?php echo $list[$i]['wr_name']?></a></td>
			<td><a href="<?php echo $list[$i]['href'] ?>"><?=$wr_content?></a></td>
            <td><?php echo $list[$i]['wr_datetime'] ?></td>
			<td><?=$ca_name?></td>
        </tr>
        <?php } ?>
        <?php if (count($list) == 0) { echo '<tr><td colspan="10" class="empty_table">요청내용이 없습니다.</td></tr>'; } ?>
        </tbody>
        </table>
    </div>
...
<fieldset id="bo_sch">
    ...
    <select name="sfl" id="sfl">
        <option value="helper_name"<?php echo get_selected($sfl, 'helper_name', true); ?>>헬퍼명</option>
