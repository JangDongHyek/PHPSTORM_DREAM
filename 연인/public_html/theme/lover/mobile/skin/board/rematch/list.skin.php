...
<div id="bo_list" style="width:<?php echo $width; ?>">
    ...
    <div class="tbl_head01 tbl_wrap">
        ...
        for ($i=0; $i<count($list); $i++) {
			...
			$sql = "SELECT COUNT(wr_id) AS cnt FROM g5_write_{$bo_table} WHERE wr_parent = '{$list[$i]['wr_id']}' AND wr_is_comment = 1 ";
			if ($member['mb_status'] == "헬퍼") {
				$sql .= " AND mb_id != '{$list[$i]['mb_id']}' ";
			}
			$row = sql_fetch($sql);
			$comment_cnt = $row['cnt'];

			if ($comment_cnt > 0) {
				$ca_name = "<span class='btn03'>리매칭완료</span>";
			} else {
        ...
        <tr style="text-align:center;">
			...
			<td><?php echo $list[$i]['wr_name']?></a></td>
