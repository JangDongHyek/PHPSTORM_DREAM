...
if ($w == '') {
    ...
    // 24.05.28 추가 => 가입시 데이터 추적을 위한 로그 테이블
    $log_sql = " insert into g5_member_log
                set mb_id = '{$mb_id}',
                log_type = '',
                     mb_password = '".get_encrypt_string($mb_password)."',
                     mb_name = '{$mb_name}',
    ...
    if (!$config['cf_use_email_certify'])
        $log_sql .= " , mb_email_certify = '".G5_TIME_YMDHIS."' ";
    sql_query($log_sql);
	...
	*/
    ...
    // 24.05.28 추가 => 회원정보 수정시 데이터 추적을 위한 로그 테이블
    $log_sql = " insert into g5_member_log
                set mb_email = '{$mb_email}',
                    log_type = 'u',
                    mb_tel = '{$mb_tel}',
