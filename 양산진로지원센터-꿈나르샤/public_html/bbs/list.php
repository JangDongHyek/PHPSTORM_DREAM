...
if ($sca || $stx || $wr_cate) {
	...
	if($bo_table == "request" && !$is_admin){
		if($sfl2 && $stx2){
			$sql_search .= " and (".$sfl2." = '".$stx2."') ";
		}		
	}

	if($bo_table == "request" && $is_admin){
		if($wr_cate){
			$sql_search .= " and (wr_1 = '".$wr_cate."') ";
		}
	}
	if($_GET[s_wr_1]){
		$sql_search.=" and wr_1 ='".$_GET[s_wr_1]."'";
	}
...
if($_POST[wr_name]&&$_POST[wr_2]){
		$sql_search.=" and wr_name='$_POST[wr_name]' and wr_2='$_POST[wr_2]'";
	}
