...
if($bo_table=="push"){
	if(empty($_POST[wr_name])){
		alert("학부모 이름을 입력하세요");
		exit;
	}
	if(empty($_POST[wr_subject])){
		alert("학부모 휴대폰번호를 입력하세요");
		exit;
	}
	if(empty($_POST[wr_content])){
		alert("거주동을 입력하세요");
		exit;
	}
	if(empty($_POST[wr_1])){
...
if($bo_table!="push"&&$bo_table!="apply"){
...
if ($w == '' || $w == 'r') {
	...
	if($bo_table=="push"){
		goSms($_POST[wr_subject], "0557850151", $_POST['wr_name']."님 행사알림 신청이 접수되었습니다.");
		goSms($m_hp1, "0557850151", $_POST['wr_name']."님 행사알림 신청이 접수되었습니다.");
		goSms($m_hp2, "0557850151", $_POST['wr_name']."님 행사알림 신청이 접수되었습니다.");
	}else if($bo_table=="apply"){
		goSms($_POST[wr_2],"0557850151", $_POST['wr_name']."님 프로그램 신청이 접수되었습니다.");
		goSms($m_hp1, "0557850151", $_POST['wr_name']."님 프로그램 신청이 접수되었습니다.");
		goSms($m_hp2, "0557850151", $_POST['wr_name']."님 프로그램 신청이 접수되었습니다.");
	}
}  else if ($w == 'u') {
...
if ($file_upload_msg){
	...
	alert($file_upload_msg, 'http://www.xn--2o2b95mbma56lv7f1zm.com/bbs/board.php?bo_table='.$bo_table.'&amp;wr_id='.$wr_id.'&amp;page='.$page.$qstr);
}else{
    //goto_url(G5_HTTP_BBS_URL.'/board.php?bo_table='.$bo_table.'&amp;wr_id='.$wr_id.$qstr);
	if($bo_table == "request"){
		alert('박람회 신청이 접수되었습니다.', 'http://www.xn--2o2b95mbma56lv7f1zm.com/bbs/write.php?bo_table='.$bo_table);
	}else if($bo_table==="push"){
		alert("알림서비스 신청이 접수되었습니다.",'http://www.xn--2o2b95mbma56lv7f1zm.com/bbs/write.php?bo_table='.$bo_table);
	}else if($bo_table=="apply"){
		alert("프로그램 신청이 접수되었습니다.",'http://www.xn--2o2b95mbma56lv7f1zm.com/');
	}else{
		goto_url('http://www.xn--2o2b95mbma56lv7f1zm.com/bbs/board.php?bo_table='.$bo_table.'&amp;wr_id='.$wr_id.$qstr);
	}
}
?>
