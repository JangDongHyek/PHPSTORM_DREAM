...
<div class="tbl_frm01 tbl_wrap">
	...
	<tr>
		<th scope="row"><label for="co_group">내용그룹</label></th>
		<td>
		<select name="co_group" id="co_group">
			<option value="">그룹 없음</option>
			<?php 
			$res = sql_query("select * from {$g5['group_content_table']} order by gc_id asc");
			for($i=0; $i<$group=sql_fetch_array($res); $i++){ 
			
			?>
			<option value="<?php echo $group['gc_name']?>" <?php if($co['co_group']==$group['gc_name']) echo "selected";?>><?php echo $group['gc_name']?></option>
			<?php } ?>
		</select>
		</td>
	</tr>
