...
<style>
.confirm_bg {width:100%; height:100%; display:block; opacity:0.8; filter:alpha(opacity=80); position:fixed; left:0; top:0; background-color:#e5e5e5; z-index:10000; visibility:hidden;}
.confirm_container {display:block; position:fixed; margin:0; padding:0; top:10%; left:50%; width:90%; height:80%; margin-left:-45%; background-color:#ffffff; border:0; z-index:10001; visibility:hidden;}
.confirm_tbl {margin:0; padding:0; width:100%; border-collapse:collapse;}
.bg03 {background-color:#aaaaaa;}
.calign {text-align:center;}
</style>
<script>
function layer_open(datas,cdays,mode){
	
	$.ajax({
		type: "POST",
		url: "<?php echo $board_skin_url ?>/ajax.php",
		data: { datas: datas, cdays: cdays, mode: mode },
		success:function( html ) {
			$("#confirm_td").html(html);
			//$( "#results" ).append( html );
		}
	});

	$(".confirm_bg").css("visibility","visible");
	$(".confirm_container").css("visibility","visible");
	$("#mode_value").html(mode);
}

function layer_close(){
	$(".confirm_bg").css("visibility","hidden");
	$(".confirm_container").css("visibility","hidden");
}
</script>
<div class="confirm_bg" onclick="layer_close();"></div>
<div class="confirm_container" style="overflow-x:hidden; overflow-y:scroll;">
	<div style="margin:0; padding:10px 0px; text-align:center; color:#fff; background:#4386f4; font-family:'Nanum Gothic', 'sans-serif', '굴림', '돋움'; font-size:16px;" id="mode_value"></div>
	<table class="confirm_tbl">
	<tbody>
	<tr>
		<td id="confirm_td" style="margin:0; padding:0px 0px; width:100%;">
	<?/*
	<tr>
		<td class="confirm_td bg01 calign" colspan="2">
			<p id="point_p"></p>
			<p class="confirm_qs">구매하시겠습니까?</p>
		</td>
	</tr>
	<tr>
		<td class="confirm_td bg02 calign" style="cursor:pointer;" onclick="sch_point();"><label class="confirm_yes">구매하기</label></td>
		<td class="confirm_td bg03 calign" style="cursor:pointer;" onclick="layer_close();"><label class="confirm_no">취소</label></td>
	</tr>
	*/?>
		</td>
	</tr>
	...
	<div style="position:absolute; display:block; width:100%; bottom:0;" onclick="layer_close();">
	<button style="margin:0; padding:15px 0px; width:100%; font-weight:bold; font-family:'Nanum Gothic', 'sans-serif', '굴림', '돋움'; font-size:16px; border:1px solid #777; background:#aaa; color:#fff;">닫기</button>
	<!--<p style="margin:0; margin-top:-1px; padding:0px 0px; padding-top:10%; height:90%; border:1px solid #777; text-align:center;">닫기</p>-->
	</div>
</div>
...
?>
	<div class="calender">
		...
		?>
		<?/*<div style="text-align:center;"><a style="position:relative; margin:0px; padding:2px 5px; height:28px; line-height:28px; background-color:#000; color:#fff; border:1px solid #000; font-size:10px;">종료</a></div>*/?>
		<?php
			}else if(count($schedule[$cday]) > 0){
		?>
		<div style="text-align:center;"><a style="position:relative; margin:0px; padding:2px 5px; height:28px; line-height:28px; background-color:#eb4b3e; color:#fff; border:1px solid #eb4b3e; font-size:10px;" onclick="layer_open('<?php echo $year; ?>-<?php echo $sel_mon; ?>%','<?php echo $cday; ?>','예약신청');">신청</a></div>
		<?php
			}
		}
		?>
		...
		<?php
		if($is_admin && count($schedule[$cday]) > 0){
		?>
		<div style="text-align:center;"><a style="position:relative; margin:0px; padding:2px 5px; height:28px; line-height:28px; background-color:#4386f4; color:#fff; border:1px solid #4386f4; font-size:10px;" onclick="layer_open('<?php echo $year; ?>-<?php echo $sel_mon; ?>%','<?php echo $cday; ?>','예약현황');">현황</a></div>
		<?php
		}
		?>
		<? 
