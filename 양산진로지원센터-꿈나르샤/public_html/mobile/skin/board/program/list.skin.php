				...
				<?php
					...
					if($now<strtotime($list[$i]['wr_1'])){
						...
						if($cRow[cnt]<$list[$i]['wr_5']){
							echo "<a href='".G5_BBS_URL."/write.php?bo_table=apply&wr_1=".$list[$i]['wr_id']."' class='btn btn_request' style='color:white'>신청가능</a>";
						}else{
							echo "<a href='javascript:alert(\'신청마감되었습니다.'\);' class='btn btn_finish' style='color:white'>마감</a>";
						}
					}else{
						echo "<a href='javascript:;' class='btn btn_finish' style='color:white'>마감</a>";
					}
					if($is_admin){
						echo "<br/><a href='".G5_BBS_URL."/board.php?bo_table=apply&s_wr_1=".$list[$i][wr_id]."'>신청목록보기</a>";
					}
				?>
			</td>
        </tr>
