    ...
    for ($i=0; $i<count($s); $i++) {
        ...
        for ($k=0; $k<count($field); $k++) { // 필드의 수만큼 다중 필드 검색 가능 (필드1+필드2...)
            ...
            switch ($field[$k]) {
                ...
                default :
					if($_GET['bo_table'] == "customer" && $field[$k] == 'wr_1'){
						$str .= "Regexp_replace(trim(lower(".$field[$k].")),'[^a-z,A-Z,0-9]','')  like '%".$s[$i]."%'";	// 연락처
					} else {
 ...
 * => 아닐경우 메인페이지로 이동
 */
function is_member($is_member){
    if (!$is_member) alert('가입회원만 접속할수 있는 페이지입니다.', G5_URL);
}
