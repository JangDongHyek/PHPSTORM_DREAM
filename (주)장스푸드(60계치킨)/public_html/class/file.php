...
class File{
    ...
    function save($file, $path = ""){
        ...
        else $path = $_SERVER['DOCUMENT_ROOT'].$path;
    ...
    function bind($file,$permission = "",$path = "") {
        if($file == "null" || $file == "undefined" || $file == null || $file == "" || $file["size"] == 0) {
            ...
            $file[status] = "read";
            $file[src] = $path."/".$src;
            $file[resize_src] = $path."/resize_".$src;
            $this->resize_image("$_SERVER[DOCUMENT_ROOT]".$file[src],"$_SERVER[DOCUMENT_ROOT]".$file[resize_src],200,100);
