...
-->
            ...
            <?php if($list[$i]["wr_7"])  { $cm_upfile1 = json_decode($list[$i]["wr_7"],true);?>
                <span class="bo_vc_hdinfo">첨부파일1 :
                    <a href="<?=$cm_upfile1["src"]?>" download> <?=$cm_upfile1["name"] ?> </a>
                </span>
            <?php } ?>

            <?php if($list[$i]["wr_8"])  { $cm_upfile2 = json_decode($list[$i]["wr_8"],true);?>
                <span class="bo_vc_hdinfo">첨부파일2 :
                    <a href="<?=$cm_upfile2["src"]?>" download> <?=$cm_upfile2["name"] ?> </a>
                </span>
            <?php } ?>
...
?>
    ...
    if($member['mb_id'] == "lets080" || $member['mb_id'] == "admin" || $member['mb_id'] == "admin2" || $member['mb_id'] == "admin3"){ ?>
        <tr>
            <th>상태</th>
            <td>
                <select id="wr_state" name="wr_state">
                    <option value="미처리" <? if($view['wr_10'] == "미처리") {?> selected <?}?>>미처리</option>
                    <option value="처리중" <? if($view['wr_10'] == "처리중") {?> selected <?}?>>처리중</option>
                    <option value="처리완료" <? if($view['wr_10'] == "처리완료") {?> selected <?}?>>처리완료</option>
                </select>
                * 상태는 변경시 즉시 변경됩니다.
            </td>
        </tr>
...
-->
        ...
        <tr>
            ...
            <td>
                <input type="file" name="cmUpfile1">
            </td>
        </tr>
        ...
        <tr>
            ...
            <td>
                <input type="file" name="cmUpfile2">
            </td>
        </tr>
        </tbody>
        </table>
...
<script>
    ...
    $(document).ready(function() {
        $("#wr_state").on("change", function () {
            let wr_state = $(this).val();
            let wr_id = "<?= $view['wr_id']?>";
            $.post("<?=G5_URL?>/ajax/set_cs_state.php",{"wr_state":wr_state, "wr_id":wr_id},function (data) {
...
//댓글 있는 경우 보이지 않도록
var cmt_amt = '<?php echo json_encode($cmt_amt); ?>';
var c_id = '<?php echo json_encode($c_id); ?>';
var w = <?php echo json_encode($w); ?>;
if (cmt_amt != '0' && c_id == 'null' && w == 'c') {
    //document.getElementById('#bo_vc_w').style.display = 'none';
