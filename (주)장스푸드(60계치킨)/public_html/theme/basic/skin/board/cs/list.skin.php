...
//내 문의내역만 확인
/*$paramMbId = $_GET['mb_id'];
if ($paramMbId) {
    $filtered_list = array_filter($list, function($item) use ($paramMbId) {
        return $item['mb_id'] === $paramMbId;
    });

    // array_values 함수를 사용하여 키를 순차적으로 재정렬
...
<!--
            ...
            <?php echo $page ?> 페이지
-->
           <a href="<?=G5_BBS_URL?>/board.php?bo_table=cs&mb_id=<?=$_SESSION['ss_mb_id']?>">내 문의내역</a>
        </div>
        ...
        <tr class="<?php if ($list[$i]['is_notice']) echo "bo_notice"; ?>">
            ...
            <td class="td_subject">
                ...
                <a href="<?php echo $list[$i]['href'] ?>">
                    ...
                    <?/*php if ($list[$i]['comment_cnt']) { ?><span class="sound_only"></span><?php echo $list[$i]['comment_cnt']; ?><span class="sound_only">개</span><?php } */?>
                </a>
            ...
            <td class="td_state"><? if(empty($list[$i]['wr_10'])){ echo "미처리"; } else { echo $list[$i]['wr_10']; }?>
<!--                --><?php
/*                    //답변있는지 확인
                    $wr_id = $list[$i]['wr_id'];
                    $result = sql_query(" select wr_subject from g5_write_cs where 1=1 and wr_parent = '${wr_id}' ");
...
<fieldset id="bo_sch">
    ...
    <select name="sfl" id="sfl">
		...
		<option value="wr_1"<?php echo get_selected($sfl, 'wr_1'); ?>>연락처</option>
