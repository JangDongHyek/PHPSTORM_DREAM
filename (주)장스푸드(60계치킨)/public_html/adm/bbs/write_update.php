...
if ($w == '' || $w == 'r') {
	...
	if($bo_table=="b_gcm"){
		if($wr_gcm){
			$temp = explode(",",$wr_gcm);
			if(count($temp) <= 1) $post_user = $wr_gcm;
			else $post_user = explode(",",$wr_gcm);
		}
		$post_no = "2";
		$post_title = "헛둘셋";
		$post_content = "새로운 알림이있습니다.";
		$post_url = G5_BBS_URL."/board.php?bo_table=b_gcm&wr_id=".$wr_id;
	...
	if($bo_table=="notice"){
		$post_no = "2";
		$post_title = "헛둘셋";
		$post_content = "새로운 공지사항이있습니다.";
		$post_url = G5_BBS_URL."/board.php?bo_table=notice&wr_id=".$wr_id;
		include(G5_PLUGIN_PATH."/gcm/send_msg.php");
	}
...
if($w!=""){
	...
	if ($file_upload_msg){
		...
		if($bo_table == "cctv"){
			goto_url(G5_ADMIN_HTTP_BBS_URL.'/board.php?bo_table='.$bo_table);
		}else{
