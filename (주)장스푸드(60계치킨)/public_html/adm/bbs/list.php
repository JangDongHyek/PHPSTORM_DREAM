...
if($bo_table == "dome" || $bo_table == "some" || $bo_table == "rental"){
	$stx_get = "&stx=".$stx;
}
...
if ($board['bo_use_category']) {
	...
	for ($i=0; $i<count($categories); $i++) {
		$category2 = trim($categories[$i]);

		if($bo_table == "rental"){			
			$category = trim(str_replace(array("(",")"),'',$categories[$i]));
		}else{
			$category = trim($categories[$i]);
		}
		if ($category=='') continue;
		$category_option .= '<li><a href="'.($category_href.$stx_get."&amp;sca=".urlencode($category)).'"';
		$category_msg = '';
		if ($category==$sca) { // 현재 선택된 카테고리라면
			$category_option .= ' id="bo_cate_on"';
			$category_msg = '<span class="sound_only">열린 분류 </span>';
		}
		$category_option .= '>'.$category_msg.$category2.'</a></li>';
	}
}
...
if($bo_table == "dome" || $bo_table == "some" || $bo_table == "rental"){
	$sql_order = " order by CAST(wr_5 as signed) desc ";
}
...
$write_pages = get_paging(G5_IS_MOBILE ? $config['cf_mobile_pages'] : $config['cf_write_pages'], $page, $total_page, './board.php?bo_table='.$bo_table.'&'.$qstr.'&amp;page=','&sca='.$sca.'&stx='.$stx.'');
...
if($is_adm == "adm"){
	include_once($board_skin_path.'/adm_list.skin.php');
}else{
