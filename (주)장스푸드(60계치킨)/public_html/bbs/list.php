...
// 분류 선택 또는 검색어가 있다면
$stx = trim($stx);
if ($sca || $stx || ($bo_table == "event_join" || $bo_table == "after")) {
    $sql_search = get_sql_search($sca, $sfl, $stx, $sop);

	//신청일시 검색
	if($bo_table == "event_join" || $bo_table == "after"){
		...
		if($day_start && $day_end){
			$sql_search .= " and ( ";
			$sql_search .= " wr_datetime between '{$day_start}' and '{$day_end}' ";
			$sql_search .= " ) ";
		}else if($day_start || $day_end){
			$sql_search .= " and ( ";
			if($day_start) {								
				$sql_search .= " wr_datetime >= '{$day_start}' ";
			}
			if($day_end) {								
				$sql_search .= " wr_datetime <= '{$day_end}' ";
			}
			$sql_search .= " ) ";
		}
	}
    ...
    if($bo_table == "cs" && $member['mb_level'] < 10){
...
/*		$wr_1_ex = explode(" ",$wr_1);
		if($REMOTE_ADDR=="121.144.134.22"){
			echo $wr_1;
		}
		$sql_search .= " and wr_1 like '{%$wr_1_ex[0]}%' and  wr_1 like '%{$wr_1_ex[1]}%'  and  wr_1 like '%{$wr_1_ex[2]}%'";
		*/
		if($si){
...
if($bo_table == "event_join"){
	$add_search = "&wr_1=".$_GET['wr_1']."";
}

if($bo_table == "store"){
	$add_search = "&si=".$_GET['si']."";
	$add_search .= "&gu=".$_GET['gu']."";
	$add_search .= "&dong=".$_GET['dong']."";
}

$write_pages = get_paging(G5_IS_MOBILE ? $config['cf_mobile_pages'] : $config['cf_write_pages'], $page, $total_page, './board.php?bo_table='.$bo_table.$qstr.'&amp;page=', $add_search);
