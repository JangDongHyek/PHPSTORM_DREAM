...
include_once($_SERVER["DOCUMENT_ROOT"]."/class/file.php");
...
if ($w == 'c') // 댓글 입력
{
    ...
    */
    ...
    //24-06-13 cs코멘트 파일첨부기능 추가
    if($write_table == "g5_write_cs") {
//        var_dump($_FILES["cmUpfile1"]);
//        var_dump($_FILES["cmUpfile2"]);
//        die();
        $uploadPath = $_SERVER["DOCUMENT_ROOT"]."/data/file/";
        $File = new File("/data/file/cs/{$wr['wr_num']}");
        $wr_7 = $File->bind($_FILES["cmUpfile1"]);
        $wr_8 = $File->bind($_FILES["cmUpfile2"]);
    }
...
else if ($w == 'cu') // 댓글 수정
{
    ...
    if($write_table == "g5_write_cs") {
        $uploadPath = $_SERVER["DOCUMENT_ROOT"]."/data/file/";
        $File = new File("/data/file/cs/{$wr['wr_num']}");
        $check_wr_7 = $File->bind($_FILES["cmUpfile1"]);
        $check_wr_8 = $File->bind($_FILES["cmUpfile2"]);
        $add_sql = "";
        if($check_wr_7) {
            $wr_7 = $check_wr_7;
            $add_sql .= "wr_7 = '$wr_7',";
