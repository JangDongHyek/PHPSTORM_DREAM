...
$msg = array();
$hpChecks = array("","02","031","032","033","041","042","043","044","052","053","054","055","061","063","064","070","080","010");

if($bo_table=="fran03"){
	$hp = $_POST[wr_subject];
	$hpCheck = 0 < strpos(" ".substr($hp,0,3),"02")?substr($hp,0,2):substr($hp,0,3);

	if(!array_search($hpCheck,$hpChecks)){
		alert("연락처가 올바르게 입력되지 않았습니다.");
		exit;
	}
	$wr3=str_replace("-","",$hp);
	if(!is_numeric($wr3)){
		alert("연락처는 숫자만 입력을 받을 수 있습니다.");
		exit;
	}
}
...
if ($file_upload_msg){
	...
	if($bo_table == "event_join"){
		alert('신청완료되었습니다. 감사합니다.', G5_BBS_URL.'/board.php?bo_table=event&wr_id='.$wr_3);
	}else if($bo_table == "after"){
		alert('신청완료되었습니다. 감사합니다.', G5_BBS_URL.'/content.php?co_id=after_complete');
	}else if($bo_table == "customer"){
		alert('접수되었습니다. 감사합니다.', G5_BBS_URL.'/write.php?bo_table=customer');
	}else if($bo_table == "report"){
        alert('접수되었습니다. 감사합니다.', G5_BBS_URL.'/write.php?bo_table=report');
    }else if($bo_table == "fran03"){
	    if($_SERVER['HTTP_HOST'] == "14.48.175.188") {
	        $rurl = "/~jangsfood/";
        }else {
            $rurl = "/";
        }

        alert('접수되었습니다. 감사합니다.', $rurl);
    }else{
