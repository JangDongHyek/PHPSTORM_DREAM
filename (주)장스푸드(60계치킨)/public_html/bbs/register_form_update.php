...
$mb_tel         = isset($_POST['mb_hp'])           ? trim($_POST['mb_hp'])         : "";
...
if ($w == '') {
    ...
    if($name != $mb_name){
        un_kmc();
        alert("다시 시도해주세요.",G5_URL);
    }

    if(hyphen_hp_number($phoneNo) != $mb_hp){
        un_kmc();
        alert("다시 시도해주세요.",G5_URL);
    }
    ...
    if(!empty($ci)){
        ...
        if (get_session("ss_reg_mb_sex") != $mb_sex || get_session("ss_reg_mb_birth") != $mb_birth) {
            un_kmc();
            alert("다시 시도해주세요.",G5_URL);
        }
    }
