...
if ($stx) {
   // $stx = preg_replace('/\//', '\/', trim($stx));
	$sql="select * from g5_write_b_search_like where INSTR(wr_content,'{$_GET[stx]}')";
    ...
    $sql = " select gr_id, bo_table, bo_read_level from {$g5['board_table']} where bo_use_search = 1 and bo_list_level <= '{$member['mb_level']}' and gr_id='PRODUCT'";
    if ($gr_id)
        $sql .= " and gr_id = '{$gr_id}' ";
    ...
    $search_query = 'sfl='.urlencode($sfl).'&amp;stx='.urlencode($_GET[stx]).'&amp;sop='.$sop;
    ...
    for ($i=0; $i<count($s); $i++) {
		...
		if(substr($search_str,0,1)==" "){
			$search_str=substr($search_str,1,strlen($search_str));
		}
	...
	if($member[mb_level]==10){
		...
		$hiddenWhere=" and wr_6!='1'";
	}
    for ($i=0; $i<count($g5_search['tables']); $i++) {
        $tmp_write_table   = $g5['write_prefix'] . $g5_search['tables'][$i];
        $sql = " select wr_id from {$tmp_write_table} where {$sql_search} $hiddenWhere";
    ...
    for ($idx=$table_index; $idx<count($search_table); $idx++) {
        $sql = " select bo_subject, bo_mobile_subject,bo_gallery_width,bo_gallery_height from {$g5['board_table']} where bo_table = '{$search_table[$idx]}' ";

        $row = sql_fetch($sql);

		$bo_gallery_width = $row['bo_gallery_width'];
		$bo_gallery_height = $row['bo_gallery_height'];
        ...
        $sql = " select * from {$tmp_write_table} where {$sql_search} $hiddenWhere order by wr_id desc limit {$from_record}, {$rows} ";
        $result = sql_query($sql);
        for ($i=0; $row=sql_fetch_array($result); $i++) {
            // 검색어까지 링크되면 게시판 부하가 일어남
            $list[$idx][$i] = $row;
			$list[$idx][$i]['bo_gallery_width']=$bo_gallery_width;
			$list[$idx][$i]['bo_gallery_height']=$bo_gallery_height;
