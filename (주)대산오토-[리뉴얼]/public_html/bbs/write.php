...
if ($w == '') {
    ...
    if (strstr($write['wr_option'], 'secret')) {
        if ($write['mb_id']) {
            ...
            if (!($write['mb_id'] == $member['mb_id'] || $member[mb_level]==10))
...
$is_orderby = false;
if ($board['bo_orderby'] && $is_admin=="super"){
	$is_orderby = true;
}
...
if ($w == '') {
    ...
    $orderby = get_text(stripslashes($write['wr_orderby']));
...
if($_GET['wr_subject']&&$_GET['wr_1']&&$_GET['wr_2']){
	$content = "품번:".$_GET['wr_1']."<br/>";
	$content .= "품명(한글) : ".$_GET['wr_subject']."<br/>";
	$content .= "품명(영어) : ".$_GET['wr_2']."<br/>";
}
if($w=="r"){
	$content.="<div>[원본글]".get_text($write['wr_content'])."</div>--------------------------------------------------------------------------------------------------";
}
