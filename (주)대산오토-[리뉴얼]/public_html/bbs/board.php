...
if (isset($wr_id) && $wr_id) {
    ...
    if (($write['mb_id'] && $write['mb_id'] == $member['mb_id']) || $is_admin ) {
        ...
        if (strstr($write['wr_option'], "secret"))
        {
			...
			$is_owner=$member[mb_level]==10?true:$is_owner;
    ...
    if ($member['mb_level'] < $board['bo_list_level']) {
        if ($member['mb_id'])
			...
			alert('대산오토 내부 인트라넷 입니다.', G5_URL);
        else
            alert('대산오토 내부 인트라넷 입니다.\\n로그인 후 이용 가능합니다.', './login.php?'.$qstr.'&url='.urlencode(G5_BBS_URL.'/board.php?bo_table='.$bo_table.($qstr?'&amp;':'')));
    }
