...
if ($is_admin && ($member['mb_level_adm'] == "B2")) $is_checkbox = false;
?>
...
<? } ?>
    ...
    <div class="tbl_head01 tbl_wrap">
        ...
        <tr>
            ...
            <th scope="col">제목</th>
            <th scope="col">글쓴이</th>
            <th scope="col" class="hidden-xs"><?php echo subject_sort_link('wr_hit', $qstr2, 1) ?>상태</a></th>
        ...
        <tr class="<?php if ($list[$i]['is_notice']) echo "bo_notice"; ?>">
            ...
            <td class="td_subject">
                ...
                <p class="hidden-lg hidden-md hidden-sm" style="font-size:0.85em">
				<span style="color:#999 !important"><i class="fad fa-user-edit"></i>&nbsp;<?php echo $list[$i]['name'] ?></span>
                &nbsp;<i class="fal fa-calendar-alt"></i><span style="color:#777 !important"><?php echo $list[$i]['datetime'] ?></span>
                &nbsp;<i class="fal fa-eye"></i><span style="color:#777 !important"><?php echo $list[$i]['wr_hit'] ?></span>
                </span>
                <span class="<?php echo $list[$i]['wr_2']==""?"none":"compl";?>" onclick="setSuccess('<?=$list[$i]['wr_id']?>','<?=$list[$i]['wr_2']?>')">
				<?php echo $list[$i]['wr_2']==""?"미완료":$list[$i]['wr_2']?>
				</span>
                </p>
            </td>
            <td class="td_name hidden-xs sv_use"><?php echo $list[$i]['name'] ?></td>
            <td class="td_name hidden-xs sv_use">
				<span class="<?php echo $list[$i]['wr_2']==""?"none":"compl";?>" onclick="setSuccess('<?=$list[$i]['wr_id']?>','<?=$list[$i]['wr_2']?>')">
				<?php echo $list[$i]['wr_2']==""?"미완료":$list[$i]['wr_2']?>
				</span>
			</td>
...
<!-- 게시판 검색 시작 { -->
<fieldset id="bo_sch" class="hidden-xs">
    <legend>게시물 검색</legend>
    ...
    <select name="sfl" id="sfl">
        ...
        <option value="wr_name"<?php echo get_selected($sfl, 'wr_name'); ?>>글쓴이</option>
...
function setSuccess(wr_id,wr_2){
	var msg=wr_2==""?"완료로 전환하시겠습니까?":"미완료로 전환하시겠습니까?";
	var e_wr_2=wr_2==""?"완료":"";
	if(confirm(msg)){
		location.href=g5_bbs_url+"/intranet_update.php?bo_table=<?=$bo_table?>&wr_id="+wr_id+"&wr_2="+e_wr_2+"&page=<?=$page?>&sca=<?=$sca?>&sop=<?=$sop?>&sfl=<?=$sfl?>&stx=<?=$stx?>";
	}
}
</script>
<?php } ?>
