...
<? } ?>
    ...
    <div class="tbl_head01 tbl_wrap">
        ...
        <tr class="<?php if ($list[$i]['is_notice']) echo "bo_notice"; ?>">
            ...
            <?php } ?>
            <td class="td_subject">
               <textarea name="wr_content" id="wr_content"<?php echo $list[$i]['wr_id'] ?>" style="width:100%;" rows=3><?=$list[$i]['wr_content']?></textarea>
            </td>
            <td align="center"><a href="javascript:;" onclick="searchEdit('<?=$list[$i]['wr_id']?>','<?=$list[$i]['num']?>')">수정</a></td>
        </tr>
        <?php } ?>
...
<script type="text/javascript">
	function searchEdit(wr_id,no){
		if(confirm(no+"내용을 수정하시겠습니까?")){
			$.ajax({
				url:g5_bbs_url+"/list.form.update.php",
				data:{bo_table:"<?=$bo_table?>",wr_id:wr_id,wr_content:$("#wr_content").val()},
				dataType:"GET",
				type:"GET",
			...
			//location.href=g5_bbs_url+"/list.form.update.php?bo_table=<?=$bo_table?>&wr_id="+wr_id+"&wr_content="+$("#wr_content"+no).val();
		}
	}
</script>
<!-- 페이지 -->
