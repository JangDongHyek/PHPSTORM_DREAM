...
if( strcmp( $db_passwd, get_password_str($password) ) ){ // 두 변수를 비교하여 같으면 else{}, 다르면 if()를 실행함
	...
	if( headers_sent() ){
		...
		$url = str_replace( "!", "&", $url );
			...
			///////////////////////////////등급제////////////////////////////////////////
			if($username != "admin"){
				$sql = "select count(*) from order_buy where status='3' and id='$username'"; //총구매횟수
				$res = mysql_query( $sql, $dbconn );
				$tot_count = mysql_result($res,0,0);

				$sql2 = "select sum(mon_tot) from order_buy where status='3' and id='$username'"; //총구매금액
				$res2 = mysql_query( $sql2, $dbconn );
				$tot_price = mysql_result($res2,0,0);
				
				if($tot_count >= 100 || $tot_price >= 2000000){ //VIP회원
					$now_year = date("Y");
					$now_month = date("m");
					...
					$mem_grade = "4";
					echo"<script>alert('안녕하세요! {$MemberName}님은 VIP회원 회원입니다^^');</script>";
				}elseif($tot_count >= 60 || $tot_price >= 1000000){ //단골회원
					$now_year = date("Y");
					$now_month = date("m");
					$sql = "select * from mart_member_new where username='$username'";
					$res = mysql_query( $sql, $dbconn );
					...
					$mem_grade = "3";
					echo"<script>alert('안녕하세요! {$MemberName}님은 단골회원 회원입니다^^');</script>";
				}elseif($tot_count >= 20 || $tot_price >= 500000){ //일반회원
					$mem_grade = "2";
					echo"<script>alert('안녕하세요! {$MemberName}님은 일반회원 회원입니다^^');</script>";
				}else{ //새내기회원
					$mem_grade = "1";
					echo"<script>alert('안녕하세요! {$MemberName}님은 새내기 회원입니다^^');</script>";
				}
