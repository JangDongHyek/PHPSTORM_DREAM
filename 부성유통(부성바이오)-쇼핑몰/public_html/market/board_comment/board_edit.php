...
<script>
		...
		ed.init(); //웹에디터 삽입
</script>					<table width="96%" align="center" border="0" cellspacing="0" cellpadding="0">
						<tr>
							<td height="50" align="center"><input type='image' onfocus='blur();' src="../image/helpdesk/bu_modify.gif" border="0">&nbsp;<a href="board_list.php?item_no=<?=$item_no?>&mart_id=<?=$mart_id?>&bbs_no=<?=$bbs_no?>"><img src="../image/helpdesk/bu_cancel.gif" border="0"></a></td>
						</tr>
					</table>
					</form>
<!---------------------- 게시판 끝 ------------------------------------------------------>
...
<?
	...
	if( $userfile_name ){//첨부 파일을 업로드 했으면 파일명을 수정함
		...
		$sql = "update $New_BoardTable set userfile='$userfile_name' where index_no='$index_no' and mart_id='$mart_id'";
		$res = mysql_query( $sql, $dbconn );
	...
	if( $userfile1_name ){//첨부 파일을 업로드 했으면 파일명을 수정함
		...
		$sql = "update $New_BoardTable set userfile1='$userfile1_name' where index_no='$index_no' and mart_id='$mart_id'";
		$res = mysql_query( $sql, $dbconn );
	...
	$SQL = "update $New_BoardTable set writer='$writer', email='$email', subject_new='$subject_new_tmp[0]', content='$content', if_secret='$if_secret', point='$point' $notice_sql where index_no ='$index_no' and bbs_no='$bbs_no' and mart_id='$mart_id'";

	$dbresult = mysql_query($SQL, $dbconn);
	if ($dbresult == false) echo "쿼리 실행 실패!";

	echo "<meta http-equiv='refresh' content='0; URL=board_read.php?item_no=$item_no&mart_id=$mart_id&index_no=$index_no&bbs_no=$bbs_no&page=$page&keyset=$keyset&searchword=$searchword'>";
}
?>
<?
mysql_close($dbconn);
