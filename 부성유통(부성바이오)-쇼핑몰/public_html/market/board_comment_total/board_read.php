...
if($best_change){
	//커뮤니티 index 하단 베스트게시물
	$SQL = "update $New_BoardTable set best_chk='$best_change' where index_no = '$index_no' and bbs_no='$bbs_no' and mart_id='$mart_id'";
	$dbresult = mysql_query($SQL, $dbconn);
	if($best_change == 'y'){
			echo ("		
				<script>
				window.alert('베스트 게시물로 등록되었습니다.');
				</script>
			");
			...
			echo ("		
				<script>
				window.alert('베스트 게시물에서 삭제되었습니다.');
				</script>
			");
	}
...
if( $board_class == 1 || $board_class == 3 ){ //자신의 글 수정,삭제는 본인만
	if( $username == "$UnameSess" ){
		$mod_perm = "<a href='board_edit.php?mart_id=$mart_id&index_no=$index_no&bbs_no=$bbs_no&page=$page&keyset=$keyset&searchword=$searchword'><img src='../image/helpdesk/bu_modify.gif'  border='0'></a>";
	...
	$mod_perm = "<a href='board_edit.php?mart_id=$mart_id&index_no=$index_no&bbs_no=$bbs_no&page=$page&keyset=$keyset&searchword=$searchword'><img src='../image/helpdesk/bu_modify.gif' border='0'></a>";
	$del_perm = "<a href='board_delete.php?mart_id=$mart_id&flag=del&index_no=$index_no&bbs_no=$bbs_no&page=$page&keyset=$keyset&searchword=$searchword'><img src='../image/helpdesk/bu_delete.gif'  border='0'></a>";
}
...
?>
					<table width="680" align="center" border="0" cellspacing="0" cellpadding="0">
						<tr>
							...
							<td background="../image/helpdesk/table1_bg.gif"> &nbsp;&nbsp;<?=$subject_new?>
							</td>
					...
					<table width="680" align="center" border="0" cellspacing="0" cellpadding="0">
						<tr>
							<td height="50">
								...
								<a href='board_reply.php?item_no=<?=$item_no?>&mart_id=<?=$mart_id?>&index_no=<?=$index_no?>&bbs_no=<?=$bbs_no?>&page=<?=$page?>&keyset=<?=$keyset?>&searchword=<?=$searchword?>'><img src="../image/helpdesk/bu_reply.gif" border="0"></a>
								<?}?>							
								...
								?>
									...
									if($best_chk == 'y'){	
									?>
										<a href="board_read.php?best_change=n&mart_id=<?=$mart_id?>&bbs_no=<?=$bbs_no?>&index_no=<?=$index_no?>&page=<?=$page?>&keyset=<?=$keyset?>&searchword=<?=$searchword?>">[베스트게시물에서 삭제]</a>
									<?
									}else{
									?>
										<a href="board_read.php?best_change=y&mart_id=<?=$mart_id?>&bbs_no=<?=$bbs_no?>&index_no=<?=$index_no?>&page=<?=$page?>&keyset=<?=$keyset?>&searchword=<?=$searchword?>">[베스트게시물로 등록]</a>
									<?}?>
								<?}?>
							...
							<td align="right"><a href='board_list.php?mart_id=<?=$mart_id?>&bbs_no=<?=$bbs_no?>&page=<?=$page?>&keyset=<?=$keyset?>&searchword=<?=$searchword?>'><img src="../image/helpdesk/bu_list.gif" border="0"></a></td>
						</tr>
</table>
...
<input type="Hidden" name="searchword" value="<?=$searchword?>">
            	...
            	<tr>
                	...
                	<tr>
                		<td width="50" height="30"><div align="center"><img src="../image/helpdesk/add_name.gif"></div></td>
                		<td width="74"><input type=text name=cmt_name size=10 class="input_03" maxlength=20 value='<?=$MemberName?>' <?=$writer_readonly?>>
							</td>
                		<td width="60"><?if(!$Mall_Admin_ID&&$MemberLevel!=1){?><img src="../image/helpdesk/add_pass.gif"><?}?></td>
                		<td width="500"><?if(!$Mall_Admin_ID&&$MemberLevel!=1){?><input type=password name=cmt_password  size=12 class="input_03"><?}?>&nbsp;</td>
                	</tr>
                	</table>
            			<table width="680"  border="0" align="center" cellpadding="0" cellspacing="0">
                  	<tr>
                  		<td width="50"><div align="center"><img src="../image/helpdesk/add_contents.gif"></div></td>
                  		<td width="563">
								<textarea name=cmt_comment cols="83" rows="3" class="input_03"></textarea></td>
                 		  <td width="120" align="center">
							  <input type='image' onfocus='blur();' src="../image/helpdesk/bu_regist.gif" align="absmiddle" border="0">
							 </tr>
                 	</table></td>
           		</tr>
            	<tr>
