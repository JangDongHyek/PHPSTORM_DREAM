...
function really(order_num){
	if(confirm("\n정말로 삭제하시겟습니까?\n\n삭제하시면 현주문과 관련된 데이타가\n\n삭제되며 복구는 되지않습니다.")){
		window.location.href='order_detail2.php?status_flag=<?=$status_flag?>&page=<?=$page?>&keyset=<?=$keyset?>&searchword=<?=$searchword?>&cnfPagecount=<?=$cnfPagecount?>&QryFromDate=<?=$QryFromDate?>&QryToDate=<?=$QryToDate?>&update_flag=delete_data&order_num='+order_num;
		return true;
	}
	return false; 
}
					...
					?>	
									...
									?>
											...
											<?
												for($zz=0;$rows=mysql_fetch_array($dbresult);$zz++){
													if($pro_delivery == $rows[pro_delivery]){
														$checked_pd = "selected";
													}
											?>
												<option value="<?=$rows[pro_delivery]?>" <?=$checked_pd?>><?=$rows[pro_delivery]?></option>
											<?
												$checked_pd = "";
											}
											?>
									...
									?>
											...
											<?
												for($zz=0;$rows=mysql_fetch_array($dbresult);$zz++){
											?>
												<option value="<?=$rows[pro_delivery]?>"><?=$rows[pro_delivery]?></option>
											<?
											}
											?>
											</select>				
