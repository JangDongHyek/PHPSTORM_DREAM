...
function checkform(frm){
	...
	if (frm.op_name3.value==""){
	 ...
	 var item_explain = ed.getHtml(); //대체한 textarea에 작성한HTML값 전달
...
function opt_add(frm){
	...
	option.text=frm.optName.value+"(+"+opt_price+"원 재고: "+opt_ea+"개)";
...
function opt_modify(frm){
	...
	frm.opt1.options[k+1].text=opt_name[k]+"(+"+opt_price[k]+"원 재고 :"+opt_ea[k]+"개)";
...
function opt_add2(frm){
	...
	option.text=frm.optName2.value+"(+"+opt_price2+"원 재고: "+opt_ea2+"개)";
...
function opt_modify2(frm){
	...
	frm.opt2.options[k+1].text=opt_name2[k]+"(+"+opt_price2[k]+"원 재고 :"+opt_ea2[k]+"개)";
...
function opt_add3(frm){
	...
	option.text=frm.optName3.value+"(+"+opt_price3+"원 재고: "+opt_ea3+"개)";
...
function opt_modify3(frm){
	...
	frm.opt3.options[k+1].text=opt_name3[k]+"(+"+opt_price3[k]+"원 재고 :"+opt_ea3[k]+"개)";
...
function opt_add4(frm){
	...
	option.text=frm.optName4.value+"(+"+opt_price4+"원 재고: "+opt_ea4+"개)";
...
function opt_modify4(frm){
	...
	frm.opt4.options[k+1].text=opt_name4[k]+"(+"+opt_price4[k]+"원 재고 :"+opt_ea4[k]+"개)";
...
if($thumbnail == 'y'){//썸네일 사용시
	...
	for($i=0;$i<sizeof($opt_name)-1;$i++){
		$sql="insert into $OptionTable(item_no,opt_name,opt_price,opt_ea,opt_order) values('$item_no','$opt_name[$i]','$opt_price[$i]','$opt_ea[$i]','$opt_order[$i]')";
		$result=mysql_query($sql);
	}
	...
	for($i=0;$i<sizeof($opt_name2)-1;$i++){
		$sql="insert into $OptionTable2(item_no,opt_name,opt_price,opt_ea,opt_order) values('$item_no','$opt_name2[$i]','$opt_price2[$i]','$opt_ea2[$i]','$opt_order2[$i]')";
		$result=mysql_query($sql);
	}
	...
	for($i=0;$i<sizeof($opt_name3)-1;$i++){
		$sql="insert into $OptionTable3(item_no,opt_name,opt_price,opt_ea,opt_order) values('$item_no','$opt_name3[$i]','$opt_price3[$i]','$opt_ea3[$i]','$opt_order3[$i]')";
		$result=mysql_query($sql);
	}
	...
	for($i=0;$i<sizeof($opt_name4)-1;$i++){
		$sql="insert into $OptionTable4(item_no,opt_name,opt_price,opt_ea,opt_order) values('$item_no','$opt_name4[$i]','$opt_price4[$i]','$opt_ea4[$i]','$opt_order4[$i]')";
		$result=mysql_query($sql);
	}
