...
if($flag == "addorder"){
	...
	if($opt_ea2<$quantity&&$if_opt_jaego2&&$opt2){
		echo "
			<script>
			alert(\"$opt_name2 재고량을 초과하여 입력하셨습니다. $opt_ea2 이하로 입력하세요.\");
			history.go(-1);
			</script>
			";
		exit;
	...
	if($opt_ea3<$quantity&&$if_opt_jaego3&&$opt3){
		echo "
			<script>
			alert(\"$opt_name3 재고량을 초과하여 입력하셨습니다. $opt_ea3 이하로 입력하세요.\");
			history.go(-1);
			</script>
			";
		exit;
	...
	if($opt_ea4<$quantity&&$if_opt_jaego4&&$opt4){
		echo "
			<script>
			alert(\"$opt_name4 재고량을 초과하여 입력하셨습니다. $opt_ea4 이하로 입력하세요.\");
			history.go(-1);
			</script>
			";
		exit;
	}
	$SQL = "select order_pro_no, quantity from $Order_ProTable where order_num='$order_num' and mart_id='$mart_id' and item_no='$item_no' and opt='$opt' and opt2='$opt2' and opt3='$opt3' and opt4='$opt4' and status = '0'";
	$dbresult = mysql_query($SQL, $dbconn);
	$numRows = mysql_num_rows($dbresult);
	if ($numRows > 0) {
		...
		$SQL = "insert into $Order_ProTable (order_num, mart_id, item_no, item_name, item_code, opt, z_price, quantity, bonus, use_bonus, status, date, provider_id, provide_price,opt2,opt3,opt4,opt_price,opt_price2,opt_price3,opt_price4) values ('$order_num', '$mart_id', $item_no, '$item_name', '$item_code', '$opt', $z_price, $quantity, $bonus, $use_bonus, '0', '$cur_date', '$provider_id', '$provide_price','$opt2','$opt3','$opt4','$opt_price','$opt_price2','$opt_price3','$opt_price4')";	
		$dbresult = mysql_query($SQL, $dbconn);
	}
	if( $direct_submit_flag == "direct_submit" ){//바로 구매일때
		...
		$log_price = ($z_price * $quantity) + $opt_price + $opt_price2 + $opt_price3 + $opt_price4;
	?>
...
var _nasa={};
 _nasa["cnv"] = wcs.cnv("3","<?=$log_price?>"); // 전환유형, 전환가치 설정해야함. 설치매뉴얼 참고
</script> 
