...
if($update_flag == ''){
	...
	/***** 페이앱 세팅 STR *****/
	//$method = "<a href='http://pgweb.dacom.net/' target='_new'>[LG데이콤 카드결제현황]</a>";
	$method = "<a href='https://seller.payapp.kr/admin_member/admin_login.html' target='_new'>[페이앱 카드&계좌이체 결제현황]</a>";
	/***** 페이앱 세팅 END *****/
...
?>
							...
							<input type='hidden' name='status_old[]' value='<?=$good_status?>'>
								...
								<td  bgcolor='#FFFFFF'><a href='order_detail.php?index_no=<?=$ary[index_no]?>&page=<?=$page?>&keyset=<?=$keyset?>&searchword=<?=$searchword?>&flag=<?=$flag?>&cnfPagecount=<?=$cnfPagecount?>&QryFromDate=<?=$QryFromDate?>&QryToDate=<?=$QryToDate?>&status_flag=<?=$status_flag?>'><?=$order_num?></a></td>
			...
			<tr align="center">
				<td width="100%" bgcolor="#FFFFFF">
					...
					if($page == 1){
						...
						echo ("
						<a href='order_new.php?page=1&keyset=$keyset&searchword=$searchword&flag=$flag&cnfPagecount=$cnfPagecount&QryFromDate=$QryFromDate&QryToDate=$QryToDate&status_flag=$status_flag'>처음</a> 
						");
					}
				
					if($start_page > 1){
						echo ("
						<a href='order_new.php?page=$prev_start_page&keyset=$keyset&searchword=$searchword&flag=$flag&cnfPagecount=$cnfPagecount&QryFromDate=$QryFromDate&QryToDate=$QryToDate&status_flag=$status_flag'>
						◁&nbsp; 
						</a>
						");
					}
					...
					for($i=$start_page;$i<=$end_page;$i++){
						if($i == $page){
							...
							echo ("
						<a href='order_new.php?page=$i&keyset=$keyset&searchword=$searchword&flag=$flag&cnfPagecount=$cnfPagecount&QryFromDate=$QryFromDate&QryToDate=$QryToDate&status_flag=$status_flag'>$i</a> 
							");
						}
					}
					if($end_page < $total_page){
						echo ("
						<a href='order_new.php?page=$next_start_page&keyset=$keyset&searchword=$searchword&flag=$flag&cnfPagecount=$cnfPagecount&QryFromDate=$QryFromDate&QryToDate=$QryToDate&status_flag=$status_flag'>
						&nbsp;▷
						</a>
						");
					}
					...
					if($page == $total_page){
						...
						echo ("
						<a href='order_new.php?page=$total_page&keyset=$keyset&searchword=$searchword&flag=$flag&cnfPagecount=$cnfPagecount&QryFromDate=$QryFromDate&QryToDate=$QryToDate&status_flag=$status_flag'>끝</a> 
						");
					}
					?>
			  </td>
