...
<div id="bo_list" style="width:<?php echo $width; ?>">
    ...
    <div class="tbl_head01 tbl_wrap">
        ...
        <tr>
            ...
            <?php if ($is_checkbox) { ?>
            <th scope="col">
                <label for="chkall" class="sound_only">List all</label>
                <input type="checkbox" id="chkall" onclick="if (this.checked) all_checked(true); else all_checked(false);">
            </th>
            <?php } ?>
            <th scope="col">Company Profile and Checking Board</th>
            <th scope="col">COMPANY NAME</th>
            <th scope="col" class="hidden-xs"><?php echo subject_sort_link('wr_datetime', $qstr2, 1) ?>Date</a></th>
            <th scope="col" class="hidden-xs"><?php echo subject_sort_link('wr_hit', $qstr2, 1) ?>Hits</a></th>
            <?php if ($is_good) { ?><th scope="col"><?php echo subject_sort_link('wr_good', $qstr2, 1) ?>Recommended</a></th><?php } ?>
            <?php if ($is_nogood) { ?><th scope="col"><?php echo subject_sort_link('wr_nogood', $qstr2, 1) ?>Nonrecommended</a></th><?php } ?>
        </tr>
        </thead>
        <tbody>
        <?php
        ...
        <?php } ?>
        <?php if (count($list) == 0) { echo '<tr><td colspan="'.$colspan.'" class="empty_table">No postings found.</td></tr>'; } ?>
        </tbody>
        </table>
    </div>
    ...
    <div class="bo_fx">
        <?php if ($is_checkbox) { ?>
        <ul class="btn_bo_adm">
            <li><input type="submit" name="btn_submit" value="Delete Selection" onclick="document.pressed=this.value"></li>
            <li><input type="submit" name="btn_submit" value="Selective Copy" onclick="document.pressed=this.value"></li>
            <li><input type="submit" name="btn_submit" value="Selective Move" onclick="document.pressed=this.value"></li>
        </ul>
        <?php } ?>
        ...
        <ul class="btn_bo_user">
            ...
            <?php if ($write_href) { ?><li><a href="<?php echo $write_href ?>" class="btn_b02">Writer</a></li><?php } ?>
        </ul>
        <?php } ?>
    </div>
    <?php } ?>
...
<fieldset id="bo_sch">
    ...
    <select name="sfl" id="sfl">
        ...
        <option value="mb_id,1"<?php echo get_selected($sfl, 'mb_id,1'); ?>>Member ID</option>
