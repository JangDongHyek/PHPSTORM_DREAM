...
?>
            ...
            <td align="right">검색: 
              <select name="ss[0]" id="ss[0]">
                <?
	$ss_list = array("아이디","이름","핸드폰","닉네임","주민번호");
	while(list($key,$value)=each($ss_list))
    if ($key==$ss[0])
      echo "<option value='$key' selected>$value</option>";
    else
...
?>
            ...
            <td align="center" bgcolor="#FFFFFF"> 
              <?=$R[mb_log_count]?>            </td>
            <td align="center" bgcolor="#FFFFFF"><a href="member_edit.php?page=<?=$page?>&<?=$p_str?>&mode=edit&mb_num=<?=$R[mb_num]?>">수정</a></td>
