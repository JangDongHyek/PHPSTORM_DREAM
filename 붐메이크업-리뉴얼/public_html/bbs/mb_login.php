...
//	session_unregister("ss_login_ok");
	...
	if($act=='ok') {
		...
		if($ck_id=="true"){
			setcookie("ck_id_ok", 'ok', time()+86400*365, '/', NULL);
			setcookie("ck_mb_id", $login[mb_id], time()+86400*365, '/', NULL);
		}else{
			setcookie("ck_id_ok", "", time() - 3600,'/');
			setcookie("ck_mb_id", $login[mb_id], time()+86400*365, '/', NULL);
		}
		if($ck_auto=="true"){
			setcookie("ck_mb_id", $login[mb_id], time()+86400*365, '/', NULL);
			setcookie("ck_login_ok", 'ok', time()+86400*365, '/', NULL);
		}
