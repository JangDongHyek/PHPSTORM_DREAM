...
<script>
	...
	oEditors.getById["rg_content"].exec("UPDATE_CONTENTS_FIELD", []);	// 에디터의 내용이 textarea에 적용됩니다.

	var content=oEditors.getById["rg_content"].getIR();
	content=content.replace("<P>&nbsp;</P>","");
	if (content== "" ){ 
		 alert("내용을 입력하세요."); 
		 oEditors.getById["rg_content"].exec("FOCUS", []); 
		return false;
	}
	...
	function denyText(gubun){
		...
		var obj_titles="";
		var obj_conetents="";
		var obj_Content_arr=document.getElementById("rg_content").innerHTML.split(gubun);
			...
			<TD>
				<TABLE cellSpacing=1 cellPadding=1 width="100%" border=0>
					<TR> 
	                    ...
	                    <TD ><input name='rg_name' type=text id="rg_name" value='<?=$rg_name?>' maxlength=20 required itemname='이름' style=width:100%;height:22; class=b_input></TD>
					</TR>
				</TABLE>
			</TD>
		</TR>
...
<?=$show_email_begin?>
		<TR> 
			<TD>
				<TABLE cellSpacing=1 cellPadding=1 width="100%" border=0>
					<TR> 
						<TD width=131 align=right class="bbs">이메일 &nbsp;</TD>
						<TD align=left > <input name='rg_email' type=text style=width:100%;height:22; class=b_input id="rg_email" value='<?=$rg_email?>'  maxlength=100 email itemname='e-mail'></TD>
					</TR>
				</TABLE>
			</TD>
		</TR>
...
<textarea name="rg_content" id="rg_content" rows="10" cols="100" style="width:595px; height:412px; display:none;"><?=$rg_content?></textarea>
<!--textarea name="rg_content" id="rg_content" rows="10" cols="100" style="width:100%; height:412px; min-width:610px; display:none;"></textarea-->
<!--
<p>
...
nhn.husky.EZCreator.createInIFrame({
	...
	fOnAppLoad : function(){
		//예제 코드
		//oEditors.getById["rg_content"].exec("PASTE_HTML", ["로딩이 완료된 후에 본문에 삽입되는 text입니다."]);
	},
...
function pasteHTML() {
	...
	oEditors.getById["rg_content"].exec("PASTE_HTML", [sHTML]);
}

function showHTML() {
	var sHTML = oEditors.getById["rg_content"].getIR();
	alert(sHTML);
}
	
function submitContents(elClickedObj) {
	oEditors.getById["rg_content"].exec("UPDATE_CONTENTS_FIELD", []);	// 에디터의 내용이 textarea에 적용됩니다.
	alert(form.rg_content.value);
	
	// 에디터의 내용에 대한 값 검증은 이곳에서 document.getElementById("rg_content").value를 이용해서 처리하면 됩니다.
...
function setDefaultFont() {
	var sDefaultFont = '궁서';
	var nFontSize = 24;
	oEditors.getById["rg_content"].setDefaultFont(sDefaultFont, nFontSize);
}
</script>
<!--------------------------------------- 에디트 끝 ------------------------------------------------------>
...
<?=$show_upload_begin?>
		<TR> 
			<TD>
				<TABLE cellSpacing=1 cellPadding=1 width="100%" border=0>
					<TR>
                        <TD width=131 align=right class="bbs">첨부화일 #1<B>&nbsp;&nbsp; </B></TD>
		...
		<TR> 
			<TD>
				<TABLE cellSpacing=1 cellPadding=1 width="100%" border=0>
					<TR> 
					    <TD width=131 align=right class="bbs">첨부화일 #2<B>&nbsp;&nbsp; </B></TD>
					    <TD align=left class="bbs" > 
