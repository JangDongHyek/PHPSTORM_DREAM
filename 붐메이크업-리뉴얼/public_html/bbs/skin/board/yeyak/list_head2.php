...
$_SQL="select * from ".$_table_name."_body where rg_ext2>='$start_where' and rg_ext2<='$end_where'";
			...
			<table cellpadding="0" cellspacing="0" border="0" width=100%>
				<tr>
					...
					<td width="50%" align=right>
						<a href="list.php?bbs_id=<?=$bbs_id?>&year=<?=$prevyeary?>&month=<?=$pmonth?>&ss[sn]=<?=$ss[sn]?>&ss[sm]=<?=$ss[sm]?>&ss[kw]=<?=$ss[kw]?>"><img src=<?=$skin_board_url?>images/prev_year.gif border=0 align=absmiddle alt="<?=$prevyeary?>년 <?=$pmonth?>월 보기"></a>
							<a href="list.php?bbs_id=<?=$bbs_id?>&year=<?=$prevyear?>&month=<?=$prevmonth?>&ss[sn]=<?=$ss[sn]?>&ss[sm]=<?=$ss[sm]?>&ss[kw]=<?=$ss[kw]?>"><img src=<?=$skin_board_url?>images/prev_month.gif border=0 align=absmiddle alt="<?=$prevyear?>년 <?=$prevmonth?>월 보기"></a>
						<a href="list.php?bbs_id=<?=$bbs_id?>&year=<?=$thisyear?>&month=<?=$thismonth?>&ss[sn]=<?=$ss[sn]?>&ss[sm]=<?=$ss[sm]?>&ss[kw]=<?=$ss[kw]?>"><img src=<?=$skin_board_url?>images/this_month.gif border=0 align=absmiddle alt="<?=$thisyear?>년 <?=$thismonth?>월 보기"></a>
						<a href="list.php?bbs_id=<?=$bbs_id?>&year=<?=$nextyear?>&month=<?=$nextmonth?>&ss[sn]=<?=$ss[sn]?>&ss[sm]=<?=$ss[sm]?>&ss[kw]=<?=$ss[kw]?>"><img src=<?=$skin_board_url?>images/next_month.gif border=0 align=absmiddle alt="<?=$nextyear?>년 <?=$nextmonth?>월 보기"></a>
						<a href="list.php?bbs_id=<?=$bbs_id?>&year=<?=$nextyeary?>&month=<?=$pmonth?>&ss[sn]=<?=$ss[sn]?>&ss[sm]=<?=$ss[sm]?>&ss[kw]=<?=$ss[kw]?>"><img src=<?=$skin_board_url?>images/next_year.gif border=0 align=absmiddle alt="<?=$nextyeary?>년 <?=$pmonth?>월 보기"></a>
						  ...
						  <tr>
							...
							<?=$show_write_end?>
								...
								for($s=0;$s<sizeof($seong_data);$s++)	{
									$rg_ext2_arr=explode("-",$seong_data[$s][rg_ext2]);
									$rg2_date=mktime(0,0,0,$rg_ext2_arr[1],$rg_ext2_arr[2],$rg_ext2_arr[0]);
									if($rg2_date==$book)	{
