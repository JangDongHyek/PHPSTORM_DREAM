<?
	...
	if($act=='ok') {
		...
		if($bbs_id!="yeyak_list" && $rg_content=='') {
...
//		$rg_reply_mail 
		...
		if($bbs_id=="yeyak"){
			$sql = "select * from $bbs_table where rg_ext1='$rg_ext1' and rg_ext2='$rg_ext2' and rg_ext5='$rg_ext5'";
			$result = query($sql,$dbcon);
			$nrows = mysql_num_rows($result);
			if($nrows > 0){
				echo"<script>alert('같은시간에 예약이 있습니다 다른시간을 선택해주세요.');history.go(-1);</script>";
				exit;
			}
			$admin_orderby_query1.=",input_date";
			$admin_orderby_query2.=",'$input_date'";
			$rg_ext3=$rg_ext3_1."-".$rg_ext3_2."-".$rg_ext3_3;
		}

		if($bbs_id == "yeyak_list"){
			$rg_ext1_ex = explode("/",$rg_ext1);
			$rg_ext1 = $rg_ext1_ex[2]."-". $rg_ext1_ex[0]."-". $rg_ext1_ex[1];
			$rg_ext2_ex = explode("/",$rg_ext2);
			$rg_ext2 = $rg_ext2_ex[2]."-". $rg_ext2_ex[0]."-". $rg_ext2_ex[1];
			$rg_ext5=implode("-",$rg_ext5);
			$yeyak_list_query1=", `rg_ext6`, `rg_ext7`, `rg_ext8`, `rg_ext9`, `rg_ext10`, `rg_ext11`, `rg_ext12`, `rg_ext13`";
			$yeyak_list_query2=", '$rg_ext6', '$rg_ext7', '$rg_ext8', '$rg_ext9', '$rg_ext10', '$rg_ext11', '$rg_ext12', '$rg_ext13'";
		}
		...
		$dbqry="
			INSERT INTO `$bbs_table`
				( `rg_doc_num` , `rg_top_num` , `rg_parent_num` ,
					...
					`rg_ext3` , `rg_ext4` , `rg_ext5` $yeyak_list_query1 $admin_orderby_query1
				)
			VALUES 
				( '$rg_doc_num', '$rg_top_num', '$rg_parent_num',
					'$rg_sequence', '$rg_depth', '$rg_next_num', 
					...
					'$rg_vote_ip', '$rg_notice', '$rg_reply_mail', 
					'$rg_agree', '$rg_ext1', '$rg_ext2', 
					'$rg_ext3', '$rg_ext4', '$rg_ext5' $yeyak_list_query2 $admin_orderby_query2
				)
		";
		query($dbqry,$dbcon);
...
*/
		...
		if($bbs_id=="pro_hair"){
			rg_href("view.php?bbs_id=$bbs_id&doc_num=$rg_doc_num");
		}
		// 비밀글을 썻다면 글목록으로
		...
		switch($bbs[bbs_act_after]){
			case '' : 
			case '0' : 
					rg_href("list.php?$p_str&page=$page&bbs_id=$bbs_id&doc_num=$rg_doc_num&year=$year&month=$month");
					break;
			case '1' : 
