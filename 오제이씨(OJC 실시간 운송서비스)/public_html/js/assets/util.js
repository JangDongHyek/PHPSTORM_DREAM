...
function showLoadingBar() {    
    ...
    let mask = `<div id='mask'></div>`,
        ...
        loadingImg = `<div id='loadingImg' style='position: absolute; top: calc(50% + ${lodingTop}px); left: 50%; transform: translate(-50%, -50%); z-index:1051'>
                            <img src='${rootUrl}/img/loading.gif?animation=spin' style='width:80px; border-radius: 30%;'>
                      </div>`;
...
// 자동 콤마
function setComma($this){
    $this.val(comma(uncomma($this.val())));
}

// 자동 하이픈
function setHyphen($this){
    $this.val(telNoHypen(unHypen($this.val())));
}

// 자동 사업자번호 하이픈
function setBizNoHyphen($this){    
    $this.val(bizNoHyphen($this.val()));
}
