...
/* RFC 통신 공통함수 */
function getRfcData($subUrl, $data){        
    $url = RfcApiUrl.$subUrl.'?'.http_build_query($data, '', '&');
...
/* 로그인 세션 저장 */
function setMemberSession($mb_id, $auto_login = 'Y'){
    
    $time = time() + 86400 * 1; /* 하루 */
    
    if($auto_login == 'Y'){
        $time = (86400 * 365 * 50); /* 50년 */
    }
    
    $key = md5($mb_id);
    set_session('ss_mb_id', $mb_id);
    set_session('ss_mb_reg', $mb_id);
    set_session('ss_mb_key', $key);
    set_cookie('ck_mb_id', $mb_id, $time);
    set_cookie('ck_auto', $key, $time);
    set_cookie_app('mb_id', $mb_id, $time);
}

/* 전화번호 공통 하이픈 */
function telNoHyphen($str){
  $str = preg_replace('/[^0-9]/', '', $str);
  
  return preg_replace('/(^02.{0}|^01.{1}|^15.{2}|^16.{2}|^18.{2}|[0-9]{3})([0-9]+)([0-9]{4})/', '$1-$2-$3', $str);
}
...
function getPageLimit($page, $pagingCount){
    return ((int)$page - 1) * (int)$pagingCount;
}

/* date 날짜 포멧('y.m.d H:i') */
...
function getDateFormat2($date){
    return str_replace("-", ".", substr($date, 2, 14)); 
}
...
/* 한글 날짜 가져오기 */
function getKrDate($date){
    return date('Y년m월d일', strtotime($date)); /* 금일 날짜(한글) */
}

/* 기사 memberkey 가져오기 */
function getDeliveryInfo($delivery_mb_id){
...
/* dispatch 정보 가져오기 */
function getDispatchInfo($dispatch_idx){
    $sql = "
        SELECT
            *
        FROM
    ...
    $info = sql_fetch($sql);
    $info['product_string'] = json_decode($info['product_string'], true);
    $info['product_full_string'] = json_decode($info['product_full_string'], true);
    
    return $info;
}

/* 사인패드 정보 가져오기 */
function getSignPadUrl($dispatch_idx){
    $sql = "
        SELECT
            data_url
        FROM
    ...
    return sql_fetch($sql)['data_url'];
}


// 카카오 알림톡 발송
// type : 템플릿종류, params:템플릿변수, hp:휴대폰
function sendAlimTalk($type, $params=array(), $mb_hp, $dispatch_idx = 0) {
    ...
    $rep_field = array(
		...
		"month" => "#{월}", "day" => "#{일}", 
		"mainPct" => "#{대표상품}",
		"startTime" => "#{시1}", "endTime" => "#{시2}"
    );

    if ((int)$type > 1) return;	// 탬플릿 추가시 변경할것!!!

    // 1) 배송 출발 알림
	
	$template[0]['code'] = "SJB_074729";
    $template[0]['title'] = "배송 출발 알림";
	
	$template[0]['content']  = "[오제이씨_ 배송 출발 안내]\n";
	$template[0]['content'] .= "안녕하세요 #{담당자 이름} 고객님!\n";
	$template[0]['content'] .= "1%의 남다름을 제공하는 오제이씨입니다.\n";
	$template[0]['content'] .= "\n";
	$template[0]['content'] .= "주문하신 제품이 #{월}월 #{일}일 배송예정입니다.\n";
	$template[0]['content'] .= "\n";
	$template[0]['content'] .= "- 배송 정보 -\n";
	$template[0]['content'] .= "● 상품명 : #{대표상품}\n";
	$template[0]['content'] .= "● 배송 예정시간 : #{시1}시~#{시2}시\n";
	$template[0]['content'] .= "\n";
	$template[0]['content'] .= "항상 최고의 서비스를 위해 노력하는 OJC가 되겠습니다.";	

	
	$template[0]['btn_name'] = "배송 조회";
	$template[0]['btn_type'] = "WL";
	$template[0]['btn_url'] = "http://14.48.175.186/~ojc/app/nowmap.php?idx={$dispatch_idx}";	
	
	// 변수 치환
	foreach ($rep_field AS $key=>$field) {
		$val = $params[$key];
		$template[$type]['content'] = str_replace($field, $val, $template[$type]['content']);		
	}
	...
	$post_body['receiverTelNo'] = unHyphen($mb_hp);				//수신자전화번호
