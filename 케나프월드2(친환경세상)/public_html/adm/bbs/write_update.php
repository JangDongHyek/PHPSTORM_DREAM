...
if ($w == '' || $w == 'r') {
    ...
    $wr_id = sql_insert_id();
			...
			if($bo_table=="notice"){
				$sql="select * from g5_member where mb_1!=''";
				...
				for($j=0;$j<$total;$j++){
					...
					$fcmMessage=strip_tags($wr_content);
					$message=array(
								"message"=>$fcmMessage,
								"subject"=>$wr_subject,
   ...
   //     alert('올바른 방법으로 수정하여 주십시오.', G5_BBS_URL.'/board.php?bo_table='.$bo_table);
    ...
    sql_query($sql);
		if($bo_table=="as_request"&&$wr_6=="완료"){


			$sql="select * from fcm_member where mb_id = '$wr[mb_id]'";
			$result=sql_query($sql);
			$i=0;
			$tokens=array();
			...
			for($j=0;$j<$total;$j++){
				...
				$fcmMessage=strip_tags($wr_content);
				$message=array(
							"message"=>$wr[ca_name]."의 AS가 완료되었습니다..\n 송장번호:".$wr_7,
							"subject"=>$wr[ca_name]."의 AS가 완료되었습니다.",
							"goUrl"=>G5_BBS_URL."/board.php?bo_table=as_request&wr_id=".$wr[wr_id]
							);
				$fcm=sendFcm($tokens2,$message);
			}
		}
		if($bo_table=="as_request"&&$wr_10){
			$sql="select * from fcm_member where mb_id = '$wr[mb_id]'";
			$result=sql_query($sql);
			$i=0;
			$tokens=array();
			...
			for($j=0;$j<$total;$j++){
				...
				$message=array(
							"message"=>$wr[wr_10],
							"subject"=>$wr[wr_name]."님 ".$wr[ca_name]."의 답변이 왔습니다.",
							"goUrl"=>G5_BBS_URL."/board.php?bo_table=as_request&wr_id=".$wr[wr_id]
							);
				$fcm=sendFcm($tokens2,$message);
			}	
...
if ($file_upload_msg) {
	...
	if ($pageMode == 'write') { 
		goto_url(G5_URL.'/adm/bbs/write.php?bo_table='.$bo_table);
	} else {
		goto_url(G5_URL.'/adm/bbs/board.php?bo_table='.$bo_table.'&amp;wr_id='.$wr_id.$qstr);
	}
}
?>
