...
<div id="box_wrap">
    ...
    <div class="local_sch01">
        <select name="s_type" id="s_type" onchange="getMemberList(this.value)">
            <option value="99">전체회원</option>
            <? foreach ($member_group as $Key=>$val) { ?>
                <option value="<?=$Key?>"><?=$val?></option>
            <? } ?>
            <option value="150">가입날짜</option>
        </select>
        <input type="text" name="s_name" class="frm_input" placeholder="이름, 가입경로">
        <input type="text" name="start-date" id="start-date" class="frm_input" placeholder="검색 시작일">
        <span id="wave" style="display: none">~</span>
        <input type="text" name="end-date" id="end-date" class="frm_input" placeholder="검색 종료일">
    ...
    <div class="tbl_head01 tbl_wrap left">
        <table>
            ...
            <tr>
                ...
                <th>No.</th>
                <th style="display: none">회원No(idx)</th>
                <th>구분</th>
                <th>회원명</th>
                <th>연락처</th>
                <th>가입날짜</th>
...
<script>
    ...
    function getMemberList(group) {
        ...
        $.ajax({
            ...
            data : {
                "mb_group" : group,
                "mb_name" : document.querySelector('[name=s_name]').value,
                "start-date" : document.querySelector('[name=start-date]').value,
                "end-date" : document.querySelector('[name=end-date]').value
            },
            dataType : "html",
