...
<div id="sod_fin">
    ...
    <section class="sod_fin_list">
        ...
        <ul id="sod_list_inq" class="sod_list">
            ...
            for($i=0; $row=sql_fetch_array($result); $i++) {
                $image_width = 40;
                $image_height = 40;
                $image = get_it_image($row['it_id'], 40, 40, '', '', $row['it_name']);
            ...
            ?>
                ...
                <div class="li_total" style="padding-left:<?php echo $image_width + 10; ?>px;height:auto !important;height:<?php echo $image_height; ?>px;min-height:<?php echo $image_height; ?>px">
                    ...
                    <?php /*?><span class="total_point total_span"><span>적립포인트 </span><?php echo number_format($point); ?></span><?php */?>
                </div>
            <?php
                    $tot_point       += $point;
        ...
        <dl id="sod_bsk_tot">
            ...
            <dd class="sod_bsk_dvr"><strong><?php echo number_format($od['od_cart_price']); ?> P</strong></dd>

            <?php if($od['od_cart_coupon'] > 0) { ?>
            <dt class="sod_bsk_dvr">상품할인</dt>
            <dd class="sod_bsk_dvr"><strong><?php echo number_format($od['od_cart_coupon']); ?> P</strong></dd>
            <?php } ?>

            <?php if($od['od_coupon'] > 0) { ?>
            <dt class="sod_bsk_dvr">결제할인</dt>
            <dd class="sod_bsk_dvr"><strong><?php echo number_format($od['od_coupon']); ?> P</strong></dd>
            <?php } ?>

            <?php if ($od['od_send_cost'] > 0) { ?>
            <dt class="sod_bsk_dvr">배송비</dt>
            <dd class="sod_bsk_dvr"><strong><?php echo number_format($od['od_send_cost']); ?> P</strong></dd>
            <?php } ?>

            <?php if($od['od_send_coupon'] > 0) { ?>
            <dt class="sod_bsk_dvr">배송비할인</dt>
            <dd class="sod_bsk_dvr"><strong><?php echo number_format($od['od_send_coupon']); ?> P</strong></dd>
            <?php } ?>

            <?php if ($od['od_send_cost2'] > 0) { ?>
            <dt class="sod_bsk_dvr">추가배송비</dt>
            <dd class="sod_bsk_dvr"><strong><?php echo number_format($od['od_send_cost2']); ?> P</strong></dd>
            <?php } ?>

            <?php if ($od['od_cancel_price'] > 0) { ?>
            <dt class="sod_bsk_dvr">취소금액</dt>
            <dd class="sod_bsk_dvr"><strong><?php echo number_format($od['od_cancel_price']); ?> P</strong></dd>
            <?php } ?>

            <dt class="sod_bsk_cnt">총계</dt>
            <dd class="sod_bsk_cnt"><strong><?php echo number_format($tot_price); ?> P</strong></dd>
    ...
    <div id="sod_fin_view">
        ...
        if($od['od_receipt_price'] > 0)
            $od_receipt_price = display_price($od['od_receipt_point']);
        else
            $od_receipt_price =  display_price($od['od_receipt_point']);
