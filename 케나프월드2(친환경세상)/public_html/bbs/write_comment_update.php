...
for ($i=0; $i<count($_FILES['bf_file']['name']); $i++) {
    ...
    if (isset($_POST['bf_file_del'][$i]) && $_POST['bf_file_del'][$i]) {
        ...
        $row = sql_fetch(" select bf_file from {$g5['board_file_table']} where bo_table = '{$bo_table}' and wr_id = '{$comment_id}' and bf_no = '{$i}' ");
...
goto_url(G5_HTTP_BBS_URL.'/board.php?bo_table='.$bo_table.'&amp;wr_id='.$wr['wr_parent'].'&amp;'.$qstr);
?>
