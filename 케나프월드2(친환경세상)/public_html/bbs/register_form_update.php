...
$mb_password    = trim($_POST['mb_password'])		? trim($_POST['mb_password'])	 : "1234";
$mb_password_re = trim($_POST['mb_password_re']);
$mb_name        = trim($_POST['mb_name']);
$mb_nick        = trim($_POST['mb_nick'])			? trim($_POST['mb_nick'])		 : "nick".mktime();
...
if($mb_id!="01090132358"){
	$mb_recommend   = trim($_POST['mb_recommend'])&&$_POST['mb_recommend']!="없음"      ? trim($_POST['mb_recommend'])   : "없음";
}else{
	$mb_recommend   = trim($_POST['mb_recommend'])&&$_POST['mb_recommend']!="없음"      ? trim($_POST['mb_recommend'])   : "01000000000";
}
...
*/
		...
		if($mb_recommend != "없음"){
			if ($config['cf_use_recommend'] && $mb_recommend) {
				...
				$sql = " select mb_id from {$g5['member_table']} where mb_id = '{$mb_recommend}'";
				$row = sql_fetch($sql);
				if (!$row['mb_id']) {
...
if ($w == '') {
    ...
    // 회원가입 포인트 부여
    //insert_point($mb_id, $config['cf_register_point']*0.5, '회원가입 축하', '@member', $mb_id, '회원가입');

	//회원가입 L포인트 부여
    /* 240527 회원가입 포인트삭제 wc
    $registerPoint = 10000;
...
*/
    ...
    //FIXME
	alert("회원가입에 성공했습니다. 청원서 작성을 하시면 주식 5주가 무료로 지급됩니다.", G5_BBS_URL."/petition.php");
