...
if (G5_IS_MOBILE) {
    ...
    $category_skin_path = get_skin_path('category', "theme/basic");
    $category_skin_url  = get_skin_url('category', "theme/basic");
...
//날짜별로 년도 주차 나타나기
$currentYoil=date("w",strtotime(date("Y-m-d")));
$currentWeekDay=date("Y-m-d",strtotime(date("Y-m-d")." -".$currentYoil." day"));

$currentTurn=date("Y/W",strtotime($currentWeekDay)+86400);

$prevTurn=date("Y/W",strtotime($currentWeekDay." -1 week"));
/**
$currentWeek=date("Y/W",strtotime($currentWeekDay));
$prevWeek=date("Y/W",strtotime($currentWeekDay." -1 week"));
**/


//로또 회차 구하기
$lastDay=date("Y-m-t");
$lastYoil=date("w",strtotime($lastDay))+1;
$lastYoil=$lastYoil==7?0:$lastYoil;
$lastSaturday=date("Y-m-d",strtotime($lastDay." - ".$lastYoil." day"));
//로또 이번달 마지막 주 지나면 회차가 올라가게
$lastSaturdaySec=strtotime($lastSaturday);

$currentSec=time();

//echo $currentSec."<=".$lastSaturdaySec;
/*
$currentTurn=$currentSec<=$lastSaturdaySec?date("Y/m"):date("Y/m",strtotime($lastDay." +1 month"));
//지난회차 구하기
$prevTurn=date("Y/m",strtotime($currentTurn."/01 -1 month"));*/

$pointSaveArr=array("1"=>"0.01","2"=>"0.01","3"=>"0.01","4"=>"0.01",
										"5"=>"0.01","6"=>"0.01","7"=>"0.01","8"=>"0.01",
										"9"=>"0.01","10"=>"0.01","11"=>"0.01","12"=>"0.01",
										"13"=>"0.01","14"=>"0.01","15"=>"0.01");

define('SMS_SEND_NUM', '1833-5633');
#define('SMS_SEND_NUM', '051-891-0088');

// 회원가입
// - 가입구분
$member_group = array(2=>"일반회원", 10=>"관리자");
