...
<div class="tbl_head01 tbl_wrap">
    ...
    <tr class="<?php echo $bg; ?>">
        ...
        <td class="td_num td_pt" style="color: <?=($row['po_point'] > 0) ? 'red' : 'blue'?>"><?php echo number_format($row['po_point']) ?></td>
...
function excel_upload() {
    ...
    $.ajax({
        url: g5_admin_url+"/excel_point_read.php",
        type: "POST",
        data: formData,
        processData : false,
        contentType : false,
        success: function(data) {
            if (data == "fail"){
                alert("엑셀로 등록실패했습니다. 새로고침 후 다시 시도해주세요.")
            }else{
                alert("엑셀로 등록완료되었습니다.");
