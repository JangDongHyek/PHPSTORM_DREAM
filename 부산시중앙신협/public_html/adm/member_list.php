...
<div class="local_ov01 local_ov">
    <a href="?sst=mb_leave_date&amp;sod=desc&amp;sfl=<?php echo $sfl ?>&amp;stx=<?php echo $stx ?>">탈퇴회원목록</a>
    <a href="?sst=basic_mem&amp;sod=desc&amp;sfl=<?php echo $sfl ?>&amp;stx=<?php echo $stx ?>">정상회원목록</a>
    <?php echo $listall ?>
    총회원수 <?php echo number_format($total_count) ?>명 중,
    차단 <?php echo number_format($intercept_count) ?>명,
    <?php if ($sst != "basic_mem"){ ?>
    탈퇴 <?php echo number_format($leave_count) ?>명
    <?php } ?>
</div>
...
<select name="sfl" id="sfl">
    ...
    <option value="mb_name"<?php echo get_selected($_GET['sfl'], "mb_name"); ?> selected>이름</option>
...
<select name="level" id="mb_level">
    <option <?php if ($_GET['level'] == '' ) echo "selected" ?>  value="">회원등급</option>
    <?php for ($i = 1; $i <= count($level_arr); $i++){ ?>
        <option <?php if ($_GET['level'] == $i+1) echo "selected" ?>  value="<?=$i+1?>"><?=$level_arr[$i]?></option>
    <?php } ?>
</select>
<input type="submit" class="btn_submit" value="검색">
<p>* 휴대폰 검색 시 -(하이픈)을 포함하여 검색해주세요.</p>
...
<div class="btn_add01 btn_add">
    ...
    <a href="./member_form.php?mb_level=9" id="member_add">골프예약 관리자추가</a>
...
<!--        </th>-->
        ...
        <th>등급</th>
        <th><?php echo subject_sort_link('mb_id') ?>조합원번호</th>
	...
	*/ ?>
    ...
    for ($i=0; $row=sql_fetch_array($result); $i++) {
		...
		$point = '<a href="./point_list.php?sfl=mb_id&stx='.$row[mb_id].'">'.number_format($row[mb_point]).'<br>(관리)</a>';
...
<!--		</td>-->
        ...
        <td><?=$mb_id?></td>
        <td style="width: 10%">
            <?php if ($row['mb_level'] < 9){ ?>
            <select onchange="level_change('<?=$row['mb_id']?>',this.value)" id="mb_level">
                <?php for ($i = 1; $i <= count($level_arr); $i++){
                    //보조관리자
                    $select = "";
                    if ($row['mb_level'] == 8){
                        $select = "selected";
                    }elseif($row['mb_level'] == $i+1) {
                        $select = "selected";
                    }
                  ?>
                    <option <?= $select ?>  value="<?=$i+1?>"><?=$level_arr[$i]?></option>
                <?php } ?>
            </select>
            <?php }else{ ?>
                최고관리자
		...
		<td><?=$row['mb_hp']?></td>
		<td><?php if ($row['mb_year'] != "") echo $row['mb_year']."/". $row['mb_month']."/". $row['mb_day']?></td>
		<td><?=$row['mb_2']?></td>
		<td><?=substr($row['mb_datetime'],2,8)?></td>
		<td><?= ($row['mb_leave_date'] != "") ? date("y-m-d",strtotime($row['mb_leave_date'])) : ""?></td>
		<td><?=$row['mb_leave_comment']?></td>
...
<?php echo get_paging(G5_IS_MOBILE ? $config['cf_mobile_pages'] : $config['cf_write_pages'], $page, $total_page, '?'.$qstr.'&level='.$_REQUEST["level"].'&amp;page='); ?>
...
function excel_down() {
    ...
    location.href = g5_admin_url + '/new_excel_download.php?lv=<?=$_GET["level"]?>&sfl='+sfl+'&stx='+stx+'&sst='+'<?=$sst?>';
}
</script>
