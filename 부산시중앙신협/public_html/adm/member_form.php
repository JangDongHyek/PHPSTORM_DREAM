...
$mb_level = ($_REQUEST["mb_level"] == "") ? $mb["mb_level"] :  $_REQUEST["mb_level"];
...
?>
<script src="<?php echo G5_JS_URL ?>/printThis.js"></script><!-- alert -->
...
<div class="tbl_frm01 tbl_wrap">
    ...
    <?php if ($mb_level != 9){ ?>
		...
		<tr>
			<th scope="row"><label for="mb_1">조합원번호<strong class="sound_only">필수</strong></label></th>
			<td><input type="text" name="mb_1" value="<?php echo $mb['mb_1'] ?>" id="mb_1" maxlength="100"  class=" frm_input" size="30"></td>
			<th scope="row"><label for="mb_2">관리지점<strong class="sound_only">필수</strong></label></th>
			<td><input type="text" name="mb_2" value="<?php echo $mb['mb_2'] ?>" id="mb_2" maxlength="100"  class=" frm_input" size="30"></td>
		</tr>
		<tr>
			<th scope="row"><label for="user_qrcode">화면용 QR코드<strong class="sound_only">필수</strong></label></th>
			<td><img src='<?=G5_URL?>/adm/user_qrcode.php?mb_id=<?=$mb['mb_id']?>' alt=''></td>
			<th scope="row"><label for="user_qrcode">출력용 QR코드<strong class="sound_only">필수</strong></label></th>
			<td><img style="width:30px;height:30px;" id="user_qrcode"src='<?=G5_URL?>/adm/user_qrcode.php?mb_id=<?=$mb['mb_id']?>' alt=''> <input type="button" onclick="onclick_print()" value="QR코드 출력하기"></td>
		</tr>
		<tr>

			<th scope="row" ><label for="mb_hp">관리자 메모</label></th>
			<td colspan="3"><textarea style="width: 100%" name="mb_3"><?php echo $mb['mb_3'] ?></textarea></td>
		</tr>
	...
	*/ ?>
    ...
    <tr>
        <th scope="row"><label for="mb_hp">등급</label></th>
        <td>
            <select name="mb_level" id="mb_level">
                <?php for ($i = 1; $i <= count($level_arr); $i++){ ?>
                    <option <?php if ($mb['mb_level'] == $i+1) echo "selected" ?>  value="<?=$i+1?>"><?=$level_arr[$i]?></option>
                <?php } ?>
				<option <?php if ($mb['mb_level'] == 10) echo "selected" ?>  value="10">운영자</option>
            </select>
        </td>
