...
for ($i=0; $i<count($_FILES['bf_file']['name']); $i++) {
    ...
    if ($w=="u" && $_FILES['bf_file']['name'][$i] != ""){
        $_POST['bf_file_del'][$i] = true;
    }
...
//수강생리스트 추가
if ($wr_10 == "Y") {
    for ($i = 0; $i < count($_REQUEST["member_arr"]); $i++) {
        $sql = "insert into new_cucenter_member set cm_wr_id = '{$wr_id}', mb_id = '{$_REQUEST["member_arr"][$i]}', wr_datetime = '" . G5_TIME_YMDHIS . "' ";
        sql_query($sql);
    }

    $del_idx = explode(',', $_POST['del_arr']);

    for($i = 0; $i < count($del_idx); $i++) {
        $sql = " delete from new_cucenter_member where cm_wr_id = '{$wr_id}' and mb_id = '{$del_idx[$i]}' ";
        sql_query($sql);
    }
...
if ($file_upload_msg)
    alert($file_upload_msg, G5_HTTP_BBS_URL.'/culture_write.php'.$qstr);
else
    alert("완료되었습니다.",G5_ADMIN_URL.'/culture_write.php?w=u&wr_id='.$wr_id."&".$qstr);
?>
