...
if (!$is_member){
    alert("서비스 준비중 입니다.",G5_BBS_URL."/login.php?url=".$_SERVER["REQUEST_URI"]);
}
?>
...
<div id="bo_gall" style="width:<?php echo $width; ?>">
	...
	<div class="serachBoxArea">
		<ul>
			<!-- <li><select>
				<option value="0">강좌분류</option>
				<option value="0">캘리그라피</option>
			</select></li>
			<li><select>
				<option value="0">강좌분류</option>
				<option value="0">캘리그라피</option>
    ...
    <ul id="gall_ul">
        <?php /*?><?php for ($i=0; $i<count($list); $i++) {
         ...
         <?php for ($i=0; $i<count($list); $i++) {
             ...
             if ($list[$i]["wr_proc"] == 3 || G5_TIME_YMD > $list[$i]["wr_4"]) {
                 $text = "접수마감";
             } elseif (G5_TIME_YMD < $list[$i]["wr_3"]) {
                 $text = "접수대기기간";
             } elseif (G5_TIME_YMD >= $list[$i]["wr_3"]) {
                 $text = "접수중";
             }

             $sql = "select count(*) cnt from new_enrolment where wr_id = '{$list[$i]["wr_id"]}' ";
             $now_cnt = sql_fetch($sql)["cnt"];
            //접수인원이랑 등록한 수강인원 같을 경우 대기로 바꿔줌(접수인원 지정 안했을 경우)
             if ($now_cnt >= $list[$i]["wr_9"] && $text != "접수마감"  && $list[$i]["wr_9"] != 0 ){
                 $text = "접수대기";
             }
             ...
             $type2 = ($list[$i]["wr_10"] == 'Y' || $list[$i]["wr_10"] == 'M') ? "-재수강" : "";
        ...
        <li class="gall_li <?php if ($wr_id == $list[$i]['wr_id']) { ?>gall_now<?php } ?>" >
            ...
            <ul class="gall_con">
                ...
                <li class="gall_text_href">
					<div class="cate_sts">
						<span class="s_stat ing"><?=$text?></span><span class="s_c"><?=$type?>강좌<?=$type2?></span>
					</div>
                    <?php
					...
					<a href="<?php echo $list[$i]['href'] ?>" class="title">
						<p class="list_title"><?php echo $list[$i]['subject'] ?></p>
						<p class="list_s_txt"><?php echo $list[$i]['wr_7'] ?></p>
						<div class="box">
							<span>수강기간 <?= $list[$i]['wr_1']." ~ ". $list[$i]['wr_2']  ?></span>
							<span><?=$list[$i]['wr_5']?></span>
							<span class="price"><?=number_format($list[$i]['wr_6'])?> 원</span>
						</div>
					</a>
                    <?php if ($text == "접수중" || $text == "접수대기"){ ?>
                    <a href="<?php echo G5_BBS_URL ?>/board.php?bo_table=cucenter&wr_id=<?=$list[$i]['wr_id']?>" class="counsel"><img src="<?php echo $board_skin_url ?>/img/ic_click.svg" style="width:25px;height:25px;object-fit:contain;opacity:0.4;margin-right:5px;"> 수강신청</a>
                    <?php } ?>
                </li>
