...
if ($_SESSION['ss_mb_id']) { // 로그인중이라면
    ...
    } else if(!empty($ss_mb_id)) {
		set_session('ss_mb_id', $ss_mb_id);
		echo "<script type='text/javascript'> window.location.reload(); </script>";
        exit;
	}
    // 자동로그인 end ---------------------------------------
...
//CSS 버전
define('G5_CSS_VER', '2201106');
define('G5_JS_VER',  '2201106');
...
$is_app = false;
if(preg_match("/dreamforone/i", $_SERVER['HTTP_USER_AGENT'])) {
	$is_app = true;
}
...
// 카카오톡 보내기
define('WIDESHOT_KEY', 'ZFY3QmdvYlk5Z1hhTDNIS2VwTitmMlExVHloUm0ybG9JTGFjanViM0N6djVIRStwdjRvTlBRU3F4c1NEZ1Zrag==');
define('WIDESHOT_SERVER', 'https://apimsg.wideshot.co.kr');
define('WIDESHOT_CH_KEY', 'dd5c1b1a0f5bd556d258541ba377e05e23f2e2fc');	// 카카오 프로필키(채널키)
define('WIDESHOT_PULSFRIEND', '@부산시중앙신협');	// 플러스친구명
