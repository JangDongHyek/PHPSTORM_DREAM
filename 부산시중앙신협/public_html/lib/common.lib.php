...
function goSms($reserv_phone, $send_phone, $msg)
{
    ...
    $tran_callback1 = "0516111255";			// 발신번호
    $msg1 = $msg;							// 문자내용
...
function sendAlimTalk($type, $params=array(), $mb_hp, $mb_no="") {
	...
	$template = array();
	$rep_field = array(
		"name" => "#{이름}", "class_name" => "#{강좌명}", "class_cost"=>"#{강좌별 수강료}"
	);
	$od_id = 0;

	// 수강신청 알림
	$template[0]['code'] = "SJB_073925";
	$template[0]['title'] = "수강신청 알림";
	$template[0]['content'] = "#{이름}님, #{강좌명} 수강 신청되었습니다.\n";
	$template[0]['content'] .= "수강료를 지점 방문 납부 혹은 계좌 이체 해주셔야 확정됩니다.\n";
	$template[0]['content'] .= "\n";
	$template[0]['content'] .= "▶계좌 : 신협 131-008-781691\n";
	$template[0]['content'] .= "예금주 부산시중앙신협\n";
	$template[0]['content'] .= "▶금액 : #{강좌별 수강료}\n";
	$template[0]['content'] .= "▶입금 시 적요 : 성함(강좌명)\n";
	$template[0]['content'] .= "예시 : 김신협(노래교실A)\n";
	$template[0]['content'] .= "\n";
	$template[0]['content'] .= "감사합니다.\n";
