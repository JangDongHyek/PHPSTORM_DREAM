...
$mb_year       = isset($_POST['mb_year'])         ? trim($_POST['mb_year'])       : "";
$mb_month       = isset($_POST['mb_month'])         ? trim($_POST['mb_month'])       : "";
$mb_day       = isset($_POST['mb_day'])         ? trim($_POST['mb_day'])       : "";
...
$mb_leave_comment       = isset($_POST['mb_leave_comment'])         ? trim($_POST['mb_leave_comment'])       : "";
$mb_leave_comment       = clean_xss_tags($mb_leave_comment);
...
//    insert_point($mb_id, $config['cf_register_point'], '회원가입 축하', '@member', $mb_id, '회원가입');
    ...
    $sql = " update {$g5['member_table']}
                set mb_nick = '{$mb_nick}',
                    ...
                    mb_leave_comment = '{$mb_leave_comment}',
...
alert("회원가입이 완료되었습니다.", G5_URL);
} else if ($w == 'u') {

    if($_POST['mb_leave_date']){
        alert('회원 탈퇴완료', G5_BBS_URL . '/logout.php', false);
    }else{
        //
    }
    ...
    if ($old_email != $mb_email && $config['cf_use_email_certify']) {
        ...
        <title>회원정보수정</title>
        <body>
        <form name="fregisterupdate" method="post" action="'.G5_HTTP_BBS_URL.'/modify_info.php">
