...
//	for($i=0;$i < count($ss_key);$i++) {
	...
	if($auth[bbs_download]) {
		...
		$u_file3 = "{$site_url}download.php?$p_str&bbs_id=$bbs_id&page=$page&type=3&doc_num=";
		$a_file3 = "<a href=\"$u_file3$doc_num\" $class[link_bbs]>";
		$u_file4 = "{$site_url}download.php?$p_str&bbs_id=$bbs_id&page=$page&type=4&doc_num=";
		$a_file4 = "<a href=\"$u_file4$doc_num\" $class[link_bbs]>";
	} else {
		$u_file1 = "";
		$a_file1 = "<RG--";
		$u_file2 = "";
