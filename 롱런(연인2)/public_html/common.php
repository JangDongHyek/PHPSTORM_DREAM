...
if ($_REQUEST['app_mb_id'] != "") {
    $mb = get_member($_REQUEST['app_mb_id'], 'mb_id, mb_datetime');
    if ($mb) {
        // 회원아이디 세션 생성
...
롱런 (연인2)
 **********************************************/
define('MB_IMG_PATH', G5_DATA_PATH."/mb");
define('MB_IMG_URL', G5_DATA_URL."/mb");
define('COMMON_SEND_NUM', "01082600260");		// 문자발신번호
define('G5_CSS_VER', 1.12);
define('G5_JS_VER', 1.21);
define('BBS_IMG_PATH', G5_DATA_PATH."/bbs");
define('BBS_IMG_URL', G5_DATA_URL."/bbs");

// KCB본인확인 회원사코드
define('KCB_CP_ID', "V44490000000");
define('KCB_SITE_NAME', "연인");


// 시/도 배열
$city_arr = array("강원", "경기", "경남", "경북", "광주", "대구", "대전", "부산", "서울", "세종", "울산", "인천", "전남", "전북", "제주", "충남", "충북");

// 회원프로필
// - 혈액형
$blood_arr = array("A", "B", "O", "AB");

// - 흡연
$smoking_arr = array("비흡연", "궐련형", "액상담배", "연초");

// - 음주
$drinking_arr = array("비음주", "가끔", "보통", "자주");

// - 종교 (직접입력포함 1)
$religion_arr = array("기독교", "불교", "천주교", "무교", "직접입력");

// - 학력
$edu_arr = array("초졸", "중졸", "고졸", "대졸", "대학원");

// - 직업 (직접입력포함 2)
$job_arr = array("학생", "전문직", "의료직", "교육직", "공무원", "사업가", "금융직", "연구,기술직", "군인", "무직", "직접입력");

// - 성격 (직접입력포함 3)
$char_arr = array("다정다감", "활발", "애교", "4차원", "매력있는", "쾌활", "엉뚱", "도도", "착한", "순수", "유머", "성실", "매너", "직접입력");

// - 체형 (직접입력포함 4)
$body_arr = array("마른", "날씬한", "글래머한", "근육질", "슬림근육", "보통", "통통", "우람한", "탄탄한", "직접입력");

// - 취미 (직접입력포함 5)
$hobby_arr = array("드라이브", "영화보기", "카페가기", "게임하기", "운동하기", "치맥하기", "산책하기", "야구보기", "독서하기", "음악감상", "직접입력");

// 소개종류
// (수정시 관리자-정산현황, 정산현황건별팝업 배열카운팅 수정할것)
// 완열 - 상단에 array_pop($match_type_arr); 확인할것
$match_type_arr = array("계좌결제소개", "폰&카드소개", "쿠폰소개", "하트소개", "비고");

// - 이상형등록 (직접입력포함 6)
$ideal_type_arr = array('해바라기', '다정한', '집착없는', '예의 바른', '존중심있는', '배려심있는', '말 예쁜', '자기 관리하는', '매력적인', '리드 쉽 있는', '대화를 잘하는', '리액션 좋은', '내면을 중시하는', '이해심이 많은', '듬직한', '진정성 있는', '유머감각 있는', '재치있는', '밥 잘먹는', '자상한', '맞춤법 잘지키는', '밝고 활발한', '지적인 스마트한', '발랄한 애교있는', '조용하고 과묵한', '적극적인', '경청을 잘하는', '없음', '직접입력');


// 삭제된 카운슬러 체크
if ($member['mb_status'] == "카운슬러" && $member['mb_3'] == "out") {
    session_unset();
    session_destroy();
    alert("삭제된 카운슬러입니다.", G5_BBS_URL."/logout.php");
}

// 가입경로
define("MEMBER_JOIN_PATH", [1=>"페이스북&인스타그램", 2=>'네이버 카페', 3=>'네이버 블로그', 4=>'구글', 5=>'유튜브', 6=>'지인추천', 7=>'스토어 검색', 0=>'기타']);
...
$app_type = "WEB";
$inapp_vercode = 1;
if (strpos($_SERVER['HTTP_USER_AGENT'], "ALongRunApp") !== false) {
    // 앱버전확인
    $_tmp = explode("/APP_VER=", $_SERVER['HTTP_USER_AGENT']);
    $inapp_vercode = (int)$_tmp[1];
    $app_type = "AOS";
} else if (strpos($_SERVER['HTTP_USER_AGENT'], "ILongRunApp") !== false) {
    $app_type = "IOS";
}

// 나이스 모듈
define('NICE_CODE', 'BZ676');
define('NICE_PASSWORD', 'LraLCc81t0bL');
define('NICE_PATH', "/home/CPClient/CPClient");

// 롱런 모바일웹 도메인 체크 (ios등록불가로 웹처리)
// www.lovelongrun.com
$long_run_web = strpos($_SERVER['HTTP_HOST'], 'lovelongrun.com') !== false;

// 롱런 페이스북픽셀 액세스토큰
const FB_PIXEL_TOKEN = "EAAJHR4zV2u4BAJRuNgz51dwqADSNZAu3mzjKsbyBuw2Ecmmus0eJTlDdyH9kg14PexZCD4YyEeZB84WFtGrlxT8NmU5vmtvKQHRZAHBZCF2TXXEMlrgDslZCQS23HY1qreZAdhIJcjWKuwxP5nfJnQNa1aRQNDK50iDUEUAnr7ghAdFqXOLZAWRJ";
