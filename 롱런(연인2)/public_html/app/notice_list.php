...
// 공통쿼리
$sql_common = "FROM g5_bbs_basic WHERE del_yn = 'N' AND tbl_name = 'notice'";
...
?>
    <ul data-last-page="Y">
        <li><a href="javascript:void(0)"><p class="title">등록된 공지가 없습니다.</p></a></li>
    </ul>
<? } else { ?>
    <ul data-last-page="<?=$last_page?>">
        <? while($row = sql_fetch_array($result)) { ?>
        <li>
           <!--<a href="./notice_view.php?idx=<?=$row['idx']?>">-->
           <a href="#notice<?=$row['idx']?>">
                <p class="title"><strong>공지</strong><?=$row['subject']?></p>
