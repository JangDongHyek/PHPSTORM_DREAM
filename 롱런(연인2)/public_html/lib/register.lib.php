...
function valid_mb_id($reg_mb_id)
{
    if (preg_match("/[^0-9a-z_]+/i", $reg_mb_id))
        return "아이디를 4자이상 입력해 주세요. (영문/숫자)"; //return "회원아이디는 영문자, 숫자, _ 만 입력하세요.";
    else
        return "";
}
...
function count_mb_id($reg_mb_id)
{
    if (strlen($reg_mb_id) < 4)
        return "아이디를 4자이상 입력해 주세요. (영문/숫자)"; //"회원아이디는 최소 3글자 이상 입력하세요.";
    else
        return "";
}
...
function reserve_mb_id($reg_mb_id)
{
    ...
    if (preg_match("/[\,]?{$reg_mb_id}/i", $config['cf_prohibit_id']))
        return "사용 불가능한 아이디 입니다."; //"이미 예약된 단어로 사용할 수 없는 회원아이디 입니다.";
    else
        return "";
}
