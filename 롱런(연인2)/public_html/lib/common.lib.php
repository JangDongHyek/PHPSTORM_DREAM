...
function is_admin($mb_id)
{
    ...
    $mb = sql_fetch("SELECT mb_status FROM g5_member WHERE mb_id = '{$mb_id}'");
    $mb_status = $mb['mb_status'];

    if ($mb_status == "관리자") return "super";
    if ($mb_status == "카운슬러") return "super";
...
function member_delete($mb_id)
{
    ...
    */
    ...
    $sql = "insert into `v5_member_delete` set `ip` = '$ip', `mb_id` = '$member[mb_id]', `t_mb_id` = '$mb_id'";
    sql_query($sql);
...
function getMemberImg($mb_id="") {
    ...
    $result = sql_query(" SELECT idx, mi_img FROM g5_member_img WHERE mb_id = '{$mb_id}' ORDER BY idx ASC ");
    ...
    if ($result_cnt > 0) {
        ...
        for ($i = 0; $row = sql_fetch_array($result); $i++) {
            ...
            $img_url = MB_IMG_URL."/".$row['mi_img'];

            if (file_exists($img_path) && $row['mi_img'] != "") {
                $tmp_arr = array();
                $tmp_arr['idx'] = $row['idx'];
                $tmp_arr['mi_img'] = $row['mi_img'];
                $tmp_arr['src'] = $img_url;

                $img_cnt++;
                $arr['list'][] = $tmp_arr;
            } else {
                $sql = "DELETE FROM g5_member_img WHERE idx = '".$row['idx']."' AND mb_id = '{$mb_id}' ";
                sql_query($sql);
            }
        }
...
// 사진 정사각형
function getImgSquare($img_url, $base_size=60) {
    $img_path = str_replace(G5_DATA_URL, G5_DATA_PATH, $img_url);
    $img_css = "";

    if ($img_url != "" && file_exists($img_path)) {
        $img_size = getimagesize($img_path);
        $img_width = $img_size[0];
        $img_height = $img_size[1];

        $style_width = $base_size."px";		//세로>가로 기준으로
        $style_height = "auto";
        $style_margin = ( ($img_height * $base_size / $img_width) - $base_size ) / 2;
        $style_margin = "margin-top: -".(int)$style_margin."px";

        if ($img_width > $img_height) {		// 가로>세로이면
            $style_width = "auto";
            $style_height = $base_size."px";
            $style_margin = ( ($img_width * $base_size / $img_height) - $base_size ) / 2;
            $style_margin = "margin-left: -".(int)$style_margin."px";
        }

        $img = '<div><img src="'.$img_url.'" style="width: '.$style_width.'; height: '.$style_height.'; '.$style_margin.'" alt="images"></div>';

    } /*else {
		$img = '<img src="'.$img_url.'" style="width:{$base_size}px; height:{$base_size}px;">';
	}*/
...
// 카운슬러 매칭 카운트
function getMatchingCnt($helper_id, $s_date="", $e_date="") {
    // 일자추가는 나중에..

    $sql = "SELECT COUNT(*) AS cnt FROM g5_matching WHERE helper_id = '{$helper_id}'";
    $row = sql_fetch($sql);
    $cnt = $row['cnt'];
...
// 배열 재정렬 (array, 정렬 기준 key, 오름/내림차순)
function arrReSort($array, $key, $sort='asc') {
    $keys = array();
    $vals = array();
...
function reOrderHelpers($helpers, $order = 'ASC') {
    usort($helpers, function($a, $b) use ($order) {
        if ($a['total'] == $b['total']) {
            return 0;
        }
...
// 랜덤 문자열 생성 ABCDEFGHIJKLMNOPQRSTUVWXYZ
function getRandomString($leng = 6, $type="int"){
...
function sendFcm($tokens, $message)
{
    ...
    $fields = array(
        ...
        'data' => $message,
        'priority'          => 'high',
        'notification'      => array( 'title' => $message['subject'], 'body' => $message['message'],'url' => $message['goUrl'])
    );
    $headers = array(
        'Authorization:key =AAAAsVgvnKE:APA91bHgR72oXCM0E9T3ORt4WIHbxugxvOjhvAJt8vZiiUowt-8_8HsMO5w24LG5lFRcO2yUpDRhOgdVsrTjnk7XLtj24fy3PsgZIEacgip19zuXy4z6i4BVkYea9vG95OgkpEImgqBu',
...
// bbs 신규글 체크
function bbsNew($date) {
    $date_ts = strtotime($date);
    $limit_ts = strtotime("+1 days", $date_ts);

    if ($limit_ts > $date_ts) return "<img src='".G5_ADMIN_URL."/img/icon_new.gif'>";
    else "";
}

// bbs file 가져오기
function getBbsFiles($tbl_name, $idx) {
    $sql = "SELECT * FROM g5_bbs_file WHERE tbl_name = '{$tbl_name}' AND tbl_idx = {$idx} 
            ORDER BY regdate ASC, sort ASC";
    $result = sql_query($sql);

    $list = array();

    for ($i=0; $row=sql_fetch_array($result); $i++) {
        $file_path = BBS_IMG_PATH.'/'.$row['file_name'];
        if (file_exists($file_path)) { // 서버에 없으면 pass
            $tmp = array();
            $tmp['origin_name'] = $row['origin_name'];
            $tmp['file_name'] = $row['file_name'];
            $tmp['source'] = BBS_IMG_URL."/".$row['file_name'];
...
// bbs 코멘트 가져오기
function getBbsComment($tbl_name, $idx) {
    $sql = "SELECT idx, mb_no, mb_name, content, regdate, ip FROM g5_bbs_reply 
            WHERE del_yn = 'N' AND tbl_name = '{$tbl_name}' AND tbl_idx = '{$idx}' 
            ORDER BY idx ASC;";
    $result = sql_query($sql);
    $result_cnt = sql_num_rows($result);
...
// 페이징 get 파라미터 가져오기 (qstr return)
function getQueryString($get_param=array(), $exclude_param=array()) {
    $qstr = [];
    foreach ($get_param AS $key=>$val) {
        if ($key == "page" || $key == "token" || $val == "") continue;
        if (in_array($key, $exclude_param)) continue;
        if ($key == "s_city_lst") { // 지역 선택(배열)
            foreach ($val AS $k=>$v) {
                $qstr[] = "s_city_lst[{$k}]={$v}";
            }
            continue;
        }
        if ($key == "s_gu_lst") { // 상세지역 선택(배열)
            foreach ($val AS $k=>$v) {
                $qstr[] = "s_gu_lst[{$k}]={$v}";
            }
            continue;
...
// 숫자콤마+소숫점
function numberFormatDouble($num=0) {
    return (strpos($num, ".") !== false)? number_format($num, 1) : number_format($num);
}

// 회원 잔여쿠폰 조회
function getMemberCoupon($mb_no) {
    $row = sql_fetch("SELECT COUNT(*) AS cnt FROM g5_coupon WHERE mb_no = {$mb_no} AND use_date = ''");
    return (int)$row['cnt'];
}

// 회원 잔여하트 조회
function getMemberHeart($mb_no) {
    $row = sql_fetch("SELECT mb_heart FROM g5_heart WHERE mb_no = {$mb_no} ORDER BY idx DESC LIMIT 1");
    return (int)$row['mb_heart'];
}

// 회원 하트 적립/차감
function setMemberHeart($mb_no=0, $plus=0, $minus=0, $desc="", $ref_idx=0) {
    // 잔여 하트 조회
    $last_heart = getMemberHeart($mb_no);
    // 새로운 잔여 하트 계산
    $calc_heart = $last_heart + (int)$plus - (int)$minus;
    if ($calc_heart < 0) $calc_heart = 0; // 마이너스 제외

    $sql = "INSERT INTO g5_heart SET 
            mb_no = '{$mb_no}',
            plus_heart = '{$plus}',
            minus_heart = '{$minus}',
            mb_heart = '{$calc_heart}',
...
function getCounselorList() {
    $sql = "SELECT mb_no, mb_id, mb_name, mb_1 AS kakao_id, mb_4 AS work_time, mb_profile, mb_2, mb_3 AS work_on,
        (SELECT mi_img FROM g5_member_img WHERE mb_id = g5_member.mb_id ORDER BY idx DESC LIMIT 1) AS mb_img
        FROM g5_member
        WHERE mb_status = '카운슬러' AND mb_3 IN ('on', 'off')
        ORDER BY (CASE mb_3 WHEN 'on' THEN 1 ELSE 2 END), mb_no DESC";
    $result = sql_query($sql);
    $result_cnt = sql_num_rows($result);
