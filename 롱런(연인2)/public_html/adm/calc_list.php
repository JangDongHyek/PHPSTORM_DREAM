...
// if ($private) $sql_common = " AND match_date >= '2023-08-01' AND match_date <= '2023-08-19 19:39:00'";


// 카운슬러 리스트
$sql = "SELECT mb_id AS helper_id, mb_name AS helper_name FROM g5_member 
		WHERE mb_level = '10' AND mb_status = '카운슬러' AND mb_3 != 'out' ORDER BY mb_no ASC;";
...
for ($i = 0; $i < $helper_cnt; $i++) {
	...
	$helpers[$i][$match_type_arr[2]] = $_coupon;	    // 쿠폰소개
	$helpers[$i][$match_type_arr[3]] = $_heart;	        // 하트소개
	$helpers[$i]['total'] = $rs['case1'] + $rs['case2'] + $_coupon + $_heart; // 해당 카운슬러 총 건수
}

// total순으로 재정렬
// $helpers_re = arrReSort($helpers, 'total', 'desc');
$helpers_re = reOrderHelpers($helpers, 'DESC');
...
<div class="max1200" id="calc_list">
	<div class="tbl_head02 tbl_wrap mb_tbl">
		...
		<tr>
			...
			<? 
			// 계좌결제소개, 폰&카드소개, 쿠폰소개, 하트소개
			for ($j = 0; $j < count($match_type_arr); $j++) {
				$unq_no = $j.$i;
                $match_cnt = $helpers_re[$i][$match_type_arr[$j]];
			?>
			<td>
				<span id="col<?=$unq_no?>"><?=numberFormatDouble($match_cnt)?></span>
				<? if ($match_cnt > 0) { ?>
            ...
            ?>
			<td><strong><?=numberFormatDouble($helpers_re[$i]['total'])?></strong></td>
		</tr>
		<?php } ?>
        ...
        <tr class="total_rows">
            ...
            <? for ($j = 0; $j < count($match_type_arr); $j++) { ?>
            <td><?=numberFormatDouble($type_total_count[$j], 1)?></td>
            <? } ?>
            <td><?=numberFormatDouble($type_total_count['total'], 1)?></td>
        </tr>
		...
		<br>
		<p>※ 총 건수 = 계좌결제소개 + 폰&카드소개 + 쿠폰소개 + 하트소개<br>※ 쿠폰소개, 하트소개는 1건당 0.5건으로 집계됩니다.</p>
	</div>
