...
$sql_search = "WHERE mb_level < 10 ";
$sql_order_add = "";

// 220921. 카운슬러 소개이력에서 제외한 회원 조회
...
// 리스트
$sql = "select *,
        (SELECT COUNT(*) FROM g5_coupon WHERE mb_no = {$g5['member_table']}.mb_no AND use_date = '') AS coupon_cnt, /* 쿠폰개수 */
        (SELECT mb_heart FROM g5_heart WHERE mb_no = {$g5['member_table']}.mb_no ORDER BY idx DESC LIMIT 1) AS mb_heart /* 잔여하트 개수 */
        {$sql_common} {$sql_search} {$sql_order} limit {$from_record}, {$rows} ";
$result = sql_query($sql);

////// 회원숨김 권한
$mb_hide_auth = false;
if ($member['mb_id'] == "admin" || $member['mb_id'] == "lets080") { //|| $member['mb_id'] == "tncjs2" || $member['mb_id'] == "ssd231" ||
    $mb_hide_auth = true;
}
    ...
    <table>
        ...
        <tr>
            ...
            <td>
                ...
                <input type="text" name="stx" value="<?=$_GET['stx']?>" class="frm_input" placeholder="예) 이름,이름,···">
            </td>
            <th>회원상태</th>
            <td>
                <select name="s_swt">
            ...
            for ($i=0; $row=sql_fetch_array($result); $i++) {
                ...
                <tr class="<?php echo $bg; ?>">
                    ...
                    <td>
                        <? if ($row['mb_sex'] == "남") { ?>
                        <!-- 쿠폰 (남자만) -->
                        <? $coupon_btn_cls = ($row['coupon_cnt']==0)? "btn02" : "btn01"; ?>
                        <button type="button" class="<?=$coupon_btn_cls?>" onclick="getWinPop('coupon', '<?=$mb_no?>')"><?=$row['coupon_cnt']?>개</button>
                        <? } // end 남 ?>
                    </td>
                    <td>
                        <? if ($row['mb_sex'] == "남") { ?>
                        <!-- 하트 (남자만) -->
                        <?
                        $mb_heart = (int)$row['mb_heart'];
                        if ($member['mb_status'] == "관리자") {
                            echo "<a href='javascript:void(0)' onclick=\"getWinPop('heart', '{$mb_no}')\">{$mb_heart}개</a>";
                        } else {
                            echo "{$mb_heart}개";
                        }
                        ?>
...
<script>
    ...
    function getWinPop(mode, mb_id) {
        ...
        switch (mode) {
            ...
            case "coupon" :		// 쿠폰
                url = g5_admin_url + "/member_coupon.php?mb_no=" + mb_id;
                title = "쿠폰";
                break;

            case "heart" :      // 하트
                url = g5_admin_url + "/member_heart.php?mb_no=" + mb_id;
                title = "하트";
                break;
        }
