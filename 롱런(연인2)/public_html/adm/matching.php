...
function matchSubmit(f) {
    ...
    // 소개종류 > 하트소개이면 남자회원 하트 체크
    if (match_heart_cnt > 0) {
        if (f.mb_sex.value == "남") { // 매칭 주체가 남자이면, (내하트 개수 >= 총 하트소개 횟수*10) 여야 매칭가능
            if (f.mb_heart.value >= (match_heart_cnt * 10)) console.log('하트소개 매칭가능');
            else {
                alert("매칭가능한 하트가 부족합니다.\n(하트소개 1건 = 하트 10개 필요)");
                return false;
            }
        } else {  // 타겟이 남자이면, 타겟의 하트 확인 (소개1건 = 하트10개)
            var _submit = true;
            $.each($('select.valid_type'), function(index, elem) {
                if (elem.value == "하트소개") {
                    var _num = $(elem).data("num");
                    ...
                    if (parseInt(_target_heart) < 10) {
                        var _name = $("[name='target_name[" + _num + "]']").val();
                        alert("매칭가능한 하트가 부족합니다. (" + _name + ")");
                        _submit = false;
                    }
                }
            });
...
// 회원검색-선택
var list_no = 1;
function setMatchTarget(id, name, sex, age, tel, si, gu, no) {
	var obj = {};
