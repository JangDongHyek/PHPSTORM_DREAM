...
<link rel="stylesheet" href="<?=G5_ADMIN_URL?>/css/jquery-ui.min.css">
<script src="<?=G5_ADMIN_URL?>/js/jquery-ui.min.js"></script>
<script>
    function adminInfoPop(mb_id) {
        var url = g5_admin_url + "/member_form.php?w=u&mb_id=" + mb_id;
        var pop_w = 1200;
        var pop_h = 800;
        var left = Math.floor((window.innerWidth - pop_w) / 2);
        var top = Math.floor((window.innerHeight - pop_h) / 2);
        window.open(url, "관리자 정보수정","width="+pop_w+"px,height="+pop_h+"px,top="+top+",left="+left+",scrollbars=yes");
    }
</script>
...
<header id="hd">
    <div id="hd_wrap">
        ...
        <ul id="tnb">
            ...
            <? } ?>
			<!--<li style="font-weight:900;"><a href="<?php echo G5_URL ?>/" target="_blank">사이트바로가기</a></li>-->
			*/ ?>
            <li style="color:#FFF;">접속자 : <a href="javascript:void(0)" onclick="adminInfoPop('<?=$member['mb_id']?>')" style="color:#FFF;"><?=$member['mb_name']?>(<?=$member['mb_id']?>)</a></li>
