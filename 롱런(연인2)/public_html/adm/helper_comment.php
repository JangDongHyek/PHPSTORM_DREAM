...
<div id="comment_wrap">
	...
	<div class="frm">
		<form name="fComment" id="fComment">
			...
			<button type="button" onclick="calcCommentSubmit('w', document.fComment);">등록</button>
		</form>
	</div>
	<!-- // 입력폼 -->
	<!-- 리스트 -->
		...
		<ul>
			...
			<? 
				...
				foreach ($comment_list as $key=>$val) {
					$helper_name = ($val['helper_name'] != "")? $val['helper_name'] : "삭제된 카운슬러입니다.";
...
function getFrmLoad(num, idx) {
	...
	var input = $("<input type='hidden' name='helper_id' value='"+ helper_id +"'>"),
		...
		btn = $("<button type='button' onclick='calcCommentSubmit(\"r\", document."+ frm_name +");'>등록</button>");
...
// 코멘트등록
function calcCommentSubmit(mode, f) {
	var el = $(f),
