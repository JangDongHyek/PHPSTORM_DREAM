...
switch ($_POST['mode']) {
    ...
    case "heartWrite" : // 하트발급요청 글쓰기
        ...
        if ($idx == 0) {
            $sql = "INSERT INTO g5_bbs_heart SET 
                    writer_no = '{$writer_no}',
                    writer_ip = '{$_SERVER['REMOTE_ADDR']}',
                    mb_no = '{$mb_no}',
                    subject = '{$subject}',
    ...
    case "heartDelete" :   // 하트발급요청 삭제
        $sql = "UPDATE g5_bbs_heart SET del_yn = 'Y' WHERE idx IN (";
        $sql .= implode(",", $_POST['list']);
        $sql .= ")";
        $json['result'] = sql_query($sql);
        break;
