...
$sql = "SELECT A.*, B.mb_name,
        ...
        FROM g5_bbs_refund A LEFT JOIN g5_member B ON A.mb_no = B.mb_no 
...
<article id="bo_v" class="max1200">
    <header>
        ...
        <div style="padding: 0 0 10px; border-bottom: 1px solid #ddd; margin: 0 0 10px;">
            환불신청회원 <a href="" style="font-weight:bold; margin-left: 5px;"><?=$row['mb_name']?></a>
        </div>
    ...
    <div id="bo_v_top">
        <?php ob_start(); ?>
        <ul class="bo_v_com">
            <li><a href="./refund_form.php?idx=<?=$idx?>&<?=$qstr?>" class="btn_b01">수정</a></li>
            <li><a href="javascript:void(0)" onclick="deleteBbsOne('refundDelete', <?=$idx?>)" class="btn_b01">삭제</a></li>
            <li><a href="./refund.php?<?=$qstr?>" class="btn_b01">목록</a></li>
        </ul>
        <?php
