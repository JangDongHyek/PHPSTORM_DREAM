...
<h1>투자내역</h1>
<div class="tbl_frm01 tbl_wrap invest">
<form>
	<table>
		<thead>
			<tr>
				<th><label>펀드명</label></th>
                <th><label>입금일</label></th>
                <th><label>입금액</label></th>
                <th><label>목표수익율</label></th>
			    <th><label>설정일</label></th>
				<th><label>만기일</label></th>
				<th><label>고객계좌</label></th>
				<th><label>담당자</label></th>
				<th><label>메모</label></th>
				<th><label>관리</label></th>
			</tr>
		</thead>
		<tbody>
			<tr>
				<td><input id="fundname" type="text" class="frm_input"></td>
				<td><input id="dep_day" type="text" class="frm_input"></td>
				<td><input id="dep_moeny" type="text" class="frm_input"></td>
				<td><input id="goal" type="text" class="frm_input"></td>
				<td><input id="setting" type="text" class="frm_input"></td>
				<td><input id="exp_day" type="text" class="frm_input"></td>
				<td><input id="acc_num" type="text" class="frm_input"></td>
				<td><input id="admin" type="text" class="frm_input"></td>
				<td><textarea  id="memo" type="text" class="frm_input"></textarea></td>
				<td><input type="button" onclick="add();" value="추가하기" class="btn_frmline" style="background:#000"></td>
			</tr>
			<? 
				$result = sql_query("select * from `g5_write_invest` Where `mb_id` = '$mb[mb_id]'");
				while($row = sql_fetch_array($result)){ ?>
				<tr>
				<td><input id="fundname_<?=$row[wr_id]?>" type="text" value="<?=$row[wr_1]?>" class="frm_input"></td>
				<td><input id="dep_day_<?=$row[wr_id]?>" type="text" value="<?=$row[wr_2]?>" class="frm_input"></td>
				<td><input id="dep_moeny_<?=$row[wr_id]?>" type="text" value="<?=$row[wr_3]?>" class="frm_input"></td>
				<td><input id="goal_<?=$row[wr_id]?>" type="text" value="<?=$row[wr_4]?>" class="frm_input"></td>
				<td><input id="setting_<?=$row[wr_id]?>" type="text" value="<?=$row[wr_5]?>" class="frm_input"></td>
				<td><input id="exp_day_<?=$row[wr_id]?>" type="text" value="<?=$row[wr_6]?>" class="frm_input"></td>
				<td><input id="acc_num_<?=$row[wr_id]?>" type="text" value="<?=$row[wr_7]?>" class="frm_input"></td>
				<td><input id="admin_<?=$row[wr_id]?>" type="text" value="<?=$row[wr_9]?>" class="frm_input"></td>
				<td><textarea  id="memo_<?=$row[wr_id]?>"><?=$row[wr_8]?></textarea></td>
				<td><input type="button" onclick="edit(<?=$row[wr_id]?>);" value="수정하기" class="btn_frmline">  <input type="button" onclick="del(<?=$row[wr_id]?>);" value="삭제하기" class="btn_frmline"></td>
				</tr>
			<? } ?>
		</tbody>
	</table>
...
function add(){
	var mb_id = "<?php echo $mb['mb_id'] ?>";
	var mb_name = "<?php echo $mb['mb_name'] ?>";
	var fundname = $("#fundname").val();
	var dep_day = $("#dep_day").val();
	var dep_moeny = $("#dep_moeny").val();
	var goal = $("#goal").val();
	...
	$.ajax({
		type:"POST",
		url:"<?php echo G5_BBS_URL?>/ajax_add.php",
		dataType:"html",
		data: {
			"mb_id": mb_id,
			"mb_name" : mb_name,
...
function edit(wr_id){
	...
	var fundname = $("#fundname_"+wr_id).val();
	var dep_day = $("#dep_day_"+wr_id).val();
	var dep_moeny = $("#dep_moeny_"+wr_id).val();
	var goal = $("#goal_"+wr_id).val();
	var setting = $("#setting_"+wr_id).val();
	var exp_day = $("#exp_day_"+wr_id).val();
	var acc_num = $("#acc_num_"+wr_id).val();
	var admin = $("#admin_"+wr_id).val();
	var memo = $("#memo_"+wr_id).val();
