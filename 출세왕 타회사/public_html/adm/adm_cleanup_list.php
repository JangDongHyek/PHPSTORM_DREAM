...
$sql_common = " from {$g5['cleanup_table']} ";
...
if ($_GET['nr'] != "")
    $sql_search .= " and now_or_reserve = '{$_GET['nr']}' ";
...
//대기, 배차, 완료, 취소 count
$sql = "SELECT (SELECT COUNT(cu_idx) From {$g5['cleanup_table']} WHERE cu_step = 0) wait,
(SELECT COUNT(cu_idx) From  {$g5['cleanup_table']} WHERE cu_step = 1) assign,
(SELECT COUNT(cu_idx) From  {$g5['cleanup_table']} WHERE cu_step = 2) complete,
(SELECT COUNT(cu_idx) From  {$g5['cleanup_table']} WHERE cu_step = 3) cancel
from  {$g5['cleanup_table']} LIMIT 1";
...
<form id="fsearch" name="fsearch" class="local_sch01 local_sch" method="get">
    ...
    <select name="sfl" id="sfl" onchange="sfl_change();">
        ...
        <option value="cu_mb_name"<?php echo get_selected($_GET['sfl'], "cu_mb_name"); ?>>성함</option>
        <option value="cu_step"<?php echo get_selected($_GET['sfl'], "cu_step"); ?>>진행상황</option>
        <option value="cu_building"<?php echo get_selected($_GET['sfl'], "cu_building"); ?>>건물유형</option>
        <? /*
    ...
    <div class="tbl_head02 tbl_wrap mb_tbl">
        <table>
            ...
            <tr>
                ...
                <th>no</th>
                <th><?php echo subject_sort_link('cw_step') ?>진행상황</a></th>
                <th>상품명</th>
                <th>건물유형</th>
                <th>주소</th>
                <th>청소일정</th>
            ...
            for ($i=0; $row=sql_fetch_array($result); $i++) {
                $s_mod = '<a href="./adm_cleanup_form.php?'.$qstr.'&amp;w=u&amp;idx='.$row['cu_idx'].'">보기/수정</a>';
                $bg = 'bg'.($i%2);
                ...
                <tr class="<?php echo $bg; ?>">
                    <td>
                        <input type="checkbox" name="chk[]" value="<?php echo $row['cu_idx'] ?>" id="chk_<?php echo $i ?>">
                    </td>
                    <td><?=$list_no?></td>
                    <td><?= $step_list[$row["cu_step"]]?></td>
                    <td><?= $ct_list[$row["cu_type"]] ?></td>
                    <td><?= $cub_list[$row["cu_building"]]?></td>
                    <td><?= $row['cu_addr1']." ".$row['cu_addr2'] ?></td>
                    <td><?php
                        $date = $row['cu_date'];
                        $yoil = array("일","월","화","수","목","금","토");

                        $day = explode('-',$date);
                        echo $day[0] . "년 " . $day[1] . "월 " . $day[2] . "일  ".'('.$yoil[date('w', strtotime($date))].') '.$row['cu_time'];
                            ?></td>
                    <td><a href="./member_form.php?<?=$qstr?>&amp;w=u&amp;mb_id=<?=$row['mb_id']?>"><?=$row['mb_id']?></a></td>
                    <td><?=$row['cu_mb_name']?></td>
...
<script>
    ...
    function sfl_change(type) {
        ...
        if (sfl.val() == 'cu_step' || sfl.val() == 'cu_building'){
            option_list(sfl.val());
        }else{
            if (type != "ready") {
