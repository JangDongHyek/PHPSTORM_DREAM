...
foreach ($menu as $key => $value) {
    if (strpos($key, 'menu') === 0 && $key!="menu400") {

        $auth_menu_custom[$key] = array("name" => $menu[$key][0][1], "value" =>substr($menu[$key][0][0], 0, 3));
        $i++;
    }
}
...
<label for="stx" class="sound_only">회원아이디<strong class="sound_only"> 필수</strong></label>
    <span style="display: inline">회원아이디: </span>
    <input type="text" name="stx" value="<?php echo $stx ?>" id="stx" required class="required frm_input">
    <input type="submit" value="검색" id="fsearch_submit" class="btn_submit">

</form>

<form name="fauthlist" id="fauthlist" method="post" action="./management_auth_list_delete.php" onsubmit="return fauthlist_submit(this);">
...
<div class="tbl_head01 tbl_wrap">
    ...
    for ($i=0; $row=sql_fetch_array($result); $i++)
    {
        ...
        if (!isset($auth_menu_custom[$row['au_menu']]))
        {
//            sql_query(" delete from {$g5['auth_table']} where au_menu = '{$row['au_menu']}' ");
            $is_continue = true;
        }
    ...
    <tr class="<?php echo $bg; ?>">
        ...
        <td class="td_menu">
            ...
            <?php echo $auth_menu_custom[$row['au_menu']]['name'] ?>
        </td>
...
<form name="fauthlist2" id="fauthlist2" action="./management_auth_update.php" method="post" autocomplete="off">
<input type="hidden" name="sfl" value="<?php echo $sfl ?>">
<input type="hidden" name="stx" value="<?php echo $stx ?>">
<input type="hidden" name="sst" value="<?php echo $sst ?>">
<input type="hidden" name="sod" value="<?php echo $sod ?>">
...
<section id="add_admin">
    ...
    <div class="local_desc01 local_desc">
        <p>
            ...
            권한 <strong>r</strong>은 읽기권한, <strong>w</strong>는 쓰기권한입니다.
        </p>
    </div>
    ...
    <div class="tbl_frm01 tbl_wrap">
        ...
        <tr>
            ...
            <td>
                ...
                <input type="text" name="mb_id" value="<?php echo $mb_id ?>" id="mb_id" required class="required frm_input" readonly>
                <button type="button" onclick="mb_level_9_list()">관리팀장 리스트</button>
                <!--
                <a href="javascript:mb_level_9_list()" style=" padding: 5px" class="mb_level_9_list" >관리팀장 리스트</a>
                -->
                <div id="list_area" style="display:;margin-left: 150px;position: absolute;background: #f1f1f1;padding: 15px;border: 1px solid black;">
                    <?php $sql = "select * from g5_member where mb_level = 9";
                    $result = sql_query($sql);
                    ...
                    for ($i =0; $row = sql_fetch_array($result); $i++){ ?>
                        <div class="id_hover" style="cursor: pointer" onclick="id_settting('<?=$row['mb_id'] ?>')"><?=$row['mb_id']." : ".$row['mb_name']?></div>
                    <?php } ?>
        ...
        <tr>
            ...
            <td>
                <select id="au_menu" name="au_menu" required class="required">
                    ...
                    <?php
                    foreach($auth_menu_custom as $key=>$value)
                    {
                        if($value['name'] == ''){
                            continue;
                        }
...
//관리팀장 리스트
function mb_level_9_list() {
    if ($('#list_area').css('display') == "block"){
        $('#list_area').css("display","none");

    }else{

        $('#list_area').css("display","block");
    }

}

function id_settting(mb_id){
