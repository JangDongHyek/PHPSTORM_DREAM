    ...
    <div id="hd_wrap">
        ...
        <nav id="gnb">
            ...
            for ($i = 0; $auth_row = sql_fetch_array($auth_result); $i++){
                ...
                if (in_array("r",$arr)){
                    $auth_arr[$auth_row['au_menu']] = "Y";
                }else{
                    $auth_arr[$auth_row['au_menu']] = "N";
                }
            }

            if (empty($auth_arr) && $member['mb_level'] != 10){
                alert("대표자가 읽기 권한을 설정한 메뉴가 존재하지 않습니다.",G5_URL);
            }

            $gnb_str = "<ul id=\"gnb_1dul\">";
            foreach($amenu as $key=>$value) {
                $href1 = $href2 = $target = '';
                //권한이 있거나  level 이 10인 경우
                if ($auth_arr['menu'.$key] == "Y" || $member['mb_level'] == 10) {
