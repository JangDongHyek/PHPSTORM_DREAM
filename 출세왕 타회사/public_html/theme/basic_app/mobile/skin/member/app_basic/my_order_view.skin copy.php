...
<div id="my_reser" class="view">
    <div class="view_txt">
		<h2 class="title"><?= $cdt_list[$view['car_date_type']] ?><strong class="size"><?= $cs_list[$view['car_size']] ?></strong><span class="date">신청일 : <?php
                ...
                $day = explode('-',$date[0]);
                echo $day[0] . "년 " . $day[1] . "월 " . $day[2] . "일  ". date('H:i',strtotime($date[1])) ?></span></h2>
        <div class="serv">
            <dl class="tx_m cf">
                <dt>상품명</dt>
            ...
            <dl class="tx_m cf">
                ...
                <dd>
                    <?php
                    $date = $view['car_w_date'];
                    //정기세차가 아닐경우
                    if ($view['car_date_type'] != 2){
                        $yoil = array("일","월","화","수","목","금","토");

                        $day = explode('-',$date);
                        echo $day[0] . "년 " . $day[1] . "월 " . $day[2] . "일  ".'('.$yoil[date('w', strtotime($date))].') '.$view['car_w_date2'];
            ...
            <dl class="tx_m cf" onclick="document.location.href='tel:<?=$view['mb_hp']?>'">
                <dt>고객명/연락처</dt>
                <dd><?= $view['mb_name'] ?> / <?= hyphen_hp_number($view['mb_hp']) ?><span class="call"><a href='tel:<?=$view['mb_hp']?>' target="_parent"><i class="fas fa-phone-alt"></i></a></span></dd>
            </dl>
            <dl class="tx_m cf">
                <dt>주차구역사진</dt>
                <?php
            ...
            <dl class="tx_m cf">
                ...
                <dd><?php
                    ...
                    if ($date != ""){
                        ...
                        $day = explode('-',$date);
                        echo $day[0] . "년 " . $day[1] . "월 " . $day[2] . "일  ".'('.$yoil[date('w', strtotime($date))].') '.$view['rw_date2'];
