...
if ($sca || $stx) {
	...
	if ($bo_table != "review") {
        ...
        $sql = " SELECT COUNT(DISTINCT `wr_parent`) AS `cnt` FROM {$write_table} wr
            left join {$g5['member_table']} me on wr.mb_id = me.mb_id
            left join {$g5['member_table']} wr_me on wr_me.mb_id = wr.rv_wr_mb_id WHERE {$sql_search} ";
...
if ($sca || $stx) {
    if ($bo_table == "review") {
        $sql = " select distinct wr_parent,wr_me.mb_name ,me.mb_name from {$write_table} wr
            left join {$g5['member_table']} me on wr.mb_id = me.mb_id
            left join {$g5['member_table']} wr_me on wr_me.mb_id = wr.rv_wr_mb_id
            where {$sql_search} {$sql_order} limit {$from_record}, $page_rows ";
    }else{
