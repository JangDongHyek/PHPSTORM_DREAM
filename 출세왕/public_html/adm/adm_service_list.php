...
//결제한 3,5 정기실내 or 결제상관없이 나머지꺼 가져오게
$sql_search .= " and ( cw.is_payment = 'Y' or ( cw.car_date_type = 1 or cw.car_date_type = 2 or cw.car_date_type = 4) )";
...
//차 사이즈 필터
if ($_REQUEST["is_payment"] != ""){
    $sql_search .= "and is_payment = '{$_REQUEST["is_payment"]}'";
    if ($is_payment)
...
<form id="fsearch" name="fsearch" class="local_sch01 local_sch" method="get">
    ...
    <select name="sfl" id="sfl" onchange="sfl_change();">
        <option value="cw.mb_id"<?php echo get_selected($_GET['sfl'], "cw.mb_id"); ?>>고객아이디</option>
    ...
    <input type ="hidden" id="is_payment" name="is_payment" value="<?= $_REQUEST['is_payment']?>" >
...
<div class="btn_add01 btn_add" style="float: right">
    <a href="./adm_service_form.php">서비스등록</a>
</div>
<?php } ?>
    ...
    <div class="tbl_head02 tbl_wrap mb_tbl">
        <table>
            ...
            <tr>
                ...
                <th style="cursor: pointer">차 사이즈
                    <select id="car_size" onchange="sst_change2(this.value,'car_size')">
                        <option value="">선택</option>
                        <option value="1" <?=$car_size==1?'selected':''?>>소형/중형</option>
                        <option value="2" <?=$car_size==2?'selected':''?>>대형</option>
                    </select>
                ...
                <th>결제여부
                    <select id="is_payment" onchange="sst_change2(this.value,'is_payment')">
                        <option value="">선택</option>
                ...
                <th>작업완료일
                    <select  onchange="sst_change2(this.value,'complete_datetime')">
                        <option value="">선택</option>
                        <option value="desc">내림차순</option>
                        <option value="asc">오름차순</option>
                    </select>
            ...
            for ($i=0; $row=sql_fetch_array($result); $i++) {
                $s_mod = '<a href="./adm_service_form.php?'.$qstr.'&amp;w=u&amp;idx='.$row['cw_idx'].'">보기/수정</a>';
                $bg = 'bg'.($i%2);
...
<script>
    ...
    function cw_step_change(val){
        ...
        location.href = g5_admin_url + "/adm_service_list.php?<?=$qstr?>&cw_step=" + val;
    ...
    function date_change() {
        ...
        location.href = g5_admin_url + "/adm_service_list.php"+ params;
