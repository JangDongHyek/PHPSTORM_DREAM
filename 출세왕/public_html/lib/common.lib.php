...
// 경고메세지를 경고창으로
function alert($msg='', $url='', $type ='',$error=true, $post=false)
{
...
//23.04.17 날짜표시함수 wc
function successking_date($date, $date2=''){
    $yoil = array("일","월","화","수","목","금","토");
    $day = explode('-',$date);
    //날짜 형식 맞는지 체크
    if( $day[0] &&  $day[1] && $day[2]){
        echo $day[0] . "년 " . $day[1] . "월 " . $day[2] . "일  ".'('.$yoil[date('w', strtotime($date))].') '.$date2;
        ...
        //월화수목금토일이면 매주요일
    }else if(strpos($date,'월') !== false || strpos($date,'화') !== false || strpos($date,'수') !== false
        || strpos($date,'목') !== false || strpos($date,'금') !== false
        || strpos($date,'토') !== false || strpos($date,'일') !== false){
        echo '매주 '.$date.'요일'.' '.$date2;
    }else{
        
        //이도저도 아니다 싶으면 그냥 이렇게표시
        if($date){
            echo '('.$date.') ';
        }
        echo $date2;
