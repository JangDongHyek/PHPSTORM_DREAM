...
?>
    ...
    <!-- 출장세차 예약현황 -->
<?php
$sql = " SELECT * FROM new_autopay_history where userId = '{$_SESSION["ss_mb_id"]}'";
$jung_pay_query = sql_query($sql);
?>

    <div id="my_reser">
        <!--상단카테고리-->
        <ul class="cate02 cf">
            <li class="active"><a href="<?php echo G5_BBS_URL ?>/my_payment.php">정기결제</a></li>
            <li><a href="<?php echo G5_BBS_URL ?>/my_payment2.php">단기결제</a></li>
        </ul>

        <?php if (sql_num_rows($jung_pay_query) > 0){?>
        <!--내용부분-->
        <div class="in">
            <div class="cslist">
                <?php for ($i = 0; $row = sql_fetch_array($jung_pay_query); $i++){?>
                    ...
                    <div class="bx">
                        <div class="tx">
                            <dl class="tx_m">
                                <dt>결제일</dt>
                                <dd><?php
                                    $date = explode(" ",$row['regdate']);
                                    $day = explode('-',$date[0]);
                                    echo $day[0] . "년 " . $day[1] . "월 " . $day[2] . "일  ". date('H:i',strtotime($date[1])) ?></dd>
                            </dl>
                            <dl class="tx_m">
                                <dt>카드정보</dt>
                                <dd><?= $row['goodsName'] ?> / <?= $row['appCardName'] ?> / <?= $row['cardNum'] ?></dd>
                            </dl>
                            <dl class="tx_m">
                                <dt>금액</dt>
                                <dd>
