...
//비회원일경우 팅겨냄
//if ($is_guest && $arr_last != "login.php" && $arr_last != "register_form.php" &&
//        $arr_last != "register_form_manager.php" && $arr_last != "password_lost.php" && $arr_last != 'login_check.php' && $arr_last != 'register_result.php'){
//    alert("로그인 후 이용해주세요.",G5_BBS_URL.'/login.php','error');
//}

// dbconfig.php 권한없어서.. ㅠㅠ;
$g5['g5_shop_coupon_table'] = G5_TABLE_PREFIX . 'shop_coupon'; // 쿠폰관리
...
//관리팀장 읽기 권한 확인 후 메뉴 뿌려줌
$sql = "select * from g5_auth where mb_id = '{$member['mb_id']}' ";
$auth_result = sql_query($sql);
for ($i = 0; $auth_row = sql_fetch_array($auth_result); $i++){
    $arr = explode(',',$auth_row["au_auth"]);
    if (in_array("w",$arr)){
        $auth_arr[$auth_row['au_menu']."_write"] = "Y";
    }else{
        $auth_arr[$auth_row['au_menu']."_write"] = "N";
    }
...
$level_list = array(
    1=> array("value" => '2', "name" => "고객"),
    2=> array("value" => '3', "name" => "매니저"),
    3=> array("value" => '9', "name" => "관리팀장"),
    4=> array("value" => '10', "name" => "대표자"),
);

$card_list = array(
  '01' => '비씨','02' => '국민','03' => '외환','04' => '삼성','06' => '신한','07' => '현대','08' => '롯데','11' => '씨티','12' => 'NH농혐','13' => '수협','15' => '우리','16' => '하나','21' => '광주','22' => '전북','23' => '제주','24' => '산은','32' => '우체국','33' => '저축은행','35' => '새마을금고'
);
