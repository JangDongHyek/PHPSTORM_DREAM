 ...
 *****************************************************************************************/
	...
	if($mode=="modify") {
		...
		if($data[is_secret]&&!$is_admin&&$data[ismember]!=$member[no]&&$HTTP_COOKIE_VARS[zb_s_check]!=$setup[no]."_".$no) error("정상적인 방법으로 수정하세요");
			...
			if($data[file_name3])$file_name3="<br>&nbsp;".$data[s_file_name3]."이 등록되어 있습니다. <input type=checkbox name=del_file3 value=1> 삭제";
			if($data[file_name4])$file_name4="<br>&nbsp;".$data[s_file_name4]."이 등록되어 있습니다. <input type=checkbox name=del_file4 value=1> 삭제";
			if($data[file_name5])$file_name5="<br>&nbsp;".$data[s_file_name5]."이 등록되어 있습니다. <input type=checkbox name=del_file5 value=1> 삭제";
