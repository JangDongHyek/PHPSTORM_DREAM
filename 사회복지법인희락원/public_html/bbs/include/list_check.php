...
function list_check(&$data,$view_check=0) {
	...
	if(!$setup[only_board]) {
		...
		$file_name3=$data[s_file_name3];
		$file_name4=$data[s_file_name4];
		$file_name5=$data[s_file_name5];
		$file_download1=$data[download1];
		$file_download2=$data[download2];
		$file_download3=$data[download3];
		$file_download4=$data[download4];
		$file_download5=$data[download5];
		if($file_name1) {
		...
		if($file_name3) {
			$file_size3=@GetFileSize(filesize($data[file_name3]));
			$a_file_link3="<a href='download.php?$href$sort&no=$data[no]&filenum=3'>";
		} else {
			$file_size3=0;
			$a_file_link3="<Zeroboard";
		}
		if($file_name4) {
			$file_size4=@GetFileSize(filesize($data[file_name4]));
			$a_file_link4="<a href='download.php?$href$sort&no=$data[no]&filenum=4'>";
		} else {
			$file_size4=0;
			$a_file_link4="<Zeroboard";
		}
		if($file_name5) {
			$file_size5=@GetFileSize(filesize($data[file_name5]));
			$a_file_link5="<a href='download.php?$href$sort&no=$data[no]&filenum=5'>";
		} else {
			$file_size5=0;
			$a_file_link5="<Zeroboard";
		}
		...
		if(eregi("\.jpg",$file_name3)||eregi("\.gif",$file_name3)||eregi("\.png",$file_name3)) $upload_image3="<img src=$data[file_name3] border=0 name=zb_target_resize style=\"cursor:hand\" onclick=window.open(this.src)><br>";
		if(eregi("\.jpg",$file_name4)||eregi("\.gif",$file_name4)||eregi("\.png",$file_name4)) $upload_image4="<img src=$data[file_name4] border=0 name=zb_target_resize style=\"cursor:hand\" onclick=window.open(this.src)><br>";
		if(eregi("\.jpg",$file_name5)||eregi("\.gif",$file_name5)||eregi("\.png",$file_name5)) $upload_image5="<img src=$data[file_name5] border=0 name=zb_target_resize style=\"cursor:hand\" onclick=window.open(this.src)><br>";
	}
