...
if($chuchun == "y"){
	$sql = "select * from chuchun where bbs_no='$bbs_no' and index_no='$index_no' and username='$UnameSess'";
	...
	if($cnt > 0){
			echo ("		
				<script>
				window.alert('이미 추천 하셨습니다..');history.go(-1);
				</script>
			");
			exit;
	}else{	
		$sql = "insert into chuchun (seq_num,bbs_no,index_no,username) values ('','$bbs_no','$index_no','$UnameSess')";
		$res = mysql_query($sql,$dbconn);
			echo ("		
				<script>
				window.alert('추천 하셨습니다');
				location.href='board_read.php?item_no=$item_no&index_no=$index_no&bbs_no=$bbs_no&page=$page&mart_id=$mart_id&keyset=$keyset&searchword=$searchword';
				</script>
			");
			exit;
	}
...
if($board_class == 1){	
	...
	if($numRows < 1 && !$_SESSION["Mall_Admin_ID"]){
		echo ("		
			...
			//parent.location.href='../member/login.html?url=$url&item_no=$item_no&mart_id=$mart_id';
			parent.location.href='../member/login.php?url=$url&item_no=$item_no&mart_id=$mart_id';
			</script>
		");
		exit;	
	}
...
function delcheck(num1){
	...
	if(confirm(remessage)){
		location.href="board_delete_3.php?item_no=<?=$item_no?>&mart_id=<?=$mart_id?>&flag=delete&index_no=<?=$index_no?>&=&page=<?=$page?>&keyset=<?=$keyset?>&searchword=<?=$searchword?>&bbs_no="+num1;
	}
}
</script>
<?
...
if( $board_class == 1 || $board_class == 3 ){ //자신의 글 수정,삭제는 본인만
	if( $username == "$UnameSess" ){
		$mod_perm = "<a href='board_edit.php?item_no=$item_no&mart_id=$mart_id&index_no=$index_no&bbs_no=$bbs_no&page=$page&keyset=$keyset&searchword=$searchword'><img src='../image/helpdesk/bu_modify.gif' border='0'></a>";
		$del_perm = "<img src='../image/helpdesk/bu_delete.gif' border='0' style='cursor:hand' onclick='delcheck($bbs_no)'>";
	}else{
		$mod_perm = "";
		$del_perm = "";
	}
	...
	$mod_perm = "<a href='board_edit.php?item_no=$item_no&mart_id=$mart_id&index_no=$index_no&bbs_no=$bbs_no&page=$page&keyset=$keyset&searchword=$searchword'><img src='../image/helpdesk/bu_modify.gif'  border='0'></a>";
	$del_perm = "<a href='board_delete.php?item_no=$item_no&mart_id=$mart_id&flag=del&index_no=$index_no&bbs_no=$bbs_no&page=$page&keyset=$keyset&searchword=$searchword'><img src='../image/helpdesk/bu_delete.gif'  border='0'></a>";
}
								...
								<td background="../image/helpdesk/view_table_bg.gif"><table width="100%"  border="0" cellspacing="0" cellpadding="10">
										<tr>
											<td height="200" align="center"><table width="33%"  border="0" cellspacing="0" cellpadding="0">
													<tr>
														<td width="100"><img src="../image/helpdesk/pass_img.gif" align="absmiddle"></td>
														<td><table width="100%"  border="0" cellspacing="0" cellpadding="0">
																<tr>
																	<td align="center"><img src="../image/helpdesk/pass_type1.gif"></td>
																</tr>
																<tr>
...
?>
					...
					<table width="680" align="center"  border="0" cellspacing="0" cellpadding="0">
						...
						<tr>
							<td width="70" height="30" align="center"><img src="../image/helpdesk/view_email.gif"></td>
							<td width="1"><img src="../image/helpdesk/view_line.gif" width="1" height="30"></td>
							<td width="10"></td>
							<td><?=$email?></td>
						</tr>
...
<?
/*
	$sql = "select * from chuchun where bbs_no='$bbs_no' and index_no='$index_no'";
	$res = mysql_query($sql,$dbconn);
	$tot_cnt = mysql_num_rows($res);
...
?>
						...
						<tr>
							<td height="30" align="center"><img src="../image/helpdesk/view_date.gif"></td>
							<td><img src="../image/helpdesk/view_line.gif" width="1" height="30"></td>
							<td width="10"></td>
							<td class="point"><?=$write_date_str?>   &nbsp;조회:<?=$read_num?> &nbsp;추천:<?=$tot_cnt?></td>
						</tr>
		<?
		}
		?>
						...
						<tr>
							<td height="30" align="center"><img src="../image/helpdesk/view_hit.gif"></td>
							<td><img src="../image/helpdesk/view_line.gif" width="1" height="30"></td>
							<td width="10"></td>
								...
								<table width="100%"  border="0" cellspacing="0" cellpadding="10">
									...
									<tr>
										<td height="80" valign="top" align=center>
											...
											if($UnameSess){
											?>
											<a href="<?=$PHP_SELF?>?item_no=<?=$item_no?>&index_no=<?=$index_no?>&bbs_no=<?=$bbs_no?>&page=<?=$page?>&mart_id=<?=$mart_id?>&keyset=<?=$keyset?>&searchword=<?=$searchword?>&chuchun=y"><img src="../images/chuchun.gif" alt="추천"></a>
											<?}?>
											<br>
											...
											switch(media) { 
											case "tw": 
											sendUrl = "http://twitter.com/home?status=" + encodeURIComponent('1111112222222') + " " + escape(nowUrl); 
											openSNSWin(sendUrl); 
											break; 
											case "me": 
											sendUrl = "http://me2day.net/posts/new?new_post[body]="+encodeURIComponent('[365]\"111122222333\":')+escape(nowUrl)+"&new_post[tags]"+encodeURIComponent('1112223333444'); 
											openSNSWin(sendUrl); 
											break; 
											case "fa": 
											sendUrl = "http://www.facebook.com/sharer.php?u=" + escape(nowUrl) + "&t=" + encodeURIComponent('12345'); 
											openSNSWin(sendUrl); 
											break; 
											} 
											} 
											</script> 
											<a onclick="javascript:sendSNS('tw');"><img src="../images/twiter.gif" title="트위터" style="cursor:hand;"></a>
											<a onclick="javascript:sendSNS('fa');"><img src="../images/facebook.gif" title="페이스북" style="cursor:hand;"></a>
											<a onclick="javascript:sendSNS('me');"><img src="../images/metoday.gif" title="미투데이" style="cursor:hand;"></a>
										</td>
									</tr>
								</table>
							</td>
...
<?
	$SQL = "select index_no from $New_BoardTable where area='$item_no' and bbs_no = '$bbs_no' and mart_id = '$mart_id' and ansno > $ansno order by ansno limit 1";
	$dbresult = mysql_query($SQL, $dbconn);
	if ($dbresult == false) echo "쿼리 실행 실패!";
	if(mysql_num_rows($dbresult)>=1)
	$prevno = mysql_result($dbresult, 0,0);
	
	$SQL = "select index_no from $New_BoardTable where area='$item_no' and bbs_no = '$bbs_no' and mart_id = '$mart_id' and ansno < $ansno order by ansno desc limit 1";
	$dbresult = mysql_query($SQL, $dbconn);
	if ($dbresult == false) echo "쿼리 실행 실패!";
	if(mysql_num_rows($dbresult)>=1)
...
?>
					<table width="680" align="center" border="0" cellspacing="0" cellpadding="0">
						<tr>
							<td height="50">
								<?=$mod_perm?><?=$del_perm?><a href='board_reply.php?item_no=<?=$item_no?>&mart_id=<?=$mart_id?>&index_no=<?=$index_no?>&bbs_no=<?=$bbs_no?>&page=<?=$page?>&keyset=<?=$keyset?>&searchword=<?=$searchword?>'><img src="../image/helpdesk/bu_reply.gif"  border="0"></a>
							</td>
							<td align="right"><a href='board_list.php?item_no=<?=$item_no?>&mart_id=<?=$mart_id?>&bbs_no=<?=$bbs_no?>&page=<?=$page?>&keyset=<?=$keyset?>&searchword=<?=$searchword?>'><img src="../image/helpdesk/bu_list.gif"  border="0"></a><a href='board_write.php?item_no=<?=$item_no?>&mart_id=<?=$mart_id?>&bbs_no=<?=$bbs_no?>&page=<?=$page?>&keyset=<?=$keyset?>&searchword=<?=$searchword?>'><img src="../image/helpdesk/bu_write.gif"  border="0"></a></td>
						</tr>
</table>
              			...
              			<tr>
                			...
                			<td bgColor="#FFFFFF" height="10" colspan="2"></td>
									...
									<tr valign='top'>
										...
										<td width='10%' align="right"><a onfocus='blur()' style='cursor:hand' onClick="window.open('comment_check.php?item_no=<?=$item_no?>&flag=comdel&c_no=<?=$c_no?>&index_no=<?=$index_no?>&bbs_no=<?=$bbs_no?>&page=<?=$page?>&mart_id=<?=$mart_id?>&keyset=<?=$keyset?>&searchword=<?=$searchword?>','비밀번호확인','width=450,height=300,toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=no,resizable=no')"><img src='../image/helpdesk/bu_del.gif' border='0'></a></td>
									</tr>
								</table> 
                			</td>
              			</tr>
