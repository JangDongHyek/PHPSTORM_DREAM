 ...
 */
 $(function(){
	 let iframe = document.querySelector("#innopay_frm");
 });
function goPay(f) {
	setEdiDate();
	checkDevice(f);
	...
	if(checkData(f)){
		...
		if(f.FORWARD.value == 'Y'){
			...
			var winopts= "left="+left+",top="+top+",toolbar=no,location=no,directories=no, status=no,menubar=no,scrollbars=no, resizable=no,width=680,height=680";
			var targetWin="payWindow";
			var InnopayWin =  window.open("", targetWin, winopts);
			...
			$(".innopay-layer").css("display","block");
			f.target="innopay_frm";
			f.submit();
		}
		return false;
	}else{
...
function makeEncKey(frm){
	...
	var DutyFreeAmt = 0;
	try{
		if(frm.DutyFreeAmt.value!=undefined && frm.DutyFreeAmt.value!=null && frm.DutyFreeAmt.value.trim()!=''){
			if(!isNaN(parseInt(frm.DutyFreeAmt.value))){DutyFreeAmt=parseInt(frm.DutyFreeAmt.value);}
			Amt = parseInt(Amt) + DutyFreeAmt;
		}
	}catch(e){}
