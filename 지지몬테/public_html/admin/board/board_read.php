...
if($flag == 'move'){
	...
	for ($i=0; $i < 1; $i++) {	
		mysql_data_seek($dbresult, $i);
		$ary = mysql_fetch_array($dbresult);
		$query4 = "update $New_BoardTable set ansno = ansno + 1 where (ansno > 0) and mart_id = '$mart_id' and bbs_no='$to_bbs_no'";
		mysql_query( $query4, $dbconn );
		//$index_no_tmp = $ary["index_no"];
		$step_tmp = $ary["step"];
		$thread_tmp = $ary[thread];//답변글을 얻기 위함
		...
		if($numRows1>0)
			...
			$ansno_tmp = 0;
		$index_no_tmp=$index_no_tmp+1;
		$ansno_tmp=$ansno_tmp+1;
		$index_no_tmp2=$index_no_tmp+1;
		

		$sql="select max(thread) as thread from $New_BoardTable where bbs_no='$to_bbs_no' and mart_id='$mart_id' order by ansno desc";
		$result3=mysql_query($sql);
		$rs3=mysql_fetch_array($result3);
		$thread_r=$rs3[thread]+1;

		//답변글 쿼리문
		$sql="select * from $New_BoardTable where bbs_no='$from_bbs_no' and thread='$thread_tmp' and index_no>'$ary[index_no]'";
		...
		if(0<$numRows2){
			$ans_no=2;
			while($ary2=mysql_fetch_array($result2)){
				$SQL = "insert into $New_BoardTable (index_no, bbs_no, notice_no, mart_id, code, username, writer, passwd, doc_type, if_secret , write_date, msgno, ansno, thread,step, email, subject_new, content, userfile, userfile1,open_chk) values ('$index_no_tmp2', '$to_bbs_no', '$ary2[notice_no]', '$mart_id', '$ary2[code]', '$mart_id', '$ary2[writer]', '$ary2[passwd]', '$ary2[doc_type]', '$ary2[if_secret]', '$ary2[write_date]', $maxMsgNo+1, '$ans_no', '$thread_r','$ary2[step]', '$ary2[email]', '$ary2[subject_new]', '$ary2[content]', '$ary2[userfile]', '$ary2[userfile1]', '$ary2[open_chk]' )";
				$result=mysql_query($SQL);
				if($result){
					$sql="delete from $New_BoardTable where index_no='$ary2[index_no]' and mart_id='$mart_id' and bbs_no='$from_bbs_no'";
					$result=mysql_query($sql);
				}
				//답글 코멘트 이동
				$sql="update board_comment set index_no='$index_no_tmp2',bbs_no='$to_bbs_no' where index_no='$ary2[index_no]' and bbs_no='$from_bbs_no'";
				mysql_query($sql);
				$ans_no++;
			}
		}
		
		$SQL = "insert into $New_BoardTable (index_no, bbs_no, notice_no, mart_id, code, username, writer, passwd, doc_type, if_secret , write_date, msgno, ansno, thread, email, subject_new, content, userfile, userfile1,open_chk) values ('$index_no_tmp', '$to_bbs_no', '$ary[notice_no]', '$mart_id', '$ary[code]', '$mart_id', '$ary[writer]', '$ary[passwd]', '$ary[doc_type]', '$ary[if_secret]', '$ary[write_date]', $maxMsgNo+1, '1', '$thread_r', '$ary[email]', '$ary[subject_new]', '$ary[content]', '$ary[userfile]', '$ary[userfile1]', '$ary[open_chk]' )";
		$result=mysql_query($SQL);
		if($result){
			$sql="delete from $New_BoardTable where index_no='$ary[index_no]' and mart_id='$mart_id' and bbs_no='$from_bbs_no'";
			$result=mysql_query($sql);
		}
		//원글 코멘트이동
		$sql="update board_comment set index_no='$index_no_tmp',bbs_no='$to_bbs_no' where index_no='$ary[index_no]' and bbs_no='$from_bbs_no'";
		mysql_query($sql);
