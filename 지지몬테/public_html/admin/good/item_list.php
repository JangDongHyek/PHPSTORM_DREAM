...
//		if($Mall_Admin_ID == $mart_id){ // 내상품이면
			...
			for ($i=0; $i < $numRows; $i++) {
				...
				for($k=0;$k<100;$k++){
					...
					$output_3 = "../../data/editor/".$output_2[6]."/".$output_2[7];
					$output_3_c = "../../data/editor/".$output_2[6]."/c_".$output_2[7];	
				

					if($output_2[7]){
						if(file_exists($output_3)){							
							copy ("$output_3","$output_3_c");	//업로드 파일 저장
							$item_explain = str_replace($output_2[7],"c_".$output_2[7],$item_explain);
						}
					}					
				}
