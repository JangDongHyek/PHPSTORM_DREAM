...
<section id="bo_w">
    ...
    <form name="fwrite" id="fwrite" action="<?php echo $action_url ?>" onSubmit="return fwrite_submit(this);" method="post" enctype="multipart/form-data" autocomplete="off" style="width:<?php echo $width; ?>">
        ...
        <!--신규 예약폼-->
        <input type="hidden" name="wr_8" id="wr_8" value="<?=$write[wr_8]?>"><!--계산요금-->
                ...
                <dd>
                    <div class="scroll-box" tabindex="0">
                        ...
                        <p>
                            ...
                            5. “회사” 라 함은 임차인에게 대여사업용 승합자동차를 대여하며 운전기사를 알선하는 (주)유카를 의미합니다.<br>
        ...
        <div class="form-agree">
            ...
            <dl>
                ...
                <dd>
                    <div class="scroll-box" tabindex="0">
                        <p>경남주차장은 기업/단체 및 개인의 정보 수집 및 이용 등 처리에 있어 아래의 사항을 관계법령에 따라 고지하고 안내해 드립니다.&nbsp;</p>
...
<script>
    ...
    $(function() {
        ...
        // 계산하기
        const calButton = document.querySelector('#btn-cal');
        if (calButton) calButton.addEventListener('click', calcParkingFee);
        ...
        // 휴대폰
        const hpInput = document.querySelector('#wr_3');
        if (hpInput) {
            hpInput.addEventListener('input', function(e) {
                formatPhoneNumber(this);
            });
            hpInput.addEventListener('paste', function(e) {
                formatPhoneNumber(this);
            });
        }
    ...
    function calcParkingFee() {
        const formData = {
            startDate: $("input[name='wr_1[0]']").val(),
            startTime: $("#wr_1_1").val() + $("#wr_1_2").val(),
            endDate: $("input[name='wr_2[0]']").val(),
            endTime: $("#wr_2_1").val() + $("#wr_2_2").val(),
        };

        if (!formData.startDate) {
            return alert('이용예정일시를 선택하세요.');
        }
        if (!formData.endDate) {
            return alert('도착예정일시를 선택하세요.');
        }

        $.ajax({
            url: "./ajax.calc_parking_fee.php",
            data: formData,
            dataType: "json",
            type: "POST",
            success: function(data) {
                ...
                if (response.status != 'success') {
                    const message = response.message || '요금조회에 실패했습니다. 잠시 후 다시 시도해 주세요.';
                    alert(message);
                    return;
                }

                // 모달 내
                $("#start_date").html(formData.startDate + ' ' + formData.startTime);
                $("#end_date").html(formData.endDate + ' ' + formData.endTime);
                $("#price").html(addCommaRegex(response.data.fee) + '원');
                $("#day").html(response.data.days + '일');
                $("#wr_8").val(response.data.fee);
    ...
    $('#checkAll').on('change', function() {
        ...
        $('input[name="agree"], input[name="agree2"], input[name="agree3"]').prop('checked', isChecked);
    });

    // 개별 체크박스 상태에 따라 전체 체크박스 상태 변경
    $('input[name="agree"], input[name="agree2"], input[name="agree3"]').on('change', function() {
        let allChecked = $('input[name="agree"]').prop('checked') &&
            $('input[name="agree2"]').prop('checked') &&
            $('input[name="agree3"]').prop('checked');
        $('#checkAll').prop('checked', allChecked);
    });

    // 콤마
    function addCommaRegex(number) {
