		...
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					...
					<h4 class="modal-title" id="appModalLabel">Gift bunker</h4>
				</div>
				<div class="modal-body">
					<div class="txt">
						<h2>Please enter the bunker to be gifted.</h2>
						<ul class="list_product charge">
							<li>
								<i>BUNKER</i>
								<input type="text" class="frm_input" placeholder="BUNKER" id="bunker" name="bunker" onkeyup="comma_number(this);">
							</li>
						</ul>
						<a href="javascript:confirmModal();">Give a gift</a>
					</div>
				</div>
			</div>
		</div>
			...
			<div class="modal-content">
				<div class="modal-body">
					<div class="txt confirm">
						<h2> Would you like to present a bunker to <span class="blue gift_bunker"><?=getNickOrId($you['mb_id'])?>?</span></h2>
						<em>(The bunker you have given as a gift cannot be retrieved.) </em>
					</div>
					<ul class="madal_btn">
						<li data-dismiss="modal">Cancel</li>
						<li class="ok" onclick="giftBunker();">Confirm</li>
					</ul>
				</div>
			</div>
		</div>
            ...
            <div class="modal-content">
                <div class="modal-body">
                    <div class="txt confirm">
                        ...
                        <em>(If you leave the chat room, all conversations will be deleted.)</em>
                    </div>
                    <ul class="madal_btn">
                        <li data-dismiss="modal">Cancel</li>
                        <li class="ok" onclick="chatOut('<?=$room_idx?>');">Confirm</li>
                    </ul>
                </div>
            </div>
        </div>
    ...
    <div id="chat_room">
        <div class="chat_hd">
            ...
            <ul class="edit_list edit_list_q" style="display: none;">
                ...
                <!--<li class="bunker"><a data-toggle="modal" data-target="#presentModal">Gift bunker</a></li>-->
                <?php } ?>
                <li class="delete"><a data-toggle="modal" data-target="#chatOutModal">Leave the chat room</a></li>
            </ul>
        </div>
        ...
        <div class="chat_ft">
            ...
            <div class="chat_input"><textarea placeholder="Please enter your details." name="msg" id="msg" class="msg-form"></textarea></div>
        ...
        <div class="chat_profile">
            ...
            <div class="cop_info">
                ...
                <div class="area_star">
                    ...
                    <span><?=companyScore($you['mb_id'], 'Y')?> stars</span>
                </div>
            </div>
        </div>
        <div class="corp_home"><a href="<?=G5_BBS_URL?>/company.php?mb_no=<?=$you['mb_no']?>" target="_blank">View corporate website</a></div>
        <!--<div class="area_nm">
			<a href="" data-toggle="modal" data-target="#presentModal" class="btn_bunker">Gift bunker</a>
        </div>-->
		<div class="info_box">
            <div class="chat_box">
                <h3>Introduce</h3>
            ...
            <div class="chat_box">
                ...
                <div class="cont">
                    <ul class="list_info">
                        ...
                        <li>
                            <em>Address</em>
                            <span><?=$you['mb_addr2'].' '.$you['mb_addr1']?></span>
                        </li>
                        <li>
                            <em>Featured products and services</em>
                            <span>
                                <?php
                                $mb_goods_service = '';
                                if(!empty($you['mb_goods_service'])) {
                                    ...
                                    for($i=0; $i<count($temp); $i++) {
                                        $mb_goods_service .= $temp[$i].',';
                                    }
                                }
                                ?>
                                <?=str_replace(',', ', ',substr($mb_goods_service, 0, -1));?>
                            </span>
                        </li>
                        <li>
                            <em>Phone number</em>
    ...
    <div id="chat_info" class="basic">
        ...
        <div class="chat_profile">
            ...
            <div class="cop_info">
				...
				<em>Podosea sea miles </em> <span class="blue"><?=number_format($you['mb_grade_point'])?>NM</span>
            </div>
            ...
            <div class="chat_box">
                ...
                <div class="cont">
                    <ul class="list_info">
                        ...
                        <li>
                            <em>Help Me Questions</em>
                            <span><?=number_format($q_count)?></span>
                        </li>
                        <li>
                            <em>Help Me Answers</em>
                            <span><?=number_format($a_count)?></span>
                        </li>
                        <li>
                            <em>accepted answer</em>
                            <span><?=number_format($s_count)?></span>
                        </li>
                    </ul>
                </div>
            </div>
...
<script>
    ...
    function giftBunker() {
        ...
        if('<?=$member['mb_bunker']*1?>' < $('#bunker').val().replace(',','')*1) {
            swal('There are not enough bunkers.\nMy bunker <?=$member['mb_bunker']*1?> BUNKER');
	    ...
	    $.ajax({
            ...
            success: function(data) {
                if(data == 'success') {
                    swal('Gave a bunker as a gift to <?=getNickOrId($you['mb_id'])?>.')
                    .then(()=>{
                        location.reload();
                    });
                }
