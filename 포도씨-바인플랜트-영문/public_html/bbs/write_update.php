...
if (isset($_POST['wr_content'])) {
    //$wr_content = substr(trim($_POST['wr_content']),0,65536); // 내용이 길어지면 잘려서 주석
...
if ($w == '' || $w == 'r') {
    ...
    // 공지사항 글 작성 시 푸시 ==> IOS 스토어 등록 때문에 추가 - 푸시 사용 시 쿼리 수정 필요 (mb_level 체크)
    // goUrl 지정 안하면 ios 에러남
    if($w == '' && $bo_table == 'notice') {
        $sql = " select * from g5_fcm where token != '' and agent = 'IOS' ";
        ...
        for($i=0; $row=sql_fetch_array($result); $i++) {
            ...
            $message=array(
                ...
                "goUrl"=>G5_BBS_URL."/board.php?bo_table=notice&wr_id=".$wr_id,
                //"goUrl"=>"",
            );
