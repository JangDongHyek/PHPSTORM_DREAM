...
if($status_code == 200) {
    ...
    if($status_code == 200) {
        ...
        if (isset($mb['mb_id'])) {
            ...
            alert('회원 정보가 없습니다.\n회원 가입을 완료해 주세요.', G5_BBS_URL.'/register.php?id='.$id.'&email='.$email.'&name='.$name.'&mobile='.$mobile.'&sns=naver', false);
            //goto_url(G5_BBS_URL.'/register.php?id='.$id.'&email='.$email.'&name='.$name.'&mobile='.$mobile.'&sns=naver');
        }
    } else {
