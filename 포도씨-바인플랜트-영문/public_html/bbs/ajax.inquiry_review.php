...
if($mode == 'select') { // 거래 후기 작성, 작성 완료 시 거래 회사 변경 불가능
    ...
    sql_query(" update g5_company_estimate set ce_state = 'Transaction Complete' where idx = '{$estimate_idx}' ");
    sql_query(" update g5_company_estimate set ce_state = 'Agreement Incomplete' where idx != '{$estimate_idx}' and company_inquiry_idx = '{$idx}' ");
    ...
    if(!empty($no_estimate_arr)) {
        sql_query(" insert into g5_company_inquiry_result set type = '미체결', inquiry_mb_id = '{$member['mb_id']}', inquiry_idx = '{$idx}', estimate_idx = '{$no_estimate_idx}', review = 4, review_etc = 'Choose third party company quote', wr_datetime = '" . G5_TIME_YMDHIS . "' ");
    }
    // ==//선택 받지 못한 회사의 견적은 상태-미체결로 업데이트 / 선택 된 회사의 견적은 상태-거래완료로 업데이트
}
...
if($mode == 'thanks') { // 감사 인사 작성
    ...
    if(!$is_admin) {
        ...
        $msg = bunkerHistory($member['mb_id'], '차감', $bunker, 'Sending thanks to the bunker ('.$ci['mb_id'].')', $ci['mb_id'], 'g5_company_estimate', $estimate_idx); // 견적 회사 차감
        if(!$msg) {
            echo 'no_bunker';
            exit;
        } else {
            bunkerHistory($ci['mb_id'], '적립', $bunker, 'Sending thanks to the bunker ('.$ce['mb_id'].')', $member['mb_id'], 'g5_company_estimate', $estimate_idx); // 의뢰 회사 지급
        }
    }
}
