...
{
    // 헬프미 답변 정보 DELETE
    $result = sql_query(" update g5_helpme_answer set del_yn = 'Y' where idx = {$helpme_an_idx}; ");
...
else if($mode == 'delete')
{
    ...
    $result = sql_query(" update g5_helpme set del_yn = 'Y' where idx = {$helpme_idx}; ");
    $result = sql_query(" update g5_helpme_answer set del_yn = 'Y' where helpme_idx = {$helpme_idx}; ");

    // 글 삭제 시 포인트 차감
    gradePointInsert($member['mb_id'], '차감','게시글', '5', '헬프미 글 삭제', '', 'g5_helpme', $helpme_idx);

    // 글 삭제 시 벙커 설정하였으면 회수
    bunkerHistory($member['mb_id'], '적립', $help['he_bunker'], '헬프미 질문 삭제 BUNKER 회수', '', 'g5_helpme', $helpme_idx, 'correct');
...
// 답변 채택
    ...
    if(!empty($help['he_bunker'])) { // 질문에 BUNKER를 입력했을 때
        ...
        $result = bunkerHistory($help['answer_mb_id'], '적립', $bunker, '헬프미 답변 채택 ('.$txt.')', '', 'g5_helpme_answer', $helpme_an_idx, $gubun);
    }

    // 등급포인트 - 답변 채택 시 20NM 적립
    gradePointInsert($help['answer_mb_id'], '적립', '채택', '20', '헬프미 답변 채택', '', 'g5_helpme_answer', $helpme_an_idx);

    // 등급포인트 - 미채택 된 답변 5NM 적립
    // 헬프미 글에 달린 미채택 답변 조회
    $selection = selectCount('g5_helpme_answer', 'helpme_idx', $help['idx'], 'an_selection', 'Y');
    if($selection > 1) { // 베스트/우수 답변 2개를 채택할 수 있으므로 답변 2개 채택 완료 시 미채택된 답변들에게 포인트 적립
        sql_query(" update g5_helpme set he_selection = 'Y' where idx = '{$help['idx']}' "); // 답변 2개 선택 시 해당 질문은 더이상 채택할 수 없도록 마감 처리를 하기 위한 표시
        $an_rlt = sql_query(" select * from g5_helpme_answer where helpme_idx = '{$help['idx']}' and an_selection is null ");
        while($an = sql_fetch_array($an_rlt)) {
            gradePointInsert($an['mb_id'], '적립', '미채택', '5', '헬프미 답변 미채택', '', 'g5_helpme_answer', $an['idx']);
        }
    }
