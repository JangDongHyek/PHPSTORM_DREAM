...
$answer_count = selectCount("g5_community_answer", "community_idx", $idx); // 댓글수
	...
	<div class="inr v3">
			...
			<div id="help_list">
				<div class="help_question top">
					<div class="title">
						...
						<div class="list_info">
                            ...
                            if($co['co_open'] != 'private') {
                                if($co['co_category'] == '일반') { // 일반 프로필
                            ?>
                            <!-- 토글메뉴 -->
                            <ul class="user_list sm answer02">
                                <?php if(($member['mb_id'] != $co['mb_id'] && $member['mb_category'] == '일반') || $is_admin == 'super') { // (로그인자!=작성자 && 로그인자카테고리=='일반') || 관리자?>
                                    <li onclick="likeFriend('<?=$co['mb_id']?>', '<?=$mode?>');"><?=$txt?></li>
                                    <li onclick="chatting('<?=$co['mb_id']?>');">채팅하기</li>
                                <?php } ?>
                                <li onclick="profileOpen('<?=$co['mb_category']?>', '<?=$co['mb_id']?>')">프로필보기</li>
                                <li data-toggle="modal" data-target="#reportModal">신고하기</li>
                            </ul>
                            <?php
                            }
                            else { // 기업 프로필
                            ?>
                            <ul class="user_list user_list_main">
				...
				<div class="area_answer">
					<h2>총 <span class="blue"><?=number_format($answer_count)?>개</span>의 댓글이 있습니다.</h2>
                    <?php
                    // 댓글 리스트
                    $sql = " select an.*, mb.mb_no, mb.mb_nick, mb.mb_grade, mb.mb_category from g5_community_answer as an left join g5_member as mb on an.mb_id = mb.mb_id where community_idx = '{$idx}' order by an.wr_datetime desc ";
                    ...
                    for($i=0; $row=sql_fetch_array($result); $i++) {
                        ...
                        // 답글
                        $answer_reply_count = selectCount("g5_community_answer_reply", 'community_an_idx', $row['idx']);
                        $add_style = '';
                    ...
                    <div class="help_question">
                        <div class="title">
                            ...
                            <div class="profile_info">
                                <div class="area_name">
                                    ...
                                    if($member['mb_id'] != $row['mb_id'] && ($member['mb_category'] == '일반' && $row['mb_category'] == '일반') && $row['an_open'] != 'private') {
                                    ?>
                                    ...
                                    <!-- 토글메뉴 -->
                                    <?php if($row['an_open'] != 'private') { ?>
									<?php if($row['mb_category'] == '일반') { // 일반 프로필 ?>
									<ul class="answer_<?=$row['idx']?> user_list answer01 sm">
										<?php if(($member['mb_id'] != $row['mb_id'] && $member['mb_category'] == '일반') || $is_admin == 'super') { // (로그인자!=작성자 && 로그인자카테고리=='일반') || 관리자?>
									...
									<?php } else { // 기업 프로필 ?>
									<ul class="answer_<?=$row['idx']?> user_list answer01">
                            ...
                            <?php } ?>
							<!--			
                            <ul class="user_list user_list_<?=$row['idx']?>">
                                <li><a href="<?=G5_BBS_URL?>/company.php?mb_no=<?=$row['mb_no']?>">기업홈피로 이동</a></li>
                                <li>의뢰건수 <em class="blue"><?=inquiryCount($row['mb_id'])?></em>건</li>
                                <li>거래건수 <em class="blue"><?=completeCount($row['mb_id'])?></em>건</li>
                            </ul>
                        ...
                        <div class="area_bottom">
                            <div class="bottom">
                                ...
                                <div class="info">
                                    <a class="btn_reply count" href="javascript:void(0);">답글 <span id="answer_reply_count_<?=$row['idx']?>"><?=number_format($answer_reply_count)?></span>개</a>
                                    <?php if($is_member) { ?><a class="btn_reply reply_<?=$row['idx']?> reply_write" href="javascript:void(0);">답글쓰기</a><?php } ?><!-- 클릭하면 .area_comment_input display:block-->
                                    <ul class="thums">
                            ...
                            <div class="area_comment area_comment_<?=$row['idx']?>" <?=$add_style?>>
                                <ul class="list_comment list_comment_<?=$row['idx']?>">
                                    <?php
                                    $sql_reply = " select re.*, mb.mb_no, mb.mb_nick, mb.mb_category from g5_community_answer_reply as re left join g5_member as mb on re.mb_id = mb.mb_id where community_an_idx = {$row['idx']} order by wr_datetime desc ";
                                    ...
                                    <li>
                                        <h3>
											...
											<!-- 토글메뉴 -->
											<?php if($reply['mb_category'] == '일반') { // 일반 프로필 ?>
											<ul class="reply_<?=$reply['idx']?> user_list answer02 sm">
												<?php if(($member['mb_id'] != $reply['mb_id'] && $member['mb_category'] == '일반') || $is_admin == 'super') { // (로그인자!=작성자 && 로그인자카테고리=='일반') || 관리자?>
											...
											<?php } else { // 기업 프로필 ?>
											<ul class="reply_<?=$reply['idx']?> user_list answer02">
...
$(function () {
	...
	/* 답변 토글 */
	$('.help_question .profile_info h4').on('click', function () {
        var cls = $(this).siblings('.user_list')[0]['classList'][0]; // 클래스 추가되거나 변경되면 배열 다시 확인
        $(this).toggleClass('active');
        $('.help_question .profile_info .'+cls).toggleClass('active');
        return false;
    });

	/* 답변 댓글 토글 */
	$('.help_question .list_comment h3').on('click', function () {
        var cls = $(this).siblings('.user_list')[0]['classList'][0]; // 클래스 추가되거나 변경되면 배열 다시 확인
        $(this).toggleClass('active');
        $('.help_question .list_comment h3 .'+cls).toggleClass('active');
        return false;
    });
