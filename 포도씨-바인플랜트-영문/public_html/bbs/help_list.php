            ...
            <div class="modal-content">
                <div class="modal-body">
                    <div class="txt confirm">
                        <h2>There are answers that have not been adopted.</h2>
                    </div>
                    <ul class="madal_btn">
                        <li data-dismiss="modal">Confirm</li>
                        <?php
                        if($member['mb_level'] == 2) { $url = "./mypage_help.php"; }
                        else { $url = "./mypage_chelp.php"; }
                        ?>
                        <li class="ok" onclick="location.href='<?=$url?>'">Mypage</li>
                    </ul>
                </div>
            </div>
        </div>
		...
		<div id="help_list">
			...
			<div class="top_filter">
				<ul class="tabs">
					...
					<li class="active" rel="tab1"><span>Waiting for answer</span></li>
					<li rel="tab2"><span>Answered</span></li>
				</ul>
				<ul class="sort_list">
					<li class="selected"><span>By Popularity</span></li>
					<li><span>By Latest</span></li>
					<li><span>By Bunker</span></li>
				</ul>
				<div class="msort_list">
					<span data-toggle="modal" data-target="#listModal">By Popularity</span>
				</div>
			</div>
			<div class="mbox_cate">
    ...
    $(function() {
        ...
        $("ul.tabs li").click(function () {
            if (!($(this).find('a').length > 0)) {
                ...
                var txt = '';
                if($(this)[0].innerText == 'Waiting for answer') { txt = '답변대기'; }
                else { txt = '답변완료'; } // Answered
        ...
        $.ajax({
            ...
            success : function(data) {
                if(data){
                    ...
                    $(".left a").highlight(search); // 검색어 있을 시 하이라이트
