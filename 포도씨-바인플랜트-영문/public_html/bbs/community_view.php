	...
	<div class="inr v3">
					...
					<div class="title">
						...
						<div class="list_info">
							...
							<ul class="edit_list edit_list_q" style="display: none;">
								<li class="modify"><a href="<?=G5_BBS_URL?>/community_write.php?idx=<?=$co['idx']?>&w=u">Edit</a></li>
								<li class="delete"><a href="javascript:community_action_chk('community');">Delete</a></li>
							</ul>
                            ...
                            <ul class="user_list_main user_list sm">
                                ...
                                <li onclick="profileOpen('<?=$co['mb_category']?>', '<?=$co['mb_id']?>')">View Profile</li>
                                <?php } ?>
                                ...
                                <li><a href="<?=G5_BBS_URL?>/company.php?mb_no=<?=$co['mb_no']?>">Go to Company Homepage</a></li>
                                <li>RFQs <em class="blue"><?=inquiryCount($co['mb_id'])?></em></li>
                                <li>Transactions <em class="blue"><?=completeCount($co['mb_id'])?></em></li>
                                <?php } ?>
                                <?php if(!$self && $co['mb_category'] == '일반') { // 내가쓴글아님 && 작성자일반회원?>
                                <li onclick="chatting('<?=$co['mb_id']?>');">Chat</li>
                                <?php } ?>
                                <?php } ?>
                                <?php if(!$self) { // 내가쓴글아님?>
                                <li onclick="reportOpen('<?=$co['mb_id']?>', 'g5_community', '<?=$co['idx']?>')">Report</li>
                                <?php } ?>
                            </ul>
                            <!-- //토글메뉴 -->
						</div>
                ...
                <form id="fanswer" name="fanswer" method="post" autocomplete="off" action="<?=G5_BBS_URL?>/community_view_update.php">
                    ...
                    <div class="help_write">
                        ...
                        <div class="area_textarea">
                            <textarea id="an_contents" name="an_contents" placeholder="Please post a comment."></textarea>
                        </div>
                        ...
                        <div class="w_filter">
                            <ul class="area_filter">
                                <li>
                                    ...
                                    <label for="an_open">
                                        <span></span>
                                        <em>Register anonymously</em>
                                    </label>
                                </li>
                            </ul>
                        </div>
                        <button type="button" class="btn_confirm" onclick="community_answer_register();">Post</button>
                    </div>
                </form>
                <?php } ?>

				<div class="area_answer">
					<h2><span class="blue">There are <?=number_format($answer_count)?></span> comments.</h2>
                    <?php
                    // 댓글 리스트
                    $sql = " select an.*, mb.mb_no, mb.mb_nick, mb.mb_grade, mb.mb_category from g5_community_answer as an left join g5_member as mb on an.mb_id = mb.mb_id where community_idx = '{$idx}' and del_yn is null order by an.wr_datetime desc ";
                    ...
                    <div class="help_question">
                        <div class="title">
                            <div class="area_name" onclick="userToggle('answer_<?=$row['idx']?>');">
                                ...
                                <div class="profile_info">
                                    ...
                                    <ul class="answer_<?=$row['idx']?> user_list answer01 sm">
                                        ...
                                        <?php } ?>
                                        <?php if(!$self) { // 내가쓴글아님?>
                                        <li onclick="reportOpen('<?=$row['mb_id']?>', 'g5_community_answer', '<?=$row['idx']?>')">Report</li>
                                        <?php } ?>
                                    </ul>
                                    <!-- //토글메뉴 -->
                            ...
                            <ul class="edit_list edit_list_<?=$row['idx']?>" style="display: none;">
                                <li class="modify"><a href="javascript:community_action('answer_update', <?=$row['idx']?>);">Edit</a></li>
                                <li class="delete"><a href="javascript:community_action_chk('answer', <?=$row['idx']?>);">Delete</a></li>
                            </ul>
                            <?php } else if($is_admin) { // 관리자?>
                            <a href="javascript:edit_open('edit_list_<?=$row['idx']?>');" class="btn_more"></a>
                            <ul class="edit_list edit_list_<?=$row['idx']?>" style="display: none;">
                        ...
                        <div class="area_bottom">
                            <div class="bottom">
                                ...
                                <div class="info">
                                    <a class="btn_reply count" href="javascript:void(0);">Reply <span id="answer_reply_count_<?=$row['idx']?>"><?=number_format($answer_reply_count)?></span></a>
                                    <?php if($is_member) { ?><a class="btn_reply reply_<?=$row['idx']?> reply_write" href="javascript:void(0);">Write a reply</a><?php } ?><!-- 클릭하면 .area_comment_input display:block-->
                                    <ul class="thums">
                                ...
                                <div class="area_comment_input area_comment_input_<?=$row['idx']?>" style="display: none;">
                                    <textarea id="input_reply_<?=$row['idx']?>" name="input_reply_<?=$row['idx']?>" placeholder="Please register a reply."></textarea>
                                    <div class="comment_bottom">
                                        ...
                                        <div class="list_btn">
                                            <button class="btn_cancle reply_<?=$row['idx']?>">Cancel</button><!-- 클릭하면 .area_comment_input display:none-->
                            ...
                            <div class="area_comment area_comment_<?=$row['idx']?>" <?=$add_style?>>
                                <ul class="list_comment list_comment_<?=$row['idx']?>">
                                    <?php
                                    $sql_reply = " select re.*, mb.mb_no, mb.mb_nick, mb.mb_category from g5_community_answer_reply as re left join g5_member as mb on re.mb_id = mb.mb_id where community_an_idx = {$row['idx']} and del_yn is null order by wr_datetime desc ";
                                    ...
                                    <li>
                                        ...
                                        <ul class="reply_<?=$reply['idx']?> user_list answer02 sm">
                                            ...
                                            <li>RFQs <em class="blue"><?=inquiryCount($reply['mb_id'])?></em></li>
                                            ...
                                            <?php } ?>
                                            <?php if(!$self) { // 내가쓴글아님?>
                                            <li onclick="reportOpen('<?=$reply['mb_id']?>', 'g5_community_answer_reply', '<?=$reply['idx']?>')">Report</li>
                                            <?php } ?>
                                        </ul>
                                        <!-- //토글메뉴 -->
                                        </div>
		...
		<div id="area_popular">
			<div class="area_write">
				<a href="<?php echo G5_BBS_URL ?>/community_write.php"><span>Writing</span></a>
			</div>
			<div class="list_best">
			<h3>TOP 10</h3>
			<ul>
		...
		<div class="area_btn"><a class="btn_list" href="<?=G5_BBS_URL?>/community_list.php?op=<?=$op?>"><span>List</span></a></div>
...
function community_action_chk(op, idx) {
    var txt = ' post';
    if(op == 'answer') { txt = ' comment'; }

    swal({
        text: "Are you sure you want to delete" + txt + "?",
        icon: "warning",
        buttons: {
            defeat: "Confim",
            cancel: "Cancle",
