var url = "https://www.podosea.com:5003"; // chat.js 충돌 오류로 const에서 var로 변경
        ...
        for (var i = 0; i < data.chats.length; i++) {
            ...
            if (i == 0) {
                ...
                if(data.chats[i].msgdate.substring(0,10) != data.chats[i].last_msgdate.substring(0,10) || data.chats[i].msg_count < 20) { // 21.05.20 21번째 메세지(scrollTopView data) 날짜와 다를 경우 날짜 표시
                    chat_view_html += '<div class="data today get_message">'+msgdate_format+'</div>';
                }
            }
            else { // 21.05.20 이전메세지와 다음메세지의 날짜가 다를 경우 날짜 표시
                if(data.chats[i].msgdate.substring(0,10) != data.chats[i-1].msgdate.substring(0,10)) {
                    chat_view_html += '<div class="data today">'+msgdate_format+'</div>';
                }
            }

            // 상대방 메세지
            if (data.user_id != data.chats[i].user_id) {
                chat_view_html += `<div class="receive">`;
                chat_view_html += `<div class="area_img">${data.user_img}</div>`;
                chat_view_html += `<div class="area_msg">`;
                chat_view_html += `<div class="name">${document.getElementById('room_name').value}</div>`; // 일반은 닉네임 or 아이디 / 기업은 회사명
                chat_view_html += `<div class="cont_box msg `+img_class+`">`;
                if(img_class == 'file') { // 파일
                    chat_view_html += `<a class="file_box" href="https://www.podosea.com/node/uploads/${data.chats[i].server_file_name}" download="${data.chats[i].file_name}" width="100%">
                                            <div class="icon"><img src="https://www.podosea.com/img/icon_chat_file.svg"></div>
                                            <div class="file_info">
                                                <div class="subject">${data.chats[i].file_name}</div>
                                                <div class="size">용량:4.38MB</div>
                                            </div>
                                       </a>`;
                } else { // 그 외
                    chat_view_html += `${msg}`;
                }
                chat_view_html += `</div>`; // ==.conb_box 닫음
                chat_view_html += `<div class="time date">${convert12H(data.chats[i].msgdate.substring(11, 16))}</div>`; // 21.05.14 채팅방 재입장 시 시간 표시 문제로 수정 (수정 전 코드 : data.chats[i].msgdate)
                chat_view_html += `</div>`; // ==.area_msg 닫음
                chat_view_html += `</div>`; // ==.receive 닫음
            }
            // 내 메세지
            else {
                chat_view_html += `<div class="send">`;
                chat_view_html += `<div class="area_msg">`;
                chat_view_html += `<div class="cont_wrap `+img_class+`">`;
                chat_view_html += `<div class="cont_box msg `+img_class+`">`;
                if(img_class == 'file') { // 파일
                    chat_view_html += `<a class="file_box" href="https://www.podosea.com/node/uploads/${data.chats[i].server_file_name}" download="${data.chats[i].file_name}" width="100%">
                                            <div class="icon"><img src="https://www.podosea.com/img/icon_chat_file.svg"></div>
                ...
                if (data.chats[i].read_count != 0) { // 읽음 표시
                    chat_view_html += `<i class="read read-status" id="read-status${data.chats[i].id}">${data.chats[i].read_count}</i>`;
                }
                chat_view_html += `</div>`; // ==.cont_wrap 닫음
                chat_view_html += `<div class="time date">${convert12H(data.chats[i].msgdate.substring(11, 16))}</div>`; // 21.05.14 채팅방 재입장 시 시간 표시 문제로 수정 (수정 전 코드 : data.chats[i].msgdate)
                chat_view_html += `</div>`; // ==.area_msg 닫음
                chat_view_html += `</div>`; // ==.send 닫음
            }
        }
...
socket.on('scrollTopView', function (data) {
    ...
    if (0 < data.chats.length) {
        for (var i = 0; i < data.chats.length; i++) {
            ...
            if (data.user_id != data.chats[i].user_id) {
                chat_view_html += `<div class="receive">`;
                chat_view_html += `<div class="area_img">${data.chats[i].user_img}</div>`;
                chat_view_html += `<div class="area_msg">`;
                chat_view_html += `<div class="name">${document.getElementById('room_name').value}</div>`; // 일반은 닉네임 or 아이디 / 기업은 회사명
                chat_view_html += `<div class="cont_box msg `+img_class+`">`;
                if(img_class == 'file') { // 파일
            ...
            // 내 메세지
                ...
                chat_view_html += `</div>`; // ==.cont_box 닫음
                if (data.chats[i].read_count != 0) { // 읽음 표시
                    chat_view_html += `<i class="read read-status" id="read-status${data.chats[i].id}">${data.chats[i].read_count}</i>`;
                }
                chat_view_html += `</div>`; // ==.cont_wrap 닫음
...
socket.on('chat_view', function (data) {
    ...
    console.log("chat_view : chat_send after");
    const user_id = document.getElementById("user_id").value;
    const chat_msg = document.getElementById("chat-msg");
    ...
    if (user_id != data.user_id) {
        chat_view_html += `<div class="receive">`;
        chat_view_html += `<div class="area_img">${g_user_img}</div>`;
        chat_view_html += `<div class="area_msg">`;
        chat_view_html += `<div class="name">${document.getElementById('room_name').value}</div>`; // 일반은 닉네임 or 아이디 / 기업은 회사명
        chat_view_html += `<div class="cont_box msg `+img_class+`">`;
        if(img_class == 'file') { // 파일
            chat_view_html += `<a class="file_box" href="https://www.podosea.com/node/uploads/${data.server_file_name}" download="${data.file_name}" width="100%">
                                    <div class="icon"><img src="https://www.podosea.com/img/icon_chat_file.svg"></div>
                                    <div class="file_info">
        ...
        chat_view_html += `</div>`; // ==.cont_box 닫음
        chat_view_html += `<div class="time date">${convert12H(data.time.substring(3, 8))}</div>`; // 21.05.14 채팅방 재입장 시 시간 표시 문제로 수정 (수정 전 코드 : data.chats[i].msgdate)
        chat_view_html += `</div>`; // ==.area_msg 닫음
        chat_view_html += `</div>`; // ==.receive 닫음
    }
    // 내 메세지
        ...
        if (readLength != 0) { // 읽음 표시
            chat_view_html += `<i class="read read-status" id="read-status${data.msg_idx}">${readLength}</i>`;
        }
        chat_view_html += `</div>`; // ==.cont_wrap 닫음
        chat_view_html += `<div class="time date">${convert12H(data.time.substring(3, 8))}</div>`; // 21.05.14 채팅방 재입장 시 시간 표시 문제로 수정 (수정 전 코드 : data.chats[i].msgdate)
        chat_view_html += `</div>`; // ==.area_msg 닫음
...
try {
    ...
    document.getElementById("chat-msg").addEventListener('scroll', function () {
        ...
        if (Math.round(chat_msg.scrollTop) == 0 && is_chat_scroll) {
            // console.log("이전페이지");
            var data = {room_idx: room_idx, msg_idx: msg_idx, user_img: g_user_img};
            // console.log(data);
