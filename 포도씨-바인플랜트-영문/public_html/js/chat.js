var url = "https://www.podosea.com:5006"; // chat.js 충돌 오류로 const에서 var로 변경
...
socket.on('chat_view', function (data) {
    ...
    if (user_id != data.user_id) {
        ...
        if(img_class == 'file') { // 파일
            chat_html += `<a class="file_box" href="https://www.podosea.com/bbs/file_download.php?mode=chat&temp=${data.server_file_name}&real=${data.file_name}" width="100%">
                                <div class="icon"><img src="https://www.podosea.com/img/icon_chat_file.svg"></div>
                                <div class="file_info">
                                    <div class="subject">${data.file_name}</div>
                                    <div class="size">size ${file_size}</div>
                                </div>
                           </a>`;
        } else if(img_class == 'img') {
            chat_html += `<a href="https://www.podosea.com/bbs/file_download.php?mode=chat&temp=${data.server_file_name}&real=${data.file_name}">`
            chat_html += `${msg}`;
            chat_html += `</a>`;
        } else { // 그 외
            chat_html += `${msg}`;
...
socket.on('filesize_check', function (data) {
    ...
    alert('The file exceeds the maximum capacity of 10 MB.');
});
...
function getChatData(data, func) {
    ...
    for (var i = 0; i < data.chats.length; i++) {
        ...
        if (data.user_id != data.chats[i].user_id) {
            ...
            chat_html += `<div class="cont_box msg `+img_class+`">`;
            if(img_class == 'file') { // 파일
                chat_html += `<a class="file_box" href="https://www.podosea.com/bbs/file_download.php?mode=chat&temp=${data.chats[i].server_file_name}&real=${data.chats[i].file_name}" width="100%">
                                    <div class="icon"><img src="https://www.podosea.com/img/icon_chat_file.svg"></div>
                                    <div class="file_info">
                ...
                chat_html += `<a href="https://www.podosea.com/bbs/file_download.php?mode=chat&temp=${data.chats[i].server_file_name}&real=${data.chats[i].file_name}">`
                chat_html += `${msg}`;
                chat_html += `</a>`;
            } else { // 그 외
                chat_html += `${msg}`;
        ...
        // 내 메세지
            ...
            chat_html += `<div class="cont_box msg `+img_class+`">`;
            if(img_class == 'file') { // 파일
                //<a class="file_box" href="https://www.podosea.com/node/uploads/${data.chats[i].server_file_name}" download="${data.chats[i].file_name}" width="100%">
                //<a class="file_box" href="https://www.podosea.com/bbs/file_download.php?mode=chat&temp=${data.chats[i].server_file_name}&real=${data.chats[i].file_name}" width="100%">
                chat_html += `<a class="file_box" href="https://www.podosea.com/bbs/file_download.php?mode=chat&temp=${data.chats[i].server_file_name}&real=${data.chats[i].file_name}" width="100%">
                                    <div class="icon"><img src="https://www.podosea.com/img/icon_chat_file.svg"></div>
                                    <div class="file_info">
                                        <div class="subject">${data.chats[i].file_name}</div>
