    ...
    $.ajax({
        ...
        success: function (data) {
            if(data) {
                var msg = '';
                if(mode == 'add') {
                    msg = 'Register as Interested Friends.';
                } else {
                    msg = 'Remove as Interested Friends.';
                }
    ...
    $.ajax({
        ...
        success: function (data) {
            if(data) {
                var msg = '';
                if(mode == 'add') {
                    msg = 'Register as Interested Companies.';
                } else {
                    msg = 'Remove as Interested Companies.';
                }
...
function memberCheck(category, mb_no) {
    if(category == '일반') {
        swal('Only corporate members can use it.');
    } else {
        var param = '';
...
function reportAction() {
    ...
    if(reason == '') {
        swal('Please select a reason for reporting.');
        is_check = false;
        return false;
    }
    ...
    $.ajax({
        ...
        success: function(data) {
            if(data) {
                swal('Report completed!\nAction will be taken immediately after confirmation by the administrator.')
                .then(()=>{
                    is_check = false;
...
function lengthChk(el) {
    ...
    // var reg_ko = /[ㄱ-ㅎㅏ-ㅣ가-힣\s]/g; // 한글체크
    // var reg_eng = /[a-zA-Z\s]/g; // 영문체크

    if(reg.test(str)) {
        if(length > 20) {
            swal("Exceeded the number of characters.\n(Up to 20 characters)");
...
function myInquiryDelete(mode) {
    ...
    if(idx == '') {
        swal('Please select the data to be deleted.');
        return false;
    }
...
// 입력 체크
const checkEmptyInputs = (elems, errorMessage) => {
...
function profileCheck(data, menu, flag, mb_id) {
    if(mb_id == '' || mb_id == undefined) {
        swal("Please try again after logging in.")
        .then(()=>{
