...
function itforoneMailer($from,$from_name,$to,$to_name,$subject,$content){
    ...
    //$post = funcCharsetConvert($post, "UTF-8", "EUC-KR"); // EUC-KR 을 UTF-8로 변환
    ...
    // $data = funcCharsetConvert($data, "UTF-8", "EUC-KR"); // UTF-8을 EUC-KR로 변환
...
/******************************************************************************
- 함수명 : funcCharsetConvert($str, $to_charset, $from_charset)
- 용  도 : 문자열의 캐릭터셋을 변환하는 함수. - 2016.08.23 추가
- 속성값 : $str          - 캐릭터셋을 변환할 문자열 또는 문자열 배열.
$to_charset   - 변환하고자하는 캐릭터셋.
$from_charset - 문자열 또는 문자열 배열의 원본 캐릭터셋.
- 결과값 : 캐릭터셋이 변환된 문자열 또는 문자열 배열.
 ******************************************************************************/

function funcCharsetConvert($str, $to_charset, $from_charset) {
    if(is_array($str)) {
        $rslt = array();

        foreach($str as $key => $val) {
            $rslt[$key] = is_array($val) ? funcCharsetConvert($val, $to_charset, $from_charset) : mb_convert_encoding($val, $to_charset, $from_charset);

            unset($key, $val);
        }
    }
    else {
        $rslt = mb_convert_encoding($str, $to_charset, $from_charset);
    }
