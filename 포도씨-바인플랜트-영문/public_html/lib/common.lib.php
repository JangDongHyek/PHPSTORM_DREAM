...
function loginCheck($mb_id, $category) {
    ...
    $mb = get_member($mb_id);
    if($mb['mb_category'] != $category) { // 기업회원만 접근
        alert('Not the correct path.', G5_URL);
    }
}
...
function bunkerHistory($mb_id, $mode, $bunker, $contents, $rel_mb_id, $rel_table, $rel_idx, $etc, $etc2) {
    ...
    if($mode == '적립') {
        ...
        if($etc != 'withdraw' && $etc != 'gift') { // 출금/선물은 보너스 벙커 사용하지 않음
            if(!empty($mb['mb_bunker_bonus']) && $mb['mb_bunker_bonus'] >= $bunker) { // 보너스 BUNKER가 0이 아니고 차감 BUNKER보다 많거나 같을 때
                ...
                // 내 BUNKER 잔액 확인
                if(!empty($mb['mb_bunker']) && $mb['mb_bunker'] >= $temp_bunker) { // 내 BUNKER가 0이 아니고 남은 차감 BUNKER보다 많거나 같을 때
                    $re_bonus = 0; // 보너스 잔여
    ...
    if(!empty($etc2)) { $insert_add .= ", etc2 = '{$etc2}' "; }

    //if($mode == '적립') { $mode = 'Saving'; }
    //else if($mode == '차감') { $mode = 'Deduction'; }
...
// 보유 벙커 랭킹
function getRanking($mb_id) {
    $sql = "SELECT mb_id, mb_bunker, ranking 
            FROM (SELECT mb_id, mb_bunker, CASE
            WHEN @pv = mb_bunker THEN @vrank 
            WHEN @pv := mb_bunker THEN @vrank := @vrank + 1 
            END AS ranking 
            FROM g5_member as mb, (SELECT @vrank := 0, @pv := NULL) as r 
            where mb_bunker != 0
            ORDER BY mb_bunker DESC) AS ranking;";
    $rlt = sql_query($sql);

    $ranking = 0; // 보유 벙커 없으면 0으로 표시 (임시)
    while($rank = sql_fetch_array($rlt)) {
        if($rank['mb_id'] == $mb_id) {
...
function profileUpdateCheck($mb_id, $mb_level) {
    ...
    if($mb_level == 2) { // mb_level == 2 (일반회원)
        ...
        $profile = sql_fetch(" select count(*) as cnt from g5_member where mb_id = '{$mb_id}' and mb_hashtag is not null and mb_hashtag != '' ")['cnt']; // 해시태그까지 입력완료해야 프로필 작성 완료한걸로 판단
    }
