...
function valid_mb_id($reg_mb_id)
{
    if (preg_match("/[^0-9a-z_]+/i", $reg_mb_id))
        return "Enter only English letters, numbers, and _ for the ID.";
    else
        return "";
}
...
function count_mb_id($reg_mb_id)
{
    if (strlen($reg_mb_id) < 3)
        return "Please enter at least 3 characters for your member ID.";
    else
        return "";
}
...
function valid_mb_email($reg_mb_email)
{
    if (!preg_match("/([0-9a-zA-Z_-]+)@([0-9a-zA-Z_-]+)\.([0-9a-zA-Z_-]+)/", $reg_mb_email))
        return "The E-mail address is not in the correct format.";
    else
        return "";
}
...
function exist_mb_company_num($reg_mb_company_num, $reg_mb_id)
{
    global $g5;
    $row = sql_fetch(" select count(*) as cnt from {$g5['member_table']} where mb_company_num != '' and mb_company_num = '$reg_mb_company_num' and mb_id <> '$reg_mb_id' ");
    if ($row['cnt'])
        return "This is a business registration number that already exists.";
    else
        return "";
}
