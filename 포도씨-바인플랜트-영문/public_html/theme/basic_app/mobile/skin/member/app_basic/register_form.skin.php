...
<div class="mbskin">
	...
	<article class="box-article">
		...
		<div id="area_join">
			<div id="join_info">
			<h2><?php if($w == ""){ ?>Sign up as general member<? }else { ?>Check and edit member information<?php } ?> <p></h2>
				<div class="box-body">
					...
					<dl class="row">
						...
						<dd>
							<div class="input">
								<input type="text" name="mb_name" value="<?=$mb_name?>" id="reg_mb_name" class="regist-input f_empty" <?=$readonly?> placeholder="Enter name.">
							</div>
						</dd>
						<dd class="error col-xs-12"></dd>
					</dl>
					...
					<!--<dl class="row">
						...
						<dd>
							<div class="input">
								<input type="text" name="mb_nick" value="<?php /*echo $member['mb_nick'] */?>" id="reg_mb_nick" class="regist-input f_empty" placeholder="닉네임을 입력해 주세요.">
							</div>
						</dd>
						<dd class="error col-xs-12"></dd>
					</dl>-->
					...
					<dl class="row hp">
						...
						<dd>
							<div class="input <?php if($w == "") { ?>v1<?php } ?>">
								<input type="text" name="mb_email" id="reg_mb_email" class="regist-input f_empty email" minlength="3" maxlength="50" placeholder="Enter email." value="<?=$mb_email?>">
								<?php if($w=='') { ?><button type="button" class="btn_hp" onclick="email_certification();">Authenticate</button><?php } ?>
							</div>
						</dd>
						<dd class="error col-xs-12"></dd>
					</dl>
					...
					<dl class="row hp">
						...
						<dd>
							<div class="input <?php if($w == "") { ?>v1<?php } ?>">
								<input type="tel" name="mb_hp" value="<?=$mb_hp?>" id="reg_mb_hp" class="regist-input" placeholder="휴대폰번호를 입력해 주세요." minlength="10" maxlength="13">
                                <?php if($w == "") { ?><button type="button" class="btn_hp" onclick="hpCertify();">인증번호</button><?php } ?>
							</div>
                            <?php if($w == "") { ?>
					...
					<dl class="row">
						<dt>Please select the field of business in which you are active</dt>
						<dd>
							<div class="input">
								<select id="mb_active_business" name="mb_active_business">
        ...
        <h2>Terms of Service Agreement</h2>
            ...
            <div class="box-body">
                ...
                <dl class="row agree-row">
                    ...
                    <dd class="col-xs-4 text-right"><input type="button" value="View" class="btn btn-agr"></dd>
	...
	<ul>
		<li> - The Terms of Service shall be effective as of January 5, 2022.</li>
                ...
                <dl class="row agree-row">
                    ...
                    <dd class="col-xs-3 text-right"><input type="button" value="View" class="btn btn-agr"></dd>
...
function hpCertify() {
    ...
    if($.trim($('#reg_mb_hp').val()).length != 13) {
        swal('Please check your mobile phone number.');
        return false;
    }
    ...
    $.ajax({
        ...
        success: function(data) {
            if(data == 'success') {
                swal('A verification code has been sent.')
                .then(()=>{
...
$(function (){
	...
	$("#reg_mb_hp").keyup(function (){
		...
		if (regHp.test(mb_hp)){
			...
			state.removeClass("pas").addClass("err");
			err.addClass("on").html("Please enter only 10 to 12 digits for your mobile phone number.");
		}
...
function fregisterform_submit(f){
    ...
    if(f.w.value == "") {
        ...
        if (f.mb_password.value != f.mb_password_re.value) {
            swal('The passwords are not the same.');
    ...
    if(f.mb_active_business.value == '') {
        swal('Please select the field of business.');
        return false;
    }
    ...
    <?php /*if($w == "") { ?>
    if(f.cert_no.value == "") {
        swal("Enter the verification code.");
        return false;
    }
    if($("#cert_no").is(".chk_err") === true) {
        swal("Check the verification code.");
        return false;
    }
    ...
    $.ajax({
        ...
        success: function(data) {
            if (data == 'no_cert') {
                swal('The mobile phone number has been changed.\n Please proceed with a new authentication.');
                submit = false;
            }
        },
    });
