...
<? if($name=="notice") { ?>
	<body<?php echo isset($g5['body_script']) ? $g5['body_script'] : ''; ?> class="notice">
<?}?>
...
<div id="bo_list<?php if ($is_admin) echo "_admin"; ?>">
    ...
    <div class="bo_fx">
        <div id="bo_list_total">
            <span>Total <?php echo number_format($total_count) ?> Posts</span> / Page <?php echo $page ?>
        </div>
    ...
    <div class="tbl_head01 tbl_wrap">
        ...
        <tr>
            <?php if ($is_checkbox) { ?>
            <th scope="col">
                <label for="chkall">All current page posts</label>
        ...
        <?php } ?>
        <?php if (count($list) == 0) { echo '<tr><td colspan="'.$colspan.'" class="empty_table">There are no registered post.</td></tr>'; } ?>
        </tbody>
        </table>
    </div>

    <?php if ($list_href || $is_checkbox || $write_href) { ?>
    <div class="bo_fx bottom">
        <ul class="btn_bo_adm">
            <?php if ($list_href) { ?>
        ...
        <ul class="btn_bo_user">
            <li><?php if ($write_href) { ?><a href="<?php echo $write_href ?>" class="btn_b02">등록</a><?php } ?></li>
        </ul>
    </div>
    <?php } ?>
    </form>
...
<fieldset id="bo_sch">
    ...
    <input name="stx" value="<?php echo stripslashes($stx) ?>" placeholder="Search" required id="stx" class="required frm_input" size="20" maxlength="20">
