...
function get_paging($write_pages, $cur_page, $total_page, $url, $add="")
{
    ...
    if ($start_page > 1) $str .= '<a href="'.$url.($start_page-1).$add.'" class="pg_page pg_prev">이전</a>';
    ...
    if ($total_page > $end_page) $str .= '<a href="'.$url.($end_page+1).$add.'" class="pg_page pg_next">다음</a>';
...
function get_member($mb_id, $fields='*')
{
    ...
    //$member = sql_fetch(" select $fields from {$g5['member_table']} where mb_id = TRIM('$mb_id') ");
    $member = sql_fetch(" select mem.*,f.token from {$g5['member_table']} mem left join g5_fcm f on mem.mb_id = f.mb_id where mem.mb_id = TRIM('$mb_id') ");

    if ($member['mb_8'] ==2){
        $member['mb_nick'] = '탈퇴한 사용자';
    }else{
        $member['mb_nick'] = $member['mb_nick'];
    }
...
//이미지 사이즈 지정해서 넣어주기
function size_image($file,$new_file_width,$new_file_height,$dest_path,$multi_or_one,$i){

    // 다중 업로드 시
    if ($multi_or_one == 'multi') {
        $file_type = $file['type'][$i];
        $file_info = getimagesize($file['tmp_name'][$i]);
        $file_width = $file_info[0]; //이미지 가로 사이즈
        $file_height = $file_info[1]; //이미지 세로 사이즈
        $tmp_file = $file['tmp_name'][$i];
        $new_quality = (int)($file['size'][$i]);
        //하나만 업로드 시
    }else{
        $file_type = $file['type'];
        ...
        $new_quality = (int)($file['size']);

    }

    if ($file_type == ""){
        die("파일이 정상적으로 넘어오지 않았습니다.");
    }
    ...
    if ($image != "") {
        fn_imagejpeg($image, $dest_path, $new_file_width, $new_file_height, $file_width, $file_height, $new_quality);
    }else{
        die('이미지 확장자가 변경되어 사용할 수 없는 이미지입니다.');
    }
}

function common_code($code,$name,$html,$add=''){

    global $g5;


    $sql = "select * from {$g5['code_table']} where {$name} = '{$code}' {$add} and code_use = '1' order by code_number ";
    $result = sql_query($sql);

    // 23.04.25 아래 배열아닌데 배열로넣어주서 오류난거 수정 wc
    $arr = array();
    ...
    if ($html == 'html') {
        ...
        for ($i = 0; $row = sql_fetch_array($result); $i++) {
            ...
            $arr .= '<option value="' . $row['code_idx'] . '">' . $row['code_name'] . '</option>';

        }
    }else if ($html == 'json' || $html == ''){

        for ($i = 0; $row = sql_fetch_array($result); $i++) {
            $arr[] =  array('idx' => $row['code_idx'], 'name' => $row['code_name'],'p_idx' =>  $row['code_p_idx'],'type' =>  $row['code_ctg']);
        }
    }
...
//1차 카테고리 찾기
function p_common_code($code){

    global $g5;


    $sql = "select * from {$g5['code_table']} where code_idx = '{$code}' and code_use = '1' order by code_p_idx";
    $result = sql_fetch($sql);

    $sql = "select * from {$g5['code_table']} where code_idx = '{$result['code_p_idx']}' and code_use = '1' order by code_p_idx";
    $row = sql_fetch($sql);

    $arr =  array('idx' => $row['code_idx'], 'name' => $row['code_name'],'p_idx' => $row['code_p_idx']);
...
function like_cnt($idx,$table){
    global $g5;
    //like 갯수
    $sql = "select count(li_idx) cnt from {$g5['like_table']} where ta_idx = '{$idx}' and li_table = '{$table}' ";
    $like_cnt = sql_fetch($sql);

    return $like_cnt['cnt'];
}

function comp_apply_cnt($idx){

    //참여자 수
    $sql = "select count(ap_idx) cnt from new_competition_apply where ap_cp_idx = '{$idx}' ";
    $cnt = sql_fetch($sql);
...
function banner($place){
    ...
    $sql = "select * from new_adm_banner where ba_place = '{$place}' and ba_use = 1 order by ba_number desc ";
    $result = sql_query($sql);
    $html = "";
//    $html .= '<div class="bx-viewport" style="width: 100%; overflow: hidden; position: relative; height: 510px;">';
    for($i = 0; $btm_img = sql_fetch_array($result); $i++ ){

        $html .= '<li>
                    <a href="#" id="banner_'.$i.'" data-link = "'.$btm_img['ba_link'].'" onclick="open_tab(this,'.$btm_img['ba_new_tab'].')" >
                        <img src="'.G5_DATA_URL.'/banner/'.$btm_img['image'].'" >
                    </a>
                </li>
                ';
    }
...
function goSms($reserv_phone, $send_phone, $msg)
{
    ...
    //6회부터 오류창 표시
    if ($result['cnt'] > 5){
        die(json_encode(array( 'msg' => "하루 인증횟수(5회)를 초과하였습니다.")));
        exit();
    }
    if(!$tran_callback1){
...
function review_cnt($idx){

    $sql = "select count(idx) cnt from new_payment_review where ta_idx = '{$idx}' ";
    $result = sql_fetch($sql)['cnt'];
    $result = number_format($result);
    return $result;

}

function review_avg($idx){

    $sql = "select avg(rating) as avg from new_payment_review where ta_idx = '{$idx}' ";
    $result = sql_fetch($sql)['avg'];
    $result = number_format($result,1);
    return $result;

}
function payment_history($mb_id, $content, $amt, $total_amt,$bo_table,$is_mileage,$content_idx,$bo_idx,$idx_table){
    if ($is_mileage == 'Y'){
        $total = "ph_total_mileage = {$total_amt}";

    }else{
        $total = "ph_total_amt = {$total_amt}";

    }

    //히스토리 내역에 추가
    $sql = "insert into new_payment_history set mb_id = '{$mb_id}', ph_content_idx = '{$content_idx}',ph_content = '{$content}', ph_amt = '{$amt}',
               {$total} ,ph_bo_table = '{$bo_table}',ph_bo_idx = '{$bo_idx}',ph_bo_idx_table = '{$idx_table}', wr_datetime = '".G5_TIME_YMDHIS."' ";
    sql_query($sql);

}

function point_history($mb_id, $content_idx,$content, $amt, $total_amt, $type){

    //히스토리 내역에 추가
    $sql = "insert into new_point_history set mb_id = '{$mb_id}', p_content_idx = '{$content_idx}', p_content = '{$content}'
            , p_point = '{$amt}', p_total_point = '{$total_amt}', p_type = '{$type}', wr_datetime = '".G5_TIME_YMDHIS."' ";
    sql_query($sql);
}
...
function alimtalk($hp,$arr,$type){

    //23.11.24 알림톡제한 아이피랑 번호 같은거 하루 3번만 wc
    $ip       = $_SERVER['REMOTE_ADDR'];
    $tmp_sql = " select count(*) as cnt from alimtalk where mb_hp = '{$hp}' and ip = '$ip' and DATE(wr_datetime) = DATE(NOW()); ";
    $tmp_row = sql_fetch($tmp_sql)['cnt'];
    if($tmp_row>=3){

        $log_dir = '/home/jobgo/public_html/lib/';
        ...
        $logfile = fopen( $log_dir . "alimtalk.log", "a+" );
        fwrite( $logfile,"************************************************\r\n");
        fwrite( $logfile,"PageCall  : ".$PageCall."\r\n");
        fwrite( $logfile,"hp  : ".$hp."\r\n");
        fwrite( $logfile,"type  : ".$type."\r\n");
        fwrite( $logfile,"ip  : ".$ip."\r\n");
        fwrite( $logfile,"arr  : ".print_r($arr,true)."\r\n");
        fwrite( $logfile,"************************************************");
        fclose( $logfile );
    ...
    //    $_apiURL = "https://api.wideshot.co.kr/api/v1/message/alimtalk/";
            $_apiURL = 'https://apimsg.wideshot.co.kr/api/v1/message/alimtalk/';
    //    $_header = array('sejongApiKey:cVhmRlZlZHNkNThYcnhwbllGTGEycmIvMkNTOVNvbUZnV1VHSnowOFFkV0ZDNzNTNmZFRXBIeUtuNGE1b2VhRE1XbTRYTG8rN2VmdS9OTzltaGZQS0E9PQ==');
            $_header = array('sejongApiKey:OUF0RVNQb2QyL1l5MmhicFIvb0xzK1NYa1NQMzh3SXl3SWVkTXh6YUx3ZndQYkNURHoxNURReUN1SFQ3bWN5Ug==');

            $send_code = rand(0000000, 9999999);
            $form = alimtalk_form($hp,$arr,$type,$send_code);
            $oCurl = curl_init();
    ...
    //    curl_setopt ($oCurl, CURLOPT_POSTFIELDS, "plusFriendId=@ddrcall01&senderKey=0c9258d222e4ce1ec29800cd5b52d7c47be4b27f&templateCode=SJTW1_4682_1&contents=매칭완료&receiverTelNo=01082268928&userKey=yujin689&Title=매칭완료");
            curl_setopt ($oCurl, CURLOPT_POSTFIELDS, http_build_query($form));
    ...
    //    $error_msg = curl_error($oCurl);
            ...
            //23.11.24 알림톡 보내는거 db남기기 wc

            $sql = "insert into alimtalk (mb_hp, wr_contents, ip) value ('{$hp}', '{$type}', '{$ip}')";
            sql_query($sql);
...
function alimtalk_form($hp,$arr,$type,$send_code){

//템플릿 변수를 제외한 모든 부분 등록된 템플릿과 틀리면 안됌
//공백도 주의할 것.
if ($type == "register") {

    $templateCode = 'SJTW1_9774_1';
    $title = '회원가입';
    $content =  '안녕하세요, '.$arr["mb_nick"].'님,
고객님의 회원가입이 정상적으로 완료되었습니다.
청년재능 거래 플랫폼 JOBGO에 가입해 주셔서 감사합니다.
앞으로 더욱 빛날 고객님의 미래, JOBGO가 함께하겠습니다.
-> 잡고 바로가기: https://jobgo.ac';

}else if($type == 'comment_update'){

     $templateCode = 'SJTW1_9775_1';
    $title = '문의등록';
    $content =  '안녕하세요, '.$arr["talent_nick"].'님,
고객님의 '.$arr["talent_title"].' 재능에 문의사항이 등록되었습니다.';
    $attachment = '{ "attachment": {

                             "button": [{ 
                                 "name": "문의사항 확인",
                                 "type": "WL",
                                 "url_pc":  "https://www.jobgo.ac/bbs/my_inquiry.php",
                                 "url_mobile": "https://www.jobgo.ac/bbs/my_inquiry.php"
                                 }]
                       }
                   }';
    ...
    $templateCode = 'SJTW1_9776_1';
    $title = '문의답변등록';
    $content =  '안녕하세요, '.$arr["comment_nick"].'님,
고객님의 '.$arr["talent_title"].' 문의에 대한 답변이 등록되었습니다.';
    $attachment = '{ "attachment": {
...
}else if($type == 'talent_write'){

    $templateCode = 'SJTW1_9777_1';
    $title = '재능등록';
    $content =  '안녕하세요, '.$arr["mb_nick"].'님,
고객님의 재능이 등록되었습니다.
※ 판매 규정에 어긋나는 내용이 발견된 경우, 임의 수정/삭제가 진행될 수 있습니다.';

}else if($type == 'talent_buy'){

    $templateCode = 'SJTW1_9778_1';
    $title = '재능구매';
    $content =  '안녕하세요, '.$arr["mb_nick"].'님,
'.$arr["talent_title"].' 건에 대한 주문이 정상 접수되었습니다.';
    $attachment = '{ "attachment": {

                             "button": [{ 
                                 "name": "주문내역 상세보기",
                                 "type": "WL",
                                 "url_pc":  "https://www.jobgo.ac/bbs/my_item.php?tab=3",
                                 "url_mobile": "https://www.jobgo.ac/bbs/my_item.php?tab=3"
                                 }]
                       }
                   }';
}else if($type == 'buy_req'){

    $templateCode = 'SJTW1_9780_1';
    $title = '구매요청';
    $content =  '안녕하세요, '.$arr["mb_nick"].'님,
고객님의 '.$arr["talent_title"].' 재능에 구매요청이 들어왔습니다.
의뢰인과 소통하며 즐거운 거래되시기 바랍니다.';
    $attachment = '{ "attachment": {

                             "button": [{ 
                                 "name": "주문내역 상세보기]",
                                 "type": "WL",
                                 "url_pc":  "https://www.jobgo.ac/bbs/my_item.php?tab=2",
                                 "url_mobile": "https://www.jobgo.ac/bbs/my_item.php?tab=2"
                                 }]
                       }
                   }';
}else if($type == 'buy_change'){

    $templateCode = 'SJTW1_9781_1';
    $title = '주문상태 변경(작업상태)';
    $content =  '안녕하세요, '.$arr["mb_nick"].'님
구매하신 재능의 작업이 시작되었습니다.
재능인과 지속적인 연락으로 만족스러운 거래 되시길 바랍니다.';
    $attachment = '{ "attachment": {

                             "button": [{ 
                                 "name": "판매자 문의 링크",
                                 "type": "WL",
                                 "url_pc":  "https://www.jobgo.ac/bbs/message.php?ta_idx='.$arr["ta_idx"].'&room_name='.urldecode($arr["seller_nick"]).'&sub_id='.$arr["seller_id"].'}&ailmtalk=Y",
                                 "url_mobile":  "https://www.jobgo.ac/bbs/message.php?ta_idx='.$arr["ta_idx"].'&room_name='.urldecode($arr["seller_nick"]).'&sub_id='.$arr["seller_id"].'}&ailmtalk=Y"
                                 }]
                       }
                   }';
}else if($type == 'talent_comp'){

    $templateCode = 'SJTW1_9814_1';
    $title = '재능거래 완료(최종)';
    $content =  '안녕하세요, '.$arr["mb_nick"].'님
고객님의 재능거래가 완료되었습니다.
거래가 마음에 드셨다면 구매한 재능에대한 후기를 남겨주세요.';
    $attachment = '{ "attachment": {

                             "button": [{ 
                                 "name": "후기 링크",
                                 "type": "WL",
                                 "url_pc":  "https://www.jobgo.ac/bbs/item_view.php?idx='.$arr['idx'].'",
                                 "url_mobile":  "https://www.jobgo.ac/bbs/item_view.php?idx='.$arr['idx'].'"
                                 }]
                       }
                   }';
}else if($type == 'cashe'){

    $templateCode = 'SJTW1_9783_1';
    $title = '캐쉬환전요청';
    $content =  '안녕하세요, '.$arr["mb_nick"].'님
고객님의 재능거래가 완료되었습니다.
생성된 캐시의 환전요청을 해주시기 바랍니다.
※ 캐시 환전은 요청일 기준 1~2일정도 소요될 수 있습니다.';
    $attachment = '{ "attachment": {

                             "button": [{ 
                                 "name": "캐시환전 링크",
                                 "type": "WL",
                                 "url_pc":  "https://www.jobgo.ac/bbs/my_withdraw.php",
                                 "url_mobile":  "https://www.jobgo.ac/bbs/my_withdraw.php"
                                 }]
                       }
                   }';
...
}else if($type == 'chat_buy'){

    $templateCode = 'SJB_071802';
    $title = '문의 도착 알림2';
    $content =  '[JOBGO '.$arr["buy_mb"].'님,
'.$arr["seller_mb"].'님의 '.$arr["talent_title"].'에 대한 문의가 도착했습니다.
...
잡고 플랫폼에서 확인해주세요.]';
    ...
    $attachment = '{ "attachment": {
                             ...
                             "button": [{ 
                                 "name": "문의내용 확인하기",
                                 "type": "WL",
                                 "url_pc":  "https://www.jobgo.ac/bbs/message.php",
                                 "url_mobile": "https://www.jobgo.ac/bbs/message.php"
                                 }]
                       }
                   }';
    ...
    $hp = str_replace ( '-' , '', $hp);
    $_variables = array(
        'plusFriendId' => '@jobgo_2020',
        'senderKey' => '3bde82257e48085b4bd8de1451f901d0c3e87a47',
...
//알람톡 결과 확인하는 함수
function api_result($send_code){

    $_apiURL = "https://api.wideshot.co.kr/api/v1/message/result?sendCode=".$send_code;
//    $_apiURL = 'https://api-dev.wideshot.co.kr';
    $_header = array('sejongApiKey:cVhmRlZlZHNkNThYcnhwbllGTGEycmIvMkNTOVNvbUZnV1VHSnowOFFkV0ZDNzNTNmZFRXBIeUtuNGE1b2VhRE1XbTRYTG8rN2VmdS9OTzltaGZQS0E9PQ==');
...
function sendFcm($tokens, $message)
{
    ...
    $fields = array(
        ...
        'data' => $message,
        'priority' => 'high',
        'notification' => array( 'title' => $message['subject'], 'body' => $message['message'],'url' => "https://jobgo.ac")
    );
    $headers = array(
        'Authorization:key =AAAAlQ8K2UU:APA91bHxfUj3odq-Z3h1oLS-Z5BxI8gHoqAp6x__N4Jt9cvZlcH1D4lxfIv7nBPR3akFrry6mbalsIdy6rfEkQmKUs0iomDHe04tcB-L_K4SeTYpQjYJ-bDNi4_RKIkTrYgOGPkYzmLn',
