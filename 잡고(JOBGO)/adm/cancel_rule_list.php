    ...
    <div class="tbl_head02 tbl_wrap mb_tbl">
        <table>
            ...
            for ($i=0; $row=sql_fetch_array($result); $i++) {
                ...
                $code = common_code($row['cr_category1'],'code_idx','json');

                ?>
                <tr class="<?php echo $bg; ?>" style="cursor: pointer" onclick="update_setting(<?=$row['code_idx']?>,'<?=$row['code_use']?>','<?=$code_name?>',<?=$row['code_p_idx']?>,'<?=$row['code_ctg']?>')">
                    <!--	<td>
...
<script>
    ...
    $("ul.cate li").on("click", function() {
        ...
        location.href = g5_admin_url + "/cancel_rule_list.php" + params;
    });
