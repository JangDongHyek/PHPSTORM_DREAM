...
<div class="tbl_frm01 tbl_wrap">
    ...
    <tr>
        ...
        <th scope="row"><label for="cp_company_name">회사명</label></th>
        <td>
            <input type="text" name="cp_company_name" value="<?=$row['cp_company_name'] ?>" class="frm_input" size="40">
        </td>
    </tr>
    <tr>
        <th scope="row"><label for="cp_company_explain">회사소개</label></th>
        <td><textarea style="width: 200%" name="cp_company_explain" id="cp_company_explain" class="frm_input"><?php echo $row['cp_company_explain'] ?></textarea></td>
    </tr>
    <tr>
    ...
    <tr>
        <th scope="row"><label for="cp_logo_content">내용</label></th>
        <td><textarea style="width: 200%" name="cp_logo_content" id="cp_logo_content" class="frm_input"><?php echo $row['cp_logo_content'] ?></textarea></td>
    </tr>

    <tr>
        <th scope="row"><label for="cp_logo_sty">로고스타일</label></th>
        <td>
            <input type="text" name="cp_logo_sty" value="<?=$row['cp_logo_sty']?>" class="frm_input" size="40">
        </td>
    ...
    <tr>
        ...
        <td>
            <input type="text" name="cp_reward" value="<?=$row['cp_reward']?>" class="frm_input" size="40">  만원
        </td>
    ...
    <tr>
        <th scope="row"><label for="image">메인이미지</label></th>
        <td colspan="3">
            <!--            <input type="file" multiple="multiple" name="bf_file[]" id="bf_file[]">-->
            <div>
                <?php
                if (isset($row['cp_idx'])) {
                    $sql = "select * from {$g5['board_file_table']} where bo_table = 'competition_main' and wr_id = '{$row['cp_idx']}'";
                    $result = sql_query($sql, false);
//                $icon_file = G5_DATA_PATH.'/service/'.$file['bf_file'];
                    if (sql_num_rows($result) > 0) {
                        for ($b = 0; $file = sql_fetch_array($result); $b++) {
                            $icon_url = G5_DATA_URL . '/file/competition_main/' . $file['bf_file'];
                            echo '<span style="margin-right: 8px"><img style="width: 100px; height: 100px" src="' . $icon_url . '" alt=""></span>';
                        }
                    }else{
                        echo ' <p>사진없음</p>';
                    }
    ...
    <tr>
        <th scope="row"><label for="image">원하는 작품</label></th>
        <td colspan="3">
<!--            <input type="file" multiple="multiple" name="bf_file[]" id="bf_file[]">-->
            <div>
                <?php
                if (isset($row['cp_idx'])) {
                    $sql = "select * from {$g5['board_file_table']} where bo_table = 'competition_sub' and wr_id = '{$row['cp_idx']}'";
                    $result = sql_query($sql, false);
//                $icon_file = G5_DATA_PATH.'/service/'.$file['bf_file'];
                    if (sql_num_rows($result) > 0) {
                        for ($b = 0; $file = sql_fetch_array($result); $b++) {
                            $icon_url = G5_DATA_URL . '/file/competition_sub/' . $file['bf_file'];
                            echo '<span style="margin-right: 8px"><img style="width: 100px; height: 100px" src="' . $icon_url . '" alt=""></span>';
                        }
                    }else{
                        echo ' <p>사진없음</p>';
...
<div class="btn_confirm01 btn_confirm">
    ...
    <a href="./competition_list.php?<?php echo $qstr ?>">목록</a>
</div>
