...
if(strpos($filename, 'data/editor')) {
    ...
    $filepath = G5_DATA_PATH.'/'.$editor_file;
} else if (strpos($bo_table, 'buis_') === 0) {
    $bo_arr = explode('_',$bo_table);

    $filepath = G5_DATA_PATH.'/member/'.$bo_arr[1].'/'.$filename;
...
if(is_file($filepath)) {
    ...
    elseif (strpos($bo_table, 'buis_')===0)
        $fileurl = G5_DATA_URL.'/member/'.$bo_arr[1].'/'.$filename;
