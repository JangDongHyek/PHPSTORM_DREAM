...
if ($mb['mb_division'] == 2 ) {
    $sql = "select count(pf_idx) cnt from {$g5['profile_table']} where mb_id = '{$mb['mb_id']}' ";
    $profile_result = sql_fetch($sql);
    if ($profile_result['cnt'] < 1) {
        alert("프로필 등록이 되어 있지 않습니다. 프로필 등록을 해주십시오.",G5_BBS_URL.'/register_expert_form03.php?re=y');
    }
}
