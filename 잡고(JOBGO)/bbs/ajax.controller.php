...
function thum_file($file,$idx){
    ...
    //기존 데이터 삭제
    $sql = " select bf_file from {$g5['board_file_table']} where bo_table = '{$bo_table}' and wr_id = '{$idx}' ";
    $bf_file = sql_fetch($sql)['bf_file'];

    if ($bf_file != "") {
        @unlink(G5_DATA_PATH . '/file/' . $bo_table . '/' . $bf_file);
        $sql = "delete from {$g5['board_file_table']} where bo_table = '{$bo_table}' and wr_id = '{$idx}' ";
        sql_query($sql);
    }
...
if ($mode == 'certi_mail_send') {
    ...
    itforoneMailer('jobgo@jobgo.ac', 'jobgo', $reg_mb_email, $reg_mb_email, $subject, $content);
...
elseif ($mode == 'pro_step') {
    ...
    if ($_REQUEST['tab'] == '3' && $_REQUEST['move_mode'] == "final_save") {
        ...
        //알림톡
        if($_REQUEST['w'] != 'u' ) {
            alimtalk($member['mb_hp'], array('mb_nick' => $member['mb_nick']), 'talent_write');
        }

        //썸네일 저장
        if ($private) {
            thum_file($_FILES['thumbf_file'], $idx);
        }
    ...
    if ($result == 1) {
        if ($_REQUEST['move_mode'] == 'next') {
            if (!empty($_REQUEST['page'])) { // 01-기본정보 or 02-가격정보 or 03-서비스상세 선택
                ...
                goto_url(G5_BBS_URL . '/' . $_REQUEST['page'] . '?w='.$_REQUEST['w'].'&ta_idx=' . $idx);
            } else { // 다음단계 선택
                goto_url(G5_BBS_URL . '/pro_step02.php?w='.$_REQUEST["w"].'&ta_idx=' . $idx);
            }
        }else{
            echo $result;
        }
    ...
    if($count > 0) { // 이미 저장된 가격정보가 있을 경우 update
        ...
        // standard, deluxe, premium ==> 패키지로 가격설정 지우고 한번에 저장
        for($i=1; $i<=3; $i++) {
            $sql = " select count(*) as count from {$g5['pay_talent_table']} where ta_idx = {$idx} and pta_info = {$i} ";
            $count = sql_fetch($sql)['count'];

            if($i==1) { $needle = 'standard_'; } else if($i==2) { $needle = 'deluxe_'; } else { $needle = 'premium_'; }
            if($count > 0) { // update
        ...
        // deluxe, premium ==> 패키지로 가격설정 지우고 한번에 저장

        for($i=2; $i<=3; $i++) {
            if ($i == 2 && $_REQUEST["deluxe_pta_pay"] == 0){
                continue;
            }
            if ($i == 3 && $_REQUEST["premium_pta_pay"] == 0){
                continue;
            }
            $sql = " insert into {$g5['pay_talent_table']} set ";
            $sql .= " pta_info = '" . $i . "', ";
    ...
    if($_REQUEST['move_mode'] == 'save') { // 임시저장
        ...
        if(!empty($_REQUEST['page'])) { // 01-기본정보 or 03-서비스상세 선택
            ...
            goto_url(G5_BBS_URL . '/pro_step03.php?w='.$_REQUEST['w'].'&ta_idx=' . $idx);
        }
    }
...
//    exit;
   ...
   if ($arr['pf_idx'] != "" || $_REQUEST['re'] == 'y'){
    ...
    $sql = "select count(*) cnt from new_like where ta_idx = {$ta_idx} and li_table = '{$li_table}' and mb_id = '{$member['mb_id']}' ";
    $h_cnt = sql_fetch($sql)['cnt'];

    if ($h_cnt == 0 ){

        $sql = "insert into new_like set ta_idx = {$ta_idx}, mb_id = '{$member['mb_id']}',li_table = '{$li_table}', wr_datetime = '".G5_TIME_YMDHIS."' ";
        $type = 'on';
    }else{
        $sql = "delete from new_like where ta_idx = {$ta_idx} and li_table = '{$li_table}' and mb_id = '{$member["mb_id"]}' ";
        $type = 'off';
...
//    }
    ...
    //history
    payment_history($member['mb_id'], $exchange_content['content'],$mileage_fee,  $mb['mb_6'],'@pay_minus','',$exchange_content['idx'],$idx,'new_mileage');
    payment_history($member['mb_id'], $mileage_buy_content['content'],$mileage_fee,  $mb['mb_7'],'@mileage_plus','Y',$mileage_buy_content['idx'],$idx,'new_mileage');

    if($result) {
        die('success');
    }

}else if($mode == "mb_1_write"){

    $sql = "update g5_member set mb_1 = '".$_REQUEST["mb_1"]."', mb_2 = '".$_REQUEST["mb_2"]."', mb_3 = '".$_REQUEST["mb_3"]."'
            where mb_id = '{$member['mb_id']}'";
    $result = sql_query($sql);

    if($result == "1"){
        die(json_encode(array('code'=>'success', "mb_1" =>$bank_list[$_REQUEST["mb_1"]] )));
    }

}else if($mode == "comp_chk"){

    $sql = "update new_payment set comp_chk = 'Y', comp_chk_datetime = '".G5_TIME_YMDHIS."' where pa_idx = '{$_REQUEST['pa_idx']}' ";
    $result = sql_query($sql);

    //구매확정 시 알림톡
    $sql = "select seller_id from new_payment where pa_idx = {$_REQUEST['pa_idx']} ";
    $row = sql_fetch($sql);
    $seller_mb = get_member($row['seller_id']);
    alimtalk($seller_mb['mb_hp'], array("mb_nick"=>$seller_mb['mb_nick'] ), 'cashe');


    echo $result;

}elseif ($mode == 'app_share'){
    $code = date("ymdHis", strtotime(G5_TIME_YMDHIS)).rand("100000", "999999");
    $sql = "insert into new_recommend set
            mb_id = '{$member['mb_id']}' ,code = '{$code}', wr_datetime = '" . G5_TIME_YMDHIS ."' ";
    $result = sql_query($sql);

    if ($result){
        die($code);
    }else{
        alert("공유하기가 실패 했습니다. 관리자에게 문의해주세요.");
    }

}else if ($mode == "chat_alimtalk"){

    $seller_mb = get_member($_REQUEST["sale_mb_id"]);
    alimtalk($seller_mb['mb_hp'], array("mb_nick"=>$seller_mb['mb_nick'] ), 'chat');

}else if ($mode == "chat_alimtalk_buy"){

    $sql = "select ta_title from new_talent where ta_idx = {$_REQUEST['idx']} ";
    $row = sql_fetch($sql);
    if($row){
        $talent_title = '['.iconv_substr($row['ta_title'], 0, 5, "utf-8").'...]';
    }else{
        $talent_title = '재능';
    }

    $seller_mb = get_member($_REQUEST["sale_mb_id"]);
    $buy_mb = get_member($_REQUEST["buy_mb_id"]);
    alimtalk($buy_mb['mb_hp'], array("buy_mb"=>$buy_mb['mb_nick'],"seller_mb"=>$seller_mb['mb_nick'] ,"talent_title"=> $talent_title), 'chat_buy');

}elseif ($mode == "report_update"){
    $sql = "select count(*) cnt from new_report where r_p_idx = '{$_REQUEST["r_p_idx"]}' and mb_id = '{$member["mb_no"]}' ";
    ...
    $sql = "insert into new_report set ";
    foreach ($_REQUEST as $key => $value) {
        if (strpos($key, 'r_') === 0 ) {
