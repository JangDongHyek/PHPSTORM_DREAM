    ...
    <div class="modal-dialog">
        ...
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title"><i class="fad fa-trophy-alt"></i>&nbsp;공모전 참여하기</h4>
            </div>
            ...
            <div class="modal-body">
                <div id="pro_step">
                    ...
                    <div class="in">
                        <form id="applyfrm" method="post">
                            ...
                            <div class="form-group">
                                ...
                                <div class="txt_bx">
                                    <textarea name="ap_content" id="ap_content" class="form-control txt doc_text" rows="5" placeholder="공모전 관련 의뢰인에게 하시고 싶으신 말씀을 입력하세요."></textarea>
                            ...
                            <div class="form-group">
                                ...
                                <div class="img_up">
                                    ...
                                    <div class="size">ai,psd파일등 원본 파일을 등록해주세요.</div>
                                    <div style="margin: 20px 0">
                                    <input type="file" name="bf_file[]">
                                    </div>
                                </div><!--img_up-->

                                <div class="img_up_gongmo">
                    ...
                    <!--저장 부분-->
                    <div class="f_save cf">
                        <div class="arr"><a href="javascript: form_ajax()">등록완료</a></div>
                    </div><!--f_save-->
...
<article id="contest_view">
                    ...
                    <div class="thm">
                            <div class="mg">
                                ...
                                <div class="heart" id="heart_div_<?=$row['cp_idx']?>">
                                    ...
                                    <?php }else{?>
                                    <button type="button" class="heart off" onclick="like_chk('on',<?=$row['cp_idx']?>,'competition')"><img src="<?php echo G5_THEME_IMG_URL ?>/main/heart_off.png" alt="좋아요off" title="좋아요off"></button><!--좋아요 누르기전-->
                                    <?php } ?>
                                </div>
                                <div class="mg_in">
                                    <div class="over">
                                        <?php
                                        $main_file = G5_DATA_PATH.'/file/competition_main/'.$main_result['bf_file'];
                                        if (file_exists($main_file) && $main_result['bf_file'] != "") {
                                            $html = "<img src = '" . G5_DATA_URL . "/file/competition_main/" . $main_result['bf_file'] . "'>";
                            ...
                            <div class="info">
                                ...
                                <div class="tit"><?= $row['cp_title']?></div><!--공모전 제목(최대1줄까지만 추출, 나머지는 ... 처리함)-->
                                <div class="cont"><?= $row['cp_company_explain']?></div><!--공모전 설명(최대2줄까지만 추출, 나머지는 ... 처리함)-->
                                <div class="rate cf">
                                ...
                                <div class="price">총상금 <?= number_format($row['cp_reward']) ?>만원</div><!--상품가격-->
                            </div>
                    </div>
                </div><!--list-->
            </div><!--in-->
    ...
    <div class="wrapper">
        <div class="tabs cf">
            ...
            <label for="tab1">공모전 의뢰 내용</label>
            <input name="tabs" id="tab2" type="radio">
            <label for="tab2">공모전 참여<?php if ($member['mb_id'] == $row['mb_id'] || $member['mb_id'] == 'admin' ){ ?><span class="badge"><?= comp_apply_cnt($row['cp_idx'])?></span><?php } ?></label>
            ...
            <div id="tab-content1" class="tab-content">
                <div class="contest_cont">
                    ...
                    <section>
                        <h3><i class="far fa-newspaper"></i> 공모전 의뢰내용</h3>
                    ...
                    <section>
                        <h3><i class="fal fa-images"></i> 선호하는 디자인 스타일을 선택해주세요.<span>(최대 3작품 선택)</span></h3>
                    ...
                    <section>
                        <h3><i class="fal fa-images"></i> 디자이너들이 참고할 자료가 있으신가요?</h3>
                        <div class="cont">
                            <?php
                    ...
                    <section>
                        <h3><i class="fal fa-folder-tree"></i> 공모전 상세내용</h3>
            ...
            <div id="tab-content2" class="tab-content">
                ...
                <div class="contest_cont partici">
                    ...
                    <div class="text-center empty_list" style="margin-top: 30px">
                        ...
                        <p class="t_padding17">공모전 참여작이 없습니다. </p>
                    </div>
                ...
                <div class="btn_partici text-right">
                    <?php if (($member['mb_id'] == $row['mb_id'] || $member['mb_id'] == 'admin') && $row['cp_win_apply'] == '0' || $row['cp_win_apply'] == '' ){ ?>
                      <a href="javascript:comp_win_idx(<?=$row['cp_idx']?>)"><i class="far fa-trophy-alt"></i> 당선작 선정</a>
                    <?php } ?>
                    <?php
                    //공모전 작성자가 아니고, 당선작이 정해지지 않았고, 마감기간이 전 일 경우
                    if ( $member['mb_id'] != $row['mb_id'] &&
                    ($row['cp_win_apply'] == '0' || $row['cp_win_apply'] == '')&&
                        $row['cp_progress'] == 1
                    ){ ?>
                      <a href="javascript:comp_apply_modal()">공모전 참여하기</a>
                    <?php } ?>
                    <?php if ( $row['cp_progress'] == '3' && $member['mb_id'] == $row['mb_id'] ){ ?>
                        <a href="javascript:pay_update(<?=$row['cp_idx']?>)">지급 완료하기</a>
                    <?php } ?>
                    <?php if ( $row['cp_progress'] == '4' && $member['mb_id'] == $row['mb_id'] ){ ?>
                            <div class="text-center empty_list">
                                   <i class="fal fa-trophy-alt fa-4x"></i>
                                   <p class="t_padding17">수상자에게 상금 지급 완료되었습니다.</p>
                            </div>
                    <?php } ?>
                </div>
            </div>
            ...
            <div id="tab-content3" class="tab-content">
                <div class="contest_cont">
                    ...
                    <section class="cmt">
                        ...
                        <p>*디자인에 관한 문의사항만 등록해 주세요. 불만, 비방성 댓글은 의뢰자나 관리자에 의해 경고 없이 삭제될 수 있으며, 사이트 이용 제한이 발생할 수 있습니다.</p>
                    </section>

                    <section>
                        <!--<h3><i class="fal fa-images"></i> 공모전 문의사항 <span>(4)</span></h3>-->
                        <div id="item_review">
                            <div class="in">
                                <div class="rev cf">
            ...
            success: function (data) {
                if (data == 1) {
                    swal("공모전 등록이 완료되었습니다.")
                        .then(() => {
                            location.href = g5_bbs_url +"/my_contest.php?tab=2"
                        });
