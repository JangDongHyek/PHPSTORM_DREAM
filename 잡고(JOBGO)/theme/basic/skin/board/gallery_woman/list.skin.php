...
<div id="bo_gall" style="width:<?php echo $width; ?>">
	...
	<div class="top_form">
		...
		<select id="sfl_married" name="sfl_married">
			<? for($i=0; $i<count($married_cate); $i++){
					if($i==0){
			?>				
				<option value="">-결혼-</option>	
				<option value="<?=$married_cate[$i]?>" <?if($graduate_cate[$i]==$sfl_married) echo 'selected'?>><?=$married_cate[$i]?></option>
			<?}else{?>
				<option value="<?=$married_cate[$i]?>" <?if($graduate_cate[$i]==$sfl_married) echo 'selected'?>><?=$married_cate[$i]?></option>
				<?}}?>
		</select>
			<select id="sfl_height" name="sfl_height">
			<?
				for($i=151; $i<181; $i++){
				if($i==151){
			?>				
			<option value="">-신장-</option>
			<option value="<?=$i?>" <?if($i==$sfl_height) echo 'selected'?>><?=$i?></option>
			<?}else{?>
			<option value="<?=$i?>" <?if($i==$sfl_height) echo 'selected'?>><?=$i?></option>		
			<?}}?>
		</select>
			<select id="sfl_weight" name="sfl_weight">
	<?
				for($i=40; $i<61; $i++){
				if($i==40){
			?>				
			<option value="">-체중-</option>
			<option value="<?=$i?>" <?if($i==$sfl_weight) echo 'selected'?>><?=$i?></option>
			<?}else{?>
			<option value="<?=$i?>" <?if($i==$sfl_weight) echo 'selected'?>><?=$i?></option>		
			<?}}?>

		</select>
			<select id="sfl_graduate" name="sfl_graduate">
		<? for($i=0; $i<count($graduate_cate); $i++){
					if($i==0){
			?>				
				<option value="">-학력-</option>	
				<option value="<?=$graduate_cate[$i]?>" <?if($graduate_cate[$i]==$sfl_graduate) echo 'selected'?>><?=$graduate_cate[$i]?></option>
			<?}else{?>
				<option value="<?=$graduate_cate[$i]?>" <?if($graduate_cate[$i]==$sfl_graduate) echo 'selected'?>><?=$graduate_cate[$i]?></option>
				<?}}?>

		</select>
		<select id="sfl_jobs" name="sfl_jobs">
		<? for($i=0; $i<count($job_cate); $i++){
					if($i==0){
			?>				
				<option value="">-직업-</option>	
				<option value="<?=$job_cate[$i]?>" <?if($job_cate[$i]==$sfl_jobs) echo 'selected'?>><?=$job_cate[$i]?></option>
			<?}else{?>
				<option value="<?=$job_cate[$i]?>" <?if($job_cate[$i]==$sfl_jobs) echo 'selected'?>><?=$job_cate[$i]?></option>
				<?}}?>
		</select>
		<span class="age">나이 : <input type="text" id="sfl_age" name="sfl_age" value="<?=$sfl_age?>">
		<input type="submit" value="검색" class="sch"></span>
	</form>
	</div>
