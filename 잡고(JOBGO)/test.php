...
if ($_SERVER['REQUEST_URI'] == '/favicon.ico') {
    header("HTTP/1.1 204 No Content"); // 404 대신 빈 응답 반환
    exit;
}
...
if (isset($_REQUEST['lv']))  { // search part (검색 파트[구간])
    $lv = $_REQUEST['lv'];
    if ($lv)
        $qstr .= '&amp;lv=' . urlencode($lv);
} else {
    $lv = '';
}

if (isset($_REQUEST['big_ctg']))  { // search part (검색 파트[구간])
    $big_ctg = $_REQUEST['big_ctg'];
    if ($big_ctg)
        $qstr .= '&amp;big_ctg=' . urlencode($big_ctg);
} else {
    $big_ctg = '';
}

if (isset($_REQUEST['small_ctg']))  { // search part (검색 파트[구간])
    $small_ctg = $_REQUEST['small_ctg'];
    if ($small_ctg)
        $qstr .= '&amp;small_ctg=' . urlencode($small_ctg);
} else {
    $small_ctg = '';
}
...
//나이스 모듈
define('NICE_CODE', 'BT614');
define('NICE_PASSWORD', 'cULPp1zzElB1');	//비밀번호는 [ cULPp1zzElB1 ] 입니다.
define('NICE_PATH', "/home/jobgo/CPClient/CPClient");
//CSS 버전
...
$competition_content = array('content' => "공모전 작성 수상금액 차감",'idx' => 1);
$competition_win_content = array('content' =>"공모전 수상금액 적립",'idx' => 2);
$payment_content = array('content' =>"출금 요청",'idx' => 3);
$talent_buy_content = array('content' =>"재능 판매자 금액적립",'idx' => 4);
$mileage_content = array('content' =>"재능 판매자 마일리지 적립",'idx' => 5);
$mileage_buy_content = array('content' =>"마일리지 구매",'idx' => 6);
$exchange_content = array('content' =>"마일리지 구매로 인한 금액차감",'idx' => 7);
$delay_content = array('content' =>"관리자 보류 금액입금",'idx' => 8);
$delay_cancel_content = array('content' =>"관리자 보류상태 변경 금액차감",'idx' => 9);
$cache_buy_content = array('content' =>"캐쉬 충전",'idx' => 10);
$cancel_content = array('content' =>"취소상태 환불 처리차감",'idx' => 11);
$cancel_mileage_content = array('content' =>"취소상태 환불 처리 마일리지차감",'idx' => 12);
$register_point_plus = array('content' =>"회원가입 적립금 적립",'idx' => 13);
$sale_point_plus = array('content' =>"재능구매 적립금 적립",'idx' => 14);

// 업체에 오픈하지 않고 내부적으로 작업 시
$private = false;
//if($ip == '183.103.22.103' || $ip == '119.196.57.233' || $ip == '220.68.190.132' ) {
//if ($member["mb_id"] == "test@naver.com"){
if ($ip == "121.140.204.65"){
    $private = true;
}

$user_agent = $_SERVER['HTTP_USER_AGENT'];
$aos_app_user_agent = "AGold"; // AOD USER AGENT
$mobile = false;
$android = false;
$chat_ver = 'pc';
if(strpos($user_agent, $aos_app_user_agent) !== false || strpos(strtolower($user_agent), 'mobile') !== false) { // 모바일 웹 또는 안드로이드 접속 시
    $mobile = true;
}
if(strpos($user_agent, $aos_app_user_agent) !== false) { // 안드로이드 접속 시
    $android = true;
}
if(strpos($_SERVER['PHP_SELF'], 'chat_list.php') !== false || strpos($_SERVER['PHP_SELF'], 'chat_list.php') !== false) {
    $chat_ver = 'app';
}

$app_id = '1399865833725146'; // 페이스북 디벨로퍼 JOBGO 앱 ID
$app_icon_ver = 2;
?>
