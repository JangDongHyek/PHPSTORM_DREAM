<?php
/********************************************************************
*** 이 페이지 수정시 ./register_aflt.skin.php도 함께 수정!!! -- 제휴사 가입페이지
********************************************************************/
...
<div class="mbskin">
    ...
    <div class="tbl_frm01 tbl_wrap">
		...
		*/?>
		...
		<tr>
            <th scope="row"><label>회사 주소<strong class="sound_only">필수</strong></label></th>
            <td>
				<?
				$addr_event2 = "win_zip('fregisterform', 'mb_cp_zip', 'mb_cp_addr1', 'mb_cp_addr2', '', '');";
				?>
                <input type="text" name="mb_cp_zip" value="<?=$member['mb_cp_zip']; ?>" id="reg_mb_cp_zip" class="frm_input" size="5" maxlength="6" onclick="<?=$addr_event2?>" onkeyup="<?=$addr_event2?>" placeholder="우편번호">
                <span class="add_sh"><button type="button" class="btn_frmline" onclick="<?=$addr_event2?>">주소 검색</button></span><br>
                <input type="text" name="mb_cp_addr1" value="<?=get_text($member['mb_cp_addr1']) ?>" class="frm_input frm_address" size="50" placeholder="기본주소" readonly><br>
                <input type="text" name="mb_cp_addr2" value="<?=get_text($member['mb_cp_addr2']) ?>" class="frm_input frm_address" size="50" placeholder="상세주소">
            </td>
        </tr>
        ...
        <tr>
			...
			<td>
				...
				if ($w == "u" && ($member['mb_route'] == "직접입력" || $member['mb_route'] == "추천인")) { //!in_array($member['mb_route'], $regist_route)) {
					$route_css = "";
				}
				?>
		...
		<tr>
            ...
            <td>
				...
				<!-- 증정품주소 -->
                <input type="text" name="mb_gift_zip" value="<?=$member['mb_gift_zip']; ?>" id="reg_mb_gift_zip" class="frm_input" size="5" maxlength="6" onclick="<?=$addr_event2?>" onkeyup="<?=$addr_event2?>" placeholder="우편번호" required>
                <span class="add_sh"><button type="button" class="btn_frmline" onclick="<?=$addr_event2?>">주소 검색</button></span><br>
