		...
		*/ ?>
        <dl>
            ...
            <dd>
				<?
				if ($member['mb_addr1'] != "") {
					echo sprintf("(%s%s)", $member['mb_zip1'], $member['mb_zip2']).' '.print_address($member['mb_addr1'], $member['mb_addr2'], $member['mb_addr3'], $member['mb_addr_jibeon']);
				} else {
					echo "미등록";
				}
				?>
			</dd>
			<dt class="ov_addr">회원증서</dt>
            <dd class="ov_addr">
				<?
				if ($member['mb_cert_img'] != "" && file_exists(MB_CERT_PATH."/".$member['mb_cert_img'])) {
					$cert_url = MB_CERT_URL."/".$member['mb_cert_img'];
				?>
				<a href="javascript:void(0)" onclick="window.open('<?=$cert_url?>', '_blank')">회원증서보기</a>
				<? } else { ?>
				미등록
				<? } ?>
			</dd>
