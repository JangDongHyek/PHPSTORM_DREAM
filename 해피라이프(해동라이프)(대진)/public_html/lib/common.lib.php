...
function getGiftList($mode='') {
	$sql = "SELECT * FROM g5_gift WHERE gf_use = 'Y' ORDER BY gf_order ASC;";
	if ($mode == 'all') {
		$sql = "SELECT * FROM g5_gift order by (CASE gf_use WHEN 'Y' THEN 1 ELSE 2 END) ASC, gf_order ASC";
	}
	$result = sql_query($sql);
