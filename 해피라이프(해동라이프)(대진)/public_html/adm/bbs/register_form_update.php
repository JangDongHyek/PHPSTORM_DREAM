...
if ($w == '' || $w == 'u') {
	...
	*/
	...
	if ($_POST['mb_email_head'] != "" && $_POST['mb_email_tail'] != '') { // if ($mb_email != "") {//
...
$mb_birth = $_POST['mb_year']."-".$_POST['mb_month']."-".$_POST['mb_day'];
$mb_bank_amt = preg_replace("/[^0-9]*/s", "", $_POST['mb_bank_amt']);

$mb_hp = hyphen_hp_number($mb_hp);

$sql_common = " , mb_group = '{$mb_group}'
				, mb_birth = '{$mb_birth}'
				, mb_sex = '{$mb_sex}'
				, mb_hp = '{$mb_hp}'
				, mb_route = '{$mb_route}'
				, mb_route_input = '{$mb_route_input}'
				, mb_gift_idx = '{$mb_gift_idx}'
				, mb_gift_zip = '{$mb_gift_zip}'
				, mb_gift_addr1 = '{$mb_gift_addr1}'
				, mb_gift_addr2 = '{$mb_gift_addr2}'
...
// 스코피 데이터
if ($_POST['skopi_memberCd'] != "") $sql_common .= ", skopi_memberCd = '{$skopi_memberCd}'";
if ($_POST['skopi_seqNo'] != "") $sql_common .= ", skopi_seqNo = '{$skopi_seqNo}'";
