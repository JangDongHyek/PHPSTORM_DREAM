...
<form name="fitemlistupdate" method="post" action="./itemlistupdate.php" onsubmit="return fitemlist_submit(this);" autocomplete="off" id="fitemlistupdate">
	...
	<div class="tbl_head01 tbl_wrap">
		...
		<tr>
			...
			<th scope="col" rowspan="2"><?php echo subject_sort_link('it_price', 'sca='.$sca); ?>판매가격</a></th>
		...
		<tr class="<?=$bg?>" data-no="<?=$i?>">
			...
			<!-- 상품명 -->
			<td rowspan="2"><input type="text" name="it_name[<?=$i?>]" value="<?php echo htmlspecialchars2(cut_str($row['it_name'],250, "")); ?>" id="name_<?=$i?>" required class="tbl_input required" size="30"></td>
			<!-- 분류 -->
			<td>
			...
			<td rowspan="2" class="td_mng td_mng_s">
				...
				<!--<a href="./itemcopy.php?it_id=<?=$it_id?>&amp;ca_id=<?php echo $row['ca_id']; ?>" class="itemcopy btn btn_02" target="_blank">복사</a>-->
			</td>
		</tr>
		<tr class="<?=$bg?>" data-no="<?=$i?>">
			<!-- 상품코드 -->
...
echo get_paging(G5_IS_MOBILE ? $config['cf_mobile_pages'] : $config['cf_write_pages'], $page, $total_page, '?'.$paging_params); 
?>
