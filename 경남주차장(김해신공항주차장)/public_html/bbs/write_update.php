...
if($bo_table=="b_reserv"){
	$hpCheck = 0 < strpos(" ".substr($_POST[wr_3],0,3),"02")?substr($_POST[wr_3],0,2):substr($_POST[wr_3],0,3);

	if(!array_search($hpCheck,$hpChecks)){
		alert("연락처가 올바르게 입력되지 않았습니다.");
		exit;
	}
	$wr3=str_replace("-","",$_POST[wr_3]);
	if(!is_numeric($wr3)){
...
if($bo_table=="b_reserv"){
	$wr_1=$_POST[wr_1][0]." ".$_POST[wr_1][1].$_POST[wr_1][2];
	$wr_2=$_POST[wr_2][0]." ".$_POST[wr_2][1].$_POST[wr_2][2];
	$wr_8 = $_POST['wr_8'];

	//$wr_8=(ceil((strtotime($_POST[wr_2][0])-strtotime($_POST[wr_1][0]))/86400)+1)*5000;
	//echo $wr_8;
	//exit;
}
$addColumn="";
if($_POST['wr_23']){
	$addColumn.=",wr_23='$_POST[wr_23]'";
}
if($_POST['wr_24']){
	$addColumn.=",wr_24='$_POST[wr_24]'";
}
if($_POST['wr_25']){
	$addColumn.=",wr_25='$_POST[wr_25]'";
}
...
if ($w == '' || $w == 'r') {
	...
	if($bo_table=="b_reserv"){
		...
		if($wr_subject=="경남주차장"){
			...
			$msg=$wr_subject."- 부산 강서구 공항앞길 221번길 44(대저2동) 유카주차장입니다.";
		}else{
			$msg="경남주차장-부산광역시 강서구 공항로811번가길 20-6(대저2동) 김해국제공항 경남주차장 입니다.";
