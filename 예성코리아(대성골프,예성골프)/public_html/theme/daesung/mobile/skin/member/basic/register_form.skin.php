			...
			<div id="join_info" class="v1">
				...
				<div class="box-body">
					<?php
						if($w==""){
					?>
					<dl class="row" style="height:40px;padding:10px;line-height:50px">
					    <dd class="col-xs-11">
							<label for="reg_mb_id" class="sound_only">회원구분</label>
							<input type="radio" name="mb_level" value="2"<?php echo $w==""?" checked":"";?> id="mb-level2"><label for="mb-level2">일반회원</label>
							<input type="radio" name="mb_level" value="3" id="mb-level3"><label for="mb-level3">도매회원</label>
						</dd>
					</dl>
					<?php }?>
					<dl class="row">
					...
					<dl class="row adrBox">
						...
						<dd class="col-xs-11">
							...
							<br>
							<input type="hidden" name="mb_addr3" value="<?php echo get_text($member['mb_addr3']) ?>" id="reg_mb_addr3" class="f_empty" size="50" readonly="readonly">
					...
					<?php }?>
					<dl class="row mb-level-3" style="display:<?php echo $member[mb_level]=="3"?"":"none";?>">
						<dd class="col-xs-12">
							<label for="mb_1">상호명</label>
							<input type="text" name="mb_1" id="mb_1" class="regist-input " minlength="3" maxlength="50" value="<?=$member[mb_1]?>">
						</dd>
					...
					<dl class="row mb-level-3" style="display:<?php echo $member[mb_level]=="3"?"":"none";?>">
						<dd class="col-xs-12">
							<label for="mb_1">사업자등록번호</label>
							<input type="text" name="mb_2" id="mb_2" class="regist-input " minlength="3" maxlength="50" value="<?=$member[mb_2]?>">
						</dd>
						<dd class="status_ico"><i class="fas fa-check"></i></dd>
						<dd class="error col-xs-12"></dd>
					</dl>
					<dl class="row" class="row mb-level-3" style="display:<?php echo $member[mb_level]=="3"?"":"none";?>">
						
						<dd class="col-xs-1 req">*</dd>
						<dd class="col-xs-11">
							<input type="file" name="mb_3f" id="mb_3f" class="frm_input" placeholder="사업자등록증" style="border:0px"  onchange="fileUpload(event,'')">
							<input type="hidden" name="mb_3" id="mb_3" value="<?php echo $member[mb_3]?>">
						</dd>
						<dd class="status_ico <?/*=$pas*/?>"><i class="fas fa-check"></i></dd>
						<dd class="error col-xs-12"></dd>
					</dl>
					<dl class="row mb-level-3" style="display:<?php echo $member[mb_level]=="3"?"":"none";?>">
						<dd class="col-xs-12">
							<label for="mb_3f">사업자등록증<br/>미리보기</label>
							<img src="<?=G5_DATA_URL?>/member/<?=$member[mb_3]?>" width="100" id="mb-3-view" style="display:<?php echo $member[mb_3]?"":"none"?>">
						</dd>
						<dd class="status_ico"><i class="fas fa-check"></i></dd>
						<dd class="error col-xs-12"></dd>
					</dl>
					<?php if ($w == "" && $config['cf_use_recommend']) {  ?>
			 			<dl class="row">
							<dd class="col-xs-1 req"></dd>
							<dd class="col-xs-11">
								<label for="reg_mb_recommend"  class="sound_only">추천인</label>
								<input type="text" name="mb_recommend" id="reg_mb_recommend" class="regist-input f_empty" minlength="3" maxlength="50" placeholder="추천인 아이디를 입력하세요" value="<?php echo $member['mb_recommend']; ?>" onkeyup="$('#chk-recommend').html('')">
								<script type="text/javascript">
									let is_commend = true;
									$(function(){
										$("#reg_mb_recommend").blur(function(){
											if($(this).val()!==""){
												let mb_recommend = $(this).val();
												$.ajax({
													url:`${g5_bbs_url}/ajax.mb_recommend.php`,
													data:{mb_recommend:$(this).val()},
													dataType:"html",
													type:"post",
													success:function(data){
														...
														if(data!==""){
															is_commend = false;
															$("#chk-recommend").css("color","red");
															$("#chk-recommend").html(data);
														}else{
															is_commend = true;
															$("#chk-recommend").css("color","blue");
															$("#chk-recommend").html(`${mb_recommend}을 추천하셨습니다.`);
														}
								...
								<div id="chk-recommend" style="color:red"></div>
							</dd>
						</dl>
            ...
            <?php } ?>
				...
				<div  class="v1">
				 <div class="box-body">
					 <dl class="row adrBox">
						<dd class="col-xs-1 req"></dd>
						<dd class="col-xs-11"><?php echo captcha_html(); ?></dd>
					</dl>
				 </div>
				</div>
				<!-- device 아이디 값 가져오기 -->
		...
		$.ajax({
				...
				success: function(data, textStatus, jqXHR) { 
					...
					if(json.success=="오류"){
						...
						$("#mb-3-view").attr("src",json.imgsrc);
						$("#mb-3-view").css("display","block");
						$("#mb_3").val(json.filename);
					}
...
/*				if(data!="오류"){
    ...
    $(function () {
		$("input[name=mb_level]").click(function(){
			$(this).val()==="3"?$(".mb-level-3").css("display",""):$(".mb-level-3").css("display","none")
		});
		...
		$("#reg_mb_name").keyup(function (event){
			regexp = /[a-z0-9]|[ \[\]{}()<>?|`~!@#$%^&*-_+=,.;:\"'\\]/g;
            v = $(this).val();
            if (regexp.test(v)) {
                alert("한글만 입력가능 합니다.");
