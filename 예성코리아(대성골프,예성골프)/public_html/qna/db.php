...
class QnaDB
{
	...
	function getList($mid, $page = 1, $type = "all")
	{
		...
		if ($type == "notice") {
			...
			$sql = "SELECT *, 
					...
					FROM project_qna WHERE mid = '{$mid}' AND is_notice = 'N' ORDER BY idx DESC {$sql_limit}";
		}
	...
	function getDbRows($type, $mid, $idx = 0)
	{
		...
		switch ($type) {
			case "count": 	// 총 조회수
				$sql = "SELECT COUNT(*) AS cnt FROM project_qna WHERE mid = '{$mid}' AND is_notice = 'N'";
				break;
			case "view": 	// 상세보기, 글수정시
				$sql = "SELECT * FROM project_qna WHERE mid = '{$mid}' AND idx = '{$idx}'";
				break;
			case "notice":	// 공지
