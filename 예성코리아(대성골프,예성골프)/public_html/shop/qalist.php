...
if($is_guest)
    alert('회원이시라면 로그인 후 이용해 보십시오.', '../bbs/login.php?url='.urlencode(G5_SHOP_URL.'/qalist.php'));
...
if ($qaconfig['qa_category']) {
    $category_href = G5_SHOP_URL.'/qalist.php';
...
if(is_file($skin_file)) {
    ...
    for($i=0; $row=sql_fetch_array($result); $i++) {
        ...
        $list[$i]['view_href'] = G5_SHOP_URL.'/qaview.php?qa_id='.$row['qa_id'].$qstr;
    ...
    $list_href = G5_SHOP_URL.'/qalist.php';
