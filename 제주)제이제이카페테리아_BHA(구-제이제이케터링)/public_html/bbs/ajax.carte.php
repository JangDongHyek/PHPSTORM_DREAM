...
$sql = "select wr_2 from $write_table where wr_3 = '{$now_date}' and wr_6 = {$now_sheet} and wr_1 = '{$sme}' group by wr_2 order by wr_id "; 
...
for ($i=0; $row=sql_fetch_array($view_tmcate); $i++){
	...
	for ($i=0; $row=sql_fetch_array($result_menu); $i++){
				array_push($arr_menu[$row['wr_2']], $row['wr_5']);
	}
