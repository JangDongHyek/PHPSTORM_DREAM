...
<aside id="poll">
    <header>
        ...
        <p class="poll_title"><span class="qicon">Q</span><?php echo $po['po_subject'] ?></p>
    </header>
    <ul>
        <?php for ($i=1; $i<=9 && $po["po_poll{$i}"]; $i++) { ?>
        <label for="gb_poll_<?php echo $i ?>"><li><input type="radio" name="gb_poll" value="<?php echo $i ?>" id="gb_poll_<?php echo $i ?>"> <?php echo $po['po_poll'.$i] ?></li>
        <?php } ?></label>
    </ul>
    <footer>
