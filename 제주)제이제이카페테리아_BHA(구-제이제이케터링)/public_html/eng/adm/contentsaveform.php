...
// 상단, 하단 파일경로 필드 추가
if(!sql_query(" select co_include_head from {$g5['content_save_table']} limit 1 ", false)) {
    $sql = " ALTER TABLE `{$g5['content_save_table']}`  ADD `co_include_head` VARCHAR( 255 ) NOT NULL ,
                                                    ADD `co_include_tail` VARCHAR( 255 ) NOT NULL ";
    sql_query($sql, false);
}

// html purifier 사용여부 필드
if(!sql_query(" select co_tag_filter_use from {$g5['content_save_table']} limit 1 ", false)) {
    sql_query(" ALTER TABLE `{$g5['content_save_table']}`
                    ADD `co_tag_filter_use` tinyint(4) NOT NULL DEFAULT '0' AFTER `co_content` ", true);
    sql_query(" update {$g5['content_save_table']} set co_tag_filter_use = '1' ");
}

// 모바일 내용 추가
if(!sql_query(" select co_mobile_content from {$g5['content_save_table']} limit 1", false)) {
    sql_query(" ALTER TABLE `{$g5['content_save_table']}`
                    ADD `co_mobile_content` longtext NOT NULL AFTER `co_content` ", true);
}

// 스킨 설정 추가
if(!sql_query(" select co_skin from {$g5['content_save_table']} limit 1 ", false)) {
    sql_query(" ALTER TABLE `{$g5['content_save_table']}`
    ...
    sql_query(" update {$g5['content_save_table']} set co_skin = 'basic', co_mobile_skin = 'basic' ");
}
...
if ($w == "u")
{
    ...
    $sql = " select * from {$g5['content_save_table']} where co_id = '$co_id' ";
    $co = sql_fetch($sql);
    if (!$co['co_id'])
        alert('등록된 자료가 없습니다.');
}
...
<form name="frmcontentform" id="frmcontentform" action="./contentsaveformupdate.php" onsubmit="return frmcontentform_check(this);" method="post" enctype="MULTIPART/FORM-DATA" >
<input type="hidden" name="w" value="<?php echo $w; ?>">
<input type="hidden" name="co_html" value="1">
<input type="hidden" name="token" value="">
<input type="hidden" name="co_no" value="<?php echo $co['co_no']?>">
<div class="tbl_frm01 tbl_wrap">
    ...
    <tr>
        ...
        <td>
			...
			<input type="button" value="적용하기" class="btn_frmline" accesskey="s" onclick="setContent()" style="background:#ff3061; cursor:pointer;">
        </td>
    </tr>
    <tr>
    ...
    <tr>
        ...
        <td>
            ...
            <?php
            $himg = G5_DATA_PATH.'/content_save/'.$co['co_id'].'_h '.$co['co_datetime'];
            if (file_exists($himg)) {
                ...
                $himg_str = '<img src="'.G5_DATA_URL.'/content_save/'.$co['co_id'].'_h '.$co['co_datetime'].'" width="'.$width.'" alt="">';
            }
            if ($himg_str) {
    ...
    <tr>
        ...
        <td>
            ...
            <?php
            $timg = G5_DATA_PATH.'/content_save/'.$co['co_id'].'_t '.$co['co_datetime'];
            if (file_exists($timg)) {
                ...
                $timg_str = '<img src="'.G5_DATA_URL.'/content_save/'.$co['co_id'].'_t '.$co['co_datetime'].'" width="'.$width.'" alt="">';
            }
            if ($timg_str) {
                echo '<div class="banner_or_img">';
                echo $timg_str;
...
<div class="btn_confirm01 btn_confirm">
    ...
    <a href="./contentsavelist.php">목록</a>
</div>
...
function setContent(){
	if(confirm("정말 해당내용을 적용하시겠습니까?")==false){
		return false;
	}
	
	var f = document.frmcontentform;
	...
	f.action = "contentformupdate.php";
	f.submit();
}
</script>
