...
$week_str = array("SUN", "MON", "TUE", "WED", "THU", "FRI", "SAT");
...
$sql = "select * from g5_carte_images where date= '{$today}' and tmcate='{$sme}' and sheet='{$now_sheet}'  ";
...
<div class="swiper-container" style="<? if ($cnt_images <= 0) echo 'display:none'; ?>">
    ...
    <div class="swiper-wrapper" id="swiper_content">
        ...
        for ($i = 0; $row = sql_fetch_array($result_images); $i++) {
            ...
            ?>
            <div class="swiper-slide"><img src="<?= G5_DATA_IMG_URL . '/file/carte/' . $row['file_path'] ?>"/></div>
        <? } ?>
...
<div id="bo_list" style="width:<?php echo $width; ?>">
    <div class="date_title">
        <a href="#" onclick="location.href = '<?php echo $pday_url ?>&sme='+document.getElementById('sme').value;"
           class="arrow_left"> </a>
        <?php echo $year ?> . <?php echo $month ?> . <?php echo $day ?> ( <?php echo $week_str[date("w", $mktime)]; ?> )
        <a href="#" onclick="location.href = '<?php echo $nday_url ?>&sme='+document.getElementById('sme').value;"
           class="arrow_right"> </a>
    </div>
    <div id="menu">
        <?php for ($i = 0; $i < count($meal); $i++) { ?>
            <input type="button" name="menu[]" value="<?php echo $meal[$i] ?>"
                   class="<?php echo $sme == $meal[$i] ? "btn_submit" : "btn_cancel" ?>">
        <?php } ?>
    </div>
...
<form name="fwrite" id="fwrite" action="./upload.image.php" onsubmit="return fwrite_submit(this);" method="post"
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-header">
                    ...
                    <h4 class="modal-title">Food image registration</h4>
                    <!-- header title -->
                </div>
                <!-- body -->
...
<? if ($member['mb_level'] == "10") { ?>
    <a href="javascript:void(0);" class="btn_cancel" onclick="upload_modal()">Food image registration</a>
    <a href="javascript:void(0);" class="btn_cancel" onclick="delete_imgs()">Delete food image</a>
<? } ?>
<script>
    var swiper;
