...
$mb_val2=str_replace("-","",$mb_val2);

$sql = " select count(*) as cnt from {$g5['member_table']} where  replace($field2,'-','')='$mb_val2' and mb_id='$mb_id'";
$row = sql_fetch($sql);


$sql = " select mb_no, mb_id, mb_name, mb_nick, mb_datetime from {$g5['member_table']} where replace($field2,'-','')='$mb_val2' and mb_id='$mb_id' ";
$mb = sql_fetch($sql);
if (!$mb['mb_id'])
    alert('존재하지 않는 회원입니다.');
...
itforoneMailer( "itforyou@hanmail.net","itforyou@hanmail.net", $_POST['mb_email'],$mb['mb_name'], $subject, $content);
