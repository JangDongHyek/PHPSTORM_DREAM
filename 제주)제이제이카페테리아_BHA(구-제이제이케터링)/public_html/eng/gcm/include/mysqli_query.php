...
class Mysqli_Query{
	...
	function &getInstance() {
		...
		if(!$theInstance) $theInstance = new Mysqli_Query();
	...
	function connent(){
		if($this->isConnected())
			return $this->conn;
		$this->conn=mysqli_connect($this->config->db_info->db_hostname, $this->config->db_info->user_id, $this->config->db_info->password,$this->config->db_info->db,$this->config->db_info->db_port);
		if(!isset($this->conn) || $this->conn === false)
			die('Database connection failed: ' . mysqli_error($this->conn));
		$this->dbSel=mysqli_select_db($this->conn, $this->config->db_info->db) or die('Unable to connect to database. ');
		if(!isset($this->dbSel) || $this->dbSel === false)
	...
	function insert($db_table,$cols,$values,&$insert_id = null){
		if(!$this->isConnected()){
			die("Not Open DB");
		}
		$insert_id = null;
	...
	function max_count_toQuery($tmpQuery){
		...
		$query = $this->query("SELECT COUNT(*) AS `count` FROM ({$tmpQuery}) AS ROW_COUNT");
	...
	function query($query_string = ""){
		...
		return mysqli_query($this->conn,$query_string);
	}
