					 ...
					 <tr>
						...
						<table cellSpacing="1" cellPadding="3" width="100%" bgColor="#f7f7f7" border="0">
						  <tr bgColor="#FCB663" height="25" align='center'>
							 ...
							 <td width="12%"><b><font color="#ffffff">포인트</font></b></td>
	...
	for ($i=$skipNum; $i < ($cnfPagecount+$skipNum); $i++) {
		...
		echo "					
						  ...
						  <tr align='center' bgcolor='$bgcolor'>
							 ...
							 <td>$j</td>
							 <td align='left'><a href='item_edit_old.php?item_no=$item_no' onfocus='this.blur()' target='mainpage' onclick='OpenWindow()'>$item_name</a></td>
							 <td>$membername</td>
...
if($flag == 'update'){
	for($i=0; $i<count($item_no); $i++) {
		...
		if($mart_id_tmp == $Mall_Admin_ID){//내상품
			if($if_gnt_item == '1')
				$SQL = "update $ItemTable set z_price = $z_price[$i], bonus = $bonus[$i], if_hide='$if_hide[$i]', member_price='$member_price[$i]', price_ok='n', price_date='$date' where item_no = $item_no[$i] and mart_id='$mart_id'";
			else
				$SQL = "update $ItemTable set z_price = $z_price[$i], bonus = $bonus[$i], if_hide='$if_hide[$i]', member_price='$member_price[$i]', price_ok='n', price_date='$date' where item_no = $item_no[$i] and mart_id='$mart_id'";
		}
		else{//gnt 사품
...
if($flag == 'diff'){
	for($i=0; $i<count($item_no); $i++) {
		...
		if($mart_id_tmp == $Mall_Admin_ID){//내상품
			if($target == 'z_price'){
				if($type == 'amount')
					$SQL = "update $ItemTable set z_price = z_price $p_m $amount , price_ok='n', price_date='$date' where item_no = $item_no[$i] and mart_id='$mart_id'";
				if($type == 'rate')
					$SQL = "update $ItemTable set z_price = TRUNCATE(z_price $p_m (z_price * $amount /100),-2) , price_ok='n', price_date='$date' where item_no = $item_no[$i] and mart_id='$mart_id'";
			}
			if($target == 'bonus'){
				if($type == 'amount')
					$SQL = "update $ItemTable set bonus = bonus $p_m $amount , price_ok='n', price_date='$date' where item_no = $item_no[$i] and mart_id='$mart_id'";
				if($type == 'rate')
					$SQL = "update $ItemTable set bonus = bonus $p_m (bonus * $amount /100) , price_ok='n', price_date='$date' where item_no = $item_no[$i] and mart_id='$mart_id'";
			}
			if($target == 'member_price'){
				if($type == 'amount')
					$SQL = "update $ItemTable set member_price = member_price $p_m $amount , price_ok='n', price_date='$date' where item_no = $item_no[$i] and mart_id='$mart_id'";
				if($type == 'rate')
					$SQL = "update $ItemTable set member_price = TRUNCATE(member_price $p_m (member_price * $amount /100),-2) , price_ok='n', price_date='$date' where item_no = $item_no[$i] and mart_id='$mart_id'";
			}	
		}
		else{//gnt 사품
			if($target == 'z_price'){
