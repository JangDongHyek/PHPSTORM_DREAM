						...
						<tr>
							...
							<td><input type="text" class="input_03" size="20" name="writer" value='<?=$writer?>' style='ime-mode:active'>
												...
												열람분류 : <select name="code" class="input_03" style='width:100px'>
												   <option value='1' <?if($ary[code] == 1){echo"selected";}?>>전체공개</option>
													<option value='2' <?if($ary[code] == 2){echo"selected";}?>>등록권한</option>
													<option value='3' <?if($ary[code] == 3){echo"selected";}?>>수정권한</option>
													<option value='4' <?if($ary[code] == 4){echo"selected";}?>>삭제권한</option>
													<option value='5' <?if($ary[code] == 5){echo"selected";}?>>일반회원</option>
...
<?
	...
	$SQL = "update $New_BoardTable set writer='$writer', code='$code', email='$email', subject_new='$subject_new_tmp[0]', content='$content', if_secret='$if_secret' $notice_sql where index_no ='$index_no' and bbs_no='$bbs_no' and mart_id='$mart_id'";
