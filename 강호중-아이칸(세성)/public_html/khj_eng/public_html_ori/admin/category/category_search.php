...
if($sea_num){
	$seq_num_query = " and sea_num='$sea_num' ";
}
if($sung_num){
	$sung_num_query = " and sung_num='$sung_num' ";
}
if($khan_num){
	$khan_num_query = " and khan_num='$khan_num' ";
}

$SQL = "select * from $CategoryTable where 1 $seq_num_query $sung_num_query $khan_num_query order by cat_order desc ";
...
?>
							...
							<tr onmouseover="this.style.backgroundColor='#DDF0FF'" onmouseout="this.style.backgroundColor='#FFFFFF'" bgcolor="#FFFFFF" align='center'>
								...
								<td><?=$j?></td>
								<td><a href="../good/item_frame.html?pu_search=<?=$pu_search?>&category_num_search=<?=$row[category_num]?>&category_num_parent=<?=$category_num_parent?>"><?=$row[sea_num]?><?=$row[sung_num]?><?=$row[khan_num]?></a></td>
