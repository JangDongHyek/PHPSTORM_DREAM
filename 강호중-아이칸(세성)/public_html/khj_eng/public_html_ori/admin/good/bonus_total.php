...
function del(username,num){
	if(confirm("삭제하시겠습니까?")){
		window.location.href='bunus.html?flag=del&mode=<?=$mode?>&provider_id=<?=$provider_id?>&username='+username+'&num='+num;
	}
	else return;
}
</script>
	 ...
	 <table cellSpacing="0" cellPadding="0" width="100%" border="0">
		<tr>
		  ...
		  <table width="100%" border="0">
			 <tr>
				<td width="100%" height="20"><p align="center"><b>[회원 충전금내역]</b></td>
			 </tr>
		  </table>
		  </td>
		</tr>
...
$sum_sql = "select sum(bonus) as bonus_total from $BonusTable where mart_id ='$mart_id'";
		...
		<tr>
		  ...
		  <?
				if($page == 1){
					...
					echo ("
					<a href='bunus.html?username=$rows[item_code]&page=1'>처음</a> 
					");
				}
			
				if($start_page > 1){
					echo ("
					<a href='bunus.html?username=$rows[item_code]&page=$prev_start_page'>
					◁&nbsp; 
					</a>
					");
				}
				...
				for($i=$start_page;$i<=$end_page;$i++){
					if($i == $page){
						...
						echo ("
					<a href='bunus.html?username=$rows[item_code]&page=$i'>$i</a> 
						");
					}
				}
				if($end_page < $total_page){
					echo ("
					<a href='bunus.html?username=$rows[item_code]&page=$next_start_page'>
					&nbsp;▷
					</a>
					");
				}
				...
				if($page == $total_page){
					...
					echo ("
					<a href='bunus.html?username=$rows[item_code]&page=$total_page'>끝</a> 
					");
				}
				?>
				</td>
...
<?
	...
	echo "<meta http-equiv='refresh' content='0; URL=bunus.html?username=$username&provider_id=$provider_id&mode=$mode'>";
