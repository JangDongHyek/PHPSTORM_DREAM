...
?>
							...
							<?}?>
								...
								<td><span style="cursor:hand;" onclick="javascript:window.open('../../market/board_mem/board_read.php?seq_num=<?=$seq_num?>&bbs_no=<?=$bbs_no?>&index_no=<?=$index_no?>','','width=800,height=500,top=100,left=200,scrollbars=yes');">[게시글보기]</span></td>
								<td><?=$regdate?></td>
							</tr>
<?
}
...
if($flag == "modify_item"){		
	...
	for($i=0; $i<count($checkSel); $i++) {
		...
		$sql = "update new_board set del_chk='y' where bbs_no='4' and index_no='$index_no'";
		$res = mysql_query($sql,$dbconn);
