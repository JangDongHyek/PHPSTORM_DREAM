...
//		if($Mall_Admin_ID == $mart_id){ // 내회원이면
	...
	echo "<meta http-equiv='refresh' content='0; URL=item_list.php?category_num=$prevno&searchword=$searchword&select_key=$select_key&date_expire=$date_expire&page=$page&pu=$pu'>";
}
//========================================================================================
if (isset($flag) == false) {
...
$today = date("Y-m-d");
if($date_expire == 'y'){
	$add_query = " and end_date < '$today' and (end_date + INTERVAL $gigan_value[gigan] DAY) > '$today'";//회원기간 만료자 보기
}
...
//총회원수
$SQL1 = "select count(item_no) from $ItemTable where $con and mart_id='$mart_id' and if_hide='0'";
...
//보류회,기존회원
$SQL2 = "select end_date from $ItemTable where $con and mart_id='$mart_id' and if_hide='0'";
$dbresult2 = mysql_query($SQL2, $dbconn);
$numRows1_tot=0;
$numRows2_tot=0;
for($o=0;$numRows2 = mysql_fetch_array($dbresult2);$o++){
	//만료일 +10일
	$date = $numRows2[end_date];
	...
	if($numRows2[end_date] < $today && $today < $sdate){ //오늘이 만료일보다 크고 만료일+10일 한것보다 작을때 보류회원임
		$numRows2_tot += 1; //보류회원수
	}

	if($today > $sdate){ //만료일+10일 한것보다 오늘이 클때 기존회원임
		$numRows1_tot += 1; //기존회원수
	}
	

}
$numRows3 = $numRows1 - $numRows1_tot - $numRows2_tot; //총회원수=기존회원수 - 보류회원수
					...
					<tr>
						...
						<td width="50%" bgcolor="#FFFFFF">
							...
							if($page == 1){
								...
								echo ("
								<a href='item_list.php?category_num=$tmp_category_num&page=1&searchword=$searchword&select_key=$select_key&date_expire=$date_expire&pu=$pu'>처음</a>
								");
							}
						
							if($start_page > 1){
								echo ("
								<a href='item_list.php?category_num=$tmp_category_num&page=$prev_start_page&searchword=$searchword&select_key=$select_key&date_expire=$date_expire&pu=$pu'>
								◁&nbsp;
								</a>
								");
							}
							...
							for($i=$start_page;$i<=$end_page;$i++){
								if($i == $page){
									...
									echo ("
								<a href='item_list.php?category_num=$tmp_category_num&page=$i&searchword=$searchword&select_key=$select_key&date_expire=$date_expire&pu=$pu'>$i</a>
									");
								}
							}
							if($end_page < $total_page){
								echo ("
								<a href='item_list.php?category_num=$tmp_category_num&page=$next_start_page&searchword=$searchword&select_key=$select_key&date_expire=$date_expire&pu=$pu'>
								&nbsp;▷
								</a>
								");
							}
							...
							if($page == $total_page){
								...
								echo ("
								<a href='item_list.php?category_num=$tmp_category_num&page=$total_page&searchword=$searchword&select_key=$select_key&date_expire=$date_expire&pu=$pu'>끝</a>
								");
							}
							?>
							</span>
						...
						<td width="50%" bgcolor="#FFFFFF" height="0" align="center">
							...
							<input type=checkbox name="date_expire" value="y" <?if($date_expire=='y'){echo"checked";}?>>보류회원보기</a>&nbsp;
...
//	}else{
		$link_str = "<a href='item_edit.php?item_no=$item_no&prevno=$prevno&prevno2=$prevno2&category_num=$category_num&page=$page&searchword=$searchword&select_key=$select_key&date_expire=$date_expire&pu=$pu'>$item_name</a> $icon_str $hide_str";
		$link_str2 = "<a href='item_edit.php?item_no=$item_no&prevno=$prevno&prevno2=$prevno2&category_num=$category_num&page=$page&searchword=$searchword&select_key=$select_key&date_expire=$date_expire&pu=$pu'>$co_name</a> $icon_str $hide_str";
//	}
?>
							<tr onmouseover="this.style.backgroundColor='#DDF0FF'" onmouseout="this.style.backgroundColor='#FFFFFF'" bgcolor="#FFFFFF" align='center'>
								...
								<td><?=$j?></td>
								<td><a href='item_edit.php?item_no=<?=$item_no?>&prevno=<?=$prevno?>&prevno2=<?=$prevno2?>&category_num=<?=$category_num?>&page=<?=$page?>&searchword=<?=$searchword?>&select_key=<?=$select_key?>&date_expire=<?=$date_expire?>&pu=<?=$pu?>'><?=$sea_num?><?=$sung_num?><?=$khan_num?><?=$sudong_num?></a></td>
								<td><a href='item_edit.php?item_no=<?=$item_no?>&prevno=<?=$prevno?>&prevno2=<?=$prevno2?>&category_num=<?=$category_num?>&page=<?=$page?>&searchword=<?=$searchword?>&select_key=<?=$select_key?>&date_expire=<?=$date_expire?>&pu=<?=$pu?>'><?=$item_code?></a></td>
								<td><?=$link_str?></td>
...
if($flag == "to_item"){							
	for($i=0; $i<count($itemno); $i++) {
		...
		$SQL = "update $ItemTable set item_order='$item_order[$i]' where item_no = '$item_no' and mart_id='$mart_id' and if_hide='0'";;
		$dbresult = mysql_query($SQL, $dbconn);
	}
	echo "<meta http-equiv='refresh' content='0; URL=item_list.php?category_num=$prevno&searchword=$searchword&select_key=$select_key&date_expire=$date_expire&page=$page&pu=$pu'>";
}
