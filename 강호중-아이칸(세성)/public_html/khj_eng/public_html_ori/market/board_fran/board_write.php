...
<!---------------------- 게시판 시작 ---------------------------------------------------->
					...
					<table width="60%" align="center" border="0" cellspacing="0" cellpadding="0">
						<tr>
							...
							<textarea rows=10 cols=120>“칸” 가맹점 약관
...
제3조 [가맹점의 준수사항]
   1) 가맹점은 “칸”회원이 아닌 자와 동등한 가격에 가맹점을 이용 하       도록 하여서는 안 됩니다.
...
약관의 변경은 회사운영의 상황에 따라 약관을 변경 할 수 있습니다.
						...
						<tr>
							<td width="100%" align=center>
									...
									<input type=checkbox name="yakwan" value="y"> 위의 약관에 동의합니다.
...
?>
    ...
    <tr>
      ...
      <td width=35% bgcolor="#FFFFFF">
								...
								?>	
									<option value="<?=$row[category_num]?>"><?=$row[category_name]?></option>
								<?
								}
								?>
								</select>
    ...
    <tr>
      <td align=center width=15% bgcolor="#c8dfec">상호명</td> 
...
<SELECT id=khan_area name=khan_area ></SELECT>
	  ...
	  <!--나머지주소<input name="address" value='<?=$address?>' type="text" class="input_03" size="30" style='ime-mode:inactive' >--></td>
    </tr>
	<tr>
      <td align=center width=15% bgcolor="#c8dfec">이메일</td> 
      <td width=85% bgcolor="#FFFFFF" colspan=3><input name="email" value='<?=$email?>' type="text" class="input_03" size="30" style='ime-mode:inactive' ></td>
    </tr>
	<tr>
      <td align=center width=15% bgcolor="#c8dfec">전화번호</td> 
      <td width=85% bgcolor="#FFFFFF" colspan=3><input name="tel" value='<?=$tel?>' type="text" class="input_03" size="30" style='ime-mode:inactive' > </td>
    </tr>
	<tr>
      <td align=center width=15% bgcolor="#c8dfec">게시기간</td> 
      <td width=85% bgcolor="#FFFFFF" colspan=3><?=date("Y-m-d");?> ~ <input name='end_date' type=text size=15 class=b_input id="end_date"  value='<?=$end_date?>' title="YYYY-MM-DD" readonly required='required' itemname='등록일'  onclick="displayCalendarFor('end_date');"></td>
    </tr>
<?
...
#######################금지단어팅구기################################	
	...
	$SQL = "insert into $New_BoardTable (index_no, firstno, prevno, thirdno, category_num, bbs_no, notice_no, mart_id, code, username, writer, passwd, write_date, ansno, step, thread, email, subject_new, content, userfile, userfile1, if_secret, writer_ip, area, address,tel,hobby, open_address,open_tel,open_hobby,open_email,my_bank_name,my_bank_account,my_bank_master,open_bank,end_date,open_auth,del_chk,bunryu,bubun,halin,sea_area,sung_area,khan_area) values ('$index_no', '$firstno', '$prevno', '$thirdno', '$category_num', $bbs_no, '$max_notice', '$mart_id', '$code', '$_SESSION[Mall_Admin_ID]', '$writer', '$passwd', '$write_date', '1', '0', '$thread', '$email', '$subject_new_tmp[0]', '$content', '$file', '$file1', '$if_secret', '$REMOTE_ADDR', '$item_no', '$address','$tel','$hobby','$open_address','$open_tel','$open_hobby','$open_email','$my_bank_name','$my_bank_account','$my_bank_master','$open_bank','$end_date','y','n','$bunryu','$bubun','$halin','$sea_area','$sung_area','$khan_area')";
	$dbresult = mysql_query($SQL, $dbconn);
	if( !$dbresult ){
		echo "
			<script>
