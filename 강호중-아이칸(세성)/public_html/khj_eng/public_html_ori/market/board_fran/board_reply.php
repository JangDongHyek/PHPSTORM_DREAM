...
#######################################################################
	...
	$SQL = "insert into $New_BoardTable(index_no, bbs_no, notice_no, mart_id, username, writer, passwd, write_date, ansno, step, thread, email, subject_new, content, userfile, userfile1, if_secret,writer_ip, user_id) values($maxIndex_no+1, $bbs_no, '0','$mart_id', '$UnameSess', '$writer', '$passwd', '$write_date', '$ansno', '$step', '$thread', '$email', '$subject_new_tmp[0]', '$content', '$file', '$file1', '$if_secret', '$REMOTE_ADDR', '$user_id_name')";
	$dbresult = mysql_query($SQL, $dbconn);
	if ($dbresult == false) echo "쿼리 실행 실패!";
