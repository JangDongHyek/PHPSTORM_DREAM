...
if($item_no){
	$url="../main/product_info.html?mart_id=$mart_id&flag=&item_no=$item_no";
	$url = str_replace( "?", "|", $url );
	$url = str_replace( "&", "!", $url );
}
...
<!---------------------- 게시판 시작 ---------------------------------------------------->
<?
$SQL = "select * from $New_BoardTable where area='$item_no' and bbs_no='$bbs_no' and index_no=$index_no and mart_id = '$mart_id'";
$dbresult = mysql_query($SQL, $dbconn);
...
#######################################################################
	...
	$SQL = "insert into $New_BoardTable(index_no, bbs_no, mart_id, username, writer, passwd, write_date, ansno, step, thread, email, subject_new, content, userfile, userfile1, if_secret,writer_ip, user_id, area) values($maxIndex_no+1, $bbs_no, '$mart_id', '$UnameSess', '$writer', '$passwd', '$write_date', '$ansno', '$step', '$thread', '$email', '$subject_new_tmp[0]', '$content', '$file', '$file1', '$if_secret', '$REMOTE_ADDR', '$user_id_name', '$item_no')";
	$dbresult = mysql_query($SQL, $dbconn);
	if ($dbresult == false) echo "쿼리 실행 실패!";
