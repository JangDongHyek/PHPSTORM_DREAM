...
<?php
	...
	**************************************************************************/
	...
	$keyPath = "/home/khj/public_html/okname/safecert_".$memId.".key";	// 운영  키파일
	...
	if($ret == 0) {
		...
		// 인증결과를 확인하여 페이지분기등의 처리를 수행해야한다.
	 	if ($retcode == "B000") {
			...
			echo ("<script>opener.document.location.href='./auth.html';self.close();</script>");
		}
	} else {
		//인증결과 복호화 실패
