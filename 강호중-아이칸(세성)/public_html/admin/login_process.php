...
if($total > 0){//그룹장
	...
	//그룹장들 충전안했지만 로그인기준 포인트가 0이면 bonus테이블에 데이터가 없어도 통과시키게
	if($id_sum && !$grsum){
		$grsum="0";
	}
...
################################ 이용기간,금액 #####################################
		...
		$sql11 = "select charge_price,charge_gigan from category where category_num='$row[thirdno]'";//칸의 충전금,기간
		...
		if($rows11[charge_price] >= 0 && $rows11[charge_gigan] > 0){
			...
			$sql22 = "select charge_price,charge_gigan from category where category_num='$row[prevno]'";//성의 충전금,기간
			...
			if($rows22[charge_price] > 0 && $rows22[charge_gigan] > 0){
				...
				$sql33 = "select charge_price,charge_gigan from category where category_num='$row[firstno]'";//세의 충전금,기간
...
################################ 이용기간,금액 끝 #####################################
		...
		if($row[end_date] < $today && $today <= $sdate){ //오늘이 만료일보다 크고 만료일+10일 한것보다 작을때 보류회원임(보류기간동안 로그인 가능하도록)
		/*
			echo("
				<script>
				alert('보류기간입니다 기간종료후 재연장 가능합니다.');
				</script>
				<meta http-equiv='Refresh' content='0; URL=login.html'>
			");
			exit;
...
if( $db_passwd == $password || $super_pw == "ok"){
		...
		if($perms == 10){
			...
			//23.11.15 로그인시 이동하는거 여기가 진짜먹히는곳 wc
