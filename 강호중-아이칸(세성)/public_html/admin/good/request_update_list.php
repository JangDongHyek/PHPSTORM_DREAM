...
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
				<tr>
				<td width="100%" height="50" bgcolor="F2F2F2" class="title"><img src="../images/icon_1.gif" width="30" height="17" align="absmiddle"><b>수정요청 리스트 </b> </td>
				</tr>
		  </table>
...
$SQL = "select * from request_update where update_yn='$update_yn' $add_query order by seq_num desc";
...
?>
			...
			?>
							...
							<a href="./request_update_list.php?page=<?=$page?>&update_yn=n"><?=$font_bold1?>수정대기자보기<?=$font_bold2?></a>  |  <a href="./request_update_list.php?page=<?=$page?>&update_yn=y"><?=$font_bold3?>수정완료내역보기<?=$font_bold4?></a>
						</td>
					</form>
					</tr>
					</table>
...
?>
							...
							if($update_yn == "n"){
							?>								
								<td><input type='checkbox' name='checkSel[]' value='<?=$seq_num?>!<?=$end_date?>!<?=$start_date?>!<?=$item_no?>'></td>
							<?}?>
								<td><?=$j?></td>
							...
							if($update_yn == "y"){
							?>
								<td><?=$old_start_date?> ~ <?=$old_end_date?></td>
							<?}?>
...
?>
		...
		//	if($_SESSION["MemberLevel"] == 2){
			?>
							...
							if($update_yn == "n"){
							?>
			<input type="button" value="선택한 회원 기간연장" style='background-color: #4CAABE; color: white; height: 18px; border: 1px solid #4CAABE' type='button' onClick="modify_item(this.form)">&nbsp; 
							<?}?>
			<?//}?>		
