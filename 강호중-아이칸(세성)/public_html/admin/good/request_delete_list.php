...
if($flag == "modify_item"){		
	...
	for($i=0; $i<count($checkSel); $i++) {
		...
		*/
		...
		$query = "select * from $ItemTable where item_no ='$item_no'";
		...
		//포인트삭제
		$sql  = "delete from bonus  where id='$mem_num'";
		$res  = mysql_query($sql,$dbconn);	

		//회원수정요청 삭제
		$sql  = "delete from request_update  where item_no='$item_no'";
		$res  = mysql_query($sql,$dbconn);	

		//정보검색 수정요청 삭제
		$sql  = "delete from request_update_board   where item_no='$item_no'";
		$res  = mysql_query($sql,$dbconn);	

		//정보검색 상세화면 댓글삭제
		$sql  = "delete from offer   where item_no='$row[item_id]'";
		$res  = mysql_query($sql,$dbconn);	

	
		//삭제요청내역 삭제
		$sql  = "delete from request_delete  where item_no='$item_no'";
		$res  = mysql_query($sql,$dbconn);	

		//월별근무현황일지 my_list
		$sql  = "delete from my_list  where my_id='$row[item_id]'";
		$res  = mysql_query($sql,$dbconn);	
