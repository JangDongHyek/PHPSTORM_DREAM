...
function del(username,num){
	if(confirm("삭제하시겠습니까?")){
		window.location.href='bonus_my.php?flag=del&mode=<?=$mode?>&provider_id=<?=$provider_id?>&username='+username+'&num='+num;
	}
	else return;
}
</script>
...
$sum_sql = "select sum(bonus) as bonus_my from $BonusTable where bonus > '0' and mart_id ='$mart_id' $search_qry";
$sum_rs = mysql_query($sum_sql, $dbconn);
$sum_rows = mysql_fetch_array($sum_rs);
$bonus_my_str1 = $sum_rows[bonus_my];

$sum_sql = "select sum(bonus) as bonus_my from $BonusTable where bonus < '0' and mart_id ='$mart_id' $search_qry";
$sum_rs = mysql_query($sum_sql, $dbconn);
$sum_rows = mysql_fetch_array($sum_rs);
$bonus_my_str2 = $sum_rows[bonus_my];

			

$bonus_my_str2 = eregi_replace('[-a-z!#$%&\'*+/=?^_`{|}~<>]', '',$bonus_my_str2);



$bonus_my_str = $bonus_my_str1 - $bonus_my_str2;



?>
				<p align="center"><b>[내포인트 현황] </b><br>
				<br>
				
				<!--총 충전금 : <?=number_format($bonus_my_str1);?>원 &nbsp; 사용한충전금 : <?=number_format($bonus_my_str2);?>원 =<b>합계 : <?=number_format($bonus_my_str);?>원</b>--> 
...
$SQL = "select * from $BonusTable where mart_id ='$mart_id' and groupjang='y' and bonus < '0' order by num desc";
		...
		<tr>
		  ...
		  <?
				if($page == 1){
					...
					echo ("
					<a href='bonus_my.php?username=$rows[item_code]&page=1&searchword=$searchword&searchword2=$searchword2'>처음</a> 
					");
				}
			
				if($start_page > 1){
					echo ("
					<a href='bonus_my.php?username=$rows[item_code]&page=$prev_start_page&searchword=$searchword&searchword2=$searchword2'>
					◁&nbsp; 
					</a>
					");
				}
				...
				for($i=$start_page;$i<=$end_page;$i++){
					if($i == $page){
						...
						echo ("
					<a href='bonus_my.php?username=$rows[item_code]&page=$i&searchword=$searchword&searchword2=$searchword2'>$i</a> 
						");
					}
				}
				if($end_page < $total_page){
					echo ("
					<a href='bonus_my.php?username=$rows[item_code]&page=$next_start_page&searchword=$searchword&searchword2=$searchword2'>
					&nbsp;▷
					</a>
					");
				}
				...
				if($page == $total_page){
					...
					echo ("
					<a href='bonus_my.php?username=$rows[item_code]&page=$total_page&searchword=$searchword&searchword2=$searchword2'>끝</a> 
					");
				}
				?>
				</td>
...
<?
	...
	if($_SESSION["MemberLevel"] == 4){//일반회원
			...
			echo("
				...
				<meta http-equiv='refresh' content='0; URL=bonus_my.php?username=$username&provider_id=$provider_id&mode=$mode'>
			");
			exit;
	...
	$SQL = "update $Mart_Member_NewTable set bonus_my = bonus_my - $bonus 
	...
	echo "<meta http-equiv='refresh' content='0; URL=bonus_my.php?username=$username&provider_id=$provider_id&mode=$mode'>";
}
?>
<?
mysql_close($dbconn);
