...
?></td>
							...
							if($update_yn == "y"){
							?>
								<td><?=$content?><span style="cursor:hand;" onclick="javascript:window.open('../../market/<?=$board_dir?>/board_read.php?seq_num=<?=$seq_num?>&bbs_no=<?=$bbs_no?>&index_no=<?=$index_no?>','','width=800,height=500,top=100,left=200,scrollbars=yes');">[게시글보기] </span></td>
							<?}else{?>
								<td><?=$content?><span style="cursor:hand;" onclick="javascript:window.open('../../market/<?=$board_dir?>/board_read.php?seq_num=<?=$seq_num?>&bbs_no=<?=$bbs_no?>&index_no=<?=$index_no?>','','width=800,height=500,top=100,left=200,scrollbars=yes');">[게시글보기]</span></td>
							<?}?>
...
if($flag == "modify_item"){		
	...
	for($i=0; $i<count($checkSel); $i++) {
		...
		$sql = "update request_update_board set update_yn='y', regdate='$update_date' where seq_num='$seq_num'";
		$res = mysql_query($sql,$dbconn);
