...
if($_SESSION["MemberLevel"] == 4){ //메뉴관리자일때
	...
	if($rows2[admin_level] == 1){//wic
		...
		$category_num = $rows3[category_num];
	...
	elseif($rows2[admin_level]  == 3){//c
		$group_num1 = substr($rows2[group_num],0,3);
		$group_num2 = substr($rows2[group_num],3,2);
		$group_num3 = substr($rows2[group_num],5,2);
		$SQL3 = "select * from $CategoryTable where category_degree='2' and sea_num='$group_num1'  and sung_num='$group_num2'   and khan_num='$group_num3'";
		$dbresult3 = mysql_query($SQL3, $dbconn);
		$rows3= mysql_fetch_array($dbresult3);
		$category_num = $rows3[category_num];
...
function del_item(f){
  ...
  if (confirm("선택한 회원을 삭제하시겠습니까?")){
		document.list.flag.value='del_item1';
...
function to_item(f){
  if (confirm("선택한 회원의 순서를 변경하시겠습니까?")){
		document.list.flag.value='to_item';
		document.list.submit();
	}
	return true;
}
...
function modify_item(f){
  ...
  if (confirm("선택한 회원을 수정요청하시겠습니까??")){
		document.list.flag.value='modify_item';
		document.list.submit();
	}
	return true;
}
...
function delete_item(f){
  ...
  if (confirm("선택한 회원을 삭제요청하시겠습니까?")){
		document.list.flag.value='delete_item';
		document.list.submit();
	}
	return true;
}
...
function see_item(f){
  ...
  if (confirm("선택한 회원을 출력하시겠습니까?")){
		document.list.flag.value='see_item';
		document.list.submit();
	}
	return true;
}
		 ...
		 <td>
			...
			<tr>
				<td width="100%" bgcolor="#FFFFFF" height="3" valign="top">
					...
					<input type='hidden' name='gamaeng' value='<?=$gamaeng?>'>
...
if($gamaeng==1){
	$gamaeng_query = " and gamaeng='1'";//가맹점회원
}else{
	$gamaeng_query = " and gamaeng='0'";//일반회원
}

if($searchword !=''){
		if($select_key == "item_code"){
			$sea_num = substr($searchword,0,3);
			$sung_num = substr($searchword,3,2);
			$khan_num = substr($searchword,5,2);
			$sudong_num = substr($searchword,7,4);
			$SQL = "select count(item_no) from $ItemTable where $con and sea_num='$sea_num' and sung_num='$sung_num' and khan_num='$khan_num' and sudong_num='$sudong_num' and if_hide='0' and mart_id='$mart_id' $add_query $gamaeng_query";
		}else{
			$SQL = "select count(item_no) from $ItemTable where $con and $select_key like '%$searchword%' and if_hide='0' and mart_id='$mart_id' $add_query $gamaeng_query";
		}
}else{
	$SQL = "select count(item_no) from $ItemTable where $con and mart_id='$mart_id' and if_hide='0' $add_query $gamaeng_query";
}
...
//유효기간수
$sql1 = "select * from $ItemTable where $con and  if_hide='0' and provider_id != '' and start_date <= '$today' and end_date >= '$today' $gamaeng_query";
...
//유효기간보류수
$sql2 = "select * from $ItemTable where $con and if_hide='0' and provider_id != '' and end_date < '$today' and (date_format(date_add(end_date, interval 1 day), '%Y-%m-%d') >= '$today') $gamaeng_query";
$result2 = mysql_query($sql2,$dbconn);
$numRows2 = @mysql_num_rows($result2);

//유효기간완료수
$sql3 = "select * from $ItemTable where $con and if_hide='0' and provider_id != '' and  (date_format(date_add(end_date, interval 1 day), '%Y-%m-%d') < '$today') $gamaeng_query";
...
?>
					...
					<tr>
						<td width="100%" colspan="2" align=right>
							...
							<table border="0" width="270" cellspacing="0" cellpadding="0" class="box3">
								<tr>
			...
			<table width="100%" border="0" cellpadding="0" cellspacing="0">
				<tr>
				<td width="100%" height="50" bgcolor="F2F2F2" class="stitle"><img src="../images/icon_1.gif" width="30" height="17" align="absmiddle"><b>현재그룹 : <?=$cur_category_name?></b></td>
				</tr>
			</table>
					<table border="0" width="100%" cellspacing="0" cellpadding="0">
...
if($searchword !=''){
		...
		if($select_key == "item_code"){
			$SQL = "select * from $ItemTable where $con and sea_num='$sea_num' and sung_num='$sung_num' and khan_num='$khan_num' and sudong_num='$sudong_num' and if_hide='0' and mart_id='$mart_id' and provider_id != '' $add_query $gamaeng_query";
		}else{
			$SQL = "select * from $ItemTable where $con and $select_key like '%$searchword%' and $con and mart_id='$mart_id' and if_hide='0' and provider_id != '' $add_query $gamaeng_query order by item_order desc, item_no desc";
		}
	
}else{
	$SQL = "select * from $ItemTable where $con and mart_id='$mart_id' and if_hide='0' and provider_id != '' $add_query $gamaeng_query order by item_order desc, item_no desc ";
}
...
?>
					<tr>
						...
						<td width="80%" bgcolor="#FFFFFF" height="0" align="right">
							...
							<br>
							<br>
							<input type=image src='../images/btn_search.gif' style='cursor:hand;'>
							<img src='../images/btn_clear.gif' onClick="location.href='<?=$PHP_SELF?>?pu=<?=$pu?>&category_num=<?=$category_num?>'" style='cursor:hand;'>
							
<?
if($_SESSION["MemberLevel"] == 3){
?>
						&nbsp;&nbsp;&nbsp;	<img src='../images/btn_mem.gif' onClick="check_ver('<?=$first_no?>','<?=$second_no?>','<?=$thirdno?>','<?=$category_num?>')" style='cursor:hand;'>
						 &nbsp;
<?
}
?>
				...
				<td width="100%" bgcolor="#FFFFFF" valign="top" align="center">
					...
					<tr>
						<td width="100%">
							...
							<tr>
								<td width="100%"  height="40" class="title3" colspan="9">
									현재 그룹에 등록된 회원 리스트
								</td>
							</tr>
							<tr align="center">
								...
								<td width="10%" class="title">고유번호</td>
								<td width="7%" class="title">회원번호</td>
								<td width="10%" class="title">회원명</td>
								<td width="18%" class="title">회원기간</td>
...
?>
							<tr onMouseOver="this.style.backgroundColor='#DDF0FF'" onMouseOut="this.style.backgroundColor='#FFFFFF'" bgcolor="#FFFFFF" align='center'>
								...
								<td><?=$j?></td>
								<td><a href='item_edit.php?gamaeng=<?=$gamaeng?>&item_no=<?=$item_no?>&prevno=<?=$prevno?>&prevno2=<?=$prevno2?>&category_num=<?=$category_num?>&page=<?=$page?>&searchword=<?=$searchword?>&select_key=<?=$select_key?>&date_expire=<?=$date_expire?>&date_expire2=<?=$date_expire2?>&pu=<?=$pu?>'><?=$sea_num?><?=$sung_num?><?=$khan_num?><?=$sudong_num?></a></td>
								<td><a href='item_edit.php?gamaeng=<?=$gamaeng?>&item_no=<?=$item_no?>&prevno=<?=$prevno?>&prevno2=<?=$prevno2?>&category_num=<?=$category_num?>&page=<?=$page?>&searchword=<?=$searchword?>&select_key=<?=$select_key?>&date_expire=<?=$date_expire?>&date_expire2=<?=$date_expire2?>&pu=<?=$pu?>'><?=$item_code?></a></td>
								<td><?=$link_str?></td>
...
<?			
	if($_SESSION["MemberLevel"] == 3 || $_SESSION["MemberLevel"] == 10){
?>

				<img src='../images/btn_modify.gif' onClick="modify_item(this.form)" style='cursor:hand;'>
				<img src='../images/btn_delete.gif' onClick="delete_item(this.form)" style='cursor:hand;'>
<?
}else if (($_SESSION["Admin_type"] == "mem" || $_SESSION["Admin_type"] == "gamaeng") && $_SESSION["Admin_level"] == 3){
?>	
				<img src='../images/btn_modify.gif' onClick="modify_item(this.form)" style='cursor:hand;'>
				<img src='../images/btn_delete.gif' onClick="delete_item(this.form)" style='cursor:hand;'>
...
if($flag == "modify_item"){		
	...
	for($i=0; $i<count($checkSel); $i++) {
		...
		echo "<meta http-equiv='refresh' content='0; URL=item_list.php?gamaeng=$gamaeng&category_num=$prevno&searchword=$searchword&select_key=$select_key&date_expire=$date_expire&date_expire2=$date_expire2&page=$page&pu=$pu'>";
}
//삭제요청
if($flag == "delete_item"){		
	for($i=0; $i<count($checkSel); $i++) {
