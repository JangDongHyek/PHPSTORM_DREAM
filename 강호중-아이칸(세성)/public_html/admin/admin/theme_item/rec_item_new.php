      ...
      <tr>
        ...
        <td valign="top" background="../img/top_2_bg.gif"><div align="right">
          <table width="100%" border="0" cellspacing="0" cellpadding="0">
            ...
            <tr>
              <td><div align="right"><img src="../img/top_icon.gif" width="13" height="15" align="absmiddle"> <span class="text_gray2_t">현재페이지</span><span class="text_gray2"> : <a href="../good/board_frame8.html">HOME</a> &gt; </span><span class="text_gray2_c">상품관리 &gt; 테마상품 관리 </span> </div></td>
            </tr>
            <tr>
              <td height="28">&nbsp;</td>
            </tr>
...
<br>
				...
				<table border="1" cellpadding="5" cellspacing="0" width="100%" bordercolordark="white" bordercolorlight="#E1E1E1" align="center">
					...
					<td width="90%" bgcolor="#FFFFFF" height="35">
						<strong>[추천상품 등록]</strong> 등록되어진 제품 중 
						추천상품으로 등록합니다.<br>
						등록한 추천상품은 메인화면의 추천상품영역에 출력됩니다.
					  </td>
					</tr>
					<tr>
...
?>
							...
							<tr>
								<td width="100%" bgcolor="#CCCCCC">
									...
									<form action='rec_item_new.php' method='post' name='list'>
									<input type='hidden' name='flag' value='to_rec_item'>
									<?
									if($flag == 'search'){
										if($keyset != 'category_num'){
											$SQL = "select item_no, item_name, item_company, z_price, reg_date from $ItemTable T1,$CategoryTable T2 where T1.mart_id='$mart_id' and T1.if_hide='0' and T1.category_num=T2.category_num and T2.if_hide='0' and T1.$keyset like '%$searchword%' UNION select T1.item_no, T1.item_name, T1.item_company, T1.z_price, T1.reg_date from $ItemTable T1, $Gnt_ItemTable T2 where T2.seller_id='$Mall_Admin_ID' and T1.item_no= T2.item_no and T1.$keyset like '%$searchword%' order by $order asc";
									...
									elseif($category_degree == 2)
										...
										$item_keyset = "category_num";
											...
											$SQL = "select T1.mart_id, item_no, item_name, z_price, item_company, reg_date from $ItemTable T1,$CategoryTable T2 where T1.if_hide='0' and T1.category_num=T2.category_num and T2.if_hide='0' and T1.$item_keyset ='$searchword' UNION select T1.mart_id, T1.item_no, T1.item_name, T1.z_price, T1.item_company, T1.reg_date from $ItemTable T1, $Gnt_ItemTable T2 where T2.seller_id = '$Mall_Admin_ID' and T1.item_no = T2.item_no and T2.$keyset ='$searchword' order by $order asc";
									...
									<td width='100%' height='5' align='center' colspan='6'>
										...
										<td width='6%' height='10' bgcolor='$bgcolor' align='center'>
											...
											<input type='hidden' name='checkSel_name[]' value='$item_name'></td>
										<td width='6%' height='10' bgcolor='$bgcolor' align='center'>
											$j</td>
...
if ($flag == "to_rec_item"){
	for($i=0; $i<count($checkSel); $i++) {
		...
		$sql0 = "select firstno, prevno, category_num from $ItemTable where item_no = $item_no ";
		...
		$SQL = "select count(*) from $Rec_ItemTable where mart_id='$mart_id' and item_no = $item_no";
		$dbresult = mysql_query($SQL, $dbconn);
		...
		$SQL = "select max(rec_item_order), count(*) from $Rec_ItemTable";
		$dbresult = mysql_query($SQL, $dbconn);

		if ($dbresult == false) echo "쿼리 실행 실패!";

		if (mysql_result($dbresult,0,1) > 0) 
			$maxRec_Item_order = mysql_result($dbresult, 0, 0);
		else
			$maxRec_Item_order = 0;
		
		$maxRec_Item_order = $maxRec_Item_order + 1;
		
		$SQL = "insert into $Rec_ItemTable (item_no, firstno, prevno, category_num, mart_id, provider_id, rec_item_order) values ( '$item_no', '$firstno', '$prevno', '$category_num', '$mart_id', '$provider_id', $maxRec_Item_order)";
