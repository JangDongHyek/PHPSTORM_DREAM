...
<?
//================== 1차 분류 ========================================================
$SQL = "select * from jungbo_cate where mart_id='$mart_id' and category_num > 28 and prevno='0' order by cat_order desc";
...
?>
								...
								<tr onMouseOver="this.style.backgroundColor='#DDF0FF'" onMouseOut="this.style.backgroundColor='#FFFFFF'" bgcolor="#FFFFFF">
									<td align='left'>&nbsp;&nbsp;<b><?=$category_name?></b></a>
									</td>
									<td align='center'><?=$gigan?>일</td>
									<td align='center'><?=number_format($money)?>원</td>
									<td align=center>
										
										<input class='aa' onClick="location.href='category_edit.php?category_num=<?=$category_num?>'" style='BACKGROUND-COLOR: white; BORDER-BOTTOM: #5a5a5a 1px solid; BORDER-LEFT: #5a5a5a 1px solid; BORDER-RIGHT: #5a5a5a 1px solid; BORDER-TOP: #5a5a5a 1px solid; COLOR: black; HEIGHT: 18px' type='button' value='구입하기'>
<?
...
?>
									...
									if($today > $row[end_date]){
										$font_color1 = "<font color=#BDBDBD>";
										$font_color2 = "</font>";
									}
									?>
									<?=$font_color1?><?=$row[start_date]?><?=$font_color2?>
									
									</td>
									<td align='center'>
									
									<?=$font_color1?><?=$row[end_date]?><?=$font_color2?>
