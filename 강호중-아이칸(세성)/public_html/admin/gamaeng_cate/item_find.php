...
$if_gnt_item = mysql_result($dbresult,0,0); //0:일반홈페이지 1:공급홈페이지 2:판매홈페이지
...
if ($flag == "del_item") {
	for($i=0; $i<count($checkSel); $i++) {
		...
		//if($Mall_Admin_ID == $mart_id){ // 내상품이면
			...
			//다른 홈페이지들 gnt_item 테이블에서 삭제
			$SQL = "delete from $Gnt_ItemTable where item_no = $item_no";
			//echo "sql=$SQL";
			$dbresult = mysql_query($SQL, $dbconn);
			
			//내홈페이지과 다른 홈페이지들 신상품, 인기상품, 추천상품, 선물 에서 삭제
			//신상품에서 삭제	
