    	...
    	<table border="0" width="100%" cellspacing="0" cellpadding="0">
      	...
      	<tr>
        	<td width="100%" bgcolor="#F2F2F2"><p style="padding-left: 5px"><span class="bb"><br>
        		<small>▶</small> <font face="돋움">쇼핑몰 <strong>분류 순서</strong>를<strong> 
        		<br>
        		&nbsp;&nbsp; 조정</strong>합니다.<br>
        		</font><br>
        		</span></td>
    	...
    	<table border="0" width="100%" cellspacing="0" cellpadding="0">
      	<tr>
        	<td width="90%" bgcolor="#FFFFFF" valign="top"><br>
        		<p style="padding-left: 10px"><span class="aa">분류의 순서를 변경하실 수 
        		있습니다. <br>
        		강조하고 싶거나 혹은 중요한 분류를 상위에 출력시킬 수 
        		있습니다. <br>
        		순서조정은 각 분류에 해당하는 이미지 ▽(아래), △(위) 를 
        		클릭하세요.<br>
        		<br>
        		<br>
        		</span>
        	...
        	<td width="100%" bgcolor="#FFFFFF" valign="top">
            			...
            			<table border="0" width="100%" cellspacing="1" cellpadding="3">
              			<tr>
                			<td width="100%" bgcolor="#8FBECD" colspan="2">
                				<table border="0" width="100%" cellspacing="0" cellpadding="0">
                  				<tr>
                    				<td width="50%">&nbsp; <strong><span class="dd">등록된 분류 목록</span></strong></td>
                    				<td width="50%"></td>
                  				</tr>
                				</table>
                			</td>
              			...
              			<?
						$SQL = "select * from jungbo_cate where prevno=0 and mart_id='$mart_id' order by cat_order desc";
						$dbresult = mysql_query($SQL, $dbconn);
						$numRows = mysql_num_rows($dbresult);
						?>
      					<tr>
                			<td width="23%" bgcolor="#EEEEEE" align="left">
                				<span class="aa"><p align="center">분류명</span></td>
              ...
              ");
					     	...
					     	if($i < $numRows - 1){
									...
									<img src='../images/down.gif' border='0' alt='현재 분류 내리기' WIDTH='23' HEIGHT='17'></a>
									");	
								}
								else{
									echo ("
								...
								if($i > 0){	
									...
									<img src='../images/up.gif' alt='현재 분류 올리기' border='0' WIDTH='23' HEIGHT='17'></a>
									");
								}
...
if($flag == "up"){
	$SQL = "select cat_order from jungbo_cate where prevno=0 and cat_order > $cat_order and mart_id='$mart_id' order by cat_order Asc";
	$dbresult = mysql_query($SQL, $dbconn);
	$numRows = mysql_num_rows($dbresult);
...
if($flag == "down"){
	$SQL = "select cat_order from jungbo_cate where prevno=0 and cat_order < $cat_order and mart_id='$mart_id' order by cat_order Desc";
	$dbresult = mysql_query($SQL, $dbconn);
	$numRows = mysql_num_rows($dbresult);
	mysql_data_seek($dbresult, 0);	
	$ary=mysql_fetch_array($dbresult);	
