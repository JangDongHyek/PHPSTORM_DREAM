...
$rows = mysql_fetch_array($dbresult);
	...
	if($rows[job_gubun] == 1){
		echo"<font size=3>[구인업체]</font>";
	}
	elseif($rows[job_gubun] == 2){
		echo"<font size=3>[용역업체]</font>";
	}
	elseif($rows[job_gubun] == 3){
		echo"<font size=3>[근로자]</font>";
	}
?>
</div>
...
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="box2">
	...
	<tr>
      <td align=center width=15% class="title">종목</td> 
      <td width=85% bgcolor="#FFFFFF" colspan=3>
			<select name="ext1" class="select_01">
			<option value=''>=종목선택=</option>
			<?
			$sql = "select * from job_jongmok where 1 order by cat_order desc";	
			$res = mysql_query($sql,$dbconn);
			...
			<p class="flex">금액 : <input type=text name="ext3" id="ext3" value="" class="input_03"></p>
            <p class="flex">인원수 : <input type=text name="ext2" id="ext2" value="" class="input_03"></p>
		</td>
    </tr>
	...
	<tr>
      <td align=center width=15% class="title">종목</td> 
      <td width=85% bgcolor="#FFFFFF" colspan=3>
			<select name="ext11" class="select_01">
			<option value=''>=종목선택=</option>
			<?
			$sql = "select * from job_jongmok where 1 order by cat_order desc";	
			$res = mysql_query($sql,$dbconn);
          ...
          <p class="flex">금액 : <input type=text name="ext12" id="ext12" value="" class="input_03"></p>
          <p class="flex">인원수 : <input type=text name="ext13" id="ext13" value="" class="input_03"></p>
		</td>
    </tr>
	...
	<tr>
      <td align=center width=15% class="title">종목</td> 
      <td width=85% bgcolor="#FFFFFF" colspan=3>
			<select name="ext14" class="select_01">
			<option value=''>=종목선택=</option>
			<?
			$sql = "select * from job_jongmok where 1 order by cat_order desc";	
			$res = mysql_query($sql,$dbconn);
          ...
          <p class="flex">금액 : <input type=text name="ext15"  value="" class="input_03"></p>
          <p class="flex">인원수 : <input type=text name="ext16"  value="" class="input_03"></p>
		</td>
    </tr>
	...
	<tr>
      <td align=center width=15% class="title">종목</td> 
      <td width=85% bgcolor="#FFFFFF" colspan=3>
			<select name="ext17" class="select_01">
			<option value=''>=종목선택=</option>
			<?
			$sql = "select * from job_jongmok where 1 order by cat_order desc";	
			$res = mysql_query($sql,$dbconn);
          ...
          <p class="flex">금액 : <input type=text name="ext18"  value="" class="input_03"></p>
          <p class="flex">인원수 : <input type=text name="ext19"  value="" class="input_03"></p>
		</td>
    </tr>
	...
	<tr>
      <td align=center width=15% class="title">종목</td> 
      <td width=85% bgcolor="#FFFFFF" colspan=3>
			<select name="ext20" class="select_01">
			<option value=''>=종목선택=</option>
			<?
			$sql = "select * from job_jongmok where 1 order by cat_order desc";	
			$res = mysql_query($sql,$dbconn);
          ...
          <p class="flex">금액 : <input type=text name="ext21"  value="" class="input_03"></p>
          <p class="flex">인원수 : <input type=text name="ext22"  value="" class="input_03" /></p>
		</td>
    </tr>
    ...
    <tr>
      <td align=center width=15% class="title">용역업체</td> 
      <td width=85% bgcolor="#FFFFFF" colspan=3>
			<select name="ext5" class="select_01">
			<option value=''>=선택=</option>
			<?
			$sql = "select * from item where job_gubun='2' order by item_no desc";	
			$res = mysql_query($sql,$dbconn);
			for($i=0;$rows=mysql_fetch_array($res);$i++){
			?>
    ...
    <tr>
      <td align=center width=15% class="title">결제사인</td> 
      <td width=85% bgcolor="#FFFFFF" colspan=3>
...
*/
	...
	$SQL = "insert into $New_BoardTable (index_no, firstno, prevno, thirdno, category_num, country_num,sea_num,sung_num,khan_num,bbs_no, notice_no, mart_id, code, username, writer, passwd, write_date, ansno, step, thread, email, subject_new, content, userfile, userfile1, if_secret, writer_ip, area, address,address2,tel,hobby,goyu_num,open_num,open_address,open_address2,open_tel,open_hobby,open_email,my_bank_name,my_bank_account,my_bank_master,open_bank,end_date,open_auth,del_chk,bunryu,bubun,price,jungbo_gubun,gigan_day,danwi,sea_area,sung_area,khan_area,ext1,ext2,ext3,ext4,ext5,ext6,ext7,ext8,ext9,ext10,ext11,ext12,ext13,ext14,ext15,ext16,ext17,ext18,ext19,ext20,ext21,ext22) values ('$index_no', '$firstno', '$prevno', '$thirdno', '$category_num','$country_num','$sea_num','$sung_num','$khan_num', '$bbs_no', '$max_notice', '$mart_id', '$code', '$_SESSION[Mall_Admin_ID]', '$writer', '$passwd', '$write_date', '1', '0', '$thread', '$email', '$subject_new_tmp[0]', '$content', '$file', '$file1', '$if_secret', '$REMOTE_ADDR', '$item_no', '$address22','$address2','$tel','$hobby','$goyu_num','$open_num','$open_address','$open_address2','$open_tel','$open_hobby','$open_email','$my_bank_name','$my_bank_account','$my_bank_master','$open_bank','$end_date','y','n','$bunryu','$bubun','$price','$jungbo_gubun','$gigan_day','$danwi','$sea_area','$sung_area','$khan_area','$ext1','$ext2','$ext3','$ext4','$ext5','$ext6','$ext7','$ext8','$ext9','$ext10','$ext11','$ext12','$ext13','$ext14','$ext15','$ext16','$ext17','$ext18','$ext19','$ext20','$ext21','$ext22')";
