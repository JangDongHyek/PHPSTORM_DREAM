...
*/
?>
<?=$ary[ga_sea_num]?><?=$ary[ga_sung_num]?><?=$ary[ga_khan_num]?><?=$ary[ga_last_num]?><?=$ary[ga_sudong_num]?>
					...
					<table width="100%" align="center" border="0" cellspacing="0" cellpadding="0">
						<tr>
							...
							*/
							...
							if($username == $_SESSION[Mall_Admin_ID]  || $_SESSION["Admin_type"]=="gamaeng"){
							?>
							...
							<?
							//가맹점 게시판은 칸 본인만 수정,삭제요청
							}elseif($_SESSION["Admin_type"]=="gamaeng" && $_SESSION[Admin_level]=="3"){
							?>
								<span style="cursor:hand;" onclick="javascript:window.open('./request_update.php?bbs_no=<?=$bbs_no?>&index_no=<?=$index_no?>','','width=500,height=500,top=100,left=200');"><img src="../image/helpdesk/btn_modify2.gif" border=0></span>
...
/*
	...
	*/
	...
	if(f.price.value==""){
		...
		var len =f.price.value.length;
		var ret;
		ret =false;		
		for(var i=0;i<len;i++){  
		    var ch = f.price.value.substring(i,i+1);
...
<?
$goyu_num = $ary[ga_sea_num].$ary[ga_sung_num].$ary[ga_khan_num].$ary[ga_last_num].$ary[ga_sudong_num];	
?>
				<form action='board_read.php' method='post' onsubmit="return frm_val(this)">
				<input type='hidden' name='jungbo_price' value='y'>
				<input type='hidden' name='group_id' value='<?=$username?>'>
				<input type='hidden' name='bbs_no' value='<?=$bbs_no?>'>
				<input type='hidden' name='index_no' value='<?=$index_no?>'>
				<input type='hidden' name='goyu_num' value='<?=$goyu_num?>'>
				<input type='hidden' name='sangho' value='<?=$ary[subject_new]?>'>
				...
				<b>보낼충전금</b>						
						<input name="price" class="input_03" size="15"> 
...
if($jungbo_price == "y"){
			...
			$mem_num = $row[sea_num].$row[sung_num].$row[khan_num].$row[last_num].$row[sudong_num];
			...
			if($bonus_total_str < $bonus){
				...
				//받는사람저장
				$content = $row[sea_num].$row[sung_num].$row[khan_num].$row[last_num].$row[sudong_num];
				$mode = "ug";
				$id = $goyu_num;//받는사람정보
			
				$SQL = "insert into $BonusTable (mart_id, bonsa_id, group_id, id, write_date, bonus, content, mode, bbs_no, index_no)  values ('$mart_id', '$bonsa_id', '$group_id', '$id', '$write_date', '$bonus', '$content', '$mode','$bbs_no','$index_no')";
				...
				//보낸사람저장
				$content = $goyu_num."님께 충전금 보냄(가맹점명:".$sangho.")";
				
				$mode = "us";
				$id=$row[sea_num].$row[sung_num].$row[khan_num].$row[last_num].$row[sudong_num];
				...
				echo("
					<script>
					alert('$goyu_num 님께 충전금 $bonus 원을 보냈습니다.');
					</script>
					<meta http-equiv='Refresh' content='0; URL=board_read.php?mart_id=$mart_id&bbs_no=$bbs_no&index_no=$index_no'>
				");
				exit;
