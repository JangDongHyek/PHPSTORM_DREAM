...
$mem_sql = "select * from $ItemTable where item_id ='$_SESSION[Mall_Admin_ID]'";
$mem_res = mysql_query( $mem_sql, $dbconn );
$mem_row = mysql_fetch_array( $mem_res );

$mem_num = $mem_row[sea_num].$mem_row[sung_num].$mem_row[khan_num].$mem_row[sudong_num];
$sum_sql = "select sum(bonus) as bonus_total from $BonusTable where id='$mem_num'";
...
?>
								...
								if($_SESSION["MemberLevel"] == 4){ //회원만
									if($username != $_SESSION[Mall_Admin_ID]){ //자기글은 신청안되게
										if($bonus_total_str >= $ary[meet_point]){
								?>
								<div align=center style="cursor:hand;" onclick="javascript:window.open('./offer.php?bbs_no=<?=$bbs_no?>&index_no=<?=$index_no?>','','width=500,height=550,top=100,left=200');"><b>[만남요청]</b></div>
								<?
										}else{
								?>
								<div align=center style="cursor:hand;" onclick="javascript:alert('포인트가 부족합니다.');"><b>[만남요청]</b></div>
								<?
									}
								}
}
...
<tr>
	...
	<tr>
      <td align=center class="title">작성자</td> 
      <td bgcolor="#FFFFFF">
			<?=$ary[writer]?>
	  </td>
      <td align=center class="title">포인트</td> 
      <td bgcolor="#FFFFFF">
			<?=number_format($ary[meet_point])?>
	  </td>
	...
	<tr>
      <td align=center class="title">성별</td> 
	...
	<tr>
      <td align=center class="title">예명및암구호</td> 
      <td bgcolor="#FFFFFF" colspan=3>
			<?=$ary[ext2]?> 
	  </td>
	...
	<tr>
      <td align=center class="title">계좌번호</td> 
      <td bgcolor="#FFFFFF" colspan=3>
		<?=$ary[ext3]?> 
	  </td>
	...
	<tr>
      <td align=center class="title">생년월일</td> 
      <td bgcolor="#FFFFFF" colspan=3>
			<?=$ary[ext4]?> 
	  </td>
...
?>
	<tr>
      <td align=center class="title">전화번호</td> 
      <td bgcolor="#FFFFFF" colspan=3><?=$ary[tel]?></td>
    </tr>
<?}?>
...
?>
					<table width="100%" align="center" border="0" cellspacing="0" cellpadding="0">
						<tr>
							...
							<?
							//자기글일때 수정요청,삭제요청	
							if($username == $_SESSION[Mall_Admin_ID] || $_SESSION[Admin_type]=="jmeet"){
							?>
							...
							<?
							//자기글일때 수정요청,삭제요청	
							}else if($_SESSION[Admin_type]=="jmeet"  && $_SESSION[Admin_level]=="3"){
							?>
								<span style="cursor:hand;" onclick="javascript:window.open('./request_update.php?bbs_no=<?=$bbs_no?>&index_no=<?=$index_no?>','','width=500,height=500,top=100,left=200');"><img src="../images/board/modify_butten.gif" border=0></span>
...
<?
	if( $username == $_SESSION[Mall_Admin_ID] ){
		$sql1 = "select * from offer where pre_username='$_SESSION[Mall_Admin_ID]' and index_no = '$index_no'"; 
...
?>
              			<tr>
                			...
                			<td bgColor="#FFFFFF" height="10" colspan="2"></td>
									...
									<tr valign='top'>
										<td width='10%'><span style="cursor:hand;" onclick="javascript:window.open('./offer_detail.php?seq_num=<?=$ary1[seq_num]?>&username=<?=$username?>&meet_point=<?=$meet_point?>','','width=500,height=500,top=100,left=200,scrollbars=yes');">
										<?if($ary1[state]=='y'){?>
										<font color=blue>
										<?}?>
										<?if($ary1[misu]=='y'){?>
