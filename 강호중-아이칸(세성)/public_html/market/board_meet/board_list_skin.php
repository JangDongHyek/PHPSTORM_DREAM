...
<script language="javascript">
						...
						<input type="hidden" name="page1" value="<?=$page?>">
							<tr>
								...
								<td width='80' align='center' class="title">작성자</td>
								<td width='80' align='center' class="title">성별</td>
...
<?
	...
	if($address22){
		$open_chk_query .= " and address like '%$address22%' ";
	}
	if($price){
        $price = str_replace(",", "", $price);
        $open_chk_query .= " and ext8 = '$price' ";
	}
	if($jungbo_gubun){
	...
	if($_REQUEST['order'] == 'recent'){
        ...
        $SQL3 = "order by meet_point asc limit $skipNum, $cnfPagecount";
    }else if($_REQUEST['order'] == 'price_desc'){
        $SQL3 = "order by meet_point desc limit $skipNum, $cnfPagecount";
    }else{
...
<?
	...
	for ($i=0; $i < $numRows; $i++){
		...
		//if($my_list=='y'){
			$sql9 = "select * from offer where pre_username='$_SESSION[Mall_Admin_ID]' and index_no = '$index_no'"; 
...
?>
							<tr height='30' align='center' onclick="location.href='board_read.php?index_no=<?=$index_no?>&bbs_no=<?=$bbs_no?>&page=<?=$page?>&mart_id=<?=$mart_id?>&keyset=<?=$keyset?>&searchword=<?=$searchword?>&my_list=<?=$my_list?>'">
								...
								<td width='100' >
									...
									<div class="meet_img"><?=$img_file?></div>
								</tD>
								<td width='100' >
									<?
			...
			?>
								...
								<a href='board_read.php?index_no=<?=$index_no?>&bbs_no=<?=$bbs_no?>&page=<?=$page?>&mart_id=<?=$mart_id?>&keyset=<?=$keyset?>&searchword=<?=$searchword?>&my_list=<?=$my_list?>'><?=$row[writer]?>
