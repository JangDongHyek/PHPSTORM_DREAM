...
<script language="javascript">
						...
						<input type="hidden" name="page1" value="<?=$page?>">
							<tr>
								...
								<td width='80' align='center' class="title">상호</td>
								<td width='80' align='center' class="title">구인/구직</td>
								<td width='80' align='center' class="title">모집구분</td>
								<td width='80' align='center' class="title">급여</td>
...
<?
	...
	if($ext8){
		$open_chk_query .= " and ext8 = '$ext8' ";
	}
	if($price){
        $price = str_replace(",", "", $price);
		$open_chk_query .= " and ext9 = '$price' ";
	}
	if($ext11){
		$open_chk_query .= " and ext11 = '$ext11' ";
	}
	...
	$SQL2_1 = "and (binary writer like '%$searchword%' 
					...
					or ext11 like '%$searchword%'
					or ext1 like '%$searchword%'
					or ext9 like '%$searchword%'
					) ";
	$SQL3 = "";
    ...
    if($_REQUEST['order'] == 'recent'){
        ...
        $SQL3 = "order by ext9 asc limit $skipNum, $cnfPagecount";
    }else if($_REQUEST['order'] == 'price_desc'){
        $SQL3 = "order by ext9 desc limit $skipNum, $cnfPagecount";
    }else{
	...
	for ($i=0; $i < $numRows; $i++){
		...
		//========================= userfile =====================================================
			if( eregi("\.jpg", $fRow['file_name']) || eregi("\.gif", $fRow['file_name']) || eregi("\.JPG", $fRow['file_name']) || eregi("\.GIF", $fRow['file_name'])|| eregi("\.png", $fRow['file_name'])|| eregi("\.PNG", $fRow['file_name']) ){
                ...
                //23.10.27 이미지 오류나면 안보이게 수정 wc
				$img_file = "<img src='$encode_target' width='100' border='0' hspace='5' style='border: #000000; border-style: solid; border-width:1px' onerror='this.style.display=\"none\"'><br>".$img_file;
			}else if(eregi("\.swf", $fRow['file_name'])){
				$img_file = "<embed src='$encode_target' border='0'><br>".$img_file;
			}
		}
?>
							<tr height='30' align='center' onclick="location.href='board_read.php?index_no=<?=$index_no?>&bbs_no=<?=$bbs_no?>&page=<?=$page?>&mart_id=<?=$mart_id?>&keyset=<?=$keyset?>&searchword=<?=$searchword?>&my_list=<?=$my_list?>'">
								<td width='70' class='point'><?if($row[notice_no] >= '100000'){echo"<b>[공지]</b>";}else{echo"$k";}?></td>
								<td width='100' ><?=$img_file?></td>
								<td width='100' >
									<?
			...
			?>
								...
								<a href='board_read.php?index_no=<?=$index_no?>&bbs_no=<?=$bbs_no?>&page=<?=$page?>&mart_id=<?=$mart_id?>&keyset=<?=$keyset?>&searchword=<?=$searchword?>&my_list=<?=$my_list?>'><?if($row[notice_no] >= '100000'){?><b><?}?>
									<?
									...
									?>									
											...
											<?}else{?>
			<a onclick="javascript:alert('아이칸 등록후 이용 가능합니다.');"><b><?=$subject_new?></b></a>
			<?}?>
                                ...
                                <td class="40">[<?=$numRows9?>]</td>
								<td width='80'>
									<?			
									if($_SESSION[Mall_Admin_ID]){		
									?>
										<a href='board_read.php?index_no=<?=$index_no?>&bbs_no=<?=$bbs_no?>&page=<?=$page?>&mart_id=<?=$mart_id?>&keyset=<?=$keyset?>&searchword=<?=$searchword?>&my_list=<?=$my_list?>'><?=$row[ext8]?>				
									<?}else{?>
										<a onclick="javascript:alert('샘플화면 입니다.');"><b><?=$subject_new?></b></a>
									<?}?>		
								</td>
								<td width='80'><?=$row[ext11]?></td>
								<td width='80'><?=$row[ext1]?></td>
								<td width='80'><?=number_format($row[ext9])?></td>
								<td width='80' class='point'><?=$write_date_str?></td>
								<td width='70' class='point'><?=$read_num?></td>
							</tr>
							<tr>
								<td bgcolor='E1E1E1' height='1' colspan='10'></td>
							</tr>
<?     				
	$img_file="";}
?>
