...
<tr>
	...
	<tr>
      <td align=center class="title">면적</td> 
      <td bgcolor="#FFFFFF" colspan=3>
			<?=$ext1?>
	  </td>
	...
	<tr>
      <td align=center class="title">공시지가</td> 
      <td bgcolor="#FFFFFF" colspan=3>
			<?=$ext3?>
	  </td>
	...
	<tr>
      <td align=center class="title">준공일자</td> 
      <td bgcolor="#FFFFFF" colspan=3>
		<?=$ext4?>
	  </td>
	...
	<tr>
      <td align=center class="title">가격</td> 
      <td bgcolor="#FFFFFF" colspan=3>
			<?
			if($ext5){
			?>
			보증금:<?=number_format($ext5)?>만원 월임대료:<?=number_format($ext6)?>만원
			<?}?>
			<?
			if($ext6){
			?>
			전세:<?=number_format($ext7)?>만원
			<?}?>
			<?
			if($ext7){
			?>
			매매:<?=number_format($ext8)?>만원
			<?}?>	  </td>
					...
					<table width="100%" align="center" border="0" cellspacing="0" cellpadding="0">
						<tr>
							...
							*/
							...
							<?
							//자기글일때 수정요청,삭제요청	
							if($username == $_SESSION[Mall_Admin_ID] || $_SESSION[Admin_type]=="jreal"){
							?>
							...
							<?
							//자기글일때 수정요청,삭제요청	
							}elseif($_SESSION[Admin_type]=="jreal"   && $_SESSION[Admin_level]=="3"){
							?>
								<span style="cursor:hand;" onclick="javascript:window.open('./request_update.php?bbs_no=<?=$bbs_no?>&index_no=<?=$index_no?>','','width=500,height=500,top=100,left=200');"><img src="../images/board/modify_butten.gif" border=0></span>
