...
<?
	...
	if($price){
		...
		$open_chk_query .= " and ( ext5 = '$price' or ext6 = '$price' or ext7 = '$price' or ext8 = '$price' ) ";
	}
	if($jungbo_gubun){
    ...
    if($_REQUEST['order'] == 'recent'){
        ...
        $SQL3 = "order by ext5 asc limit $skipNum, $cnfPagecount";
    }else if($_REQUEST['order'] == 'price_desc'){
        $SQL3 = "order by ext5 desc limit $skipNum, $cnfPagecount";
    }else{
...
?>
			...
			if($_SESSION[Mall_Admin_ID]){		
			?>
								<a href='board_read.php?index_no=<?=$index_no?>&bbs_no=<?=$bbs_no?>&page=<?=$page?>&mart_id=<?=$mart_id?>&keyset=<?=$keyset?>&searchword=<?=$searchword?>&my_list=<?=$my_list?>'><?=$row[address]?>
			<?}else{?>
			...
			if($row[ext5]){
			?>
			보증금:<?=number_format($row[ext5])?>만원<br/> 월임대료:<?=number_format($row[ext6])?>만원
			<?}?>
			<?
			if($ext6){
			?>
			전세:<?=number_format($row[ext7])?>만원<br/>
			<?}?>
			<?
			if($ext7){
			?>
			매매:<?=number_format($row[ext8])?>만원<br/>
			<?}?>									
								</td>
