...
add_stylesheet('<link rel="stylesheet" href="'.G5_ADMIN_URL.'/css/admin-lnb.css">', 1);
add_stylesheet('<link rel="stylesheet" href="'.G5_THEME_CSS_URL.'/all.min.css">', 1);
...
<header id="hd">
    <div id="hd_wrap">
        ...
        <div id="logo"><a href="<?php echo G5_ADMIN_URL ?>/member_list.php"><img src="https://dreamforone.kr/~marien/theme/basic/img/common/logo.svg" alt="<?php echo $config['cf_title'] ?> 관리자"></a></div>
...
foreach($menu['menu'.$menu_key] as $key=>$value) {
    if($key > 0) {
       /* if ($is_admin != 'super' && (!array_key_exists($value[0],$auth) || !strstr($auth[$value[0]], 'r')))
        ...
        echo $nl.'<li'.$on_class.'><a href="'.$value[2].'"'.$svc_class.'>'.$value[1].'</a></li>';
        $nl = PHP_EOL;
    }
}
?>
