...
function member_delete($mb_id)
{
    ...
    //삭제를 탈퇴로
    $sql = " update {$g5['member_table']} set  mb_leave_date = '".date('Ymd', G5_SERVER_TIME)."',mb_memo = '".date('Ymd', G5_SERVER_TIME)." 탈퇴함\n{$mb['mb_memo']}' where mb_id = '{$mb_id}' ";
    sql_query($sql);

    // 회원 삭제
    //sql_query(" delete from {$g5['member_table']} where mb_id = '$mb_id' ");
    //sql_query(" delete from g5_member_contract where mb_id = '$mb_id' ");
...
function sendAlimTalk($type, $params=array(), $mb_hp, $mb_no="") {
    ...
    //240704 스팸으로인한 알람톡 막아둠wc
    //return;

    $template = array();
    $rep_field = array(
        "reject_reason" => "#{반려사유}", "send_date" => "#{입금월,일}", "name"=>"#{입금자명}", "member_name"=>"#{회원명}","register_name"=>"#{회원이름}","recommend_name"=>"#{추천인 정보}"
    );
    $od_id = 0;

    // 1) 업체알림용)간편상담신청
    $template[0]['code'] = "SJB_079643";
    $template[0]['title'] = "간편상담신청";
    $template[0]['content'] = "안녕하세요.\n";
    $template[0]['content'] .= "\n";
    $template[0]['content'] .= "#{회원명}님이, 간편상담신청을 신청하였습니다.";

    // 2) 업체알림용)데이터 폼 작성
    $template[1]['code'] = "SJB_079644";
    $template[1]['title'] = "데이터 폼 작성";
    $template[1]['content'] = "안녕하세요.\n";
    $template[1]['content'] .= "\n";
    $template[1]['content'] .= "#{회원명}님이, 데이터 폼 작성을 완료하였습니다.";

    // 3) 업체알림용)회원가입계약서 작성
    $template[2]['code'] = "SJB_079645";
    $template[2]['title'] = "계약서 작성";
    $template[2]['content'] = "안녕하세요.\n";
    $template[2]['content'] .= "\n";
    $template[2]['content'] .= "#{회원명}님이, 회원가입계약서 작성을 완료하였습니다.";

    // 4) 업체알림용)상담신청 작성
    $template[3]['code'] = "SJB_080958";
    $template[3]['title'] = "상담신청";
    $template[3]['content'] = "안녕하세요.\n";
    $template[3]['content'] .= "\n";
    $template[3]['content'] .= "#{회원명}님이, 무료상담신청을 신청하였습니다.";

    // 5) 업체알림용)자녀결혼문의 작성
    $template[4]['code'] = "SJB_080959";
    $template[4]['title'] = "자녀결혼문의";
    $template[4]['content'] = "안녕하세요.\n";
    $template[4]['content'] .= "\n";
    $template[4]['content'] .= "#{회원명}님이, 자녀결혼 문의 작성을 완료하였습니다.";
