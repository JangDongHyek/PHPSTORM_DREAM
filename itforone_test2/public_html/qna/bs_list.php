...
// 공통쿼리
$sql_common = "FROM project_qna WHERE is_notice = 'N' AND mid != '8910088' AND is_busan = 'Y' ";
...
else if (!empty($mid)) { $sql_common .= " AND mid = '{$mid}'"; $get_params['mid'] = $mid; }	// 업체명

if (!empty($stt)) { $sql_common .= " AND qa_status = '{$stt}'"; $get_params['stt'] = $stt; }	// 처리상태
if (!empty($stx)) { 	// 검색어
	if ($sfl == "mng_name") {
        $sql_common .= " AND qa_tel LIKE '%{$stx}%'";
	...
	<form name="sfrm" autocomplete="off" onsubmit="return frmSrch(this);">
		<ul>
			<li class="data">
                ...
                &nbsp;
                <em>확인요망</em>&nbsp;&nbsp;
                <span><input type="checkbox" name="chk" value="y" <?=($_GET['chk']=='y')?"checked":""?> style="width:auto;"></span>
			</li>
            <?/*
			<li class="company">
				<em>업체명</em>
            ...
            */?>
			<li class="state" style="padding:0;">
				<div class="state01">
					<em>처리상태</em>
					<span>
						<select name="stt">
		...
		<button type="button" onclick="location.href='./bs_list.php'">초기화</button>
	</form>
</div>
...
<div class="list_tbl">
    ...
    */?>
	...
	<!--<button type="button" class="btn btn_01" onclick="location.href='./bs_write.php?noti=y'">공지등록</button>-->
	<button type="button" class="btn btn_01" onclick="location.href='./bs_write.php'" style="margin-right: 5px;">수정접수등록</button>
	<dl>
		<dt>
		...
		<dd>
			...
			<div class="title col40">
				<a href="./bs_view.php?idx=<?=$row['idx']?><?=$params?>">
					<?=$row['qa_subject']?> 
					<?php if ($is_file) { ?><img src="./img/icon_file.gif"><?php }?>
					<?php if ($reply_cnt > 0) { ?><span class="reply"><?=$reply_cnt?></span><?php }?>
...
function frmSrch(f) {
	...
	// 업체명 select
	// var mid = f.mid.options[f.mid.selectedIndex].value;
	// if (mid.length > 0) filter.push("mid="+mid);
	// 처리상태 select
	var stt = f.stt.options[f.stt.selectedIndex].value;
	...
	var get = (filter.length > 0)? "?" + filter.join("&") : "";

	location.href = "./bs_list.php" + get;
	return false;
}
