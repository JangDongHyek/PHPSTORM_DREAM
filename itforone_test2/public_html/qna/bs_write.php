...
if (!empty($idx)) {
    // 수정내역(부산)
    $sql = "SELECT * FROM project_qna WHERE idx = '{$idx}' AND is_busan = 'Y'";
    $row = sql_fetch($sql);
    // $is_notice = ($row['is_notice'] == 'Y')? true : false;
} else {
    $row['qa_name'] = $member['mb_name'];
}
...
function frmSubmit(f) {
	...
	obj.qa_name = f.qa_name.value; // 작성자명
	obj.qa_tel = f.qa_tel.value; // 업체명 (재활용)
...
<div class="write_tbl">
    ...
    <input type="hidden" name="is_notice" value="N"><!-- 공지여부 -->
	...
	<dl>
        ...
        <dd>
            ...
            <span class="conts"><input type="text" name="qa_tel" value="<?=$row['qa_tel']?>" maxlength="40" required></span>
        </dd>
		<dd>
			<span class="subj">작성자명</span>
