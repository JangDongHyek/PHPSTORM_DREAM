...
$sql = " SELECT COUNT(DISTINCT `idx`) AS `cnt` FROM `new_map_code` WHERE {$sql_search} ";
$row = sql_fetch($sql);

$total_count = $row['cnt'] + $sub_cnt;
...
$sql = "SELECT * FROM `new_map_code` where $sql_search order by `dong` limit $from_record, $page_rows ";
$result = sql_query($sql);
