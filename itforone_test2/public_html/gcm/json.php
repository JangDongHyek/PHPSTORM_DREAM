...
if(@$_POST['RegID']!='') {$RegID=$mysql_query->real_escape_string($_POST['RegID']);}
if(@$_POST['ID']!='') {$ID=$mysql_query->real_escape_string($_POST['ID']);}
if(@$_POST['app_type']!='') {$app_type=$mysql_query->real_escape_string($_POST['app_type']);}

//else {$app_type=$mysql_query->real_escape_string('Android');}
if(@$_POST['DeviceID']!='') {$DeviceID=$mysql_query->real_escape_string(($_POST['DeviceID']));}

//if(@$_POST['model']!='') {$DeviceModel=$mysql_query->real_escape_string(mpassUnlock($_POST['model']));}
if($mode=="get"){
	...
	if($RegID){
		...
		if($count == null || is_numeric($count) == false){ // 쿼리문에 이상이 있을때
			...
			$query = $mysql_query->select("g5_member","*","`mb_id`='$ID'");
			$rows = $mysql_query->fetch($query);
			$mb_gcm = $rows['mb_gcm'];
			if($mb_gcm == 1) $mb_gcm = 0;
			else $mb_gcm = 1;
			...
			if($count > 0){ //아이디가 있으면
				...
				if($rows["DeviceID"] == $DeviceID){ //DeviceID가 같고 GCM키가 같지않다면 GCM키 업데이트
					...
					$mysql_query->delete($config->db_info->user_db,"`DeviceID`='$DeviceID'");
