...
if($wr_chk){
	...
	$wr_mktime = mktime(0,0,0, substr($wr_1, 4, 2), substr($wr_1, 6, 2), substr($wr_1, 0, 4));
	$wr_week = date("w", $wr_mktime);
	if(!$wr_week) $wr_week = 6;
...
if ($w == '' || $w == 'r') {
	...
	if($member['mb_name'] != $wr_10){
		$query = "SELECT * FROM `g5_member` WHERE mb_name = '$wr_10'";
...
$post = sql_fetch("select `mb_id` from `g5_member` where mb_name = '$wr_4'");
$postNo = "2";
$post_title = $wr_subject;
$post_url = "http://letsit.kr/~itforone_test2/bbs/board.php?bo_table=schedule&wr_id=".$wr_id;
$button_string = "열기";
$post_user = $post[mb_id];
...
if ($file_upload_msg)
    ...
    goto_url(G5_HTTP_BBS_URL.'/board.php?bo_table='.$bo_table."&$edit_date");
?>
