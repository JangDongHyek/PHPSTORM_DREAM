...
$OptionTable5 = "opt_table5";
$OptionTable6 = "opt_table6";
...
$row=mysql_fetch_array($result);
for($i=1;$i<6;$i++){
	${slideImg.$i}=$row[slideImg.$i];
	${slideLink.$i}=$row[slideLink.$i];
	${mobileslideImg.$i}=$row[mobileslideLink.$i];
	${mobileslideLink.$i}=$row[mobileslideLink.$i];
}

$occChoiceArray=array("card"=>"카드","ribon"=>"리본","card+ribon"=>"카드+리본");
$optionNameArray=array("","샴페인(무알콜)","케익","캔디","코사지","초코릿","장미");
$addInfoArray=array("1"=>"결혼기념일","2"=>"생일기념일");
$priceKindArray=array("기본가격","고급가격","특품가격");
//총 포인트 쿼리문
if($_SESSION[UnameSess]){
	$sum_sql = "select sum(bonus) as bonus_total from $BonusTable where mart_id ='$mart_id' and id='$_SESSION[UnameSess]'";
