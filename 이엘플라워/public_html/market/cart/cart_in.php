...
if($mode == "del_order"){
	...
	if($ispoint!="1"){
		...
		echo "<meta http-equiv='refresh' content='0; URL=order.html?mart_id=$mart_id&provider_id=$provider_id&item_no=$item_no&category_num=$category_num&category_num1=$category_num1&category_num2=$category_num2&cate_num=$cate_num&flag=$flag&ispoint=1'>";
	}
	exit;
}
if($mode == "cart_del"){
...
if($flag == "addorder"){
	...
	if($if_cash!="2"){
		...
		if (!isset($order_point_num) || $order_point_num=="") {
			...
			//$order_num = "$mart_id"."$order_num";
			$order_point_num = $order_point_num;		// 주문번호생성
		}
	}
	...
	$SQL = "select order_pro_no, quantity from $Order_ProTable where order_num=$order_num_value and mart_id='$mart_id' and item_no='$item_no' and opt='$opt' and opt2='$opt2' and opt3='$opt3' and opt4='$opt4' and opt5='$opt5' and opt6='$opt6' and status = '0'";
	$dbresult = mysql_query($SQL, $dbconn);
	$numRows = mysql_num_rows($dbresult);
	if ($numRows > 0) {
