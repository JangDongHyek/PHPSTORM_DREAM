...
if(0 < $use_bonus_tot){ //포인트 사용시 포인트 만큼 빼 준다.
...
if($paymethod=="kakao_pay"){
	...
	if($totalQuantity>1){
		$list['item_name']=$item_name."외 ".$totalQuantity."개";//상품명
	}else{
		$list['item_name']=iconv("euc-kr","utf-8",$item_name);//상품명
	}
	$list['quantity']=$totalQuantity;//상품갯수
	$list['total_amount']=$mon_tot_freight;//결제할 금액
	$list['free_amount']="0";//배송비?? 
	//카카오페이 결제준비
	$kakaoPay=new KakaPay("1",$list,"pc");//카카오페이 멤버변수 설정하기 1 결제준비 2 결제승인 pc:pc버전 mobile: 모바일버전
