...
if( $board_class == 1 || $board_class == 3 ){
	if( $UnameSess ){ //로그인했을때 글쓰기 페이지 링크함
		$write_img = "<a href='board_write.php?mart_id=$mart_id&bbs_no=$bbs_no&page=$page&keyset=$keyset&searchword=$searchword'><img src='../image/helpdesk/bu_write.gif' border='0'></a>";
	}else{
		$write_img = "<img src='../image/helpdesk/bu_write.gif' border='0' onclick=\"membercheck($bbs_no);\" style='cursor:hand'>";
	}
...
<!---------------------- FAQ 메뉴 시작 -------------------------------------------------->
						<table width="100%" align="center"  border="0" cellspacing="0" cellpadding="0">
							<tr>
								<td height="50">&nbsp;</td>
								<td width="70"><a href="board_list.php?bbs_no=<?=$bbs_no?>&code=1"><img src="../image/helpdesk/faq_bu_01<?=$code_img1?>.gif" border="0"></a></td>
								<td width="70"><a href="board_list.php?bbs_no=<?=$bbs_no?>&code=2"><img src="../image/helpdesk/faq_bu_02<?=$code_img2?>.gif" border="0"></a></td>
								<td width="90"><a href="board_list.php?bbs_no=<?=$bbs_no?>&code=3"><img src="../image/helpdesk/faq_bu_03<?=$code_img3?>.gif" border="0"></a></td>
								<td width="70"><a href="board_list.php?bbs_no=<?=$bbs_no?>&code=4"><img src="../image/helpdesk/faq_bu_04<?=$code_img4?>.gif" border="0"></a></td>
								<td width="70"><a href="board_list.php?bbs_no=<?=$bbs_no?>&code=5"><img src="../image/helpdesk/faq_bu_05<?=$code_img5?>.gif" border="0"></a></td>
							</tr>
						</table>
<!---------------------- FAQ 메뉴 끝 ---------------------------------------------------->
						<table width="100%" align="center"  border="0" cellspacing="0" cellpadding="0">
						...
						<table width='100%' align="center"  border='0' cellspacing='0' cellpadding='0'>
							...
							<input type="hidden" name="bbs_no" value="<?=$bbs_no?>">
					          ...
					          <option value="subject_new" selected>제목</option>
									...
									<input type='text' name='searchword' value='<?=$searchword?>' class='input_03' size='15'>&nbsp;<input type='image' src='../image/bu_search3.gif' border='0' align='absmiddle' onfocus='blur();'> <a href='<?=$PHP_SELF?>?&bbs_no=<?=$bbs_no?>&page=<?=$page?>'><img src='../image/bu_cancel2.gif' border='0' align="absmiddle" onfocus='blur();'></a>&nbsp;<? if($_SESSION["Mall_Admin_ID"]){?>
									<img style="cursor:hand" id="sel" onClick="ListSel()" value="전체선택" src="../image/helpdesk/list_check.gif" align="middle">
									&nbsp;<input name="del" type="image" onClick="ListDel()" value="삭제" src="../image/helpdesk/list_del.gif" align="middle">
									<? }?></td>
...
?>
									<a href='board_list.php?bbs_no=<?=$bbs_no?>&mart_id=<?=$mart_id?>&page=1&keyset=<?=$keyset?>&searchword=<?=$searchword?>'><img src='../image/helpdesk/arrow_1_big.gif' align='absmiddle' border='0'></a>
<?
}
?>
<?
if($start_page > 1){
?>
									<a href='board_list.php?bbs_no=<?=$bbs_no?>&mart_id=<?=$mart_id?>&page=<?=$prev_start_page?>&keyset=<?=$keyset?>&searchword=<?=$searchword?>'><img src='../image/helpdesk/arrow_1.gif' align='absmiddle' border='0'></a>
<?
}else{
?>
...
?>
									<a href='board_list.php?bbs_no=<?=$bbs_no?>&mart_id=<?=$mart_id?>&page=<?=$i?>&keyset=<?=$keyset?>&searchword=<?=$searchword?>'><?=$i?></a>
<?
	}
}
?>
...
if($end_page < $total_page){
?>
									<a href='board_list.php?bbs_no=<?=$bbs_no?>&mart_id=<?=$mart_id?>&page=<?=$next_start_page?>&keyset=<?=$keyset?>&searchword=<?=$searchword?>'><img src='../image/helpdesk/arrow_2.gif' align='absmiddle' border='0'></a>
<?
}else{
?>
...
if($page < $total_page){
?>
									<a href='board_list.php?bbs_no=<?=$bbs_no?>&mart_id=<?=$mart_id?>&page=<?=$total_page?>&keyset=<?=$keyset?>&searchword=<?=$searchword?>'><img src='../image/helpdesk/arrow_2_big.gif' align='absmiddle' border='0'></a>
<?
}else{
?>
