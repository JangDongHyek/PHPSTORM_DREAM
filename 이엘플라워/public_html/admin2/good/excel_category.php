...
<?
$dbname = $mart_id;
$filename = "$mart_id_디카_".date("Ymd").".xls";
...
//================================= 상품 테이블 백업 시작 ================================
if( $table == "item" ){
	$query = "select item_no, item_name, item_company, member_price, g_margin, z_price, bonus, reg_date from $table where mart_id='$mart_id' and prevno='87' order by reg_date desc";
...
?> 
	<tr>
		...
		<td bgcolor="#F6F5CC"><?=$row[g_margin]?></td>
		<td bgcolor="#F6F5CC"><?=$row[z_price]?></td>
		<td bgcolor="#F6F5CC"><?=$row[bonus]?></td>
	</tr> 
<?
	}
}
