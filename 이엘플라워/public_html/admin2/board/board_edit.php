...
<table border="1" cellpadding="5" cellspacing="0" width="100%" bordercolordark="white" bordercolorlight="#E1E1E1" align="center">
	...
	<td width="100%">
		...
		<form method="POST" name='writeform' enctype="multipart/form-data" onsubmit='return checkform();'> 
...
if($numRows > 0 ){
	...
	$open_chk = $ary[open_chk];
	if($open_chk == y){
		$open_chk_checked = "checked";
	}
										...
										<table border="0" width="100%">
											...
											<tr>
												<td width="13%"><p align="left">
													이름 : </td>
												<td width="87%">
													<input name="writer" size="10" value='<?=$writer?>' class="input_03">
												<?
												if($bbs_no == 8){
												?>
												<input name="open_chk" type="checkbox" value="y" <?=$open_chk_checked?>>공개
												<?
												}
												?>
...
<textarea name="content" id="content" rows="10" cols="100" style="width:650px; height:412px; display:none;"><?=$content?></textarea>
...
<?
	...
	$SQL = "update $New_BoardTable set code='$code', writer='$writer', doc_type = '$doc_type', email='$email', subject_new='$subject_new_tmp[0]', content='$content', write_date = '$write_date', if_secret='$if_secret',open_chk='$open_chk' where index_no = $index_no and mart_id='$mart_id'";
