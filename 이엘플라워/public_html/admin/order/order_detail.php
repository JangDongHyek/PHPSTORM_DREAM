...
?>
	<input onclick="javascript:window.open('https://pg.innopay.co.kr/pay/issue/CardIssue.jsp?TID=<?=$field4?>','','width=520,height=1028')" style="BACKGROUND-COLOR: white; BORDER-BOTTOM: #7b7d7b 1px solid; BORDER-LEFT: #7b7d7b 1px solid; BORDER-RIGHT: #7b7d7b 1px solid; BORDER-TOP: #7b7d7b 1px solid; COLOR: #7b7d7b; HEIGHT: 18px; width=120" type="button" value="카드전표 출력">
<?}?>
<?###################################################################End 현금영수증 ####################################################################?>
						...
						<tr>
							<td width="90%" bgcolor="#999999">
								...
								<tr>
									 ...
									 <?
									 if($id!=''){
										$SQL = "select order_num,id from $Order_BuyTable where id='$id' and status='3' and mart_id='$mart_id'";//배송완료
													//echo "sql=$SQL";
													$dbresult = mysql_query($SQL, $dbconn);
													$numRows = mysql_num_rows($dbresult);
													$mon_tot = 0;
													...
													if($numRows > 0){
														?>
														<a href="javascript:opensub('../member/mem_order_list.php?username=<?=$id_tmp?>', 620, 500)">주문횟수 <?=$numRows?> 건, 총주문금액 <?=$mon_tot_str?> 원</a>(현재주문건포함 배송완료기준)
														<?
													}
													else{
														echo "
								...
								<tr>
									<td width="12%" bgcolor="#FFFFFF" align="center">전달메세지</td>
									<td width="63%" bgcolor="#FFFFFF" align="left" colspan="3">
										<table width="100%" cellpadding="0" cellspacing="0">
											<tr>
												<td width="20%">카드리본선택</td>
								...
								<tr>
									<td width="12%" bgcolor="#FFFFFF" align="center">희망배송일</td>
