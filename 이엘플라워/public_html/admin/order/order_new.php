...
<br>
			...
			<table border="1" cellpadding="5" cellspacing="0" width="100%" bordercolordark="white" bordercolorlight="#E1E1E1" align="center">
				...
				<td width="100%" bgcolor="#FFFFFF" height="3" align="center">
					<b>
						<a href='order_new.php'>[전체주문현황]</a> <?=$method?>
						<a href='order_new.php?pay=kakao'>[카카오페이로]</a>
								...
								<td width="100%" bgColor="#cccccc">
									...
									<td width="100%" bgColor="#f7f7f7" height="20">
										...
										<tr>
											<td width="13%">&nbsp; 조회기간</td>
											<td width="26%">
												<input name="QryFromDate" id="QryFromDate" value="<?=$QryFromDate?>" class="input_03" size="16"> 
												<font color="#3D918A">~</font>
												<input name="QryToDate" id="QryToDate" value="<?=$QryToDate?>" class="input_03" size="16">
											</td>
											<td width="33%">
												<input type='button' class='butt_none' style='width:40' value='오늘' onClick="changeDate('date',0)">&nbsp; 
												<input type='button' class='butt_none' style='width:40' value='내일' onClick="changeDate('date',1)">&nbsp; 
												<input type='button' class='butt_none' style='width:60' value='1개월전' onClick="changeDate('month',-1)">&nbsp; 
												<input type='button' class='butt_none' style='width:40' value='1년전' onClick="changeDate('month',-12)">&nbsp; 
												<input type='button' class='butt_none' style='width:60' value='1개월후' onClick="changeDate('month',1)">&nbsp; 
												<input type='button' class='butt_none' style='width:40' value='1년후' onClick="changeDate('month',+12)">&nbsp; 
											</td>
										</tr>
										<tr height="10">
											<td width="13%"></td>
										 ...
										 <tr>
											...
											<td width="59%" colspan="2">
												...
												<!--<option value="12" <?if($status_flag == 12) echo " selected";?> style="color:#ff6600;">대기</option>-->
												<option value="2" <?if($status_flag == 2) echo " selected";?>>입금확인</option>
												<!--<option value="6" <?if($status_flag == 6) echo " selected";?> style="color:#3300ff;">배송중</option>-->
												<option value="3" <?if($status_flag == 3) echo " selected";?>>배송완료</option>
												<option value="5" <?if($status_flag == 5) echo " selected";?>>주문취소</option>
												<!--<option value="7" <?if($status_flag == 7) echo " selected";?> style="color:#990099;">교환</option>
												<option value="11" <?if($status_flag == 11) echo " selected";?> style="color:#990099;">맞교환</option>
									...
									<tr>
									 ...
									 <table width="100%" border="0">
										<tr>
											...
											<select name='status_all' class='aa' style='height: 18px; background-color: rgb(193,219,227); border: 1px solid black' size='1' onchange="document.getElementsByName('status_all')[1].value=this.value">
												<option>주문상태를 : </option>
											...
											<button type="button" style="BORDER-RIGHT: #3d918a 1px solid; BORDER-TOP: #3d918a 1px solid; BORDER-LEFT: #3d918a 1px solid; COLOR: #3d918a; BORDER-BOTTOM: #3d918a 1px solid; height: 18px; BACKGROUND-COLOR: white" onclick="goSms()">
											문자보내기
											</button>
											</td>
										</tr>
...
if($pay=="kakao"){
	$where = " and paymethod='kakao_pay'";
}

if($flag == "today"){
	$SQL = "select * from $Order_BuyTable where mart_id='$mart_id' and status != '9' $status_str and date like '$today%' $where $order_by desc";
}else if($flag == "search"){
	if($keyset == 'item_name'){
		$SQL = "select DISTINCT T1.* from $Order_BuyTable T1, $Order_ProTable T2 where T1.mart_id='$mart_id' and T1.status != '9' $where $status_str and T1.order_num=T2.order_num and binary T2.item_name like '%$searchword%' $order_by desc";
	}else if($keyset == 'name'){
		$SQL = "select * from $Order_BuyTable where mart_id='$mart_id' and status != '9' $where $status_str and  binary name like '%$searchword%' $order_by desc";
	}else if($keyset == 'tel'){
		$SQL = "select * from $Order_BuyTable where mart_id='$mart_id' and status != '9' $where $status_str and  (binary tel1 like '%$searchword%' or binary tel2 like '%$searchword%') $order_by desc";
	}else{
		$SQL = "select * from $Order_BuyTable where mart_id='$mart_id' and status != '9' $where $status_str and binary $keyset like '%$searchword%' $order_by desc";
	}
}else if($flag == 'find'){
	$SQL = "select * from $Order_BuyTable where mart_id='$mart_id' and status != '9' $where $status_str and ( date >= '$QryFromDate' and left(date,10) <= '$QryToDate') $order_by desc";
}else{
	$SQL = "select * from $Order_BuyTable where mart_id='$mart_id' and status != '9' $where $status_str $order_by desc";
}			
$dbresult = mysql_query($SQL, $dbconn);
$numRows = mysql_num_rows($dbresult);
...
?>
							...
							<input type="hidden" id="order-hp<?=$order_num?>" value="<?=$ary[tel2]?>">
								<td bgcolor='#FFFFFF' rowspan='2'><?=$j?> <input type='checkbox' name='checkSel[]' value='<?=$order_num?>'></td>
								<td  bgcolor='#FFFFFF'><a href='order_detail.php?index_no=<?=$ary[index_no]?>&page=<?=$page?>&keyset=<?=$keyset?>&searchword=<?=$searchword?>&flag=<?=$flag?>&cnfPagecount=<?=$cnfPagecount?>&QryFromDate=<?=$QryFromDate?>&QryToDate=<?=$QryToDate?>&status_flag=<?=$status_flag?>&pay=<?=$pay?>'><?=$order_num?></a></td>
								<td bgcolor='#FFFFFF'>
								<?=$name?>
								<?php if($ary['anonymity'] == 'y') echo '<br>[익명배송]'; ?>
								</td>
			...
			<tr align="center">
				<td width="100%" bgcolor="#FFFFFF">
					...
					if($page == 1){
						...
						echo ("
						<a href='order_new.php?page=1&keyset=$keyset&searchword=$searchword&flag=$flag&cnfPagecount=$cnfPagecount&QryFromDate=$QryFromDate&QryToDate=$QryToDate&status_flag=$status_flag&pay=$pay'>처음</a> 
						");
					}
				
					if($start_page > 1){
						echo ("
						<a href='order_new.php?page=$prev_start_page&keyset=$keyset&searchword=$searchword&flag=$flag&cnfPagecount=$cnfPagecount&QryFromDate=$QryFromDate&QryToDate=$QryToDate&status_flag=$status_flag&pay=$pay'>
						◁&nbsp; 
						</a>
						");
					}
					...
					for($i=$start_page;$i<=$end_page;$i++){
						if($i == $page){
							...
							echo ("
						<a href='order_new.php?page=$i&keyset=$keyset&searchword=$searchword&flag=$flag&cnfPagecount=$cnfPagecount&QryFromDate=$QryFromDate&QryToDate=$QryToDate&status_flag=$status_flag&pay=$pay'>$i</a> 
							");
						}
					}
					if($end_page < $total_page){
						echo ("
						<a href='order_new.php?page=$next_start_page&keyset=$keyset&searchword=$searchword&flag=$flag&cnfPagecount=$cnfPagecount&QryFromDate=$QryFromDate&QryToDate=$QryToDate&status_flag=$status_flag&pay=$pay'>
						&nbsp;▷
						</a>
						");
					}
					...
					if($page == $total_page){
						...
						echo ("
						<a href='order_new.php?page=$total_page&keyset=$keyset&searchword=$searchword&flag=$flag&cnfPagecount=$cnfPagecount&QryFromDate=$QryFromDate&QryToDate=$QryToDate&status_flag=$status_flag&pay=$pay'>끝</a> 
						");
					}
					?>
			  </td>
...
if($update_flag == 'add_list_print'){
	...
	echo "<meta http-equiv='refresh' content='0; URL=order_new.php?page=$page&keyset=$keyset&searchword=$searchword&flag=$flag&cnfPagecount=$cnfPagecount&QryFromDate=$QryFromDate&QryToDate=$QryToDate&status_flag=$status_flag'>";
}
...
/**	//================== SMS DB 설정 파일을 불러옴 =======================================
	...
	for($i=0; $i<count($checkSel); $i++){
		...
		if($status_old[$i] == '3' && $status_all != 3){ //이전상태가 배송완료일때만, 구매총액 마이너스
			...
			$dbresult = mysql_query($SQL, $dbconn);
			$numRows = mysql_num_rows($dbresult);
			while($rs=mysql_fetch_array($dbresult)){
				$item_no=$rs[item_no];
				$opt=$rs[opt];
				$quantity=$rs[quantity];
				$sql="select * from $OptionTable where item_no='$item_no' and opt_no='$opt'";
				$result=mysql_query($sql);
				$rows=mysql_num_rows($result);
				if(0<$rows){
				//옵션 재고 빠지기
					$sql="update $OptionTable set opt_ea=opt_ea+$quantity where opt_no='$opt' and item_no='$item_no'";
					mysql_query($sql);
				}
			}
			if(!empty($id)){
