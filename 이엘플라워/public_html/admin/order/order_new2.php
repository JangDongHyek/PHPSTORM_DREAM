          ...
          <tr>
            ...
            <td valign="top" background="../img/top_2_bg.gif"><div align="right">
              <table width="100%" border="0" cellspacing="0" cellpadding="0">
                ...
                <tr>
                  <td><div align="right"><img src="../img/top_icon.gif" width="13" height="15" align="absmiddle"> <span class="text_gray2_t">현재페이지</span><span class="text_gray2"> : <a href="../index.html">HOME</a> &gt; </span><span class="text_gray2_c">미완료주문</span> &gt; <span class="text_gray2_c">미완료주문 </span> </div></td>
                </tr>
                <tr>
                  <td height="28">&nbsp;</td>
                </tr>
...
?>
		...
		<td>
			...
			<table width="100%" border="0" cellpadding="0" cellspacing="0">
				<tr>
				<td width="100%" height="30" bgcolor="F2F2F2" class="title"><img src="../images/icon_1.gif" width="30" height="17" align="absmiddle"><b>미완료주문 </b></td>
				</tr>
			</table>
...
<br>
			...
			<table border="1" cellpadding="5" cellspacing="0" width="100%" bordercolordark="white" bordercolorlight="#E1E1E1" align="center">
				...
				<td width="90%" bgcolor="#FFFFFF" valign="top">
					주문번호를 클릭하시면 주문에 대한 상세내역 페이지로 이동합니다.
				</td>
				</tr>
...
if($flag == "today"){
	$SQL = "select * from order_buy_temp2 where mart_id='$mart_id' and status != '9' $status_str and date like '$today%' $order_by desc";
}else if($flag == "search"){
	if($keyset == 'item_name'){
		$SQL = "select DISTINCT T1.* from order_buy_temp2 T1, $Order_ProTable T2 where T1.mart_id='$mart_id' and T1.status != '9' $status_str and T1.order_num=T2.order_num and binary T2.item_name like '%$searchword%' $order_by desc";
	}else if($keyset == 'name'){
		$SQL = "select * from order_buy_temp2 where mart_id='$mart_id' and status != '9' $status_str and  binary name like '%$searchword%' $order_by desc";
	}else if($keyset == 'tel'){
		$SQL = "select * from order_buy_temp2 where mart_id='$mart_id' and status != '9' $status_str and  (binary tel1 like '%$searchword%' or binary tel2 like '%$searchword%') $order_by desc";
	}else{
		$SQL = "select * from order_buy_temp2 where mart_id='$mart_id' and status != '9' $status_str and binary $keyset like '%$searchword%' $order_by desc";
	}
}else if($flag == 'find'){
	$SQL = "select * from order_buy_temp2 where mart_id='$mart_id' and status != '9' $status_str and ( date >= '$QryFromDate' and left(date,10) <= '$QryToDate') $order_by desc";
}else{
	$SQL = "select * from order_buy_temp2 where mart_id='$mart_id' and status != '9' $status_str $order_by desc";
}			
...
?>
							...
							<input type="hidden" id="order-hp<?=$order_num?>" value="<?=$ary[tel2]?>">
								...
								<td  bgcolor='#FFFFFF'><a href='order_detail2.php?index_no=<?=$ary[index_no]?>&page=<?=$page?>&keyset=<?=$keyset?>&searchword=<?=$searchword?>&flag=<?=$flag?>&cnfPagecount=<?=$cnfPagecount?>&QryFromDate=<?=$QryFromDate?>&QryToDate=<?=$QryToDate?>&status_flag=<?=$status_flag?>'><?=$order_num?></a></td>
								<td bgcolor='#FFFFFF'>
								<?=$name?>
...
/**	//================== SMS DB 설정 파일을 불러옴 =======================================
	...
	for($i=0; $i<count($checkSel); $i++){
		...
		$SQL = "update order_buy_temp2 set status='$status_all', payment_date ='$pay_day' where order_num='$num' and mart_id='$mart_id'";
		$dbresult = mysql_query($SQL, $dbconn);
		...
		if($status_old[$i] != '3' && $status_all == '3' && $bonus_ok == 't'){ //배송완료일때만, 포인트 사용할 때만 포인트 추가
			...
			if($numRows == 0){
				//구매자 id 알아내기
				$SQL = "select id from order_buy_temp2 where order_num = '$num' and mart_id='$mart_id'";
				$dbresult = mysql_query($SQL, $dbconn);
				$numRows = mysql_num_rows($dbresult);
				if($numRows > 0){
		...
		/*if( $status_old[$i] != '2' && $status_all == '2' ){ //입금확인시 sms보내기
			//구매자 정보 알아내기
			$SQL = "select id, tel2, name from order_buy_temp2 where order_num = '$num' and mart_id='$mart_id'";
			$dbresult = mysql_query($SQL, $dbconn);
			$id = mysql_result($dbresult,0,0);
		...
		if($status_old[$i] != '3' && $status_all == 3){ //배송완료일때만, 구매총액 추가
			...
			$SQL = "update order_buy_temp2 set send_date='$send_date' where order_num='$num' and mart_id='$mart_id'";
			$dbresult = mysql_query($SQL, $dbconn);
		}	
		...
		if($status_old[$i] == '3' && $status_all != 3){ //이전상태가 배송완료일때만, 구매총액 마이너스
			...
			$SQL = "update order_buy_temp2 set send_date='' where order_num='$num' and mart_id='$mart_id'";
			$dbresult = mysql_query($SQL, $dbconn);	
		}	
		...
		if(($status_old[$i] != '5' && $status_all == '5')||($status_old[$i] != '4' && $status_all == '4')){ //주문취소,환불시 보너스에서 빼기
			...
			//사용한 보너스 복구
			$SQL = "select id,use_bonus_tot from order_buy_temp2 where order_num = '$num' and mart_id='$mart_id'";
			$dbresult = mysql_query($SQL, $dbconn);
			$numRows = mysql_num_rows($dbresult);
