...
//========================================================================================
			...
			// 에디터 썸네일 삭제
			function delete_editor_thumbnail($contents,$phpself)
			{
				...
				for($i=0; $i<count($matchs[1]); $i++) {
					...
					$imgurl_ex = explode("data",$imgurl['path']);


						$url_gubun = explode("~",$phpself);


						if($url_gubun[1]!=""){//계정접속

							$url_gubun_ex = explode("/",$url_gubun[1]);
							$srcfile = "/home/".$url_gubun_ex[0]."/public_html/data".$imgurl_ex[1];

						}else{
							$srcfile = $_SERVER['DOCUMENT_ROOT']."/data".$imgurl_ex[1];
						}
...
if($delflag=="del_item"){
		...
		if($numRows>0) {
			...
			//에디터내의 이미지들을 삭제한다ㅣ
			delete_editor_thumbnail($item_explain,$PHP_SELF);
	...
	<tr valign="top">
		 <td>
			<table width="100%" border="0" cellpadding="0" cellspacing="0">
				<tr>
				<td width="100%" height="30" bgcolor="F2F2F2" class="title"><img src="../images/icon_1.gif" width="30" height="17" align="absmiddle"><b>현재카테고리 : <?=$cur_category_name?></b></td>
				</tr>
			</table>
			<!--내용 START~~-->
...
if($searchword !=''){
	if($select_key == "provider_id" ){
		$SQL = "SELECT * FROM $ItemTable AS a LEFT JOIN $MemberTable AS b ON a.provider_id = b.username WHERE b.mart_id='$mart_id' AND b.name LIKE '%$searchword%' and  $con1 order by a.item_order desc, a.item_no desc";
	}else{
		$SQL = "select * from $ItemTable where $con and item_name like '%$searchword%' and $con and mart_id='$mart_id' order by item_order desc, item_no desc";
	}
}else{
	$SQL = "select * from $ItemTable where $con and mart_id='$mart_id' order by item_order desc, item_no desc ";
}
//echo $SQL;
...
for ($i=$skipNum; $i < ($cnfPagecount+$skipNum); $i++) {
	...
	if($jaego_use == 1 && $jaego == 0){
		$icon_str = "<img src='../images/soldout_icon_s.gif' style='vertical-align:middle'>";
	}else{
		$icon_str = "";
	}
...
//	}else{
		$link_str = "<a href='item_edit.php?item_no=$item_no&prevno=$prevno&prevno2=$prevno2&category_num=$category_num&page=$page&searchword=$searchword&select_key=$select_key&pu=$pu'><b>$item_name</b></a> $icon_str $hide_str";
//	}
?>
...
if($img_sml != '' && file_exists("$Co_img_UP$mart_id/$img_sml")){
	if (strstr(strtolower(substr($img_sml,-4)),'.jpg') || strstr(strtolower(substr($img_sml,-4)),'.gif')){
				echo "
	<a href='item_edit.php?item_no=$item_no&prevno=$prevno&prevno2=$prevno2&category_num=$category_num_tmp&page=$page&searchword=$searchword&select_key=$select_key&pu=$pu'><img src='../..$Co_img_DOWN$mart_id/$img_sml' width='34' height='34' border='0'></a>
		";
	}
	if (strstr(strtolower(substr($img_sml,-4)),'.swf')){
				echo "
	<embed src='../..$Co_img_DOWN$mart_id/$img_sml' width='34' height='34'></embed>
		";
	}
}
?>
...
?>
			...
			<td vAlign="top" width="100%" bgColor="#ffffff" align="center">
				<table border="0" width="100%" cellspacing="0" cellpadding="0">
			  <tr>
				<td width="100%" height="10">※상품 순서는 큰숫자가 앞에 위치합니다 숫자 9999 ~ 1 까지</td>
			  </tr>
			  <tr>
...
//		if($Mall_Admin_ID == $mart_id){ // 내상품이면
			...
			for ($i=0; $i < $numRows; $i++) {
				...
				for($k=0;$k<100;$k++){
					...
					$date1 =  rand(10000000, 99999999);
				
					$output_1 = str_replace('"',"",$output[0][$k]);
					$output_2 = explode("/",$output_1);


					$output_filename = explode(".",$output_2[6]);
					

					$output_3 = "../../data/editor/".$output_2[5]."/".$output_2[6];
					$output_3_c = "../../data/editor/".$output_2[5]."/".$now_time."_".$date1.".".$output_filename[1];	
					...
					if($output_2[6]){
						if(file_exists($output_3)){							
							...
							$item_explain = str_replace($output_2[6],$now_time."_".$date1.".".$output_filename[1],$item_explain);
						}
					}					
				}
	...
	}else												// 1차일 때
	{
		$target_firstno = $target_category;
				...
				$SQL1 = "insert into $ItemTable (item_no, mart_id, provider_id, firstno, prevno, thirdno, category_num, item_name, price, z_price, z_price2, z_price3, bonus, bonus2, bonus3, 	use_bonus, jaego, img, img_big, img_big2, img_big3, img_big4, img_big5, opt,opt2,opt3,opt4,opt5,opt6, doctype, item_explain, short_explain, reg_date, item_company, read_num, item_code, 
				icon_no, use_opt1, use_opt23, item_order, jaego_use, if_strike, if_provide_item, provide_price, img_sml, 
				flash_big_width, flash_big_height, if_hide, img_high, member_price, fee, thumbnail, item_origin, item_bestbefore) 
				values ('$maxItem_no_1', '$mart_id', '$provider_id', '$target_firstno', '$target_prevno', '$target_thirdno', '$target_category', '$item_name', '$price', '$z_price', '$z_price2', '$z_price3', '$bonus', '$bonus2', '$bonus3',				'$use_bonus','$jaego','$img_new','$img_big_new','$img_big2_new','$img_big3_new','$img_big4_new','$img_big5_new','$opt','$opt2','$opt3','$opt4','$opt5','$opt6','$doctype','$item_explain','$short_explain','$reg_date', '$item_company', 0, '$item_code',
				'$icon_no','$use_opt1','$use_opt23','9999','$jaego_use','$if_strike','$if_provide_item','$provide_price',
				'$img_sml_new','$flash_big_width','$flash_big_height','$if_hide', '$img_high_new','$member_price', '$fee', '$thumbnail', '$item_origin', '$item_bestbefore')";
				...
				$sql="select * from $OptionTable5 where item_no='$item_no'";
				$result5=mysql_query($sql);
				while($rs=mysql_fetch_array($result5)){
					$sql="insert into $OptionTable5(item_no,opt_name,opt_price,opt_ea,opt_order,opt_code) values('$maxItem_no_1','$rs[opt_name]','$rs[opt_price]','$rs[opt_ea]','$rs[opt_order]','$rs[opt_code]')";
					$result=mysql_query($sql);
				}
				$sql="select * from $OptionTable6 where item_no='$item_no'";
				$result5=mysql_query($sql);
				while($rs=mysql_fetch_array($result5)){
					$sql="insert into $OptionTable6(item_no,opt_name,opt_price,opt_ea,opt_order,opt_code) values('$maxItem_no_1','$rs[opt_name]','$rs[opt_price]','$rs[opt_ea]','$rs[opt_order]','$rs[opt_code]')";
					$result=mysql_query($sql);
				}
			}
