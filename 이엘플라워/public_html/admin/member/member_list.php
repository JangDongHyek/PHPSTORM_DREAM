...
function goSms(){
	var hp="";
	var isChecked=false;
	for(var i=0;i<$("input[name='loan_number[]']").size();i++){
		if($("input[name='loan_number[]']").eq(i).prop("checked")){
			var no=$("input[name='loan_number[]']").eq(i).val();
			hp+=$("#member-hp"+no).val()+",";
			isChecked=true;
		}
	}
	...
	window.open("../sms/sms_write.php?hp="+hp,"sms","width=475;height=600,scrollbars=yes");
...
?>
		...
		<td>
			...
			<table width="100%" border="0" cellpadding="0" cellspacing="0">
				<tr>
				<td width="100%" height="30" bgcolor="F2F2F2" class="title"><img src="../images/icon_1.gif" width="30" height="17" align="absmiddle"><b>회원 관리</b></td>
				</tr>
			</table>
...
if($order == 'passport1'){
	if($orderby == 'desc'){
?>
						<a class='dd' href='member_list.php?page=<?=$page?>&keyset=<?=$keyset?>&searchword=<?=$searchword?>&order=division&orderby=asc'>회원구분</a><small>▼</small>
<?
	}else{
?>
						<a class='dd' href='member_list.php?page=<?=$page?>&keyset=<?=$keyset?>&searchword=<?=$searchword?>&order=division&orderby=desc'>회원구분</a><small>▲</small>
<?
	}	
}else{
?>
						<a class='dd' href='member_list.php?page=<?=$page?>&keyset=<?=$keyset?>&searchword=<?=$searchword?>&order=division&orderby=desc'>회원구분</a>
<?
}
?>
						</b></td>					<!-- <td width="14%"><b>쿠폰발급회원사</b></td> -->
						<td width="10%">
						<b>
<?
if($order == 'date'){
...
?>
					  <tr height='25' bgColor='#F3F3F3' align='center'>
						 ...
						 <input type="hidden" id="member-hp<?=$username?>" value="<?=$ary[tel1]?>">
...
?>
			...
			<tr align="middle">
			  <td vAlign="top" width="100%" bgColor="#ffffff">
				<button type="button" style="BORDER-RIGHT: #5a5a5a 1px solid; BORDER-TOP: #5a5a5a 1px solid; BORDER-LEFT: #5a5a5a 1px solid; COLOR: black; BORDER-BOTTOM: #5a5a5a 1px solid; HEIGHT: 18px; BACKGROUND-COLOR: white" type="button" onclick="goSms()">문자보내기</button>
