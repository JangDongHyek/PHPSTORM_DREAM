...
if($flag == "adduser"){
	...
	*/
	...
	$addinfo_date = implode("-",$addinfo_date);
		if($division == '일반회원'){
		...
		$add_qry1 = ", division, gender, birth1, birth2, birth3, sl";
		$add_qry2 = ", '{$division}', '{$gender}', '{$birth1}', '{$birth2}', '{$birth3}', '{$sl}'";
		...
		$add_qry1 = ", division, company, company_number, boss_name, ct1, ct2, charge, charge_hp";
		$add_qry2 = ", '{$division}', '{$company}', '{$company_number}', '{$boss_name}', '{$ct1}', '{$ct2}', '{$charge}', '{$charge_hp}'";

	}
	//추천인 아이디가 있을 경우
	if($recommand_id){
		$SQL = "select * from mart_member_new where username='$_POST[recommand_id]'";
		$result2=mysql_query($SQL);
		$row2=mysql_fetch_array($result2);
		
		if(!$row2[username]){

			echo "<script type='text/javascript'>alert('추천인 아이디에 등록된 사람이 없습니다.')</script>;
			<form name='form' action='join.html' method='post'>
				<input type='hidden' name='mart_id' value='$mart_id'>
				<input type='hidden' name='name' value='$name'>
				<input type='hidden' name='passport1' value='$passport1'>
	...
	$SQL = "insert into $Mart_Member_NewTable (uid, mart_id, username, password, perms, name, passport1, passport2, email, tel, tel1, zip, address, address_d, date, partner, is_member, msg, job, com_name, homepage, hobby, religion, ext1_content, ext2_content, ext3_content, ext4_content,sel_content,if_maillist, member_img {$add_qry1},addinfo,addinfo_date,recommand_id) values ('', '$mart_id', '$username', '$encode_password', '$perms', '$name', '$passport1', '$passport2','$email', '$tel', '$tel1', '$zip', '$address', '$address_d', '$date', '$partner', '$is_member', '$msg', '$job', '$com_name', '$homepage', '$hobby', '$religion', '$ext1_content', '$ext2_content', '$ext3_content', '$ext4_content', '$sel_content','$if_maillist', '$file' {$add_qry2},'$addinfo','$addinfo_date','$recommand_id')";
	$dbresult = mysql_query($SQL, $dbconn);
