<?
	...
	$rows_count = "3";
	// 이미지 가로
	$gallery_img_width = "80";
	// 이미지 세로
	$gallery_img_height = "80";
	// 썸네일 지원 여부
...
?>
							<td width="<?=intval(100/$rows_count)?>%">
								...
								<form name="frmList" action='<?=$PHP_SELF?>?&bbs_no=<?=$bbs_no?>&page=<?=$page?>' method="get" onSubmit="board_check(); return false;">
								...
								<tr>
										<td align="center">
											<table width="<?=$gallery_img_width+20?>" height="<?=$gallery_img_height+20?>" border=0 cellpadding=0 cellspacing=0>
												...
												<tr>
													...
													<td align=center bgcolor=#F7F7F7 width=100% height=100%><a href='board_read.html?index_no=<?=$index_no?>&bbs_no=<?=$bbs_no?>&page=<?=$page?>&mart_id=<?=$mart_id?>&keyset=<?=$keyset?>&searchword=<?=$searchword?>'><img src="<?=$target?>" width="<?=$gallery_img_width?>" height="<?=$gallery_img_height?>"></a></td>
									...
									<tr>
										<td align=center nowrap height=25><a href='board_read.html?index_no=<?=$index_no?>&bbs_no=<?=$bbs_no?>&page=<?=$page?>&mart_id=<?=$mart_id?>&keyset=<?=$keyset?>&searchword=<?=$searchword?>'><?=$subject_new?></a> <?=$secret_str?><?=$new_string?></td>
							        </tr>
								</table>
							</td>
