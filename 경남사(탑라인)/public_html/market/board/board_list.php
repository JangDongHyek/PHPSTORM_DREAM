...
if($bbs_no == 9){ //업체회원만 글쓰기 가능 관리자도

	$co_sql = "select * from $Mart_Member_NewTable where mart_id='$mart_id' and username='$UnameSess'";
	//echo $co_sql;
	$co_result = mysql_query($co_sql, $dbconn);
	$co_rows = mysql_fetch_array($co_result);

	if($Mall_Admin_ID || ($Membertype == "C" && $co_rows[co_auth] == "Y")){
		$write_img = "<a href='board_write_co.php?mart_id=$mart_id&bbs_no=$bbs_no&page=$page&keyset=$keyset&searchword=$searchword'><img src='../image/helpdesk/bu_write.gif' width='70' height='30' border='0'></a>";
	}else{
			$write_img = "<img src='../image/helpdesk/bu_write.gif' width='70' height='30' border='0' onclick=\"membercheck_co($bbs_no);\" style='cursor:hand'>";
	}
}else{
...
function membercheck_co(num){
	var remessage = "승인된 업체회원만 글을 쓸 수 있습니다. 로그인 하시겠습니까?";
...
<?
	...
	if($list_type == "NL")
		if($bbs_no == 8){
			...
			include "board_list_skin_co.php";
		}else{
