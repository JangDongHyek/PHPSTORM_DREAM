...
<table border="0" width="550">
  ...
  <tr>
    ...
    bordercolor="#7D7D7D" cellspacing="0" cellpadding="0" style="border: thin">
      ...
      <?
  		$SQL = "select * from $Order_ProTable where mart_id='$mart_id' and order_num = '$order_num_tmp' and status = '1' order by order_pro_no desc";
		$dbresult = mysql_query($SQL, $dbconn);
		...
		if($numRows > 0){
			...
			$status = $ary["status"];
			$date = substr($ary["date"],0,4)."/".substr($ary["date"],4,2)."/".substr($ary["date"],6,2);
    ...
    bordercolor="#7D7D7D" cellspacing="0" cellpadding="0" style="border: thin">
      ...
      <tr>
        ...
        &nbsp; 주문번호</span></td>
        <td width="71%" style="border-top: thin"><p style="padding-left: 5px"><span class="aa"><?=$order_num_tmp?></span></td>
      </tr>
      <tr>
        <td width="29%" style="border-right: thin; border-top: thin" height="25"><span class="aa"> 
        &nbsp; 주문일자</span></td>
...
<table border="0" width="550">
  <tr>
    ...
    <br>
    <input class="bb" onclick="top.rec_top.focus(); window.print();"
    style="BACKGROUND-COLOR: white; BORDER-BOTTOM: #7b7d7b 1px solid; BORDER-LEFT: #7b7d7b 1px solid; BORDER-RIGHT: #7b7d7b 1px solid; BORDER-TOP: #7b7d7b 1px solid; COLOR: #7b7d7b; HEIGHT: 18px"
    type="button" value="프린트하기">&nbsp; </td>
  </tr>
</table>
</body>
</html>
