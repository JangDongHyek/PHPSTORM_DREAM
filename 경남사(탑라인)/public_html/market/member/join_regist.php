...
if($flag == "adduser"){
	...
	if(mysql_num_rows($dbresult)>0){
		echo ("
			...
			<form name='form' action='http://www.topline.kr/market/member/join.html' method='post'>
	...
	$SQL = "insert into $Mart_Member_NewTable (uid, mart_id, username, password, perms, name, passport1, passport2, email, tel, tel1, zip, address, address_d, date, partner, is_member, msg, job, com_name, homepage, hobby, religion, ext1_content, ext2_content, ext3_content, ext4_content,sel_content,if_maillist, member_img, member_type, co_name, co_tel, co_zip, co_address1, co_address2, co_number, co_content) values ('', '$mart_id', '$username', '$encode_password', '$perms', '$name', '$passport1', '$passport2','$email', '$tel', '$tel1', '$zip', '$address', '$address_d', '$date', '$partner', '$is_member', '$msg', '$job', '$com_name', '$homepage', '$hobby', '$religion', '$ext1_content', '$ext2_content', '$ext3_content', '$ext4_content', '$sel_content','$if_maillist', '$file', '$member_type', '$co_name', '$co_tel', '$co_zip', '$co_address1', '$co_address2', '$co_number', '$co_content')";
	$dbresult = mysql_query($SQL, $dbconn);
	...
	echo "<meta http-equiv='refresh' content='0; URL=http://www.topline.kr/market/member/join_ok.html'>";
}
