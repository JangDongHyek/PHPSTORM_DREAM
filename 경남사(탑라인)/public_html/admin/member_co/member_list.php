...
?>
		...
		<table border="1" cellpadding="5" cellspacing="0" width="90%" bordercolordark="white" bordercolorlight="#E1E1E1" align="center">
			<tr>
			  <td vAlign="top" width="90%" bgColor="#ffffff">현재 쇼핑몰에 가입한 모든 업체회원들의 정보를 기록/관리하는  곳입니다. <br>
			  해당 항목의 제목을 클릭하시면 정렬순서가 바뀝니다.<br>
			  <br>
			  ID 클릭 : 업체회원구매이력 확인<br>
			  성명클릭 : 업체회원정보 수정및 삭제<br>
			  이메일 클릭 : 개별업체회원 메일발송<br>
			...
			<tr>
			  ...
			  <table width="95%" border="0" align="center">
				 <tr>
					<td width="100%" colSpan="2" height="20"><p align="center"><b>[업체회원가입 
					리스트]</b></td>
				 </tr>
				 ...
				 $SQL = "select * from $Mart_Member_NewTable where mart_id='$mart_id' and member_type='C'";
						$dbresult = mysql_query($SQL, $dbconn);
					...
					업체회원수 : 총 <font color="#ff0000"><?echo $numRows?></font> 명</b> <a href='../to_excel/member.php?keyset=<?=$keyset?>&searchword=<?=$searchword?>&order=<?=$order?>&orderby=<?=$orderby?>'><img src='../images/excel.gif' border='0' width='84' height='18' align='absmiddle'></a></td>
...
if($order == 'passport1'){
	if($orderby == 'desc'){
?>
						<a class='dd' href='member_list.php?page=<?=$page?>&keyset=<?=$keyset?>&searchword=<?=$searchword?>&order=email&orderby=asc'>주민번호</a><small>▼</small>
<?
	}else{
?>
						<a class='dd' href='member_list.php?page=<?=$page?>&keyset=<?=$keyset?>&searchword=<?=$searchword?>&order=email&orderby=desc'>주민번호</a><small>▲</small>
<?
	}	
}else{
?>
						<a class='dd' href='member_list.php?page=<?=$page?>&keyset=<?=$keyset?>&searchword=<?=$searchword?>&order=email&orderby=desc'>주민번호</a>
<?
}
?>
						</b></td>					<!-- <td width="14%"><b>쿠폰발급업체회원사</b></td> -->
						<td width="10%">
						<b>
<?
if($order == 'date'){
...
$SQL1 = "select * from $Mart_Member_NewTable where mart_id='$mart_id' and member_type='C'";
...
?>
					  <tr height='25' bgColor='#F3F3F3' align='center'>
						 ...
						 <!-- <td><?=$mem_name?></td> -->
						 <td><?=$passport1?>-<?=$passport2?></td>
					   <td><?=$date_str?></td>
						 <td>
						<?
						if($co_auth == "Y"){
						
							echo "<font color=blue><b>승인</b></font>";	
						
						}else{
							echo "<font color=black><b>미승인</b></font>";							
						}
						?>					 
						 </td>
					  </tr>
...
?>
			...
			<tr align="middle">
			  <td vAlign="top" width="100%" bgColor="#ffffff">
				 <input onclick="javascript:send_mail();" style="BORDER-RIGHT: #5a5a5a 1px solid; BORDER-TOP: #5a5a5a 1px solid; BORDER-LEFT: #5a5a5a 1px solid; COLOR: black; BORDER-BOTTOM: #5a5a5a 1px solid; HEIGHT: 18px; BACKGROUND-COLOR: white" type="button" value="선택업체회원 메일">&nbsp; 
					<input onclick="window.location.href='mail_send.php?mail_type=all'" style="BORDER-RIGHT: #5a5a5a 1px solid; BORDER-TOP: #5a5a5a 1px solid; BORDER-LEFT: #5a5a5a 1px solid; COLOR: black; BORDER-BOTTOM: #5a5a5a 1px solid; HEIGHT: 18px; BACKGROUND-COLOR: white" type="button" value="전체업체회원 메일">&nbsp; 
			  <input onclick="return check('<?echo $keyset?>','<?echo $searchword?>')" style="BORDER-RIGHT: #5a5a5a 1px solid; BORDER-TOP: #5a5a5a 1px solid; BORDER-LEFT: #5a5a5a 1px solid; COLOR: black; BORDER-BOTTOM: #5a5a5a 1px solid; HEIGHT: 18px; BACKGROUND-COLOR: white" type="button" value="검색하여 메일">&nbsp; 			 
				<input onclick="javascript:execute();" style="BORDER-RIGHT: #5a5a5a 1px solid; BORDER-TOP: #5a5a5a 1px solid; BORDER-LEFT: #5a5a5a 1px solid; COLOR: black; BORDER-BOTTOM: #5a5a5a 1px solid; HEIGHT: 18px; BACKGROUND-COLOR: white" type="button" value="선택업체회원 삭제">
			  </td>
			</tr>
			</form>
			<tr>
			  <td vAlign="top" width="100%" bgColor="#ffffff"><div align="center"><center><table width="95%" border="0">
				 <tr>
					...
					<b>선택업체회원 메일 :</b>
					업체회원리스트에서 체크박스를 선택한 업체회원에게 이메일을 동시발송하는 기능입니다.
					<br>					<b>전체업체회원 메일 :</b>
					쇼핑몰 전체 업체회원에게 이메일을 동시발송하는 기능입니다.
					<br>
					<b>검색하여 메일 : </b>주소, 생년등 필요한 항목을 검색하여 검색결과로 메일을 
					보내는 기능입니다.<br>
					(특정 분류의 업체회원 이메일마케팅에 유용합니다.) <br>
					위의 검색창에서 필요한 항목으로 검색한 후 검색하여 
					메일보내기를 클릭하시면 검색된 업체회원들만을 댓아으로 메일을 
					보내게 됩니다.
					<br>
