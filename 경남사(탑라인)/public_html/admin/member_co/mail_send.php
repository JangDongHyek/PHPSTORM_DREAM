...
?>
		<table border="1" cellpadding="5" cellspacing="0" width="90%" bordercolordark="white" bordercolorlight="#E1E1E1" align="center">
			<tr>
				<td width="90%" bgcolor="#FFFFFF"><b>&nbsp;&nbsp; [전체업체회원 이메일 발송]</b>   <br> 
									 ...
									 mysql_data_seek($dbresult,0);
										...
										$bonus_to = $ary["bonus_to"];
										  ...
										  $today_year = date("y") + 100;
											...
											$SQL1 = "select count(*) from $Mart_Member_NewTable where mart_id='$mart_id' and member_type='C'";
										...
										$dbresult1 = mysql_query($SQL1, $dbconn);
											...
											$email_list = "$grp_name 업체회원에게 발송합니다. ( 메일 수신 허용 : <font color='#FF0000'>$numRows3</font> / 전체 : $numRows1 명) ";		
							}
							else{ //전체,검색
											$loan_number_size = sizeof($loan_number);
											if($loan_number_size > 0){
														for($i=0; $i<sizeof($loan_number); $i++){
															$query = "select * from $Mart_Member_NewTable where mart_id='$mart_id' and username='$loan_number[$i]' and if_maillist='1' and member_type='C'";
															$result	=	mysql_query($query,$dbconn);
															$rows	=	mysql_fetch_array($result);
															$email = $rows[email];
															...
															$SQL = "select * from $Mart_Member_NewTable where mart_id='$mart_id' and if_maillist='1' and member_type='C'";
															$dbresult = mysql_query($SQL, $dbconn);
															$numRows1 = mysql_num_rows($dbresult);
															$email_list = "전체 업체회원에게 발송합니다. ( 메일 수신 허용 : <font color='#FF0000'>$numRows1</font> / 전체 : $numRows 명) ";		
											}											
											else{
												$SQL = "select * from $Mart_Member_NewTable where $keyset like '%$searchword%' and mart_id='$mart_id' and if_maillist='1' and member_type='C'";
												$dbresult = mysql_query($SQL, $dbconn);
												$numRows = mysql_num_rows($dbresult);
												$email_list = "";		
												for ($i=0; $i < $numRows; $i++) {
...
<?
	...
	if(!empty($grp_no)){
	 ...
	 mysql_data_seek($dbresult,0);
		...
		$SQL1 = "select email from $Mart_Member_NewTable where mart_id='$mart_id' and member_type='C'";
		$SQL_AREA = " and binary address like '%$area%' ";
		$SQL_SEX = " and substring(passport2,1,1) ='$sex'";
		$SQL_AGE = " and ($today_year - substring(passport1,1,2)*1) between $age_from and $age_to ";
		$SQL_LOGIN = " and login_count between $login_from and $login_to ";
