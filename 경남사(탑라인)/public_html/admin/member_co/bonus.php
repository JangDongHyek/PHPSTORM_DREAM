...
function del(username,num){
	if(confirm("삭제하시겠습니까?")){
		window.location.href='bonus.php?flag=del&mode=<?=$mode?>&provider_id=<?=$provider_id?>&username='+username+'&num='+num;
	}
	else return;
}
</script>
	 ...
	 <table cellSpacing="0" cellPadding="0" width="100%" border="0">
		<tr>
		  ...
		  <table width="100%" border="0">
			 <tr>
				<td width="100%" height="20"><p align="center"><b>[<?=$username?>업체회원 포인트 현황]</b></td>
			 </tr>
		  </table>
		  </td>
		</tr>
		<tr>
		  ...
		  <table width="97%" border="0">
			 <tr>
				...
				<form action='bonus.php' method='post' onsubmit="return frm_val(this)">
				...
				<tr>
					<td align="middle" bgColor="#c8dfec">지급 No.</td>
					<td align="middle" bgColor="#c8dfec">지급일&nbsp;</td>
					<td align="middle" bgColor="#c8dfec">지급량</td>
					<td align="middle" bgColor="#c8dfec">내용</td>
					<td align="middle" bgColor="#c8dfec">구분</td>
					<td align="middle" bgColor="#c8dfec">삭제</td>
<?
$sum_sql = "select sum(bonus) as bonus_total from $BonusTable where mart_id ='$mart_id' and id='$username'";
$sum_rs = mysql_query($sum_sql, $dbconn);
$sum_rows = mysql_fetch_array($sum_rs);
$bonus_total_str = $sum_rows[bonus_total];

$SQL = "select * from $BonusTable where mart_id ='$mart_id' and id='$username' order by num desc";
...
for ($i=$skipNum; $i < ($cnfPagecount+$skipNum); $i++) {
	...
	if($order_num)
		$content = "<a onclick='opener.window.location.href=\"../order/order_detail.php?order_num=$order_num\";self.close();' style='cursor:hand;'>".$content."</a>";
?>
				<tr>
					...
					<td align='left' width='40%' bgColor='#ffffff'><?=$content?></td>
					<td width='15%' bgColor='#ffffff' align='center'><?=$point_arr[$ary[mode]]?></td>
					<td width='5%' bgColor='#ffffff' align='center'>
					<input onclick="del('<?=$username?>', '<?=$num?>')" style='BORDER-RIGHT: #5a5a5a 1px solid; BORDER-TOP: #5a5a5a 1px solid; BORDER-LEFT: #5a5a5a 1px solid; COLOR: black; BORDER-BOTTOM: #5a5a5a 1px solid; HEIGHT: 18px; BACKGROUND-COLOR: white' type='button' value='삭제'></td>
				</tr>
<?
}
?>
				...
				<tr>
					<td align="middle" width="100%" bgColor="#FFFFFF" colspan="6" height="25"><p align="right"><font color="#FF0000">포인트 총액 : <big><?=number_format($bonus_total_str)?></big>원</font></td>
				</tr>
				</table>
				</td>
			 </tr>
		...
		<tr>
		  ...
		  <?
				if($page == 1){
					...
					echo ("
					<a href='bonus.php?username=$username&page=1'>처음</a> 
					");
				}
			
				if($start_page > 1){
					echo ("
					<a href='bonus.php?username=$username&page=$prev_start_page'>
					◁&nbsp; 
					</a>
					");
				}
				...
				for($i=$start_page;$i<=$end_page;$i++){
					if($i == $page){
						...
						echo ("
					<a href='bonus.php?username=$username&page=$i'>$i</a> 
						");
					}
				}
				if($end_page < $total_page){
					echo ("
					<a href='bonus.php?username=$username&page=$next_start_page'>
					&nbsp;▷
					</a>
					");
				}
				...
				if($page == $total_page){
					...
					echo ("
					<a href='bonus.php?username=$username&page=$total_page'>끝</a> 
					");
				}
				?>
				</td>
...
<?
	...
	if( $mode == "point" ){
		...
		if( !$result ){
			echo ("
				<script>
				alert('업체회원사 포인트 기록에 등록하는데 실패했습니다.');
				history.go(-1);
				</script>
			");
			exit;
	...
	echo "<meta http-equiv='refresh' content='0; URL=bonus.php?username=$username&provider_id=$provider_id&mode=$mode'>";
