...
if($mail_type == "suntak"){
	for($i=0; $i<sizeof($loan_number); $i++){
		$query = "select * from $Mart_Member_NewTable where mart_id='$mart_id' and username='$loan_number[$i]' and if_maillist='1' and member_type='N'";
									 ...
									 mysql_data_seek($dbresult,0);
										...
										$bonus_to = $ary["bonus_to"];
										  ...
										  $today_year = date("y") + 100;
											...
											$SQL1 = "select count(*) from $Mart_Member_NewTable where mart_id='$mart_id'  and member_type='N'";
										...
										$dbresult1 = mysql_query($SQL1, $dbconn);
											...
											if($loan_number_size > 0){
														for($i=0; $i<sizeof($loan_number); $i++){
															...
															$SQL = "select * from $Mart_Member_NewTable where mart_id='$mart_id' and member_type='N'";
															$dbresult = mysql_query($SQL, $dbconn);
															$numRows = mysql_num_rows($dbresult);
															
															$SQL = "select * from $Mart_Member_NewTable where mart_id='$mart_id' and if_maillist='1' and member_type='N'";
															$dbresult = mysql_query($SQL, $dbconn);
															$numRows1 = mysql_num_rows($dbresult);
															$email_list = "전체 회원에게 발송합니다. ( 메일 수신 허용 : <font color='#FF0000'>$numRows1</font> / 전체 : $numRows 명) ";		
											}											
											else{
												$SQL = "select * from $Mart_Member_NewTable where $keyset like '%$searchword%' and mart_id='$mart_id' and if_maillist='1' and member_type='N'";
												$dbresult = mysql_query($SQL, $dbconn);
												$numRows = mysql_num_rows($dbresult);
												$email_list = "";		
												for ($i=0; $i < $numRows; $i++) {
...
<?
	...
	if(!empty($grp_no)){
	 ...
	 mysql_data_seek($dbresult,0);
		...
		$SQL1 = "select email from $Mart_Member_NewTable where mart_id='$mart_id'  and member_type='N'";
		$SQL_AREA = " and binary address like '%$area%' ";
		$SQL_SEX = " and substring(passport2,1,1) ='$sex'";
		$SQL_AGE = " and ($today_year - substring(passport1,1,2)*1) between $age_from and $age_to ";
		$SQL_LOGIN = " and login_count between $login_from and $login_to ";
