...
if ($w == '' || $w == 'r') {
    ...
    if($bo_table == "qna") {
        //goSms("010-7257-7730","051-758-9305","{$wr_name}님이 제품문의에 글을 등록하셨습니다.");
...
if($board['gr_id']=="product"){
	if($wr_2){
		$sql="insert g5_product_latest set bo_table='$bo_table',wr_id='$wr_id',wr_subject='$wr_subject'";
		sql_query($sql);
	}else{
		$sql="delete from g5_product_latest where bo_table='$bo_table' and wr_id='$wr_id'";
		sql_query($sql);
	}
}
...
if($bo_table!="business"&&$bo_table!="business02"&&$bo_table!="business02_en"&&$bo_table!="business_en"&&$bo_table!="qna"&&$bo_table!="qna_en"){
	if ($file_upload_msg)
			alert($file_upload_msg, G5_HTTP_BBS_URL.'/board.php?bo_table='.$bo_table.'&amp;wr_id='.$wr_id.'&amp;page='.$page.$qstr);
	else
			goto_url(G5_HTTP_BBS_URL.'/board.php?bo_table='.$bo_table.'&amp;wr_id='.$wr_id.$qstr);
}else{
	alert("의뢰 및 문의 사항이 등록되었습니다. 추후에 연락드리겠습니다",G5_HTTP_BBS_URL.'/write.php?bo_table='.$bo_table);
}
?>
