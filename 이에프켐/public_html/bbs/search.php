...
if ($stx) {
    ...
    for ($i=0; $i<count($g5_search['tables']); $i++) {
		...
		// wr_text1 필드조회
		$tmp = sql_fetch("SHOW COLUMNS FROM {$tmp_write_table} LIKE 'wr_text1'");
		if ($tmp) {
			$sql .= " OR (wr_text1 LIKE '%{$stx}%')";
		}
    ...
    for ($idx=$table_index; $idx<count($search_table); $idx++) {
        ...
        $sql = " select * from {$tmp_write_table} where {$sql_search} ";
		// wr_text1 필드조회
		$tmp = sql_fetch("SHOW COLUMNS FROM {$tmp_write_table} LIKE 'wr_text1'");
		if ($tmp) {
			$sql .= " OR (wr_text1 LIKE '%{$stx}%') ";
		...
		$sql .= "order by wr_id desc limit {$from_record}, {$rows} ";
        $result = sql_query($sql);
        for ($i=0; $row=sql_fetch_array($result); $i++) {
